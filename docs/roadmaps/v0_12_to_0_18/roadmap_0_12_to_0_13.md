...
Status: completed as of `v0.13` (2025-12-30). Next:
`docs/roadmaps/v0_12_to_0_18/roadmap_0_13_to_0_14.md`.

## P0 — Workspace + DX

### A) Fix `tetra run -o app` on Unix

- Files: `cli/cmd/tetra/tetra_core.go`
- Acceptance:
  - `./tetra run --target linux-x64 -o app examples/smoke/basic/hello.tetra` runs without requiring
    `./app`.

### B) Add `tetra version`

- Files: `cli/cmd/tetra/tetra_core.go`, `compiler/compiler_facade.go`,
...
- Acceptance:
  - `./tetra smoke --target linux-x64` builds and runs:
    - `examples/memory/islands/islands_hello.tetra` (exit 0)
    - `examples/memory/islands/islands_i32.tetra` (exit 55)
    - `examples/memory/islands/islands_overflow.tetra` (exit 1)
    - `examples/memory/raw/mmio_smoke.tetra` (exit 123)
...
- Deliverable: a spec in `docs/spec/runtime/actors.md`.
