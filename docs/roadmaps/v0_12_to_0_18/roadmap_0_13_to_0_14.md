...
duplication. v0.14 now also includes Flow Syntax stabilization as the bridge toward the final Tetra
language surface.
...
- Files: `compiler/internal/semantics/semantics_checker.go`, `compiler/*_test.go`
- Acceptance:
  - New tests cover:
...
- Files: `compiler/internal/semantics/semantics_checker.go`,
  `compiler/internal/lower/lower_core.go`, `compiler/internal/ir/ir.go`,
  `compiler/internal/backend/x64core/x64core_core.go`, `examples/*`
- Acceptance:
...
- Use the existing `@export`, `--emit=library`, and `--runtime-object` mechanisms to prototype a
  self-hosted runtime object, starting with actors.
...
- Files: `compiler/internal/frontend/frontend_core.go`,
  `compiler/internal/frontend/frontend_suite_test.go`, `compiler/compiler_suite_test.go`,
  `examples/flow_*.tetra`
- Acceptance:
