...
**Date:** 2026-06-16.
**Owner:** compiler/runtime/memory benchmark track.
**Requested by:** user request to make Tetra memory usage move toward bytes/KB
for simple programs and controlled low-RAM behavior for large programs.
...
- compiler storage selection prefers `eliminated`, `register`, `stack`, and
  `region` before `heap`;
- complex programs use explicit memory domains and budgets instead of one
  opaque heap;
...
| `domain_bytes` | Bytes charged to `process`, `actor`, `task`, `island`, `request`, or `external` domain. |
...
- Runtime heap telemetry exists in `docs/spec/runtime_heap_telemetry.md`.
- Process RSS telemetry exists in `docs/spec/process_rss_telemetry.md`.
- MemoryBackend vocabulary exists in `docs/spec/memory_backend_vnext.md`.
- MemoryDomain vocabulary exists in `docs/spec/memory_domains_vnext.md`.
...
- `hash_table_tetra` now proves `keys` and `values` as `NoEscape`, but keeps
  storage as `Heap` because local call stack/region lowering is not proven.
- `region/island allocation` remains a separate missing-feature/build-failure
  follow-up.
- Actor benchmark rows remain blocked because backend blockers and
  actor-domain byte evidence are still missing.
- `compiler/internal/runtimeabi/memory_domain.go` already defines memory domain
  structs and summary aggregation.
...
- Keep reports honest: if a metric is not measured, mark it `unsupported` or
  `blocked`, not fake-measured.
...
Each phase below is intentionally small enough to execute as a separate goal or
checkpointed plan batch.
...
jq '.results[] | select(.category=="hash table" or .category=="region/island allocation" or .category=="actor ping-pong" or .category=="parallel map/reduce") | {category, classification, classification_reason}' reports/benchmark-vnext-memory-baseline/tier1-after-actor-track/report.json
git diff --check docs/plan/2026-06-16-tetra-memory-zero-heap-optimization-plan.md
GOCACHE=$(pwd)/.cache/go-build-memory-zero-baseline go clean -cache
```
...
**Goal:** Find the exact reason `NoEscape` allocations crossing local read-only
calls still lower to heap.
...
- `docs/design/allocation_planner_lowering.md`
- `reports/benchmark-vnext-memory-baseline/tier1-after-hash-track/artifacts/bin/hash_table_tetra.alloc.json`
...
```sh
GOCACHE=$(pwd)/.cache/go-build-call-aware-audit go test ./compiler/internal/allocplan -run 'ReadOnly|Call|NoEscape|Stack|Region' -count=1
GOCACHE=$(pwd)/.cache/go-build-call-aware-audit go clean -cache
```
...
```sh
GOCACHE=$(pwd)/.cache/go-build-call-aware-stack go test ./compiler/internal/allocplan -run TestPlannerStackLowersReadOnlyLocalCallAllocation -count=1
```
...
or reuse the existing proof vocabulary only if it already expresses this exact
case.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-call-aware-stack go test ./compiler/internal/allocplan -run 'ReadOnly|Call|NoEscape|Stack|Summary' -count=1
```
...
**Goal:** Teach validation that a proven read-only local call does not make a
stack-backed allocation escape.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-call-aware-validation go test ./compiler/internal/validation/... ./compiler/internal/lower ./compiler/internal/plir ./compiler/internal/allocplan -run 'ReadOnly|Call|Stack|Escape|Translation|Validation' -count=1
```

**Done when:** A forged or unsafe stack plan still fails validation, while the
read-only local-call case passes.

**Notes:** If validation cannot see enough call summary data, add an
investigation task before changing storage.

### Task 1.5 - Actual Stack Lowering For Fixed Small Local Calls

**Goal:** Move the first fixed-size read-only local-call allocation from heap to
stack.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-call-aware-lowering go test ./compiler/internal/allocplan ./compiler/internal/lower ./compiler/internal/validation ./compiler/internal/backend/... ./compiler -run 'ReadOnly|Call|Stack|Allocation|Lower|Backend|Translation' -count=1
GOCACHE=$(pwd)/.cache/go-build-call-aware-lowering go clean -cache
```
...
**Goal:** Apply call-aware storage to `hash_table_tetra` only if all safety
conditions are met.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-call-aware-hash go test ./compiler/internal/allocplan/... ./compiler/internal/validation/... ./compiler -run 'Hash|ReadOnly|Call|Stack|Region|Allocation|PerformanceReport' -count=1
...
jq '.results[] | select(.category=="hash table") | {classification, classification_reason, tetra: (.rows[] | select(.language=="tetra") | .tetra_metadata)}' reports/benchmark-vnext-memory-baseline/tier1-after-call-aware-stack-track/report.json
GOCACHE=$(pwd)/.cache/go-build-call-aware-hash go clean -cache
```

**Done when:** `hash table` is no longer blocked by heap allocation, or the
report explains one precise remaining blocker.

**Notes:** This is the first real memory optimization target after the completed
benchmark goal.
...
- `docs/spec/runtime_heap_telemetry.md`
...
```sh
GOCACHE=$(pwd)/.cache/go-build-zero-heap-policy go run ./tools/cmd/verify-docs --manifest docs/generated/manifest.json
git diff --check docs/spec/zero_heap_benchmark_policy.md
GOCACHE=$(pwd)/.cache/go-build-zero-heap-policy go clean -cache
```
...
**Goal:** Fail local benchmark validation when a zero-heap-required row reports
heap allocation.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-zero-heap-validator go test ./tools/cmd/validate-local-benchmark-tier1 -run 'ZeroHeap|MemoryEvidence|Heap' -count=1
GOCACHE=$(pwd)/.cache/go-build-zero-heap-validator go test ./tools/cmd/local-benchmark-tier1 ./tools/cmd/validate-local-benchmark-tier1 -count=1
GOCACHE=$(pwd)/.cache/go-build-zero-heap-validator go clean -cache
```
...
**Goal:** Add small benchmarks whose only purpose is to protect zero-heap
compiler paths.

**Modify likely files:**

- `tools/cmd/local-benchmark-tier1/types.go`
- `tools/cmd/local-benchmark-tier1/specs.go`
- `tools/cmd/local-benchmark-tier1/tetra_sources.go`
- `tools/cmd/local-benchmark-tier1/c_sources.go`
- `tools/cmd/local-benchmark-tier1/rust_sources.go`
...
- Each category must have Tetra/C/C++/Rust rows or be explicitly local-Tetra
  optimizer evidence outside the Tier 1 comparable matrix.
- Prefer a separate zero-heap suite if adding categories to Tier 1 would distort
  the P20 matrix.

**Verification:**

```sh
GOCACHE=$(pwd)/.cache/go-build-zero-heap-suite go test ./tools/cmd/local-benchmark-tier1 ./tools/cmd/validate-local-benchmark-tier1 -run 'ZeroHeap|BuildSpecs|ValidateReport|MemoryEvidence' -count=1
...
GOCACHE=$(pwd)/.cache/go-build-zero-heap-suite go clean -cache
```

**Done when:** Zero-heap categories validate and fail on intentional heap
regressions.
...
- Decide if the blocker is syntax, semantics, lowering, runtime ABI, or report
  validation.
...
jq '.results[] | select(.category=="region/island allocation")' reports/benchmark-vnext-memory-baseline/tier1-region-island-repro/report.json
GOCACHE=$(pwd)/.cache/go-build-region-island-repro go clean -cache
```
...
**Goal:** Add the smallest failing compiler/runtime test for explicit island
allocation.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-region-island-red go test ./compiler/internal/allocplan ./compiler/internal/lower ./compiler/internal/validation ./compiler/tests/semantics -run 'Island|Region|Allocation|ExplicitIsland' -count=1
```
...
**Goal:** Make the benchmark's region/island row build and run with honest
memory evidence.
...
- Reject returned island-backed pointers unless ownership/lifetime proves them
  safe.
...
jq '.results[] | select(.category=="region/island allocation") | {classification, rows}' reports/benchmark-vnext-memory-baseline/tier1-after-region-island-track/report.json
GOCACHE=$(pwd)/.cache/go-build-region-island go clean -cache
```

**Done when:** `region/island allocation` is no longer `blocked by missing
feature`, or the report names one smaller remaining blocker.

## 12. Phase 4 - Lazy Runtime Linking And Initialization

### Task 4.1 - Runtime Dependency Audit

**Goal:** Identify which runtime pieces are linked/initialized for each simple
program.
...
- If build reports do not expose runtime dependencies, add a report-only task
  before changing linking.

**Verification:**

```sh
GOCACHE=$(pwd)/.cache/go-build-lazy-runtime-audit go test ./compiler -run 'BuildReport|Runtime|Link|Explain' -count=1
GOCACHE=$(pwd)/.cache/go-build-lazy-runtime-audit go clean -cache
```
...
GOCACHE=$(pwd)/.cache/go-build-lazy-runtime-report go clean -cache
```

**Done when:** Reports can prove when a simple program does not pull actor/task
runtime.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-lazy-runtime go test ./compiler ./compiler/internal/backend/... -run 'Runtime|Link|Actor|Task|Heap|Symbol|Smoke' -count=1
GOCACHE=$(pwd)/.cache/go-build-lazy-runtime go run ./tools/cmd/local-benchmark-tier1 --out-dir reports/benchmark-vnext-memory-baseline/tier1-after-lazy-runtime-track --iterations 3
GOCACHE=$(pwd)/.cache/go-build-lazy-runtime go run ./tools/cmd/validate-local-benchmark-tier1 --report reports/benchmark-vnext-memory-baseline/tier1-after-lazy-runtime-track/report.json
GOCACHE=$(pwd)/.cache/go-build-lazy-runtime go clean -cache
```

**Done when:** Simple programs show fewer linked/initialized runtime features
and no benchmark truth validator weakens.
...
- `compiler/internal/runtimeabi`
- `compiler/internal/allocplan`
- `docs/spec/memory_backend_vnext.md`
...
```sh
GOCACHE=$(pwd)/.cache/go-build-heap-reasons go test ./compiler/internal/allocplan ./compiler/internal/runtimeabi ./compiler -run 'Heap|Allocation|Reason|Report' -count=1
GOCACHE=$(pwd)/.cache/go-build-heap-reasons go clean -cache
```
...
**Goal:** Add region allocation for short-lived temporary memory that is bigger
or more dynamic than stack.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-region-allocator go test ./compiler/internal/runtimeabi ./compiler/internal/allocplan ./compiler/internal/lower ./compiler/internal/validation ./compiler -run 'Region|FunctionTemp|Request|Allocation|Domain|Release' -count=1
GOCACHE=$(pwd)/.cache/go-build-region-allocator go clean -cache
```

**Done when:** A temporary dynamic allocation can use a region without heap and
without escaping the region lifetime.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-large-backend go test ./compiler/internal/runtimeabi ./compiler/internal/allocplan ./compiler -run 'Large|Mmap|MemoryBackend|Reserve|Commit|Release|Footprint' -count=1
GOCACHE=$(pwd)/.cache/go-build-large-backend go clean -cache
```

**Done when:** Large allocation rows are not mixed with small heap rows and
carry backend evidence.

## 14. Phase 6 - Actor Memory Domains

### Task 6.1 - Actor Domain Runtime Accounting

**Goal:** Turn actor domain vocabulary into runtime evidence for local actor
benchmarks.
...
GOCACHE=$(pwd)/.cache/go-build-actor-domain go clean -cache
```

**Done when:** Actor domain bytes are measured or explicitly blocked with a
runtime reason, not just allocation-report estimates.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-actor-byte-backpressure go test ./compiler/internal/parallelrt/... ./compiler/internal/actorsrt/... ./compiler/internal/actorsafety/... -run 'Actor|Mailbox|Backpressure|Bytes|Budget|Domain' -count=1
...
```sh
GOCACHE=$(pwd)/.cache/go-build-actor-zero-copy go test ./compiler/internal/parallelrt/... ./compiler/internal/actorsafety/... ./tools/validators/parallelprod/... -run 'ZeroCopy|OwnedRegion|Actor|Domain|Transfer|Claim' -count=1
GOCACHE=$(pwd)/.cache/go-build-actor-zero-copy go clean -cache
```

**Done when:** Local owned-region actor transfer records owner movement and does
not increment copy bytes.
...
git diff --check docs/spec/local_rss_budget_policy.md
...
**Goal:** Fail local benchmark validation when `rss_peak` exceeds a configured
local budget.
...
**Done when:** RSS regressions can fail a local gate without creating a global
RSS claim.
...
```sh
GOCACHE=$(pwd)/.cache/go-build-memory-final-docs go run ./tools/cmd/verify-docs --manifest docs/generated/manifest.json
git diff --check docs/audits/tetra-memory-zero-heap-optimization-final.md
GOCACHE=$(pwd)/.cache/go-build-memory-final-docs go clean -cache
```

**Done when:** A reader can tell which memory improvements are implemented,
which are measured, and which are still planned.
...
```sh
jq '.results[] | {category, classification, classification_reason}' reports/benchmark-vnext-memory-baseline/tier1-after-memory-zero-heap-optimization/report.json
jq '[.results[].rows[] | select(.language=="tetra") | {name, category, heap: .tetra_metadata.memory_evidence.heap_alloc_bytes, rss_peak: .tetra_metadata.memory_evidence.rss_peak, domain: .tetra_metadata.memory_evidence.domain_bytes_evidence}]' reports/benchmark-vnext-memory-baseline/tier1-after-memory-zero-heap-optimization/report.json
```
...
```sh
GOCACHE=$(pwd)/.cache/go-build-memory-final go test ./compiler/internal/allocplan/... ./compiler/internal/runtimeabi/... ./compiler/internal/ramcontract/... ./compiler/internal/memoryfacts/... ./compiler/internal/validation/... ./compiler -run 'Allocation|Memory|Region|Island|Domain|Heap|Stack|RSS|Backend|Actor|Task|ZeroCopy|Bounds|Translation' -count=1
GOCACHE=$(pwd)/.cache/go-build-memory-final-tools go test ./tools/internal/rsstelemetry ./tools/internal/heaptelemetry ./tools/cmd/local-benchmark-tier1 ./tools/cmd/validate-local-benchmark-tier1 -count=1
GOCACHE=$(pwd)/.cache/go-build-memory-final-docs go run ./tools/cmd/verify-docs --manifest docs/generated/manifest.json
git diff --check
graphify update .
```
...
```sh
GOCACHE=$(pwd)/.cache/go-build-memory-final go clean -cache
GOCACHE=$(pwd)/.cache/go-build-memory-final-tools go clean -cache
GOCACHE=$(pwd)/.cache/go-build-memory-final-docs go clean -cache
rm -rf .cache/go-build-memory-final .cache/go-build-memory-final-tools .cache/go-build-memory-final-docs
```

**Done when:** All commands pass and the final audit says exactly what is done
