...
**Requested by:** user request to turn the current focused-file split into more real directories
around `compiler/internal/semantics`, `compiler/internal/lower`, and root `compiler/`.

## 1. Goal

The goal is to continue the completed 1400-line refactor by turning selected large same-package
clusters into real domain-owned directories.

The previous refactor successfully reduced every `.go` and `.tetra` source file to at most 1400
physical lines. It mostly did that with same-package focused files because that was the safest way
...
- changing public `compiler` package APIs unless an explicit compatibility plan is approved;
...
`docs/architecture/project_structure.md` says the repository is moving from flat oversized package
directories toward domain-owned directories with small migration slices.

Important existing migration rules:

- move helpers first, then move test groups;
- do not move package-local Go tests into subdirectories until shared helpers exist in the relevant
  `testkit`;
- keep compatibility wrappers for renamed scripts until references migrate;
- each migration slice needs a guard, focused test, relevant package test, hygiene check, and
  Graphify update when code changes.
...
The difference between `find` and `go list` counts is expected because `go list` counts package
files after build constraints and package selection.
...
That ledger shows the current state is still mostly same-package focused files. This plan
intentionally moves beyond that.

### 3.4 Why a direct file move is not safe

In Go, files in a subdirectory are a different package/import path. Therefore this is not a valid
mechanical refactor:
...
If the file remains `package semantics`, Go will not treat it as part of the parent package because
it is in a different directory. If it becomes `package checker`, it loses access to unexported
parent-package types and helpers.
...
So the correct target is not "move files into folders". The correct target is "extract real packages
with explicit APIs and keep facade code in the old package".
...
The issue is not only file length. The issue is that one package owns many domains:

- public semantic facade: `Check`, `CheckWorld`, `CheckWorldOpt`;
- model/result types such as `CheckedProgram`, `FuncSig`, `TypeInfo`, `GlobalInfo`, `LocalInfo`;
...
`compiler/internal/semantics/...` cannot import parent package `semantics` if the parent imports the
child.

Therefore semantic extraction needs a shared model package before deeper domain packages can exist.
...
The central coupling point is the unexported `lowerer` type. Many files define methods on
`*lowerer`, so direct extraction into subpackages is not mechanical.

The target should be:

- keep the orchestration receiver in package `lower`;
- extract pure decision logic into subpackages;
- leave thin adapter methods in `lower` where stateful access to `lowerer` is required;
...
This is the best first area for real subpackages because the public root `compiler` package can
remain a facade while internal implementation moves behind it.
...
Important: this tree is the architectural target, not a single patch. Some directories may be
introduced later if dependency analysis shows they should be merged or renamed.
...
If a type is currently in `compiler` and needed by an internal package, choose one of these:
...
Use aliases only after verifying docs/tests do not rely on reflection of package paths or type names
in unexpected ways.
...
If a domain package needs a type currently in `semantics`, move that type to `semantics/model` first
and leave a parent alias:
...
Because `lowerer` is stateful and currently unexported, the first extraction must avoid moving
receiver methods that deeply mutate `lowerer`.
...
If a helper needs many `lowerer` fields, leave it in package `lower` until a small interface can be
introduced.

Do not export `lowerer` casually. Exporting it would turn a local implementation detail into a
...
But existing callers should not be forced to import them unless they are internal compiler tests or
explicitly migrated implementation code.
...
`lower` keeps the public lowering entrypoints already used by root compiler and tests. Exact
entrypoint names must be confirmed before implementation.
...
This is only valid after the moved package owns all required model types without importing parent
`semantics`.
...
**Goal:** Capture the current dependency and behavior baseline before moving anything.
...
go list \
  -f '{{.ImportPath}} {{.Name}} {{.GoFiles}} {{.TestGoFiles}}' \
  ./compiler/internal/semantics \
  ./compiler/internal/lower \
  ./compiler
rg --files -g '*.go' -g '*.tetra' |
...
```sh
CACHE="$(pwd)/.cache/go-build-domain-subpackages"
TMP="$(pwd)/.cache/go-tmp-domain-subpackages"
...
**Goal:** Prepare root `compiler` for subpackages by extracting stable API/model types first.
...
From `compiler/compiler_facade.go`:
...
Only move a type if it does not create import cycles. Public types should be re-exported as aliases
from `compiler`.
...
TMP="$(pwd)/.cache/go-tmp-domain-subpackages"
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler -run '^$' -count=1
BUILD_RE='Test(Pipeline|BuildWASM|BuildNativeUI|BuildCache|LinkObject'
BUILD_RE="${BUILD_RE}|BuildLinks|BuildRejectsInterface|BuildInterfaceOnly)"
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler -run "$BUILD_RE" -count=1
git diff --check
```
...
**Goal:** Move implementation domains out of root `compiler/` while preserving root facade
functions.
...
2. Keep root `BuildFile`, `BuildFileWithStats`, and the main build pipeline as facade/orchestrator
   until all dependencies are clean.
...
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  ...
  -run 'Runtime|Actor|Link|Build|WASM|Interface' \
  -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
...
**Goal:** Move report construction out of root `compiler` into a dedicated report package.
...
3. Keep root functions as wrappers if tests or docs reference old function names.
...
CACHE="$(pwd)/.cache/go-build-domain-subpackages"
TMP="$(pwd)/.cache/go-tmp-domain-subpackages"
REPORT_RE='ExplainReports|BoundsReport|AllocReport|MemoryReport'
REPORT_RE="${REPORT_RE}|BackendReport|Performance|P24Compatibility"
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler -run "$REPORT_RE" -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler -count=1
git diff --check
```
...
**Goal:** Move ABI and runtime smoke helper implementation out of root `compiler`.
...
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler \
  -run 'ABI|ABICheck|RunTargetABIChecks|RuntimeSmoke|Boundary' \
  -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler -count=1
git diff --check
...
Exact locations must be confirmed during implementation, but likely candidates include:
...
TMP="$(pwd)/.cache/go-tmp-domain-subpackages"
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler/internal/semantics -run '^$' -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler/internal/semantics -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false \
...
This is the highest leverage and highest-risk step. If a type has methods that depend on many
parent-package helpers, move only the data type and leave methods temporarily in parent package as
functions.
...
6. `checkerworld`: move world-level orchestration only after the previous packages are stable.
7. `checkerstmts` and `checkerexprs`: move last, because they usually touch the broadest surface.

**Facade rule:**

Parent `semantics.CheckWorldOpt` should remain the canonical entrypoint until all internal packages
stabilize.
...
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler/internal/semantics -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler/tests/semantics -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false \
  ./compiler/tests/callables \
  ./compiler/tests/ownership \
...
**Goal:** Move non-checker semantic domains into separate packages after checker model extraction is
stable.
...
TMP="$(pwd)/.cache/go-tmp-domain-subpackages"
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler/internal/semantics \
  -run 'Generics|Region|Ownership|Surface' \
  -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false \
  ./compiler/tests/semantics \
...
**Goal:** Prepare `lower` for real subpackages without exposing the whole `lowerer` implementation.
...
TMP="$(pwd)/.cache/go-tmp-domain-subpackages"
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler/internal/lower -run '^$' -count=1
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false ./compiler/internal/lower -count=1
git diff --check
```
...
**Goal:** Move lower-domain pure logic into subpackages while leaving stateful adapter methods in
package `lower`.
...
1. `rangeproof`: likely best first target because many helpers classify conditions/ranges.
...
The subpackage should receive explicit inputs, not the whole `*lowerer`, unless a narrow interface
...
**Goal:** Move package-local tests only after implementation package boundaries are real and shared
helpers exist.
...
4. Keep white-box tests in package directories only when they truly require unexported internals.
...
**Goal:** Prove the directory refactor works end-to-end across affected compiler surfaces.
...
CACHE="$(pwd)/.cache/go-build-domain-subpackages"
TMP="$(pwd)/.cache/go-tmp-domain-subpackages"
COUNTS="reports/domain-subpackages-line-counts.tsv"
OVER="reports/domain-subpackages-over-1400.tsv"
rg --files -g '*.go' -g '*.tetra' |
  while IFS= read -r file; do wc -l "$file"; done |
  awk '{print $1 "\t" substr($0, index($0,$2))}' |
  sort -nr > "$COUNTS"
awk '$1 > 1400 { print }' "$COUNTS" > "$OVER"
GOTELEMETRY=off GOCACHE="$CACHE" GOTMPDIR="$TMP" \
  go test -buildvcs=false \
  ./compiler/internal/semantics \
  ./compiler/internal/lower \
...
Use a `.workflow/<slug>/verification/` directory instead of `reports/` if this is executed as a
sustained goal.
...
checker_function_types_b.go        -> checkerfuncs/enum_payload.go
                                  or checkerfuncs/callable_fields.go
checker_stmts.go                  -> checkerstmts/statements.go
checker_locals.go                 -> checkerstmts/locals.go
```

Some mappings are intentionally provisional because exact helper ownership must be confirmed during
implementation.
...
The first implementation should not move all receiver methods. It should move pure helpers and leave
adapters until smaller interfaces exist.
...
Root `compiler` remains the public package. Internal packages are implementation details.
...
- `compiler/internal/semantics` has real domain subpackages, not only many same-package files;
- `compiler/internal/lower` has real domain subpackages, with parent adapters where stateful
  lowering remains centralized;
- report/build/runtime/link/ABI implementation has moved out of root `compiler/` where safe;
...
- broad `go test -buildvcs=false ./compiler/... ./cli/... ./tools/... -count=1` passes;
- `git diff --check` passes;
...
- Which root `compiler` public types can safely become aliases to `compiler/internal/buildapi`?
- Which semantic model types can move without changing JSON/report/test expectations?
...
- Should `compiler/internal/buildreports` own report structs only, or also report file writing?
- Should ABI suite remain partly root-level because tests treat it as compiler package internals?
- Which package-local tests can later move to `compiler/tests/*` after `compiler/internal/testkit`
  is stronger?
...
- build/report/runtime/link domains are easier to isolate than semantic checker state;
- early success creates the shared patterns needed for the harder semantics and lower extractions.

After root compiler extraction, move to `semantics/model`, then semantics checker domains, then
`lower/model`, then lowering domain packages.
...
Those should be last, because they are most likely to depend on broad package state.

## 16. Implementation handoff

This plan is ready to execute with a workflow that can handle many small verified slices.
...
3. `/goal` with a new workflow kernel if the refactor should run autonomously across long context
   windows.

Do not execute this as one broad mechanical move. The safe path is small package-boundary extraction
slices with tests after each one.
