...
This baseline freezes the current accepted state after P12.0. It is an evidence
anchor, not a feature promotion and not a marketing claim. Future plan slices
must cite this baseline ID when they compare new proof, allocation, backend,
optimizer, runtime, benchmark, release, or claim-boundary evidence against the
current state.
...
| Field | Value |
|---|---|
| Git HEAD | `5129f2623d9639990076a7d422e56f02b0ed3254` |
| Dump timestamp | `20260602_173943Z` |
| Dump artifacts | `dumps/tetra_language_dump_20260602_173943Z_part_001.md`, `dumps/tetra_language_dump_20260602_173943Z_part_002.md` |
| Manifest | `docs/generated/manifest.json` |
| Manifest SHA-256 | `0d8f358a019d6ac9eb98212c961303bfe68ed8b4619e75f3f654185be45d59d4` |
| Graphify report | `graphify-out/GRAPH_REPORT.md` |
| Graphify code graph counts | 18811 nodes, 60398 edges, 1090 communities |
| Source closure | `reports/master-plan-final-20260602/closure.md` |
| Dump-visible audit | `docs/audits/master-plan-final-20260602.md` |
| Dump-visible artifact map | `docs/audits/master-plan-final-20260602-artifact-map.md` |
| Active Ideal Master Plan | `/home/tetra/Downloads/tetra_ideal_master_plan_20260602.md` |
...
The words `implemented narrow slice`, `partial`, and `evidence-only` are part of
the claim boundary. They must not be collapsed into broad production claims.
...
If a future slice cannot cite this baseline because the baseline is stale, it
must first create a successor baseline with a new ID, fresh command evidence,
fresh manifest hash, fresh dump visibility, and fresh Graphify counts.

## P12.1 Verification Contract

The P12.1 slice is complete only after these checks pass in the current
worktree:

| Check | Required result |
|---|---|
| `GOCACHE=$(pwd)/.cache/go-build-ideal-plan go test ./compiler/tests/semantics -run TestVerifiedTrackCitesMasterPlanAuditDocs -count=1` | pass |
| `go run ./tools/cmd/verify-docs --manifest docs/generated/manifest.json` | pass |
| `go run ./tools/cmd/validate-manifest --manifest docs/generated/manifest.json` | pass |
| `git diff --check` | pass |
| `graphify update .` | pass after registry/code edits |
| `go run ./create_dumps.go` | creates current dump artifacts |
| `rg -n "truthful-performance-core-baseline|tetra.truthful-performance-core.baseline.20260602.v1" dumps` | proves dump visibility |
