...
This matrix is intentionally minimal. It exists only to correlate the two
Memory Ideal Vertical Slice v1 rows for enum payload and monomorphized generic
wrapper borrow closure. It is not a universal release correlation engine and
does not replace `MemoryFactGraph` as the source of truth.
...
The rows are narrow by construction. They do not claim interfaces,
function-typed values, callbacks, async, actor/task boundaries, raw pointer
semantics, target parity, or broad noalias.
