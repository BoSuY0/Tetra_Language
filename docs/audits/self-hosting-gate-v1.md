...
Self-hosting Gate v1 records the current promotion gate for any future
self-hosting claim. It defines the bounded evidence subset, records current
backend/optimizer/allocator/runtime/stdlib evidence, and keeps bootstrap work
blocked until real Tetra compiler-component and deterministic bootstrap
artifacts exist.

The report is valid only while `SelfHostingClaimed=false` and
