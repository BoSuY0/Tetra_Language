...
- Tier 1 benchmark metadata now mirrors allocation-report
  `summary.heap_reason_codes` as `tetra_metadata.heap_reason_codes`.
- Tier 1 validator now checks the allocation report itself:
  every heap allocation row must have known `heap_reason_codes`, those codes
  must also appear in `reason_codes`, and metadata must match the allocation
  report summary.
- RAM contract rows and heap blocker rows now preserve heap reason codes.
- Paired memory-report validation rejects heap allocation artifacts without
  heap reason codes.
...
MEM-8: runtime allocator classes and MemoryBackend evidence must distinguish
small heap, region, large backend, reserve, commit, release, footprint, and
