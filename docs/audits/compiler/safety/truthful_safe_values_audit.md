...
`compiler/internal/semantics/semantics_core.go` rejects assignment to `ptr` and `len` when any field
projection step reaches a fixed array, slice, or string representation.
...
Safe `window`, `prefix`, and `suffix` are implemented for `[]u8`, `[]u16`, `[]i32`, and `[]bool`.
String views, `copy`, and `borrow` remain future work. Raw slice construction is available only
through the audited `core.raw_slice_*_from_parts` unsafe gateway family and carries conservative
