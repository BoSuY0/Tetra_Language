...
This slice closes safe borrowed return semantics for supported slice/String borrowed views. It makes
`-> borrow` useful only when the returned borrowed view is tied to a caller-visible borrowed source,
and it keeps unsafe or unknown provenance conservative.

## Implemented Rules

| Rule                                                                                               | Evidence                                                                                                         |
| -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Borrowed return signatures parse and preserve `ReturnOwnership`.                                   | `compiler/internal/frontend/frontend_core.go`, `compiler/internal/frontend/frontend_suite_test.go`               |
| Borrowed returns must originate from caller-visible sources.                                       | `compiler/internal/semantics/semantics_checker.go`, `compiler/tests/semantics/semantics_async_ownership_test.go` |
...
| Module/interface boundaries preserve borrowed return contracts.                                    | `TestGenerateInterfaceFromSourcePreservesBorrowedReturnContract`, `compiler/compiler_facade.go`                  |
| PLIR preserves borrow/provenance/no-escape facts for safe borrowed views.                          | `compiler/internal/plir/plir.go`, `compiler/internal/plir/plir_test/plir_test.go`                                |
| Reports expose borrowed-return no-allocation/proof facts and report flags do not change semantics. | `compiler/compiler_external_test.go`                                                                             |

## Code Changes

- `compiler/tests/semantics/semantics_async_ownership_test.go` adds
  `TestBorrowedReturnRejectsUnsafeUnknownProvenance`.
- `compiler/internal/semantics/semantics_checker.go` now rejects explicit borrowed returns whose
  source collapses to the synthetic `"<borrow>"` marker, with diagnostic text:

```text
borrowed slice return requires caller-visible borrow source
```

This turns expression-level unsafe raw-slice borrowed returns into an explicit borrow-source failure
instead of a later generic region mismatch.
...
Initial result: failed before implementation because the fixture exposed a missing explicit
caller-visible borrow source through existing region validation.
...
Result: pass. The drift scan found only the explicit drift-guard references in `GOAL.md` and
`CONTROL.md`; no sidecar names the Surface plan as the active goal.

## Non-Claims

- P14.1 does not implement named lifetimes.
- P14.1 does not implement a full borrow graph or noalias optimizer contract; that remains P14.2.
- P14.1 does not promote raw or external provenance to known.
- P14.1 does not remove runtime checks or change safe semantics through report flags.
