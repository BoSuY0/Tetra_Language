...
Translation Validation v2 records the current supported optimizer-validation subset as
machine-checkable evidence. It builds on the existing `validation.ValidateTranslation` and
`opt.Manager` contract and adds an explicit P23.0 report/validator for scalar symbolic equivalence,
supported i32 slice memory samples, loop and call/inlining differential samples, bounds proof
