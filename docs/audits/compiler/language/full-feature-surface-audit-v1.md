...
Schema: `tetra.language.feature_surface_audit.v1` Scope: `p22.0_full_feature_surface_audit`

P22.0 records the current feature surface before Phase K promotion work. It uses `FeatureRegistry()`
as the canonical status source and copies those statuses into report rows so the audit can catch
drift between docs, manifest truth, and implementation evidence.

This audit does not promote language behavior. It classifies each P22.0 surface as bounded current
support, static-only support, experimental gate, unsupported, planned, or post-v1 until same-branch
evidence exists.
