...
The validator gate is implemented in
`tools/cmd/validate-local-benchmark-tier1/main.go`.

## Validator Behavior

For a measured Tetra row in a zero-heap-required category, validation now
requires:
...
Excluded rows still require truthful memory evidence, but they do not fail
merely because they allocate heap.
...
Rows deliberately excluded by policy still include measured heap allocation in
the fresh report, for example:
...
git diff --check -- tools/cmd/validate-local-benchmark-tier1/main.go tools/cmd/validate-local-benchmark-tier1/main_test.go docs/spec/zero_heap_benchmark_policy.md
graphify update .
```
...
```text
verify-docs: lib/core/block.tetra: effects metadata mismatch: got alloc, mem want none
```

The failure is not caused by the zero-heap validator or policy files, but it
remains a final verification risk for MEM-12.

## Remaining Work

MEM-4 is still open: dedicated zero-heap microbenchmarks must either be added
outside the comparable Tier 1 matrix or the plan must record a final placement
