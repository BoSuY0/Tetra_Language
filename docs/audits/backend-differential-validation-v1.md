...
The backend differential matrix now compares supported i32 rows across source,
Stack IR, optimized Stack IR, SSA, Machine IR, and Linux x64 native execution
lanes. It covers deterministic scalar, branch/loop, call-loop, and proof-tagged
slice-sum rows, plus bounded randomized samples and first-mismatch reducer
metadata.
...
This audit is supported-subset evidence. It does not claim exhaustive random
testing, a complete source interpreter, a full native differential suite for
every target, or full formal proof. Unsupported rows remain explicit
unsupported evidence until later slices promote them safely.
