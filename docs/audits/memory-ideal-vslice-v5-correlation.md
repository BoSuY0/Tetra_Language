...
Vertical Slice v0/v1/v2/v3/v4 correlation pattern to raw pointer unsafe
contracts without making arbitrary unsafe memory safe. `MemoryFactGraph`
remains the source of truth; this document is only a projection/audit
correlation.
...
```bash
go run ./tools/cmd/validate-memory-correlation --file docs/audits/memory-ideal-vslice-v5-correlation.md
```
...
- the row set is exactly `MEM-UNSAFE-001`, `MEM-UNSAFE-002`,
  `MEM-UNSAFE-003`, and `MEM-UNSAFE-004`.

## Update Policy

Unknown external raw pointers remain `unsafe_unknown` and may not produce
`safe_known`, `provenance_known`, or `no_alias` facts. Verified
`core.alloc_bytes` roots may produce bounded allocation-base metadata, but the
metadata remains unsafe-origin evidence. Runtime-checkable unsafe contracts are
limited to nonnull, alignment, and length/bounds. Unsafe noalias, lifetime, and
region contracts remain static-untrusted unless a separate later proof exists.

This document does not claim arbitrary external pointer safety, an FFI lifetime
system, broad unsafe noalias, safe wrapper promotion, actor/task/runtime
expansion, target parity, or performance.
