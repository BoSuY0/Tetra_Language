...
This audit closes MEM-6 with compiler-owned runtime object/link/init evidence.
It does not claim ELF symbol-table proof.
...
- Backend reports also expose native runtime object plan evidence:
  - `summary.runtime_object_plan.evidence_class =
    native_runtime_object_plan`
  - `summary.runtime_object_plan.evidence_method =
    native_link_runtime_object_plan_v1`
  - `runtime_used`
...
- `runtime_object_plan` is built from the same semantic runtime usage collectors
  and `buildruntime.DecideRuntimeObjectPlan` decision path used by native
  linking.
- Tier 1 metadata mirrors both evidence groups.
- Tier 1 validator rejects missing or mismatched runtime feature/object-plan
  evidence.
...
- Current generated Tetra binaries are static ELF files with no section header
  and no symbols, so `nm` and `readelf -s` cannot prove absence of runtime
  symbols.
- The claim is narrower and compiler-owned: the native runtime object decision
  path reports no runtime object linked or initialized for simple protected
  rows, and the validator requires that report evidence.
- This does not optimize RSS by itself; it makes lazy runtime link/init evidence
  visible and enforceable in local benchmark artifacts.

## Next

Proceed to MEM-7: remaining heap allocations need explicit reason codes
explaining why they could not become eliminated, register, stack, region/island,
or domain allocations.
