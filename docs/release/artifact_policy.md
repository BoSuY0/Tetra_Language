...
Status: v1.0 release policy. This file defines which artifacts are tracked and
which are regenerated into a report directory.
...
| `docs/generated/v1_0/*` | Yes only for reviewed release-prep snapshots | `bash scripts/release_v1_0_gate.sh --report-dir <dir>` then copy reviewed outputs | Do not refresh casually during feature work. |
...
| Known issues | Report artifact by default, tracked only for reviewed release snapshots | `bash scripts/release_v1_0_gate.sh --report-dir <dir>` | Gate writes `<dir>/artifacts/known_issues.md`; reviewed snapshots may be copied to `docs/generated/v1_0/known_issues.md`. |
...
for example `/tmp/tetra-v1-rc1-gate`. Only reviewed snapshots should be copied
into `docs/generated/v1_0/`.

## Checklist Link

`docs/checklists/v1_0_release_gate.md` is the authoritative checklist for which
artifact evidence is mandatory before `v1.0.0`.
