...
| `v0.4.0` completion audit | Yes for the scoped v0.4.0 objective | `go run ./tools/cmd/validate-v0-4-completion-audit --audit docs/release/v0_4_0_completion_audit.md --expected-status achieved` | Validates the prompt-to-artifact checklist, achieved result classifications, the six-column `Release Evidence Matrix` (`requirement -> files -> tests -> docs -> evidence -> status`), positive and negative test evidence, manifest/docs/report/Graphify/CI evidence keys, and no dirty-green pass rows. |
