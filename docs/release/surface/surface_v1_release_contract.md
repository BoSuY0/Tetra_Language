...
## Native Surface Host v1 Boundary

`linux-x64-real-window` is current release evidence for the bounded
`surface-v1-linux-web` scope. It is not Native Surface Host v1 proof.

Native Surface Host v1 is tracked by `tetra.surface.native-host.v1` and
`host_evidence.level = linux-x64-native-surface-host-v1`. That proof requires a
compiled linux-x64 Tetra app, the official Wayland Surface host process,
`tetra.surface.host-ipc.v1`, app-produced RGBA frames, real pointer/key/close
events observed through the host, and no PNG/SVG/HTML/browser-canvas/viewer/
pre-rendered UI delivery path.

Release notes, audits, scripts, reports, and docs must not upgrade
`linux-x64-real-window`, memfd starter, browser-canvas, screenshot, or probe
evidence into a Native Surface Host v1 completion claim. `tetra surface run`
is the intended shortcut for the same native-host-required path. There is no
release/current Native Host v1 claim until the strict native-host gate passes
with real pointer, key, close, and app-frame evidence.
...
- Target: `headless`
  - Release status: release-test-supported
  - Required evidence: deterministic runtime/text/toolkit/accessibility evidence
- Target: `linux-x64`
  - Release status: current
  - Required evidence: real Wayland shm window probe/release evidence, native
    event pump, text/clipboard/IME, toolkit, accessibility bridge,
    `linux-app-shell-subset-v1`, `electron-feature-ledger-v1`,
    `surface-security-permission-v1`, `surface-performance-budget-v1`, and
    `surface-dev-workflow-v1`.
  - Nonclaim: this row does not claim `tetra.surface.native-host.v1`
    completion.
- Target: `wasm32-web`
  - Release status: current
  - Required evidence: real browser canvas, browser input, clipboard/IME,
    toolkit, accessibility snapshot/mirror,
    `tetra.surface.browser-surface.v1` evidence with DOM host canvas only
- Target: `macos-x64`
  - Release status: unsupported for Surface v1
  - Required evidence: `tetra.surface.target-host-status.v1` records
    `UNSUPPORTED` nonclaim evidence; build-only artifacts must not promote
...
- Target: `windows-x64`
  ...
  - Required evidence: `tetra.surface.target-host-status.v1` records
    ...
    Surface runtime support
- Target: `wasm32-wasi`
  - Release status: unsupported for Surface UI
  - Required evidence: must not claim UI runtime
