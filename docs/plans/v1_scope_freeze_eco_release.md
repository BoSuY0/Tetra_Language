...
Status: approved scope-decision artifact for unresolved TODO closure.
Date: 2026-04-26.
Applies to: `docs/plans/2026-04-26-tetra-language-todo.md` unresolved items in Eco/release/execution-order sections.
...
`release_v1_0_gate.sh` intentionally blocks v1 labeling while `./tetra version` is not `v1.0.x`.
...
| Permission model stabilization | `blocked-by-prerequisite` | Define enforceable permission semantics and diagnostics for capsule graph validation. | `go test ./cli/... ./tools/... -run 'Eco|Vault|Capsule|Lock' && bash scripts/test_all.sh --full` |
...
| Reproducible build basics | `blocked-by-prerequisite` | Define reproducibility baseline format and acceptance criteria for one native and one WASM path. | `bash scripts/release_v1_0_gate.sh` |
...
| --- | --- | --- | --- |
| Update version only when release branch is ready | `blocked-by-prerequisite` | Do not bump until all mandatory v1 checks are real and green. | `./tetra version && bash scripts/release_v1_0_gate.sh` |
...
| Finalize release notes | `blocked-by-prerequisite` | Keep draft language until v1 release branch readiness is proven. | `bash scripts/release_v1_0_gate.sh` |
| Check every release-gate checklist item | `blocked-by-prerequisite` | Checklist cannot be fully checked while blocked capabilities remain. | `bash scripts/release_v1_0_gate.sh` |
...
| Run WASI smoke in WASI runner | `blocked-by-prerequisite` | Requires implemented WASI backend and runner integration. | `bash scripts/release_v1_0_gate.sh` |
| Run web UI smoke through browser automation | `blocked-by-prerequisite` | Requires implemented UI model + web backend + automation harness. | `bash scripts/release_v1_0_gate.sh` |
...
| Verify reproducible builds for native + WASM | `blocked-by-prerequisite` | Reproducibility policy and WASM implementation are not complete. | `bash scripts/release_v1_0_gate.sh` |
...
| 5. Type system stabilization before ownership/race freedom | `blocked-by-prerequisite` | Ownership/race claims remain blocked until type-stability criteria pass. | `bash scripts/release_v1_0_gate.sh` |
| 6. Ownership/race freedom before safe-code guarantees | `blocked-by-prerequisite` | Safe-code guarantee stays blocked until ownership/race checks are complete. | `bash scripts/release_v1_0_gate.sh` |
| 7. Add WASM before UI web release checks | `blocked-by-prerequisite` | UI web release checks remain blocked while WASM targets are planned. | `bash scripts/release_v1_0_gate.sh` |
| 8. Stabilize stdlib/tooling before final release notes | `blocked-by-prerequisite` | Release notes remain draft until stdlib/tooling gate items are real. | `bash scripts/release_v1_0_gate.sh` |
| 9. Run final v1.0 gate only after placeholders are real | `blocked-by-prerequisite` | Keep intentional v1 gate refusal until real implementation replaces placeholders. | `bash scripts/release_v1_0_gate.sh` |
