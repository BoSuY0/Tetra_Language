...
actors/task/task-group smoke, single-spawn typed-task/staged typed-task/typed task-group/actor-state smoke, plus `fs_exists` filesystem runtime
...
`linux-x32` has self-host time, bounded two-spawn actors/task/task-group, single-spawn typed-task/staged typed-task/typed task-group,
...
remain build-only/host-probed evidence. They may run source/test slices only on a host that can execute the
...
4. **Finish `linux-x86`.**
   ...
   raw pointer-slot base/offset executable, scoped island/free executable, and bounded two-spawn self-host smokes into runtime startup/allocator/free/panic/time/task/actor/syscall bridge,
...
`runner network socket`, `runner network options`, and `runner task join`. On an unsupported x86/x32 runner host, they must be JSON
