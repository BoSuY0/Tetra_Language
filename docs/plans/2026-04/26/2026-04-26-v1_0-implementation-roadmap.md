...
- [x] `deferred-post-v1`, `blocked-by-prerequisite`, and `scope-freeze` notes do not count as v1.0
      completion.
...
- [x] Keep normalization only as migration tooling (optional command/tool path), not canonical
      frontend.
...
- [x] Add complete positive/negative ownership test matrix for borrow/inout/consume/actor/task
      paths.
...
- [x] Keep LSP diagnostics/hover/completion/definition/references/rename/formatting/code-actions
      stable across expanded language surface.
...
- [x] All generated release artifacts for docs, manifest, release notes, and API diff proofs are
