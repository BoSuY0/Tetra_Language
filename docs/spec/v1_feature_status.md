...
| --- | --- | --- |
| Closures and callable/function-value surface | `split by level` | Current `v0.2.0` support is the constrained Level 0 callable MVP: closure literals, top-level `closure`, function type refs (`fn(...) -> ...`), direct local calls for let-bound non-capturing closure values, callback-parameter calls when the caller passes a known symbol-backed immutable function-typed local or a direct named non-generic non-throwing function/closure symbol, function return of symbol-backed non-generic non-throwing values in function-typed return paths, and immutable function-typed local-to-local binding when signatures match. Level 1 non-capturing expansion is experimental only. Level 2 captured closures, arbitrary escape/passing/storing, full capture matrix, and ABI redesign remain planned/post-v1 until borrow/lifetime and ABI evidence exist; do not describe them as full first-class functions in `v0.2.0`. |
