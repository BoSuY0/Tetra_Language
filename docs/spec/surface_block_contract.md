...
This contract freezes `Block` as the runtime primitive for the Surface
Block/Morph product track. Compatibility helpers such as Button, Card, TextBox,
TextField, Sidebar, and Modal may exist as recipes or helper APIs, but they are
not core Surface primitives.

The machine-readable contract lives in `docs/spec/surface_block_contract.json`
and is validated by:
...
`BlockProps` is intentionally compact enough for the current 10-slot ABI limit.
...
Reports with `block_graph` evidence must also include paint command, layout
pass, and Block accessibility tree evidence. A graph-only screenshot or
metadata-only report is not enough.
...
GPU renderer production support remains a nonclaim until a later packet proves
and gates it.
...
- `core_primitives` containing Button, Card, TextField, TextBox, Sidebar, or
  Modal;
- `block_graph` reports without paint command, layout pass, and accessibility
  evidence.
