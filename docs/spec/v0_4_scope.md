...
The current release truth remains `docs/spec/current_supported_surface.md` until
the `v0.4.0` implementation and release gate are complete. This document records
the production scope selected for the requested `v0.4.0` line after the release
objective was narrowed to Linux x64 first, with EcoNet explicitly excluded.

## Scope Rule

`v0.4.0` is a Linux-x64 production release. It can be promoted only when every
selected Linux-x64 feature and runtime decision has implementation, tests,
documentation, release-gate evidence, and security review where applicable.
...
- `wasm.runtime-execution`, `wasm32-wasi`, and `wasm32-web` production runtime
  claims.
- `windows-x64` and `macos-x64` production runtime evidence.
- GTK/Qt/OS toolkit native UI backends, broad platform accessibility
  integration, and non-Linux native UI runtime claims.

Existing implementation or experimental support outside this list may remain in
the repository, but it must not be used as a `v0.4.0` production claim unless a
future scope decision explicitly promotes it.

## Required Release Artifacts

Before `v0.4.0` can be marked current, these artifacts must exist and point at
the same intended release commit:
