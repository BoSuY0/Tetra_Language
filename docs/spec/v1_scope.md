...
| Planned lifetime SSA solver | Planned | Full SSA lifetime analysis for richer interprocedural lifetime proofs, precise merge reasoning, and broader alias modeling is future work; current ownership/resource safety remains a conservative MVP | future gated compiler evidence plus docs verification | safety agent |
| Ownership, lifetime, island, actor/task transfer, and race-safety checks | Required before release label | Negative tests for use-after-move, escaping borrows, aliasing, invalid transfers, and actor/task races; current support is the conservative MVP above, not a full SSA lifetime solver | `go test ./compiler/... -run 'Ownership|Borrow|Lifetime|Island|Actor|Task' -count=1` | safety agent |
