...
`tools/cmd/gen-docs` emits Markdown API docs with a small machine-readable metadata comment directly below the top-level heading:
...
The metadata is intentionally alpha-scoped. It gives release gates and Eco-local tooling a stable prototype for API diff checks without defining a final package registry format.
