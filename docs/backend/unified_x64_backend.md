...
This repository uses an “hourglass” x64 backend: CPU-only logic is implemented once, while OS/ABI
differences are
constrained to a small adapter layer.

Status: the unified backend refactor described below is **implemented** (see `x64core`, `x64abi`,
`x64obj`), and the
...
- calls: `IRCall` (argument packing, stack alignment rules are ABI-dependent, but the IR-level
  behavior is shared)
...
- Linux, macOS and Windows linkers all have distinct data sections and support `RelocDataDisp32` to
  patch
...
Implemented:
- Shared TOBJ builder: `compiler/internal/backend/x64obj` (data blobs + call/import patches +
  relocs).
...
- `windows_x64`: TOBJ object path uses `x64core.NewEmitFunc(x64abi.NewWin64())` with import
  collection enabled.
...
- TOBJ writer/reader preserves `target/module/code/data/symbols/relocs` and rejects invalid
  magic/version/header strings.
...
- Call lowering validates stack/register argument boundaries (including stack-argument cases above
  register windows).
...
- native linkers reject cross-target objects with a clear `linker target mismatch` diagnostic before
  format writing.

### Build-only target expectations

- native build smoke is verified for `linux-x64`, `macos-x64`, `windows-x64` on object/executable
  paths without requiring
  cross-host execution.
- execution evidence remains host-bound (`host == target`); non-host targets are build-verified
  only.

## Remaining OS-specific areas

- **ABI/OS services**: syscalls vs imports, stack alignment, and calling conventions are handled in
  `x64abi`.
- **Executable format writers**: ELF/Mach-O/PE are format-specific by nature (they sit on top of
  `linkcore`).
- **Runtime objects**: some runtime components (for example actors) are currently emitted as TOBJ
  objects and still have
  low-level, ABI-aware code paths that should stay thin and well-documented.

## Notes on future architectures (e.g. ARM64)

This “hourglass” split makes it much easier to share *OS-agnostic* compiler logic across targets,
but adding a new CPU
...
In other words: the codebase is architecturally prepared for additional architectures, but they are
not “trivial”.
