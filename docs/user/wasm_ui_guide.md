...
- `runtime_trace` containing `window-mount:ok`, `root-mount:ok`, layout,
  text, button, input, list, panel, focus, input-event, change, select, click,
  timer, async-command, redraw-update, error-recovery, and
  `ui-event-dispatch:web-command-dispatch` markers
