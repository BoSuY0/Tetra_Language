...
`docs/spec/core/current_supported_surface.md`. Validate with:
...
Generated docs may show examples with two spellings. If an example source file declares
`module ...`, generated docs render its dotted module path, such as
`examples.core.data.core_math_smoke`. If an example source file has no module declaration, generated
docs render its portable file path, such as `examples/flow/flow_hello.tetra`.

This index always lists repository file paths under `examples/` so smoke-list validation and release
evidence stay portable. When comparing this index with generated docs, map dotted `examples.*`
module names back to their source files before treating the rendering difference as drift.

## Surface Claim Tier Notes

Surface examples use the claim tiers from `docs/spec/core/current_supported_surface.md`:
`PROD_STABLE_SCOPED`, `BETA_TARGET_HOST`, `EXPERIMENTAL`, `UNSUPPORTED`, and `NONCLAIM`. In this
index, release-supported examples are current only inside the bounded `surface-v1-linux-web`
evidence scope, experimental Block/Morph entries remain evidence tracks, and unsupported
targets/features must stay explicit nonclaims. The product gate command is
`bash scripts/release/surface/product-gate.sh --report-dir reports/surface-product-v1`, but that
...
- `examples/surface/release/surface_release_form.tetra`: scoped `production-widgets-v1` release
  subset form evidence for the bounded linux-x64/web Surface release scope; it is not a
  platform-native widget or future core primitive claim.
- `examples/surface/release/surface_release_text_input.tetra`: text/input, invalid UTF-8, multiline
  storage, selection copy/paste, clipboard, and IME/composition baseline evidence.
- `examples/surface/release/surface_release_accessibility.tetra`: accessibility metadata plus
  platform bridge evidence for supported Linux and web targets.
- `examples/surface/toolkit/surface_linux_app_shell_notes.tetra`: scoped Linux app-shell notes
  reference for lifecycle, multi-window, resize/DPI/cursor, clipboard, IME, accessibility, and
  blocked-pass shell-feature evidence without native widgets.
...
These examples remain useful regression evidence, but the public release docs point new work to
`ui.surface-toolkit-v1`, `ui.surface-text-input-v1`, and `ui.surface-accessibility-v1`.

## Experimental Block-First Beauty Examples

These examples are P15-P18 evidence for polished UI built from `Block` configuration only. The
complete Block-system gate is
`scripts/release/surface/block-system-gate.sh --report-dir reports/surface-block/p18-budget`; it
writes same-commit headless, linux-x64 real-window, and wasm32-web browser-canvas Block reports plus
artifact hashes and `block_system.memory_budget` evidence. The headless smoke also writes
`surface-block-examples.json`. These examples do not introduce core
Button/Card/TextField/Sidebar/Modal abstractions and do not promote Block to production support.

- `examples/surface/block_apps/surface_block_command_palette.tetra`: command palette overlay,
  editable query field, and command rows using Block layout, layered paint, text, assets,
  accessibility, state selectors, motion, and scene checksum evidence.
- `examples/surface/block_apps/surface_block_project_dashboard.tetra`: sidebar-like shell, metric
  panels, activity card, and action affordance built as Block configurations.
- `examples/surface/block_apps/surface_block_settings.tetra`: settings form with labels, editable
  fields, save/reset actions, focus order, and label relationships through Block metadata.
- `examples/surface/block_apps/surface_block_editor_shell.tetra`: editor shell with rail, tabs,
  scrollable code panel, selected line styling, and deterministic state/motion evidence.
- `examples/surface/block_apps/surface_block_glass_panel.tetra`: glass overlay/control panel with
  image/icon assets, overlay paint, rounded capsules, focus order, and motion-backed interaction
  evidence.

## Experimental Morph Capsule Examples

The `ui.surface-morph-capsule` P08 evidence set imports `lib.core.morph`, uses capsule recipes,
records recipe expansions, and then validates the resulting `BlockTree`:
...
The gate is `scripts/release/surface/morph-gate.sh --report-dir reports/surface-morph/gate`. It
writes `tetra.surface.morph.v1` headless evidence plus a `tetra.surface.morph.gate.v1` summary.
These examples are not Surface v1 production support and do not introduce core
Button/Card/TextField/Sidebar or Modal primitives.
...
The generated sources use `lib.core.surface`, `lib.core.block`, and `lib.core.morph`; the notes
template also uses the scoped `lib.core.surface_app_shell` helpers. The template smoke gate is
`scripts/release/surface/surface-template-smoke.sh --report-dir reports/surface-templates/gate`. It
writes `tetra.surface.template-smoke.v1` / `surface-template-smoke-v1` evidence for generation,
check, build, run, inspection, visual diff, and packaging.

## Surface Reference App Suite

The `ui.surface-reference-app-suite-v1` gate proves practical product shapes with Block/Morph
authoring over Block. Run it with:
...
It writes `tetra.surface.reference-app-suite.v1` / `surface-reference-app-suite-v1` plus
`tetra.surface.visual-regression.v1` evidence for headless, linux-x64 real-window, and wasm32-web
browser-canvas targets. Every app is checked, built, run, visually diffed, and recorded with
token/theme, layout, interaction, accessibility, performance, and artifact-hash evidence.
`lib.core.widgets` is allowed only in the migration compatibility example.
...
Legacy `tetra.ui.v1` metadata examples are compatibility evidence for `ui.metadata-v1`, not Surface
v1 release evidence.

Entries:

- Example: `examples/smoke/basic/hello.tetra`
...
- Example: `examples/memory/islands/islands_hello.tetra`
  - Purpose: Minimal island program.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/memory/islands/islands_i32.tetra`
  - Purpose: Island integer access.
  - Target group: native
  - Expected behavior: exits 55

- Example: `examples/memory/islands/islands_overflow.tetra`
  - Purpose: Island bounds diagnostic smoke.
  - Target group: native
  - Expected behavior: exits 1

- Example: `examples/memory/islands/islands_double_free.tetra`
...
- Example: `examples/memory/raw/mmio_smoke.tetra`
  - Purpose: MMIO builtin smoke.
  - Target group: native
  - Expected behavior: exits 123

- Example: `examples/memory/raw/cap_mem_smoke.tetra`
  - Purpose: Memory capability smoke.
  - Target group: native
  - Expected behavior: exits 77

- Example: `examples/memory/raw/cap_mem_ptr_smoke.tetra`
  - Purpose: Pointer load/store through `cap.mem`.
  - Target group: native
  - Expected behavior: exits 77

- Example: `examples/memory/raw/memset_smoke.tetra`
  - Purpose: Memory set helper smoke.
  - Target group: native
  - Expected behavior: exits 88

- Example: `examples/actors/actors_pingpong.tetra`
  - Purpose: Actor ping-pong runtime smoke.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/actors/actor_sleep_pingpong.tetra`
  - Purpose: Actor timer wake smoke.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/actors/actors_decl_spawn.tetra`
  - Purpose: Actor declaration spawn target smoke.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/actors/actors_tagged_stress.tetra`
  - Purpose: Tagged actor message stress smoke.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/flow/flow_hello.tetra`
  - Purpose: Minimal canonical Flow program.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/flow/flow_struct_smoke.tetra`
  - Purpose: Flow struct syntax and field access.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/flow/flow_islands_smoke.tetra`
  - Purpose: Flow syntax with islands.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/flow/flow_unsafe_cap_mem_smoke.tetra`
  - Purpose: Flow unsafe capability memory path.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/flow/flow_grammar_surface_smoke.tetra`
...
- Example: `examples/ui/ui_native_shell_smoke.tetra`
  - Purpose: UI metadata native shell smoke.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/smoke/scalars/bool_smoke.tetra`
  - Purpose: Boolean branch smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/control/for_range_smoke.tetra`
  - Purpose: Range loop smoke.
  - Target group: native
  - Expected behavior: exits 55

- Example: `examples/smoke/control/for_collection_smoke.tetra`
  - Purpose: Collection loop smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/control/for_collection_u8_smoke.tetra`
  - Purpose: Byte collection loop smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/control/loop_control_smoke.tetra`
  - Purpose: Break and continue control flow.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/control/complex_control_flow_smoke.tetra`
  - Purpose: Nested control flow coverage.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/scalars/unary_not_smoke.tetra`
  - Purpose: Unary boolean negation.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/scalars/const_smoke.tetra`
  - Purpose: Global const expression smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/scalars/const_bool_smoke.tetra`
  - Purpose: Boolean constant smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/scalars/local_const_smoke.tetra`
  - Purpose: Local const binding smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/language/globals_smoke.tetra`
  - Purpose: Top-level `var`/`val` global storage smoke.
  - Target group: native
  - Expected behavior: exits 49

- Example: `examples/smoke/scalars/compound_assignment_smoke.tetra`
  - Purpose: Compound assignment smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/control/else_if_smoke.tetra`
  - Purpose: Else-if lowering smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/types/enum_match_smoke.tetra`
  - Purpose: Enum match smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/types/enum_exhaustive_match_smoke.tetra`
  - Purpose: Exhaustive enum match smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/types/enum_payload_smoke.tetra`
  - Purpose: Enum payload constructor and match smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/effects/effects_io_smoke.tetra`
  - Purpose: IO effect declaration smoke.
  - Target group: native wasm
  - Expected behavior: exits 0

- Example: `examples/effects/effects_mem_smoke.tetra`
  - Purpose: Memory effect declaration smoke.
  - Target group: native
  - Expected behavior: exits 17

- Example: `examples/effects/effects_actors_smoke.tetra`
  - Purpose: Actor effect declaration smoke.
  - Target group: native
  - Expected behavior: exits 0

- Example: `examples/smoke/types/optional_smoke.tetra`
  - Purpose: Optional value smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/types/optional_match_smoke.tetra`
  - Purpose: Optional none match smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/types/optional_match_some_smoke.tetra`
  - Purpose: Optional some match smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/memory/ownership/ownership_smoke.tetra`
  - Purpose: Ownership transfer and optional borrow smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/memory/safe_view/safe_view_borrow_return.tetra`
...
- Example: `examples/memory/safe_view/safe_view_string_borrow_return.tetra`
...
- Example: `examples/memory/safe_view/safe_view_copy_escape.tetra`
...
- Example: `examples/memory/safe_view/safe_view_actor_copy_boundary.tetra`
...
- Example: `examples/memory/safe_view/safe_view_task_copy_boundary.tetra`
...
- Example: `examples/memory/safe_view/safe_view_aggregate_copy_escape.tetra`
...
- Example: `examples/core/memory/core_capability_smoke.tetra`
...
- Example: `examples/surface/toolkit/surface_app_model.tetra`
...
- Example: `examples/surface/toolkit/surface_linux_app_shell_notes.tetra`
...
- Example: `examples/surface/morph_core/surface_morph_command_palette.tetra`
...
- Example: `examples/surface/morph_core/surface_morph_project_dashboard.tetra`
...
- Example: `examples/surface/morph_core/surface_morph_settings.tetra`
  - Purpose: Experimental Morph Capsule settings reference app using form field, field text, tab,
             and action recipes over Block.
  - Target group: native
  - Expected behavior: exits 0 through recipe expansion and BlockTree validation

- Example: `examples/surface/morph_core/surface_morph_editor_shell.tetra`
...
- Example: `examples/surface/morph_core/surface_morph_glass_panel.tetra`
...
- Example: `examples/core/memory/core_memory_negative_length_smoke.tetra`
...
- Example: `examples/core/runtime/core_postgres_prepared_smoke.tetra`
...
- Example: `examples/core/runtime/core_postgres_result_smoke.tetra`
...
- Example: `examples/experimental/experimental_math_smoke.tetra`
...
- Example: `examples/experimental/experimental_memcpy_smoke.tetra`
...
- Example: `examples/smoke/language/extension_smoke.tetra`
  - Purpose: Extension method smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/language/generic_smoke.tetra`
  - Purpose: Generic function smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/smoke/language/generic_struct_smoke.tetra`
...
- Example: `examples/smoke/language/struct_ctor_smoke.tetra`
  - Purpose: Call-style struct constructor smoke.
  - Target group: native
  - Expected behavior: exits 94

- Example: `examples/smoke/language/protocol_impl_smoke.tetra`
  - Purpose: Protocol implementation smoke.
  - Target group: native
  - Expected behavior: exits 42

- Example: `examples/surface/runtime/surface_counter.tetra`
...
- Example: `examples/surface/runtime/surface_browser_counter.tetra`
...
- Example: `examples/surface/runtime/surface_window_counter.tetra`
...
- Example: `examples/surface/runtime/surface_component_counter.tetra`
...
- Example: `examples/surface/runtime/surface_text_input.tetra`
...
- Example: `examples/surface/runtime/surface_textbox_app.tetra`
  ...
  - Expected behavior: exits 1 after click focus, text insertion, caret movement, backspace/delete,
                       ...
                       `examples/surface/release/surface_release_text_input.tetra`, while rich text,
                       bidi shaping, platform accessibility tree, user JS, DOM UI, and legacy
                       sidecars remain nonclaims

- Example: `examples/surface/toolkit/surface_tree_app.tetra`
...
- Example: `examples/surface/toolkit/surface_toolkit_form.tetra`
...
- Example: `examples/surface/toolkit/surface_toolkit_settings.tetra`
...
- Example: `examples/surface/toolkit/surface_accessibility_settings.tetra`
...
- Example: `examples/surface/migration/surface_migration_ui_web_smoke.tetra`
  - Purpose: Pure-Tetra Surface migration of `examples/ui/ui_web_smoke.tetra`.
...
- Example: `examples/surface/migration/surface_migration_ui_native_shell_smoke.tetra`
  - Purpose: Pure-Tetra Surface migration of `examples/ui/ui_native_shell_smoke.tetra`.
  - Target group: native surface-migration
...
- Example: `examples/surface/migration/surface_migration_dogfood_web_ui.tetra`
...
- Example: `examples/surface/migration/surface_migration_tetra_control_center.tetra`
...
- Example: `examples/smoke/basic/tooling_tests.tetra`
...
- Example: `examples/ui/ui_web_smoke.tetra`
...
The `./tetra smoke --list --format=json` report also emits `excluded_examples`. These examples are
intentionally outside the default linux-x64 smoke profile, but remain visible here with the exact
exclusion reason reported by the smoke list.

| Example                                                 | Reason                              |
| ------------------------------------------------------- | ----------------------------------- |
| `examples/actors/actors_decl_spawn.tetra`               | not part of linux-x64 smoke profile |
| `examples/actors/actors_tagged_stress.tetra`            | not part of linux-x64 smoke profile |
| `examples/memory/raw/cap_mem_ptr_smoke.tetra`           | not part of linux-x64 smoke profile |
| `examples/memory/raw/ctx_switch_sysv_smoke.tetra`       | not part of linux-x64 smoke profile |
| `examples/memory/raw/ctx_switch_win64_smoke.tetra`      | not part of linux-x64 smoke profile |
| `examples/smoke/types/enum_payload_smoke.tetra`         | not part of linux-x64 smoke profile |
| `examples/experimental/experimental_math_smoke.tetra`   | not part of linux-x64 smoke profile |
| `examples/experimental/experimental_memcpy_smoke.tetra` | not part of linux-x64 smoke profile |
| `examples/flow/flow_grammar_surface_smoke.tetra`        | not part of linux-x64 smoke profile |
| `examples/smoke/language/generic_struct_smoke.tetra`    | not part of linux-x64 smoke profile |
| `examples/smoke/language/globals_smoke.tetra`           | not part of linux-x64 smoke profile |
| `examples/smoke/basic/hello.tetra`                      | not part of linux-x64 smoke profile |
| `examples/memory/islands/islands_double_free.tetra`     | not part of linux-x64 smoke profile |
...
| `examples/smoke/language/struct_ctor_smoke.tetra`       | not part of linux-x64 smoke profile |
| `examples/tasks/task_bounded_stress.tetra`              | not part of linux-x64 smoke profile |
| `examples/smoke/basic/tooling_tests.tetra`              | not part of linux-x64 smoke profile |
| `examples/ui/ui_web_smoke.tetra`                        | not part of linux-x64 smoke profile |
...
- Validator schema IDs may retain historical artifact names even when the current release profile
  advances. `validate-example-index`, `validate-smoke-list`, and `validate-test-report` enforce
  strict JSON shape, deterministic smoke profiles, and failure evidence shape for the current branch
  state.
...
- `examples/smoke/basic/hello.tetra` is intentionally excluded from linux-x64 smoke matrix; this is
  unsupported profile scope, not a compiler/runtime break.
- If `examples/flow/flow_hello.tetra` or `examples/smoke/scalars/bool_smoke.tetra` stop
  compiling/running on native, treat as a regression and rerun `./tetra smoke --list --format=json`.

### Control-flow examples (`V020-0706..0710`)

- Loop/control examples should keep deterministic exits (`42` or `55`) in native smoke; any parser
  or lowering failure is a regression.
- If only `examples/smoke/control/for_collection_u8_smoke.tetra` fails while others pass, suspect
  byte-collection semantics rather than global smoke config.

### Const and assignment examples (`V020-0711..0715`)

- `const` and compound assignment failures are regressions when they fail formatting, parsing, or
  expected exit checks in smoke/test.
- Unsupported behavior should be documented explicitly; silent drift in exit codes is treated as
  broken behavior.

### Enum/match examples (`V020-0716..0720`)

- `examples/smoke/types/enum_match_smoke.tetra` and
  `examples/smoke/types/enum_exhaustive_match_smoke.tetra` are required native smoke coverage.
- Missing exhaustiveness diagnostics or changed exit contracts indicate a regression, not an
  unsupported target limitation.

### Optional/error examples (`V020-0721..0725`)

- `optional` and `typed error` smoke examples are release-covered on native and must keep stable
  expected exits.
...
- `generic`, `protocol`, and `extension` MVP examples are required in native smoke and should fail
  loudly on semantic regressions.
- `generic_struct` coverage is experimental evidence only unless the feature registry promotes
  generic structs to `current`.
- Enum payload constructor/match examples are current only for the narrow positional
  match/catch/if-let slice recorded in `docs/spec/core/current_supported_surface.md`.

### Safety/runtime examples (`V020-0731..0735`)

- `ownership`, `async`, `task`, `time_sleep`, `task_sleep_deadline`, `task_join_wait`,
  `task_group_cancel`, `deadline_aware_waits`, `actors_pingpong`, and `actor_sleep_pingpong` are
  native release-covered examples with deterministic exits.
- Scheduling-related nondeterminism is considered broken for these smokes; unsupported status must
  be documented as an exclusion.

### Memory/capability examples (`V020-0736..0740`)

- `islands_*`, `cap_mem`, `mmio`, and `memset` examples are split between required smoke cases and
  profile exclusions by design.
- If an excluded example appears as a failing smoke case unexpectedly, verify smoke-list config
  drift before changing code.

### UI/WASM examples (`V020-0741..0745`)

- `ui_web` and dogfood wasm/web examples are allowed as artifact/import preflight evidence on wasm
  targets.
- Native smoke exclusion for wasm-specific examples is expected; compile/link failures on wasm
  targets remain regressions.

### Project dogfood examples (`V020-0746..0750`)

- `dogfood_cli` and `dogfood_actor_task` are required native smoke entries with exit `0`; failures
  are regressions.
- `eco_dogfood` is intentionally excluded from linux-x64 smoke profile; local
  `./tetra run --target linux-x64 examples/projects/eco_dogfood/src/main.tetra` is the fallback
  check.
