...
mailbox are drained into the runtime message-pool free list when that actor
becomes `done`; they are not delivered after completion. `core.actor_link(peer)`
provides bounded local abnormal-exit propagation for nonzero linked exits. There
is no actor status, actor join, actor exit-code, actor close API, supervision
tree, restart behavior, monitor DOWN delivery, trap-exit message delivery, or
OTP-style lifecycle contract in the current runtime.
