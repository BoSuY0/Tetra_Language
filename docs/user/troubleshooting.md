...
command. These `release_v1_0_*` helpers are shared maintainer tooling; a report
counts for current support only when it is generated fresh in the selected
release or post-v0.4 report directory and passes its validator.
...
`release_v1_0_*` helper name alone as a runtime support claim.
