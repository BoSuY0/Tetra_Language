# v0.1.2 Production Release Gate

Status: checked against the canonical `v0.1.2` release evidence archive.
Required checkboxes below are tied to
`/tmp/tetra-v0_1_2-final-release-gate-20260427`, produced by
`scripts/release/v0_1_2/gate.sh` with `33` passing steps and `0` failures.
...
- [x] Version preflight: `./tetra version` reports `v0.1.2`.
  ...
  - Evidence artifacts: `<report-dir>/logs/01-bootstrap-tetra-binaries.log`,
    `<report-dir>/logs/02-version-preflight-v0-1-2-required.log`, and
    `<report-dir>/artifacts/release-state.json`.
  - Blocking gate: `scripts/release/v0_1_2/gate.sh`.
...
- [x] No required checklist item is checked without an artifact, log, or test
  ...
  - Evidence command: inspect `<report-dir>/summary.json`, `<report-dir>/summary.md`, and artifact
    paths listed below.
...
- [x] Parser, formatter, and diagnostics cover the supported Flow syntax
  ...
  - Blocking gate: `scripts/release/v0_1_2/gate.sh` runs `tetra fmt --check`
...
- [x] Unsafe/capability/privacy/Eco/WASM security review has complete evidence
  ...
  - Evidence workflow:
    `bash scripts/release/v1_0/security-review.sh --write-template <report-dir>/security-review.md`,
    then `bash scripts/release/v1_0/security-review.sh --signoff <report-dir>/security-review.md`.
...
- [x] WASM build-only smoke passes for `wasm32-wasi` and `wasm32-web`.
  - Evidence command: `./tetra smoke --target wasm32-wasi --run=false --report <path>` and
    `./tetra smoke --target wasm32-web --run=false --report <path>`.
...
- [x] Native shell UI emits a deterministic metadata sidecar for the native
  ...
  - Evidence validation:
    `go run ./tools/cmd/smoke-report-to-checklist --validate-only --report <path>`.
...
- [x] JSON diagnostics, test reports, target reports, doctor reports, smoke
  ...
  - Blocking gate: `scripts/release/v0_1_2/gate.sh` includes the
...
- [x] `bash scripts/ci/test-all.sh --full --keep-going --report-dir <dir>/test-all`
  - Evidence artifact: `<report-dir>/artifacts/test-all/summary.json`.
- [x] `bash scripts/release/v0_1_2/gate.sh --report-dir <dir>/release-gate`
...
Optional/beta items must not be required for `v0.1.2` unless promoted through
scope review and added to `docs/spec/v1_scope.md`.

## Informational Post-v1 Items

These are not required for `v0.1.2`:
...
On the `v0.1.2` release branch:

- `bash scripts/dev/bootstrap.sh && ./tetra version && ./t version` reports
  `v0.1.2`.
- `bash scripts/release/v0_1_2/gate.sh --report-dir <dir>/release-gate` must pass
