...
`scripts/release_v0_1_3_gate.sh` with `33` passing steps and `0` failures.
...
- [x] Version preflight: `./tetra version` reports `v0.1.3`.
  ...
  - Blocking gate: `scripts/release_v0_1_3_gate.sh`.
...
- [x] Parser, formatter, and diagnostics cover the supported Flow syntax
  ...
  - Blocking gate: `scripts/release_v0_1_3_gate.sh` runs `tetra fmt --check`
...
- [x] JSON diagnostics, test reports, target reports, doctor reports, smoke
  ...
  - Blocking gate: `scripts/release_v0_1_3_gate.sh` includes the
...
- [x] `bash scripts/test_all.sh --full --keep-going --report-dir <dir>/test-all`
  - Evidence artifact: `<report-dir>/artifacts/test-all/summary.json`.
- [x] `bash scripts/release_v0_1_3_gate.sh --report-dir <dir>/release-gate`
