...
func assertChildDirectoriesHaveReadmes(t *testing.T, parentRel string) {
	...
	if len(missing) > 0 {
		...
		t.Fatalf(
			"every %s child directory must document its boundary: %s",
			parentRel,
...
func TestNoWrapperRootScriptEntryPointsAreRemoved(t *testing.T) {
	...
	for _, entry := range entries {
		...
		if strings.HasSuffix(entry.Name(), ".sh") || info.Mode().Perm()&0111 != 0 ||
			hasShebang(t, path) {
	...
	if len(rootFiles) > 0 {
		...
		t.Fatalf(
			"root-level scripts entrypoint wrappers must be removed; found: %s",
...
func TestNoWrapperCompilerRootTestsAreDocumentedExceptions(t *testing.T) {
	...
	allowed := map[string]bool{
		...
		"actors_capacity_test.go":                   true,
		"actors_declaration_state_test.go":          true,
		"actors_runtime_symbols_test.go":            true,
		"actors_scheduler_targets_test.go":          true,
		"actors_test.go":                            true,
		"actors_typed_messages_test.go":             true,
		"atomic_suite_test.go":                      true,
		"atomic_target_diagnostics_test.go":         true,
		"compatibility_stability_v1_test.go":        true,
		"compiler_build_helpers_test.go":            true,
		"compiler_core_runtime_test.go":             true,
		"compiler_examples_microservices_test.go":   true,
		"compiler_external_test.go":                 true,
		"compiler_interface_only_callable_test.go":  true,
		"compiler_interface_only_region_test.go":    true,
		"compiler_interface_only_resources_test.go": true,
		...
		"explain_reports_alloc_test.go":             true,
		"explain_reports_bounds_test.go":            true,
		"explain_reports_memory_test.go":            true,
		"explain_reports_test.go":                   true,
		"feature_surface_audit_test.go":             true,
		"ffi_target_diagnostics_test.go":            true,
		"filesystem_runtime_test.go":                true,
		"first_class_callables_coverage_test.go":    true,
		"formal_core_v1_test.go":                    true,
		"fuzz_property_differential_v1_test.go":     true,
		"fuzz_suite_test.go":                        true,
		"link_object_contract_test.go":              true,
		"manifest_test.go":                          true,
		"memory_fuzz_oracle_v1_test.go":             true,
		"net_runtime_http_test.go":                  true,
		"net_runtime_linux_x64_epoll_test.go":       true,
		"net_runtime_linux_x64_socket_test.go":      true,
		"net_runtime_target_helpers_test.go":        true,
		"net_runtime_target_smoke_test.go":          true,
		...
		"runtime_heap_telemetry_test.go":            true,
		"runtime_hardening_v1_test.go":              true,
		"runtime_override_test.go":                  true,
		"security_review_gate_v1_test.go":           true,
		"self_hosting_gate_v1_test.go":              true,
		"surface_runtime_test.go":                   true,
		"task_runtime_cancellation_actor_test.go":   true,
		"task_runtime_deadlines_test.go":            true,
		"task_runtime_group_cancel_test.go":         true,
		"task_runtime_helpers_test.go":              true,
		"task_runtime_lowering_test.go":             true,
		"task_runtime_targets_test.go":              true,
		"task_runtime_test.go":                      true,
		"tetra_bug_regression_test.go":              true,
		"translation_validation_v2_test.go":         true,
	...
	if len(unexpected) > 0 {
		...
		t.Fatalf(
			("compiler root tests must move under compiler/tests or be " +
				"documented in compiler/tests/README.md: %s"),
	...
	for name := range allowed {
		if !strings.Contains(string(readme), "`"+name+"`") {
			t.Fatalf(
				"compiler root test exception %s must be documented in compiler/tests/README.md",
				name,
			)
		}
	}
