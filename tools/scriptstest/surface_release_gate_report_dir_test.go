...
func TestSurfaceReleaseGateRejectsStaleReportDir(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(reportPath, "surface-release-summary.json"), []byte("{}\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestSurfaceReleaseGateRejectsTraversalReportDir(t *testing.T) {
	...
	for _, want := range []string{
		"surface_release_gate: refusing unsafe report directory: parent traversal is not accepted (" + reportDir + ")",
...
func TestSurfaceReleaseGateRejectsUnsafeReportDirForms(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "absolute", reportDir: filepath.Join(root, "reports", "absolute"), want: "surface_release_gate: refusing unsafe report directory: absolute paths are not accepted"},
		{name: "repo-root", reportDir: ".", want: "surface_release_gate: refusing unsafe report directory: repo root is not a release report directory"},
		{name: "non-directory", reportDir: fileRel, want: "surface_release_gate: refusing to use non-directory report path: " + fileRel},
	} {
...
func assertSurfaceReleaseGateRejectedBeforeSideEffects(t *testing.T, out []byte) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(string(out), forbidden) {
			t.Fatalf("release gate should reject report dir before sub-gate or raw shell side effects:\n%s", out)
		}
	}
}
