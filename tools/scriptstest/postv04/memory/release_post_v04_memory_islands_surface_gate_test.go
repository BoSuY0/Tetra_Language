...
func TestReleasePostV04MemoryIslandsSurfaceProductionGateRunsStrictOrderedEvidence(t *testing.T) {
	...
	for _, want := range []string{
		("Usage: bash scripts/release/post_v0_4/memory-islands-surface-" +
			"production-gate.sh [--report-dir DIR]"),
	...
	for _, line := range strings.Split(text, "\n") {
		if strings.Contains(line, `"command":`) && strings.Contains(line, "json_string") {
			t.Fatalf(
				"integrated manifest command lines must not embed JSON string literals inside JSON strings: %s",
				line,
			)
		}
	}
...
func assertOrderedFragments(t *testing.T, text string, fragments ...string) {
	...
	for _, fragment := range fragments {
		start := last + 1
		idx := strings.Index(text[start:], fragment)
		if idx < 0 {
...
func runIntegratedGate(t *testing.T, root string, args ...string) ([]byte, error) {
	...
	cmdArgs := append(
		[]string{"scripts/release/post_v0_4/memory-islands-surface-production-gate.sh"},
		args...)
