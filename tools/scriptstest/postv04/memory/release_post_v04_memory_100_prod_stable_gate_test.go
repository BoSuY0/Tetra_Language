...
func TestReleasePostV04Memory100ProdStableGateRunsStrictOrderedEvidence(t *testing.T) {
	...
	if contract.Producer != "scripts/release/post_v0_4/memory-100-prod-stable-gate.sh" ||
		...
		t.Fatalf(
			"Memory100 contract producer/entrypoint = %q/%q",
			contract.Producer,
			contract.Entrypoint,
		)
	}
	...
	assertEqualOrderedStrings(
		...
		"Memory100 contract host_preconditions",
	)
	if contract.ArtifactHashes == nil || !contract.ArtifactHashes.Enabled ||
		...
		t.Fatalf(
			"Memory100 contract artifact_hashes = %#v, want enabled required sha256",
...
func memory100RequiredReports() []memory100RequiredReport {
	return []memory100RequiredReport{
		...
		},
		{
			...
			validator: "validate-memory-fuzz-oracle",
		},
		{
		...
		},
		{
			...
			validator: "validate-memory-islands-surface-production",
		},
		{
...
func assertMemory100RequiredReports(
	t *testing.T,
	contract gatecontract.Contract,
	want []memory100RequiredReport,
) {
	t.Helper()
	...
	for _, report := range contract.RequiredReports {
		...
		if !report.SameCommitRequired {
			t.Fatalf(
				"Memory100 required report %q same_commit_required = false, want true",
				report.Path,
			)
		}
		if !report.ArtifactHashRequired {
			t.Fatalf(
				"Memory100 required report %q artifact_hash_required = false, want true",
				report.Path,
			)
		}
