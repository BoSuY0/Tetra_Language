...
func TestReleaseV030GateRefreshesReleaseStateAfterFinalSummaryWrite(t *testing.T) {
	...
	if !finalReleaseStateRefreshFollowsSummary(text) {
		t.Fatalf("v0.3.0 release gate must refresh release-state after the final pass summary is written and validated")
	}
...
if [[ "$failed_count" -eq 0 && "$ci_missing_security_signoff" -eq 1 ]]; then`
	if !strings.Contains(text, summaryBeforeCIBranch) {
		t.Fatalf("v0.3.0 release gate must write and validate summary.json before CI missing-signoff branch")
	}
	ciBlockedReleaseState := `write_summary "blocked"
  validate_summary
  if check_release_state; then`
	if !strings.Contains(text, ciBlockedReleaseState) {
		t.Fatalf("v0.3.0 release gate must write blocked release-state artifacts before CI missing-signoff exit")
	}
	if strings.Index(text, `write_summary "pass"`) > strings.Index(text, `run_step "release state audit" check_release_state`) {
		t.Fatalf("v0.3.0 release gate writes pass summary after release-state audit")
	}
	if strings.Index(text, `validate_summary`) > strings.Index(text, `run_step "release state audit" check_release_state`) {
...
validate_summary
if check_release_state; then`) {
		t.Fatalf("v0.3.0 release gate must validate final summary before guarded final release-state refresh")
	}
