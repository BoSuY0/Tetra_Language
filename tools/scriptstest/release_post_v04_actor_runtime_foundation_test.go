...
func TestReleasePostV04ActorRuntimeFoundationGateRunsStrictOrderedEvidence(t *testing.T) {
	...
	if contract.Producer != "scripts/release/post_v0_4/actor-runtime-foundation-linux-x64-gate.sh" ||
		...
		t.Fatalf("actor runtime foundation contract producer/entrypoint = %q/%q", contract.Producer, contract.Entrypoint)
	}
	...
	if contract.FreshReportDirPolicy != "require-empty-or-new" {
		t.Fatalf("actor runtime foundation contract fresh_report_dir_policy = %q", contract.FreshReportDirPolicy)
	}
	assertEqualOrderedStrings(t, contract.HostPreconditions, []string{"linux", "go", "fresh-report-dir"}, "actor runtime foundation host_preconditions")
	if contract.ArtifactHashes == nil || !contract.ArtifactHashes.Enabled || !contract.ArtifactHashes.Required || contract.ArtifactHashes.Algorithm != "sha256" {
		t.Fatalf("actor runtime foundation contract artifact_hashes = %#v, want enabled required sha256", contract.ArtifactHashes)
	}
	assertActorRuntimeFoundationRequiredReports(t, contract, actorRuntimeFoundationRequiredReports())
	assertActorRuntimeFoundationCIArtifacts(t, contract, append(actorRuntimeFoundationRequiredReportPaths(), "logs/*.log"))
...
func loadActorRuntimeFoundationContract(t *testing.T, root string) gatecontract.Contract {
	t.Helper()
	contractPath := filepath.Join(root, "scripts", "release", "post_v0_4", "contracts", "actor-runtime-foundation-linux-x64.json")
...
func actorRuntimeFoundationRequiredReports() []actorRuntimeFoundationRequiredReport {
	return []actorRuntimeFoundationRequiredReport{
		{path: "actor-runtime-foundation-manifest.json", schema: "tetra.actor.production_foundation.v1", validator: "validate-actor-runtime-foundation"},
		{path: "parallel-production-linux-x64/parallel-production-linux-x64.json", schema: "tetra.parallel.production.v1", validator: "validate-parallel-production"},
		{path: "parallel-production-linux-x64/artifact-hashes.json", schema: "tetra.release-artifact-hashes.v1alpha1", validator: "validate-artifact-hashes"},
		{path: "distributed-actors-linux-x64/distributed-actors-linux-x64.json", schema: "tetra.actors.distributed-runtime.v1", validator: "validate-distributed-actor-runtime"},
		{path: "distributed-actors-linux-x64/artifact-hashes.json", schema: "tetra.release-artifact-hashes.v1alpha1", validator: "validate-artifact-hashes"},
...
func assertActorRuntimeFoundationRequiredReports(t *testing.T, contract gatecontract.Contract, want []actorRuntimeFoundationRequiredReport) {
	t.Helper()
	...
	for _, report := range contract.RequiredReports {
		...
		if !report.SameCommitRequired {
			t.Fatalf("actor runtime foundation required report %q same_commit_required = false, want true", report.Path)
		}
		if !report.ArtifactHashRequired {
			t.Fatalf("actor runtime foundation required report %q artifact_hash_required = false, want true", report.Path)
		}
...
func assertActorRuntimeFoundationCIArtifacts(t *testing.T, contract gatecontract.Contract, want []string) {
	t.Helper()
	got := make([]string, 0, len(contract.CIArtifacts))
	for _, artifact := range contract.CIArtifacts {
		if !artifact.Required {
			t.Fatalf("actor runtime foundation ci_artifacts entry %q required = false, want true", artifact.Path)
		}
