...
func TestLinuxRealWindowRequiresWayland(t *testing.T) {
	...
	if err := copyFile(
		filepath.Join(repoRoot(t), "scripts", "release", "surface", "surface-linux-x64-release-window-smoke.sh"),
	...
	cmd := exec.Command("bash", "scripts/release/surface/surface-linux-x64-release-window-smoke.sh", "--report-dir", reportRel)
	cmd.Dir = root
	...
	reportPath := filepath.Join(root, filepath.FromSlash(reportRel), "surface-linux-x64-release-window.json")
...
func envWithoutDisplay(root string) []string {
	...
	for _, entry := range os.Environ() {
		if strings.HasPrefix(entry, "WAYLAND_DISPLAY=") || strings.HasPrefix(entry, "DISPLAY=") || strings.HasPrefix(entry, "GOCACHE=") {
			continue
		}
