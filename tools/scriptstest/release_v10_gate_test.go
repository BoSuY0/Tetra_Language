...
func TestReleaseV10GateUsesRealV1Boundary(t *testing.T) {
	root := repoRoot(t)
	assertLegacyFileRemoved(t, "scripts/release_v1_0_gate.sh", "scripts/release/v1_0/gate.sh directly")
  ...
  go run ./tools/cmd/validate-release-state`
	...
	artifactIdx := strings.Index(text, `run_step "artifact hash manifest" check_artifact_hash_manifest`)
...
func TestReleaseV10GateRunsDedicatedV1Workflow(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !seen[want] {
			t.Fatalf("v1.0 dedicated workflow missing step %q in summary:\n%s", want, string(rawSummary))
		}
	}
	...
	if releaseState.LastGateEvidence.StepCount != len(summary.Steps) {
		t.Fatalf("release-state step_count %d contradicts final summary steps %d", releaseState.LastGateEvidence.StepCount, len(summary.Steps))
	}
...
func TestReleaseV10GateRejectsMissingReportDirArgument(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(string(out), forbidden) {
			t.Fatalf("missing report-dir argument should reject before workflow side effects:\n%s", out)
		}
	}
}
...
func TestReleaseV10GateRejectsNonEmptyReportDirBeforeSideEffects(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(string(out), forbidden) {
			t.Fatalf("gate should reject stale report dir before raw shell or workflow side effects:\n%s", out)
		}
	}
}
...
func TestReleaseV10GateAcceptsDashPrefixedFreshReportDirBeforeBootstrapSentinel(t *testing.T) {
	...
	out, err := cmd.CombinedOutput()
	if err == nil {
		t.Fatalf("minimal fake root should fail at bootstrap/version sentinel after accepting report dir\n%s", out)
	}
	if !strings.Contains(string(out), "release/v1_0/gate: bootstrapping local binaries before v1 version preflight") {
		t.Fatalf("dash-prefixed fresh report dir should reach the bootstrap stage, not fail in path setup:\n%s", out)
	}
	assertOutputAvoidsRawPathUtilityErrors(t, out)
	...
	for _, rel := range []string{"logs", "artifacts"} {
		if _, err := os.Stat(filepath.Join(reportDir, rel)); err != nil {
			t.Fatalf("dash-prefixed report dir should create normalized %s directory: %v\n%s", rel, err, out)
		}
	}
}
...
func TestReleaseV10GateRejectsSymlinkReportDirBeforeSideEffects(t *testing.T) {
	...
	if !strings.Contains(string(out), "release/v1_0/gate: refusing to use symlink report path: "+reportDir) {
	...
	for _, rel := range []string{"logs", "artifacts"} {
		if _, err := os.Stat(filepath.Join(targetDir, rel)); !os.IsNotExist(err) {
			t.Fatalf("symlink report dir should not write through target %s, stat err = %v\n%s", rel, err, out)
		}
	}
}
...
func TestReleaseV10GateAcceptsDashPrefixedSecuritySignoff(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, signoffPath), []byte("# Security Review\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("v1.0 gate should accept dash-prefixed security signoff source path: %v\n%s", err, out)
	}
