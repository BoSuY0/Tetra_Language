...
func TestReleaseV10BinarySizeWorkflowLivesInVersionedReleaseScript(t *testing.T) {
	...
	assertLegacyFileRemoved(
		t,
		"scripts/release_v1_0_binary_size.sh",
...
func TestReleaseV10BinarySizeRejectsDirectoryReportBeforeBuild(t *testing.T) {
	...
	if !strings.Contains(
		string(out),
		"release/v1_0/binary-size: refusing to use directory report path: "+reportPath,
	) {
...
func TestReleaseV10BinarySizeReportIsDeterministicForIdenticalBuilds(t *testing.T) {
	root := t.TempDir()
	copyReleaseV10BinarySizeScripts(t, root)
	if err := os.MkdirAll(filepath.Join(root, "examples", "flow"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(
		filepath.Join(root, "examples", "flow", "flow_struct_smoke.tetra"),
		[]byte("func main() -> Int:\n    return 0\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
...
`
	...
	if string(firstRaw) != string(secondRaw) {
		t.Fatalf(
			"binary size report should be byte-stable across identical runs\nfirst:\n%s\nsecond:\n%s",
			string(firstRaw),
			string(secondRaw),
		)
	}
	...
	if report.TargetCount != 5 || report.PassCount != 5 || report.WarnCount != 0 ||
		report.FailCount != 0 {
...
func copyReleaseV10BinarySizeScripts(t *testing.T, root string) {
	...
	if err := copyFile(
		filepath.Join(repoRoot(t), "scripts", "release", "v1_0", "binary-size.sh"),
		filepath.Join(root, "scripts", "release", "v1_0", "binary-size.sh"),
		0o755,
	); err != nil {
		t.Fatal(err)
	}
