...
func TestReleaseV10APIDiffWorkflowLivesInVersionedReleaseScript(t *testing.T) {
	...
	assertLegacyFileRemoved(
		t,
		"scripts/release_v1_0_api_diff.sh",
...
func TestReleaseV10APIDiffRejectsSymlinkReportDirBeforeWork(t *testing.T) {
	...
	if !strings.Contains(
		string(out),
		"release/v1_0/api-diff: refusing to use symlink report path: "+reportDir,
	) {
...
func TestReleaseV10APIDiffRejectsDashPrefixedNonEmptyReportDirBeforeRawFind(t *testing.T) {
	...
	if err := os.WriteFile(
		filepath.Join(reportDirPath, "api-docs.md"),
		[]byte("# stale\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
...
func TestReleaseV10APIDiffRejectsMissingBaselineBeforeDocsWork(t *testing.T) {
	...
	for _, path := range []string{
		...
		if _, err := os.Stat(path); !os.IsNotExist(err) {
			t.Fatalf(
				"missing baseline should block before docs work/artifact write: %s stat err = %v",
				path,
				err,
			)
		}
...
func TestAPIDiffReportClassifiesSignatureDriftAsChanged(t *testing.T) {
  ...
  "symbols": [
    {
      "id": "examples/flow/flow_hello.tetra::Functions::func main() -> Int uses io",
      "module": "examples/flow/flow_hello.tetra",
      "section": "Functions",
...
<!-- tetra-api-metadata: {"schema":"tetra.api.v1alpha1","api_hash":"sha256:87fd6e31a578f76509e7220bcdb43eea2a11dd71a62bc573f7951fea8a563425","module_count":1,"entry_count":1} -->
...
`
	...
	cmd := exec.Command(
		"node",
		filepath.Join(repoRoot(t), "scripts", "tools", "api_diff_report.mjs"),
		"--docs",
		docsPath,
		"--baseline",
		baselinePath,
	...
	if diff.Changes[0].BeforeEntry != "func main() -> Int uses io" ||
		diff.Changes[0].AfterEntry != "func main() -> i32 uses io" {
		t.Fatalf("changed entry should preserve before/after text: %#v", diff.Changes[0])
	}
	if diff.Changes[0].ReviewStatus != "breaking_requires_review" ||
		!strings.Contains(diff.Changes[0].ReviewNote, "signature or metadata changed") {
...
func TestAPIDiffReportClassifiesAdditionsAndRemovalsForReview(t *testing.T) {
  ...
  "symbols": [
    {
      "id": "examples/flow/flow_hello.tetra::Functions::func old_api",
      "module": "examples/flow/flow_hello.tetra",
      "section": "Functions",
...
<!-- tetra-api-metadata: {"schema":"tetra.api.v1alpha1","api_hash":"sha256:12b9ce8e557cff37a20f0362022116f5a9e7df1103d41b5960b14be8710606f9","module_count":1,"entry_count":1} -->
...
`
	...
	if string(first) != string(second) {
		t.Fatalf(
			"baseline should be byte-stable across identical writes\nfirst:\n%s\nsecond:\n%s",
			string(first),
			string(second),
		)
	}
...
func releaseV10APIDiffFakeRepo(t *testing.T) string {
	...
	if err := copyFile(
		filepath.Join(repoRoot(t), "scripts", "release", "v1_0", "api-diff.sh"),
