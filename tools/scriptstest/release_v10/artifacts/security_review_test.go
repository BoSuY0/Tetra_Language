...
func TestReleaseV10SecurityReviewWorkflowLivesInVersionedReleaseScript(t *testing.T) {
	...
	assertLegacyFileRemoved(
		t,
		"scripts/release_v1_0_security_review.sh",
...
func TestReleaseV10SecurityReviewRejectsMissingPathArguments(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		if !strings.Contains(string(out), tc.want) {
			t.Fatalf(
				"missing %s argument output missing controlled error %q:\n%s",
				tc.flag,
				tc.want,
				out,
			)
...
func TestReleaseV10SecurityReviewRejectsDirectoryTemplatePath(t *testing.T) {
	...
	if !strings.Contains(
		string(out),
		"security_review: refusing to use directory template path: "+templatePath,
	) {
...
func TestReleaseV10SecurityReviewAcceptsDashPrefixedSignoffPath(t *testing.T) {
	...
	if err := os.WriteFile(
		signoffPath,
		[]byte(validReleaseV10SecuritySignoff("v1.0.0", "0123456789abcdef0123456789abcdef01234567")),
		0o644,
	); err != nil {
		t.Fatal(err)
	}
...
`
	...
	if err != nil {
		t.Fatalf(
			"security review signoff with concrete smoke report paths should validate: %v\n%s",
			err,
			out,
		)
	}
...
func TestReleaseV040SecurityReviewWritesV040Template(t *testing.T) {
	...
	for _, want := range []string{
		...
		("`go run ./cli/cmd/tetra smoke --target linux-x64 --run=true --" +
...
- ` + ("`go run ./tools/cmd/validate-v0-4-readiness --features " +
		"<features.json> --targets <targets.json> --manifest docs/generated/" +
		"manifest.json --scope-decisions docs/release/v0_4/data/v0_4_0_scope_" +
		"decisions.json`: pass") + `
- ` + "`go test ./compiler/... ./cli/... ./tools/... -count=1`: pass" + `
- ` + "`go run ./tools/cmd/verify-docs --manifest docs/generated/manifest.json`: pass" + `
- ` + ("`go run ./cli/cmd/tetra smoke --target linux-x64 --run=true --" +
		"report reports/v0.4.0/linux-host-smoke.json`: pass") + `
- ` + ("`bash scripts/release/v0_4_0/distributed-actors-linux-x64-" +
		"smoke.sh --report-dir reports/v0.4.0`: pass") + `
- ` + ("`bash scripts/release/v0_4_0/native-ui-linux-x64-smoke.sh --" +
		"report-dir reports/v0.4.0`: pass") + `
- ` + "`git diff --check`: pass" + `
...
func TestReleaseV040SecurityReviewWorkflowLivesInVersionedReleaseScript(t *testing.T) {
	...
	assertLegacyFileRemoved(
		t,
		"scripts/release_v0_4_0_security_review.sh",
		"scripts/release/v0_4_0/security-review.sh directly",
	)
	raw, err := os.ReadFile(
		filepath.Join(root, "scripts", "release", "v0_4_0", "security-review.sh"),
	)
	if err != nil {
...
func TestV0_3_0_security_signoff_rejects_mismatched_canonical_artifact(t *testing.T) {
	...
	writeFile(
		hashesPath,
		`{"schema":"tetra.release-artifact-hashes.v1alpha1","root":".","artifacts":[]}`+"\n",
	)
...
`
	...
	if !strings.Contains(
		string(out),
		"missing required canonical gate artifact listing: release-state.json",
	) {
		t.Fatalf("validator should name missing canonical release-state artifact:\n%s", out)
	}
}

func TestV0_3_0_security_signoff_canonical_hash_lookup_ignores_bullets_outside_artifact_hashes(
	t *testing.T,
) {
	dir := t.TempDir()
	reportDir := filepath.Join(dir, "report")
...
`
	...
	if err == nil {
		t.Fatalf(
			"v0.3.0 signoff must ignore canonical summary hash outside Artifact Hashes\n%s",
			out,
		)
	}
	if !strings.Contains(
		string(out),
		"missing required canonical gate artifact hash: summary.json",
	) {
		t.Fatalf(
			"validator should require canonical summary hash in Artifact Hashes section:\n%s",
			out,
		)
	}
}
...
func releaseV10SecurityReviewPathFakeRepo(t *testing.T) string {
	...
	if err := copyFile(
		filepath.Join(repoRoot(t), "scripts", "release", "v1_0", "security-review.sh"),
		filepath.Join(root, "scripts", "release", "v1_0", "security-review.sh"),
		0o755,
	); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(
		filepath.Join(root, "compiler", "internal", "version", "version.go"),
...
func runReleaseV10SecurityReview(t *testing.T, root string, args ...string) ([]byte, error) {
	...
	cmd := exec.Command(
		"bash",
		append([]string{"scripts/release/v1_0/security-review.sh"}, args...)...)
	cmd.Dir = root
	cmd.Env = append(
		os.Environ(),
