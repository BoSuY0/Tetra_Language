...
func TestReleaseV10ReproRejectsDirectoryReportBeforeBuild(t *testing.T) {
	...
	if !strings.Contains(
		string(out),
		"release/v1_0/reproducible-build: refusing to use directory report path: "+reportPath,
	) {
...
`
	...
	if string(firstRaw) != string(secondRaw) {
		t.Fatalf(
			"proof should be byte-stable across identical runs\nfirst:\n%s\nsecond:\n%s",
			string(firstRaw),
			string(secondRaw),
		)
	}
	...
	if proof.Source != "examples/ui/ui_web_smoke.tetra" {
	...
	if len(proof.NativeTargets) != 3 {
		t.Fatalf(
			"native_targets = %#v, want linux/macos/windows target-aware entries",
			proof.NativeTargets,
		)
	}
	...
	for _, target := range []string{"linux-x64", "macos-x64", "windows-x64"} {
		if !seenNative[target] {
			t.Fatalf(
				"repro proof missing native target %s in native_targets: %#v",
				target,
				proof.NativeTargets,
			)
		}
	...
	for _, want := range []string{
		...
		if !seenExtensions[want] {
			t.Fatalf(
				"repro proof missing sidecar coverage %s in artifacts: %#v",
				want,
				proof.Artifacts,
			)
		}
...
`
	...
	for _, artifact := range proof.Artifacts {
		if artifact.Target == "wasm32-web" && artifact.Extension == ".mjs" && artifact.Sidecar &&
			!artifact.Match {
			return
		}
	}
...
`
	...
	if err == nil {
		t.Fatalf(
			"expected reproducibility proof to fail when expected UI sidecars are missing\n%s",
			out,
		)
	}
...
func copyReleaseV10ReproScripts(t *testing.T, root string) {
	...
	if err := copyFile(
		filepath.Join(repoRoot(t), "scripts", "release", "v1_0", "reproducible-build.sh"),
		filepath.Join(root, "scripts", "release", "v1_0", "reproducible-build.sh"),
		0o755,
	); err != nil {
		t.Fatal(err)
	}
...
func runReleaseV10Repro(t *testing.T, root string, args ...string) ([]byte, error) {
	...
	cmd := exec.Command(
		"bash",
		append([]string{"scripts/release/v1_0/reproducible-build.sh"}, args...)...)
	cmd.Dir = root
