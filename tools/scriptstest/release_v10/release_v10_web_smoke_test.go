...
func TestReleaseV10WebSmokeWorkflowLivesInVersionedReleaseScript(t *testing.T) {
	...
	assertLegacyFileRemoved(
		t,
		"scripts/release_v1_0_web_smoke.sh",
...
func TestReleaseV10WebSmokeScriptValidatesReportBeforeExit(t *testing.T) {
	raw, err := os.ReadFile(
		filepath.Join(repoRoot(t), "scripts", "release", "v1_0", "web-smoke.sh"),
	)
	if err != nil {
...
func Test_release_v1_0_web_smokeRejectsDirectoryReportBeforeSmokeOrBrowserSideEffects(
	t *testing.T,
) {
	root := releaseV10WebSmokeFakeRepo(t)
	...
	if !strings.Contains(
		string(out),
		"release/v1_0/web-smoke: refusing to use directory report path: "+reportPath,
	) {
	...
	if _, err := os.Stat(filepath.Join(root, "tetra-build.log")); !os.IsNotExist(err) {
		t.Fatalf(
			"directory report path should block before build/browser side effects, stat err = %v",
			err,
		)
	}
}
...
func Test_release_v1_0_web_smokeRejectsSymlinkReportBeforeSmokeOrBrowserSideEffects(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "tetra-build.log")); !os.IsNotExist(err) {
		t.Fatalf(
			"symlink report path should block before build/browser side effects, stat err = %v",
			err,
		)
	}
}
...
func Test_release_v1_0_web_smokeDiscoversGoogleChromeFallback(t *testing.T) {
	...
	if report.Automation != ("google-chrome --headless --no-sandbox --disable-gpu --disable-" +
		"dev-shm-usage --disable-crash-reporter --disable-breakpad --dump-dom") {
...
func Test_release_v1_0_web_smokeBrowserArgOverridesDiscovery(t *testing.T) {
	...
	report := readWebSmokeReport(t, reportPath)
	if report.Automation != ("custom-chrome --headless --no-sandbox --disable-gpu --disable-" +
		"dev-shm-usage --disable-crash-reporter --disable-breakpad --dump-dom") {
...
func Test_release_v1_0_web_smokeMissingExplicitBrowserWritesBlockedReport(t *testing.T) {
	...
	if report.Automation != ("missing-chrome --headless --no-sandbox --disable-gpu --disable-" +
		"dev-shm-usage --disable-crash-reporter --disable-breakpad --dump-dom") {
...
func Test_release_v1_0_web_smokeRejectsMissingPythonBeforeBrowserPhase(t *testing.T) {
	...
	sourcePath := filepath.Join("examples", "ui", "ui_web_smoke.tetra")
	if err := os.MkdirAll(filepath.Join(root, "examples", "ui"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(
		filepath.Join(root, sourcePath),
		[]byte("func main() -> Int:\n    return 0\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	...
	if !strings.Contains(report.Blocker, "python3") ||
		!strings.Contains(report.Blocker, "http.server") {
	...
	for _, suffix := range []string{".dom.html", ".chromium.err"} {
		if _, err := os.Stat(strings.TrimSuffix(reportPath, ".json") + suffix); !os.IsNotExist(
			err,
		) {
...
if [ "${1:-}" = "build" ]; then
  ...
  cat > "$out.mjs" <<'JS'
export async function runTetra() {
  return 0;
}

export async function instantiateTetra() {
  return { instance: { exports: { tetra_main() {} } } };
}
JS
  case "$out" in
    *web_smoke)
      printf '%s\n' '{"schema":"tetra.ui.v0.4.0","views":[]}' > "$out.ui.json"
      cat > "$out.ui.web.mjs" <<'JS'
export async function mountTetraUI() {
  return { schema: 'tetra.ui.v0.4.0', views: [] };
}
JS
      ;;
  esac
