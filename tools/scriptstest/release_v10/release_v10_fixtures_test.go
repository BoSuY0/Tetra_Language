...
func releaseV10GateFakeRepo(t *testing.T) string {
	...
	if err := os.WriteFile(
		filepath.Join(root, "examples", "flow", "flow_hello.tetra"),
		[]byte("func main() -> Int:\n    return 0\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	...
	if err := os.WriteFile(
		filepath.Join(root, "examples", "ui", "ui_web_smoke.tetra"),
		[]byte("func main() -> Int:\n    return 0\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	...
	if err := os.WriteFile(
		filepath.Join(root, "docs", "generated", "v1_0", "performance-regression.json"),
		[]byte("{\"status\":\"pass\"}\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	...
	if err := os.WriteFile(
		filepath.Join(root, "docs", "baselines", "api-diff-baseline.v1alpha1.json"),
		[]byte("{}\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	...
	if err := os.WriteFile(
		filepath.Join(root, "docs", "release", "v1_0", "v1_0_final_handoff.md"),
		[]byte("# Tetra v1.0 Final Handoff\n\nStatus: ready for lint fixture.\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	}
...
case "${1:-}" in
  ls-files)
    printf 'examples/flow/flow_hello.tetra\nexamples/ui/ui_web_smoke.tetra\n'
    ;;
  status|diff)
    exit 0
    ;;
...
func releaseV10WASISmokeFakeRepo(t *testing.T) string {
	...
	if err := os.WriteFile(
		filepath.Join(root, "examples", "projects", "dogfood_wasi", "src", "main.tetra"),
		[]byte("func main() -> Int:\n    return 0\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
...
case "$cmd" in
  smoke)
    ...
    for arg in "$@"; do
      if [[ "$arg" == "--list" ]]; then
        printf '{"target":"wasm32-wasi","cases":[{"name":"dogfood_wasi","src_path":"examples/projects/dogfood_wasi/src/main.tetra"},{"name":"ui_web_smoke","src_path":"examples/ui/ui_web_smoke.tetra"}]}\n'
        exit 0
      fi
    done
    report=""
    ...
    if [[ "$list_mode" == "true" ]]; then
      printf '{"target":"wasm32-wasi","build_only":true,"run_supported":true,"total":5,"islands_debug":false,"cases":[{"name":"legacy_hello","src_path":"examples/smoke/basic/hello.tetra","target_group":"wasm","expected_exit":0},{"name":"effects_io_smoke","src_path":"examples/effects/effects_io_smoke.tetra","target_group":"wasm","expected_exit":0},{"name":"ui_web_smoke","src_path":"examples/ui/ui_web_smoke.tetra","target_group":"wasm","expected_exit":0},{"name":"dogfood_wasi","src_path":"examples/projects/dogfood_wasi/src/main.tetra","target_group":"wasm","expected_exit":0},{"name":"dogfood_web_ui","src_path":"examples/projects/dogfood_web_ui/src/main.tetra","target_group":"wasm","expected_exit":0}]}\n'
      exit 0
    fi
    if [[ -z "$report" ]]; then
  ...
  build)
    ...
    printf 'wasm:%s\n' "$src" >"$out"
    if [[ "$src" == "examples/ui/ui_web_smoke.tetra" ]]; then
...
func releaseV10WebSmokeFakeRepo(t *testing.T) string {
	t.Helper()
	root := t.TempDir()
	for _, dir := range []string{"bin", "examples/flow", "scripts/release/v1_0"} {
		if err := os.MkdirAll(filepath.Join(root, filepath.FromSlash(dir)), 0o755); err != nil {
			t.Fatal(err)
		}
	}
...
if [ "${1:-}" = "smoke" ]; then
  ...
  if [ "$list_mode" = "true" ]; then
    printf '%s\n' '{"target":"wasm32-web","build_only":true,"run_supported":false,"total":5,"islands_debug":false,"cases":[{"name":"legacy_hello","src_path":"examples/smoke/basic/hello.tetra","target_group":"wasm","expected_exit":0},{"name":"effects_io_smoke","src_path":"examples/effects/effects_io_smoke.tetra","target_group":"wasm","expected_exit":0},{"name":"ui_web_smoke","src_path":"examples/ui/ui_web_smoke.tetra","target_group":"wasm","expected_exit":0},{"name":"dogfood_wasi","src_path":"examples/projects/dogfood_wasi/src/main.tetra","target_group":"wasm","expected_exit":0},{"name":"dogfood_web_ui","src_path":"examples/projects/dogfood_web_ui/src/main.tetra","target_group":"wasm","expected_exit":0}]}'
    exit 0
  fi
fi
if [ "${1:-}" = "build" ]; then
