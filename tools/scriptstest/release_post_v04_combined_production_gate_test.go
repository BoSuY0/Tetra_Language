...
func TestReleasePostV04CombinedProductionGateRunsOrderedLayerGates(t *testing.T) {
	root := repoRoot(t)
	path := filepath.Join(root, "scripts", "release", "post_v0_4", "memory-parallel-ui-production-linux-x64-gate.sh")
	...
	if strings.Index(text, "parallel-production-linux-x64-smoke.sh") > strings.Index(text, "ui-production-runtime-linux-x64-smoke.sh") {
		t.Fatalf("combined production gate must run parallelism before UI")
	}
	if strings.Index(text, "validate-memory-production") > strings.Index(text, "validate-parallel-production") {
		t.Fatalf("combined production gate must validate memory before parallelism")
	}
	if strings.Index(text, "validate-parallel-production") > strings.Index(text, "validate-ui-production-runtime") {
