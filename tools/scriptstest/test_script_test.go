...
func TestCanonicalTestScriptHasFrontendFocusedTarget(t *testing.T) {
	...
	if err := copyFile(filepath.Join(repoRoot(t), "scripts", "ci", "test.sh"), filepath.Join(root, "scripts", "ci", "test.sh"), 0o755); err != nil {
		t.Fatal(err)
	}
...
`), 0o755); err != nil {
	...
	for _, want := range []string{
		"test ./compiler/internal/frontend ./compiler -run Lex|Parser|Flow|Diagnostic|Stabilization -count=1",
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(log, forbidden) {
			t.Fatalf("frontend-focused target should not run canonical full suite command %q; go calls:\n%s", forbidden, log)
		}
	}
...
func Test_cheatsheet_docs_example_eco_pack_unpack(t *testing.T) {
	root := repoRoot(t)
	commands := cheatsheetShellCommandsInSection(t, filepath.Join(root, "docs", "user", "cli_cheatsheet.md"), "## Eco And Artifacts")
	...
	if strings.Join(snippet, "\n") != strings.Join(want, "\n") {
		t.Fatalf("cheatsheet eco pack/unpack docs example is not the validated command form\nwant:\n%s\ngot:\n%s", strings.Join(want, "\n"), strings.Join(snippet, "\n"))
	}
...
func Test_stdlib_standard_library_guide_tetra_doc_examples_invariant(t *testing.T) {
	root := repoRoot(t)
	guidePath := filepath.Join(root, "docs", "user", "standard_library_guide.md")
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(text, forbidden) {
			t.Fatalf("standard_library_guide contains outdated slices/strings wording %q", forbidden)
		}
	}
	...
	for _, want := range []string{
		...
		if !strings.Contains(workflow, want) {
			t.Fatalf("standard_library_guide verification workflow missing %q in commands:\n%s", want, workflow)
		}
	}
	stdlibDocs := workflow[:strings.Index(workflow, `go run ./tools/cmd/validate-api-docs --docs reports/stdlib-api-docs.md`)]
	if strings.Contains(stdlibDocs, "./tetra doc examples") || !strings.Contains(stdlibDocs, "lib/core") || !strings.Contains(stdlibDocs, "lib/experimental") {
		t.Fatalf("standard_library_guide stdlib docs workflow must document lib/core and lib/experimental before examples-only docs:\n%s", workflow)
	}
}

func Test_core_capability_smoke_examples_index_invariant(t *testing.T) {
	root := repoRoot(t)
	examplePath := filepath.Join(root, "examples", "core_capability_smoke.tetra")
	...
	for _, want := range []string{
		"module examples.core_capability_smoke",
	...
	rawIndex, err := os.ReadFile(filepath.Join(root, "docs", "user", "examples_index.md"))
	if err != nil {
...
func Test_examples_index_has_rows_for_every_core_smoke(t *testing.T) {
	root := repoRoot(t)
	coreSmokes, err := filepath.Glob(filepath.Join(root, "examples", "core_*_smoke.tetra"))
	if err != nil {
