...
func TestReleaseV030GateUsesDedicatedV030Boundary(t *testing.T) {
	root := repoRoot(t)
	assertLegacyFileRemoved(t, "scripts/release_v0_3_0_gate.sh", "scripts/release/v0_3_0/gate.sh directly")
...
func TestReleaseV030ChecklistIsNonClaimingAndVersionScoped(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join(repoRoot(t), "docs", "checklists", "v0_3_0_release_gate.md"))
	if err != nil {
...
func TestReleaseV030ChecklistAndGateRequireSecuritySignoff(t *testing.T) {
	checklistRaw, err := os.ReadFile(filepath.Join(repoRoot(t), "docs", "checklists", "v0_3_0_release_gate.md"))
	if err != nil {
		t.Fatalf("read v0.3.0 checklist: %v", err)
	}
	...
	for _, want := range []string{
		...
		"final source of truth for signoff validity is `bash scripts/release/v0_3_0/security-review.sh --signoff <report-dir>/artifacts/security-review.md`",
	} {
...
func TestReleaseV030GateGoTestStepUnsetsReleaseInputEnv(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(gate, want) {
			t.Fatalf("v0.3.0 release gate go test step should unset release input env; missing %q", want)
		}
	}
}
...
func TestReleaseV030SecurityReviewWrapperUsesV030Name(t *testing.T) {
	root := repoRoot(t)
	assertLegacyFileRemoved(t, "scripts/release_v0_3_0_security_review.sh", "scripts/release/v0_3_0/security-review.sh directly")
	raw, err := os.ReadFile(filepath.Join(root, "scripts", "release", "v0_3_0", "security-review.sh"))
	if err != nil {
	...
	assertNoLegacyMention(t, text, "scripts/release_v0_3_0_security_review.sh", "scripts/release/v0_3_0/security-review.sh")
}
