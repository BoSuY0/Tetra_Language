...
func TestReleaseSurfaceGateRunsAllSurfaceEvidenceSlices(t *testing.T) {
	...
	for _, want := range []string{
		...
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-linux-x64-real-window.json\""),
		"go run ./tools/cmd/validate-surface-runtime --report \"$report_dir/surface-wasm32-web.json\"",
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-wasm32-web-browser-canvas.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-headless-text-focus-input.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-linux-x64-real-window-text-focus-input.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-wasm32-web-browser-canvas-text-focus-input.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-headless-component-tree.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-linux-x64-real-window-component-tree.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-wasm32-web-browser-canvas-component-tree.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-headless-component-tree-api.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-linux-x64-real-window-component-tree-api.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-wasm32-web-browser-canvas-component-tree-api.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-headless-minimal-toolkit.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-linux-x64-real-window-minimal-toolkit.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-wasm32-web-browser-canvas-minimal-toolkit.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-headless-toolkit-reuse.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-linux-x64-real-window-toolkit-reuse.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-wasm32-web-browser-canvas-toolkit-reuse.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-headless-accessibility-metadata.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-linux-x64-real-window-accessibility-metadata.json\""),
		("go run ./tools/cmd/validate-surface-runtime --report \"$report_" +
			"dir/surface-wasm32-web-browser-canvas-accessibility-metadata.json\""),
		("go run ./tools/cmd/validate-artifact-hashes --write --root " +
...
func TestReleaseSurfaceFinalReleaseGateRunsCurrentSurfaceV1Evidence(t *testing.T) {
	...
	assertEqualOrderedStrings(
		t,
		parseBashStringArray(t, gate, "required_reports"),
		reportPaths,
		"release-gate required_reports",
	)
	...
	for _, want := range []string{
		...
		("Surface v1 release gate must fail, not skip, when Chromium-" +
			"compatible browser, Linux Wayland/display, accessibility probe, or " +
			"clipboard harness evidence is unavailable."),
	} {
		if !strings.Contains(gate, want) {
	...
	hashWrite := strings.Index(
		gate,
		("go run ./tools/cmd/validate-artifact-hashes --write --root " +
			"\"$report_dir\" --out \"$report_dir/artifact-hashes.json\""),
	)
	stateValidate := strings.Index(
		gate,
		("go run ./tools/cmd/validate-surface-release-state --report-dir " +
			"\"$report_dir\" --expected-status current --scope surface-v1-linux-web --" +
			"manifest docs/generated/manifest.json"),
	)
	if hashWrite < 0 || stateValidate < 0 {
		t.Fatalf(
			"Surface final release gate must include artifact hash write and release-state validation",
		)
	}
	if hashWrite > stateValidate {
		t.Fatalf(
			("Surface final release gate must write artifact-hashes.json " +
				"before validate-surface-release-state reads it"),
		)
	}
	delegationBlock := strings.Index(
		gate,
		"go run ./tools/cmd/run-gate --contract \"$gate_contract\" --report-dir \"$report_dir_arg\"\n  exit $?\nfi",
	)
	guardedContractIDCheck := strings.Index(
		gate,
		`if [[ "${TETRA_RUN_GATE_CONTRACT_ID:-}" != "$gate_contract_id" ]]; then`,
	)
	contractPreflight := strings.Index(
		gate,
		`go run ./tools/cmd/run-gate --contract "$gate_contract" --report-dir "$report_dir_arg" --dry-run >/dev/null`,
	)
	freshReportGuard := strings.Index(
		gate,
		`surface_release_require_fresh_report_dir "$report_dir" "$repo_root" "surface_release_gate:"`,
	)
	if delegationBlock < 0 || guardedContractIDCheck < 0 || contractPreflight < 0 ||
		freshReportGuard < 0 {
		t.Fatalf(
			("Surface final release gate must include non-dry-run delegation, " +
				"guarded contract check, contract preflight, and fresh report-dir guard"),
		)
	}
	if strings.Count(gate, "--dry-run") != 1 {
		t.Fatalf(
			"Surface final release gate must use dry-run exactly once inside guarded execution",
		)
	}
	if strings.Count(
		gate,
		...
		t.Fatalf(
			("Surface final release gate must call run-gate once for non-dry-" +
				"run delegation and once for guarded dry-run validation"),
		)
	}
	if contractPreflight < freshReportGuard {
		t.Fatalf(
			("Surface final release gate must reject unsafe report " +
				"directories before guarded contract preflight"),
		)
	}
	...
	firstEvidenceStep := strings.Index(
		gate,
	...
	if delegationBlock > firstEvidenceStep {
		t.Fatalf(
			"Surface final release gate must delegate normal execution before release evidence steps",
		)
	}
	if contractPreflight > firstEvidenceStep {
		t.Fatalf(
			("Surface final release gate must validate the contract inside " +
				"guarded execution before release evidence steps"),
		)
	}
}
...
func ciArtifactPaths(t *testing.T, contract gatecontract.Contract) []string {
	...
	for _, artifact := range contract.CIArtifacts {
		if !artifact.Required {
			t.Fatalf(
				"Surface release contract ci_artifacts entry %q must be required",
				artifact.Path,
			)
		}
...
func assertSurfaceReleaseContractValidators(
	t *testing.T,
	gate string,
	...
	for _, id := range ids {
		...
		if !strings.Contains(gate, command) {
			t.Fatalf(
				"Surface final release gate missing command for contract validator %q: %q",
				id,
				command,
			)
		}
...
func surfaceReleaseValidatorsByID(
	t *testing.T,
	contract gatecontract.Contract,
) map[string]gatecontract.Validator {
	t.Helper()
...
func TestReleaseSurfaceTemplateSmokeAndGateWiring(t *testing.T) {
	...
	for _, want := range []string{
		...
		("--golden-artifact \"examples/surface/block_core/surface_block_" +
			"system.tetra,headless,1,$golden_runtime_dir/surface-headless-block-" +
			"system-artifacts/surface-block-system-frame-order-1-initial.rgba\""),
		("--golden-artifact \"examples/surface/block_core/surface_block_" +
			"system.tetra,headless,2,$golden_runtime_dir/surface-headless-block-" +
			"system-artifacts/surface-block-system-frame-order-2-focused.rgba\""),
		("--golden-artifact \"examples/surface/block_core/surface_block_" +
			"system.tetra,headless,3,$golden_runtime_dir/surface-headless-block-" +
			"system-artifacts/surface-block-system-frame-order-3-motion.rgba\""),
		"tar -czf",
	...
	gateRaw, err := os.ReadFile(
		filepath.Join(root, "scripts", "release", "surface", "release-gate.sh"),
	)
	if err != nil {
		t.Fatalf("read Surface release gate script: %v", err)
	}
	...
	for _, want := range []string{
		...
		("go run ./tools/cmd/validate-surface-template-smoke --report " +
			"\"$report_dir/surface-template-smoke.json\""),
	} {
		if !strings.Contains(gate, want) {
...
func TestReleaseSurfaceReferenceAppsSmokeAndGateWiring(t *testing.T) {
	...
	for _, want := range []string{
		...
		("go run ./tools/cmd/validate-surface-reference-apps --report " +
...
func TestReleaseSurfacePackageSmokeAndGateWiring(t *testing.T) {
	...
	script := string(raw)
	for _, want := range []string{
		("Usage: bash scripts/release/surface/surface-package-smoke.sh [--" +
			"report-dir DIR] [--source PATH] [--app-id ID] [--app-title TITLE] [--" +
...
func TestSurfaceBrowserCanvasHostKeepsStudioShellFullFrame(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(section, forbidden) {
			t.Fatalf(
				"studio-shell browser input must not use cropped counter behavior %q:\n%s",
				forbidden,
				section,
			)
		}
...
func TestReleaseSurfaceWidgetMigrationSmokeAndGateWiring(t *testing.T) {
	...
	for _, want := range []string{
		...
		("go run ./tools/cmd/validate-surface-widget-migration --report " +
			"\"$report_dir/surface-widget-migration.json\""),
	} {
		if !strings.Contains(gate, want) {
...
func TestReleaseSurfaceCrashReportSmokeAndGateWiring(t *testing.T) {
	...
	for _, want := range []string{
		...
		("go run ./tools/cmd/validate-surface-crash-report --report " +
			"\"$report_dir/surface-crash-report.json\""),
	} {
		if !strings.Contains(gate, want) {
...
func TestReleaseSurfaceDevWorkflowSmokeAndGateWiring(t *testing.T) {
	...
	for _, want := range []string{
		...
		("go run ./tools/cmd/validate-surface-dev-workflow --report " +
			"\"$report_dir/surface-dev-workflow.json\""),
	} {
		if !strings.Contains(gate, want) {
...
func TestReleaseSurfaceLinuxAppShellSmokeAndGateWiring(t *testing.T) {
	...
	for _, want := range []string{
		("Usage: bash scripts/release/surface/surface-linux-x64-release-" +
			"app-shell-smoke.sh [--report-dir DIR]"),
	...
	for _, want := range []string{
		...
		("go run ./tools/cmd/validate-surface-security-report --report " +
			"\"$report_dir/surface-linux-x64-release-app-shell.json\""),
		("go run ./tools/cmd/validate-surface-performance-budget --report " +
			"\"$report_dir/surface-linux-x64-release-app-shell.json\""),
	} {
		if !strings.Contains(gate, want) {
