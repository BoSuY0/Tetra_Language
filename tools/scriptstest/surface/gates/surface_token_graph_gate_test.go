...
func TestSurfaceMorphGateRunsTokenGraphContractValidator(t *testing.T) {
	...
	if contract.Producer != "scripts/release/surface/morph-gate.sh" ||
		contract.Entrypoint != "scripts/release/surface/morph-gate.sh" {
		t.Fatalf(
			"Surface Morph gate contract producer/entrypoint = %q/%q",
			contract.Producer,
			contract.Entrypoint,
		)
	}
	if contract.ArtifactHashes == nil || !contract.ArtifactHashes.Enabled ||
		...
		t.Fatalf(
			"Surface Morph gate artifact_hashes = %#v, want enabled required sha256",
	...
	for _, want := range []string{
		...
		`"token_graph_contract": "docs/spec/surface/surface_token_graph_contract.json"`,
	...
	assertEqualOrderedStrings(
		t,
		parseBashStringArray(t, text, "final_required_reports"),
		wantReports,
		"Surface Morph final_required_reports",
	)
	assertOrderedFragments(
		...
		`validate-surface-token-graph --contract docs/spec/surface/surface_token_graph_contract.json`,
...
func TestSurfaceMorphGatePublishesRecipeAuthoringEvidence(t *testing.T) {
	...
	for _, wantClaim := range []string{"morph_recipe_authoring", "surface_token_graph"} {
		if !containsString(report.ClaimRefs, wantClaim) {
			t.Fatalf(
				"headless Surface Morph report claim_refs = %#v, want %q",
				report.ClaimRefs,
				wantClaim,
			)
		}
...
func assertSurfaceMorphRequiredReports(
	t *testing.T,
	...
	for _, report := range contract.RequiredReports {
		...
		if !report.SameCommitRequired {
			t.Fatalf(
				"Surface Morph required report %q same_commit_required = false, want true",
				report.Path,
			)
		}
		if !report.ArtifactHashRequired {
			t.Fatalf(
				"Surface Morph required report %q artifact_hash_required = false, want true",
				report.Path,
			)
		}
	...
	for path, wantSchema := range wantSchemas {
		...
		if report.Schema != wantSchema {
			t.Fatalf(
				"Surface Morph required report %q schema = %q, want %q",
				path,
				report.Schema,
				wantSchema,
			)
		...
		if report.Validator != wantValidators[path] {
			t.Fatalf(
				"Surface Morph required report %q validator = %q, want %q",
				path,
				report.Validator,
				wantValidators[path],
			)
...
func assertSurfaceMorphCIArtifacts(t *testing.T, contract gatecontract.Contract, want []string) {
	...
	for _, artifact := range contract.CIArtifacts {
		if !artifact.Required {
			t.Fatalf(
				"Surface Morph ci_artifacts entry %q required = false, want true",
				artifact.Path,
			)
		}
...
func assertSurfaceMorphValidators(
	t *testing.T,
	gate string,
	contract gatecontract.Contract,
	ids ...string,
	...
	for _, id := range ids {
		...
		if !strings.Contains(gate, command) {
			t.Fatalf(
				"Surface Morph gate missing command for contract validator %q: %q",
				id,
				command,
			)
		}
...
func surfaceMorphRequiredReport(
	t *testing.T,
	contract gatecontract.Contract,
	path string,
) gatecontract.RequiredReport {
	t.Helper()
	for _, report := range contract.RequiredReports {
		if report.Path == path {
			return report
