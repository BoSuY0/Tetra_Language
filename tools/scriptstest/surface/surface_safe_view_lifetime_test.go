...
func TestSurfaceSafeViewLifetimeGateIsBoundedAndFocused(t *testing.T) {
	root := repoRoot(t)
	raw, err := os.ReadFile(
		filepath.Join(root, "scripts", "release", "safe-view-lifetime", "gate.sh"),
	)
	if err != nil {
	...
	for _, want := range []string{
		...
		"safe-view-step-",
		("SurfaceClose|FrameAfterClose|DoubleClose|BeginPresent|ResizeAfte" +
			"rClose|ResourceCleanup|BrowserProcessCleanup|SafeViewLifetime"),
