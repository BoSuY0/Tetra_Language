...
func TestReleaseV10WASISmokeWorkflowLivesInVersionedReleaseScript(t *testing.T) {
	...
	assertLegacyFileRemoved(t, "scripts/release_v1_0_wasi_smoke.sh", "scripts/release/v1_0/wasi-smoke.sh directly")
...
func TestReleaseV10WASISmokeUsesUnifiedCLIRuntimePath(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(text, forbidden) {
			t.Fatalf("wasi smoke script still contains legacy runtime wrapper contract %q", forbidden)
		}
	}
}
...
func TestReleaseV10WASISmokeRejectsDirectoryReportBeforeSmokeSideEffects(t *testing.T) {
	...
	if !strings.Contains(string(out), "release/v1_0/wasi-smoke: refusing to use directory report path: "+reportPath) {
...
func TestReleaseV10WASISmokeRunsUnifiedCLIAndValidatesReport(t *testing.T) {
	...
	if !strings.Contains(string(rawReport), `"runner":"unified-cli"`) {
		t.Fatalf("final report was not produced by unified CLI runtime path:\n%s", string(rawReport))
	}
	...
	if !strings.Contains(string(rawValidatorLog), "smoke-report-to-checklist --validate-only --report "+reportPath) {
		t.Fatalf("final report was not validated:\n%s", string(rawValidatorLog))
	}
	if !strings.Contains(string(rawValidatorLog), "validate-wasi-smoke-report --mode runtime --report "+reportPath) {
		t.Fatalf("final WASI runtime report was not strictly validated:\n%s", string(rawValidatorLog))
	}
}
