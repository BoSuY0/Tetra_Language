...
func TestSurfaceUITruthAuditScriptContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		"SKIPPED",
		"gate-runs",
		"TestReleaseSurface|TestCurrentSupportedSurface|TestCIWorkflowIncludesSurface|TestSurface(Tree|Toolkit|Accessibility)",
	} {
	...
	if strings.Contains(text, "-run 'Surface|surface'") {
		t.Fatalf("Surface UI truth audit script must not recursively run its own artifact-writing tests")
	}
}
	...
	for _, reportDir := range []string{
		...
		t.Run(reportDir, func(t *testing.T) {
			...
			if !strings.Contains(string(out), "refusing unsafe report dir") &&
				...
				t.Fatalf("expected controlled report-dir rejection for %q, got:\n%s", reportDir, out)
			}
