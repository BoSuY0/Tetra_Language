...
`
	...
	if foundSecurityReview {
		t.Fatalf(
			"artifact-hashes.json must exclude artifacts/security-review.md to avoid a stale hash cycle:\n%s",
			hashRaw,
		)
	}
	...
	if releaseState.SecurityReview.Status != "deferred" {
		t.Fatalf(
			"release-state security_review.status = %q, want deferred to avoid the security-review hash cycle\n%s",
			releaseState.SecurityReview.Status,
			releaseStateRaw,
		)
	}
	...
	if !strings.Contains(string(releaseStateText), "security review evidence: deferred") {
		t.Fatalf(
			"release-state.txt missing deferred security review evidence state:\n%s",
			releaseStateText,
		)
	}
}
...
`
	...
	if err != nil {
		t.Fatalf(
			"same-run security signoff should pass with detached hash attestation: %v\n%s",
			err,
			out,
		)
	}
	...
	if string(detachedRaw) != wantDetached {
		t.Fatalf(
			"detached security-review hash mismatch:\nwant %q\ngot  %q",
			wantDetached,
			detachedRaw,
		)
	}
	...
	for _, artifact := range manifest.Artifacts {
		...
		case "artifacts/security-review.md", "artifacts/security-review.md.sha256":
			t.Fatalf(
				"artifact-hashes.json must exclude %s to avoid a security-review hash cycle:\n%s",
				artifact.Path,
				hashRaw,
			)
		}
