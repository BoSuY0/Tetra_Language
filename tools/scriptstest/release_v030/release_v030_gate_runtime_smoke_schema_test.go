...
func TestReleaseV030GateRejectsWrongGitHeadRuntimeSmokeEvidence(t *testing.T) {
	...
	raw = []byte(strings.Replace(string(raw), `"git_head": "fake-head-for-release-v030-test"`, `"git_head": "stale-head"`, 1))
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: git_head is 'stale-head', want 'fake-head-for-release-v030-test'") {
...
func TestReleaseV030GateRejectsRunnerRuntimeSmokeEvidence(t *testing.T) {
	...
	raw = []byte(strings.Replace(string(raw), `"host": "macos-x64",`, `"host": "macos-x64",`+"\n  "+`"runner": "cross-runner",`, 1))
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: runner is 'cross-runner', want empty host-native runtime") {
...
func TestReleaseV030GateRejectsInvalidTimestampRuntimeSmokeEvidence(t *testing.T) {
	...
	raw = []byte(strings.Replace(string(raw), `"timestamp": "2026-04-30T12:00:00Z"`, `"timestamp": "not-a-time"`, 1))
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: timestamp is not RFC3339: 'not-a-time'") {
...
func TestReleaseV030GateRejectsLooseTimestampRuntimeSmokeEvidence(t *testing.T) {
	...
	raw = []byte(strings.Replace(string(raw), `"timestamp": "2026-04-30T12:00:00Z"`, `"timestamp": "2026-04-30 12:00:00+00:00"`, 1))
	...
	if err == nil {
		t.Fatalf("gate should reject runtime smoke evidence with a loose non-RFC3339 timestamp\n%s", out)
	}
	if !strings.Contains(string(out), "runtime smoke evidence invalid: timestamp is not RFC3339: '2026-04-30 12:00:00+00:00'") {
...
func TestReleaseV030GateRejectsMissingRequiredRuntimeSmokeCase(t *testing.T) {
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: missing required runtime case wait_composition_smoke") {
...
func TestReleaseV030GateRejectsRuntimeSmokeCaseErrorText(t *testing.T) {
	...
	raw = []byte(strings.Replace(string(raw), `"name":"actors_pingpong"`, `"name":"actors_pingpong","error":"stale failure text"`, 1))
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: case actors_pingpong has error text") {
...
func TestReleaseV030GateRejectsEmptyRuntimeSmokeCaseName(t *testing.T) {
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: contains a case with empty name") {
...
func TestReleaseV030GateRejectsNonStringRuntimeSmokeCaseName(t *testing.T) {
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: case name must be a string") {
...
func TestReleaseV030GateRejectsNonBooleanRuntimeSmokeCaseStatus(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("gate should reject runtime smoke evidence with non-boolean case status fields\n%s", out)
	}
	if !strings.Contains(string(out), "runtime smoke evidence invalid: case non_boolean_status pass must be a boolean") {
...
func TestReleaseV030GateRejectsNonIntegerRuntimeSmokeCaseExitFields(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("gate should reject runtime smoke evidence with non-integer case exit fields\n%s", out)
	}
	if !strings.Contains(string(out), "runtime smoke evidence invalid: case non_integer_exit expected_exit must be an integer") {
...
func TestReleaseV030GateRejectsNonIntegerRuntimeSmokeCounts(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("gate should reject runtime smoke evidence with non-integer summary counts\n%s", out)
	}
...
func TestReleaseV030GateRejectsNonBooleanRuntimeSmokeIslandsDebug(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("gate should reject runtime smoke evidence with non-boolean islands_debug\n%s", out)
	}
	if !strings.Contains(string(out), "runtime smoke evidence invalid: islands_debug must be a boolean") {
...
func TestReleaseV030GateRejectsNonBooleanRuntimeSmokeBuildOnly(t *testing.T) {
	...
	text := strings.Replace(string(raw), `"host": "macos-x64",`, `"host": "macos-x64",`+"\n  "+`"build_only": 0,`, 1)
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: build_only must be a boolean") {
