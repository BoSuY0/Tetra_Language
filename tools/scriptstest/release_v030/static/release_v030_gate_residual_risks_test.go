...
func TestReleaseV030GateRejectsUntriagedUnstableSeeds(t *testing.T) {
	root := releaseV030RunnableFakeRepo(t, []string{
		("| compiler | FuzzParser | testdata/fuzz/FuzzParser/bad | flaky " +
			"|  | go test ./compiler/... -run FuzzParser -count=1 |"),
	})
...
`
	...
	if _, err := os.Stat(filepath.Join(reportDir, "artifacts", "residual-risks.json")); err != nil {
		t.Fatalf(
			"residual risks artifact was not archived from dash-prefixed source: %v\n%s",
			err,
			out,
		)
	}
...
`
	...
	if err == nil {
		t.Fatalf(
			"expected residual risk with missing required fields to block release gate\n%s",
			out,
		)
	}
	for _, want := range []string{
...
func TestReleaseV030RunnableGateFiltersAmbientResidualRisksEnv(t *testing.T) {
	...
	if envHasPrefix(env, "TETRA_RESIDUAL_RISKS_JSON=") {
		t.Fatalf(
			"filteredReleaseV030GateEnv leaked ambient TETRA_RESIDUAL_RISKS_JSON into release gate test env",
		)
	}
}
