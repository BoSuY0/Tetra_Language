...
func TestReleaseV030GateBlocksFinalSummaryWhenPostSummaryArtifactHashCheckFails(t *testing.T) {
	...
	if summary.Status != "blocked" {
		t.Fatalf(
			"post-summary artifact hash failure left summary status = %q, want blocked\n%s",
			summary.Status,
			summaryRaw,
		)
	}
	...
	if summary.FailedCount != failedSteps || failedSteps == 0 {
		t.Fatalf(
			"post-summary artifact hash failure summary has failed_count=%d but %d failed step(s), want coherent nonzero failure evidence\n%s",
			summary.FailedCount,
			failedSteps,
			summaryRaw,
		)
...
func TestReleaseV030GateBlocksFinalSummaryWhenDetachedSecurityHashFails(t *testing.T) {
	...
	if summary.Status != "blocked" {
		t.Fatalf(
			"detached hash failure left summary status = %q, want blocked\n%s",
			summary.Status,
			summaryRaw,
		)
	}
	...
	if summary.FailedCount != failedSteps || failedSteps == 0 {
		t.Fatalf(
			"detached hash failure summary has failed_count=%d but %d failed step(s), want coherent nonzero failure evidence\n%s",
			summary.FailedCount,
			failedSteps,
			summaryRaw,
		)
...
func TestReleaseV030GateRecordsCIMissingSignoffFinalArtifactHashRefreshFailure(t *testing.T) {
	...
	if err == nil {
		t.Fatalf(
			"expected CI missing-signoff final artifact-hashes refresh failure to block release gate\n%s",
			out,
		)
	}
	if !strings.Contains(string(out), "CI missing-security-signoff artifact hash refresh failed") {
		t.Fatalf(
			"CI missing-signoff final artifact hash failure output missing blocked reason:\n%s",
			out,
		)
	}
	...
	if summary.Status != "blocked" {
		t.Fatalf(
			"CI final artifact hash failure left summary status = %q, want blocked\n%s",
			summary.Status,
			summaryRaw,
		)
	}
	...
	if summary.FailedCount != failedSteps || failedSteps == 0 {
		t.Fatalf(
			"CI final artifact hash failure summary has failed_count=%d but %d failed step(s), want coherent nonzero failure evidence\n%s",
			summary.FailedCount,
			failedSteps,
			summaryRaw,
		)
...
func TestReleaseV030GateCanonicalizesArtifactManifestWithPython3WhenPythonIsUnavailable(
	t *testing.T,
) {
	root := releaseV030RunnableFakeRepo(t, []string{
		("| compiler | FuzzParser | testdata/fuzz/FuzzParser/bad | flaky " +
	...
	if err != nil {
		t.Fatalf(
			"release gate should use python3 when bare python is unavailable: %v\n%s",
			err,
			out,
		)
	}
	if strings.Contains(string(out), "bare python should not be required") {
		t.Fatalf(
			"release gate invoked bare python during artifact manifest canonicalization:\n%s",
			out,
		)
	}
	...
	if !strings.Contains(string(markerRaw), filepath.Join(reportDir, "artifact-hashes.json")) {
		t.Fatalf(
			"python3 canonicalizer marker does not record artifact-hashes.json:\n%s",
			markerRaw,
		)
	}
	...
	for _, artifact := range manifest.Artifacts {
		...
		case "artifacts/security-review.md", "artifacts/security-review.md.sha256":
			t.Fatalf(
				"python3 canonicalizer left cycle-prone artifact %s in manifest:\n%s",
				artifact.Path,
				hashRaw,
			)
		}
...
func TestReleaseV030GateRefreshesReleaseStateAfterFinalSummary(t *testing.T) {
	...
	if releaseState.LastGateEvidence.StepCount != summary.StepCount {
		t.Fatalf(
			"release-state step_count %d contradicts final summary step_count %d",
	...
	if releaseState.LastGateEvidence.FailedCount != summary.FailedCount {
		t.Fatalf(
			"release-state failed_count %d contradicts final summary failed_count %d",
	...
	if !strings.Contains(string(releaseStateText), wantText) {
		t.Fatalf(
			"release-state.txt does not reflect final summary step count %q:\n%s",
			wantText,
			releaseStateText,
		)
	}
