...
func TestReleaseV030GateCIModeAllowsMissingSecuritySignoffWithArtifact(t *testing.T) {
	...
	if strings.Contains(string(out), "missing TETRA_SECURITY_REVIEW_SIGNOFF") {
		t.Fatalf("CI mode should use explicit missing-signoff policy, not missing-env failure:\n%s", out)
	}
	if strings.Contains(string(out), "release_v0_3_0_gate: passed") {
		t.Fatalf("CI mode with missing security signoff must not look like a full evidence pass:\n%s", out)
	}
	if !strings.Contains(string(out), "CI missing security signoff recorded; not a full release evidence pass") {
	...
	if summary.Status != "blocked" {
		t.Fatalf("CI missing-signoff summary status = %q, want blocked\n%s", summary.Status, summaryRaw)
	}
	...
	if releaseState.LastGateEvidence.Status != "blocked" {
		t.Fatalf("CI release-state status = %q, want blocked\n%s", releaseState.LastGateEvidence.Status, releaseStateRaw)
	}
	...
	if !strings.Contains(string(releaseStateText), "last gate evidence: blocked") {
		t.Fatalf("CI release-state.txt does not record blocked gate evidence:\n%s", releaseStateText)
	}
...
func TestReleaseV030GateCIMissingSignoffWritesDetachedHashOutsideCanonicalManifest(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("CI missing-signoff gate should complete as blocked evidence with detached hash: %v\n%s", err, out)
	}
	...
	if string(detachedRaw) != wantDetached {
		t.Fatalf("CI detached security-review hash mismatch:\nwant %q\ngot  %q", wantDetached, detachedRaw)
	}
	...
	for _, artifact := range manifest.Artifacts {
		...
		case "artifacts/security-review.md", "artifacts/security-review.md.sha256":
			t.Fatalf("artifact-hashes.json must exclude %s for blocked CI missing-signoff evidence:\n%s", artifact.Path, hashRaw)
		}
	}
}
...
func TestReleaseV030GateRequiresSecuritySignoffOutsideCIMode(t *testing.T) {
	...
	if !strings.Contains(string(out), "missing TETRA_SECURITY_REVIEW_SIGNOFF=<security-review.md>") {
...
func TestReleaseV030GateRequireCleanRequiresSecuritySignoffEvenInCIMode(t *testing.T) {
	...
	reportDir := filepath.Join(root, "report")
	cmd := exec.Command("bash", "scripts/release/v0_3_0/gate.sh", "--require-clean", "--report-dir", reportDir)
	cmd.Dir = root
	...
	if err == nil {
		t.Fatalf("tag-ready gate should require security signoff even when CI mode env is set\n%s", out)
	}
	if !strings.Contains(string(out), "missing TETRA_SECURITY_REVIEW_SIGNOFF=<security-review.md>") {
