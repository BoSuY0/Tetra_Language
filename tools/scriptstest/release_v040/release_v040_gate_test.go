...
func TestReleaseV040GateUsesDedicatedReadinessPreflight(t *testing.T) {
	...
	assertLegacyFileRemoved(
		t,
		"scripts/release_v0_4_0_gate.sh",
	...
	for _, want := range []string{
		...
		`go run ./cli/cmd/tetra features --format=json`,
		`> "$features_json"`,
		`go run ./cli/cmd/tetra targets --format=json`,
		`> "$targets_json"`,
	...
	if strings.Contains(
		text,
		`--expected-status not-achieved`,
	) {
		t.Fatalf(
			("v0.4.0 release gate should validate the achieved completion " +
				"audit, not the pre-completion audit"),
		)
	}
}
...
func TestReleaseV040GateWritesBlockedSummaryOnReadinessFailure(t *testing.T) {
	...
	if err := copyFile(
		filepath.Join(repoRoot(t), "scripts", "release/v0_4_0/gate.sh"),
		filepath.Join(root, "scripts", "release/v0_4_0/gate.sh"),
		0o755,
	); err != nil {
		t.Fatal(err)
	}
...
`
	...
	if summary.Status != "blocked" || summary.ReleaseVersion != "v0.4.0" ||
		summary.ReleaseArtifact != "tetra.release.v0_4_0.gate-report.v1" ||
		summary.ReleaseGateCommand != "bash scripts/release/v0_4_0/gate.sh" ||
		summary.StepCount != 1 ||
	...
	if step.Name != "readiness preflight" || step.Status != "fail" || step.ExitCode == 0 ||
		step.Log != "logs/01-readiness-preflight.log" {
	...
	if risks.Schema != "tetra.release.residual-risks.v1" || risks.ReleaseVersion != "v0.4.0" ||
		risks.Artifact != "residual-risks.json" {
	...
	if firstRisk.ID != "v0.4.0-readiness-preflight" || firstRisk.Severity != "critical" ||
		firstRisk.Owner == "" ||
		firstRisk.Status != "blocked" ||
		!strings.Contains(firstRisk.Summary, "readiness preflight") ||
		firstRisk.Evidence != "logs/01-readiness-preflight.log" {
		t.Fatalf("unexpected blocked readiness risk: %#v\n%s", firstRisk, riskRaw)
	}
	blockersRaw, err := os.ReadFile(
		filepath.Join(reportDir, "artifacts", "readiness-blockers.json"),
	)
	if err != nil {
	...
	if blockers.Schema != "tetra.release.v0_4_0.readiness-blockers.v1" ||
		blockers.ReleaseVersion != "v0.4.0" ||
		blockers.Artifact != "readiness-blockers.json" ||
		blockers.SourceLog != "logs/01-readiness-preflight.log" {
		t.Fatalf("unexpected readiness blockers identity: %#v\n%s", blockers, blockersRaw)
	}
	if len(blockers.Blockers) != 1 || blockers.Blockers[0].ID != "readiness-preflight" ||
		blockers.Blockers[0].Status != "blocked" ||
		!strings.Contains(blockers.Blockers[0].Detail, "fake readiness failure") {
	...
	if !strings.Contains(string(summaryMD), "readiness preflight") ||
		!strings.Contains(string(summaryMD), "blocked") {
...
func TestReleaseV040GateRejectsNonEmptyReportDirBeforeSideEffects(t *testing.T) {
	...
	if err := os.WriteFile(
		filepath.Join(reportDir, "stale-summary.json"),
		[]byte("{}\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
...
func assertReleaseV040RejectsNonDirectoryReportPath(t *testing.T, root, reportDir string) {
	...
	if strings.Contains(string(out), "tetra should not run") ||
		strings.Contains(string(out), "mkdir:") {
		t.Fatalf(
			"gate should reject invalid report dir before raw shell or tetra side effects:\n%s",
			out,
		)
	}
}
