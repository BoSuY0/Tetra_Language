...
func TestReleaseV011GateRejectsNonDirectoryReportPathBeforeSideEffects(t *testing.T) {
	...
	assertOldGateRejectsNonDirectoryReportPath(
		t,
		root,
...
func TestReleaseV011GateRejectsNonEmptyReportDirBeforeSideEffects(t *testing.T) {
	...
	assertOldGateRejectsNonEmptyReportDir(
		t,
		root,
		"scripts/release/v0_1_1/gate.sh",
		"release_v0_1_1_gate:",
...
func releaseOldGateMinimalRoot(t *testing.T, gateRel string) string {
	...
	if err := copyFile(
		filepath.Join(repoRoot(t), filepath.FromSlash(gateRel)),
		filepath.Join(root, filepath.FromSlash(gateRel)),
		0o755,
	); err != nil {
		t.Fatal(err)
	}
...
func assertOldGateRejectsNonDirectoryReportPath(
	t *testing.T,
	root, gateRel, prefix, reportDir string,
) {
	t.Helper()
...
func assertOldGateRejectsNonEmptyReportDirWithArg(
	t *testing.T,
	root, gateRel, prefix, reportDirArg, reportDirPath string,
) {
	t.Helper()
...
func assertOldGateRejectedBeforeSideEffects(t *testing.T, out []byte) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(string(out), forbidden) {
			t.Fatalf(
				"gate should reject report dir before raw shell or workflow side effects:\n%s",
				out,
			)
		}
	}
