...
func TestReleaseV030GateRejectsBuildOnlyRuntimeSmokeEvidence(t *testing.T) {
	...
	if !strings.Contains(string(out), "runtime smoke evidence invalid: case actors_pingpong did not run") {
		t.Fatalf("gate did not report strict runtime evidence failure:\n%s", out)
	}
	if _, statErr := os.Stat(filepath.Join(reportDir, "artifacts", "macos-runtime-smoke.json")); !errors.Is(statErr, os.ErrNotExist) {
		t.Fatalf("invalid macOS runtime smoke report should not be archived as release evidence, stat err=%v", statErr)
	}
}
...
func TestReleaseV030GateRejectsWrongVersionRuntimeSmokeEvidence(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("gate should reject runtime smoke evidence from the wrong release version\n%s", out)
	}
	if !strings.Contains(string(out), "runtime smoke evidence invalid: version is 'v0.2.0', want 'v0.3.0'") {
...
func TestReleaseV030GateDoesNotArchivePartialRuntimeEvidenceWhenWindowsReportInvalid(t *testing.T) {
	...
	for _, artifact := range []string{"macos-runtime-smoke.json", "windows-runtime-smoke.json"} {
		if _, statErr := os.Stat(filepath.Join(reportDir, "artifacts", artifact)); !errors.Is(statErr, os.ErrNotExist) {
			t.Fatalf("partial runtime smoke report %s should not be archived after pair validation failure, stat err=%v", artifact, statErr)
		}
	}
}
...
if [[ "${1:-}" == ` + shellSingleQuote(windowsReport) + ` && "${2:-}" == *windows-runtime-smoke.json ]]; then
...
`
	...
	for _, artifact := range []string{"macos-runtime-smoke.json", "windows-runtime-smoke.json"} {
		if _, statErr := os.Stat(filepath.Join(reportDir, "artifacts", artifact)); !errors.Is(statErr, os.ErrNotExist) {
			t.Fatalf("partial runtime smoke report %s should not be archived after copy failure, stat err=%v\n%s", artifact, statErr, out)
		}
	}
}
...
func TestReleaseV030GateAcceptsRuntimeSmokeSourcePathStartingWithDash(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(reportDir, "artifacts", "macos-runtime-smoke.json")); err != nil {
		t.Fatalf("macOS runtime smoke artifact was not archived from dash-prefixed source: %v\n%s", err, out)
	}
}
