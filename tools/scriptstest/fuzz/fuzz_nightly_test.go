...
func TestFuzzNightlyDocsNameWrapperAndCrashers(t *testing.T) {
	raw, err := os.ReadFile(
		filepath.Join(repoRoot(t), "docs", "testing", "fuzz_property_stress.md"),
	)
	if err != nil {
...
`), 0o755); err != nil {
	...
	fixtures := map[string]string{
		("compiler/internal/frontend/testdata/fuzz/FuzzLexer/corpus-a/" +
			"seed-2"): "go test fuzz v1\n[]byte(\"b\")\n",
		("compiler/internal/frontend/testdata/fuzz/FuzzLexer/crashers/" +
			"crasher-1"): "go test fuzz v1\n[]byte(\"boom\")\n",
		("compiler/internal/frontend/testdata/fuzz/FuzzParser/corpus-b/" +
			"seed-1"): "go test fuzz v1\n[]byte(\"parse\")\n",
		("cli/cmd/tetra/testdata/fuzz/FuzzParseCapsuleDoesNotPanic/corpus/" +
			"seed-1"): "go test fuzz v1\n[]byte(\"capsule\")\n",
		("cli/cmd/tetra/testdata/fuzz/FuzzParseCapsuleDoesNotPanic/corpus/" +
			"not-counted.tmp~"): "scratch\n",
		("cli/cmd/tetra/testdata/fuzz/FuzzParseCapsuleDoesNotPanic/corpus/" +
			".not-counted-hidden"): "scratch\n",
		("compiler/internal/frontend/testdata/fuzz/FuzzLexer/crashers/" +
			".not-counted-hidden-crash"): "scratch\n",
		("compiler/internal/frontend/testdata/fuzz/FuzzLexer/crashers/not-" +
			"counted-crash.tmp~"): "scratch\n",
	}
	...
	cmd := exec.Command(
		"bash",
		filepath.Join(repoRoot(t), "scripts", "dev", "fuzz-nightly.sh"),
		"--short",
		"--out-dir",
		outDir,
	)
	...
	if len(inventory.ScannedRoots) != 8 || inventory.Counts.Roots != 8 ||
		inventory.Counts.Existing != 2 {
		t.Fatalf(
			"unexpected root counts: counts=%+v roots=%+v",
			inventory.Counts,
			inventory.ScannedRoots,
		)
	}
	if inventory.Counts.Targets != 3 || inventory.Counts.CorpusFiles != 3 ||
		inventory.Counts.CrasherFiles != 1 ||
		inventory.Counts.TotalFiles != 4 {
...
`), 0o755); err != nil {
	...
	if summary.Mode != "short" || summary.Status != "pass" || summary.ExitCode != 0 ||
		summary.Fuzztime != "2s" {
		t.Fatalf("unexpected top-level summary fields: %+v", summary)
	}
	if summary.StepCount != 11 || summary.FailedCount != 0 ||
		len(summary.Steps) != summary.StepCount {
		t.Fatalf(
			"unexpected summary counts: step_count=%d failed_count=%d len=%d",
			summary.StepCount,
			summary.FailedCount,
			len(summary.Steps),
		)
...
`), 0o755); err != nil {
	...
	if err := json.Unmarshal(raw, &summary); err != nil {
		t.Fatalf(
			"summary.json with tab/CR paths and strings must remain valid JSON: %v\n%s",
			err,
			raw,
		)
	}
	...
	if summary.Artifacts.SummaryJSON != filepath.Join(outDir, "summary.json") {
		t.Fatalf(
			"summary_json path did not round-trip control characters: got %q want %q",
	...
	if len(summary.Steps) != summary.StepCount {
		t.Fatalf(
			"unexpected summary step count: step_count=%d len=%d",
			summary.StepCount,
			len(summary.Steps),
		)
	}
...
`), 0o755); err != nil {
	...
	for _, step := range summary.Steps {
		...
		if step.Name == "property-stress-regressions" && step.Status == "pass" &&
			step.ExitCode == 0 {
			sawLaterStep = true
		}
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := os.Stat(sentinel); err == nil {
				t.Fatalf(
					"fuzztime was evaluated by a shell and created %s\noutput:\n%s",
					sentinel,
					out,
				)
			} else if !os.IsNotExist(err) {
...
`), 0o755); err != nil {
	...
	if !strings.Contains(string(out), "invalid --fuzztime") ||
		!strings.Contains(string(out), "max 10m") {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			default:
				...
				if err := os.WriteFile(
					filepath.Join(staleDir, "summary.json"),
					[]byte("{}\n"),
					0o644,
				); err != nil {
					t.Fatal(err)
...
`), 0o755); err != nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(string(out), tt.wantMessage) ||
				!strings.Contains(string(out), tt.outDir) {
