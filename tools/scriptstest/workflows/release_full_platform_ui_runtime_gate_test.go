...
func TestReleaseFullPlatformUIRuntimeGateRunsMandatoryEvidence(t *testing.T) {
	...
	for _, want := range []string{
		...
		`bash scripts/release/full_platform/target-host-evidence-request.sh --out-dir "$report_dir/target-host-request"`,
		"target-host-evidence-request-generate",
		`go run ./tools/cmd/validate-target-host-evidence-request --report "$report_dir/target-host-request/target-host-evidence-request.json"`,
		"target-host-evidence-request-validate",
...
func TestReleaseFullPlatformActionsAvailabilityPreflightIsNotRuntimeEvidence(t *testing.T) {
	...
	script := string(raw)
	for _, want := range []string{
		("Usage: bash scripts/release/full_platform/actions-availability-" +
			"preflight.sh [--repo OWNER/REPO] [--branch BRANCH] [--report FILE]"),
...
func TestReleaseFullPlatformGitHubActionsStartupDiagnosticIsBlockedOnly(t *testing.T) {
	...
	script := string(raw)
	for _, want := range []string{
		("Usage: bash scripts/release/full_platform/github-actions-" +
			"startup-diagnostic.sh [--repo OWNER/REPO] [--branch BRANCH] [--report " +
			"FILE] [--canary-branch BRANCH]"),
		"gh run list",
...
func TestReleaseFullPlatformTargetHostHelperDocumentsManualEvidence(t *testing.T) {
	scriptPath := filepath.Join(
		...
		"release",
		"full_platform",
		"target-host-ui-runtime-smoke.sh",
	)
	raw, err := os.ReadFile(scriptPath)
	if err != nil {
	...
	script := string(raw)
	for _, want := range []string{
		("Usage: bash scripts/release/full_platform/target-host-ui-" +
			"runtime-smoke.sh [--target TARGET] [--report FILE]"),
		"windows-x64",
		"macos-x64",
...
func TestReleaseFullPlatformTargetHostEvidenceRequestBundle(t *testing.T) {
	...
	script := string(raw)
	for _, want := range []string{
		("Usage: bash scripts/release/full_platform/target-host-evidence-" +
			"request.sh [--out-dir DIR] [--repo OWNER/REPO] [--branch BRANCH]"),
	...
	cmd := exec.Command(
		...
		outDir,
		"--repo",
		"BoSuY0/Tetra_Language",
		"--branch",
		"codex/full-platform-ui-runtime",
	)
	cmd.Dir = repoRoot(t)
	...
	if err != nil {
		t.Fatalf(
			"target-host evidence request helper with inferred repo failed: %v\n%s",
			err,
			inferredOutput,
		)
	}
	inferredRaw, err := os.ReadFile(
		filepath.Join(inferredOutDir, "target-host-evidence-request.json"),
	)
	if err != nil {
...
func TestReleaseFullPlatformWindowsPowerShellTargetHostHelper(t *testing.T) {
	...
	script := string(raw)
	for _, want := range []string{
		("Usage: pwsh -File scripts/release/full_platform/windows-ui-" +
			"runtime-smoke.ps1 [-Report FILE] [-ReportDir DIR]"),
		"Windows_NT",
		"OSArchitecture",
		"IsPathRooted",
		...
		("go run ./tools/cmd/validate-windows-ui-runtime --report $Report " +
			"--expected-version $ExpectedVersion --expected-git-head $ExpectedGitHead"),
