...
func TestCIWorkflowIncludesStabilizationAndRobustnessJobs(t *testing.T) {
	...
	for _, want := range []string{
		...
		"coverage-linux:",
		("go test ./compiler/... ./cli/... ./tools/... -covermode=atomic -" +
	...
	for _, blocked := range []string{
		...
		if strings.Contains(text, blocked) {
			t.Fatalf(
				"ci workflow must not auto-trigger while GitHub Actions billing is locked; found %q",
				blocked,
			)
		}
	}
...
func TestCIWorkflowIncludesSurfaceReleaseReadinessJob(t *testing.T) {
	...
	for _, want := range []string{
		...
		("bash scripts/release/surface/api-stability-gate.sh --report-dir " +
		...
		("go run ./tools/cmd/validate-artifact-hashes --write --root " +
			"reports/surface-ui-production-final --out reports/surface-ui-production-" +
			"final/artifact-hashes.json"),
		("go run ./tools/cmd/validate-artifact-hashes --manifest reports/" +
			"surface-ui-production-final/artifact-hashes.json"),
		("go run ./tools/cmd/validate-surface-final-readiness --report-" +
			"dir reports/surface-ui-production-final --expected-scope surface-v1-" +
			"linux-web --require-clean --require-package"),
	...
	if strings.Contains(text, "continue-on-error: true") {
		t.Fatalf(
			"Surface release readiness job must not silently continue after missing production dependencies",
		)
	}
}
...
func TestCIWorkflowIncludesSurfaceProductSliceJob(t *testing.T) {
	...
	for _, want := range []string{
		...
		("bash scripts/release/surface/surface-product-slice-gate.sh --" +
			"report-dir reports/surface-product-slice-ci"),
...
func TestCIWorkflowIncludesIntegratedMemoryIslandsSurfaceReadinessJob(t *testing.T) {
	...
	for _, want := range []string{
		...
		("bash scripts/release/post_v0_4/memory-islands-surface-" +
			"production-gate.sh --report-dir reports/memory-islands-surface-" +
			"production"),
	...
	if strings.Contains(text, "continue-on-error: true") {
		t.Fatalf(
			("integrated Memory/Islands/Surface readiness job must not " +
				"silently continue after missing production dependencies"),
		)
	}
}
...
func TestCIWorkflowIncludesMemory100ProdStableGateJob(t *testing.T) {
	...
	for _, want := range []string{
		...
		("bash scripts/release/post_v0_4/memory-100-prod-stable-gate.sh --" +
			"report-dir reports/memory-100/final"),
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(section, forbidden) {
			t.Fatalf(
				"Memory100 CI gate must not contain bypass or tmpfs cache marker %q",
				forbidden,
			)
		}
	}
...
func TestCIWorkflowIncludesActorRuntimeFoundationGateJob(t *testing.T) {
	...
	for _, want := range []string{
		...
		("bash scripts/release/post_v0_4/actor-runtime-foundation-linux-" +
			"x64-gate.sh --report-dir ") + reportRoot,
		...
		("reports/actor-runtime-foundation/final/distributed-actors-linux-" +
			"x64/distributed-actors-linux-x64.json"),
	...
	assertOrderedFragments(
		...
		"name: Actor runtime foundation gate",
		("bash scripts/release/post_v0_4/actor-runtime-foundation-linux-" +
			"x64-gate.sh --report-dir reports/actor-runtime-foundation/final"),
		"name: Upload actor runtime foundation reports",
		"uses: actions/upload-artifact@v4",
	)
...
func TestSurfaceProductGateWorkflowWiringHasNoBypass(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(section, forbidden) {
			t.Fatalf(
				"Surface product gate CI job must not contain bypass or tmpfs cache marker %q",
				forbidden,
			)
		}
	}
...
func TestSurfaceProductSliceGateWorkflowWiringHasNoBypass(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(section, forbidden) {
			t.Fatalf(
				"Surface product-slice CI job must not contain bypass or tmpfs cache marker %q",
				forbidden,
			)
		}
	}
...
func TestCIWorkflowIncludesCurrentV040ReleaseReadinessJob(t *testing.T) {
	...
	for _, want := range []string{
		...
		"github.event_name == 'schedule' || github.event_name == 'workflow_dispatch'",
		"needs: test",
		"mkdir -p reports/ci-release-v0.4.0-readiness reports/v0.4.0",
		"./tetra features --format=json > reports/v0.4.0/features.json",
		"./tetra targets --format=json > reports/v0.4.0/targets.json",
		"./tetra smoke --target linux-x64 --run=true --report reports/v0.4.0/linux-host-smoke.json",
		...
		"cp reports/v0.4.0/features.json reports/ci-release-v0.4.0-readiness/features.json",
		"cp reports/v0.4.0/targets.json reports/ci-release-v0.4.0-readiness/targets.json",
		...
		"go run ./tools/cmd/verify-docs --manifest docs/generated/manifest.json",
		("go run ./tools/cmd/validate-v0-4-completion-audit --audit docs/" +
			"release/v0_4/v0_4_0_completion_audit.md --expected-status achieved"),
		"name: tetra-v0.4.0-${{ github.sha }}-release-readiness-linux",
		"path: |",
		"reports/ci-release-v0.4.0-readiness",
		"reports/v0.4.0",
	} {
		if !strings.Contains(text, want) {
...
func TestCIWorkflowValidatesTechEmpowerReportSchemas(t *testing.T) {
	...
	for _, want := range []string{
		...
		("go run ./tools/cmd/validate-techempower-report --report docs/" +
			"benchmarks/techempower_local_smoke_skip_db_report.json --allow-skip-db"),
		("go run ./tools/cmd/validate-techempower-report --report docs/" +
			"benchmarks/techempower_scram_single_query_local_report.json"),
		("go run ./tools/cmd/validate-techempower-report --report docs/" +
			"benchmarks/techempower_scram_single_query_matrix_local_report.json"),
		("go run ./tools/cmd/validate-techempower-report --report docs/" +
			"benchmarks/techempower_scram_endpoint_matrix_local_report.json"),
	} {
		if !strings.Contains(text, want) {
...
func TestCIWorkflowValidatesSmokeJSONReportsBeforeUpload(t *testing.T) {
	...
	validateIndex := strings.LastIndex(
		text,
		"go run ./tools/cmd/smoke-report-to-checklist --validate-only --report",
	)
...
func assertWorkflowUploadsContractArtifacts(
	t *testing.T,
	workflowSection, reportRoot string,
	...
	if len(missing) > 0 {
		t.Fatalf(
			"workflow upload paths do not cover Surface release contract ci_artifacts under %s\nmissing: %q\nupload paths: %q",
			reportRoot,
			missing,
			uploadPaths,
		)
