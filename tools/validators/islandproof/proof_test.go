...
func TestValidateAcceptsValidProof(t *testing.T) {
	if err := Validate(
		[]byte(validProofReport()),
		Options{MemoryReport: []byte(validMemoryReport())},
	); err != nil {
...
func TestValidateRejectsMalformedAndUnknownSchema(t *testing.T) {
	if err := Validate(
		[]byte(`{"schema":"tetra.island.proof.v1"`),
		Options{MemoryReport: []byte(validMemoryReport())},
	); err == nil {
...
func TestValidateRejectsRequiredProofFields(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name: "git head",
			old:  `"git_head": "0123456789abcdef0123456789abcdef01234567"`,
			new:  `"git_head": ""`,
			want: "git_head",
		},
		{
			name: "proof id",
			old:  `"proof_id": "proof:island:borrow:1"`,
			new:  `"proof_id": ""`,
			want: "proof_id",
		},
		{
			name: "operation",
			old:  `"operation": "island_borrow"`,
			new:  `"operation": ""`,
			want: "operation",
		},
		{
			name: "subject",
			old:  `"subject_base_id": "alloc:main:0"`,
			new:  `"subject_base_id": ""`,
			want: "subject_base_id",
		},
...
func TestValidateRejectsMissingProofProvenanceMetadata(t *testing.T) {
	...
	err := Validate([]byte(proof), Options{MemoryReport: []byte(validMemoryReport())})
	if err == nil || !strings.Contains(err.Error(), "provenance_class") ||
		!strings.Contains(err.Error(), "unsafe_class") {
...
func TestValidateRejectsProofWithoutMemoryFact(t *testing.T) {
	err := Validate(
		[]byte(validProofReport()),
		Options{MemoryReport: []byte(`{"schema_version":"tetra.memory-report.v1","rows":[]}`)},
	)
...
func TestValidateRejectsMismatchedIslandBaseAndEpoch(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name: "island",
			old:  `"island_id": "island:main:0"`,
			new:  `"island_id": "island:other"`,
			want: "island_id",
		},
		{name: "epoch", old: `"epoch": 1`, new: `"epoch": 2`, want: "epoch"},
		{
			name: "base",
			old:  `"proof_subject_base_id": "alloc:main:0"`,
			new:  `"proof_subject_base_id": "alloc:other"`,
			want: "subject_base_id",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
func TestValidateRejectsMemoryReportIdentityFieldDrift(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name: "base id",
			old:  `"base_id": "alloc:main:0"`,
			new:  `"base_id": "alloc:other"`,
			want: "base_id",
		},
		{
			name: "proof subject base id",
			old:  `"proof_subject_base_id": "alloc:main:0"`,
			new:  `"proof_subject_base_id": "alloc:other"`,
			want: "proof_subject_base_id",
		},
		{
			name: "provenance class",
			old:  `"provenance_class": "safe_known"`,
			new:  `"provenance_class": "safe_owned"`,
			want: "provenance_class",
		},
		{
			name: "unsafe class",
			old:  `"unsafe_class": "safe"`,
			new:  `"unsafe_class": "unsafe_checked"`,
			want: "unsafe_class",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
func TestValidateRejectsMemoryReportValidatorSubstitution(t *testing.T) {
	memory := strings.Replace(
		...
		`"validator_name": "memory_report_validator"`,
		1,
	)
	err := Validate([]byte(validProofReport()), Options{MemoryReport: []byte(memory)})
...
func TestValidateRejectsNoAliasWithoutDistinctLiveIslands(t *testing.T) {
	...
	proof = strings.Replace(
		...
		`"distinct_live_islands": ["island:main:0"]`,
		1,
	)
	memory := strings.Replace(
		validMemoryReport(),
...
func TestValidateRejectsStorageProofHeapFallback(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "storage") ||
		!strings.Contains(err.Error(), "Heap") {
