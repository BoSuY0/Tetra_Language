...
func writeValidReportDir(t *testing.T) string {
	...
	writeJSON(t, filepath.Join(dir, "parallel-production-linux-x64.json"), validParallelReport(t))
	writeJSON(t, filepath.Join(dir, "ui-production-runtime-linux-x64.json"), validUIReport())
	writeJSON(
		t,
		filepath.Join(dir, "native-ui-runtime-linux-x64.integration.json"),
		validNativeUIReport(),
	)
...
func writeHashManifest(t *testing.T, dir string, includeAudit bool) {
	...
	artifacts := []map[string]any{
		...
		{
			"path":   "parallel-production-linux-x64.json",
			"sha256": strings.Repeat("1", 64),
			"size":   1,
			"schema": parallelprod.SchemaV1,
		},
		{
			"path":   "ui-production-runtime-linux-x64.json",
			"sha256": strings.Repeat("2", 64),
			"size":   1,
			"schema": uiprod.SchemaV1,
		},
		{
			"path":   "native-ui-runtime-linux-x64.integration.json",
...
func validMemoryReport() memoryprod.Report {
	...
	return memoryprod.Report{
		...
		Contracts: []memoryprod.ContractReport{
			{
				Name:     "allocator runtime model",
				Status:   "pass",
				Evidence: "allocator lifecycle returns deterministic handles and failure status",
			},
			{
				Name:     "allocator failure semantics",
				Status:   "pass",
				Evidence: "linux-x64 allocation failure exits deterministically",
			},
			{
				Name:     "ownership escape model",
				Status:   "pass",
				Evidence: "heap, slices, structs, and closures preserve borrow/consume diagnostics",
			},
			{
				Name:     "unsafe cap.mem raw memory rules",
				Status:   "pass",
				Evidence: "raw memory helpers require unsafe and explicit cap.mem",
			},
			{
				Name:     "runtime bounds diagnostics",
				Status:   "pass",
				Evidence: "out-of-bounds memory access reports deterministic runtime diagnostic",
			},
			{
				Name:   "raw pointer bounds metadata",
				Status: "pass",
				Evidence: ("allocation_base_metadata, derived_allocation_offset, checked_" +
					"external_unknown, and external_unknown raw-slice policy"),
			},
			{
				Name:   "host resource leak and finalization checks",
				Status: "pass",
				Evidence: ("actornet close-without-cancel watcher cleanup plus compiler " +
					"resource finalization diagnostics ran"),
			},
			{
				Name:     "actor task transfer rules",
				Status:   "pass",
				Evidence: "memory-bearing values cannot cross actor/task boundaries without checked transfer",
			},
		},
		Cases: []memoryprod.CaseReport{
			{Name: "allocator alloc/free lifecycle", Kind: "positive", Ran: true, Pass: true},
			{
				...
				Ran:           true,
				Pass:          true,
				ExpectedError: "allocation failure",
			},
			{
				Name:          "allocator invalid size precondition",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "invalid allocation size",
			},
			{
				Name:          "cap.mem unsafe boundary",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "only allowed in unsafe blocks",
			},
			{Name: "memcpy/memset capability path", Kind: "positive", Ran: true, Pass: true},
			{
				Name:          "runtime bounds check",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "bounds",
			},
			{
				Name:          "raw ptr_add negative offset bounds",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "negative ptr_add offset",
			},
			{
				Name:          "raw ptr_add allocation upper bound",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "allocation upper bound",
			},
			{
				Name:          "raw allocation-base i32 access width",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "i32 access width exceeds allocation",
			},
			{
				Name:          "raw allocation-base ptr access width",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "ptr access width exceeds allocation",
			},
			{
				Name:          "raw slice negative length",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "negative raw slice length",
			},
			{
				Name:          "raw slice i32 length byte overflow",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "raw slice length byte overflow",
			},
			{Name: "raw pointer bounds metadata report", Kind: "positive", Ran: true, Pass: true},
			{
				Name:          "memcpy/memset negative length",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "negative helper length",
			},
			{
				Name:          "reject use-after-free",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "use-after-free",
			},
			{
				Name:          "reject double-free",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
			...
			{
				Name:          "reject aliasing violation",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "alias",
			},
			{
				...
				Ran:           true,
				Pass:          true,
				ExpectedError: "heap-escaped function value captures mutable local",
			},
			{
				Name:          "reject actor task transfer violation",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "transfer",
			},
		...
		Audit: []memoryprod.AuditReport{
			{
				...
				Artifact:    "lib/core/memory/memory.tetra; tools/cmd/memory-production-smoke",
				Evidence:    "allocator lifecycle, invalid size, failure, and stress cases ran",
				Result:      "pass",
			},
			{
				Requirement: "ownership/borrow/consume escape model",
				Artifact:    "compiler/tests/ownership",
				Evidence: ("borrow escape, use-after-free, double-free, aliasing, callable " +
					"heap escape, and transfer diagnostics are required"),
				Result: "pass",
			},
			{
				Requirement: "heap, slices, structs, and closures memory coverage",
				Artifact: ("docs/spec/runtime/ownership_v1.md; compiler/tests/ownership; " +
					"compiler/tests/semantics/semantics_callables_closures_test.go"),
				Evidence: ("heap closure handle coverage, callable heap escape rejection, " +
					"slice struct borrow escape coverage, and function-typed slice aggregate " +
					"borrow escape coverage cases ran"),
				Result: "pass",
			},
			{
				Requirement: "unsafe/cap.mem/raw memory/memcpy/memset rules",
				Artifact:    "docs/spec/runtime/unsafe.md; lib/core/memory/memory.tetra",
				Evidence: ("cap.mem unsafe boundary plus memcpy/memset capability path and " +
					"negative length cases require unsafe cap.mem"),
				Result: "pass",
			},
			{
				Requirement: "runtime bounds checks and diagnostics",
				Artifact:    "docs/spec/runtime/runtime_abi.md",
				Evidence:    "bounds and raw ptr_add diagnostics are required cases",
				Result:      "pass",
			},
			{
				Requirement: "raw pointer bounds metadata",
				Artifact: ("compiler/internal/runtimeabi/raw_pointer_bounds.go; compiler/" +
					"internal/plir/plir.go; compiler/internal/allocplan/plan.go; tools/cmd/" +
					"memory-production-smoke"),
				Evidence: ("core.alloc_bytes allocation reports include allocation_base_" +
					"metadata and external_unknown raw-slice policy; PLIR records derived_" +
					"allocation_offset and checked_external_unknown raw pointer paths"),
				Result: "pass",
			},
			{
				Requirement: "stress/fuzz evidence",
				Artifact:    "tools/cmd/memory-production-smoke",
				Evidence:    "allocator stress and deterministic memcpy/memset fuzz cases ran",
				Result:      "pass",
			},
			{
				...
				Evidence: ("small heap allocation syscall reduction benchmark compares " +
					"estimated mmap-per-allocation baseline against 64KiB chunk refill calls"),
				Result: "pass",
			},
			{
				Requirement: "allocator benchmark evidence classification",
				Artifact:    "tools/cmd/memory-production-smoke; compiler allocation report schema v2",
				Evidence: ("small heap allocation syscall reduction benchmark is classified " +
					"as allocation_report_estimate from the emitted allocation report and " +
					"does not claim runtime RSS, pprof, MemStats, time_v, or strace " +
					"measurement"),
				Result: "pass",
			},
			{
				Requirement: "use-after-free, double-free, borrow escape, and aliasing safety",
				Artifact:    "compiler/tests/safety; compiler/tests/ownership",
				Evidence:    "required cases reject unsafe memory behavior",
				Result:      "pass",
			},
			{
				Requirement: "actor/task transfer safety",
				Artifact:    "compiler/tests/ownership",
				Evidence:    "actor/task transfer safety case is required",
				Result:      "pass",
			},
			{
				Requirement: "leak/resource finalization evidence",
				Artifact: ("cli/internal/actornet/broker_test.go; compiler/tests/runtime/" +
					"resource_finalization_test.go; tools/cmd/memory-production-smoke"),
				Evidence: ("release smoke runs actornet close-without-cancel watcher leak " +
					"coverage and compiler resource finalization diagnostics"),
				Result: "pass",
			},
			{
				Requirement: "real memory examples",
				Artifact: ("examples/core/memory/core_memory_smoke.tetra; examples/memory/" +
					"ownership/ownership_smoke.tetra; examples/flow/flow_unsafe_cap_mem_" +
					"smoke.tetra"),
				Evidence: "checked-in memory examples build and run",
				Result:   "pass",
			},
			{
				Requirement: "safe memory documentation",
				Artifact:    "docs/spec/runtime/runtime_abi.md; docs/spec/runtime/ownership_v1.md",
				Evidence:    "safe memory docs are verified",
				Result:      "pass",
			},
			{
				Requirement: "release-gate entrypoint",
				Artifact:    "scripts/release/post_v0_4/memory-production-linux-x64-smoke.sh",
				Evidence:    "memory gate writes and validates production evidence",
				Result:      "pass",
			},
		},
	}
...
func validParallelReport(t *testing.T) parallelprod.Report {
	t.Helper()
	exitZero := 0
	return parallelprod.Report{
		...
		Processes: []parallelprod.ProcessReport{
			...
			{
				Name:     "parallel smoke app",
				Kind:     "app",
				Path:     "/tmp/parallel-smoke",
				Ran:      true,
				Pass:     true,
				ExitCode: &exitZero,
			},
			{
				...
				Kind:     "stress",
				Path:     "/tmp/parallel-stress",
				Ran:      true,
				Pass:     true,
				ExitCode: &exitZero,
			},
			{
				Name:     "parallel scheduler prototype tests",
				Kind:     "benchmark",
				Path:     "go test ./compiler/internal/parallelrt",
				Ran:      true,
				Pass:     true,
				ExitCode: &exitZero,
			},
			{
				Name:     "parallel scheduler prototype evidence",
				Kind:     "benchmark",
				Path:     "go run ./compiler/cmd/parallelrt-evidence",
				Ran:      true,
				Pass:     true,
				ExitCode: &exitZero,
			},
		...
		Benchmarks: []parallelprod.BenchmarkReport{
			...
			},
			{
				...
				Claim: ("Actor backpressure latency benchmark prep row exists as Tier 0 " +
					"local smoke only; no real-world SLA or comparative latency claim is " +
					"published."),
				RawOutputArtifacts: []string{
					"reports/actor-runtime-foundation/P15/parallelrt-evidence.raw.json",
				},
		...
		ActorMemoryDomains: validActorMemoryDomainReports(t),
		Contracts: []parallelprod.ContractReport{
			{
				Name:     "production task scheduler",
				Status:   "pass",
				Evidence: "scheduler fairness and lifecycle cases ran",
			},
			{
				Name:     "join cancel deadline select group lifecycle",
				Status:   "pass",
				Evidence: "join, cancel, deadline, select, and group lifecycle diagnostics are stable",
			},
			{
				Name:     "actor mailbox backpressure and failure handling",
				Status:   "pass",
				Evidence: "mailbox capacity and actor failure cases are covered",
			},
			{
				Name:     "task actor thread boundary transfer rules",
				Status:   "pass",
				Evidence: "ownership transfer diagnostics protect boundaries",
			},
			{
				Name:     "race safety model",
				Status:   "pass",
				Evidence: "shared mutable state crossing parallel boundaries is rejected conservatively",
			},
			{
				Name:     "safe unsafe forbidden parallelism boundary",
				Status:   "pass",
				Evidence: "docs and diagnostics define safe unsafe forbidden parallel behavior",
			},
		},
		Cases: []parallelprod.CaseReport{
			...
			{
				Name:          "actor recv cancel wake",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "actor recv cancel wake",
			},
			{
				...
				Ran:           true,
				Pass:          true,
				ExpectedError: "backpressure",
			},
			{
				Name:          "message pool exhaustion",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "message pool exhausted",
			},
			{
				Name:          "invalid actor handle send",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "invalid actor handle",
			},
			{
				Name:          "done actor send",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "done actor",
			},
			{
				Name:          "actor failure handling",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "actor failed",
			},
			{
				Name:          "invalid handle diagnostics",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "invalid handle",
			},
			{
				Name:          "resource double join diagnostic",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "joined",
			},
			{
				Name:          "task group use-after-close diagnostic",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "closed",
			},
			{
				Name:          "ownership transfer across task boundary",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "transfer",
			...
			{
				Name:          "ownership transfer across actor boundary",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "transfer",
			...
			{
				Name:          "race-safety shared mutable rejection",
				Kind:          "negative",
				Ran:           true,
				Pass:          true,
				ExpectedError: "shared mutable",
			},
			...
			{
				Name: "safe unsafe forbidden boundary coverage",
				Kind: "positive",
				Ran:  true,
				Pass: true,
			},
			{
				Name:              "actor fanout mailbox drain soak",
				Kind:              "stress",
				Ran:               true,
				Pass:              true,
				Iterations:        512,
				DeterministicSeed: "actor-fanout-mailbox-drain-v1",
				MaxDurationMS:     90000,
			},
			{
				Name:              "many tasks stress",
				Kind:              "stress",
				Ran:               true,
				Pass:              true,
				Iterations:        64,
				DeterministicSeed: "task-bounded-stress-seed-17",
				MaxDurationMS:     10000,
			},
			{
				Name:              "many actor messages stress",
				Kind:              "stress",
				Ran:               true,
				Pass:              true,
				Iterations:        256,
				DeterministicSeed: "actors-tagged-stress-v1",
				MaxDurationMS:     10000,
			},
			{
				...
				Pass:              true,
				Iterations:        16,
				DeterministicSeed: "parallel-cancellation-storm-v1",
				MaxDurationMS:     10000,
			},
			{
				Name:              "timeouts stress",
				Kind:              "stress",
				Ran:               true,
				Pass:              true,
				Iterations:        1,
				DeterministicSeed: "deadline-aware-waits-v1",
				MaxDurationMS:     10000,
			},
		},
		Diagnostics: []parallelprod.DiagnosticReport{
			...
			},
			{
				...
				Code:          "TASK_GROUP_CANCEL_WAKE_JOIN",
				Severity:      "error",
				Category:      "task",
				Position:      "runtime",
				ExpectedError: "cancelled before deadline",
			...
			},
			{
				...
				Code:          "ACTOR_MAILBOX_BACKPRESSURE",
				Severity:      "error",
				Category:      "actor",
				Position:      "runtime",
				ExpectedError: "backpressure",
			},
			{
				...
				Code:          "ACTOR_MESSAGE_POOL_EXHAUSTED",
				Severity:      "error",
				Category:      "actor",
				Position:      "runtime",
				ExpectedError: "message pool exhausted",
			},
			{
				...
				Code:          "ACTOR_INVALID_HANDLE_SEND",
				Severity:      "error",
				Category:      "actor",
				Position:      "runtime",
				ExpectedError: "invalid actor handle",
			...
			},
			{
				...
				Code:          "ACTOR_MISSING_NODE_FAILURE",
				Severity:      "error",
				Category:      "actor",
				Position:      "runtime",
				ExpectedError: "actor failed",
			},
			{
				...
				Code:          "ACTOR_INVALID_HANDLE_DIAGNOSTIC",
				Severity:      "error",
				Category:      "actor",
				Position:      "cli-json",
				ExpectedError: "invalid handle",
			...
			},
			{
				...
				Code:          "RACE_SHARED_MUTABLE_REJECTED",
				Severity:      "error",
		...
		Audit: []parallelprod.AuditReport{
			{
				...
				Artifact:    "compiler/compiler_suite_test.go",
				Evidence:    "scheduler fairness and many tasks stress cases ran",
				Result:      "pass",
			},
			{
				Requirement: "join/cancel/deadline/select/group lifecycle",
				Artifact:    "compiler/compiler_suite_test.go",
				Evidence: ("join cancellation deadline select task group cancel-wakes-" +
					"deadline-join and nested cancellation cases are required"),
				Result: "pass",
			},
			{
				Requirement: "actor mailbox backpressure and failure handling",
				Artifact:    "compiler/compiler_suite_test.go",
				Evidence:    "mailbox backpressure task actor mailbox handoff and failure cases are required",
				Result:      "pass",
			},
			{
				Requirement: "task/actor/thread-boundary transfer rules",
				Artifact:    "compiler/tests/ownership; cli/cmd/tetra/tetra_suite_test.go",
				Evidence:    "task and actor transfer double join and closed group diagnostics are required",
				Result:      "pass",
			},
			{
				Requirement: "race-safety model or conservative rejections",
				Artifact:    "compiler/tests/ownership; docs/spec/runtime/actors.md",
				Evidence:    "shared mutable crossing is rejected conservatively",
				Result:      "pass",
			},
			{
				...
				Evidence: ("many tasks stress, many actor messages stress, actor fanout " +
					"mailbox drain soak, cancellation storm, timeouts stress, and actor " +
					"broker leak cleanup cases are required with bounded metadata"),
				Result: "pass",
			},
			{
				Requirement: "safe/unsafe/forbidden parallelism documentation",
				Artifact: ("docs/spec/runtime/actors.md; docs/user/platform/async_actors_" +
					"guide.md; docs/spec/runtime/runtime_abi.md; compiler/tests/semantics/" +
					"semantics_async_ownership_test.go; compiler/tests/safety/effects/" +
					"effects_test.go"),
				Evidence: ("safe unsafe forbidden boundary coverage case runs compiler " +
					"tests and docs define boundaries"),
				Result: "pass",
			},
			{
				Requirement: "stable parallel diagnostics",
				Artifact: ("compiler/compiler_suite_test.go; compiler/compiler_suite_" +
					"test.go; compiler/tests/ownership/actor_task/actor_task_ownership_" +
					"test.go; cli/cmd/tetra/tetra_suite_test.go"),
				Evidence: ("negative parallel cases require stable expected_error evidence " +
					"for cancellation deadline backpressure invalid handle double join use-" +
					"after-close transfer and shared mutable rejection"),
				Result: "pass",
			},
			{
				...
				Evidence: ("parallelrt evidence emits Tier 0 actor ping-pong, fanout/fanin, " +
					"mailbox throughput, backpressure latency, and zero_copy_move local " +
					"typed mailbox prep rows with raw artifact references; Tier 1 remains " +
					"preparation-only here, with no benchmark superiority, no C++/Rust " +
			...
			},
			{
				...
				Evidence:    "parallel gate writes and validates production evidence",
				Result:      "pass",
			},
		},
	}
...
func validActorMemoryDomainReports(t *testing.T) []parallelprod.ActorMemoryDomainReport {
	t.Helper()
	var reports []parallelprod.ActorMemoryDomainReport
	if err := json.Unmarshal([]byte(validActorMemoryDomainsJSON), &reports); err != nil {
		t.Fatalf("unmarshal actor memory domain fixture: %v", err)
	}
	return reports
}

const validActorMemoryDomainsJSON = `[
	...
	return uiprod.Report{
		...
		Processes: []uiprod.ProcessReport{
			...
			},
			{
				...
				Kind:     "runtime",
				Path:     "tools/cmd/ui-production-runtime-smoke",
				Ran:      true,
				Pass:     true,
				ExitCode: &exitZero,
			},
			{
				Name:     "native shell runtime integration",
				Kind:     "runtime",
				Path:     "go run ./tools/cmd/native-ui-runtime-smoke",
				Ran:      true,
				Pass:     true,
				ExitCode: &exitZero,
			},
			{
				Name:     "native runtime evidence validator",
				Kind:     "runtime",
				Path:     "go run ./tools/cmd/validate-native-ui-runtime",
				Ran:      true,
				Pass:     true,
				ExitCode: &exitZero,
			},
			{
				Name:     "desktop UI widget stress",
				Kind:     "stress",
				Path:     "/tmp/ui-widget-stress",
				Ran:      true,
				Pass:     true,
				ExitCode: &exitZero,
			},
		...
		Contracts: []uiprod.ContractReport{
			{
				Name:     "Linux-x64 desktop UI runtime",
				Status:   "pass",
				Evidence: "desktop UI and sidecar-driven native runtime process evidence ran on linux-x64",
			},
			{Name: "window lifecycle", Status: "pass", Evidence: "window lifecycle is covered"},
			{
				Name:     "layout system",
				Status:   "pass",
				Evidence: "layout measure/place and panel nesting cases ran",
			},
			{
				Name:     "buttons text input lists panels state binding",
				Status:   "pass",
				Evidence: "button text input focus change list panel and bound state widgets are present",
			},
			{
				Name:     "event loop",
				Status:   "pass",
				Evidence: "focus input change select click and timer events ran through runtime",
			},
			{
				Name:     "async UI commands",
				Status:   "pass",
				Evidence: "async command completion case runs",
			},
			{
				Name:     "timers",
				Status:   "pass",
				Evidence: "timer scheduled redraw case records a tick event and timer_tick operation",
			},
			{
				Name:     "redraw update model",
				Status:   "pass",
				Evidence: "redraw lifecycle case records state to redraw",
			},
			{
				Name:     "error crash handling",
				Status:   "pass",
				Evidence: "invalid widget command failure and crash handling cases are required",
			},
			{
				Name:     "real dogfood applications",
				Status:   "pass",
				Evidence: "dogfood application smoke case uses real Tetra UI source",
			},
		},
		Widgets: []uiprod.WidgetReport{
			{
				...
				Enabled: true,
				Visible: true,
				Bounds:  uiprod.Bounds{Width: 640, Height: 480},
			},
			{
				ID:      "RootPanel",
				Kind:    "panel",
			...
			},
			{
				...
				Parent:  "RootPanel",
				Binding: "state.title",
				Value:   "Saved after timer",
				Enabled: true,
				Visible: true,
				Bounds:  uiprod.Bounds{Width: 608, Height: 32},
			},
			{
				ID:      "NameInput",
				Kind:    "input",
			...
			},
			{
				...
				Enabled: true,
				Visible: true,
				Bounds:  uiprod.Bounds{Width: 608, Height: 240},
			},
			{
				ID:      "SaveButton",
				Kind:    "button",
				...
				Enabled: true,
				Visible: true,
				Bounds:  uiprod.Bounds{Width: 200, Height: 44},
			},
		},
		Events: []uiprod.EventReport{
			{
				...
				Command:     "focusName",
				Pass:        true,
				BeforeState: map[string]string{"AppState.focused": "none"},
				AfterState:  map[string]string{"AppState.focused": "NameInput"},
				Operations: []uiprod.OperationReport{
					{
						Kind:       "focus",
						Target:     "widget.NameInput",
						Value:      "focused",
						StateField: "focused",
						StateValue: "NameInput",
					},
				},
				WidgetUpdates: []uiprod.WidgetUpdateReport{
					{ID: "TitleText", Before: "Ready", After: "Editing name"},
				},
			},
			{
				Order:       2,
				...
				Command:     "setName",
				Pass:        true,
				BeforeState: map[string]string{"AppState.name": "tetra"},
				AfterState:  map[string]string{"AppState.name": "tetra-lang"},
				Operations: []uiprod.OperationReport{
					{
						Kind:       "state_set",
						Target:     "state.name",
						Value:      "tetra-lang",
						StateField: "name",
						StateValue: "tetra-lang",
					},
				},
				WidgetUpdates: []uiprod.WidgetUpdateReport{
					{ID: "NameInput", Before: "tetra", After: "tetra-lang"},
				},
			},
			{
				Order:    3,
				...
				WidgetUpdates: []uiprod.WidgetUpdateReport{
					{ID: "NameInput", Before: "tetra-lang", After: "tetra-prod"},
				},
			},
			{
				Order:       4,
				...
				Command:     "selectItem",
				Pass:        true,
				BeforeState: map[string]string{"AppState.selected": "item-1"},
				AfterState:  map[string]string{"AppState.selected": "item-2"},
				Operations: []uiprod.OperationReport{
					{
						Kind:       "state_set",
				...
				WidgetUpdates: []uiprod.WidgetUpdateReport{
					{ID: "ItemList", Before: "item-1", After: "item-2"},
				},
			},
			{
				Order:       5,
				...
				Command:     "saveAsync",
				Pass:        true,
				BeforeState: map[string]string{"AppState.saved": "false"},
				AfterState:  map[string]string{"AppState.saved": "true"},
				Operations: []uiprod.OperationReport{
					{
						Kind:       "async_command",
						Target:     "command.saveAsync",
						Value:      "completed",
						StateField: "saved",
						StateValue: "true",
					},
				...
				WidgetUpdates: []uiprod.WidgetUpdateReport{
					{ID: "TitleText", Before: "Editing name", After: "Saved"},
				},
			},
			{
				Order:       6,
				...
				Command:     "timerTick",
				Pass:        true,
				BeforeState: map[string]string{"AppState.dirty": "true"},
				AfterState:  map[string]string{"AppState.dirty": "false"},
				Operations: []uiprod.OperationReport{
					{
						Kind:       "timer_tick",
						Target:     "timer.redraw",
		...
		Audit: []uiprod.AuditReport{
			{
				Requirement: "Linux-x64 desktop UI runtime",
				Artifact:    "tools/cmd/ui-production-runtime-smoke",
				Evidence:    "build app desktop runtime native runtime and stress processes ran",
				Result:      "pass",
			},
			{
				Requirement: "window lifecycle",
				Artifact:    "examples/ui/ui_desktop_runtime_smoke.tetra",
				Evidence:    "window lifecycle case is required",
				Result:      "pass",
			},
			{
				Requirement: "layout system",
				Artifact:    "compiler/internal/lower/lower_runtime_ui.go; docs/spec/ui/ui_v0.4.0.md",
				Evidence:    "layout measure/place and panel nesting are required",
				Result:      "pass",
			},
			{
				Requirement: "buttons/text/input/lists/panels widgets",
				Artifact:    "examples/ui/ui_desktop_runtime_smoke.tetra",
				Evidence:    "widget tree includes required widgets",
				Result:      "pass",
			},
			{
				Requirement: "state binding",
				Artifact:    "tools/validators/uiprod",
				Evidence:    "state binding update plus input focus/change widget update evidence are required",
				Result:      "pass",
			},
			{
				Requirement: "event loop and redraw/update model",
				Artifact:    "tools/cmd/ui-production-runtime-smoke",
				Evidence:    "focus input change select click timer and redraw lifecycle cases are required",
				Result:      "pass",
			},
			{
				Requirement: "async commands and timers",
				Artifact:    "tools/cmd/ui-production-runtime-smoke",
				Evidence: ("async command, timer tick event evidence, and timer scheduled " +
					"redraw cases are required"),
				Result: "pass",
			},
			{
				Requirement: "error/crash handling",
				Artifact:    "tools/validators/uiprod",
				Evidence:    "invalid widget command failure and crash cases are required",
				Result:      "pass",
			},
			{
				Requirement: "real examples and dogfood applications",
				Artifact:    "examples/ui/ui_desktop_runtime_smoke.tetra",
				Evidence:    "dogfood smoke and native runtime integration cases are required",
				Result:      "pass",
			},
			{
				Requirement: "compiler-emitted UI bundle/native-shell trace load evidence",
				Artifact: ("examples/ui/ui_desktop_runtime_smoke.tetra; <output>.ui.json; " +
					"<output>.ui.shell.json"),
				Evidence: ("UI production smoke loads compiler-emitted UI bundle and native-" +
					"shell trace before accepting runtime evidence"),
				Result: "pass",
			},
			{
				Requirement: "sidecar-driven native UI runtime integration",
				Artifact: ("tools/cmd/native-ui-runtime-smoke; tools/cmd/validate-native-ui-" +
					"runtime; native-ui-runtime-linux-x64.integration.json"),
				Evidence: ("UI production smoke runs and validates tetra.ui.native-" +
					"runtime.v1 consistency evidence"),
				Result: "pass",
			},
			{
				Requirement: "stable UI diagnostics",
				Artifact:    "tools/cmd/ui-production-runtime-smoke; tools/validators/uiprod",
				Evidence:    "negative UI cases require stable expected_error evidence",
				Result:      "pass",
			},
			{
				Requirement: "release-gate entrypoint rejecting runtime-less evidence",
				Artifact:    "scripts/release/post_v0_4/ui-production-runtime-linux-x64-smoke.sh",
				Evidence:    "validator rejects non-runtime evidence",
				Result:      "pass",
			},
		},
	}
...
func validNativeUIReport() nativeui.Report {
	...
	return nativeui.Report{
		...
		Widgets: []nativeui.WidgetReport{
			{
				...
				Enabled: true,
				Visible: true,
				Bounds:  nativeui.Bounds{Width: 320, Height: 96},
			},
			{
				ID:      "CounterView.count",
				Kind:    "text",
				Parent:  "CounterView",
				Binding: "count",
				Value:   "0",
				Enabled: true,
				Visible: true,
				Bounds:  nativeui.Bounds{Width: 304, Height: 24},
			},
			{
				ID:      "CounterView.increment",
				Kind:    "action",
				Parent:  "CounterView",
				Event:   "click",
				Command: "increment",
		...
		Events: []nativeui.EventReport{
			{
				Order:       1,
				WidgetID:    "CounterView.increment",
				Event:       "click",
				Command:     "increment",
				Pass:        true,
				BeforeState: map[string]string{"CounterState.count": "0"},
				AfterState:  map[string]string{"CounterState.count": "1"},
				Operations: []nativeui.OperationReport{
					{
						Kind:       "state_add",
						Target:     "state.count",
						Value:      "1",
				...
				WidgetUpdates: []nativeui.WidgetUpdateReport{
					{ID: "CounterView.count", Before: "0", After: "1"},
				},
			},
		},
		Cases: []nativeui.CaseReport{
			...
			},
			{
				...
				Ran:           true,
				Pass:          true,
				ExpectedError: "malformed metadata",
			},
			{
