...
percentiles (`p50`, `p90`, `p95`, `p99`, `p99.9`, `max`), positive matrix run
repeat metadata, observed content types, endpoint semantic check lists, SCRAM
evidence, matrix semantic probes, resource snapshots, declared matrix grid
coverage, or clean shutdown evidence when a soak is present.
