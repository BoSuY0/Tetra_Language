...
func TestValidateSurfaceInspectorReportRejectsIncompleteMorphToPixelsSections(t *testing.T) {
	raw := strings.Replace(validSurfaceInspectorReportWithMorphToPixelsJSON(), `"golden_diff":{"present":true`, `"golden_diff":{"present":false`, 1)
...
func validSurfaceInspectorReportWithMorphToPixelsJSON() string {
	...
	raw = strings.Replace(raw, `}],"source_locations":`, `},{"kind":"morph-rendered-beauty","path":"reports/surface-inspector/inputs/morph-rendered-beauty.json","schema":"tetra.surface.morph-rendered-beauty.v1","source":"`+source+`","target":"headless","pass":true}],"source_locations":`, 1)
	raw = strings.Replace(raw, `}],"sections":`, `},{"kind":"morph-rendered-beauty","path":"`+source+`","line":1,"column":1}],"sections":`, 1)
	raw = strings.Replace(raw, `"perf_counters":{"present":true,"count":4,"source":"surface_performance_budget"}}`, `"perf_counters":{"present":true,"count":4,"source":"surface_performance_budget"},"recipe_expansions":{"present":true,"count":4,"source":"morph_to_pixels.recipe_expansion_count"},"block_scene_nodes":{"present":true,"count":12,"source":"morph_to_pixels.block_scene_node_count"},"render_commands":{"present":true,"count":10,"source":"morph_to_pixels.render_command_count"},"frame_artifacts":{"present":true,"count":1,"source":"morph_to_pixels.frame_artifact"},"golden_diff":{"present":true,"count":1,"source":"morph_to_pixels.golden_artifact + diff metrics"}}`, 1)
	raw = strings.Replace(raw, `,"static_artifacts":`, `,"morph_to_pixels":`+validMorphToPixelsChainJSON(source)+`,"static_artifacts":`, 1)
	return raw + "\n"
}
