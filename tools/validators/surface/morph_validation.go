...
func validateMorphEvidence(report Report) []string {
	...
	if !isSupportedMorphRuntimeEvidence(report) {
		issues = append(issues, "morph evidence requires deterministic headless Surface runtime evidence or wasm32-web browser-canvas runtime evidence")
	}
	if strings.TrimSpace(morph.Source) == "" {
		issues = append(issues, "morph source is required")
	}
	if !isSurfaceMorphReportSource(morph.Source) {
		issues = append(issues, fmt.Sprintf("morph source is %q, want examples/surface_morph_command_palette.tetra, a reference app, or generated Surface project template source", morph.Source))
	}
...
func isSurfaceMorphReportSource(source string) bool {
	...
	return source == "examples/surface_morph_command_palette.tetra" ||
...
func validateMorphEvidenceContract(contract MorphEvidenceContractReport, morph *MorphReport, report Report) []string {
	...
	required := []struct {
		...
		{"memory_budget", contract.MemoryBudget && morph.MemoryBudget.Schema == "tetra.surface.morph-memory-budget.v1" && morph.MemoryBudget.FrameCount > 0},
