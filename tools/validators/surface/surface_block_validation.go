...
func validateAccessibilityTreeEvidence(report Report) []string {
	...
	if !isSurfaceAccessibilitySettingsSource(report.Source) && !releaseAccessibility &&
		!releaseWindow {
		issues = append(
			issues,
			fmt.Sprintf(
				("accessibility_tree source path must match "+
	...
	if report.Target == "wasm32-web" &&
		!isBrowserCanvasHostEvidenceLevel(report.HostEvidence.Level) {
		issues = append(
			issues,
			("accessibility_tree browser evidence must use browser canvas " +
				"native input, not Node-only wasm32-web evidence"),
		)
	}
	if report.AccessibilityTree == nil {
		return append(
			issues,
			("accessibility_tree evidence is required for " +
				"examples/surface/toolkit/surface_accessibility_settings.tetr" +
				"a"),
		)
	}
	...
	if releaseWindow {
		issues = append(
			issues,
			validateLinuxReleaseWindowAccessibilityBridgeEvidence(report, tree)...)
		return issues
	}
	if tree.Schema != "tetra.surface.accessibility-tree.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree schema is %q, want tetra.surface.accessibility-tree.v1",
				tree.Schema,
			),
		)
	}
	if tree.AccessibilityLevel != "metadata-tree-v1" &&
		...
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree accessibility_level is %q, want metadata-tree-v1 or platform-bridge-v1",
	...
	if normalizeEvidencePath(tree.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree source %q must match report source %q",
				tree.Source,
				report.Source,
			),
		)
	...
	if tree.Module != "lib.core.accessibility" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree module is %q, want lib.core.accessibility",
				tree.Module,
			),
		)
	}
	if tree.WidgetModule != "lib.core.widgets" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree widget_module is %q, want lib.core.widgets",
				tree.WidgetModule,
			),
		)
	}
	if releaseAccessibility {
		...
		if tree.AccessibilityLevel != "metadata-tree-v1" {
			issues = append(issues, fmt.Sprintf(("accessibility_tree accessibility_level is %q, want "+
				"metadata-tree-v1"), tree.AccessibilityLevel))
		}
		...
		if tree.PlatformHostIntegration {
			issues = append(issues, ("accessibility_tree platform_host_integration must be false " +
				"for metadata-only Surface evidence"))
		}
	...
	if !tree.DerivedFromComponentTree || !tree.UsesComponentTreeAPI || !tree.UsesWidgetToolkit {
		issues = append(
			issues,
			("accessibility_tree must be derived from component_tree, " +
				"component_tree_api, and widget toolkit evidence"),
		)
	}
	...
	if !tree.NoDOMUI || !tree.NoUserJS || !tree.NoPlatformWidgets || !tree.NoLegacySidecars {
		issues = append(
			issues,
			("accessibility_tree must prove no_dom_ui, no_user_js, " +
				"no_platform_widgets, and no_legacy_sidecars"),
		)
	}
	if tree.ComponentTreeSchema != "tetra.surface.component-tree.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree component_tree_schema is %q, want tetra.surface.component-tree.v1",
	...
	if tree.ComponentTreeAPISchema != "tetra.surface.component-tree-api.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree component_tree_api_schema is %q, want tetra.surface.component-tree-api.v1",
				tree.ComponentTreeAPISchema,
			),
		)
	}
	if tree.ToolkitSchema != "tetra.surface.toolkit.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree toolkit_schema is %q, want tetra.surface.toolkit.v1",
				tree.ToolkitSchema,
			),
		)
	}
	...
	if report.ComponentTree == nil {
		...
		if report.ComponentTree.DynamicLevel != wantLevel {
			issues = append(issues, fmt.Sprintf((("accessibility_tree component_tree dynamic_level "+
				"is %q, want ")+
				"%s"), report.ComponentTree.DynamicLevel, wantLevel))
		}
		if !intSlicesEqual(report.ComponentTree.FocusOrder, []int{5, 7, 9, 10}) {
			issues = append(issues, fmt.Sprintf((("accessibility_tree component_tree focus_order = "+
				"%v, want [5 ")+
				"7 9 10]"), report.ComponentTree.FocusOrder))
		}
	...
	if tree.NodeCount != len(tree.Nodes) {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree node_count = %d, want len(nodes) %d",
				tree.NodeCount,
				len(tree.Nodes),
			),
		)
	...
	if tree.NodeCount != 12 || len(tree.Nodes) != 12 {
		issues = append(
			issues,
			fmt.Sprintf("accessibility_tree node_count = %d, want 12", tree.NodeCount),
		)
	}
	...
	for role := range allowedRoles {
		if !contains(tree.RolesPresent, role) {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree roles_present missing %s", role),
			)
		}
	}
	...
	for _, node := range tree.Nodes {
		...
		if strings.TrimSpace(node.Name) == "" {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree node %d name is required", node.ID),
			)
		...
		if !allowedRoles[node.Role] {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree node %s has unknown role %q", node.Name, node.Role),
			)
		}
		...
		if prev, exists := componentIDs[node.ComponentID]; exists {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree duplicate component_id %d used by %s and %s",
					node.ComponentID,
					prev,
					node.Name,
				),
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree node %s component_id %d is not in component_tree",
					node.Name,
					node.ComponentID,
				),
			)
			...
			if componentNode.Name != node.Name {
				issues = append(issues, fmt.Sprintf(("accessibility_tree node %s component_id %d names "+
					"component_tree node %s"), node.Name, node.ComponentID, componentNode.Name))
			}
			if componentNode.ParentID != node.ParentID {
				issues = append(issues, fmt.Sprintf(("accessibility_tree node %s parent_id = %d, want "+
					"component_tree parent_id %d"), node.Name, node.ParentID, componentNode.ParentID))
			}
			if !rectsEqual(node.Bounds, componentNode.Bounds) {
				issues = append(issues, fmt.Sprintf(("accessibility_tree node %s bounds %+v do not match "+
					"component_tree bounds %+v"), node.Name, node.Bounds, componentNode.Bounds))
			}
		}
		if node.Bounds.W <= 0 || node.Bounds.H <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree node %s bounds are required", node.Name),
			)
		}
		if !node.Visible || !node.Enabled {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree node %s must be visible and enabled", node.Name),
			)
		}
		if node.Focused {
			focusedCount++
		...
		if node.Role == "textbox" && !node.Editable {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree textbox %s must be editable", node.Name),
			)
		}
		if node.Role != "textbox" && node.Editable {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree non-textbox node %s must not be editable",
					node.Name,
				),
			)
		}
		if node.Readonly || node.Required || node.Pressed || node.Invalid {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree node %s unexpected readonly/required/pressed/invalid state",
					node.Name,
				),
			)
		}
	...
	if focusedCount != 1 {
		issues = append(
			issues,
			fmt.Sprintf("accessibility_tree focused node count = %d, want exactly 1", focusedCount),
		)
	}
	...
	if roleCounts["label"] != tree.LabelCount || roleCounts["textbox"] != tree.TextBoxCount ||
		roleCounts["button"] != tree.ButtonCount ||
		roleCounts["status"] != tree.StatusCount {
	...
	for _, expected := range []struct {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree missing node id %d (%s)",
					expected.id,
					expected.name,
				),
			)
		...
		if node.Name != expected.name || node.Role != expected.role ||
			node.ParentID != expected.parent {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree node %d = %s/%s parent %d, want %s/%s parent %d",
					expected.id,
					node.Name,
					node.Role,
					node.ParentID,
		...
		if node.FocusIndex != expected.focus {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree node %s focus_index = %d, want %d",
					expected.name,
					node.FocusIndex,
					expected.focus,
				),
		...
		if node.ReadingIndex != expected.reading {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree node %s reading_index = %d, want %d",
					expected.name,
					node.ReadingIndex,
					expected.reading,
				),
		...
		for _, action := range expected.actions {
			if !contains(node.Actions, action) {
				issues = append(
					issues,
					fmt.Sprintf(
						"accessibility_tree node %s actions missing %s",
						expected.name,
						action,
					),
				)
	...
	for _, relation := range []struct {
		...
		if !hasAccessibilityRelationship(
			tree.Relationships,
			relation.kind,
			relation.from,
			relation.to,
			...
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree relationships missing %s %s %s",
					relation.from,
					relation.kind,
					relation.to,
				),
	...
	if nameBox, ok := nodesByName["NameTextBox"]; ok && nameBox.LabelledBy != "NameLabel" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree NameTextBox labelled_by is %q, want NameLabel",
				nameBox.LabelledBy,
			),
		)
	}
	if emailBox, ok := nodesByName["EmailTextBox"]; ok && emailBox.LabelledBy != "EmailLabel" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree EmailTextBox labelled_by is %q, want EmailLabel",
				emailBox.LabelledBy,
			),
		)
	}
	if nameLabel, ok := nodesByName["NameLabel"]; ok && nameLabel.LabelFor != "NameTextBox" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree NameLabel label_for is %q, want NameTextBox",
				nameLabel.LabelFor,
			),
		)
	}
	if emailLabel, ok := nodesByName["EmailLabel"]; ok && emailLabel.LabelFor != "EmailTextBox" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree EmailLabel label_for is %q, want EmailTextBox",
				emailLabel.LabelFor,
			),
		)
	}
	...
	if !stringSlicesEqual(tree.FocusOrder, wantFocusOrder) {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree focus_order = %v, want %v",
				tree.FocusOrder,
				wantFocusOrder,
			),
		)
	}
	wantReadingOrder := []string{
		"TitleText",
		"NameLabel",
		"NameTextBox",
		"EmailLabel",
	...
	if !stringSlicesEqual(tree.ReadingOrder, wantReadingOrder) {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree reading_order = %v, want %v",
				tree.ReadingOrder,
				wantReadingOrder,
			),
		)
	...
	for _, action := range []struct {
		...
		if !hasAccessibilityAction(tree.Actions, action.target, action.action, action.semantic) {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree actions missing %s %s/%s",
					action.target,
					action.action,
					action.semantic,
				),
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree report requires %s evidence", required),
			)
		}
	}
	if releaseAccessibility {
		for _, required := range []string{
			...
			if !caseNameContains(report.Cases, required) {
				issues = append(
					issues,
					fmt.Sprintf("accessibility_tree release report requires %s evidence", required),
				)
			}
		}
	}
...
func validateReleaseAccessibilityBridgeEvidence(
	report Report,
	...
	if tree.AccessibilityLevel != "platform-bridge-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree accessibility_level is %q, want platform-bridge-v1",
				tree.AccessibilityLevel,
			),
		)
	}
	if tree.ReleaseScope != "surface-v1-linux-web" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree release_scope is %q, want surface-v1-linux-web",
				tree.ReleaseScope,
			),
		)
	}
	if tree.Experimental {
		issues = append(
			issues,
			"accessibility_tree experimental must be false for release accessibility reports",
		)
	}
	if !tree.ProductionClaim {
		issues = append(
			issues,
			"accessibility_tree production_claim must be true for release accessibility reports",
		)
	}
	if !tree.MetadataTree {
		issues = append(
			issues,
			"accessibility_tree metadata_tree must be true for platform-bridge-v1",
		)
	}
	if !tree.PlatformExport {
		issues = append(
			issues,
			"accessibility_tree platform_export must be true for platform-bridge-v1",
		)
	}
	...
	if !ok || screenReaderEvidence == "" {
		issues = append(
			issues,
			"accessibility_tree screen_reader_evidence must name the exact platform-tree evidence",
		)
	}
	if strings.Contains(screenReaderEvidence, "full") ||
		strings.Contains(screenReaderEvidence, "screen-reader-support") {
		issues = append(
			issues,
			("accessibility_tree screen_reader_evidence must not claim " +
				"full screen-reader support without a real probe"),
		)
	}
	...
	case "linux-x64":
		if report.HostEvidence.Level != "linux-x64-real-window" {
			issues = append(
				issues,
				"accessibility_tree linux release evidence must use linux-x64 real-window host evidence",
			)
		}
		if !report.HostEvidence.AccessibilityBridge {
			issues = append(
				issues,
				"accessibility_tree linux release host_evidence.accessibility_bridge must be true",
			)
		}
		if !tree.PlatformHostIntegration {
			issues = append(
				issues,
				"accessibility_tree platform_host_integration must be true for linux platform-bridge-v1",
			)
		}
		if tree.PlatformBridge != "linux_accessibility_host_bridge_v1" {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree platform_bridge is %q, want linux_accessibility_host_bridge_v1",
					tree.PlatformBridge,
				),
			)
		}
		if !tree.LinuxPlatformProbe {
			issues = append(
				issues,
				"accessibility_tree linux_platform_probe must be true for linux platform-bridge-v1",
			)
		}
		if strings.TrimSpace(tree.LinuxProbeArtifact) == "" {
			issues = append(
				issues,
				"accessibility_tree linux_probe_artifact is required for linux platform-bridge-v1",
			)
		}
		if screenReaderEvidence != "linux_accessibility_host_bridge_v1" {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree screen_reader_evidence is %q, want linux_accessibility_host_bridge_v1",
		...
		if !hasRuntimeProcessName(report.Processes, "surface linux accessibility platform probe") {
			issues = append(
				issues,
				("accessibility_tree linux release requires surface linux " +
		...
		if !artifactKindContains(report.Artifacts, "linux-accessibility-platform-probe") {
			issues = append(
				issues,
				"accessibility_tree linux release requires linux-accessibility-platform-probe artifact",
			)
		}
	case "wasm32-web":
		if !isBrowserCanvasHostEvidenceLevel(report.HostEvidence.Level) {
			issues = append(
				issues,
				("accessibility_tree browser release evidence must be " +
					"browser-canvas input, not Node-only wasm32-web evidence"),
			)
		}
		if !report.HostEvidence.BrowserAccessibilitySnapshot {
			issues = append(
				issues,
				"accessibility_tree browser release host_evidence.browser_accessibility_snapshot must be true",
			)
		}
		if !report.HostEvidence.BrowserAccessibilityMirror {
			issues = append(
				issues,
				"accessibility_tree browser release host_evidence.browser_accessibility_mirror must be true",
			)
		}
		if tree.PlatformBridge != "browser_accessibility_mirror_v1" {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree platform_bridge is %q, want browser_accessibility_mirror_v1",
					tree.PlatformBridge,
				),
			)
		}
		if !tree.BrowserAccessibilitySnap {
			issues = append(
				issues,
				"accessibility_tree browser_accessibility_snapshot must be true for browser platform-bridge-v1",
			)
		}
		if !tree.BrowserAccessibilityMirror {
			issues = append(
				issues,
				"accessibility_tree browser_accessibility_mirror must be true for browser platform-bridge-v1",
			)
		}
		if screenReaderEvidence != "browser_accessibility_snapshot_v1" {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree screen_reader_evidence is %q, want browser_accessibility_snapshot_v1",
					screenReaderEvidence,
				),
			)
		}
		if !hasRuntimeProcessName(report.Processes, "surface wasm32-web browser canvas trace") {
			issues = append(
				issues,
				"accessibility_tree browser release requires browser canvas trace process evidence",
			)
		}
		if !artifactKindContains(report.Artifacts, "runner-trace") {
			issues = append(
				issues,
				"accessibility_tree browser release requires runner-trace artifact",
			)
		}
	case "headless":
		if tree.PlatformBridge == "" {
			issues = append(
				issues,
				"accessibility_tree platform_bridge is required for headless platform-bridge-v1",
			)
		}
		if tree.PlatformHostIntegration || tree.LinuxPlatformProbe ||
			...
			issues = append(
				issues,
				("accessibility_tree headless platform bridge must not claim " +
					"linux platform probe or browser accessibility mirror"),
			)
		}
	default:
		issues = append(
			issues,
			fmt.Sprintf("accessibility_tree release target %q is unsupported", report.Target),
		)
	}
	return issues
}

func validateLinuxReleaseWindowAccessibilityBridgeEvidence(
	report Report,
	tree *AccessibilityTreeReport,
) []string {
	var issues []string
	...
	if tree.Experimental || !tree.ProductionClaim {
		issues = append(
			issues,
			"accessibility_tree linux release window requires experimental=false and production_claim=true",
		)
	}
	if !tree.PlatformHostIntegration || !tree.MetadataTree || !tree.PlatformExport {
		issues = append(
			issues,
			("accessibility_tree linux release window requires " +
				"platform_host_integration, metadata_tree, and " +
				"platform_export"),
		)
	}
	if tree.PlatformBridge != "linux_accessibility_host_bridge_v1" {
	...
	if !tree.LinuxPlatformProbe || strings.TrimSpace(tree.LinuxProbeArtifact) == "" {
		issues = append(
			issues,
			"accessibility_tree linux release window requires linux_platform_probe and linux_probe_artifact",
		)
	}
	screenReaderEvidence, ok := screenReaderEvidenceString(tree.ScreenReaderEvidence)
	...
	if tree.DOMARIAIntegration || !tree.NoDOMUI || !tree.NoUserJS || !tree.NoPlatformWidgets ||
		!tree.NoLegacySidecars {
		issues = append(
			issues,
			("accessibility_tree linux release window must not claim DOM " +
				"ARIA, DOM UI, user JS, platform widgets, or legacy sidecars"),
		)
	}
	if !tree.DerivedFromComponentTree || !tree.UsesComponentTreeAPI || !tree.UsesWidgetToolkit {
		issues = append(
			issues,
			("accessibility_tree linux release window must derive from " +
				"component_tree, component_tree_api, and toolkit evidence"),
		)
	}
	if tree.ComponentTreeSchema != "tetra.surface.component-tree.v1" ||
		...
		issues = append(
			issues,
			("accessibility_tree linux release window requires " +
				"component-tree, component-tree-api, and toolkit schemas"),
		)
	}
	if report.ComponentTree == nil || report.ComponentTree.DynamicLevel != "production-widgets-v1" {
		issues = append(
			issues,
			"accessibility_tree linux release window requires production-widgets-v1 component_tree evidence",
		)
	}
	if report.Toolkit == nil || report.Toolkit.ToolkitLevel != "production-widgets-v1" {
		issues = append(
			issues,
			"accessibility_tree linux release window requires production-widgets-v1 toolkit evidence",
		)
	}
	for _, role := range []string{"root", "textbox", "checkbox", "button", "status"} {
		if !contains(tree.RolesPresent, role) {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree linux release window roles_present missing %s",
					role,
				),
			)
		}
	...
	for _, focus := range []string{
		...
		if !contains(tree.FocusOrder, focus) {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree linux release window focus_order missing %s",
					focus,
				),
			)
		}
...
func validateAccessibilityToolkitEvidence(report Report) []string {
	...
	if toolkit.Schema != "tetra.surface.toolkit.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree toolkit schema is %q, want tetra.surface.toolkit.v1",
				toolkit.Schema,
			),
		)
	}
	if toolkit.ToolkitLevel != "toolkit-reuse-v1" ||
		toolkit.ReuseLevel != "multi-form-widget-reuse-v1" {
		issues = append(
			issues,
			"accessibility_tree toolkit must reuse toolkit-reuse-v1 multi-form widgets",
		)
	}
	if normalizeEvidencePath(toolkit.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree toolkit source %q must match report source %q",
				toolkit.Source,
				report.Source,
			),
		)
	...
	if toolkit.Module != "lib.core.widgets" {
		issues = append(
			issues,
			fmt.Sprintf(
				"accessibility_tree toolkit module is %q, want lib.core.widgets",
				toolkit.Module,
			),
		)
	}
	if !toolkit.Experimental || toolkit.ProductionClaim || !toolkit.UsesComponentTreeAPI ||
		toolkit.ManualBookkeeping ||
		toolkit.DemoSpecificWidgetStructs {
		issues = append(
			issues,
			("accessibility_tree toolkit must be experimental reusable " +
				"component-tree evidence without production/manual/demo " +
				"widget claims"),
		)
	}
	if !toolkit.NoMagicWidgets || !toolkit.NoPlatformWidgets || !toolkit.NoDOMUI ||
		!toolkit.NoUserJS {
		issues = append(
			issues,
			"accessibility_tree toolkit must prove no magic, platform, DOM, or user-JS widgets",
		)
	}
	if toolkit.ExampleCount < 3 ||
		!contains(
			toolkit.Sources,
		...
		issues = append(
			issues,
			"accessibility_tree toolkit example_count must include surface_accessibility_settings",
		)
	}
	if toolkit.TextBoxCount < 2 || toolkit.ButtonCount < 2 || !toolkit.MultiTextBoxEvidence ||
		!toolkit.MultiFormEvidence {
		issues = append(
			issues,
			"accessibility_tree toolkit requires multi TextBox and multi Button reuse evidence",
		)
	}
	...
	for _, required := range []struct {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree toolkit widget evidence missing %s", required.name),
			)
			continue
		}
		if widget.Kind != required.kind || widget.NodeID != required.nodeID {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree toolkit widget %s = %s/%d, want %s/%d",
					required.name,
					widget.Kind,
					widget.NodeID,
					required.kind,
		...
		if required.role != "" && widget.Role != required.role {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree toolkit widget %s role is %q, want %q",
					required.name,
					widget.Role,
					required.role,
				),
		...
		if required.action != "" && widget.Action != required.action {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree toolkit widget %s action is %q, want %q",
					required.name,
					widget.Action,
					required.action,
				),
		...
		if required.editable && !widget.Editable {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree toolkit widget %s must prove editable=true",
					required.name,
				),
			)
		}
		if !widget.Reusable || !widget.OrdinaryTetraStruct {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree toolkit widget %s must be reusable ordinary Tetra struct evidence",
					required.name,
				),
			)
		}
	...
	for _, source := range []string{
		...
		if !contains(toolkit.ReusableSources, source) {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree toolkit reusable_sources missing %s", source),
			)
		}
	}
	return issues
...
func validateAccessibilitySnapshots(snapshots []AccessibilitySnapshotReport) []string {
	...
	for _, snapshot := range snapshots {
		...
		if snapshot.Generation <= lastGeneration {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree snapshot %s generation = %d, want greater than %d",
					snapshot.Name,
					snapshot.Generation,
					lastGeneration,
				),
		...
		if !validChecksumLike(snapshot.BoundsChecksum) {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree snapshot %s bounds_checksum is invalid",
					snapshot.Name,
				),
			)
		}
		if !validChecksumLike(snapshot.MetadataChecksum) {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree snapshot %s metadata_checksum is invalid",
					snapshot.Name,
				),
			)
		}
		if !validChecksumLike(snapshot.FrameChecksum) {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree snapshot %s frame_checksum is invalid",
					snapshot.Name,
				),
			)
		}
	...
	for _, expected := range []struct {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("accessibility_tree snapshots missing %s", expected.name),
			)
			continue
		}
		if snapshot.Focused != expected.focused ||
			snapshot.FocusedComponentID != expected.componentID ||
			snapshot.FocusedAccessibilityNodeID != expected.nodeID {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree snapshot %s focused = %s/%d/%d, want %s/%d/%d",
					expected.name,
					snapshot.Focused,
					snapshot.FocusedComponentID,
					snapshot.FocusedAccessibilityNodeID,
					expected.focused,
		...
		if snapshot.NameValueLen != expected.nameLen ||
			snapshot.EmailValueLen != expected.emailLen {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree snapshot %s value lengths = %d/%d, want %d/%d",
					expected.name,
					snapshot.NameValueLen,
					snapshot.EmailValueLen,
					expected.nameLen,
					expected.emailLen,
				),
			)
		...
		if snapshot.StatusValue != expected.status {
			issues = append(
				issues,
				fmt.Sprintf(
					"accessibility_tree snapshot %s status_value = %q, want %q",
					expected.name,
	...
	if afterNameText, ok := byName["after_name_text"]; ok {
		if afterNameFocus, ok := byName["after_name_focus"]; ok &&
			afterNameText.MetadataChecksum == afterNameFocus.MetadataChecksum {
			issues = append(
				issues,
				"accessibility_tree metadata_checksum must change after_name_text",
			)
		}
	}
	if afterEmailText, ok := byName["after_email_text"]; ok {
		if afterEmailFocus, ok := byName["after_email_focus"]; ok &&
			afterEmailText.MetadataChecksum == afterEmailFocus.MetadataChecksum {
			issues = append(
				issues,
				"accessibility_tree metadata_checksum must change after_email_text",
			)
		}
	}
	if afterResize, ok := byName["after_resize"]; ok {
		if afterReset, ok := byName["after_reset"]; ok {
			if afterResize.BoundsChecksum == afterReset.BoundsChecksum {
				issues = append(
					issues,
					"accessibility_tree bounds_checksum must change after_resize",
				)
			}
			if afterResize.FrameChecksum == afterReset.FrameChecksum {
				issues = append(
					issues,
					"accessibility_tree frame_checksum must change after_resize",
				)
			}
		}
	}
...
func validateAccessibilityNegativeGuards(guards AccessibilityNegativeGuardsReport) []string {
	...
	return []string{
		fmt.Sprintf("accessibility_tree negative_guards missing %s", strings.Join(missing, ", ")),
	}
}

func hasAccessibilityRelationship(
...
func isAccessibilityMetadataReport(report Report) bool {
	if isSurfaceAccessibilitySettingsSource(report.Source) ||
		isSurfaceReleaseAccessibilitySource(report.Source) {
		return true
	}
	...
	return caseNameContains(report.Cases, "accessibility metadata tree") ||
		caseNameContains(report.Cases, "accessibility platform bridge")
}

func isSurfaceAccessibilitySettingsSource(source string) bool {
	return strings.HasSuffix(
		normalizeEvidencePath(source),
		"examples/surface/toolkit/surface_accessibility_settings.tetra",
	)
}
...
func isPlatformBridgeAccessibilityReport(report Report) bool {
	return report.AccessibilityTree != nil &&
		report.AccessibilityTree.AccessibilityLevel == "platform-bridge-v1"
}
...
func validateAppModelEvidence(report Report) []string {
	...
	if !isSurfaceAppModelSource(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"app_model source path must match examples/surface/toolkit/surface_app_model.tetra, got %q",
				report.Source,
			),
		)
	}
	if report.AppModel == nil {
		return append(
			issues,
			"app_model evidence is required for examples/surface/toolkit/surface_app_model.tetra",
		)
	}
	app := report.AppModel
	if app.Schema != "tetra.surface.app-model.v1" {
		issues = append(
			issues,
			fmt.Sprintf("app_model schema is %q, want tetra.surface.app-model.v1", app.Schema),
		)
	}
	if app.AppModelLevel != "explicit-command-reducer-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"app_model app_model_level is %q, want explicit-command-reducer-v1",
				app.AppModelLevel,
			),
		)
	}
	if app.ReleaseScope != "surface-v1-linux-web" {
		issues = append(
			issues,
			fmt.Sprintf(
				"app_model release_scope is %q, want surface-v1-linux-web",
				app.ReleaseScope,
			),
		)
	}
	if normalizeEvidencePath(app.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"app_model source %q must match report source %q",
				app.Source,
				report.Source,
			),
		)
	...
	if app.Module != "lib.core.surface_app" {
		issues = append(
			issues,
			fmt.Sprintf("app_model module is %q, want lib.core.surface_app", app.Module),
		)
	}
	if !app.UsesComponentTreeAPI || !app.CallerOwnedState || !app.ExplicitEventBindings ||
		!app.DeterministicReducer {
		issues = append(
			issues,
			("app_model requires component-tree API use, caller-owned " +
				"state, explicit event bindings, and deterministic reducer " +
				"evidence"),
		)
	}
	if app.HiddenAppState || app.ReactRuntime || app.ElectronRuntime || app.DOMRuntime ||
		app.DOMEventModel ||
		app.UserJS ||
		app.PlatformWidgets {
		issues = append(
			issues,
			("app_model must not claim hidden app state, " +
				"React/Electron/DOM runtime, DOM event model, user JS, or " +
				"platform widgets"),
		)
	}
	...
	issues = append(
		issues,
		validateAppModelEventBindings(
			app.EventBindings,
			eventByOrder,
			commandSet,
			bindingByEventCommand,
		)...)
	issues = append(
		issues,
		validateAppModelCommandDispatches(
			app.CommandDispatches,
			report.Components,
			commandSet,
			bindingByEventCommand,
		)...)
...
func validateAppModelEventBindings(
	bindings []AppModelEventBindingReport,
	eventByOrder map[int]EventReport,
	commandSet map[string]bool,
	bindingByEventCommand map[string]bool,
) []string {
	...
	for _, binding := range bindings {
		if binding.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model event binding order %d is not strictly greater than previous order %d",
					binding.Order,
					lastOrder,
				),
			)
		...
		if binding.EventOrder <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model event binding %d event_order must be positive",
					binding.Order,
				),
			)
		}
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model event binding %d references missing event order %d",
					binding.Order,
					binding.EventOrder,
				),
			)
			...
			if event.Kind != binding.EventKind || event.TargetComponent != binding.Target {
				issues = append(
					issues,
					fmt.Sprintf(
						"app_model event binding %d = %s/%s, want event %d %s/%s",
						binding.Order,
						binding.EventKind,
						binding.Target,
						event.Order,
			...
			if !stringSlicesEqual(binding.DispatchPath, event.DispatchPath) {
				issues = append(issues, fmt.Sprintf((("app_model event binding %d dispatch_path = %v, "+
					"want event ")+
					"path %v"), binding.Order, binding.DispatchPath, event.DispatchPath))
			}
		}
		if !binding.Explicit {
			issues = append(
				issues,
				fmt.Sprintf("app_model event binding %d must be explicit", binding.Order),
			)
		}
		if strings.TrimSpace(binding.Command) == "" || !commandSet[binding.Command] {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model event binding %d references unregistered command %q",
					binding.Order,
					binding.Command,
				),
			)
...
func validateAppModelCommandDispatches(
	dispatches []AppModelCommandDispatchReport,
	...
	for _, dispatch := range dispatches {
		if dispatch.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model command dispatch order %d is not strictly greater than previous order %d",
					dispatch.Order,
					lastOrder,
				),
			)
		...
		if strings.TrimSpace(dispatch.Command) == "" || !commandSet[dispatch.Command] {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model command dispatch %d references unregistered command %q",
					dispatch.Order,
					dispatch.Command,
				),
			)
		...
		if strings.TrimSpace(dispatch.Kind) == "" {
			issues = append(
				issues,
				fmt.Sprintf("app_model command dispatch %d kind is required", dispatch.Order),
			)
		}
		...
		if strings.TrimSpace(dispatch.Target) == "" || !componentSet[dispatch.Target] {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model command dispatch %d target %q is not in component evidence",
					dispatch.Order,
					dispatch.Target,
				),
			)
		...
		if !dispatch.Handled {
			issues = append(
				issues,
				fmt.Sprintf("app_model command dispatch %d must be handled", dispatch.Order),
			)
		}
		if len(dispatch.BeforeState) == 0 || len(dispatch.AfterState) == 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model command dispatch %d requires before_state and after_state",
					dispatch.Order,
				),
			)
		}
		if dispatch.EventOrder > 0 &&
			!bindingByEventCommand[appModelEventCommandKey(dispatch.EventOrder, dispatch.Command)] {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model command dispatch %d has no explicit event binding for event %d command %s",
					dispatch.Order,
					dispatch.EventOrder,
					dispatch.Command,
				),
		...
		if dispatch.Kind == "edit" && (!dispatch.Reversible || dispatch.HistoryIndex <= 0) {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model edit command dispatch %d requires reversible history evidence",
					dispatch.Order,
				),
			)
		}
		if strings.HasPrefix(dispatch.Kind, "async_") &&
			strings.TrimSpace(dispatch.AsyncTaskID) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model async command dispatch %d requires async_task_id",
					dispatch.Order,
				),
			)
		}
	...
	for _, requiredKind := range []string{"focus", "edit", "async_start", "async_complete"} {
		if !seenKinds[requiredKind] {
			issues = append(
				issues,
				fmt.Sprintf("app_model command dispatches missing %s kind", requiredKind),
			)
		}
	}
	return issues
...
func validateAppModelNavigation(
	transitions []AppModelNavigationReport,
	commandSet map[string]bool,
) []string {
	var issues []string
	...
	for _, transition := range transitions {
		if transition.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model navigation order %d is not strictly greater than previous order %d",
					transition.Order,
					lastOrder,
				),
			)
		...
		if !commandSet[transition.Command] {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model navigation %d references unregistered command %q",
					transition.Order,
					transition.Command,
				),
			)
		...
		default:
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model navigation %d operation is %q, want push, replace, or back",
					transition.Order,
		...
		if len(transition.StackBefore) == 0 || len(transition.StackAfter) == 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model navigation %d requires stack_before and stack_after",
					transition.Order,
				),
			)
		}
		if strings.TrimSpace(transition.BeforeRoute) == "" ||
			strings.TrimSpace(transition.AfterRoute) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model navigation %d requires before_route and after_route",
					transition.Order,
				),
			)
		}
		if transition.UnderflowRejected {
			seenUnderflow = true
			if transition.BeforeRoute != transition.AfterRoute ||
				!stringSlicesEqual(transition.StackBefore, transition.StackAfter) {
				issues = append(
					issues,
					fmt.Sprintf(
						"app_model navigation %d underflow rejection must preserve route and stack",
						transition.Order,
					),
				)
			}
	...
	if !seenPush || !seenBack || !seenUnderflow {
		issues = append(
			issues,
			"app_model navigation requires push, back, and underflow rejection evidence",
		)
	}
	return issues
}
	...
	for _, transition := range transitions {
		if transition.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model focus scope order %d is not strictly greater than previous order %d",
					transition.Order,
					lastOrder,
				),
			)
		...
		lastOrder = transition.Order
		if strings.TrimSpace(transition.Scope) == "" ||
			strings.TrimSpace(transition.AfterFocus) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model focus scope %d requires scope and after_focus",
					transition.Order,
				),
			)
		}
		if transition.Escaped {
			issues = append(
				issues,
				fmt.Sprintf("app_model focus scope %d escaped active scope", transition.Order),
			)
		}
...
func validateAppModelAsyncTasks(
	tasks []AppModelAsyncTaskReport,
	commandSet map[string]bool,
) []string {
	var issues []string
	if len(tasks) == 0 {
	...
	for _, task := range tasks {
		...
		if !commandSet[task.Command] {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model async task %s references unregistered command %q",
					task.ID,
					task.Command,
				),
			)
		...
		if len(task.BeforeState) == 0 || len(task.AfterState) == 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model async task %s requires before_state and after_state",
					task.ID,
				),
			)
		}
		...
		case "start":
			...
			if task.Status != "pending" || task.Canceled {
				issues = append(
					issues,
					fmt.Sprintf(
						"app_model async task %s start must be pending and not canceled",
						task.ID,
					),
				)
			}
		case "complete":
			...
			if task.Status != "completed" || task.Canceled || task.CompletionOrder <= 0 {
				issues = append(
					issues,
					fmt.Sprintf(
						"app_model async task %s completion requires completed status and completion_order",
						task.ID,
					),
				)
			}
		case "cancel":
			...
			if task.Status != "canceled" || !task.Canceled {
				issues = append(
					issues,
					fmt.Sprintf("app_model async task %s cancel must be canceled", task.ID),
				)
			}
			if appModelCanceledTaskMutatesBusinessState(task.BeforeState, task.AfterState) {
				issues = append(
					issues,
					fmt.Sprintf(
						"app_model async task %s canceled command must not mutate app state beyond pending_task",
						task.ID,
					),
				)
			}
		default:
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model async task %s operation is %q, want start, complete, or cancel",
					task.ID,
					task.Operation,
				),
			)
	...
	for id := range completed {
		if !started[id] {
			issues = append(
				issues,
				fmt.Sprintf("app_model async task %s completed without matching start", id),
			)
		}
	}
	if !seenCancel {
...
func validateAppModelUndoRedo(
	transitions []AppModelUndoRedoReport,
	commandSet map[string]bool,
) []string {
	var issues []string
	if len(transitions) == 0 {
	...
	for _, transition := range transitions {
		if transition.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model undo/redo order %d is not strictly greater than previous order %d",
					transition.Order,
					lastOrder,
				),
			)
		...
		if !commandSet[transition.Command] {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model undo/redo %d references unregistered command %q",
					transition.Order,
					transition.Command,
				),
			)
		...
		default:
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model undo/redo %d operation is %q, want record, undo, or redo",
					transition.Order,
					transition.Operation,
				),
			)
		...
		if transition.HistoryIndex <= 0 || !transition.MatchedHistoryEntry || !transition.Applied {
			issues = append(
				issues,
				fmt.Sprintf(
					"app_model undo/redo %d requires matched applied history entry",
					transition.Order,
				),
			)
		}
		if transition.Before == transition.After {
			issues = append(
				issues,
				fmt.Sprintf("app_model undo/redo %d must change value", transition.Order),
			)
		}
	}
	for _, required := range []string{"record", "undo", "redo"} {
		if !seen[required] {
			issues = append(
				issues,
				fmt.Sprintf("app_model undo_redo_transitions missing %s operation", required),
			)
		}
	}
	return issues
...
func validateAppModelNegativeGuards(guards AppModelNegativeGuardsReport) []string {
	...
	return []string{
		fmt.Sprintf("app_model negative_guards missing %s", strings.Join(missing, ", ")),
	}
}
...
func appModelCanceledTaskMutatesBusinessState(
...
// ---- block_accessibility_validation.go ----
...
func validateBlockAccessibilityEvidence(report Report) []string {
	...
	if !isSurfaceBlockAccessibilitySource(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree source path must match a Block accessibility/system example, got %q",
				report.Source,
			),
		)
	}
	if tree.Schema != "tetra.surface.block-accessibility-tree.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree schema is %q, want tetra.surface.block-accessibility-tree.v1",
				tree.Schema,
			),
		)
	}
	if tree.AccessibilityLevel != "block-metadata-tree-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree accessibility_level is %q, want block-metadata-tree-v1",
				tree.AccessibilityLevel,
			),
		)
	}
	if normalizeEvidencePath(tree.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree source %q must match report source %q",
				tree.Source,
				report.Source,
			),
		)
	...
	if tree.Module != "lib.core.block" {
		issues = append(
			issues,
			fmt.Sprintf("block_accessibility_tree module is %q, want lib.core.block", tree.Module),
		)
	}
	if tree.QualityLevel != "block-derived-accessibility-metadata-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree quality_level is %q, want block-derived-accessibility-metadata-v1",
				tree.QualityLevel,
			),
		)
	}
	if tree.BlockGraphSchema != "tetra.surface.block-graph.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree block_graph_schema is %q, want tetra.surface.block-graph.v1",
				tree.BlockGraphSchema,
			),
		)
	}
	if normalizeEvidencePath(graph.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree block_graph source %q must match report source %q",
				graph.Source,
				report.Source,
			),
		)
	...
	if !tree.DerivedFromBlockGraph {
		issues = append(
			issues,
			"block_accessibility_tree must declare derived_from_block_graph=true",
		)
	}
	if tree.ManualBookkeeping {
	...
	if tree.PlatformHostIntegration {
		issues = append(
			issues,
			("block_accessibility_tree platform_host_integration must be " +
				"false for metadata-only Block evidence"),
		)
	}
	if tree.DOMARIAIntegration {
	...
	if screenReaderEvidenceTruthy(tree.ScreenReaderEvidence) {
		issues = append(
			issues,
			("block_accessibility_tree screen_reader_evidence must be " +
				"false without platform assistive-tech proof"),
		)
	}
	if !tree.NoDOMUI || !tree.NoUserJS || !tree.NoPlatformWidgets {
		issues = append(
			issues,
			"block_accessibility_tree must prove no_dom_ui, no_user_js, and no_platform_widgets",
		)
	}
	if tree.NodeCount != len(tree.Nodes) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree node_count = %d, want len(nodes) %d",
				tree.NodeCount,
				len(tree.Nodes),
			),
		)
	...
	if !intSlicesEqual(tree.FocusOrder, expectedFocus) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree focus_order = %v, want block_graph focus_order %v",
				tree.FocusOrder,
				expectedFocus,
			),
		)
	...
	if !intSlicesEqual(tree.ReadingOrder, expectedReading) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree reading_order = %v, want block_graph accessibility_order %v",
				tree.ReadingOrder,
				expectedReading,
			),
		)
	...
	for _, node := range tree.Nodes {
		if node.ID != node.BlockID {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree node id %d must match block_id %d",
					node.ID,
					node.BlockID,
				),
			)
		...
		if strings.TrimSpace(node.Name) == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree node %d name is required", node.BlockID),
			)
		} else if _, exists := nodesByName[node.Name]; exists {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree duplicate node name %s", node.Name),
			)
		}
		nodesByName[node.Name] = node
		if _, exists := nodesByBlockID[node.BlockID]; exists {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree duplicate block_id %d", node.BlockID),
			)
		}
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree node %s block_id %d is not in block_graph",
					node.Name,
					node.BlockID,
				),
			)
			...
			if node.ParentBlockID != graphNode.ParentID {
				issues = append(issues, fmt.Sprintf((("block_accessibility_tree node %s parent_block_"+
					"id = %d, want ")+
					"block_graph parent_id %d"), node.Name, node.ParentBlockID, graphNode.ParentID))
			}
			if !rectsEqual(node.Bounds, graphNode.Bounds) {
				issues = append(issues, fmt.Sprintf((("block_accessibility_tree node %s bounds %+v do "+
					"not match ")+
					"block_graph bounds %+v"), node.Name, node.Bounds, graphNode.Bounds))
			}
			if normalizeAccessibilityRole(
				node.Role,
			) != normalizeAccessibilityRole(
				graphNode.AccessibilityRole,
			) {
				issues = append(issues, fmt.Sprintf(("block_accessibility_tree node %s role is %q, want "+
					"block_graph role %q"), node.Name, node.Role, graphNode.AccessibilityRole))
			}
			if node.Focusable != graphNode.Focusable {
				issues = append(issues, fmt.Sprintf(("block_accessibility_tree node %s focusable = %t, want "+
					"block_graph focusable %t"), node.Name, node.Focusable, graphNode.Focusable))
			}
		}
		...
		if role == "" || role == "none" {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree node %s role is required", node.Name),
			)
		}
		roleCounts[role]++
		if !containsNormalized(tree.RolesPresent, role) {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree roles_present missing %s", role),
			)
		}
		if node.Bounds.W <= 0 || node.Bounds.H <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree node %s bounds are required", node.Name),
			)
		}
		if !node.Visible || !node.Enabled {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree node %s must be visible and enabled",
					node.Name,
				),
			)
		}
		if node.Focusable {
			...
			if node.FocusIndex < 0 {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_accessibility_tree focusable node %s requires focus_index",
						node.Name,
					),
				)
			}
		} else if node.FocusIndex >= 0 {
			issues = append(issues, fmt.Sprintf((("block_accessibility_tree non-focusable node %s "+
				"focus_index ")+
				"= %d, want -1"), node.Name, node.FocusIndex))
		}
		if node.Focused {
			focusedCount++
		}
		if node.ReadingIndex < 0 {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree node %s requires reading_index", node.Name),
			)
		}
		if (node.Focusable || len(node.Actions) > 0) && strings.TrimSpace(node.Name) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree actionable focusable block %d requires accessible name",
					node.BlockID,
				),
			)
		}
		if node.Focusable && len(node.Actions) == 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree focusable node %s requires actions",
					node.Name,
				),
			)
		}
	...
	if tree.FocusableCount != focusableCount {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree focusable_count = %d, want %d",
				tree.FocusableCount,
				focusableCount,
			),
		)
	...
	if focusedCount > 1 {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_accessibility_tree focused node count = %d, want at most 1",
				focusedCount,
			),
		)
	}
	for _, blockID := range expectedReading {
		if _, ok := nodesByBlockID[blockID]; !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree missing block_graph accessibility node %d",
					blockID,
				),
			)
		}
	...
	for i, blockID := range expectedFocus {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree missing block_graph focus node %d", blockID),
			)
			continue
		}
		if node.FocusIndex != i {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree node %s focus_index = %d, want %d",
					node.Name,
					node.FocusIndex,
					i,
				),
	...
	for i, blockID := range expectedReading {
		...
		if node.ReadingIndex != i {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree node %s reading_index = %d, want %d",
					node.Name,
					node.ReadingIndex,
					i,
				),
	...
	issues = append(
		issues,
		validateBlockAccessibilityRelationships(tree.Relationships, nodesByName)...)
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree report requires %s evidence", required),
			)
		}
	}
	return issues
...
func validateBlockAccessibilityRelationships(
	relationships []AccessibilityRelationshipReport,
	nodes map[string]BlockAccessibilityNodeReport,
) []string {
	...
	for _, relationship := range relationships {
		...
		if !fromOK {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree relationship from %q is not a node",
					relationship.From,
				),
			)
			continue
		...
		if !toOK {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree relationship to %q is not a node",
					relationship.To,
				),
			)
			continue
		...
		case "label_for":
			if from.LabelFor != to.Name || to.LabelledBy != from.Name {
				issues = append(
					issues,
					fmt.Sprintf(
						("block_accessibility_tree label relationship mismatch: %s "+
							"label_for %s must match reciprocal labelled_by"),
						from.Name,
						to.Name,
					),
				)
		...
		case "labelled_by":
			if from.LabelledBy != to.Name || to.LabelFor != from.Name {
				issues = append(
					...
					fmt.Sprintf(
						("block_accessibility_tree label relationship mismatch: %s "+
							"labelled_by %s must match reciprocal label_for"),
						from.Name,
						to.Name,
					),
				)
		...
		default:
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree relationship kind %q is unsupported",
					relationship.Kind,
				),
			)
		}
...
func validateBlockAccessibilityActions(
	actions []AccessibilityActionReport,
	nodes map[string]BlockAccessibilityNodeReport,
) []string {
	if len(actions) == 0 {
	...
	for _, action := range actions {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree action target %q is not a node",
					action.Target,
				),
			)
			continue
		...
		if !contains(node.Actions, action.Action) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_accessibility_tree action %s missing from node %s actions",
					action.Action,
					action.Target,
				),
			)
		...
		if strings.TrimSpace(action.Semantic) == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_accessibility_tree action %s requires semantic", action.Target),
			)
		}
	}
	return issues
...
func validateBlockAccessibilityNegativeGuards(
	guards BlockAccessibilityNegativeGuardsReport,
) []string {
	var missing []string
	...
	return []string{
		fmt.Sprintf(
			"block_accessibility_tree negative_guards missing %s",
...
func isSurfaceBlockAccessibilitySource(source string) bool {
	...
	return strings.HasSuffix(
		...
		strings.HasSuffix(source, "examples/surface/block_core/surface_block_system.tetra") ||
		strings.HasSuffix(
			source,
...
func validateBlockAssetEvidence(report Report) []string {
	...
	if report.BlockAssetQualityLevel != "deterministic-local-block-assets-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_asset_quality_level is %q, want deterministic-local-block-assets-v1",
	...
	if report.BlockAssetManifest == nil {
		...
		if manifest.Schema != "tetra.surface.block-assets.v1" {
			issues = append(issues, fmt.Sprintf(("block_asset_manifest schema is %q, want "+
				"tetra.surface.block-assets.v1"), manifest.Schema))
		}
		if manifest.Quality != "deterministic-local-block-assets-v1" {
			issues = append(issues, fmt.Sprintf(("block_asset_manifest quality is %q, want "+
				"deterministic-local-block-assets-v1"), manifest.Quality))
		}
		if manifest.HashAlgorithm != "sha256" {
			issues = append(
				issues,
				fmt.Sprintf("block_asset_manifest hash_algorithm is %q, want sha256", manifest.HashAlgorithm),
			)
		}
		...
		for i, asset := range manifest.Assets {
			...
			if !validBlockAssetKind(kind) {
				issues = append(issues, fmt.Sprintf((("block_asset_manifest asset %q kind is %q, want "+
					"font, icon, ")+
					"or image"), id, asset.Kind))
			}
			kindCounts[kind]++
			...
			if isNetworkAssetPath(asset.Path) {
				issues = append(
					issues,
					fmt.Sprintf("block_asset_manifest asset %q uses network path %q", id, asset.Path),
				)
			}
			if !asset.Local && !asset.Embedded {
				issues = append(
					issues,
					fmt.Sprintf("block_asset_manifest asset %q must be local or embedded", id),
				)
			}
			...
			if kind == "font" && strings.TrimSpace(asset.Family) == "" {
				issues = append(
					issues,
					fmt.Sprintf("block_asset_manifest font asset %q family is required", id),
				)
			}
			if (kind == "icon" || kind == "image") && (asset.Width <= 0 || asset.Height <= 0) {
				issues = append(issues, fmt.Sprintf(("block_asset_manifest %s asset %q width/height must be "+
					"positive"), kind, id))
			}
		}
	...
	if normalizeStateToken(cache.Strategy) != "bounded_lru" {
		issues = append(
			issues,
			fmt.Sprintf("block_asset_cache strategy is %q, want bounded-lru", cache.Strategy),
		)
	}
	if !cache.Bounded {
	...
	if cache.BudgetBytes <= 0 || cache.BudgetBytes > 1<<20 {
		issues = append(
			issues,
			fmt.Sprintf("block_asset_cache budget_bytes = %d, want 1..1048576", cache.BudgetBytes),
		)
	}
	...
	if cache.RepeatedLoads <= cache.EntryCount {
		issues = append(
			issues,
			"block_asset_cache repeated_loads must exceed entry_count to prove reuse",
		)
	}
	...
	for i, diagnostic := range report.BlockAssetDiagnostics {
		if diagnostic.Order <= lastDiagnosticOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_asset_diagnostics order %d is not strictly greater than previous order %d",
					diagnostic.Order,
					lastDiagnosticOrder,
				),
			)
		...
		if strings.TrimSpace(diagnostic.AssetID) == "" ||
			strings.TrimSpace(diagnostic.Kind) == "" ||
			strings.TrimSpace(diagnostic.Code) == "" ||
			strings.TrimSpace(diagnostic.Message) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_asset_diagnostics[%d] asset_id, kind, code, and message are required",
					i,
				),
			)
		}
		...
		if code == "missing_asset_fallback" {
			...
			if strings.TrimSpace(diagnostic.FallbackID) == "" {
				issues = append(
					issues,
					"block_asset_diagnostics missing asset fallback requires fallback_id",
				)
			}
		}
		if code == "network_asset_rejected" {
			...
			if !isNetworkAssetPath(diagnostic.RejectedURL) {
				issues = append(
					issues,
					"block_asset_diagnostics network rejection requires rejected_url",
				)
			}
		}
	}
	...
	if !hasNetworkDiagnostic {
		issues = append(
			issues,
			"block_asset_diagnostics require network asset rejection diagnostic",
		)
	}
	...
	for i, command := range report.BlockAssetRenderCommands {
		if command.Order <= lastCommandOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_asset_render_commands order %d is not strictly greater than previous order %d",
					command.Order,
					lastCommandOrder,
				),
			)
		...
		if strings.TrimSpace(command.AssetID) == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_asset_render_commands[%d] asset_id is required", i),
			)
		}
		if command.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("block_asset_render_commands[%d] block_id must be positive", i),
			)
		}
		if command.Rect.W <= 0 || command.Rect.H <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("block_asset_render_commands[%d] rect dimensions must be positive", i),
			)
		}
		if strings.TrimSpace(command.Quality) == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_asset_render_commands[%d] quality is required", i),
			)
		}
		if !validChecksumLike(command.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("block_asset_render_commands[%d] checksum must be sha256 evidence", i),
			)
		}
		...
		if name == "tint_icon" {
			issues = append(issues, validateRasterProof(
				fmt.Sprintf("block_asset_render_commands[%d]", i),
		...
		if name == "scale_image" && command.Scale < 2 {
			issues = append(
				issues,
				"block_asset_render_commands scale_image requires scale evidence",
			)
		}
	}
	for _, required := range []string{"load_font", "tint_icon", "scale_image", "fallback_missing"} {
		if !commands[required] {
			issues = append(
				issues,
				fmt.Sprintf("block_asset_render_commands require %s command", required),
			)
		}
	}
	...
	for _, requirement := range []struct {
		...
		if !hasTransition(report.StateTransitions, requirement.component, requirement.field) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block asset evidence requires %s %s state transition",
	...
	if len(report.Frames) < 2 || strings.TrimSpace(report.Frames[0].Checksum) == "" ||
		report.Frames[0].Checksum == report.Frames[1].Checksum {
		issues = append(
			issues,
			"block asset frame checksum evidence must show asset-driven visual change",
		)
	}
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("block asset report requires %s evidence", required),
			)
		}
	}
	return issues
...
func validateBlockEventFocusEvidence(report Report) []string {
	...
	if report.BlockEventQualityLevel != "deterministic-block-events-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_event_quality_level is %q, want deterministic-block-events-v1",
				report.BlockEventQualityLevel,
			),
		)
	}
	if report.BlockEventPolicy != "capture-bubble-direct-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_event_policy is %q, want capture-bubble-direct-v1",
				report.BlockEventPolicy,
			),
		)
	}
	...
	for _, route := range report.BlockEventRoutes {
		if route.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_event_routes order %d is not strictly greater than previous order %d",
					route.Order,
					lastOrder,
				),
			)
		...
		if kind == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_event_routes[%d] kind is required", route.Order),
			)
		}
		if !validBlockEventKind(kind) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_event_routes[%d] kind is %q, want supported Block event kind",
					route.Order,
					route.Kind,
				),
			)
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_event_routes[%d] target_id %d is not in block_graph",
					route.Order,
					route.TargetID,
				),
			)
		...
		if strings.TrimSpace(route.TargetName) == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_event_routes[%d] target_name is required", route.Order),
			)
		} else if route.TargetName != node.Name {
			issues = append(issues, fmt.Sprintf((("block_event_routes[%d] target_name is %q, want "+
				"block_graph ")+
				"node name %q"), route.Order, route.TargetName, node.Name))
		}
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_event_routes[%d] target_id %d is not reachable from root",
					route.Order,
					route.TargetID,
				),
			)
		...
		if !intSlicesEqual(route.DispatchPath, wantPath) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_event_routes[%d] dispatch_path = %v, want %v",
					route.Order,
					route.DispatchPath,
					wantPath,
				),
		...
		if len(route.HitTestPath) > 0 {
			if !intSlicesEqual(route.HitTestPath, wantPath) {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_event_routes[%d] hit_test_path = %v, want %v",
						route.Order,
						route.HitTestPath,
						wantPath,
					),
		...
		if route.DirectTargetID != route.TargetID {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_event_routes[%d] direct_target_id = %d, want target_id %d",
					route.Order,
					route.DirectTargetID,
					route.TargetID,
				),
			)
		}
		if route.Delivered == route.Rejected {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_event_routes[%d] must be exactly one of delivered or rejected",
					route.Order,
				),
			)
		}
		if route.Rejected && strings.TrimSpace(route.RejectReason) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_event_routes[%d] rejected route requires reject_reason",
					route.Order,
				),
			)
		}
		if strings.TrimSpace(route.Policy) == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_event_routes[%d] policy is required", route.Order),
			)
		}
		if normalizeEventPolicy(route.Policy) == "capture-bubble-direct-v1" {
			if !blockEventCapturePathMatches(route.CapturePath, wantPath) {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_event_routes[%d] capture_path = %v, want ancestors",
						route.Order,
						route.CapturePath,
					),
				)
			...
			if !blockEventBubblePathMatches(route.BubblePath, wantPath) {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_event_routes[%d] bubble_path = %v, want reverse ancestors",
						route.Order,
						route.BubblePath,
					),
				)
		...
		if kind == "click" && route.Disabled && route.Rejected && !route.Delivered &&
			strings.Contains(reason, "disabled") {
			hasDisabledReject = true
		}
		if kind == "text" && route.Editable && route.Rejected && !route.Delivered &&
			strings.Contains(reason, "unfocused") &&
			route.FocusedID != route.TargetID {
			hasUnfocusedTextReject = true
		}
		if kind == "text" && route.Editable && route.Delivered && !route.Rejected &&
			route.FocusedID == route.TargetID &&
			route.TextLen > 0 &&
			strings.TrimSpace(route.TextBytesHex) != "" {
			payload, err := hex.DecodeString(route.TextBytesHex)
			if err != nil {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_event_routes[%d] text_bytes_hex is not valid hex",
						route.Order,
					),
				)
			} else if len(payload) != route.TextLen {
				issues = append(issues, fmt.Sprintf((("block_event_routes[%d] text_len = %d, want "+
					"decoded payload ")+
					"length %d"), route.Order, route.TextLen, len(payload)))
			}
	...
	if !hasFocusedTextDeliver {
		issues = append(
			issues,
			"block_event_routes require focused editable text delivery evidence",
		)
	}
	...
	for _, transition := range report.BlockFocusTransitions {
		if transition.Order <= lastFocusOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_focus_transitions order %d is not strictly greater than previous order %d",
					transition.Order,
					lastFocusOrder,
				),
			)
		...
		if transition.Helper != "tree_focus_next" && transition.Helper != "tree_focus_prev" {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_focus_transitions[%d] helper is %q, want tree_focus_next or tree_focus_prev",
					transition.Order,
					transition.Helper,
				),
			)
		...
		if !transition.GraphDerived {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_focus_transitions[%d] must prove graph_derived",
					transition.Order,
				),
			)
		}
		if !containsInt(expectedFocus, transition.BeforeID) ||
			!containsInt(expectedFocus, transition.AfterID) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_focus_transitions[%d] before/after must be in block_graph focus_order %v",
					transition.Order,
					expectedFocus,
				),
			)
		...
		if transition.Wrapped && len(expectedFocus) >= 2 {
			if transition.BeforeID != expectedFocus[len(expectedFocus)-1] ||
				transition.AfterID != expectedFocus[0] {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_focus_transitions[%d] wrap = %d -> %d, want %d -> %d",
						transition.Order,
						transition.BeforeID,
						transition.AfterID,
						expectedFocus[len(expectedFocus)-1],
						expectedFocus[0],
					),
				)
			}
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("block_event report requires %s evidence", required),
			)
		}
	}
	return issues
...
func validateBlockGraphEvidence(report Report) []string {
	...
	if graph.Schema != "tetra.surface.block-graph.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph schema is %q, want tetra.surface.block-graph.v1",
				graph.Schema,
			),
		)
	}
	if graph.APILevel != "block-tree-builder-v1" {
		issues = append(
			issues,
			fmt.Sprintf("block_graph api_level is %q, want block-tree-builder-v1", graph.APILevel),
		)
	}
	if normalizeEvidencePath(graph.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph source %q must match report source %q",
				graph.Source,
				report.Source,
			),
		)
	...
	if graph.Builder.RootCreatedBy != "tree_add_root" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph builder root_created_by is %q, want tree_add_root",
				graph.Builder.RootCreatedBy,
			),
		)
	}
	if graph.Builder.ChildrenCreatedBy != "tree_add_child" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph builder children_created_by is %q, want tree_add_child",
				graph.Builder.ChildrenCreatedBy,
			),
		)
	}
	if graph.Builder.NodeCount != graph.NodeCount {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph builder node_count = %d, want block_graph node_count %d",
				graph.Builder.NodeCount,
				graph.NodeCount,
			),
		)
	}
	if graph.Builder.Capacity < graph.NodeCount {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph builder capacity = %d, want at least node_count %d",
				graph.Builder.Capacity,
				graph.NodeCount,
			),
		)
	}
	...
	if graph.Invariants.TreeValidateStatus != 0 {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph tree_validate_status = %d, want 0",
				graph.Invariants.TreeValidateStatus,
			),
		)
	}
	...
	if !graph.Invariants.ParentChildLinksChecked || !graph.Invariants.ChildOrderChecked ||
		!graph.Invariants.FocusOrderChecked ||
		!graph.Invariants.HitTestPathChecked ||
		!graph.Invariants.AccessibilityChecked {
		issues = append(
			issues,
			("block_graph invariants must check parent/child links, child " +
				"order, focus order, hit-test path, and accessibility order"),
		)
	}
	if graph.NodeCount != len(graph.Nodes) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph node_count = %d, want len(nodes) %d",
				graph.NodeCount,
				len(graph.Nodes),
			),
		)
	...
	if graph.NodeCount < 5 {
		issues = append(
			issues,
			fmt.Sprintf("block_graph node_count = %d, want at least 5", graph.NodeCount),
		)
	}
	...
	for _, node := range graph.Nodes {
		...
		if node.Bounds.W < 0 || node.Bounds.H < 0 {
			issues = append(
				issues,
				fmt.Sprintf("block_graph node %d bounds must be non-negative", node.ID),
			)
		}
		if node.ChildCount < 0 {
			issues = append(
				issues,
				fmt.Sprintf("block_graph node %d child_count must be non-negative", node.ID),
			)
		}
		if node.ChildCount == 0 && node.FirstChild != -1 {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_graph leaf node %d first_child = %d, want -1",
					node.ID,
					node.FirstChild,
				),
			)
	...
	if !ok {
		...
		issues = append(
			issues,
			fmt.Sprintf("block_graph root %d parent_id = %d, want -1", root.ID, root.ParentID),
		)
	}
	...
	for _, node := range graph.Nodes {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("block_graph node %d parent_id %d is unknown", node.ID, node.ParentID),
			)
			continue
		}
		if !rectContainsRect(parent.Bounds, node.Bounds) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_graph node %d bounds must be inside parent %d bounds",
					node.ID,
					parent.ID,
				),
			)
		...
		if _, ok := blockGraphPathToRoot(node.ID, nodes); !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_graph node %d has missing parent or cycle in root path",
					node.ID,
				),
			)
		}
	...
	for parentID, children := range childrenByParent {
		...
		if parent.ChildCount != len(children) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_graph node %d child_count = %d, want %d",
					parentID,
					parent.ChildCount,
					len(children),
				),
		...
		for _, child := range children {
			if child.ChildIndex < 0 || child.ChildIndex >= len(children) {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_graph child node %d child_index = %d, want 0..%d",
						child.ID,
						child.ChildIndex,
						len(children)-1,
					),
			...
			if prev, exists := seenIndex[child.ChildIndex]; exists {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_graph sibling child_index %d is used by nodes %d and %d",
						child.ChildIndex,
						prev,
						child.ID,
					),
		...
		if len(expectedChildren) > 0 && parent.FirstChild != expectedChildren[0] {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_graph node %d first_child = %d, want %d",
					parentID,
					parent.FirstChild,
					expectedChildren[0],
				),
		...
		if !hasBlockGraphChildOrder(graph.ChildOrders, parentID, expectedChildren) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_graph child_orders require parent %d children %v",
					parentID,
					expectedChildren,
				),
			)
	...
	if !intSlicesEqual(graph.FocusOrder, expectedFocus) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph focus_order = %v, want focusable node order %v",
				graph.FocusOrder,
				expectedFocus,
			),
		)
	...
	if !intSlicesEqual(graph.AccessibilityOrder, expectedAccessibility) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_graph accessibility_order = %v, want accessible node order %v",
				graph.AccessibilityOrder,
	...
	issues = append(
		issues,
		validateBlockGraphPaths("hit_tests", "tree_hit_test_path", graph.HitTests, nodes)...)
	issues = append(
		issues,
		validateBlockGraphPaths(
			"dispatch_paths",
			"tree_build_dispatch_path",
			graph.DispatchPaths,
			nodes,
		)...)
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("block_graph report requires %s evidence", required),
			)
		}
	}
	return issues
...
func hasBlockGraphChildOrder(
	orders []BlockGraphChildOrderReport,
	parentID int,
	expected []int,
) bool {
...
func validateBlockGraphPaths(
	field string,
	helper string,
	paths []BlockGraphPathReport,
	nodes map[int]BlockGraphNodeReport,
) []string {
	if len(paths) == 0 {
	...
	for _, path := range paths {
		if path.Helper != helper {
			issues = append(
				issues,
				fmt.Sprintf("block_graph %s helper is %q, want %s", field, path.Helper, helper),
			)
		}
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_graph %s target_id %d is not reachable from root",
					field,
					path.TargetID,
				),
			)
		...
		if !intSlicesEqual(path.Path, wantPath) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_graph %s target_id %d path = %v, want %v",
					field,
					path.TargetID,
					path.Path,
					wantPath,
...
func validateBlockLayoutEvidence(report Report) []string {
	...
	if report.LayoutQualityLevel != "deterministic-block-layout-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"layout_quality_level is %q, want deterministic-block-layout-v1",
				report.LayoutQualityLevel,
			),
		)
	}
	...
	for _, constraint := range report.LayoutConstraints {
		...
		if constraint.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("layout_constraints %q block_id must be positive", id),
			)
		}
		if !validLayoutMode(constraint.Mode) {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_constraints %q mode is %q, want supported Block layout mode",
					id,
					constraint.Mode,
				),
			)
		...
		if !validLayoutPolicy(widthPolicy) {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_constraints %q width_policy is %q, want fixed, fit, or fill",
					id,
					constraint.WidthPolicy,
				),
			)
		}
		if !validLayoutPolicy(heightPolicy) {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_constraints %q height_policy is %q, want fixed, fit, or fill",
					id,
					constraint.HeightPolicy,
				),
			)
		}
		...
		if constraint.Padding < 0 || constraint.Margin < 0 || constraint.Gap < 0 {
			issues = append(
				issues,
				fmt.Sprintf("layout_constraints %q spacing values must be non-negative", id),
			)
		}
		if constraint.Padding > 0 || constraint.Margin > 0 || constraint.Gap > 0 {
			hasSpacing = true
		...
		if !validLayoutAlign(constraint.Align) {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_constraints %q align is %q, want start, center, end, or stretch",
					id,
					constraint.Align,
				),
			)
		...
		if !validLayoutJustify(constraint.Justify) {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_constraints %q justify is %q, want start, center, end, or space-between",
					id,
					constraint.Justify,
				),
			)
		...
		if !validLayoutOverflow(constraint.Overflow) {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_constraints %q overflow is %q, want visible, clip, or scroll",
					id,
					constraint.Overflow,
				),
			)
		...
		if constraint.Clip || normalizeLayoutToken(constraint.Overflow) == "clip" ||
			normalizeLayoutToken(constraint.Overflow) == "scroll" {
			hasClip = true
		}
	...
	for _, policy := range []string{"fixed", "fit", "fill"} {
		if !hasPolicy[policy] {
			issues = append(
				issues,
				fmt.Sprintf("layout_constraints require %s sizing policy evidence", policy),
			)
		}
	}
	if !hasMin {
	...
	for i, pass := range report.LayoutPasses {
		if pass.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_passes order %d is not strictly greater than previous order %d",
					pass.Order,
					lastOrder,
				),
			)
		...
		if !validLayoutMode(mode) {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_passes[%d] mode is %q, want supported Block layout mode",
					i,
					pass.Mode,
				),
			)
		...
		if pass.Resolved.W <= 0 || pass.Resolved.H <= 0 || pass.Measured.W <= 0 ||
			pass.Measured.H <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_passes[%d] resolved and measured dimensions must be positive",
					i,
				),
			)
		}
		...
		if !validChecksumLike(pass.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("layout_passes[%d] checksum must be sha256 evidence", i),
			)
		}
	...
	for i, scroll := range report.LayoutScrolls {
		...
		if scroll.Viewport.W <= 0 || scroll.Viewport.H <= 0 || scroll.Content.W <= 0 ||
			scroll.Content.H <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_scrolls[%d] viewport and content dimensions must be positive",
					i,
				),
			)
		}
		if scroll.Content.H <= scroll.Viewport.H {
			issues = append(
				issues,
				fmt.Sprintf(
					"layout_scrolls[%d] content.h must exceed viewport.h for scroll evidence",
					i,
				),
			)
		}
		if scroll.OffsetY < 0 || scroll.MaxOffsetY < 0 || scroll.OffsetY > scroll.MaxOffsetY {
			issues = append(
				issues,
				fmt.Sprintf("layout_scrolls[%d] offset_y must be within max_offset_y", i),
			)
		}
		if !scroll.Clipped {
		...
		if !validChecksumLike(scroll.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("layout_scrolls[%d] checksum must be sha256 evidence", i),
			)
		}
		if scroll.Clipped && scroll.Content.H > scroll.Viewport.H &&
			scroll.OffsetY <= scroll.MaxOffsetY {
			hasScroll = true
		}
	}
	if !hasScroll {
	...
	if !hasTransition(report.StateTransitions, "BlockLayoutApp", "width") ||
		!hasTransition(report.StateTransitions, "ScrollBlock", "scroll_y") {
...
func validateBlockLayoutDensityEvidence(density BlockLayoutDensityReport) []string {
	...
	if density.TargetDPI < 96 {
		issues = append(
			issues,
			fmt.Sprintf("layout_density target_dpi is %d, want >= 96", density.TargetDPI),
		)
	}
	if density.ScaleMilli < 1000 || density.ScaleMilli > 4000 {
		issues = append(
			issues,
			fmt.Sprintf("layout_density scale_milli is %d, want 1000..4000", density.ScaleMilli),
		)
	}
	...
	if normalizeLayoutToken(density.RoundingPolicy) != "integer_half_up_v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"layout_density rounding_policy is %q, want integer-half-up-v1",
				density.RoundingPolicy,
			),
		)
	}
	...
	for _, breakpoint := range []string{"small", "medium", "large"} {
		if !layoutFeatureContains(density.Breakpoints, breakpoint) {
			issues = append(
				issues,
				fmt.Sprintf("layout_density breakpoints require %s", breakpoint),
			)
		}
	}
...
func validateBlockMotionEvidence(report Report) []string {
	...
	if report.MotionQualityLevel != "deterministic-block-motion-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"motion_quality_level is %q, want deterministic-block-motion-v1",
	...
	if report.MotionClock != "deterministic-test-clock-v1" {
		issues = append(
			issues,
			fmt.Sprintf("motion_clock is %q, want deterministic-test-clock-v1", report.MotionClock),
		)
	}
	...
	if report.MotionFrameBudget <= 0 || report.MotionFrameBudget > 16 {
		issues = append(
			issues,
			fmt.Sprintf("motion_frame_budget = %d, want 1..16", report.MotionFrameBudget),
		)
	}
	...
	if report.MotionFrameBudget > 0 && len(report.MotionFrames) > report.MotionFrameBudget {
		issues = append(
			issues,
			fmt.Sprintf(
				"motion_frames length %d exceeds motion_frame_budget %d",
				len(report.MotionFrames),
				report.MotionFrameBudget,
			),
		)
	}
	...
	for i := range report.MotionFrames {
		...
		if frame.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"motion_frames order %d is not strictly greater than previous order %d",
					frame.Order,
					lastOrder,
				),
			)
		...
		if frame.TimestampMS < lastTimestamp {
			issues = append(
				issues,
				fmt.Sprintf(
					"motion_frames[%d] timestamp_ms %d is before previous timestamp %d",
					frame.Order,
					frame.TimestampMS,
					lastTimestamp,
				),
		...
		if frame.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("motion_frames[%d] block_id must be positive", frame.Order),
			)
		}
		if strings.TrimSpace(frame.Trigger) == "" {
			issues = append(
				issues,
				fmt.Sprintf("motion_frames[%d] trigger is required", frame.Order),
			)
		}
		if frame.DurationMS <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("motion_frames[%d] duration_ms must be positive", frame.Order),
			)
		}
		if frame.DelayMS < 0 {
			issues = append(
				issues,
				fmt.Sprintf("motion_frames[%d] delay_ms must be non-negative", frame.Order),
			)
		}
		if frame.Progress < 0 || frame.Progress > 1000 {
			issues = append(
				issues,
				fmt.Sprintf(
					"motion_frames[%d] progress = %d, want 0..1000",
					frame.Order,
					frame.Progress,
				),
			)
		...
		if normalizeStateToken(frame.Easing) == "" {
			issues = append(
				issues,
				fmt.Sprintf("motion_frames[%d] easing is required", frame.Order),
			)
		}
		if frame.Opacity <= 0 || frame.Opacity > 255 {
			issues = append(
				issues,
				fmt.Sprintf(
					"motion_frames[%d] opacity = %d, want 1..255",
					frame.Order,
					frame.Opacity,
				),
			)
		...
		if frame.Scale <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("motion_frames[%d] scale must be positive", frame.Order),
			)
		}
		if !validChecksumLike(frame.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("motion_frames[%d] checksum must be sha256 evidence", frame.Order),
			)
		}
		if frame.Settled && frame.Scheduled {
			issues = append(
				issues,
				fmt.Sprintf(
					"motion_frames[%d] settled frame must not keep scheduling",
					frame.Order,
				),
			)
		}
		if frame.ReducedMotion {
			if frame.Progress != 1000 || frame.Scheduled || !frame.Settled {
				issues = append(
					issues,
					fmt.Sprintf(
						"motion_frames[%d] reduced motion must instantly settle without scheduling",
						frame.Order,
					),
				)
			}
		...
		if frame.Progress == 1000 && (!frame.Settled || frame.Scheduled) {
			issues = append(
				issues,
				fmt.Sprintf(
					"motion_frames[%d] completed motion must be settled and stop scheduling",
					frame.Order,
				),
			)
		}
	...
	if startFrame == nil || midFrame == nil || finalFrame == nil {
		issues = append(
			issues,
			"motion_frames require start, interpolated middle, and settled final frames",
		)
	}
	if reducedFrame == nil {
	...
	if startFrame != nil && midFrame != nil && finalFrame != nil {
		if startFrame.Opacity == finalFrame.Opacity || midFrame.Opacity == startFrame.Opacity ||
			midFrame.Opacity == finalFrame.Opacity {
			issues = append(issues, "motion_frames require opacity interpolation evidence")
		}
		if startFrame.Color == finalFrame.Color || midFrame.Color == startFrame.Color ||
			midFrame.Color == finalFrame.Color {
			issues = append(issues, "motion_frames require color interpolation evidence")
		}
		if startFrame.Scale == finalFrame.Scale || midFrame.Scale == startFrame.Scale ||
			midFrame.Scale == finalFrame.Scale {
			issues = append(issues, "motion_frames require scale interpolation evidence")
		}
		if startFrame.TranslateX == finalFrame.TranslateX ||
			midFrame.TranslateX == startFrame.TranslateX ||
			midFrame.TranslateX == finalFrame.TranslateX {
	...
	for _, field := range []string{
		...
		if !hasTransition(report.StateTransitions, "MotionBlock", field) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block motion evidence requires MotionBlock %s state transition",
					field,
				),
			)
		}
	...
	if isWASM32WebBrowserCanvasMorphRuntimeReport(report) ||
		isLinuxX64RealWindowMorphRuntimeReport(report) {
		if len(report.Frames) < 2 || strings.TrimSpace(report.Frames[0].Checksum) == "" ||
			report.Frames[0].Checksum == report.Frames[1].Checksum {
			issues = append(
				issues,
				"block motion frame checksum evidence must show motion-driven visual changes",
			)
		}
	} else if len(
		report.Frames,
	) < 3 || strings.TrimSpace(
		report.Frames[0].Checksum,
	) == "" || report.Frames[0].Checksum == report.Frames[1].Checksum || report.Frames[1].Checksum == report.Frames[2].Checksum {
		issues = append(issues, ("block motion frame checksum evidence must show " +
			"motion-driven visual changes"))
	}
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("block motion report requires %s evidence", required),
			)
		}
	}
	return issues
...
func validateBlockPaintEvidence(report Report) []string {
	...
	if report.PaintQualityLevel != "deterministic-software-paint-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"paint_quality_level is %q, want deterministic-software-paint-v1",
				report.PaintQualityLevel,
			),
		)
	}
	if report.PaintCacheBudgetBytes <= 0 || report.PaintCacheBudgetBytes > 1024*1024 {
		issues = append(
			issues,
			fmt.Sprintf(
				"paint_cache_budget_bytes = %d, want 1..1048576",
				report.PaintCacheBudgetBytes,
			),
		)
	}
	...
	for _, layer := range report.PaintLayers {
		...
		if layer.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("paint_layers %q block_id must be positive", layer.ID),
			)
		}
		if kind == "" {
		...
		if kind == "radius_clip" && layer.Radius <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("paint_layers %q radius must be positive for radius_clip", layer.ID),
			)
		}
		if (kind == "border" || kind == "outline") && layer.Width <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("paint_layers %q width must be positive for %s", layer.ID, kind),
			)
		}
		if kind == "shadow" && layer.Blur <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"paint_layers %q blur must be positive for shadow approximation",
					layer.ID,
				),
			)
		}
		if kind == "overlay" && (layer.Opacity <= 0 || layer.Opacity > 255) {
			issues = append(
				issues,
				fmt.Sprintf("paint_layers %q opacity must be 1..255 for overlay", layer.ID),
			)
		}
		if kind == "blur" || kind == "backdrop_blur" {
			issues = append(
				issues,
				"paint_layers unsupported blur/backdrop_blur layer is not allowed",
			)
		}
	...
	if len(report.PaintCommands) > 0 && len(report.PaintCommands) != len(expectedCommands) {
		issues = append(
			issues,
			fmt.Sprintf(
				"paint_commands count = %d, want %d deterministic renderer commands",
				len(report.PaintCommands),
	...
	for i, command := range report.PaintCommands {
		...
		if command.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"paint_commands order %d is not strictly greater than previous order %d",
					command.Order,
					lastOrder,
				),
			)
		...
		if i < len(expectedCommands) && name != expectedCommands[i] {
			issues = append(
				issues,
				fmt.Sprintf(
					"paint_commands[%d] command is %q, want deterministic %q",
					i,
					command.Command,
					expectedCommands[i],
				),
		...
		if command.Rect.W <= 0 || command.Rect.H <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("paint_commands[%d] rect dimensions must be positive", i),
			)
		}
		if strings.TrimSpace(command.LayerID) == "" {
			...
			issues = append(
				issues,
				fmt.Sprintf("paint_commands[%d] layer_id %q is not in paint_layers", i, command.LayerID),
			)
		}
		if strings.TrimSpace(command.Quality) == "" {
		...
		if !validChecksumLike(command.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("paint_commands[%d] checksum must be sha256 evidence", i),
			)
		}
		if command.Opacity < 0 || command.Opacity > 255 {
			issues = append(
				issues,
				fmt.Sprintf("paint_commands[%d] opacity must be 0..255 when present", i),
			)
		}
		if name == "fill" || name == "gradient" || name == "image_fill" || name == "border" ||
			name == "radius_clip" ||
			name == "overlay" ||
			name == "outline" {
			if command.Radius <= 0 {
				issues = append(
					issues,
					fmt.Sprintf("paint_commands[%d] %s radius must be positive", i, name),
				)
			}
		}
...
func validateRendererEvidence(renderer *RendererReport, expectedCommands []string) []string {
	...
	if renderer.Schema != RendererFeatureSchemaV1 {
		issues = append(
			issues,
			fmt.Sprintf("renderer schema is %q, want %s", renderer.Schema, RendererFeatureSchemaV1),
		)
	}
	if normalizePaintToken(renderer.Backend) != "software_rgba" {
		issues = append(
			issues,
			fmt.Sprintf("renderer backend is %q, want software-rgba", renderer.Backend),
		)
	}
	if strings.ToLower(strings.TrimSpace(renderer.ColorFormat)) != "rgba8" {
		issues = append(
			issues,
			fmt.Sprintf("renderer color_format is %q, want rgba8", renderer.ColorFormat),
		)
	}
	if renderer.QualityLevel != "deterministic-software-renderer-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"renderer quality_level is %q, want deterministic-software-renderer-v1",
	...
	if !renderer.SoftwareRenderer {
		issues = append(
			issues,
			"renderer software_renderer must be true for the P05 software RGBA baseline",
		)
	}
	if renderer.GPUProductionClaim {
		issues = append(
			issues,
			"renderer gpu production claim must be false without target-host GPU report",
		)
	}
	if renderer.BlurProductionClaim {
		issues = append(
			issues,
			"renderer blur production claim must be false without supported backend evidence",
		)
	}
	if renderer.BackdropBlurProductionClaim {
		issues = append(
			issues,
			"renderer backdrop blur production claim must be false without supported backend evidence",
		)
	}
	if len(renderer.CommandOrder) != len(expectedCommands) {
		issues = append(
			issues,
			fmt.Sprintf(
				"renderer command_order count = %d, want %d",
				len(renderer.CommandOrder),
				len(expectedCommands),
			),
		)
	}
	for i, expected := range expectedCommands {
		...
		if normalizePaintToken(renderer.CommandOrder[i]) != expected {
			issues = append(
				issues,
				fmt.Sprintf(
					"renderer command_order[%d] is %q, want %s",
					i,
					renderer.CommandOrder[i],
					expected,
				),
			)
		}
	...
	for _, effect := range []string{"gpu-production", "blur", "backdrop-blur"} {
		if !containsNormalizedPaint(renderer.UnsupportedEffectsRejected, effect) {
			issues = append(
				issues,
				fmt.Sprintf("renderer unsupported_effects_rejected requires %s", effect),
			)
		}
	}
	if len(renderer.DeterministicFrameChecksums) < 2 {
		issues = append(
			issues,
			"renderer deterministic_frame_checksums require before and after checksums",
		)
	} else {
		...
		if !validChecksumLike(first) || !validChecksumLike(second) || first == second {
			issues = append(issues, ("renderer deterministic_frame_checksums must be distinct " +
				"sha256 evidence"))
		}
	}
...
func validateRendererCompositorLayers(layers []RendererCompositorLayerReport) []string {
	...
	for i, layer := range layers {
		...
		if kind == "" {
			issues = append(
				issues,
				fmt.Sprintf("renderer compositor_layers[%d] kind is required", i),
			)
		}
		kinds[kind] = true
		if layer.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"renderer compositor_layers order %d is not strictly greater than previous order %d",
					layer.Order,
					lastOrder,
				),
			)
		...
		if layer.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("renderer compositor_layers[%d] block_id must be positive", i),
			)
		}
		if !validPositiveRect(layer.Rect) {
			issues = append(
				issues,
				fmt.Sprintf("renderer compositor_layers[%d] rect dimensions must be positive", i),
			)
		}
		if layer.Opacity <= 0 || layer.Opacity > 255 {
			issues = append(
				issues,
				fmt.Sprintf("renderer compositor_layers[%d] opacity must be 1..255", i),
			)
		}
		if layer.Opacity > 0 && layer.Opacity < 255 {
		...
		if transform == "" {
			issues = append(
				issues,
				fmt.Sprintf("renderer compositor_layers[%d] transform is required", i),
			)
		}
		...
		if layer.ClipApplied {
			if !validPositiveRect(layer.Clip) {
				issues = append(
					issues,
					fmt.Sprintf(
						"renderer compositor_layers[%d] clip dimensions must be positive when clip_applied",
						i,
					),
				)
			} else {
		...
		if !validChecksumLike(layer.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("renderer compositor_layers[%d] checksum must be sha256 evidence", i),
			)
		}
	}
	for _, kind := range []string{"root", "content", "overlay", "text", "icon"} {
		if !kinds[kind] {
			issues = append(
				issues,
				fmt.Sprintf("renderer compositor_layers require %s layer", kind),
			)
		}
	}
...
func validateRendererDirtyRects(rects []RendererDirtyRectReport) []string {
	...
	for i, rect := range rects {
		if rect.FrameOrder <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("renderer dirty_rects[%d] frame_order must be positive", i),
			)
		}
		if rect.FrameOrder < lastFrame {
			issues = append(
				issues,
				fmt.Sprintf("renderer dirty_rects[%d] frame_order must not go backwards", i),
			)
		}
		...
		if !validPositiveRect(rect.Rect) {
			issues = append(
				issues,
				fmt.Sprintf("renderer dirty_rects[%d] rect dimensions must be positive", i),
			)
		}
		...
		if !validChecksumLike(rect.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("renderer dirty_rects[%d] checksum must be sha256 evidence", i),
			)
		}
	}
	return issues
	...
	for i, invalidation := range invalidations {
		if invalidation.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"renderer invalidations order %d is not strictly greater than previous order %d",
					invalidation.Order,
					lastOrder,
				),
			)
		...
		if invalidation.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("renderer invalidations[%d] block_id must be positive", i),
			)
		}
		...
		if !validPositiveRect(invalidation.DirtyRect) {
			issues = append(
				issues,
				fmt.Sprintf("renderer invalidations[%d] dirty_rect dimensions must be positive", i),
			)
		}
		if !invalidation.Repaint {
			issues = append(
				issues,
				fmt.Sprintf("renderer invalidations[%d] repaint must be true", i),
			)
		}
	}
	return issues
...
func validateRendererCacheStats(cache RendererCacheStatsReport) []string {
	...
	if cache.BudgetBytes <= 0 || cache.BudgetBytes > 1024*1024 {
		issues = append(
			issues,
			fmt.Sprintf("renderer cache budget_bytes = %d, want 1..1048576", cache.BudgetBytes),
		)
	}
	if cache.UsedBytes < 0 || (cache.BudgetBytes > 0 && cache.UsedBytes > cache.BudgetBytes) {
		issues = append(
			issues,
			fmt.Sprintf(
				"renderer cache used_bytes = %d exceeds budget %d",
				cache.UsedBytes,
				cache.BudgetBytes,
			),
		)
...
// ---- block_scene_snapshot_validation.go ----
...
func validateBlockSceneSnapshotEvidence(report Report) []string {
	...
	if normalizeEvidencePath(snapshot.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_scene_snapshot source %q must match report source %q",
				snapshot.Source,
				report.Source,
			),
		)
	...
	if snapshot.SurfaceScope != "surface-morph-rendered-beauty-linux-web" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_scene_snapshot surface_scope is %q, want surface-morph-rendered-beauty-linux-web",
	...
	if len(snapshot.CorePrimitives) != 1 ||
		!stringSliceContainsFold(snapshot.CorePrimitives, "Block") {
	...
	if snapshot.CompactPropsOnly {
		issues = append(
			issues,
			"block_scene_snapshot compact_props_only must be false; rich visual specs are required",
		)
	}
	...
	if snapshot.NodeCount != len(snapshot.Nodes) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_scene_snapshot node_count = %d, want len(nodes) %d",
				snapshot.NodeCount,
				len(snapshot.Nodes),
			),
		)
...
func validateBlockSceneNodes(nodes []BlockSceneNodeReport, graph *BlockGraphReport) []string {
	...
	for _, node := range nodes {
		...
		if seen[node.BlockID] {
			issues = append(
				issues,
				fmt.Sprintf("block_scene_snapshot duplicate node block_id %d", node.BlockID),
			)
		}
		...
		if strings.TrimSpace(node.Name) == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_scene_snapshot node %d name is required", node.BlockID),
			)
		}
		if strings.TrimSpace(node.Recipe) == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_scene_snapshot node %d recipe is required", node.BlockID),
			)
		}
		if graph != nil {
			graphNode, ok := graphNodes[node.BlockID]
			if !ok {
				issues = append(
					issues,
					fmt.Sprintf("block_scene_snapshot node %d is not in block_graph", node.BlockID),
				)
			} else {
				if node.ParentID != graphNode.ParentID {
					issues = append(issues, fmt.Sprintf(("block_scene_snapshot node %d parent_id = %d, want "+
						"block_graph parent_id %d"), node.BlockID, node.ParentID, graphNode.ParentID))
				}
				if node.Layout != nil {
					rect := RectReport{X: node.Layout.X, Y: node.Layout.Y, W: node.Layout.W, H: node.Layout.H}
					if !rectsEqual(rect, graphNode.Bounds) {
						issues = append(issues, fmt.Sprintf((("block_scene_snapshot node %d layout bounds "+
							"%+v must match ")+
							"block_graph bounds %+v"), node.BlockID, rect, graphNode.Bounds))
					}
				}
			}
		}
	...
	if len(nodes) > 0 && rootCount != 1 {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot root count = %d, want 1", rootCount),
		)
	}
	for _, node := range nodes {
		if node.ParentID >= 0 && !seen[node.ParentID] {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_scene_snapshot node %d parent_id %d is not in snapshot nodes",
					node.BlockID,
					node.ParentID,
				),
			)
...
func validateBlockSceneRichSpecs(node BlockSceneNodeReport) []string {
	...
	if node.Layout == nil {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d layout spec is required", node.BlockID),
		)
	} else if !validLayoutMode(node.Layout.Mode) || node.Layout.W <= 0 || node.Layout.H <= 0 {
		issues = append(issues, fmt.Sprintf(("block_scene_snapshot node %d layout spec must include "+
			"supported mode and positive bounds"), node.BlockID))
	}
	if node.Paint == nil {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d paint spec is required", node.BlockID),
		)
	} else {
		if node.Paint.LayerCount != len(node.Paint.Layers) {
			issues = append(issues, fmt.Sprintf((("block_scene_snapshot node %d paint layer_count "+
				"= %d, want ")+
				"len(layers) %d"), node.BlockID, node.Paint.LayerCount, len(node.Paint.Layers)))
		}
		if node.Paint.LayerCount == 0 {
			issues = append(
				issues,
				fmt.Sprintf("block_scene_snapshot node %d paint layers are required", node.BlockID),
			)
		}
		for _, layer := range node.Paint.Layers {
			if strings.TrimSpace(layer.Kind) == "" {
				issues = append(
					issues,
					fmt.Sprintf("block_scene_snapshot node %d paint layer kind is required", node.BlockID),
				)
			}
		}
	}
	if node.Text == nil {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d text spec is required", node.BlockID),
		)
	} else if node.Text.TextLen < 0 || node.Text.HintLen < 0 {
		issues = append(issues, fmt.Sprintf(("block_scene_snapshot node %d text lengths must be "+
			"non-negative"), node.BlockID))
	}
	if node.Image == nil {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d image spec is required", node.BlockID),
		)
	} else if strings.TrimSpace(node.Image.AssetID) == "" || strings.TrimSpace(node.Image.Mode) == "" {
		issues = append(issues, fmt.Sprintf(("block_scene_snapshot node %d image asset_id and mode are "+
			"required"), node.BlockID))
	}
	if node.Input == nil {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d input spec is required", node.BlockID),
		)
	} else if strings.TrimSpace(node.Input.Kind) == "" {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d input kind is required", node.BlockID),
		)
	}
	if node.Event == nil {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d event spec is required", node.BlockID),
		)
	} else if strings.TrimSpace(
		node.Event.PointerAction,
	) == "" || strings.TrimSpace(
		node.Event.KeyAction,
	) == "" {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d event actions are required", node.BlockID),
		)
	}
	if node.State == nil {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d state spec is required", node.BlockID),
		)
	} else if strings.TrimSpace(node.State.Variant) == "" {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d state variant is required", node.BlockID),
		)
	}
	if node.Motion == nil {
		issues = append(
			issues,
			fmt.Sprintf("block_scene_snapshot node %d motion spec is required", node.BlockID),
		)
	} else if node.Motion.DurationMS <= 0 || strings.TrimSpace(
		node.Motion.Easing,
	) == "" || !node.Motion.ReducedMotionSafe {
		issues = append(issues, fmt.Sprintf((("block_scene_snapshot node %d motion spec "+
			"requires positive ")+
			"duration, easing, and reduced_motion_safe"), node.BlockID))
	}
	if node.Accessibility == nil {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_scene_snapshot node %d accessibility spec is required",
				node.BlockID,
			),
		)
	} else if strings.TrimSpace(
		node.Accessibility.Role,
	) == "" || node.Accessibility.LabelLen <= 0 || node.Accessibility.ReadingIndex <= 0 {
		issues = append(issues, fmt.Sprintf(("block_scene_snapshot node %d accessibility spec requires "+
			"role, label_len, and reading_index"), node.BlockID))
	}
	return issues
}

// ---- block_state_validation.go ----
...
func validateBlockStateEvidence(report Report) []string {
	...
	if report.BlockStateQualityLevel != "deterministic-block-state-resolver-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_state_quality_level is %q, want deterministic-block-state-resolver-v1",
	...
	if !normalizedStringListEqual(report.BlockStateResolverOrder, expectedOrder) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_state resolver order = %v, want %v",
				report.BlockStateResolverOrder,
				expectedOrder,
			),
		)
	}
	...
	expectedSelectors := map[string]struct {
		...
		"hover": {
			flag:  1,
			check: func(selector BlockStateSelectorReport) bool { return selector.Hovered },
		},
		"pressed": {
			flag:  2,
			check: func(selector BlockStateSelectorReport) bool { return selector.Pressed },
		},
		"focused": {
			flag:  4,
			check: func(selector BlockStateSelectorReport) bool { return selector.Focused },
		},
		"selected": {
			flag:  8,
			check: func(selector BlockStateSelectorReport) bool { return selector.Selected },
		},
		"disabled": {
			flag:  16,
			check: func(selector BlockStateSelectorReport) bool { return selector.Disabled },
		},
		"error": {
			flag:  32,
			check: func(selector BlockStateSelectorReport) bool { return selector.Error },
		},
		"loading": {
			flag:  64,
			check: func(selector BlockStateSelectorReport) bool { return selector.Loading },
		},
	}
	...
	for _, selector := range report.BlockStateSelectors {
		if selector.Order <= lastSelectorOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_selectors order %d is not strictly greater than previous order %d",
					selector.Order,
					lastSelectorOrder,
				),
			)
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_selectors[%d] name is %q, want supported Block state selector",
					selector.Order,
					selector.Name,
				),
			)
		...
		if selector.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("block_state_selectors[%d] block_id must be positive", selector.Order),
			)
		}
		if selector.Flags != spec.flag {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_selectors[%d] %s flags = %d, want %d",
					selector.Order,
					name,
					selector.Flags,
					spec.flag,
		...
		if !spec.check(selector) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_selectors[%d] %s selector boolean evidence is missing",
					selector.Order,
					name,
				),
			)
	...
	for name := range expectedSelectors {
		if !seenSelectors[name] {
			issues = append(
				issues,
				fmt.Sprintf("block_state_selectors require %s selector evidence", name),
			)
		}
	}
	...
	for _, resolution := range report.BlockStateResolutions {
		if resolution.Order <= lastResolutionOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_resolutions order %d is not strictly greater than previous order %d",
					resolution.Order,
					lastResolutionOrder,
				),
			)
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_resolutions[%d] selector is %q, want supported selector or motion",
					resolution.Order,
					resolution.Selector,
				),
			)
		...
		if resolution.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_resolutions[%d] block_id must be positive",
					resolution.Order,
				),
			)
		}
		if step == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_resolutions[%d] resolver_step is required",
					resolution.Order,
				),
			)
		} else if step != selector {
			issues = append(issues, fmt.Sprintf(("block_state_resolutions[%d] resolver_step is %q, want "+
				"selector %q"), resolution.Order, resolution.ResolverStep, resolution.Selector))
		}
		if property == "" {
			issues = append(
				issues,
				fmt.Sprintf("block_state_resolutions[%d] property is required", resolution.Order),
			)
		}
		if strings.TrimSpace(resolution.Before) == "" || strings.TrimSpace(resolution.After) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_resolutions[%d] before and after values are required",
					resolution.Order,
				),
			)
		}
		if !resolution.Applied {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_resolutions[%d] %s %s override must be applied",
					resolution.Order,
					selector,
					property,
				),
		...
		if resolution.Applied &&
			strings.TrimSpace(resolution.Before) == strings.TrimSpace(resolution.After) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_state_resolutions[%d] %s %s before/after must change",
					resolution.Order,
					selector,
					property,
				),
		...
		for property, seen := range properties {
			if !seen {
				issues = append(
					issues,
					fmt.Sprintf(
						"block_state_resolutions require %s %s evidence",
						selector,
						property,
					),
				)
	...
	for _, field := range []string{
		...
		if !hasTransition(report.StateTransitions, "StateBlock", field) {
			issues = append(
				issues,
				fmt.Sprintf("block_state evidence requires StateBlock %s state transition", field),
			)
		}
	}
	if len(report.Frames) < 2 || strings.TrimSpace(report.Frames[0].Checksum) == "" ||
		...
		issues = append(
			issues,
			"block_state frame checksum evidence must show state-driven visual change",
		)
	}
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("block_state report requires %s evidence", required),
			)
		}
	}
	return issues
	...
	check := func(location string, value string) {
		if token, ok := forbiddenBlockCorePrimitiveToken(value); ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system fake core primitive %s rejected in %s %q; use Block parameters instead",
					token,
					location,
					value,
				),
...
func validateBlockSystemEvidence(report Report) []string {
	...
	if linuxRealWindowBlockSystem &&
		(report.Target != "linux-x64" || report.Runtime != "surface-linux-x64" || !isLinuxRealWindowHostEvidenceLevel(
			report.HostEvidence.Level,
		)) {
		issues = append(
			issues,
			"linux-x64 real-window block_system requires linux-x64 real-window runtime evidence",
		)
	}
	if wasmBrowserCanvasBlockSystem &&
		...
		issues = append(
			issues,
			"wasm32-web browser-canvas block_system requires wasm32-web browser-canvas runtime evidence",
		)
	}
	switch {
	...
	case report.Target == "linux-x64" && report.Runtime == "surface-linux-x64" && isLinuxRealWindowHostEvidenceLevel(
		report.HostEvidence.Level,
	):
		...
		if !report.HostEvidence.RealWindow || !report.HostEvidence.NativeInput ||
			...
			issues = append(
				issues,
				("linux-x64 real-window block_system requires framebuffer, " +
					"real_window, and native_input host evidence"),
			)
		}
		if !hasFrameOrderDimensions(report.Frames, 5, 400, 240, 1600) {
			issues = append(
				issues,
				"linux-x64 real-window block_system requires order-5 400x240 presented frame evidence",
			)
		}
		if !eventKindContains(report.Events, "mouse_up") ||
			!eventKindContains(report.Events, "key_down") ||
			!eventKindContains(report.Events, "resize") ||
			!eventKindContains(report.Events, "close") {
			issues = append(
				issues,
				"linux-x64 real-window block_system requires native input, resize, and close event evidence",
			)
		}
		if !hasTransition(report.StateTransitions, "SubmitBlock", "pressed") {
			issues = append(
				issues,
				"linux-x64 real-window block_system requires native input state transition evidence",
			)
		}
	case report.Target == "wasm32-web" && report.Runtime == "surface-wasm32-web" && report.HostEvidence.Level == "wasm32-web-browser-canvas-input":
		...
		if !report.HostEvidence.Framebuffer || !report.HostEvidence.NativeInput ||
			!report.HostEvidence.BrowserCanvas ||
			!report.HostEvidence.BrowserInput {
			issues = append(
				issues,
				("wasm32-web browser-canvas block_system requires framebuffer," +
					" native_input, browser_canvas, and browser_input host " +
					"evidence"),
			)
		}
		if report.HostEvidence.RealWindow {
			issues = append(
				issues,
				"wasm32-web browser-canvas block_system must not claim OS real_window evidence",
			)
		}
		if !hasFrameOrderDimensions(report.Frames, 5, 400, 240, 1600) {
			issues = append(
				issues,
				"wasm32-web browser-canvas block_system requires order-5 400x240 RGBA readback frame evidence",
			)
		}
		if !eventKindContains(report.Events, "mouse_up") ||
			!eventKindContains(report.Events, "key_down") ||
			!eventKindContains(report.Events, "resize") ||
			!eventKindContains(report.Events, "text_input") {
			issues = append(
				issues,
				("wasm32-web browser-canvas block_system requires browser " +
					"input, resize, and text input event evidence"),
			)
		}
		if !hasTransition(report.StateTransitions, "SubmitBlock", "pressed") {
			issues = append(
				issues,
				("wasm32-web browser-canvas block_system requires browser " +
					"native input state transition evidence"),
			)
		}
		if !hasProcessNameAndPathMarkers(
			...
			"chrom",
			"scenario=block-system",
		) {
			issues = append(
				issues,
				("wasm32-web browser-canvas block_system requires " +
					"Chromium-compatible browser-canvas app process evidence"),
			)
		}
	...
	default:
		issues = append(
			issues,
			("block_system requires deterministic headless, linux-x64 " +
				"real-window, or wasm32-web browser-canvas runtime evidence"),
		)
	}
	if expectedQuality != "" && system.QualityLevel != expectedQuality {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system quality_level is %q, want %s",
				system.QualityLevel,
				expectedQuality,
			),
		)
	...
	if normalizeEvidencePath(system.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system source %q must match report source %q",
				system.Source,
				report.Source,
			),
		)
	...
	if expectedRenderer != "" && system.Renderer != expectedRenderer {
		issues = append(
			issues,
			fmt.Sprintf("block_system renderer is %q, want %s", system.Renderer, expectedRenderer),
		)
	}
	...
	if system.FrameCount != len(system.Frames) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system frame_count = %d, want len(frames) %d",
				system.FrameCount,
				len(system.Frames),
			),
		)
	...
	if len(report.LayoutConstraints) == 0 || len(report.LayoutPasses) == 0 ||
		len(report.LayoutScrolls) == 0 {
	...
	for i, frame := range system.Frames {
		if frame.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system frames order %d is not strictly greater than previous order %d",
					frame.Order,
					lastOrder,
				),
			)
		...
		if frame.Width <= 0 || frame.Height <= 0 || frame.Stride <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system frame %d dimensions and stride must be positive",
					frame.Order,
				),
			)
		}
		if !validChecksumLike(frame.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("block_system frame %d checksum must be sha256 evidence", frame.Order),
			)
		}
		if !validChecksumLike(frame.RepeatChecksum) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system frame %d repeat_checksum must be sha256 evidence",
					frame.Order,
				),
			)
		}
		if !validChecksumLike(frame.GoldenChecksum) {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system frame %d golden_checksum must be sha256 evidence",
					frame.Order,
				),
			)
		}
		if strings.TrimSpace(frame.Checksum) != "" &&
			strings.TrimSpace(frame.RepeatChecksum) != "" &&
			frame.Checksum != frame.RepeatChecksum {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system frame %d nondeterministic repeat checksum %q, want %q",
					frame.Order,
		...
		if strings.TrimSpace(frame.Checksum) != "" &&
			strings.TrimSpace(frame.GoldenChecksum) != "" &&
			frame.Checksum != frame.GoldenChecksum {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system frame %d golden checksum %q, want %q",
					frame.Order,
					frame.GoldenChecksum,
					frame.Checksum,
				),
			)
		}
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system frame %d is missing from runtime frame evidence",
					frame.Order,
				),
			)
		} else {
			if reportFrame.Width != frame.Width || reportFrame.Height != frame.Height || reportFrame.Stride != frame.Stride {
				issues = append(issues, fmt.Sprintf((("block_system frame %d dimensions do not match "+
					"runtime frame ")+
					"evidence"), frame.Order))
			}
			if reportFrame.Checksum != frame.Checksum {
				issues = append(issues, fmt.Sprintf((("block_system frame %d checksum %q must match "+
					"runtime frame ")+
					"checksum %q"), frame.Order, frame.Checksum, reportFrame.Checksum))
			}
			if !reportFrame.Presented {
				issues = append(issues, fmt.Sprintf(("block_system frame %d requires presented runtime frame "+
					"evidence"), frame.Order))
			}
		...
		if !frame.PaintEvidence {
			issues = append(
				issues,
				fmt.Sprintf("block_system frame %d missing paint evidence", frame.Order),
			)
		}
		if !frame.LayoutEvidence {
			issues = append(
				issues,
				fmt.Sprintf("block_system frame %d missing layout evidence", frame.Order),
			)
		}
		if !frame.AccessibilityEvidence {
			issues = append(
				issues,
				fmt.Sprintf("block_system frame %d missing accessibility evidence", frame.Order),
			)
		}
	}
	if len(system.Frames) != len(report.Frames) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system frames length %d must match runtime frames length %d",
				len(system.Frames),
				len(report.Frames),
			),
		)
	...
	for _, required := range requiredCases {
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("block_system report requires %s evidence", required),
			)
		}
	}
	return issues
...
func validateBlockMemoryBudgetEvidence(report Report, budget BlockMemoryBudgetReport) []string {
	...
	if budget.Schema != "tetra.surface.block-memory-budget.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget schema is %q, want tetra.surface.block-memory-budget.v1",
				budget.Schema,
			),
		)
	}
	if budget.Scope != "surface-block-system-local-budget-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget scope is %q, want surface-block-system-local-budget-v1",
				budget.Scope,
			),
		)
	}
	if budget.BlockCount != len(report.Components) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget block_count = %d, want component count %d",
				budget.BlockCount,
	...
	if budget.StressBlockCount < 128 {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget stress_block_count = %d, want at least 128",
				budget.StressBlockCount,
			),
		)
	}
	if budget.RenderLoopCount < 16 {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget render_loop_count = %d, want at least 16",
				budget.RenderLoopCount,
			),
		)
	}
	if budget.StateLoopCount < len(report.StateTransitions) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget state_loop_count = %d, want at least state transition count %d",
				budget.StateLoopCount,
				len(report.StateTransitions),
			),
		)
	}
	if budget.MotionFrameCount != len(report.MotionFrames) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget motion_frame_count = %d, want %d",
				budget.MotionFrameCount,
				len(report.MotionFrames),
			),
		)
	}
	if budget.InputEventCount != len(report.Events) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget input_event_count = %d, want %d",
				budget.InputEventCount,
				len(report.Events),
			),
		)
	}
	if budget.PaintCommandCount != len(report.PaintCommands) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget paint_command_count = %d, want %d",
				budget.PaintCommandCount,
				len(report.PaintCommands),
			),
		)
	}
	if budget.TextRenderCommandCount != len(report.TextRenderCommands) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget text_render_command_count = %d, want %d",
				budget.TextRenderCommandCount,
				len(report.TextRenderCommands),
			),
		)
	}
	if budget.AssetRenderCommandCount != len(report.BlockAssetRenderCommands) {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget asset_render_command_count = %d, want %d",
				budget.AssetRenderCommandCount,
				len(report.BlockAssetRenderCommands),
			),
		)
	}
	...
	if budget.PeakFramebufferBytes != peakFramebufferBytes {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget peak_framebuffer_bytes = %d, want %d",
				budget.PeakFramebufferBytes,
				peakFramebufferBytes,
			),
		)
	}
	if budget.TotalFramebufferBytes != totalFramebufferBytes {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget total_framebuffer_bytes = %d, want %d",
				budget.TotalFramebufferBytes,
				totalFramebufferBytes,
			),
		)
	}
	if budget.FramebufferBudgetBytes < peakFramebufferBytes ||
		budget.FramebufferBudgetBytes > 16*1024*1024 {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget framebuffer_budget_bytes = %d outside bounded range for peak %d",
				budget.FramebufferBudgetBytes,
				peakFramebufferBytes,
			),
		)
	}
	...
	if budget.PaintCacheUsedBytes != expectedPaintUsed {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget paint_cache_used_bytes = %d, want %d",
				budget.PaintCacheUsedBytes,
				expectedPaintUsed,
			),
		)
	}
	if budget.PaintCacheBudgetBytes != report.PaintCacheBudgetBytes {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget paint_cache_budget_bytes = %d, want %d",
				budget.PaintCacheBudgetBytes,
				report.PaintCacheBudgetBytes,
			),
		)
	}
	if budget.TextCacheUsedBytes != expectedTextUsed {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget text_cache_used_bytes = %d, want %d",
				budget.TextCacheUsedBytes,
				expectedTextUsed,
			),
		)
	}
	if budget.TextCacheBudgetBytes != report.TextCacheBudgetBytes {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget text_cache_budget_bytes = %d, want %d",
				budget.TextCacheBudgetBytes,
	...
	if budget.AssetCacheUsedBytes != expectedAssetUsed {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget asset_cache_used_bytes = %d, want %d",
				budget.AssetCacheUsedBytes,
				expectedAssetUsed,
			),
		)
	}
	if budget.AssetCacheBudgetBytes != report.BlockAssetCache.BudgetBytes {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget asset_cache_budget_bytes = %d, want %d",
				budget.AssetCacheBudgetBytes,
	...
	if budget.TotalCacheUsedBytes != expectedTotalCacheUsed {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget total_cache_used_bytes = %d, want %d",
				budget.TotalCacheUsedBytes,
				expectedTotalCacheUsed,
			),
		)
	}
	if budget.TotalCacheBudgetBytes != expectedTotalCacheBudget {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget total_cache_budget_bytes = %d, want %d",
				budget.TotalCacheBudgetBytes,
				expectedTotalCacheBudget,
			),
		)
	}
	if expectedTotalCacheBudget <= 0 || expectedTotalCacheUsed < 0 ||
		expectedTotalCacheUsed > expectedTotalCacheBudget {
		issues = append(
			issues,
			"block_system memory_budget cache totals must be bounded and within budget",
		)
	}
	if budget.EstimatedAllocationBytes < totalFramebufferBytes+expectedTotalCacheUsed {
		issues = append(
			issues,
			fmt.Sprintf(
				("block_system memory_budget estimated_allocation_bytes = %d, "+
					"want at least framebuffer+cache %d"),
				budget.EstimatedAllocationBytes,
				totalFramebufferBytes+expectedTotalCacheUsed,
			),
		)
	}
	if budget.RSSMeasured {
		if budget.PeakRSSBytes <= 0 || budget.PeakRSSBytes > 512*1024*1024 {
			issues = append(
				issues,
				fmt.Sprintf(
					"block_system memory_budget peak_rss_bytes = %d outside scoped RSS range",
					budget.PeakRSSBytes,
				),
			)
		}
	} else if budget.PeakRSSBytes != 0 {
		issues = append(issues, ("block_system memory_budget peak_rss_bytes must be 0 when " +
			"rss_measured is false"))
	}
	...
	if strings.TrimSpace(budget.StressScene) != "deterministic-block-stress-128" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget stress_scene is %q, want deterministic-block-stress-128",
				budget.StressScene,
			),
		)
	}
	if strings.TrimSpace(budget.PerformanceClaim) != "none" {
		issues = append(
			issues,
			fmt.Sprintf(
				"block_system memory_budget performance_claim is %q, want none",
	...
	for _, claim := range budget.NonClaims {
		issues = append(
			issues,
			forbiddenBlockPerformanceClaimIssues("block_system memory_budget nonclaims", claim)...)
	}
	for _, required := range []string{
		...
		if !containsTextFold(budget.NonClaims, required) {
			issues = append(
				issues,
				fmt.Sprintf("block_system memory_budget nonclaims missing %q", required),
			)
		}
	}
	return issues
	...
	for _, field := range fields {
		...
		for _, marker := range []string{
			...
			if strings.Contains(lower, marker) {
				issues = append(
					issues,
					fmt.Sprintf("%s contains forbidden performance claim %q", label, marker),
				)
			}
		}
	}
...
func validateBlockSystemBrowserCanvasArtifacts(artifacts []ArtifactReport) []string {
	...
	for _, artifact := range artifacts {
		...
		if strings.Contains(kind, "user-js") || strings.Contains(path, ".user.js") ||
			strings.HasSuffix(path, "/user.js") {
			issues = append(
				issues,
				fmt.Sprintf(
					"wasm32-web browser-canvas block_system must not include user JS artifact %q",
					artifact.Path,
				),
			)
		}
		if strings.Contains(kind, "dom-ui") || strings.Contains(path, ".dom.") ||
			strings.HasSuffix(path, ".html") {
			issues = append(
				issues,
				fmt.Sprintf(
					"wasm32-web browser-canvas block_system must not include DOM UI artifact %q",
					artifact.Path,
				),
			)
		}
...
func validateBlockSystemNegativeGuards(guards BlockSystemNegativeGuardsReport) []string {
	...
	return []string{
		fmt.Sprintf("block_system negative_guards missing %s", strings.Join(missing, ", ")),
	}
}

// ---- block_text_validation.go ----
...
func validateBlockTextEvidence(report Report) []string {
	...
	if report.TextQualityLevel != "deterministic-fallback-text-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"text_quality_level is %q, want deterministic-fallback-text-v1",
	...
	if report.TextCacheBudgetBytes <= 0 || report.TextCacheBudgetBytes > 1024*1024 {
		issues = append(
			issues,
			fmt.Sprintf(
				"text_cache_budget_bytes = %d, want 1..1048576",
				report.TextCacheBudgetBytes,
			),
		)
	}
	...
	for _, measurement := range report.TextMeasurements {
		...
		if measurement.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("text_measurements %q block_id must be positive", measurement.ID),
			)
		}
		if measurement.TextLen <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("text_measurements %q text_len must be positive", measurement.ID),
			)
		}
		if strings.TrimSpace(measurement.FontFamily) == "" {
			issues = append(
				issues,
				fmt.Sprintf("text_measurements %q font_family is required", measurement.ID),
			)
		}
		if measurement.FontSize <= 0 || measurement.LineHeight <= 0 || measurement.FontWeight <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("text_measurements %q font metrics must be positive", measurement.ID),
			)
		}
		if measurement.MaxWidth <= 0 || measurement.Measured.W <= 0 ||
			measurement.Measured.H <= 0 ||
			measurement.LineCount <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_measurements %q measured dimensions and line_count must be positive",
					measurement.ID,
				),
			)
		}
		if measurement.MaxWidth > 0 && measurement.Measured.W > measurement.MaxWidth {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_measurements %q measured width %d exceeds max_width %d",
					measurement.ID,
					measurement.Measured.W,
					measurement.MaxWidth,
				),
			)
		}
		...
		if wrap != "none" && wrap != "word" && wrap != "char" {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_measurements %q wrap is %q, want none, word, or char",
					measurement.ID,
					measurement.Wrap,
				),
			)
		...
		if overflow != "clip" && overflow != "ellipsis" {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_measurements %q overflow is %q, want clip or ellipsis",
					measurement.ID,
		...
		if align != "start" && align != "center" && align != "end" {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_measurements %q align is %q, want start, center, or end",
					measurement.ID,
					measurement.Align,
				),
			)
		...
		if strings.TrimSpace(measurement.Quality) == "" {
			issues = append(
				issues,
				fmt.Sprintf("text_measurements %q quality is required", measurement.ID),
			)
		}
		if !validChecksumLike(measurement.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_measurements %q checksum must be sha256 evidence",
					measurement.ID,
				),
			)
		}
		if measurement.Ellipsis || overflow == "ellipsis" {
			if measurement.EllipsizedTextLen <= 0 ||
				measurement.EllipsizedTextLen >= measurement.TextLen {
				issues = append(
					issues,
					fmt.Sprintf(
						"text_measurements %q ellipsis requires ellipsized_text_len between 1 and text_len-1",
						measurement.ID,
					),
				)
			}
		...
		if (wrap == "word" || wrap == "char") && overflow == "ellipsis" && measurement.Ellipsis &&
			...
			measurement.EllipsizedTextLen > 0 &&
			measurement.EllipsizedTextLen < measurement.TextLen {
	...
	for _, fallback := range report.FontFallbacks {
		...
		if strings.TrimSpace(fallback.RequestedFamily) == "" ||
			strings.TrimSpace(fallback.ResolvedFamily) == "" {
			issues = append(
				issues,
				"font_fallback requested_family and resolved_family are required",
			)
		}
		if len(fallback.Chain) < 2 {
			issues = append(
				issues,
				"font_fallback chain must include at least requested and fallback families",
			)
		}
		if fallback.MissingGlyphs != 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"font_fallback %q missing_glyphs = %d, want 0 for smoke coverage",
					fallback.ID,
					fallback.MissingGlyphs,
				),
			)
		}
	...
	for _, cache := range report.GlyphCaches {
		...
		if cache.BudgetBytes <= 0 || cache.BudgetBytes > report.TextCacheBudgetBytes {
			issues = append(
				issues,
				fmt.Sprintf(
					"glyph cache %q budget_bytes = %d outside text cache budget %d",
					cache.ID,
					cache.BudgetBytes,
					report.TextCacheBudgetBytes,
				),
		...
		if cache.UsedBytes < 0 || cache.UsedBytes > cache.BudgetBytes {
			issues = append(
				issues,
				fmt.Sprintf(
					"glyph cache %q used_bytes = %d outside budget %d",
					cache.ID,
					cache.UsedBytes,
					cache.BudgetBytes,
				),
		...
		if cache.EntryCount <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("glyph cache %q entry_count must be positive", cache.ID),
			)
		}
	...
	for i, command := range report.TextRenderCommands {
		...
		if command.Order <= lastOrder {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_render_commands order %d is not strictly greater than previous order %d",
					command.Order,
					lastOrder,
				),
			)
		...
		if command.BlockID <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("text_render_commands[%d] block_id must be positive", i),
			)
		}
		if command.Rect.W <= 0 || command.Rect.H <= 0 || command.Clip.W <= 0 ||
			command.Clip.H <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_render_commands[%d] rect and clip dimensions must be positive",
					i,
				),
			)
		}
		if !rectContainsRect(command.Clip, command.Rect) &&
			!rectContainsRect(command.Rect, command.Clip) {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_render_commands[%d] rect/clip must overlap as scoped text bounds",
					i,
				),
			)
		}
		if _, ok := measurementIDs[command.MeasurementID]; !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_render_commands[%d] measurement_id %q is not in text_measurements",
					i,
		...
		if name != "measure" && name != "render_glyphs" && name != "render_caret" {
			issues = append(
				issues,
				fmt.Sprintf(
					"text_render_commands[%d] command is %q, want measure, render_glyphs, or render_caret",
					i,
					command.Command,
				),
			)
		...
		if strings.TrimSpace(command.Color) == "" || command.Opacity <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("text_render_commands[%d] color and opacity are required", i),
			)
		}
		if strings.TrimSpace(command.Quality) == "" {
			issues = append(issues, fmt.Sprintf("text_render_commands[%d] quality is required", i))
		}
		if strings.Contains(strings.ToLower(command.Quality), "marker") {
			issues = append(
				issues,
				fmt.Sprintf("text_render_commands[%d] quality must not be marker-only evidence", i),
			)
		}
		if name == "render_glyphs" {
			issues = append(issues, validateRasterProof(
				fmt.Sprintf("text_render_commands[%d]", i),
		...
		if !validChecksumLike(command.Checksum) {
			issues = append(
				issues,
				fmt.Sprintf("text_render_commands[%d] checksum must be sha256 evidence", i),
			)
		}
		seenCommands[name] = true
	}
	...
	if !hasEventTargetKind(report.Events, "InputBlock", "text_input") {
		issues = append(
			issues,
			"block text editable input requires text_input targeted to InputBlock",
		)
	}
	if !hasTransition(report.StateTransitions, "InputBlock", "buffer") ||
		!hasTransition(report.StateTransitions, "InputBlock", "caret") {
		issues = append(
			issues,
			"block text editable input requires InputBlock buffer and caret state transitions",
		)
	}
	if len(report.Frames) < 2 || strings.TrimSpace(report.Frames[0].Checksum) == "" ||
		report.Frames[0].Checksum == report.Frames[1].Checksum {
...
func validateComponentTreeEvidence(report Report) []string {
	...
	releaseAccessibility := isSurfaceReleaseAccessibilitySource(report.Source) ||
		isPlatformBridgeAccessibilityReport(report)
	...
	if accessibility {
		if !isSurfaceAccessibilitySettingsSource(report.Source) &&
			...
			issues = append(
				issues,
				fmt.Sprintf(
					("component_tree accessibility source path must match "+
						"examples/surface/toolkit/surface_accessibility_settings.tetr"+
						"a or "+
						"examples/surface/release/surface_release_accessibility.tetra"+
						", got %q"),
		...
		if !isSurfaceReleaseFormSource(report.Source) {
			issues = append(issues, fmt.Sprintf(("component_tree production toolkit source path must match "+
				"examples/surface/release/surface_release_form.tetra, got %q"), report.Source))
		}
	} else if toolkitReuse {
		if !isSurfaceToolkitSettingsSource(report.Source) {
			issues = append(issues, fmt.Sprintf(("component_tree toolkit reuse source path must match "+
				"examples/surface/toolkit/surface_toolkit_settings.tetra, "+
				"got %q"), report.Source))
		}
	} else if minimalToolkit {
		if !isSurfaceToolkitFormSource(report.Source) {
			issues = append(issues, fmt.Sprintf(("component_tree toolkit source path must match "+
				"examples/surface/toolkit/surface_toolkit_form.tetra, got %q"), report.Source))
		}
	} else if !isSurfaceTreeAppSource(report.Source) {
		issues = append(issues, fmt.Sprintf(("component_tree source path must match "+
			"examples/surface/toolkit/surface_tree_app.tetra, got %q"), report.Source))
	}
	if report.ComponentTree == nil {
		if accessibility {
			return append(
				issues,
				("component_tree evidence is required for " +
					"examples/surface/toolkit/surface_accessibility_settings.tetr" +
					"a"),
			)
		}
		if productionToolkit {
			return append(
				issues,
				"component_tree evidence is required for examples/surface/release/surface_release_form.tetra",
			)
		}
		if minimalToolkit {
			return append(
				issues,
				"component_tree evidence is required for examples/surface/toolkit/surface_toolkit_form.tetra",
			)
		}
		return append(
			issues,
			"component_tree evidence is required for examples/surface/toolkit/surface_tree_app.tetra",
		)
	}
	...
	if tree.Schema != "tetra.surface.component-tree.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree schema is %q, want tetra.surface.component-tree.v1",
				tree.Schema,
			),
		)
	}
	if accessibility {
		...
		if tree.DynamicLevel != wantLevel {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree dynamic_level is %q, want %s",
					tree.DynamicLevel,
					wantLevel,
				),
			)
		...
		if tree.DynamicLevel != "production-widgets-v1" {
			issues = append(issues, fmt.Sprintf(("component_tree dynamic_level is %q, want "+
				"production-widgets-v1"), tree.DynamicLevel))
		}
	} else if toolkitReuse {
		if tree.DynamicLevel != "toolkit-reuse-widget-tree" {
			issues = append(issues, fmt.Sprintf(("component_tree dynamic_level is %q, want "+
				"toolkit-reuse-widget-tree"), tree.DynamicLevel))
		}
	} else if minimalToolkit {
		if tree.DynamicLevel != "minimal-toolkit-widget-tree" {
			issues = append(issues, fmt.Sprintf(("component_tree dynamic_level is %q, want "+
				"minimal-toolkit-widget-tree"), tree.DynamicLevel))
		}
	} else if tree.DynamicLevel != "semi-dynamic-child-list" {
		issues = append(issues, fmt.Sprintf(("component_tree dynamic_level is %q, want "+
			"semi-dynamic-child-list"), tree.DynamicLevel))
	}
	if tree.NodeCount != len(tree.Nodes) {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree node_count = %d, want len(nodes) %d",
				tree.NodeCount,
				len(tree.Nodes),
			),
		)
	...
	if tree.NodeCount < 7 || len(tree.Nodes) < 7 {
		issues = append(
			issues,
			fmt.Sprintf("component_tree node_count = %d, want at least 7", tree.NodeCount),
		)
	}
	...
	for _, node := range tree.Nodes {
		...
		if node.Bounds.W <= 0 || node.Bounds.H <= 0 {
			issues = append(
				issues,
				fmt.Sprintf("component_tree node %d layout bounds are required", node.ID),
			)
		}
		if node.Focusable {
			focusableCount++
		...
		if node.ChildCount == 0 && node.FirstChild != -1 {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree leaf node %d first_child = %d, want -1",
					node.ID,
					node.FirstChild,
				),
			)
		...
		if node.ChildCount < 0 {
			issues = append(
				issues,
				fmt.Sprintf("component_tree node %d child_count must be non-negative", node.ID),
			)
		}
	}
	...
	if !ok {
		issues = append(
			issues,
			fmt.Sprintf("component_tree root_id %d is not in nodes", tree.RootID),
		)
	} else {
		if root.ParentID != -1 {
			issues = append(
				issues,
				fmt.Sprintf("component_tree root %d parent_id = %d, want -1", root.ID, root.ParentID),
			)
		}
	...
	if _, ok := nodes[tree.FocusedID]; !ok {
		issues = append(
			issues,
			fmt.Sprintf("component_tree focused_id %d is not in nodes", tree.FocusedID),
		)
	}
	...
	for _, node := range tree.Nodes {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree node %d parent_id %d is unknown",
					node.ID,
					node.ParentID,
				),
			)
		...
		if !rectContainsRect(parent.Bounds, node.Bounds) {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree node %d bounds must be inside parent %d bounds",
					node.ID,
					parent.ID,
				),
			)
	...
	for parentID, children := range childrenByParent {
		...
		if parent.ChildCount != len(children) {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree node %d child_count = %d, want %d",
					parentID,
					parent.ChildCount,
					len(children),
				),
		...
		for _, child := range children {
			if child.ChildIndex < 0 || child.ChildIndex >= len(children) {
				issues = append(
					issues,
					fmt.Sprintf(
						"component_tree child node %d child_index = %d, want 0..%d",
						child.ID,
						child.ChildIndex,
						len(children)-1,
					),
			...
			if prev, exists := seenChildIndex[child.ChildIndex]; exists {
				issues = append(
					issues,
					fmt.Sprintf(
						"component_tree sibling child_index %d is used by nodes %d and %d",
						child.ChildIndex,
						prev,
						child.ID,
					),
		...
		if len(children) > 0 && parent.FirstChild != firstID {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree node %d first_child = %d, want child_index 0 node %d",
					parentID,
					parent.FirstChild,
					firstID,
				),
	...
	if productionToolkit && !hasCheckbox {
		issues = append(
			issues,
			"component_tree requires SubscribeCheckbox node for production toolkit",
		)
	}
	if (accessibility || productionToolkit || toolkitReuse) && !hasSecondTextBox {
		if accessibility {
			issues = append(
				issues,
				"component_tree requires EmailTextBox node for accessibility metadata",
			)
		} else if productionToolkit {
	...
	if focusableCount < 3 {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree focusable node count = %d, want at least 3",
				focusableCount,
			),
		)
	}
	...
	if (accessibility || productionToolkit || toolkitReuse) && hasSecondTextBox &&
		!containsInt(tree.FocusOrder, secondTextBox.ID) {
		...
		if !intSlicesEqual(tree.FocusOrder, wantFocusOrder) {
			if productionToolkit {
				issues = append(
					issues,
					fmt.Sprintf(
						("component_tree focus_order = %v, want NameTextBox -> "+
							"EmailTextBox -> SubscribeCheckbox -> SaveButton -> "+
							"ResetButton (%v)"),
						tree.FocusOrder,
						wantFocusOrder,
					),
				...
				issues = append(issues, fmt.Sprintf(("component_tree focus_order = %v, want NameTextBox -> "+
					"EmailTextBox -> SaveButton -> ResetButton (%v)"), tree.FocusOrder, wantFocusOrder))
			} else {
				issues = append(issues, fmt.Sprintf(("component_tree focus_order = %v, want TextBox -> "+
					"SubmitButton -> ResetButton (%v)"), tree.FocusOrder, wantFocusOrder))
			}
		}
	}
	...
	if hasTextBox && !componentTreeHasResizeLayoutPass(tree.LayoutPasses, textBox.ID) {
		issues = append(
			issues,
			"component_tree layout_passes require TextBox initial and resize bounds",
		)
	}
	if (accessibility || productionToolkit || toolkitReuse) && hasSecondTextBox &&
		!componentTreeHasResizeLayoutPass(tree.LayoutPasses, secondTextBox.ID) {
		issues = append(
			issues,
			"component_tree layout_passes require EmailTextBox initial and resize bounds",
		)
	}
	...
	issues = append(
		issues,
		validateComponentTreeDispatchPaths(tree.DispatchPaths, expectedPaths, nodes)...)
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("component_tree report requires %s evidence", required),
			)
		}
	}
	...
	if hasTextBox && hasSubmit && hasReset {
		if productionToolkit && hasSecondTextBox && hasCheckbox {
			if !hasComponentTreeTabFocus(
				report.Events,
				fmt.Sprint(textBox.ID),
				fmt.Sprint(secondTextBox.ID),
			) ||
				!hasComponentTreeTabFocus(
					report.Events,
					fmt.Sprint(secondTextBox.ID),
					fmt.Sprint(checkbox.ID),
				) ||
				!hasComponentTreeTabFocus(
					report.Events,
					fmt.Sprint(checkbox.ID),
					fmt.Sprint(submit.ID),
				) ||
				!hasComponentTreeTabFocus(
					report.Events,
					fmt.Sprint(submit.ID),
				...
				issues = append(
					issues,
					("component_tree requires Tab focus traversal NameTextBox -> " +
						"EmailTextBox -> SubscribeCheckbox -> SaveButton -> " +
			...
			if !hasComponentTreeTabFocus(
				...
				issues = append(issues, ("component_tree requires Tab focus traversal NameTextBox -> " +
					"EmailTextBox -> SaveButton -> ResetButton -> NameTextBox"))
			}
		} else if !hasComponentTreeTabFocus(
			report.Events,
			fmt.Sprint(textBox.ID),
			fmt.Sprint(submit.ID),
		) ||
			!hasComponentTreeTabFocus(report.Events, fmt.Sprint(submit.ID), fmt.Sprint(reset.ID)) ||
			!hasComponentTreeTabFocus(report.Events, fmt.Sprint(reset.ID), fmt.Sprint(textBox.ID)) {
			issues = append(issues, ("component_tree requires Tab focus traversal TextBox -> " +
				"SubmitButton -> ResetButton -> TextBox including " +
				"ResetButton -> TextBox wrap"))
		}
	}
	...
	if !hasComponentTreeButtonAction(report.Events) {
		issues = append(
			issues,
			"component_tree requires keyboard button action dispatch through tree path",
		)
	}
	if !hasComponentTreeResizeRelayout(report.Events, report.StateTransitions) {
		issues = append(
			issues,
			"component_tree resize relayout requires changed TextBox bounds while preserving focused_id",
		)
	}
	if len(report.Frames) >= 2 &&
		report.Frames[0].Checksum == report.Frames[len(report.Frames)-1].Checksum {
		issues = append(
			issues,
			"component_tree rendered frame update requires changed frame checksum",
		)
	}
	return issues
}

func validateComponentTreeSiblingLayout(
	nodes map[int]ComponentTreeNodeReport,
	childrenByParent map[int][]ComponentTreeNodeReport,
) []string {
	var issues []string
			...
			case "column":
				if child.Bounds.Y < prev.Bounds.Y+prev.Bounds.H {
					issues = append(
						issues,
						fmt.Sprintf(
							("component_tree Column node %d child_index %d node %d "+
								"overlaps or precedes child_index %d node %d"),
							parentID,
							child.ChildIndex,
							child.ID,
							prev.ChildIndex,
			...
			case "row":
				if child.Bounds.X < prev.Bounds.X+prev.Bounds.W {
					issues = append(
						issues,
						fmt.Sprintf(
							"component_tree Row node %d child_index %d node %d overlaps child_index %d node %d",
							parentID,
							child.ChildIndex,
							child.ID,
							prev.ChildIndex,
...
func validateComponentTreeAPIEvidence(
	report Report,
	tree *ComponentTreeReport,
	expectedPaths map[int][]int,
	nodes map[int]ComponentTreeNodeReport,
) []string {
	if tree == nil {
	...
	if api == nil {
		return []string{
			"component_tree_api evidence is required for component tree API hardening reports",
		}
	}
	var issues []string
	...
	if api.Schema != "tetra.surface.component-tree-api.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree_api schema is %q, want tetra.surface.component-tree-api.v1",
				api.Schema,
			),
		)
	}
	if api.APILevel != "builder-layout-dispatch-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree_api api_level is %q, want builder-layout-dispatch-v1",
				api.APILevel,
			),
		)
	}
	if normalizeEvidencePath(api.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree_api source %q must match report source %q",
				api.Source,
				report.Source,
			),
		)
	...
	if api.Builder.RootCreatedBy != "tree_add_root" {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree_api builder root_created_by is %q, want tree_add_root",
				api.Builder.RootCreatedBy,
			),
		)
	}
	if api.Builder.ChildrenCreatedBy != "tree_add_child" {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree_api builder children_created_by is %q, want tree_add_child",
				api.Builder.ChildrenCreatedBy,
			),
		)
	}
	if api.Builder.NodeCount != tree.NodeCount {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree_api builder node_count = %d, want component_tree node_count %d",
				api.Builder.NodeCount,
				tree.NodeCount,
			),
		)
	}
	if api.Builder.Capacity < tree.NodeCount {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree_api builder capacity = %d, want at least node_count %d",
				api.Builder.Capacity,
				tree.NodeCount,
			),
		)
	}
	...
	if api.Invariants.TreeValidateStatus != 0 {
		issues = append(
			issues,
			fmt.Sprintf(
				"component_tree_api tree_validate_status = %d, want 0",
				api.Invariants.TreeValidateStatus,
			),
		)
	}
	if !api.Invariants.ParentChildLinksChecked || !api.Invariants.ChildIndicesChecked ||
		!api.Invariants.ChildCountChecked ||
		!api.Invariants.FirstChildChecked {
		issues = append(
			issues,
			("component_tree_api invariants must check parent/child links," +
				" child indices, child_count, and first_child"),
		)
	}
	if !hasComponentTreeAPILayout(
		api.LayoutHelpers,
		[]string{"tree_layout_column", "widgets.column_layout"},
		"Column",
	) {
		issues = append(
			issues,
			("component_tree_api layout_helpers require changed " +
				"tree_layout_column or widgets.column_layout evidence for " +
				"Column"),
		)
	}
	if !hasComponentTreeAPILayout(
		api.LayoutHelpers,
		[]string{"tree_layout_row", "widgets.row_layout"},
		"ButtonRow",
	) {
		issues = append(
			issues,
			("component_tree_api layout_helpers require changed " +
				"tree_layout_row or widgets.row_layout evidence for ButtonRow"),
		)
	}
	focusPairs := [][2]string{
	...
	for _, pair := range focusPairs {
		if !hasComponentTreeAPIFocusTransition(api.FocusHelpers, pair[0], pair[1]) {
			issues = append(
				issues,
				fmt.Sprintf("component_tree_api focus_helpers require %s -> %s", pair[0], pair[1]),
			)
		}
	}
	primaryTextBox := dispatchTargets[0]
	if !hasComponentTreeAPIHitTest(
		api.HitTests,
		primaryTextBox,
		expectedPathForComponentTreeTarget(nodes, expectedPaths, primaryTextBox),
	) {
		issues = append(
			issues,
			fmt.Sprintf("component_tree_api hit_tests require %s path evidence", primaryTextBox),
		)
	}
	if (accessibility || isProductionToolkitReport(report) || isToolkitReuseReport(report)) &&
		!hasComponentTreeAPIHitTest(
			api.HitTests,
			"EmailTextBox",
			expectedPathForComponentTreeTarget(nodes, expectedPaths, "EmailTextBox"),
		) {
	...
	if isProductionToolkitReport(report) &&
		!hasComponentTreeAPIHitTest(
			api.HitTests,
			"SubscribeCheckbox",
			expectedPathForComponentTreeTarget(nodes, expectedPaths, "SubscribeCheckbox"),
		) {
		issues = append(
			issues,
			"component_tree_api hit_tests require SubscribeCheckbox path evidence",
		)
	}
	if !hasComponentTreeAPIHitTest(
		api.HitTests,
		"ResetButton",
		expectedPathForComponentTreeTarget(nodes, expectedPaths, "ResetButton"),
	) &&
		!hasComponentTreeAPIHitTest(
			api.HitTests,
			"SubmitButton",
			expectedPathForComponentTreeTarget(nodes, expectedPaths, "SubmitButton"),
		) &&
		!hasComponentTreeAPIHitTest(
			api.HitTests,
			"SaveButton",
			expectedPathForComponentTreeTarget(nodes, expectedPaths, "SaveButton"),
		) {
	...
	for _, target := range dispatchTargets {
		...
		if !hasComponentTreeAPIDispatchPath(api.DispatchPaths, target, wantPath) {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree_api dispatch_paths require tree_build_dispatch_path %s path %v",
					target,
					wantPath,
				),
			)
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("component_tree_api report requires %s evidence", required),
			)
		}
	}
	return issues
...
func hasComponentTreeAPILayout(
	layouts []ComponentTreeAPILayoutHelperReport,
	helpers []string,
	target string,
) bool {
...
func hasComponentTreeAPIFocusTransition(
	focus []ComponentTreeAPIFocusHelperReport,
	before string,
	after string,
) bool {
...
func hasComponentTreeAPIHitTest(
	hits []ComponentTreeAPIHitTestReport,
	target string,
	wantPath []int,
) bool {
	if len(wantPath) == 0 {
	...
	for _, hit := range hits {
		if (hit.Helper == "tree_hit_test" || hit.Helper == "widgets.hit_test" || strings.HasPrefix(
			hit.Helper,
			"widgets.hit_test_",
		)) &&
			hit.Target == target &&
			intSlicesEqual(hit.Path, wantPath) {
			return true
		}
	}
	return false
...
func hasComponentTreeAPIDispatchPath(
	paths []ComponentTreeAPIDispatchPathReport,
	target string,
	wantPath []int,
) bool {
	for _, path := range paths {
		if path.Helper == "tree_build_dispatch_path" && path.Target == target &&
			intSlicesEqual(path.Path, wantPath) {
			return true
		}
	}
	return false
...
func expectedPathForComponentTreeTarget(
	nodes map[int]ComponentTreeNodeReport,
	expectedPaths map[int][]int,
	target string,
) []int {
...
func isSurfaceTreeAppSource(source string) bool {
	return strings.HasSuffix(
		normalizeEvidencePath(source),
		"examples/surface/toolkit/surface_tree_app.tetra",
	)
}

func componentTreeNodeByName(
	nodes []ComponentTreeNodeReport,
	name string,
) (ComponentTreeNodeReport, bool) {
	for _, node := range nodes {
		if node.Name == name {
			return node, true
		}
...
func componentTreeDrawOrderCoversNodes(
	drawOrder []int,
	nodes map[int]ComponentTreeNodeReport,
) bool {
...
func componentTreeHasResizeLayoutPass(passes []ComponentTreeLayoutPassReport, id int) bool {
	...
	for _, pass := range passes {
		if pass.ComponentID != id || pass.Bounds.W <= 0 || pass.Bounds.H <= 0 ||
			pass.Measured.W <= 0 ||
			pass.Measured.H <= 0 {
			continue
		}
...
func validateComponentTreeDispatchPaths(
	paths []ComponentTreeDispatchPathReport,
	...
	for _, path := range paths {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("component_tree dispatch path target_id %d is unknown", path.TargetID),
			)
			continue
		}
		if target.ChildCount != 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree dispatch path target_id %d must be a leaf",
					path.TargetID,
				),
			)
		}
		if strings.TrimSpace(path.Event) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree dispatch path target_id %d event is required",
					path.TargetID,
				),
			)
		}
		for _, id := range path.Path {
			if _, ok := nodes[id]; !ok {
				issues = append(
					issues,
					fmt.Sprintf(
						"component_tree dispatch path for target_id %d contains unknown node id %d",
						path.TargetID,
						id,
					),
				)
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree dispatch path cannot resolve parent chain for target_id %d",
					path.TargetID,
				),
			)
			continue
		...
		if !intSlicesEqual(path.Path, parentPath) {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree dispatch path for target_id %d = %v, want parent chain %v",
					path.TargetID,
					path.Path,
					parentPath,
				),
		...
		if want, ok := expected[path.TargetID]; ok && !intSlicesEqual(path.Path, want) {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree dispatch path for target_id %d = %v, want %v",
					path.TargetID,
					path.Path,
					want,
				),
		...
		if !rectContainsPoint(target.Bounds, path.X, path.Y) {
			issues = append(
				issues,
				fmt.Sprintf(
					"component_tree dispatch path target_id %d coordinates %d,%d are outside target bounds",
					path.TargetID,
					path.X,
					path.Y,
				),
			)
		} else if hitID, ok := componentTreeHitTest(
			nodes,
			path.X,
			path.Y,
		); !ok || hitID != path.TargetID {
			issues = append(issues, fmt.Sprintf((("component_tree dispatch path coordinates %d,%d "+
				"hit node %d, ")+
				"want target_id %d"), path.X, path.Y, hitID, path.TargetID))
		}
	...
	for targetID := range expected {
		...
		if !found {
			issues = append(
				issues,
				fmt.Sprintf("component_tree dispatch path missing target_id %d", targetID),
			)
		}
	}
	if len(uniqueLeafTargets) < 2 {
		issues = append(
			issues,
			"component_tree dispatch paths require at least two different leaf targets",
		)
	}
	return issues
}
...
func hasComponentTreeTextInsertion(events []EventReport) bool {
	for _, event := range events {
		if event.Kind != "text_input" || !event.Handled || !event.Pass ||
			!strings.HasSuffix(event.TargetComponent, "TextBox") {
			continue
		}
...
func hasComponentTreeResizeRelayout(
	events []EventReport,
	...
	for _, event := range events {
		...
		beforeFocus, beforeOK := stateValueWithSuffix(event.BeforeState, ".focused_id")
		afterFocus, afterOK := stateValueWithSuffix(event.AfterState, ".focused_id")
		if beforeOK && afterOK && beforeFocus == afterFocus &&
			textBoxBoundsChanged(event.BeforeState, event.AfterState) {
			seenEvent = true
		}
	}
	seenTransition := false
	for _, transition := range transitions {
		if transition.Cause == "resize" &&
			strings.HasSuffix(transition.Field, "TextBox.bounds.w") &&
...
func validateLinuxReleaseWindowEvidence(report Report) []string {
	...
	if report.Target != "linux-x64" {
		issues = append(
			issues,
			fmt.Sprintf("linux release target is %q, want linux-x64", report.Target),
		)
	}
	if !isSurfaceReleaseFormSource(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				("linux release source path must match "+
	...
	if report.HostEvidence.Level != "linux-x64-release-window-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"linux release host_evidence.level is %q, want linux-x64-release-window-v1",
				report.HostEvidence.Level,
			),
		)
	}
	...
	if !hasFrameOrderDimensions(report.Frames, 5, 560, 420, 2240) {
		issues = append(
			issues,
			"linux release requires order-5 560x420 real-window presented frame evidence",
		)
	}
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("linux release report requires %s evidence", required),
			)
		}
	}
	...
	if !hasRuntimeProcessName(report.Processes, "surface linux accessibility platform probe") {
		issues = append(
			issues,
			"linux release requires accessibility platform probe process evidence",
		)
	}
	if !artifactKindContains(report.Artifacts, "linux-accessibility-platform-probe") {
		issues = append(
			issues,
			"linux release requires linux-accessibility-platform-probe artifact",
		)
	}
	if report.Toolkit == nil || report.Toolkit.ToolkitLevel != "production-widgets-v1" {
	...
	if report.AccessibilityTree == nil ||
		...
		issues = append(
			issues,
			"linux release requires platform-bridge-v1 accessibility_tree evidence",
		)
	}
	return issues
}
...
func validateLinuxAppShellEvidence(report Report) []string {
	...
	if report.Target != "linux-x64" {
		issues = append(
			issues,
			fmt.Sprintf("linux app-shell target is %q, want linux-x64", report.Target),
		)
	}
	if !isSurfaceLinuxAppShellNotesSource(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				("linux app-shell source path must match "+
	...
	if report.HostEvidence.Level != "linux-x64-release-window-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"linux app-shell host_evidence.level is %q, want linux-x64-release-window-v1",
				report.HostEvidence.Level,
			),
		)
	}
	if report.HostEvidence.Backend != "wayland-shm-rgba-release-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"linux app-shell host_evidence.backend is %q, want wayland-shm-rgba-release-v1",
				report.HostEvidence.Backend,
			),
		)
	}
	for _, check := range []struct {
		...
		if !check.ok {
			issues = append(
				issues,
				fmt.Sprintf("linux app-shell host_evidence.%s must be true", check.name),
			)
		}
	}
	if report.HostEvidence.UserFacingPlatformWidgets {
		issues = append(
			issues,
			"linux app-shell must not expose GTK/Qt/native widget UI or user-facing platform widgets",
		)
	}
	if !hasFrameOrderDimensions(report.Frames, 6, 720, 540, 2880) {
		issues = append(
			issues,
			"linux app-shell requires order-6 720x540 presented resize/DPI frame evidence",
		)
	}
	for _, kind := range []string{"mouse_up", "key_down", "text_input", "resize", "close"} {
		if !eventKindContains(report.Events, kind) {
	...
	for _, process := range []string{
		...
		if !hasRuntimeProcessName(report.Processes, process) {
			issues = append(
				issues,
				fmt.Sprintf("linux app-shell requires %s process evidence", process),
			)
		}
	}
	for _, kind := range []string{
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("linux app-shell report requires %s evidence", required),
			)
		}
	}
	if report.LinuxAppShell == nil {
		return append(
			issues,
			("linux_app_shell evidence is required for " +
				"examples/surface/toolkit/surface_linux_app_shell_notes.tetra"),
		)
	}
	...
	if normalizeEvidencePath(app.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"linux_app_shell source %q must match report source %q",
				app.Source,
				report.Source,
			),
		)
	...
	issues = append(
		issues,
		validateLinuxAppShellCapability(
			"clipboard",
			app.Clipboard,
			"clipboard-text-v1",
			"linux-app-shell-host-trace",
	...
	issues = append(
		...
		validateLinuxAppShellCapability(
			"ime",
			app.IME,
			"composition-baseline-v1",
			"linux-app-shell-host-trace",
			[]string{"start", "update", "commit", "cancel"},
		)...)
	issues = append(
		issues,
		validateLinuxAppShellCapability(
			...
			"platform-bridge-v1",
			"linux-accessibility-platform-probe",
			[]string{"metadata_tree", "platform_export"},
		)...)
...
func validateLinuxAppShellLifecycle(rows []LinuxAppShellLifecycleReport) []string {
	...
	if len(rows) < 3 {
		issues = append(
			issues,
			"linux_app_shell window_lifecycle requires open, close, and reopen evidence",
		)
	}
	seen := map[string]bool{}
	for _, row := range rows {
		...
		if !row.HostTrace || !row.Pass {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell lifecycle %s must have host_trace=true and pass=true",
					row.Operation,
				),
			)
		}
...
func validateLinuxAppShellWindows(rows []LinuxAppShellWindowReport) []string {
	...
	if len(rows) < 2 {
		issues = append(
			issues,
			"linux_app_shell windows requires at least two real windows for multi-window notes",
		)
	}
	seen := map[string]bool{}
	for _, row := range rows {
		...
		if !row.RealWindow || !row.Presented {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell window %s must be real_window=true and presented=true",
					row.ID,
				),
			)
		}
		if row.Width <= 0 || row.Height <= 0 || row.DPIScaleMilli <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell window %s requires positive size and dpi_scale_milli",
					row.ID,
				),
			)
		}
	...
	for _, id := range []string{"notes-main", "notes-inspector"} {
		if !seen[id] {
			issues = append(
				issues,
				fmt.Sprintf("linux_app_shell windows missing %s multi-window notes evidence", id),
			)
		}
	}
	return issues
	...
	for _, row := range rows {
		if !row.HostTrace || !row.Pass {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell resize_dpi %s must have host_trace=true and pass=true",
					row.Operation,
				),
			)
		}
		if row.BeforeWidth <= 0 || row.BeforeHeight <= 0 || row.AfterWidth <= 0 ||
			row.AfterHeight <= 0 ||
			row.DPIScaleMilli <= 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell resize_dpi %s requires positive dimensions and dpi_scale_milli",
					row.Operation,
				),
			)
		}
...
func validateLinuxAppShellCursors(rows []LinuxAppShellCursorReport) []string {
	...
	for _, row := range rows {
		if !row.HostTrace || !row.Pass {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell cursor %s must have host_trace=true and pass=true",
					row.Cursor,
				),
			)
		}
		if strings.TrimSpace(row.Target) == "" {
			issues = append(
				issues,
				fmt.Sprintf("linux_app_shell cursor %s target is required", row.Cursor),
			)
		}
	...
	for _, cursor := range []string{"pointer", "text", "resize"} {
		if !seen[cursor] {
			issues = append(
				issues,
				fmt.Sprintf("linux_app_shell cursor_transitions missing %s", cursor),
			)
		}
	}
	return issues
...
func validateLinuxAppShellCapability(
	name string,
	cap LinuxAppShellCapabilityReport,
	wantLevel string,
	...
	if cap.Level != wantLevel {
		issues = append(
			issues,
			fmt.Sprintf("linux_app_shell %s level is %q, want %q", name, cap.Level, wantLevel),
		)
	}
	if !cap.HostTrace || !cap.Pass {
		issues = append(
			issues,
			fmt.Sprintf("linux_app_shell %s requires host_trace=true and pass=true", name),
		)
	}
	if cap.ArtifactKind != wantArtifactKind {
		issues = append(
			issues,
			fmt.Sprintf(
				"linux_app_shell %s artifact_kind is %q, want %q",
				name,
				cap.ArtifactKind,
				wantArtifactKind,
			),
...
func validateLinuxAppShellFeatures(rows []LinuxAppShellFeatureReport) []string {
	...
	for _, row := range rows {
		...
		if !row.Pass || !row.HostTrace || !row.NoNativeWidgetUI {
			issues = append(
				issues,
				fmt.Sprintf(
					("linux_app_shell shell_feature %s requires host_trace=true, "+
						"no_native_widget_ui=true, and pass=true"),
					row.Name,
				),
			)
		}
		if !linuxAppShellKnownFeature(row.Name) {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell shell_feature %s is not part of the supported P16 feature ledger",
					row.Name,
				),
			)
		}
	...
	for _, name := range []string{
		...
		if feature.Status != "target_evidenced" || !feature.Claimed ||
			strings.TrimSpace(feature.BlockedReason) != "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell %s must be target_evidenced, claimed=true, and carry no blocked_reason",
					name,
				),
			)
		}
	...
	for _, name := range []string{"app_menu", "crash_recovery", "error_report"} {
		...
		if feature.Status != "scoped_adapter" || !feature.Claimed ||
			...
			issues = append(
				issues,
				fmt.Sprintf(
					("linux_app_shell %s must be a claimed scoped_adapter with "+
						"target evidence, not GTK/Qt/native widget UI"),
					name,
				),
			)
		}
	...
	for _, name := range []string{
		...
		if feature.Status != "blocked_pass" || feature.Claimed ||
			strings.TrimSpace(feature.BlockedReason) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"linux_app_shell %s requires blocked_pass nonclaim until target evidence exists",
					name,
				),
			)
		}
...
func validateLinuxAppShellHostTraces(rows []LinuxAppShellHostTraceReport) []string {
	...
	for _, row := range rows {
		if !row.Pass || strings.TrimSpace(row.Path) == "" {
			issues = append(
				issues,
				fmt.Sprintf("linux_app_shell host_trace %s requires path and pass=true", row.Name),
			)
		}
...
func validateLinuxAppShellNegativeGuards(guards LinuxAppShellNegativeGuards) []string {
	...
	return []string{
		("linux_app_shell negative_guards must reject GTK/Qt/native " +
			"widget UI, Electron/React runtimes, DOM UI, user JS, and " +
			"platform widgets"),
	}
}
...
func ValidateTextInputReport(raw []byte) error {
	...
	if report.Schema != TextInputSchemaV1 {
		issues = append(
			issues,
			fmt.Sprintf("schema is %q, want %q", report.Schema, TextInputSchemaV1),
		)
	}
	switch report.Target {
	...
	if normalizeEvidencePath(
		...
		issues = append(
			issues,
			fmt.Sprintf(
				"source is %q, want examples/surface/release/surface_release_text_input.tetra",
				report.Source,
			),
		)
	}
	if report.Level != "production-text-input-v1" {
		issues = append(
			issues,
			fmt.Sprintf("level is %q, want production-text-input-v1", report.Level),
		)
	}
	...
	if report.Storage != "owned-utf8-byte-buffer" {
		issues = append(
			issues,
			fmt.Sprintf("storage is %q, want owned-utf8-byte-buffer", report.Storage),
		)
	}
	for _, check := range []struct {
		field string
	...
	issues = append(
		issues,
		validateExactStringList("unsupported_claims", report.UnsupportedClaims, []string{
	...
	issues = append(issues, validateCases(Report{Cases: report.Cases})...)
	for _, required := range []string{
...
func validateTextShapingPlan(plan TextShapingPlanReport) []string {
	...
	if plan.QualityLevel != "scoped-text-shaping-plan-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"text_shaping_plan.quality_level is %q, want scoped-text-shaping-plan-v1",
				plan.QualityLevel,
			),
		)
	}
	...
	if plan.GraphemeBoundaries != "byte-offset-codepoint-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"text_shaping_plan.grapheme_boundaries is %q, want byte-offset-codepoint-v1",
				plan.GraphemeBoundaries,
			),
		)
	}
	if plan.LineBreaking != "newline-storage-plus-wrap-plan-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"text_shaping_plan.line_breaking is %q, want newline-storage-plus-wrap-plan-v1",
				plan.LineBreaking,
			),
		)
	}
	if plan.Bidi != "nonclaim-full-bidi-v1" {
		issues = append(
			issues,
			fmt.Sprintf("text_shaping_plan.bidi is %q, want nonclaim-full-bidi-v1", plan.Bidi),
		)
	}
	if plan.RichText != "nonclaim-rich-text-editor-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"text_shaping_plan.rich_text is %q, want nonclaim-rich-text-editor-v1",
				plan.RichText,
			),
		)
	}
...
func validateTextInputReferenceTraces(traces []TextInputReferenceTraceReport) []string {
	...
	for _, source := range required {
		...
		if !trace.Pass || !trace.Focus || !trace.Selection || !trace.Clipboard ||
			!trace.Composition ||
			!trace.Multiline {
			issues = append(
				issues,
				fmt.Sprintf(
					"reference_traces %s must pass focus, selection, clipboard, composition, and multiline checks",
					source,
				),
			)
		}
...
func validateMinimalToolkitEvidence(report Report) []string {
	...
	if !isSurfaceToolkitFormSource(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				("minimal toolkit source path must match "+
					"examples/surface/toolkit/surface_toolkit_form.tetra, got %q"),
				report.Source,
			),
		)
	}
	if report.Toolkit == nil {
		return append(
			issues,
			"toolkit evidence is required for examples/surface/toolkit/surface_toolkit_form.tetra",
		)
	}
	toolkit := report.Toolkit
	if toolkit.Schema != "tetra.surface.toolkit.v1" {
		issues = append(
			issues,
			fmt.Sprintf("toolkit schema is %q, want tetra.surface.toolkit.v1", toolkit.Schema),
		)
	}
	if toolkit.ToolkitLevel != "minimal-widgets-v1" {
		issues = append(
			issues,
			fmt.Sprintf("toolkit_level is %q, want minimal-widgets-v1", toolkit.ToolkitLevel),
		)
	}
	if normalizeEvidencePath(toolkit.Source) != normalizeEvidencePath(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				"toolkit source %q must match report source %q",
				toolkit.Source,
				report.Source,
			),
		)
	...
	if toolkit.Module != "lib.core.widgets" {
		issues = append(
			issues,
			fmt.Sprintf("toolkit module is %q, want lib.core.widgets", toolkit.Module),
		)
	}
	...
	if toolkit.ProductionClaim {
		issues = append(
			issues,
			"toolkit production_claim must be false for experimental minimal toolkit evidence",
		)
	}
	...
	if !toolkit.NoMagicWidgets || !toolkit.NoPlatformWidgets || !toolkit.NoDOMUI ||
		...
		issues = append(
			issues,
			"toolkit must prove no_magic_widgets, no_platform_widgets, no_dom_ui, and no_user_js",
		)
	}
	...
	for _, widget := range toolkit.Widgets {
		...
		if widget.NodeID < 0 {
			issues = append(
				issues,
				fmt.Sprintf("toolkit widget %s node_id must be non-negative", widget.Name),
			)
		}
		if !widget.Reusable || !widget.OrdinaryTetraStruct {
			issues = append(
				issues,
				fmt.Sprintf(
					"toolkit widget %s must be reusable ordinary Tetra struct evidence",
					widget.Name,
				),
			)
		}
	...
	for _, required := range []struct {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("toolkit widget evidence missing %s", required.name),
			)
			continue
		}
		if widget.Kind != required.kind {
			issues = append(
				issues,
				fmt.Sprintf(
					"toolkit widget %s kind is %q, want %q",
					required.name,
					widget.Kind,
					required.kind,
				),
		...
		if widget.NodeID != required.nodeID {
			issues = append(
				issues,
				fmt.Sprintf(
					"toolkit widget %s node_id = %d, want %d",
					required.name,
					widget.NodeID,
					required.nodeID,
				),
		...
		if required.role != "" && widget.Role != required.role {
			issues = append(
				issues,
				fmt.Sprintf(
					"toolkit widget %s role is %q, want %q",
					required.name,
					widget.Role,
					required.role,
				),
		...
		if required.action != "" && widget.Action != required.action {
			issues = append(
				issues,
				fmt.Sprintf(
					"toolkit widget %s action is %q, want %q",
					required.name,
					widget.Action,
					required.action,
				),
	...
	if report.ComponentTree == nil {
		...
		for name, widget := range widgets {
			...
			if !ok {
				issues = append(
					issues,
					fmt.Sprintf("toolkit widget %s node_id %d is not in component_tree", name, widget.NodeID),
				)
				continue
			}
			if node.Name != widget.Name {
				issues = append(
					issues,
					fmt.Sprintf(
						"toolkit widget %s node_id %d names component_tree node %s",
						name,
						widget.NodeID,
						node.Name,
					),
	...
	if !hasEventTargetKind(report.Events, "TextBox", "text_input") ||
		!hasKeyEvent(report.Events, 37) ||
		!hasKeyEvent(report.Events, 8) ||
		!hasKeyEvent(report.Events, 46) {
		issues = append(
			issues,
			"toolkit TextBox requires OK text insertion plus caret, backspace, and delete evidence",
		)
	}
	if !hasMinimalToolkitButtonAction(
		report.Events,
		"SubmitButton",
		"submit_count",
		...
		issues = append(
			issues,
			("minimal toolkit SubmitButton action requires focused " +
				"root-to-leaf dispatch path and status update"),
		)
	}
	if !hasMinimalToolkitButtonAction(
		report.Events,
		...
		issues = append(
			issues,
			("minimal toolkit ResetButton action requires focused " +
				"root-to-leaf dispatch path and status update"),
		)
	}
	if !hasTransition(report.StateTransitions, "StatusText", "status_code") {
		issues = append(
			issues,
			"minimal toolkit requires StatusText status_code transition evidence",
		)
	}
	if !hasTransition(report.StateTransitions, "ToolkitFormApp", "TextBox.bounds.w") {
		issues = append(
			issues,
			"minimal toolkit requires resize relayout evidence for TextBox.bounds.w",
		)
	}
	if len(report.Frames) >= 2 &&
		report.Frames[0].Checksum == report.Frames[len(report.Frames)-1].Checksum {
		issues = append(
			issues,
			"minimal toolkit rendered frame update requires changed frame checksum",
		)
	}
	return issues
}
...
func validateToolkitReuseEvidence(report Report) []string {
	...
	if !isSurfaceToolkitSettingsSource(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				("toolkit reuse source path must match "+
					"examples/surface/toolkit/surface_toolkit_settings.tetra, "+
					"got %q"),
				report.Source,
			),
	...
	if report.Toolkit == nil {
		return append(
			issues,
			"toolkit evidence is required for examples/surface/toolkit/surface_toolkit_settings.tetra",
		)
	}
	toolkit := report.Toolkit
	if toolkit.Schema != "tetra.surface.toolkit.v1" {
	...
	if toolkit.ToolkitLevel != "toolkit-reuse-v1" {
		issues = append(
			issues,
			fmt.Sprintf("toolkit_level is %q, want toolkit-reuse-v1", toolkit.ToolkitLevel),
		)
	}
	if toolkit.ReuseLevel != "multi-form-widget-reuse-v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"toolkit reuse_level is %q, want multi-form-widget-reuse-v1",
				toolkit.ReuseLevel,
			),
		)
	}
	...
	if toolkit.ProductionClaim {
		issues = append(
			issues,
			"toolkit production_claim must be false for experimental toolkit reuse evidence",
		)
	}
	if !toolkit.UsesComponentTreeAPI {
		issues = append(issues, "toolkit uses_component_tree_api must be true")
	...
	if toolkit.ExampleCount < 2 ||
		!contains(toolkit.Sources, "examples/surface/toolkit/surface_toolkit_form.tetra") ||
		!contains(toolkit.Sources, "examples/surface/toolkit/surface_toolkit_settings.tetra") {
		issues = append(
			issues,
			("toolkit reuse example_count must cover both " +
				"surface_toolkit_form and surface_toolkit_settings examples"),
		)
	}
	if toolkit.TextBoxCount < 2 || !toolkit.MultiTextBoxEvidence {
		issues = append(
			issues,
			"toolkit reuse requires text_box_count >= 2 with multi_textbox_evidence",
		)
	}
	...
	for _, widget := range toolkit.Widgets {
		...
		if prev, exists := ids[widget.NodeID]; exists {
			issues = append(
				issues,
				fmt.Sprintf(
					"toolkit duplicate widget node_id %d used by %s and %s",
					widget.NodeID,
					prev,
					widget.Name,
				),
	...
	for _, required := range []struct {
		...
		if required.kind == "TextBox" && !widget.Editable {
			issues = append(
				issues,
				fmt.Sprintf("toolkit widget %s must prove editable=true", required.name),
			)
		}
	}
	treeNodes := map[int]ComponentTreeNodeReport{}
	...
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("toolkit reuse report requires %s evidence", required),
			)
		}
	}
	...
	if !hasEventTargetKind(report.Events, "NameTextBox", "text_input") ||
		!hasEventTargetKind(report.Events, "EmailTextBox", "text_input") {
		issues = append(
			issues,
			"toolkit reuse requires NameTextBox and EmailTextBox text_input routing evidence",
		)
	}
	if !toolkitTextInputMutatesOnlyFocused(report.Events, "NameTextBox") ||
		!toolkitTextInputMutatesOnlyFocused(report.Events, "EmailTextBox") {
	...
	if !hasToolkitReuseButtonAction(report.Events, "SaveButton", ".save_count", false) {
		issues = append(
			issues,
			"toolkit reuse SaveButton action requires focused root-to-leaf dispatch path and status update",
		)
	}
	if !hasToolkitReuseButtonAction(report.Events, "ResetButton", ".reset_count", true) {
		issues = append(
			issues,
			("toolkit reuse ResetButton action requires focused " +
				"root-to-leaf dispatch path and TextBox clears"),
		)
	}
	if !hasTransition(report.StateTransitions, "StatusText", "status_code") {
		issues = append(issues, "toolkit reuse requires StatusText status_code transition evidence")
	}
	if !hasTransition(report.StateTransitions, "ToolkitSettingsApp", "NameTextBox.bounds.w") ||
		!hasTransition(report.StateTransitions, "ToolkitSettingsApp", "EmailTextBox.bounds.w") {
		issues = append(
			issues,
			"toolkit reuse requires resize relayout evidence for both TextBoxes",
		)
	}
	if len(report.Frames) >= 2 &&
		report.Frames[0].Checksum == report.Frames[len(report.Frames)-1].Checksum {
		issues = append(
			issues,
			"toolkit reuse rendered frame update requires changed frame checksum",
		)
	}
	if report.Target == "wasm32-web" &&
		!isBrowserCanvasHostEvidenceLevel(report.HostEvidence.Level) {
		issues = append(
			issues,
			"toolkit reuse browser evidence must be browser-canvas input, not Node-only wasm32-web evidence",
		)
	}
	return issues
}
...
func validateProductionToolkitEvidence(report Report) []string {
	...
	if !isSurfaceReleaseFormSource(report.Source) {
		issues = append(
			issues,
			fmt.Sprintf(
				("production toolkit source path must match "+
					"examples/surface/release/surface_release_form.tetra, got %q"),
				report.Source,
			),
		)
	...
	if report.Toolkit == nil {
		return append(
			issues,
			"toolkit evidence is required for examples/surface/release/surface_release_form.tetra",
		)
	}
	toolkit := report.Toolkit
	if toolkit.Schema != "tetra.surface.toolkit.v1" {
	...
	if toolkit.ToolkitLevel != "production-widgets-v1" {
		issues = append(
			issues,
			fmt.Sprintf("toolkit_level is %q, want production-widgets-v1", toolkit.ToolkitLevel),
		)
	}
	if toolkit.ReleaseScope != "surface-v1-linux-web" {
		issues = append(
			issues,
			fmt.Sprintf(
				"toolkit release_scope is %q, want surface-v1-linux-web",
				toolkit.ReleaseScope,
			),
		)
	}
	if normalizeEvidencePath(toolkit.Source) != normalizeEvidencePath(report.Source) {
	...
	if toolkit.StyleModule != "lib.core.style" {
		issues = append(
			issues,
			fmt.Sprintf("toolkit style_module is %q, want lib.core.style", toolkit.StyleModule),
		)
	}
	if toolkit.Experimental {
	...
	if !toolkit.NoMagicWidgets || !toolkit.NoPlatformWidgets || !toolkit.NoDOMUI ||
		...
		issues = append(
			issues,
			"production toolkit must prove no_magic_widgets, no_platform_widgets, no_dom_ui, and no_user_js",
		)
	}
	...
	if toolkit.ExampleCount < len(requiredSources) {
		issues = append(
			issues,
			fmt.Sprintf(
				"production toolkit example_count = %d, want at least %d scoped examples",
				toolkit.ExampleCount,
				len(requiredSources),
			),
		)
	}
	...
	if toolkit.TextBoxCount < 2 || !toolkit.MultiTextBoxEvidence {
		issues = append(
			issues,
			"production toolkit requires two TextBox widgets with multi_textbox_evidence",
		)
	}
	if toolkit.ButtonCount < 2 || !toolkit.MultiFormEvidence {
		issues = append(
			issues,
			"production toolkit requires at least two Button widgets with multi_form_evidence",
		)
	}
	for _, required := range []string{
		"Text",
		...
		if !contains(toolkit.WidgetSet, required) {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit widget_set missing %s", required),
			)
		}
	}
	for _, required := range []string{"normal", "focused", "hovered", "pressed", "disabled", "error"} {
		if !contains(toolkit.StateSet, required) {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit state_set missing %s", required),
			)
		}
	}
	for _, required := range []string{
		...
		if !contains(toolkit.LayoutFeatures, required) {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit layout_features missing %s", required),
			)
		}
	}
	if !toolkit.Theme {
	...
	for _, widget := range toolkit.Widgets {
		...
		if _, exists := widgets[widget.Name]; exists {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit duplicate widget %s", widget.Name),
			)
		}
		widgets[widget.Name] = widget
		kindCounts[widget.Kind]++
		if prev, exists := ids[widget.NodeID]; exists {
			issues = append(
				issues,
				fmt.Sprintf(
					"production toolkit duplicate widget node_id %d used by %s and %s",
					widget.NodeID,
					prev,
					widget.Name,
				),
		...
		if !widget.Reusable || !widget.OrdinaryTetraStruct {
			issues = append(
				issues,
				fmt.Sprintf(
					"production toolkit widget %s must be reusable ordinary Tetra struct evidence",
					widget.Name,
				),
			)
		}
	...
	for _, requiredKind := range []string{
		...
		if kindCounts[requiredKind] == 0 {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit widget evidence missing kind %s", requiredKind),
			)
		}
	}
	for _, required := range []struct {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit widget evidence missing %s", required.name),
			)
			continue
		}
		if widget.Kind != required.kind {
			issues = append(
				issues,
				fmt.Sprintf(
					"production toolkit widget %s kind is %q, want %q",
					required.name,
					widget.Kind,
					required.kind,
				),
		...
		if widget.NodeID != required.nodeID {
			issues = append(
				issues,
				fmt.Sprintf(
					"production toolkit widget %s node_id = %d, want %d",
					required.name,
					widget.NodeID,
					required.nodeID,
				),
		...
		if required.role != "" && widget.Role != required.role {
			issues = append(
				issues,
				fmt.Sprintf(
					"production toolkit widget %s role is %q, want %q",
					required.name,
					widget.Role,
					required.role,
				),
		...
		if required.action != "" && widget.Action != required.action {
			issues = append(
				issues,
				fmt.Sprintf(
					"production toolkit widget %s action is %q, want %q",
					required.name,
					widget.Action,
					required.action,
				),
		...
		if required.kind == "TextBox" && !widget.Editable {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit widget %s must prove editable=true", required.name),
			)
		}
	}
	if report.ComponentTree == nil {
		...
		for name, widget := range widgets {
			node, ok := treeNodes[widget.NodeID]
			if !ok {
				issues = append(issues, fmt.Sprintf(("production toolkit widget %s node_id %d is not in "+
					"component_tree"), name, widget.NodeID))
				continue
			}
			if node.Name != widget.Name {
				issues = append(issues, fmt.Sprintf(("production toolkit widget %s node_id %d names "+
					"component_tree node %s"), name, widget.NodeID, node.Name))
			}
		}
	}
	for _, source := range []string{
		...
		if !contains(toolkit.ReusableSources, source) {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit reusable_sources missing %s", source),
			)
		}
	}
	for _, required := range []string{
		...
		if !caseNameContains(report.Cases, required) {
			issues = append(
				issues,
				fmt.Sprintf("production toolkit report requires %s evidence", required),
			)
		}
	}
	if !hasEventTargetKind(report.Events, "NameTextBox", "mouse_up") {
	...
	if !hasEventTargetKind(report.Events, "NameTextBox", "text_input") ||
		...
		issues = append(
			issues,
			"production toolkit requires NameTextBox and EmailTextBox text_input routing evidence",
		)
	}
	if !hasEventTargetKind(report.Events, "SubscribeCheckbox", "key_down") ||
		!hasTransition(report.StateTransitions, "SubscribeCheckbox", "checked") {
		issues = append(
			issues,
			"production toolkit requires Checkbox keyboard toggle and checked transition evidence",
		)
	}
	if !eventKindContains(report.Events, "scroll") ||
		!hasTransition(report.StateTransitions, "TermsScroll", "offset_y") {
		issues = append(
			issues,
			"production toolkit requires Scroll event and offset transition evidence",
		)
	}
	if !hasToolkitReuseButtonAction(report.Events, "SaveButton", ".save_count", false) {
		issues = append(
			issues,
			("production toolkit SaveButton action requires focused " +
				"root-to-leaf dispatch path and status update"),
		)
	}
	if !hasToolkitReuseButtonAction(report.Events, "ResetButton", ".reset_count", true) {
		issues = append(
			issues,
			("production toolkit ResetButton action requires focused " +
				"root-to-leaf dispatch path and TextBox clears"),
		)
	}
	if !hasTransition(report.StateTransitions, "StatusText", "status_code") {
		issues = append(
			issues,
			"production toolkit requires StatusText status_code transition evidence",
		)
	}
	if !hasTransition(report.StateTransitions, "SurfaceReleaseFormApp", "NameTextBox.bounds.w") ||
		!hasTransition(report.StateTransitions, "SurfaceReleaseFormApp", "EmailTextBox.bounds.w") {
		issues = append(
			issues,
			"production toolkit requires resize relayout evidence for both TextBoxes",
		)
	}
	if len(report.Frames) >= 2 &&
		report.Frames[0].Checksum == report.Frames[len(report.Frames)-1].Checksum {
		issues = append(
			issues,
			"production toolkit rendered frame update requires changed frame checksum",
		)
	}
	if report.Target == "wasm32-web" &&
		!isBrowserCanvasHostEvidenceLevel(report.HostEvidence.Level) {
		issues = append(
			issues,
			("production toolkit browser evidence must be browser-canvas " +
				"input, not Node-only wasm32-web evidence"),
		)
	}
	return issues
}

func hasMinimalToolkitButtonAction(
	events []EventReport,
	target string,
	countSuffix string,
	statusKey string,
	...
	for _, event := range events {
		if event.TargetComponent != target || event.Kind != "key_down" || !event.Handled ||
			!event.Pass {
			continue
		}
...
func isToolkitReuseReport(report Report) bool {
	...
	if report.Toolkit != nil &&
		(report.Toolkit.ToolkitLevel == "toolkit-reuse-v1" || report.Toolkit.ReuseLevel == "multi-form-widget-reuse-v1") {
		return true
	}
...
func toolkitTextInputMutatesOnlyFocused(events []EventReport, target string) bool {
	for _, event := range events {
		if event.Kind != "text_input" || event.TargetComponent != target || !event.Handled ||
			!event.Pass {
			continue
		}
		targetChanged := false
...
func hasToolkitReuseButtonAction(
	events []EventReport,
	target string,
	countSuffix string,
	clearsTextBoxes bool,
) bool {
	for _, event := range events {
		if event.TargetComponent != target || event.Kind != "key_down" || !event.Handled ||
			!event.Pass {
		...
		if clearsTextBoxes &&
			(event.AfterState["NameTextBox.buffer"] != "" || event.AfterState["EmailTextBox.buffer"] != "") {
			continue
		}
		return true
	}
