...
func validateTargetRuntimeEvidence(report Report) []string {
	...
	case "wasm32-web":
		...
		if isBrowserCanvasHostEvidenceLevel(report.HostEvidence.Level) {
			...
			} else if !hasFrameOrderDimensions(report.Frames, requiredFrameOrder, requiredFrameWidth, requiredFrameHeight, requiredFrameStride) {
				issues = append(issues, fmt.Sprintf("wasm32-web browser canvas target requires order-%d %dx%d canvas readback frame evidence", requiredFrameOrder, requiredFrameWidth, requiredFrameHeight))
			}
		} else {
