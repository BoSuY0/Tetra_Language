...
func TestValidateReportRejectsMissingToolkitWidgetCoverage(t *testing.T) {
	raw := strings.Replace(string(validToolkitCoreReport(t)), `    {"id":"MenuItemOpen","kind":"menu-item","parent":"FileMenu","binding":"command.open","event":"activate","command":"openDialog","enabled":true,"visible":true,"focusable":true,"bounds":{"x":0,"y":0,"width":120,"height":24},"layout":{"kind":"menu","order":0},"style":{"class":"menu-item"},"accessibility":{"role":"menuitem","label":"Open","description":"Open dialog","focus_order":9,"keyboard_activation":["enter"]}},
`, "", 1)
...
func TestValidateReportRejectsMissingEventAndStateEvidence(t *testing.T) {
	raw := strings.Replace(string(validToolkitCoreReport(t)), `    {"order":8,"widget_id":"NameInput","event":"input","command":"setName","pass":true,"before_state":{"AppState.name":"tetra"},"after_state":{"AppState.name":"tetra-toolkit"},"operations":[{"kind":"two_way_bind","target":"state.name","value":"tetra-toolkit","state_field":"name","state_value":"tetra-toolkit"}],"widget_updates":[{"id":"NameInput","before":"tetra","after":"tetra-toolkit"}]},
`, "", 1)
	raw = strings.Replace(raw, `    {"name":"two-way input binding","before":{"AppState.name":"tetra"},"after":{"AppState.name":"tetra-toolkit"},"operations":["two_way_bind"],"widgets":["NameInput"]},
`, "", 1)
	err := ValidateReport([]byte(raw))
	if err == nil {
...
func TestValidateReportRejectsMissingLayoutFocusAccessibility(t *testing.T) {
	raw := strings.Replace(string(validToolkitCoreReport(t)), `    {"kind":"grid","widgets":["Toolbar","ContentPanel"],"pass":true,"evidence":"grid columns placed deterministically"},
`, "", 1)
	raw = strings.Replace(raw, `"focus_order":6`, `"focus_order":0`, 1)
	err := ValidateReport([]byte(raw))
	if err == nil {
...
func validToolkitCoreReport(t *testing.T) []byte {
  ...
  "artifacts": [
    {"name":"toolkit bundle","kind":"bundle","path":%q,"schema":"tetra.ui.toolkit.v1","sha256":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},
    {"name":"runtime trace","kind":"trace","path":%q,"schema":"tetra.ui.toolkit.trace.v1","sha256":"sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}
  ],
  "processes": [
