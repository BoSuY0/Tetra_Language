...
func TestValidateReportRejectsMissingProductionUICoverage(t *testing.T) {
	raw := strings.Replace(validUIProductionRuntimeReport(), `    {"name":"async UI commands","status":"pass","evidence":"async command completion case runs through the UI runtime"},
...
func TestValidateReportRejectsMissingInputFocusChangeEvidence(t *testing.T) {
	raw := validUIProductionRuntimeReport()
	raw = strings.Replace(raw, `    {"order":1,"widget_id":"NameInput","event":"focus","command":"focusName","pass":true,"before_state":{"AppState.focused":"none"},"after_state":{"AppState.focused":"NameInput"},"operations":[{"kind":"focus","target":"widget.NameInput","value":"focused","state_field":"focused","state_value":"NameInput"}],"widget_updates":[{"id":"TitleText","before":"Ready","after":"Editing name"}]},
`, "", 1)
	raw = strings.Replace(raw, `    {"order":3,"widget_id":"NameInput","event":"change","command":"commitName","pass":true,"before_state":{"AppState.name":"tetra-lang","AppState.changed":"false"},"after_state":{"AppState.name":"tetra-prod","AppState.changed":"true"},"operations":[{"kind":"change","target":"state.name","value":"tetra-prod","state_field":"name","state_value":"tetra-prod"},{"kind":"state_set","target":"state.changed","value":"true","state_field":"changed","state_value":"true"}],"widget_updates":[{"id":"NameInput","before":"tetra-lang","after":"tetra-prod"}]},
`, "", 1)
	raw = strings.Replace(raw, `    {"name":"input focus traversal","kind":"positive","ran":true,"pass":true},
`, "", 1)
	raw = strings.Replace(raw, `    {"name":"input change commit","kind":"positive","ran":true,"pass":true},
`, "", 1)
	err := ValidateReport([]byte(raw))
	if err == nil {
...
func TestValidateReportRejectsMissingCompilerBundleRuntimeLoad(t *testing.T) {
	raw := strings.Replace(validUIProductionRuntimeReport(), `    {"name":"compiler UI bundle runtime load","kind":"positive","ran":true,"pass":true},
`, "", 1)
	err := ValidateReport([]byte(raw))
	if err == nil {
...
func TestValidateReportRejectsMissingNativeRuntimeIntegration(t *testing.T) {
	raw := strings.Replace(validUIProductionRuntimeReport(), `    {"name":"native shell runtime integration","kind":"positive","ran":true,"pass":true},
`, "", 1)
	err := ValidateReport([]byte(raw))
	if err == nil {
...
func TestValidateReportRejectsMissingNativeRuntimeProcess(t *testing.T) {
	raw := strings.Replace(validUIProductionRuntimeReport(), `    {"name":"native shell runtime integration","kind":"runtime","path":"go run ./tools/cmd/native-ui-runtime-smoke","ran":true,"pass":true,"exit_code":0},
`, "", 1)
	err := ValidateReport([]byte(raw))
	if err == nil {
...
func TestValidateReportRejectsMissingNativeRuntimeSidecarConsistency(t *testing.T) {
	raw := strings.Replace(validUIProductionRuntimeReport(), `    {"name":"native runtime sidecar consistency","kind":"positive","ran":true,"pass":true},
`, "", 1)
	err := ValidateReport([]byte(raw))
	if err == nil {
...
`, "", 1)
	...
	if !strings.Contains(strings.ToLower(err.Error()), "sidecar-driven native ui runtime integration") {
