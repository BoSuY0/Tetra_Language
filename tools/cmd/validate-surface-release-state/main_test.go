...
func surfaceReleaseStateTemplateSmokeJSON() string {
	...
	return `{"schema":"tetra.surface.template-smoke.v1","model":"surface-template-smoke-v1","release_scope":"surface-v1-linux-web","producer":"scripts/release/surface/surface-template-smoke.sh","command":"tetra new surface-app","template_count":6,"templates":[` + strings.Join(templates, ",") + `],"inspector_evidence":{"path":"surface-template-inspector.json","model":"surface-inspector-v1","pass":true},"visual_evidence":{"path":"template-visual/surface-visual-regression.json","schema":"tetra.surface.visual-regression.v1","pass":true},"package_evidence":[{"path":"template-packages/surface-template-command-palette.tar.gz","kind":"tar.gz","sha256":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","pass":true}],"negative_guards":{"no_react_import":true,"no_electron_import":true,"no_dom_app_ui_tree":true,"no_css_runtime":true,"no_core_widgets":true,"no_platform_widgets":true,"no_user_js_app_logic":true,"cookbook_uses_block_morph":true},"pass":true}`
}
...
func surfaceReleaseStateReferenceAppsJSON() string {
	...
	for _, app := range apps {
		...
		entries = append(entries, `{"shape":"`+app.shape+`","source":"`+app.source+`","module":"examples.`+strings.TrimSuffix(strings.TrimPrefix(strings.ReplaceAll(app.source, "/", "."), "examples."), ".tetra")+`",`+imports+`,"recipes":["region.panel","field.text","control.action","command.item"],"stable_morph_recipes":true,"resolves_to_block":true,"compiles":true,"runs":true,"exit_code":0,"token_theme_conformance":true,"layout_report":true,"interaction_trace":true,"accessibility_snapshot":true,"performance_budget":true,"artifact_hashes":true,"compatibility_widgets":`+releaseStateBoolJSON(app.compatibility)+`,"targets":[`+strings.Join(targets, ",")+`]}`)
	}
