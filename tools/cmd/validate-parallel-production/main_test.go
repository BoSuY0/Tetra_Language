...
func TestValidateParallelProductionReportRejectsMissingSafeUnsafeForbiddenBoundaryCoverageCase(
	t *testing.T,
) {
	path := filepath.Join(t.TempDir(), "parallel.json")
	raw := strings.Replace(
...
func validParallelProductionReport() string {
	return strings.Join([]string{
		...
		("    {\"name\":\"parallel scheduler prototype evidence\",\"kind\":\"benchma" +
			"rk\",\"path\":\"go run ./compiler/cmd/parallelrt-evidence\",\"ran\":true," +
		...
		("    {\"name\":\"actor backpressure latency benchmark prep\",\"kind\":\"act" +
			...
			"nly; no real-world SLA or comparative latency claim is published.\",\"" +
			"raw_output_artifacts\":[\"reports/actor-runtime-foundation/P15/paralle" +
			"lrt-evidence.raw.json\"],\"ran\":false,\"pass\":true},"),
		...
		("    {\"name\":\"actor mailbox backpressure and failure handling\",\"statu" +
			"s\":\"pass\",\"evidence\":\"mailbox capacity and actor failure cases are c" +
			"overed\"},"),
		("    {\"name\":\"task actor thread boundary transfer rules\",\"status\":\"p" +
			"ass\",\"evidence\":\"ownership transfer diagnostics protect task, actor," +
		...
		("    {\"requirement\":\"join/cancel/deadline/select/group lifecycle\",\"ar" +
			...
			"cellation, deadline timeout, cancel-wakes-deadline-join, select read" +
			"iness, task group lifecycle, and nested cancellation propagation\",\"r" +
			"esult\":\"pass\"},"),
		("    {\"requirement\":\"actor mailbox backpressure and failure handling\"" +
			...
			"e_test.go\",\"evidence\":\"actor mailbox backpressure, task actor mailbo" +
			"x handoff, and actor failure handling cases are required\",\"result\":\"" +
			"pass\"},"),
		("    {\"requirement\":\"task/actor/thread-boundary transfer rules\",\"arti" +
			...
			"\"evidence\":\"task and actor ownership transfer, resource double join," +
			" and task group use-after-close diagnostics are required cases\",\"res" +
			"ult\":\"pass\"},"),
		("    {\"requirement\":\"race-safety model or conservative rejections\",\"a" +
			...
			"idence\":\"shared mutable race-safety rejection is required until a br" +
			"oader race-safe model is implemented\",\"result\":\"pass\"},"),
