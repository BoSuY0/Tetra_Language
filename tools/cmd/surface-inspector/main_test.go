...
func TestRunWritesInspectorReportFromRuntimeReports(t *testing.T) {
	...
	writeInspectorMorphRenderedBeautyReport(t, morphRenderedBeautyPath, "examples/surface_morph_rendered_studio_shell.tetra")
	...
	for _, want := range []string{"block_tree", "morph_tokens", "layout", "paint", "accessibility", "event_routes", "focus", "perf_counters", "recipe_expansions", "block_scene_nodes", "render_commands", "frame_artifacts", "golden_diff"} {
	...
	if !report.MorphToPixels.Pass ||
		...
		report.MorphToPixels.Source != "examples/surface_morph_rendered_studio_shell.tetra" ||
