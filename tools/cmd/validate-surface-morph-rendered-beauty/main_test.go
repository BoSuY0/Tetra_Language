...
func TestWriteMorphToPixelsChainFile(t *testing.T) {
	...
	if err := surface.ValidateMorphToPixelsChainReport(chain, report.MorphEvidence.Source); err != nil {
		t.Fatalf("ValidateMorphToPixelsChainReport failed: %v\n%s", err, raw)
	}
	if chain.ReportPath != filepath.ToSlash(reportPath) || chain.Source != report.MorphEvidence.Source || !chain.Pass {
...
func TestValidateMorphRenderedBeautyReportRejectsSyntheticFrameArtifactWithoutPrecomputedFlag(t *testing.T) {
	report := validMorphRenderedBeautyReportFixture()
...
func TestValidateMorphRenderedBeautyReportRejectsPixelEvidenceWithoutSourceLinks(t *testing.T) {
	...
	for _, want := range []string{"frame_producer", "app_source", "morph_recipe_hash", "block_scene_hash", "render_command_stream_hash"} {
...
func validMorphRenderedBeautyContractFixture() morphRenderedBeautyContract {
	return morphRenderedBeautyContract{
		...
		ForbiddenCorePrimitives: []string{"Button", "Card", "TextField", "TextBox", "Sidebar", "Modal"},
		SupportedTargets:        []string{"headless", "linux-x64-real-window", "wasm32-web-browser-canvas"},
...
func validMorphRenderedBeautyReportFixture() morphRenderedBeautyReport {
	return morphRenderedBeautyReport{
		...
		MorphEvidence: morphRenderedBeautyMorphEvidence{
			...
			RecipeNames:            []string{"control.action@1", "field.text@1", "command.item@1", "region.panel@1", "form.field@1", "nav.item@1", "metric.tile@1", "dialog.panel@1", "toast.notification@1", "tab.item@1", "list.row@1", "app.shell@1", "toolbar@1", "split.pane@1", "status.bar@1", "settings.form@1"},
		...
		PixelEvidence: morphRenderedBeautyPixelEvidence{
			...
			FrameChecksum:           "sha256:" + strings.Repeat("8", 64),
			FrameProducer:           "app",
			AppSource:               "examples/surface_morph_rendered_studio_shell.tetra",
...
func morphRenderedBeautyRenderCommandForTest(order int, command string) morphRenderedBeautyRenderCommand {
