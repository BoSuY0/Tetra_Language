...
func main() {
	contractPath := flag.String("contract", "", "path to tetra.surface.morph-rendered-beauty.contract.v1 JSON")
	reportPath := flag.String("report", "", "optional path to tetra.surface.morph-rendered-beauty.v1 report JSON")
	morphToPixelsChainOut := flag.String("morph-to-pixels-chain-out", "", "optional path to write a validated Morph-to-pixels chain summary from --report")
	flag.Parse()
...
func writeMorphToPixelsChainFile(reportPath string, outPath string, report surface.MorphRenderedBeautyReport) error {
