	...
	rows := []ownershipAuditRow{
		...
		},
		{
			...
			Evidence: auditParagraph(
				("Local control-flow solver, branch/match/loop task-handle " +
					"maybe-joined, task-group maybe-closed, island maybe-freed " +
					"diagnostics,"),
				("and branch/match/loop resource finalization merge " +
					"diagnostics with stable TETRA2101 JSON evidence."),
			),
			Result: "pass",
		},
		{
			...
			Evidence: auditParagraph(
				("Local return-resource summaries, typed-error throw-resource " +
					"summaries including rethrow-through-`try` are covered, " +
					"while richer interprocedural lifetime proofs remain open;"),
				("same-module and interface-only cross-module per-field " +
					...
					"match aggregate wrappers, if-let aggregate wrappers, mixed " +
					"safe/provenance aggregate branch and match returns, and " +
					"optional mixed safe/provenance aggregate branch merges are " +
					"covered in the bounded interface-only slice,"),
				("alongside generated `.t4i` " +
					...
					"provenance stubs,"),
				("plus selected same-module/cross-module transitive " +
		...
		},
		{
			...
			Evidence: auditParagraph(
				...
				("optional payload consume aliases, if-let/match optional " +
					"resource aliases, resource provenance, typed-error " +
					"throw/catch and rethrow-through-try enum-payload resource " +
				...
				("same-module/cross-module transitive interprocedural " +
					...
					"TETRA2101 CLI JSON evidence, and ambiguous resource " +
					"provenance exist; broad alias/provenance remains open."),
			),
			Result: "partial",
		},
		{
			...
			Evidence: auditParagraph(
				("Local moves, mutable reinitialization, " +
					"task/island/task-group finalization including stable " +
					"`TETRA2101` task-group use-after-close, resource " +
					"finalization,"),
				("same-module/cross-module struct/enum whole-value " +
					"call/let/return rejection after partial consume,"),
				("same-module/cross-module enum wrapper-constructor rejection " +
					"after partial field/payload consume,"),
				...
				("same-module/cross-module struct-field/enum-payload alias " +
					...
					"JSON diagnostic evidence are covered."),
			),
			Result: "partial",
		},
		{
		...
		},
		{
			...
			Evidence: auditParagraph(
				("Generic fnptr and protocol marker checks plus generic " +
					"function-typed global consume-marker preservation and " +
					"ownership mismatch diagnostics,"),
				...
				("same-module/cross-module generic protocol requirement " +
					"parameter ownership mismatch diagnostics with stable " +
					"TETRA2001 JSON diagnostic evidence, generated `.t4i` " +
					"function-typed parameter local-alias return metadata for " +
					"interface-only global-storage diagnostics, and function " +
					"type ownership markers parse/format plus function-typed " +
					"callable ownership-marker diagnostics are covered."),
			),
			Result: "partial",
		},
		{
			...
			Evidence: auditParagraph(
				("Callable escape, same-module/cross-module fixed-array alias " +
					"return, same-module/cross-module direct fixed-array global " +
					"assignment, same-module/cross-module optional fixed-array " +
					"global assignment, same-module/cross-module fixed-array " +
					"inout assignment, fixed-array escapes including inout " +
					"assignment with stable TETRA2102 diagnostic evidence, " +
				...
				("same-module/cross-module ptr optional assignment " +
					...
					"evidence, same-module/cross-module ptr-containing aggregate " +
				...
				("same-module/cross-module slice optional-payload " +
					"owned/consume/inout call rejections with stable TETRA2101 " +
					"JSON diagnostic evidence, and imported direct " +
					"ptr-containing/nested aggregate owned/consume/inout call " +
					"rejections with stable TETRA2101 JSON diagnostic evidence " +
					"are covered."),
			),
			Result: "partial",
		},
		{
			...
			Evidence: auditParagraph(
				("Local actor/task/island/resource transfer diagnostics, " +
				...
				("same-module/cross-module actor struct-field/enum-payload " +
					"alias transfer diagnostics with stable TETRA2101 JSON " +
					"diagnostic evidence, same-module/cross-module actor/task " +
					"if-let/match optional-payload alias transfer diagnostics " +
					"with stable TETRA2101 JSON diagnostic evidence, and " +
					"same-module/cross-module task-handle " +
					"struct-field/enum-payload alias transfer diagnostics with " +
		...
		},
		{
			...
			Artifact: ("ownership spec, supported surface, " +
		...
		},
		{
			...
			Evidence: ("Structured test-all report " +
...
func stableForbiddenCaseDiagnosticsFixtureEvidence() string {
	return auditParagraph(
		("use-after-move/use-after-consume, partial struct/enum " +
			"consume whole-value rejection, partial struct/enum " +
			"whole-copy rejection, partial struct/enum enum-constructor " +
			"rejection, optional payload consume/free whole-value " +
			"rejection, actor/task use-after-transfer,"),
		("same-module/cross-module actor struct-field/enum-payload " +
			"alias transfer diagnostics with stable TETRA2101 JSON " +
			"diagnostic evidence, same-module/cross-module task-handle " +
			"struct-field/enum-payload alias transfer diagnostics with " +
			"stable TETRA2101 JSON diagnostic evidence,"),
		("task-handle struct-field/enum-payload alias " +
			"use-after-transfer/join, task-group use-after-close, " +
			"branch/match/loop actor consume reuse with stable branch " +
			"actor CLI JSON, maybe-consumed joins,"),
		...
		("borrow escape, alias conflicts, use-after-free/join/close, " +
		...
		("function-typed storage/return unsupported capture rejection," +
			" callable global-storage escape, unsupported function-value " +
			"escape, unsupported function-value call, capturing closure " +
			"raw-ptr escape, captured closure explicit type-arg " +
			"rejection, function-typed explicit type-arg rejection,"),
		("generic closure/generic callback-closure capture, generic " +
			"closure pointer/direct-call, and imported mutable " +
			"function-typed global boundary JSON diagnostics, " +
			"double-drop/double-finalization, callable escape " +
			"diagnostics, and CLI JSON ownership/lifetime safety codes " +
			"for borrow-escape including fixed-array alias return/global " +
			"assignment/optional global assignment/inout assignment and " +
			"borrowed string alias return/global assignment,"),
		("slice-containing struct literal/alias/nested " +
			"struct/enum-payload return and inout assignment escapes " +
			"plus slice-containing enum direct/alias return escape CLI " +
			"JSON evidence,"),
		("slice-containing struct/enum owned/consume/inout call " +
		...
		("same-module/cross-module slice optional payload binding " +
			...
			"assignment CLI JSON evidence exists."),
		("same-module/cross-module direct slice global assignment " +
			"with stable TETRA2102 JSON diagnostic evidence exists."),
		("same-module/cross-module optional ptr global assignment " +
			"with stable TETRA2102 JSON diagnostic evidence exists."),
		("same-module/cross-module optional aggregate global " +
			"assignment with stable TETRA2102 JSON diagnostic evidence " +
		...
		("same-module/cross-module ptr enum alias return escape with " +
			"stable TETRA2102 JSON diagnostic evidence exists."),
		("same-module/cross-module ptr-containing aggregate " +
			"whole/field/alias/nested-field return escapes with stable " +
			"TETRA2102 JSON diagnostic evidence exists."),
		("same-module/cross-module whole-aggregate global assignment " +
			"with stable TETRA2102 JSON diagnostic evidence exists."),
		("same-module/cross-module ptr-containing enum whole-value " +
			"global assignment with stable TETRA2102 JSON diagnostic " +
		...
		("same-module/cross-module ptr-containing/nested aggregate " +
			"owned/consume/inout call rejections with stable TETRA2101 " +
			"JSON diagnostic evidence exists."),
		("same-module/cross-module ptr enum-payload " +
			"owned/consume/inout call rejections with stable TETRA2101 " +
			"JSON diagnostic evidence exists."),
	)
