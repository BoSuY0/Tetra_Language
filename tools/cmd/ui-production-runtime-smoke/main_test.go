...
func TestBuildReportProducesValidUIProductionRuntimeEvidence(t *testing.T) {
	...
	report := buildReport("tools/cmd/ui-production-runtime-smoke", []uiprod.ProcessReport{
		...
		},
		{
			...
			Kind:     "stress",
			Path:     "tools/cmd/ui-production-runtime-smoke --internal-check stress",
			Ran:      true,
			Pass:     true,
			ExitCode: intPtr(0),
		},
	...
	if len(events) != 6 {
		t.Fatalf(
			"events = %d, want focus, input, change, select, click, and timer tick events",
			len(events),
		)
	}
	...
	if saveEvent.Operations[0].Kind != "async_command" {
		t.Fatalf(
			"save event first operation = %q, want async_command",
			saveEvent.Operations[0].Kind,
		)
	}
...
func TestDesktopRuntimeScenarioRejectsInvalidAndCrashPaths(t *testing.T) {
	...
	if _, err := rt.dispatch("SaveButton", "click", "__missing__", 1); err == nil ||
		!strings.Contains(err.Error(), "command failed") {
		t.Fatalf("unknown command error = %v, want command failed", err)
	}
	if err := rt.recoverCrash(func() { panic("runtime panic recovered") }); err == nil ||
		!strings.Contains(err.Error(), "runtime panic recovered") {
