...
func TestValidateSurfaceMorphStableCandidateRejectsMissingRendererOwnedStableProofGate(t *testing.T) {
	fixture := validMorphStableCandidateFixture(t)
	fixture["promotion_gates"] = []any{"validate-surface-morph-report", "validate-surface-claims", "surface block-system gate", "visual regression gate", "target-host evidence"}
	path := writeMorphStableCandidateFixture(t, fixture)
