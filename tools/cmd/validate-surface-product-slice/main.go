...
func main() {
	...
	flag.StringVar(&opt.SummaryPath, "summary", "", "summary path; defaults to <report-dir>/surface-product-slice-summary.json")
	flag.StringVar(&opt.ManifestPath, "manifest", "", "artifact hash manifest path; defaults to <report-dir>/artifact-hashes.json")
	flag.Parse()
...
func validateProductSlice(opt productSliceOptions) error {
	...
	issues = append(issues, validateProductSliceRequiredArtifacts(reportDir, summary.RequiredArtifacts, covered)...)
	issues = append(issues, validateProductSliceCategories(reportDir, summary.Categories, covered)...)
	issues = append(issues, validateProductSliceMorphRenderedBeautyGate(reportDir, summary.RequiredArtifacts, summary.ProductClaim, summary.FinalSignoff)...)
...
func validateProductSliceFields(summary productSliceSummary) []string {
	...
	for _, check := range []struct {
		...
		{field: "flagship_source", got: summary.FlagshipSource, want: "examples/surface_morph_rendered_studio_shell.tetra"},
		{field: "app_id", got: summary.AppID, want: "studio-shell"},
		{field: "artifact_hash_manifest", got: summary.ArtifactHashManifest, want: "artifact-hashes.json"},
		...
		{field: "validations.flagship_runtime", got: summary.Validations.FlagshipRuntime, want: "validated"},
		...
		{field: "validations.morph_rendered_beauty", got: summary.Validations.MorphRenderedBeauty, want: "validated"},
		...
		{field: "validations.artifact_hashes", got: summary.Validations.ArtifactHashes, want: "validated"},
	} {
		if check.got != check.want {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json %s is %q, want %q", check.field, check.got, check.want))
		}
	}
	...
	if !productSliceValidGitCommit(summary.GitHead) {
		issues = append(issues, "surface-product-slice-summary.json git_head must be 40 hex characters")
	}
	if !productSliceValidGitCommit(summary.GitCommit) {
		issues = append(issues, "surface-product-slice-summary.json git_commit must be 40 hex characters")
	}
	if productSliceValidGitCommit(summary.GitHead) && productSliceValidGitCommit(summary.GitCommit) && summary.GitHead != summary.GitCommit {
	...
	issues = append(issues, validateProductSliceClaimState("surface-product-slice-summary.json", summary.ProductClaim, summary.FinalSignoff, summary.GitDirty)...)
	if !summary.Pass {
...
func validateProductSliceRequiredArtifacts(reportDir string, required map[string]string, covered map[string]bool) []string {
	want := productSliceRequiredArtifacts()
	var issues []string
	for key, path := range want {
		got := filepath.ToSlash(strings.TrimSpace(required[key]))
		if got != path {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json required_artifacts.%s is %q, want %q", key, got, path))
			continue
		}
		...
		if !covered[path] {
			issues = append(issues, fmt.Sprintf("artifact-hashes.json missing required product-slice artifact %q", path))
		}
	}
	return issues
}

func validateProductSliceCategories(reportDir string, categories []productSliceCategory, covered map[string]bool) []string {
	...
	for _, category := range categories {
		...
		if _, ok := seen[name]; ok {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json duplicate category %q", name))
			continue
		}
		seen[name] = category
	}
	if len(categories) != len(required) {
		issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json categories length = %d, want %d", len(categories), len(required)))
	}
	for name, sourceReport := range required {
		category, ok := seen[name]
		if !ok {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json categories missing %q", name))
			continue
		}
		if category.Status != "validated" {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json category %s status is %q, want validated", name, category.Status))
		}
		if filepath.ToSlash(category.SourceReport) != sourceReport {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json category %s source_report is %q, want %q", name, category.SourceReport, sourceReport))
		}
		if strings.TrimSpace(category.Evidence) == "" {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json category %s evidence is required", name))
		}
		if !category.Pass {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json category %s pass must be true", name))
		}
		issues = append(issues, productSlicePathExists(reportDir, sourceReport)...)
		if !covered[sourceReport] {
			issues = append(issues, fmt.Sprintf("artifact-hashes.json missing category source_report %q", sourceReport))
		}
	}
	return issues
}

func validateProductSliceMorphRenderedBeautyGate(reportDir string, required map[string]string, productClaim *bool, finalSignoff *bool) []string {
	...
	if path != requiredPath {
		return []string{fmt.Sprintf("surface-product-slice-summary.json required_artifacts.morph_rendered_beauty_gate is %q, want %q", path, requiredPath)}
	}
	...
	for _, check := range []struct {
		...
		{field: "producer", got: summary.Producer, want: "scripts/release/surface/morph-rendered-beauty-gate.sh"},
		{field: "morph_rendered_beauty_report", got: filepath.ToSlash(summary.MorphRenderedBeauty), want: "morph-rendered-beauty.json"},
		{field: "morph_to_pixels_report", got: filepath.ToSlash(summary.MorphToPixels), want: "morph-to-pixels.json"},
	} {
		if check.got != check.want {
			issues = append(issues, fmt.Sprintf("morph rendered beauty gate summary %s is %q, want %q", check.field, check.got, check.want))
		}
	}
	if summary.Status != "validated" && summary.Status != "validated_with_target_blockers" {
		issues = append(issues, fmt.Sprintf("morph rendered beauty gate summary status is %q, want validated or validated_with_target_blockers", summary.Status))
	}
	if !productSliceValidGitCommit(summary.GitHead) {
		issues = append(issues, "morph rendered beauty gate summary git_head must be 40 hex characters")
	}
	if !productSliceValidGitCommit(summary.GitCommit) {
		issues = append(issues, "morph rendered beauty gate summary git_commit must be 40 hex characters")
	}
	if productSliceValidGitCommit(summary.GitHead) && productSliceValidGitCommit(summary.GitCommit) && summary.GitHead != summary.GitCommit {
		issues = append(issues, "morph rendered beauty gate summary git_commit must match git_head")
	}
	issues = append(issues, validateProductSliceClaimState("morph_rendered_beauty", summary.ProductClaim, summary.FinalSignoff, summary.GitDirty)...)
	if productClaim != nil && summary.ProductClaim != nil && *productClaim != *summary.ProductClaim {
		issues = append(issues, "surface-product-slice-summary.json product_claim must match morph_rendered_beauty.product_claim")
	}
	if finalSignoff != nil && summary.FinalSignoff != nil && *finalSignoff != *summary.FinalSignoff {
		issues = append(issues, "surface-product-slice-summary.json final_signoff must match morph_rendered_beauty.final_signoff")
	}
...
func validateProductSliceClaimState(field string, productClaim *bool, finalSignoff *bool, gitDirty *bool) []string {
	var issues []string
...
func validateProductSlicePromotedMRBGate(summary productSliceMorphRenderedBeautyGate) []string {
	...
	if summary.Status != "validated" {
		issues = append(issues, fmt.Sprintf("morph rendered beauty gate summary status is %q, want validated for product_claim", summary.Status))
	}
	if len(summary.StablePromotionBlockers) > 0 {
		issues = append(issues, "morph rendered beauty gate summary stable_promotion_blockers must be empty for product_claim")
	}
	if len(summary.BridgeOwnedStableTargets) > 0 {
		issues = append(issues, "morph rendered beauty gate summary bridge_owned_stable_targets must be empty for product_claim")
	}
	...
	for _, target := range requiredTargets {
		if !productSliceContainsText(summary.RendererOwnedStableTargets, target) {
			issues = append(issues, fmt.Sprintf("morph rendered beauty gate summary renderer_owned_stable_targets missing %s", target))
		}
	}
	...
	for _, required := range requiredTargets {
		...
		if !ok {
			issues = append(issues, fmt.Sprintf("morph rendered beauty gate summary target_matrix missing %s", required))
			continue
		}
		if target.Status != "validated" {
			issues = append(issues, fmt.Sprintf("morph rendered beauty gate summary target_matrix %s status is %q, want validated", required, target.Status))
		}
		if target.RendererOwnedStableProof == nil || !*target.RendererOwnedStableProof {
			issues = append(issues, fmt.Sprintf("morph rendered beauty gate summary target_matrix %s renderer_owned_stable_proof must be true", required))
		}
		if target.ProductClaim == nil || !*target.ProductClaim {
			issues = append(issues, fmt.Sprintf("morph rendered beauty gate summary target_matrix %s product_claim must be true", required))
		}
	}
	return issues
}
...
func validateProductSliceNonclaims(nonclaims []string) []string {
	...
	for _, required := range []string{
		...
		if !set[required] {
			issues = append(issues, fmt.Sprintf("surface-product-slice-summary.json nonclaims missing %q", required))
		}
	}
	return issues
}
...
func productSliceHashCoverage(manifestPath string) (map[string]bool, []string) {
	...
	if manifest.Schema != productSliceHashSchema {
		issues = append(issues, fmt.Sprintf("artifact-hashes.json schema is %q, want %q", manifest.Schema, productSliceHashSchema))
	}
...
func productSliceRequireBool(field string, got *bool, want bool) []string {
	...
	if *got != want {
		return []string{fmt.Sprintf("surface-product-slice-summary.json %s is %t, want %t", field, *got, want)}
	}
	return nil
}
