...
func TestValidateProductSliceRejectsMissingFlagshipRuntime(t *testing.T) {
	dir := writeProductSliceFixture(t)
	if err := os.Remove(filepath.Join(dir, "flagship", "linux-x64-real-window-block-system.json")); err != nil {
		t.Fatal(err)
	}
...
func TestValidateProductSliceRejectsMissingMorphRenderedBeautyGate(t *testing.T) {
	dir := writeProductSliceFixture(t)
	if err := os.Remove(filepath.Join(dir, "morph-rendered-beauty", "morph-rendered-beauty-gate-summary.json")); err != nil {
		t.Fatal(err)
	}
	err := validateProductSlice(productSliceOptions{ReportDir: dir})
	if err == nil {
	...
	if !strings.Contains(err.Error(), "morph-rendered-beauty/morph-rendered-beauty-gate-summary.json") {
...
func TestValidateProductSliceRejectsMissingGitCommit(t *testing.T) {
	...
	raw = []byte(strings.Replace(string(raw), "  \"git_commit\": \""+productSliceTestGitCommit+"\",\n", "", 1))
	if err := os.WriteFile(path, raw, 0o644); err != nil {
		t.Fatal(err)
	}
	writeProductSliceHashManifest(t, dir)
...
func TestValidateProductSliceRejectsMismatchedMRBGateGitCommit(t *testing.T) {
	...
	raw = []byte(strings.Replace(string(raw), `"git_commit": "`+productSliceTestGitCommit+`"`, `"git_commit": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"`, 1))
	if err := os.WriteFile(path, raw, 0o644); err != nil {
		t.Fatal(err)
	}
	writeProductSliceHashManifest(t, dir)
	...
	if !strings.Contains(err.Error(), "morph rendered beauty gate summary git_commit must match git_head") {
...
func writeProductSliceFixture(t *testing.T) string {
	...
	summary := productSliceSummary{
		...
		CommandLine:          "bash scripts/release/surface/surface-product-slice-gate.sh --report-dir reports/surface-product-slice/product-gate",
		FlagshipSource:       "examples/surface_morph_rendered_studio_shell.tetra",
		AppID:                "studio-shell",
		...
		Categories: []productSliceCategory{
			{Name: "flagship-runtime", Status: "validated", SourceReport: "flagship/flagship-runtime-summary.json", Evidence: "flagship runtime", Pass: true},
			{Name: "developer-loop", Status: "validated", SourceReport: "dev-workflow/surface-dev-workflow.json", Evidence: "developer loop", Pass: true},
			{Name: "package-update", Status: "validated", SourceReport: "package/surface-package.json", Evidence: "package update", Pass: true},
			{Name: "morph-rendered-beauty", Status: "validated", SourceReport: "morph-rendered-beauty/morph-rendered-beauty-gate-summary.json", Evidence: "Morph rendered beauty gate", Pass: true},
			{Name: "claim-governance", Status: "validated", SourceReport: "claims/claim-governance-summary.json", Evidence: "claims", Pass: true},
			{Name: "docs-manifest", Status: "validated", SourceReport: "docs-manifest/docs-manifest-summary.json", Evidence: "docs manifest", Pass: true},
		},
	...
	if err := os.WriteFile(filepath.Join(dir, "surface-product-slice-summary.json"), append(raw, '\n'), 0o644); err != nil {
		t.Fatal(err)
	}
