...
func main() {
	reportDir := flag.String(
		"report-dir",
		"",
		"integrated Memory/Islands/Surface production report directory",
	)
	currentGitHead := flag.String(
		"current-git-head",
		"",
		"optional current git HEAD to require in same-commit artifacts",
	)
	flag.Parse()
	if *reportDir == "" {
...
func validateIntegratedReportDir(reportDir string, currentGitHead string) error {
	...
	issues = append(
		issues,
		validateIntegratedHashManifest(
...
func validateManifestEnvelope(manifest integratedManifest, currentGitHead string) []string {
	...
	if manifest.Schema != integratedManifestSchema {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated manifest schema is %q, want %q",
				manifest.Schema,
				integratedManifestSchema,
			),
		)
	}
	if manifest.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf("integrated manifest status is %q, want pass", manifest.Status),
		)
	}
	...
	if strings.TrimSpace(currentGitHead) != "" &&
		...
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated git_head %s does not match current git head %s",
				manifest.GitHead,
	...
	if _, err := time.Parse(time.RFC3339, manifest.GeneratedAt); err != nil {
		issues = append(
			issues,
			fmt.Sprintf("integrated manifest generated_at must be RFC3339: %v", err),
		)
	}
	if manifest.ReportDir != "." {
		issues = append(
			issues,
			fmt.Sprintf("integrated manifest report_dir is %q, want .", manifest.ReportDir),
		)
	}
	if manifest.HashManifest != "artifact-hashes.json" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated manifest hash_manifest is %q, want artifact-hashes.json",
...
func validateManifestCommands(commands []integratedCommand) []string {
	...
	for _, command := range commands {
		...
		if strings.TrimSpace(command.Command) == "" {
			issues = append(
				issues,
				fmt.Sprintf("integrated manifest command %s command is required", name),
			)
		}
		if strings.Contains(command.Command, "|| true") ||
			strings.Contains(command.Command, "continue-on-error") {
			issues = append(
				issues,
				fmt.Sprintf("integrated manifest command %s contains bypass marker", name),
			)
		}
	}
	for name, fragment := range requiredCommands {
		...
		if !found {
			issues = append(
				issues,
				fmt.Sprintf("missing integrated manifest command %s containing %q", name, fragment),
			)
		}
	}
	return issues
...
func validateManifestArtifactRefs(artifacts []integratedArtifactRef) []string {
	...
	for _, artifact := range artifacts {
		if err := validateSafeRel(artifact.Path); err != nil {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated manifest artifact path %q is invalid: %v",
					artifact.Path,
					err,
				),
			)
		...
		if strings.TrimSpace(artifact.Kind) == "" {
			issues = append(
				issues,
				fmt.Sprintf("integrated manifest artifact %s kind is required", artifact.Path),
			)
			continue
		}
		if _, ok := byKind[artifact.Kind]; ok {
			issues = append(
				issues,
				fmt.Sprintf("duplicate integrated manifest artifact kind %s", artifact.Kind),
			)
		}
		...
		if byPath[artifact.Path] {
			issues = append(
				issues,
				fmt.Sprintf("duplicate integrated manifest artifact path %s", artifact.Path),
			)
		}
	...
	for _, required := range requiredManifestArtifacts {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("missing integrated manifest artifact %s", required.Kind),
			)
			continue
		}
		if artifact.Path != required.Path {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated manifest artifact %s path is %q, want %q",
					required.Kind,
					artifact.Path,
					required.Path,
				),
		...
		if required.Schema != "" && artifact.Schema != required.Schema {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated manifest artifact %s schema is %q, want %q",
					required.Kind,
					artifact.Schema,
					required.Schema,
				),
...
func validateRequiredEvidence(reportDir string, gitHead string) []string {
	...
	if err := readJSON(
		filepath.Join(reportDir, "memory", "memory-production-linux-x64.json"),
		&memoryReport,
	); err != nil {
		...
		if memoryReport.Schema != "tetra.memory.production.v1" {
			issues = append(
				issues,
				fmt.Sprintf(("memory production report schema is %q, want "+
					"tetra.memory.production.v1"), memoryReport.Schema),
			)
		}
		if memoryReport.Status != "pass" {
			issues = append(
				issues,
				fmt.Sprintf("memory production report status is %q, want pass", memoryReport.Status),
			)
		}
		if memoryReport.Target != "linux-x64" {
			issues = append(
				issues,
				fmt.Sprintf("memory production report target is %q, want linux-x64", memoryReport.Target),
			)
		}
	}
	...
	if err := readJSON(
		filepath.Join(reportDir, "memory", "memory-release-manifest.json"),
		&memoryManifest,
	); err != nil {
		...
		if memoryManifest.Schema != "tetra.memory.release-manifest.v1" {
			issues = append(
				issues,
				fmt.Sprintf(("memory release manifest schema is %q, want tetra.memory.release-"+
					"manifest.v1"), memoryManifest.Schema),
			)
		}
		if memoryManifest.GitHead != gitHead {
			issues = append(
				issues,
				fmt.Sprintf("memory release git_head %s does not match integrated git_head %s", memoryManifest.GitHead, gitHead),
			)
		}
	}
	...
	if err := readJSON(
		filepath.Join(reportDir, "memory", "island-proof-verifier.json"),
		&islandProof,
	); err != nil {
		issues = append(
			issues,
			fmt.Sprintf("island proof verifier report missing or invalid: %v", err),
		)
	} else {
		if islandProof.Schema != "tetra.island.proof.v1" {
			issues = append(
				issues,
				fmt.Sprintf("island proof verifier schema is %q, want tetra.island.proof.v1", islandProof.Schema),
			)
		}
		if islandProof.GitHead != gitHead {
			issues = append(
				issues,
				fmt.Sprintf("island proof git_head %s does not match integrated git_head %s", islandProof.GitHead, gitHead),
			)
		}
	}
	...
	if err := readJSON(
		filepath.Join(reportDir, "memory", "island-proof-memory-report.json"),
		&islandMemory,
	); err != nil {
		issues = append(
			issues,
			fmt.Sprintf("island proof memory report missing or invalid: %v", err),
		)
	} else if schemaOf(islandMemory) != "tetra.memory-report.v1" {
		issues = append(
			issues,
			fmt.Sprintf(("island proof memory report schema is %q, want tetra.memory-"+
				"report.v1"), schemaOf(islandMemory)),
		)
	}
	...
	if err := readJSON(
		filepath.Join(reportDir, "memory", "memory-fuzz-tier1", "island-proof-fuzz-summary.json"),
		&proofFuzz,
	); err != nil {
		issues = append(
			issues,
			fmt.Sprintf("island proof fuzz summary missing or invalid: %v", err),
		)
	} else {
		...
		if proofFuzzSchema != "tetra.island-proof-fuzz-summary.v1" {
			issues = append(
				issues,
				fmt.Sprintf(("island proof fuzz summary schema is %q, want tetra.island-proof-"+
					"fuzz-summary.v1"), proofFuzzSchema),
			)
		}
		if proofFuzz.Status != "pass" {
			issues = append(
				issues,
				fmt.Sprintf("island proof fuzz summary status is %q, want pass", proofFuzz.Status),
			)
		}
		if proofFuzz.Total <= 0 || proofFuzz.Rejected != proofFuzz.Total || proofFuzz.Accepted != 0 {
			issues = append(
				issues,
				fmt.Sprintf(("island proof fuzz summary counts total=%d rejected=%d "+
					"accepted=%d, want all rejected and zero accepted"), proofFuzz.Total, proofFuzz.Rejected, proofFuzz.Accepted),
			)
		}
	}
	...
	issues = append(
		issues,
		validateIslandsDebugSmoke(filepath.Join(reportDir, "islands-debug-smoke.json"), gitHead)...)
	issues = append(
		issues,
		validateRAMContractEvidence(filepath.Join(reportDir, "memory", "ram-contract"), gitHead)...)
	issues = append(
		issues,
		validateSurfaceReleaseSummary(
			filepath.Join(reportDir, "surface-release-v1", "surface-release-summary.json"),
			gitHead,
		)...)
	issues = append(
		issues,
		validateHashManifestEnvelope(
			filepath.Join(reportDir, "memory", "artifact-hashes.json"),
			"memory hash manifest",
		)...)
	issues = append(
		issues,
		validateHashManifestEnvelope(
			filepath.Join(reportDir, "surface-release-v1", "artifact-hashes.json"),
			"surface release hash manifest",
		)...)
	issues = append(
		issues,
		validateHashManifestEnvelope(
			filepath.Join(reportDir, "surface-experimental-regression", "artifact-hashes.json"),
			"surface experimental hash manifest",
		)...)
	issues = append(
		issues,
		validateSafeViewSummary(
			filepath.Join(reportDir, "safe-view-lifetime", "safe-view-lifetime-summary.json"),
		)...)
	issues = append(
		issues,
		validateSurfaceAPISummary(
			filepath.Join(
				reportDir,
...
func validateRAMContractEvidence(ramDir string, gitHead string) []string {
	...
	required := []requiredRAMArtifact{
		{
			Rel:    "ram-contract-release-manifest.json",
			Label:  "RAM contract release manifest",
			Schema: "tetra.ram-contract.release-manifest.v1",
		},
		{
			Rel:    "ram-contract-report.json",
			Label:  "RAM contract report",
			Schema: "tetra.ram-contract-report.v1",
		},
		{
			Rel:    "memory-grade-report.json",
			Label:  "RAM memory grade report",
			Schema: "tetra.memory-grade-report.v1",
		},
		{
			Rel:    "proof-store-summary.json",
			Label:  "RAM proof store summary",
			Schema: "tetra.proof-store-summary.v1",
		},
		{
			Rel:    "validation-pipeline-coverage.json",
			Label:  "RAM validation pipeline coverage",
			Schema: "tetra.validation-pipeline-coverage.v1",
		},
		...
		{Rel: "copy-blockers.json", Label: "RAM copy blockers", Schema: "tetra.ram-blockers.v1"},
		{
			...
			Label:  "RAM contract fuzz oracle",
			Schema: "tetra.ram-contract-fuzz-oracle.v1",
		},
		{
			Rel:    "artifact-hashes.json",
			Label:  "RAM contract hash manifest",
			Schema: integratedHashSchema,
		},
	}
	var issues []string
	for _, req := range required {
		...
		if schemaOf(envelope) != req.Schema {
			issues = append(
				issues,
				fmt.Sprintf("%s schema is %q, want %s", req.Label, schemaOf(envelope), req.Schema),
			)
		}
	}
	...
	if err := readJSON(
		...
		if releaseManifest.Status != "pass" {
			issues = append(
				issues,
				fmt.Sprintf(
					"RAM contract release manifest status is %q, want pass",
		...
		if releaseManifest.GitHead != gitHead {
			issues = append(
				issues,
				fmt.Sprintf(
					"RAM contract release manifest git_head %s does not match integrated git_head %s",
					releaseManifest.GitHead,
					gitHead,
				),
			)
		}
	...
	if err := readJSON(filepath.Join(ramDir, "ram-contract-report.json"), &ramReport); err == nil &&
		ramReport.GitHead != gitHead {
		issues = append(
			issues,
			fmt.Sprintf(
				"RAM contract report git_head %s does not match integrated git_head %s",
				ramReport.GitHead,
				gitHead,
			),
		)
...
func validateIslandsDebugSmoke(path string, gitHead string) []string {
	...
	if report.Target != "linux-x64" {
		issues = append(
			issues,
			fmt.Sprintf("islands debug smoke target is %q, want linux-x64", report.Target),
		)
	}
	if report.GitHead != "" && !sameGitHead(report.GitHead, gitHead) {
		issues = append(
			issues,
			fmt.Sprintf(
				"islands debug smoke git_head %s does not match integrated git_head %s",
				report.GitHead,
				gitHead,
			),
		)
	...
	for _, c := range report.Cases {
		...
		if c.SrcPath != "examples/memory/islands/islands_overflow.tetra" {
			issues = append(
				issues,
				fmt.Sprintf(
					"islands debug smoke trap src_path is %q, want examples/memory/islands/islands_overflow.tetra",
					c.SrcPath,
				),
			)
		}
		...
		if c.ActualExit == nil {
			...
			issues = append(
				issues,
				fmt.Sprintf("islands debug smoke trap actual_exit=%d, want %d", *c.ActualExit, c.ExpectedExit),
			)
		}
		if !c.Ran || !c.Pass {
...
func validateSurfaceReleaseSummary(path string, gitHead string) []string {
	...
	if summary.Schema != "tetra.surface.release.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"surface release summary schema is %q, want tetra.surface.release.v1",
				summary.Schema,
			),
		)
	}
	if summary.Status != "current" {
		issues = append(
			issues,
			fmt.Sprintf("surface release summary status is %q, want current", summary.Status),
		)
	}
	if summary.ReleaseScope != "surface-v1-linux-web" {
		issues = append(
			issues,
			fmt.Sprintf(
				"surface release scope is %q, want surface-v1-linux-web",
	...
	if summary.GitHead != gitHead {
		issues = append(
			issues,
			fmt.Sprintf(
				"surface release git_head %s does not match integrated git_head %s",
				summary.GitHead,
				gitHead,
			),
		)
...
func validateHashManifestEnvelope(path string, label string) []string {
	...
	if manifest.Schema != integratedHashSchema {
		issues = append(
			issues,
			fmt.Sprintf("%s schema is %q, want %s", label, manifest.Schema, integratedHashSchema),
		)
	}
	if manifest.Root != "." {
...
func validateSafeViewSummary(path string) []string {
	...
	if summary.Schema != "tetra.safe-view-lifetime.gate.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"safe-view lifetime summary schema is %q, want tetra.safe-view-lifetime.gate.v1",
				summary.Schema,
			),
		)
	}
	if summary.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf("safe-view lifetime summary status is %q, want pass", summary.Status),
		)
	}
	if !summary.Bounded {
...
func validateSurfaceAPISummary(path string) []string {
	...
	if summary.Schema != "tetra.surface.api-stability.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"surface API stability summary schema is %q, want tetra.surface.api-stability.v1",
				summary.Schema,
			),
		)
	}
	if summary.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf("surface API stability summary status is %q, want pass", summary.Status),
		)
	}
	if summary.ReleaseScope != "surface-v1-linux-web" {
		issues = append(
			issues,
			fmt.Sprintf(
				"surface API stability scope is %q, want surface-v1-linux-web",
				summary.ReleaseScope,
			),
		)
	}
	if !summary.DocsManifestValidated {
		issues = append(
			issues,
			"surface API stability summary docs_manifest_validated must be true",
		)
	}
	return issues
}
...
func validateIntegratedHashManifest(path string, reportDir string) []string {
	...
	if manifest.Schema != integratedHashSchema {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated hash manifest schema is %q, want %s",
				manifest.Schema,
				integratedHashSchema,
			),
		)
	}
	if manifest.Root != "." {
		issues = append(
			issues,
			fmt.Sprintf("integrated hash manifest root is %q, want .", manifest.Root),
		)
	}
	if len(manifest.Artifacts) == 0 {
	...
	for _, artifact := range manifest.Artifacts {
		if err := validateSafeRel(artifact.Path); err != nil {
			issues = append(
				issues,
				fmt.Sprintf("integrated hash manifest path %q is invalid: %v", artifact.Path, err),
			)
			continue
		}
		if lastPath != "" && artifact.Path < lastPath {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated hash manifest artifacts must be sorted: %s appears before %s",
					artifact.Path,
					lastPath,
				),
			)
		...
		if _, ok := seen[artifact.Path]; ok {
			issues = append(
				issues,
				fmt.Sprintf("duplicate integrated hash manifest entry for %s", artifact.Path),
			)
		}
		...
		if err := validateSHA256(artifact.SHA256); err != nil {
			issues = append(
				issues,
				fmt.Sprintf("integrated hash manifest %s: %v", artifact.Path, err),
			)
		}
	}
	for _, required := range requiredHashArtifacts {
		if _, ok := seen[required]; !ok {
			issues = append(
				issues,
				fmt.Sprintf("missing integrated hash manifest entry for %s", required),
			)
		}
	}
