...
func TestValidateIntegratedReportDirRejectsMissingRAMContractArtifacts(t *testing.T) {
	...
	got := err.Error()
	if !strings.Contains(got, "ram_contract_report") &&
		!strings.Contains(got, "memory/ram-contract") {
...
func TestValidateIntegratedReportDirRejectsMismatchedGitHead(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "surface release git_head") ||
		!strings.Contains(err.Error(), "integrated git_head") {
...
func TestValidateIntegratedReportDirRejectsMissingHashEntry(t *testing.T) {
	...
	if !strings.Contains(
		err.Error(),
		"missing integrated hash manifest entry for safe-view-lifetime/safe-view-lifetime-summary.json",
	) {
...
func writeIntegratedFixture(t *testing.T) string {
	...
	writeJSONFile(
		t,
		filepath.Join(reportDir, "surface-api-stability-v1", "surface-api-stability-summary.json"),
		map[string]any{
	...
	writeJSONFile(
		t,
		filepath.Join(reportDir, "memory-islands-surface-production-manifest.json"),
		map[string]any{
			...
			"commands": []any{
				map[string]any{
					...
					"command": ("bash scripts/release/post_v0_4/memory-production-linux-x64-" +
						"smoke.sh --report-dir reports/integrated/memory"),
				},
				map[string]any{
					"name": "islands-debug-smoke",
					"command": ("go run ./cli/cmd/tetra smoke --target linux-x64 --run=true --" +
						"islands-debug --report reports/integrated/islands-debug-smoke.json"),
				},
				map[string]any{
					"name": "validate-islands-debug-smoke",
					"command": ("go run ./tools/cmd/smoke-report-to-checklist --validate-only --" +
						"report reports/integrated/islands-debug-smoke.json"),
				},
				map[string]any{
					"name": "surface-release-gate",
					"command": ("bash scripts/release/surface/release-gate.sh --report-dir " +
						"reports/integrated/surface-release-v1"),
				},
				map[string]any{
					"name": "surface-experimental-regression-gate",
					"command": ("bash scripts/release/surface/gate.sh --report-dir reports/" +
						"integrated/surface-experimental-regression"),
				},
				map[string]any{
					"name": "safe-view-lifetime-gate",
					"command": ("bash scripts/release/safe-view-lifetime/gate.sh --report-dir " +
						"reports/integrated/safe-view-lifetime"),
				},
				map[string]any{
					"name": "surface-api-stability-gate",
					"command": ("bash scripts/release/surface/api-stability-gate.sh --report-dir " +
						"reports/integrated/surface-api-stability-v1"),
				},
				map[string]any{
				...
				map[string]any{
					...
					"command": ("go run ./tools/cmd/validate-artifact-hashes --write --root " +
						"reports/integrated --out reports/integrated/artifact-hashes.json"),
				},
				map[string]any{
					"name": "artifact-hashes-validate",
					"command": ("go run ./tools/cmd/validate-artifact-hashes --manifest reports/" +
				...
				map[string]any{
					...
					"command": ("go run ./tools/cmd/validate-memory-islands-surface-production --" +
						"report-dir reports/integrated --current-git-head ") + integratedTestHead,
				},
			},
			"artifacts": []any{
				map[string]any{
