...
`)
	...
	if got, want := strings.Join(
		report.DirtyTracked,
		",",
	), "README.md,docs/release/new.md"; got != want {
		t.Fatalf("dirty tracked = %q, want %q", got, want)
	}
	if got, want := strings.Join(
		report.UntrackedReleaseArtifacts,
		",",
	), "docs/generated/v1_0/release-state.json"; got != want {
...
func TestReleaseStateReportsUntrackedNonReleaseEntriesWithoutFailing(t *testing.T) {
	...
	if got, want := strings.Join(
		report.Git.UntrackedNonReleaseEntries,
		",",
	), "scratch.txt"; got != want {
...
func TestReleaseStateRejectsStaleGateStepCount(t *testing.T) {
	...
	files[reportDir+"/summary.json"] = releaseSummaryJSON(
		"v0.2.0",
		expectedReleaseArtifact("v0.2.0"),
...
func TestReleaseStateRejectsStaleV020GateIdentity(t *testing.T) {
	...
	files[reportDir+"/summary.json"] = releaseSummaryJSON(
		"v0.1.3",
		"tetra.release.v0_1_3.gate-report.v1",
		"bash scripts/release/v0_1_3/gate.sh",
		reportDir,
		33,
		0,
		"pass",
...
func TestReleaseStateRejectsStaleV030GateIdentityForV040(t *testing.T) {
	...
	files[reportDir+"/summary.json"] = releaseSummaryJSON(
		"v0.3.0",
		expectedReleaseArtifact("v0.3.0"),
		expectedReleaseGateCommand("v0.3.0"),
		reportDir,
		1,
		1,
		"blocked",
...
func TestReleaseStateRejectsGateCommandWithCanonicalScriptOnlyAsSubstring(t *testing.T) {
	...
	files[reportDir+"/summary.json"] = releaseSummaryJSON(
		version,
		expectedReleaseArtifact(version),
		"env FOO=1 bash scripts/release/v1_0/gate.sh",
		reportDir,
		32,
		0,
		"pass",
	...
	if !strings.Contains(
		got,
		`release_gate_command is "env FOO=1 bash scripts/release/v1_0/gate.sh", want "bash scripts/release/v1_0/gate.sh"`,
	) {
		t.Fatalf(
			"issues did not reject non-canonical release_gate_command exactly: %v",
			report.Issues,
		)
	}
}
...
func TestReleaseStateDefersMissingReportDirSecurityReviewEvidenceToFinalGate(t *testing.T) {
	...
	wantValidator := ("bash scripts/release/v0_3_0/security-review.sh --signoff /tmp/" +
		"tetra-v0_3_0-gate/artifacts/security-review.md")
	if report.SecurityReview.ValidatorCommand != wantValidator {
		t.Fatalf(
			"security review validator command = %q, want %q",
...
func TestTextReportIncludesSecurityReviewEvidenceState(t *testing.T) {
	text := formatTextReport(releaseStateReport{
		...
		SecurityReview: securityReviewEvidenceReport{
			...
			Status:   "deferred",
			ValidatorCommand: ("bash scripts/release/v0_3_0/security-review.sh --signoff /tmp/" +
				"tetra-v0_3_0-gate/artifacts/security-review.md"),
		},
		LastGateEvidence: gateEvidenceReport{
			Status:      "pass",
			SummaryPath: "/tmp/tetra-v0_3_0-gate/summary.json",
		},
	})
	if !strings.Contains(
		text,
		"security review evidence: deferred (/tmp/tetra-v0_3_0-gate/artifacts/security-review.md)",
	) {
		t.Fatalf("text report missing security review evidence state:\n%s", text)
	}
	if !strings.Contains(
		text,
		("security review validator: bash scripts/release/v0_3_0/security-" +
			"review.sh --signoff /tmp/tetra-v0_3_0-gate/artifacts/security-review.md"),
	) {
...
func TestTextReportIncludesLastGateIdentitySummary(t *testing.T) {
	...
	if !strings.Contains(
		text,
		`last gate identity: fail (release_version=<unknown>, release_artifact=<unknown>, release_gate_command=<unknown>)`,
	) {
...
func TestTextReportIncludesRuntimeExecutionEvidenceSummary(t *testing.T) {
	text := formatTextReport(releaseStateReport{
		...
		RuntimeExecution: runtimeExecutionEvidenceReport{
			Required: []runtimeExecutionEvidenceCheck{
				{
					Target: "macos-x64",
					Path:   "artifacts/macos-runtime-smoke.json",
					Status: "missing",
					EvidenceCommand: ("./tetra smoke --target macos-x64 --run=true --report artifacts/" +
						"macos-runtime-smoke.json"),
				},
				{
					Target: "windows-x64",
					Path:   "artifacts/windows-runtime-smoke.json",
					Status: "pass",
					EvidenceCommand: ("./tetra smoke --target windows-x64 --run=true --report " +
						"artifacts/windows-runtime-smoke.json"),
					Host: "windows-x64",
				},
			},
	...
	if !strings.Contains(
		text,
		"runtime execution targets: macos-x64=missing, windows-x64=pass(host=windows-x64)",
	) {
		t.Fatalf("text report missing runtime execution target details:\n%s", text)
	}
	if !strings.Contains(
		text,
		("runtime execution commands: ./tetra smoke --target macos-x64 --" +
			"run=true --report artifacts/macos-runtime-smoke.json; ./tetra smoke --" +
			"target windows-x64 --run=true --report artifacts/windows-runtime-" +
			"smoke.json"),
	) {
...
func TestReleaseStateRejectsBlockedV030SecurityReviewPlaceholder(t *testing.T) {
	...
	addSecurityReviewEvidenceFiles(files, reportDir, "v0.3.0", "abcdef0")
	files[reportDir+"/artifacts/security-review.md"] = []byte(
...
`,
	...
	files[reportDir+"/artifacts/security-review.md.sha256"] = detachedSecurityReviewHash(
		files[reportDir+"/artifacts/security-review.md"],
	)
	readFile := func(path string) ([]byte, error) {
		raw, ok := files[path]
		if !ok {
	...
	if !strings.Contains(
		got,
		"security review evidence: security-review.md decision is not an approval for v0.3.0",
	) {
...
func TestReleaseStateRejectsPlaceholderSecurityReviewArtifactHashes(t *testing.T) {
	...
	files[reportDir+"/artifacts/security-review.md"] = []byte(
		strings.ReplaceAll(
			string(files[reportDir+"/artifacts/security-review.md"]),
			strings.Repeat("1", 64),
			strings.Repeat("0", 64),
		),
	)
	...
	if !strings.Contains(
		got,
		"security review evidence: security-review.md contains placeholder artifact hash",
	) {
...
func TestReleaseStateRejectsBuildOnlyV030RuntimeEvidence(t *testing.T) {
	...
	files[reportDir+"/artifacts/macos-runtime-smoke.json"] = runtimeSmokeJSON(
		"macos-x64",
		"macos-x64",
		"v0.3.0",
		"abcdef0",
	...
	files[reportDir+"/artifacts/windows-runtime-smoke.json"] = runtimeSmokeJSON(
		"windows-x64",
		"windows-x64",
		"v0.3.0",
		"abcdef0",
...
func TestReleaseStateRejectsUnsupportedV030RuntimeEvidence(t *testing.T) {
	...
	if err := json.Unmarshal(
		files[reportDir+"/artifacts/macos-runtime-smoke.json"],
		&runtime,
	); err != nil {
...
func TestReleaseStateRejectsUnsupportedV030RuntimeCaseEvidence(t *testing.T) {
	...
	if !strings.Contains(
		got,
		"macos-runtime-smoke.json case actors_pingpong is marked unsupported",
	) {
...
func TestReleaseStateRejectsStaleV1GeneratedEvidenceMetadata(t *testing.T) {
  ...
  "cases": []
	...
	got := strings.Join(report.Issues, "\n")
	for _, want := range []string{
		"host-smoke.json version is \"v0.1.1\", want \"v1.0.0\"",
		"host-smoke.json git_head is \"4c9a01d\", want \"abcdef0\"",
	} {
		if !strings.Contains(got, want) {
...
func TestReleaseStateV1ReportDirAuditsFreshArchiveInsteadOfDocsGenerated(t *testing.T) {
	...
	for _, check := range report.GeneratedArtifacts.Required {
		if strings.HasPrefix(check.Path, "docs/generated/v1_0/") {
			t.Fatalf(
				"v1 report-dir audit should not require checked-in generated artifact %s",
				check.Path,
			)
		}
	}
...
func TestReleaseStateRejectsMissingV1BackendSummary(t *testing.T) {
	...
	if !strings.Contains(
		got,
		"missing required release artifact: "+reportDir+"/artifacts/backend-summary.md",
	) {
...
func TestReleaseStateRejectsNonCanonicalV1GateCommand(t *testing.T) {
	...
	files[reportDir+"/summary.json"] = releaseSummaryJSON(
		"v1.0.0",
		expectedReleaseArtifact("v1.0.0"),
		"env bash scripts/release/v1_0/gate.sh --reuse-old-report",
		reportDir,
		32,
		0,
		"pass",
	...
	if !strings.Contains(
		got,
		`release_gate_command is "env bash scripts/release/v1_0/gate.sh --reuse-old-report", want "bash scripts/release/v1_0/gate.sh"`,
	) {
...
func TestReleaseStateRequiresReportDirForV020(t *testing.T) {
	...
	files["docs/generated/v1_0/release_gate_summary.json"] = []byte(
		`{"status":"pass","step_count":33,"failed_count":0}`,
	)
	readFile := func(path string) ([]byte, error) {
		raw, ok := files[path]
		if !ok {
...
func releaseStatePassFiles(reportDir string) map[string][]byte {
	...
	files["docs/generated/v1_0/release_gate_summary.json"] = releaseSummaryJSON(
		"v0.1.3",
		"tetra.release.v0_1_3.gate-report.v1",
		"bash scripts/release/v0_1_3/gate.sh",
		"/tmp/stale-v0_1_3-gate",
		33,
		0,
		"pass",
	)
...
func releaseStateV1FreshFiles(reportDir string, version string, gitHead string) map[string][]byte {
	...
	for _, path := range []string{
		...
		files[path] = []byte(
			`{"timestamp":"2026-04-30T12:00:00Z","version":"` + version + `","git_head":"` + gitHead + `","cases":[]}`,
		)
	}
	...
	files["docs/generated/v1_0/binary-size-thresholds.json"] = []byte(
		`{"schema":"tetra.binary-size-thresholds.v1alpha1","compiler_version":"` + version + `"}`,
	)
	files["docs/generated/v1_0/reproducible-build.json"] = []byte(
		`{"schema":"tetra.reproducible-build-proof.v1alpha1","compiler_version":"` + version + `"}`,
	)
	files["docs/generated/v1_0/performance-regression.json"] = []byte(
		`{"schema":"tetra.performance-regression.v1","git_head":"` + gitHead + `"}`,
	)
	files["docs/generated/v1_0/api-diff/api-diff.json"] = []byte(
		`{"schema":"tetra.api.diff.v1alpha1"}`,
	)
	files["docs/generated/v1_0/web-ui-smoke.json"] = []byte(
		`{"schema":"tetra.web-ui-smoke.v1alpha1","generated_at":"2026-04-30T12:00:00Z"}`,
	)
	files["docs/generated/v1_0/release-state.json"] = []byte(
		`{"schema":"tetra.release-state.v1alpha1","version":"` + version + `","expected_version":"` + version + `"}`,
	)
	files["docs/generated/v1_0/test_all_full_summary.json"] = []byte(
		`{"started_at":"2026-04-30T12:00:00Z","ended_at":"2026-04-30T12:01:00Z"}`,
	)
	files["docs/generated/v1_0/test-all/summary.json"] = []byte(
		`{"started_at":"2026-04-30T12:00:00Z","ended_at":"2026-04-30T12:01:00Z"}`,
	)
	files["docs/generated/v1_0/release_gate_summary.json"] = releaseSummaryJSON(
		version,
		expectedReleaseArtifact(version),
		expectedReleaseGateCommand(version),
		reportDir,
		32,
		0,
		"pass",
...
func releaseStateV1FreshReportDirFiles(
	reportDir string,
	version string,
	gitHead string,
) map[string][]byte {
	...
	files[reportDir+"/artifacts/binary-size-thresholds.json"] = []byte(
		`{"schema":"tetra.binary-size-thresholds.v1alpha1","compiler_version":"` + version + `"}`,
	)
	files[reportDir+"/artifacts/reproducible-build.json"] = []byte(
		`{"schema":"tetra.reproducible-build-proof.v1alpha1","compiler_version":"` + version + `"}`,
	)
	files[reportDir+"/artifacts/performance-regression.json"] = []byte(
		`{"schema":"tetra.performance-regression.v1","git_head":"` + gitHead + `"}`,
	)
	files[reportDir+"/artifacts/api-diff/api-diff.json"] = []byte(
		`{"schema":"tetra.api.diff.v1alpha1"}`,
	)
	files[reportDir+"/artifacts/web-ui-smoke.json"] = []byte(
		`{"schema":"tetra.web-ui-smoke.v1alpha1","generated_at":"2026-04-30T12:00:00Z"}`,
	)
	files[reportDir+"/summary.json"] = releaseSummaryJSON(
		version,
	...
	files[reportDir+"/artifacts/test-all/summary.json"] = []byte(
		`{"started_at":"2026-04-30T12:00:00Z","ended_at":"2026-04-30T12:01:00Z"}`,
	)
	addSecurityReviewEvidenceFiles(files, reportDir, version, gitHead)
	return files
...
func addSecurityReviewEvidenceFiles(
	files map[string][]byte,
	reportDir string,
	version string,
	gitHead string,
