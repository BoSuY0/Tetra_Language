...
func TestValidateSurfaceRuntimeReportAcceptsHeadlessEvidence(t *testing.T) {
	...
	if err := os.WriteFile(
		reportPath,
		validSurfaceRuntimeReportJSON(artifactPath, artifactSHA, artifactSize, tracePath, traceSHA, traceSize),
		0o644,
	); err != nil {
...
func TestTextInputReleaseValidatorAcceptsProductionTextInputReport(t *testing.T) {
	...
	if err := validateSurfaceRuntimeReportWithOptions(
		reportPath,
		surfaceRuntimeValidationOptions{Release: "text-input"},
	); err != nil {
...
func TestAppModelReleaseValidatorAcceptsHeadlessCommandReducerReport(t *testing.T) {
	dir := t.TempDir()
	artifactDir := surfaceArtifactFixtureDir(t, dir)
	artifactPath, artifactSHA, artifactSize := writeNamedSurfaceArtifactFixture(
		t,
		artifactDir,
		"surface-app-model",
		[]byte("surface app-model fixture\n"),
		0o755,
	)
	tracePath, traceSHA, traceSize := writeSurfaceTraceFixtureWithSourceAndFrames(
		t,
		artifactDir,
	...
	raw := validAppModelReleaseRuntimeReportJSON(
		t,
		artifactPath,
		artifactSHA,
		artifactSize,
	...
	if err := validateSurfaceRuntimeReportWithOptions(
		reportPath,
		surfaceRuntimeValidationOptions{Release: "app-model"},
	); err != nil {
...
func TestAppModelReleaseValidatorRejectsHiddenStateRuntimeSubstitute(t *testing.T) {
	...
	if err := json.Unmarshal(
		validAppModelReleaseRuntimeReportJSON(t, artifactPath, artifactSHA, artifactSize, tracePath, traceSHA, traceSize),
		&report,
	); err != nil {
	...
	err = validateSurfaceRuntimeReportWithOptions(
		reportPath,
		surfaceRuntimeValidationOptions{Release: "app-model"},
	)
	if err == nil {
	...
	if !strings.Contains(err.Error(), "app_model") ||
		!strings.Contains(err.Error(), "hidden app state") {
...
func TestLinuxAppShellReleaseValidatorAcceptsTargetHostReport(t *testing.T) {
	...
	if err := validateSurfaceRuntimeReportWithOptions(
		reportPath,
		surfaceRuntimeValidationOptions{Release: "linux-app-shell"},
	); err != nil {
...
func TestLinuxAppShellReleaseValidatorRejectsMissingP16FeatureLedger(t *testing.T) {
	...
	reportPath := filepath.Join(
		dir,
		"surface-linux-x64-release-app-shell-missing-error-report.json",
	)
	raw := validLinuxAppShellReleaseRuntimeReportJSON(t, artifactDir, func(report map[string]any) {
		appShell := report["linux_app_shell"].(map[string]any)
		appShell["shell_features"] = withoutLinuxAppShellRuntimeFeature(
...
func TestNativeSurfaceHostReleaseValidatorAcceptsDirectNativeHostReport(t *testing.T) {
	dir := t.TempDir()
	artifactDir := surfaceArtifactFixtureDir(t, dir)
	reportPath := filepath.Join(dir, "surface-linux-x64-native-host.json")
	raw := validNativeSurfaceHostReleaseRuntimeReportJSON(t, artifactDir, nil)
	if err := os.WriteFile(reportPath, raw, 0o644); err != nil {
		t.Fatalf("write report: %v", err)
	}
	err := validateSurfaceRuntimeReportWithOptions(
		reportPath,
		surfaceRuntimeValidationOptions{Release: "linux-x64-native-host"},
	)
	if err != nil {
		t.Fatalf(
			"validateSurfaceRuntimeReportWithOptions linux-x64-native-host failed: %v\n%s",
			err,
			raw,
		)
	}
...
func TestNativeSurfaceHostReleaseValidatorRejectsRealWindowProbeSubstitute(t *testing.T) {
	dir := t.TempDir()
	artifactDir := surfaceArtifactFixtureDir(t, dir)
	reportPath := filepath.Join(dir, "surface-linux-x64-native-host-probe.json")
	raw := validNativeSurfaceHostReleaseRuntimeReportJSON(
		t,
		artifactDir,
	...
	if err == nil {
		t.Fatalf("expected real-window probe substitute to fail native Surface host release gate")
	}
	lower := strings.ToLower(err.Error())
	if !strings.Contains(lower, "native") && !strings.Contains(lower, "probe") {
		t.Fatalf("error = %v, want native/probe diagnostic", err)
	}
}
...
func TestValidateSurfaceRuntimeReportReleaseModeAcceptsReleaseSummary(t *testing.T) {
	...
	if err := validateSurfaceRuntimeReportWithOptions(
		reportPath,
		surfaceRuntimeValidationOptions{Release: "surface-v1"},
	); err != nil {
...
func TestHeadlessReleaseValidatorAcceptsHeadlessRuntimeReport(t *testing.T) {
	...
	raw := headlessReleaseRuntimeReportJSON(
		artifactPath,
		artifactSHA,
		artifactSize,
		tracePath,
	...
	if err := validateSurfaceRuntimeReportWithOptions(
		reportPath,
		surfaceRuntimeValidationOptions{Release: "headless"},
	); err != nil {
...
func TestHeadlessReleaseValidatorRejectsLinuxOrBrowserSubstitute(t *testing.T) {
	...
	raw := strings.Replace(
		string(
			headlessReleaseRuntimeReportJSON(
				artifactPath,
				artifactSHA,
				artifactSize,
				tracePath,
