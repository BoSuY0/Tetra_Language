...
func TestValidateSurfaceRuntimeReportReleaseModeRejectsOldBrowserCanvasEvidence(t *testing.T) {
	...
	wasmPath, wasmSHA, wasmSize := writeNamedSurfaceArtifactFixture(
		t,
		artifactDir,
		"surface-browser-counter.wasm",
		[]byte("\x00asm\x01\x00\x00\x00surface browser wasm fixture\n"),
		0o755,
	)
	loaderPath, loaderSHA, loaderSize := writeNamedSurfaceArtifactFixture(
		t,
		artifactDir,
		"surface-browser-counter.mjs",
...
`),
	...
	raw := validWASM32WebBrowserCanvasSurfaceRuntimeReportJSON(
		wasmPath,
		wasmSHA,
		wasmSize,
		loaderPath,
...
func TestValidateSurfaceRuntimeReportReleaseModeRejectsUnknownRelease(t *testing.T) {
	...
	err := validateSurfaceRuntimeReportWithOptions(
		reportPath,
		surfaceRuntimeValidationOptions{Release: "surface-v2"},
	)
	if err == nil {
...
func TestValidateSurfaceRuntimeReportReleaseModeAcceptsReleaseEvidenceSlices(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			report: releaseToolkitSliceReportForTest(
				"linux-x64",
				surface.HostEvidenceReport{
...
func TestValidateSurfaceRuntimeReportReleaseModeRejectsAccessibilityClaimsWithoutTargetEvidence(
	t *testing.T,
) {
	for _, tc := range []struct {
		name   string
	...
	}{
		{
			...
			report: releaseAccessibilitySliceReportForTest(
				"linux-x64",
				surface.HostEvidenceReport{
					Level:       "linux-x64-real-window",
					Backend:     "wayland-shm-rgba",
...
func TestValidateSurfaceRuntimeReportAcceptsHeadlessTraceAbsoluteSource(t *testing.T) {
	...
	tracePath, traceSHA, traceSize := writeSurfaceTraceFixtureWithSourceAndFrames(
		t,
		artifactDir,
		"/repo/examples/surface/runtime/surface_counter.tetra",
...
func TestValidateSurfaceRuntimeReportAcceptsWASM32WebEvidence(t *testing.T) {
	...
	wasmPath, wasmSHA, wasmSize := writeNamedSurfaceArtifactFixture(
		t,
		artifactDir,
		"surface-counter.wasm",
		[]byte("\x00asm\x01\x00\x00\x00surface wasm fixture\n"),
		0o755,
	)
	loaderPath, loaderSHA, loaderSize := writeNamedSurfaceArtifactFixture(
		t,
...
`),
	...
	tracePath, traceSHA, traceSize := writeWASMTraceFixtureWithFrames(
		t,
		artifactDir,
		[]wasmTraceFrameFixture{
			{
				Order:     1,
				Width:     320,
				Height:    200,
	...
	raw := validWASM32WebSurfaceRuntimeReportJSON(
		wasmPath,
		wasmSHA,
		wasmSize,
		loaderPath,
...
func TestValidateSurfaceRuntimeReportRejectsUnreportedLegacySidecarInArtifactScanRoot(
	t *testing.T,
) {
	dir := t.TempDir()
	artifactDir := surfaceArtifactFixtureDir(t, dir)
	...
	tracePath, traceSHA, traceSize := writeSurfaceTraceFixture(t, artifactDir)
	if err := os.WriteFile(
		filepath.Join(artifactDir, "surface-counter.ui.html"),
		[]byte("<div>legacy ui</div>\n"),
		0o644,
	); err != nil {
...
func TestValidateSurfaceRuntimeReportRejectsRunnerTraceFrameMismatch(t *testing.T) {
	...
	artifactDir := surfaceArtifactFixtureDir(t, dir)
	artifactPath, artifactSHA, artifactSize := writeSurfaceArtifactFixture(t, artifactDir)
	tracePath, traceSHA, traceSize := writeSurfaceTraceFixtureWithFrames(
		t,
		artifactDir,
		[]surfaceTraceFrameFixture{
			{
...
`),
	...
	tracePath, traceSHA, traceSize := writeWASMTraceFixtureWithWASMAndFrames(
		t,
		artifactDir,
		filepath.Join(artifactDir, "other-surface.wasm"),
		[]wasmTraceFrameFixture{
			{
				Order:     1,
				Width:     320,
...
func TestValidateSurfaceRuntimeReportRejectsBrowserCanvasReportWithStarterRunnerTrace(
	t *testing.T,
) {
	dir := t.TempDir()
	artifactDir := surfaceArtifactFixtureDir(t, dir)
...
`),
	...
	tracePath, traceSHA, traceSize := writeBrowserCanvasTraceFixtureWithChecksums(
		t,
		artifactDir,
		wasmPath,
...
func TestValidateSurfaceRuntimeReportRejectsArtifactHashMismatch(t *testing.T) {
	...
	if err := os.WriteFile(
		reportPath,
		validSurfaceRuntimeReportJSON(artifactPath, wrongSHA, artifactSize, tracePath, traceSHA, traceSize),
		0o644,
	); err != nil {
		t.Fatalf("write report: %v", err)
	}
...
func TestValidateSurfaceRuntimeReportRejectsMissingArtifact(t *testing.T) {
	...
	if err := os.WriteFile(
		reportPath,
		validSurfaceRuntimeReportJSON(artifactPath, sha, 49172, tracePath, traceSHA, traceSize),
		0o644,
	); err != nil {
		t.Fatalf("write report: %v", err)
	}
