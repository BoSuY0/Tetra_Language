...
func resolveHashManifestPath(manifestPath string, root string, out string) (string, error) {
	...
	return "", fmt.Errorf(
		("error: --manifest is required unless --write is set; --root and " +
			"--out may be used as the validation manifest path"),
	)
}
...
func hashFile(root string, rel string) (hashedArtifact, error) {
	...
	if !info.Mode().IsRegular() {
		return hashedArtifact{}, fmt.Errorf(
			"artifact %s is not a regular file",
