...
func main() {
	reportPath := flag.String("report", "", "path to tetra.surface.runtime.v1 report with block_system evidence")
	...
	if err := validateSurfaceBlockReportWithOptions(*reportPath, blockReportValidationOptions{SameCommit: *sameCommit}); err != nil {
...
func validateSurfaceBlockReportWithOptions(path string, options blockReportValidationOptions) error {
	...
	if report.BlockSystem.Schema != "tetra.surface.block-system.v1" {
		return fmt.Errorf("block_system schema is %q, want tetra.surface.block-system.v1", report.BlockSystem.Schema)
	}
	if err := validateBlockReportArtifactLocality(path, report.ArtifactScan, report.Artifacts); err != nil {
		return err
	}
...
func validateBlockReportArtifactLocality(reportPath string, scan surface.ArtifactScanReport, artifacts []surface.ArtifactReport) error {
