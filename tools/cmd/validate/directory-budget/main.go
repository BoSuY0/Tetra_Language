...
func runDirectoryBudget(args []string, stdout, stderr io.Writer, cwd string) int {
	...
	rootsFlag := flags.String(
		"roots",
		"compiler,cli,tools,lib,examples,docs",
		"comma-separated roots to scan",
	)
	limitFlag := flags.Int("limit", 6, "maximum active source/script files per directory")
	extensionsFlag := flags.String(
		"extensions",
		".go,.tetra,.sh,.mjs,.js,.ts",
		"comma-separated active source/script extensions",
	)
	docsMarkdownFlag := flags.Bool(
		"docs-markdown",
		true,
		"count Markdown files under docs/ as maintained docs",
	)
	excludeDirsFlag := flags.String(
		"exclude-dirs",
		".cache,.tetra_cache,node_modules,generated",
		"comma-separated directory names to skip anywhere",
	)
	excludePathsFlag := flags.String(
		"exclude-paths",
		"docs/assets",
		"comma-separated repo-relative directory paths to skip",
	)
	baselinePath := flags.String(
		"baseline",
		"",
		"optional baseline JSON for ratcheting existing violations",
	)
	writeBaselinePath := flags.String(
		"write-baseline",
		"",
		"write current over-budget directories as a baseline JSON",
	)
	strict := flags.Bool(
		"strict",
		false,
		"ignore baseline allowances and require every directory to satisfy --limit",
	)
	...
	if *writeBaselinePath != "" {
		...
		fmt.Fprintf(
			stdout,
			"wrote directory budget baseline: %s (%d allowances)\n",
			*writeBaselinePath,
			len(baseline.Allowances),
		)
		return 0
	}
...
func isActiveFile(path string, rel string, cfg scanConfig) bool {
	ext := strings.ToLower(filepath.Ext(rel))
	if cfg.Extensions[ext] {
		if ext == ".go" && hasOptInBuildTag(path) {
			return false
		}
		return true
	}
...
func hasOptInBuildTag(path string) bool {
	...
	for _, line := range strings.Split(string(raw), "\n") {
		...
		if strings.HasPrefix(trimmed, "//go:build ") {
			return buildExprHasCustomTag(
				strings.TrimSpace(strings.TrimPrefix(trimmed, "//go:build ")),
			)
		}
	}
	return false
...
func buildExprHasCustomTag(expr string) bool {
	for _, tag := range buildExprTags(expr) {
		if !standardBuildTag(tag) {
			return true
		}
	}
	return false
...
func buildExprTags(expr string) []string {
	var tags []string
	for i := 0; i < len(expr); {
		ch := expr[i]
...
func standardBuildTag(tag string) bool {
	switch tag {
	case "aix",
		"android",
		"darwin",
...
func readBaseline(path string) (baselineFile, error) {
	...
	if baseline.Schema != baselineSchema {
		return baselineFile{}, fmt.Errorf(
