...
func TestScanCountsActiveFilesAndIgnoresGenerated(t *testing.T) {
	dir := t.TempDir()
	for i := 0; i < 7; i++ {
		writeFile(t, dir, filepath.Join("compiler", "internal", "semantics", fileName(i, ".go")), "package semantics\n")
...
func TestRunReportsViolations(t *testing.T) {
	...
	if exitCode != 1 {
		t.Fatalf("exit = %d, want 1; stdout=%q stderr=%q", exitCode, stdout.String(), stderr.String())
	}
...
func TestRunWithBaselinePassesKnownViolation(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	exitCode := runDirectoryBudget([]string{"--roots", "examples", "--baseline", baselinePath}, &stdout, &stderr, dir)
	if exitCode != 0 {
		t.Fatalf("exit = %d, want 0; stdout=%q stderr=%q", exitCode, stdout.String(), stderr.String())
	}
...
func TestRunWritesBaseline(t *testing.T) {
	dir := t.TempDir()
	for i := 0; i < 7; i++ {
		writeFile(t, dir, filepath.Join("tools", "scriptstest", fileName(i, ".go")), "package scriptstest\n")
	}
	baselinePath := filepath.Join(dir, "budget.json")

	var stdout, stderr bytes.Buffer
	exitCode := runDirectoryBudget([]string{"--roots", "tools", "--write-baseline", baselinePath}, &stdout, &stderr, dir)
	if exitCode != 0 {
		t.Fatalf("exit = %d, want 0; stdout=%q stderr=%q", exitCode, stdout.String(), stderr.String())
	}
