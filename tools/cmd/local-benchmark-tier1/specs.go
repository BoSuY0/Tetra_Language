...
func executeSpec(spec benchmarkSpec, opt options, env []string, versions map[string]string, tetraTool string, optimizerArtifact string) benchmarkRow {
	...
	var runErr error
	if spec.Language == "tetra" {
		measurements, heapEvidence, rssEvidence, heapArtifacts, runErr = runIterationsWithHeapTelemetry(opt.Timeout, runCommand, env, opt.Iterations, runStdout, runStderr, heapTelemetryDir, spec.Name, opt.OutDir)
	...
	if spec.Language == "tetra" {
		row.TetraMetadata = collectTetraMetadata(spec.Name, binaryPath, optimizerArtifact, heapEvidence, rssEvidence)
		if row.Status != "measured" {
			row.TetraMetadata.MemoryEvidence = blockedMemoryEvidence("Tetra benchmark run failed before runtime heap telemetry could be trusted: " + row.Error)
		}
	}
	return row
}
