...
func TestValidateMemory100ReportDirRejectsStaleAggregateGitHead(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "Memory100 manifest git_head") ||
		!strings.Contains(got, "current git head") {
...
func TestValidateMemory100ReportDirRejectsForbiddenPublicClaim(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "forbidden") ||
		!strings.Contains(err.Error(), "Memory is 100% ready") {
...
func TestValidateMemory100ReportDirRejectsMissingArtifactGitHead(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		writeMemory100JSON(
			t,
			filepath.Join(root, "memory-production", "memory-production-linux-x64.json"),
			map[string]any{
	...
	if got := err.Error(); !strings.Contains(got, "memory_production_report") ||
		!strings.Contains(got, "git_head") {
...
func TestValidateMemory100ReportDirRejectsStaleRequiredArtifactGitHead(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "raw_memory_contract_report") ||
		!strings.Contains(got, "does not match Memory100 git_head") {
...
func TestValidateMemory100ReportDirRejectsFakeMemoryProductionEnvelope(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "memory production") ||
...
func TestValidateMemory100ReportDirRejectsFakeMemoryReleaseManifestEnvelope(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		writeMemory100JSON(
			t,
			filepath.Join(root, "memory-production", "memory-release-manifest.json"),
			map[string]any{
	...
	if got := err.Error(); !strings.Contains(got, "memory release manifest") ||
...
func TestValidateMemory100ReportDirRejectsStaleMemoryReleaseManifestGeneratedAt(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "memory release manifest generated_at") ||
		!strings.Contains(got, "older than child evidence") {
		t.Fatalf("error = %v, want stale Memory release manifest generated_at rejection", err)
	}
}

func TestValidateMemory100ReportDirRejectsCopiedMemoryReleaseManifestCommandProvenance(
	t *testing.T,
) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		memoryDir := filepath.Join(root, "memory-production")
		copiedDir := "reports/memory-100/old-copy/memory-production"
		releaseManifest := memory100ArtifactJSON(
			"memory_release_manifest",
			"tetra.memory.release-manifest.v1",
			memory100TestHead,
		)
		releaseManifest["commands"] = memory100MemoryReleaseCommandsForDir(
			copiedDir,
			memory100TestHead,
		)
		releaseManifest["artifacts"] = memory100MemoryReleaseArtifactRefsForDir(
			copiedDir,
			memory100TestHead,
		)
		writeMemory100JSON(
			t,
			filepath.Join(memoryDir, "memory-release-manifest.json"),
			releaseManifest,
		)
		writeMemory100HashManifest(t, memoryDir)
		writeMemory100HashManifest(t, root)
	...
	if got := err.Error(); !strings.Contains(got, "memory release manifest command") ||
		!strings.Contains(got, "current memory production report dir") {
		t.Fatalf(
			"error = %v, want copied memory release manifest command provenance rejection",
			err,
		)
	}
}
...
func TestValidateMemory100ReportDirRejectsUnexpectedAggregateManifestArtifactRef(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(
		got,
		"unexpected Memory100 artifact kind docs_only_fake_evidence",
	) {
...
func TestValidateMemory100ReportDirRejectsUnexpectedMemoryReleaseArtifactRef(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(
		got,
		"unexpected memory release manifest artifact kind docs_only_fake_evidence",
	) {
		t.Fatalf("error = %v, want unexpected memory release artifact ref rejection", err)
	}
}

func TestValidateMemory100ReportDirRejectsMissingMemoryReleaseDeclaredArtifactHashCoverage(
	t *testing.T,
) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		memoryDir := filepath.Join(root, "memory-production")
		if err := os.Remove(filepath.Join(memoryDir, "targets.json")); err != nil &&
			!os.IsNotExist(err) {
			t.Fatal(err)
		}
		writeMemory100HashManifest(t, memoryDir)
	...
	if got := err.Error(); !strings.Contains(got, "memory production artifact-hashes.json") ||
		!strings.Contains(got, "targets.json") {
		t.Fatalf(
			"error = %v, want missing memory release declared artifact hash coverage rejection",
			err,
		)
	}
}
...
func TestValidateMemory100ReportDirRejectsFakeMemoryReleaseTargetsReport(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "memory release targets.json") ||
...
func TestValidateMemory100ReportDirRejectsFakeMemoryReleaseIslandProofVerifier(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		...
		writeMemory100JSON(
			t,
			filepath.Join(memoryDir, "island-proof-verifier.json"),
			map[string]any{
...
func TestValidateMemory100ReportDirRejectsFakeMemoryReleaseFuzzTier1Summary(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "memory release memory fuzz") ||
		!strings.Contains(got, "island-proof") {
...
func TestValidateMemory100ReportDirRejectsFakeMemoryReleaseNestedRAMContractManifest(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		...
		writeMemory100JSON(
			t,
			filepath.Join(ramDir, "ram-contract-release-manifest.json"),
			map[string]any{
	...
	if got := err.Error(); !strings.Contains(got, "memory release RAM contract release manifest") {
		t.Fatalf(
			"error = %v, want Memory release nested RAM contract manifest content rejection",
			err,
		)
	}
}
...
func TestValidateMemory100ReportDirRejectsFakeMemoryReleaseNestedRAMContractFuzzOracle(
	t *testing.T,
) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		memoryDir := filepath.Join(root, "memory-production")
		...
		writeMemory100JSON(
			t,
			filepath.Join(ramDir, "fuzz", "ram-contract-fuzz-oracle.json"),
			map[string]any{
	...
	if got := err.Error(); !strings.Contains(got, "memory release RAM contract fuzz") ||
...
func TestValidateMemory100ReportDirRejectsFakeRAMContractReleaseManifestEnvelope(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		writeMemory100JSON(
			t,
			filepath.Join(root, "ram-contract", "ram-contract-release-manifest.json"),
			map[string]any{
				"schema":   "tetra.ram-contract.release-manifest.v1",
				"git_head": memory100TestHead,
			},
	...
	if got := err.Error(); !strings.Contains(got, "RAM contract release manifest") ||
		!strings.Contains(got, "command") {
...
func TestValidateMemory100ReportDirRejectsFakeMemoryProductionHashManifest(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		writeMemory100JSON(
			t,
			filepath.Join(root, "memory-production", "artifact-hashes.json"),
			map[string]any{
	...
	if got := err.Error(); !strings.Contains(got, "memory production artifact-hashes.json") ||
		!strings.Contains(got, "artifacts") {
...
func TestValidateMemory100ReportDirRejectsFakeRAMContractHashManifest(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		writeMemory100JSON(
			t,
			filepath.Join(root, "ram-contract", "artifact-hashes.json"),
			map[string]any{
				"schema":    memory100HashSchema,
				"root":      ".",
				"artifacts": []any{},
	...
	if got := err.Error(); !strings.Contains(got, "ram contract artifact-hashes.json") ||
		!strings.Contains(got, "artifacts") {
...
func TestValidateMemory100ReportDirRejectsUnexpectedRAMReleaseArtifactRef(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(
		got,
		"unexpected RAM contract release manifest artifact kind docs_only_fake_evidence",
	) {
...
func TestValidateMemory100ReportDirRejectsRAMFuzzValidatorWithoutCurrentGitHead(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		...
		for _, rawCommand := range commands {
			...
			if command["name"] == "validate-ram-contract-fuzz-oracle" {
				...
				command["command"] = strings.Replace(
					text,
	...
	if got := err.Error(); !strings.Contains(got, "validate-ram-contract-fuzz-oracle") ||
		!strings.Contains(got, "--current-git-head") {
		t.Fatalf("error = %v, want RAM fuzz validator current git head rejection", err)
	}
}

func TestValidateMemory100ReportDirRejectsFakeIntegratedRAMContractReleaseManifestEnvelope(
	t *testing.T,
) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
	...
	if got := err.Error(); !strings.Contains(got, "integrated RAM contract release manifest") ||
		!strings.Contains(got, "command") {
...
func TestValidateMemory100ReportDirRejectsFakeIntegratedRAMContractHashManifest(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "integrated RAM contract artifact-hashes.json") ||
		!strings.Contains(got, "artifacts") {
...
func TestValidateMemory100ReportDirRejectsStaleExtraRootHashManifestEntry(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, extraRel) ||
		!strings.Contains(got, "sha256 mismatch") {
...
func TestValidateMemory100ReportDirRejectsSymlinkRequiredArtifact(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "memory_production_report") ||
		!strings.Contains(got, "must not be a symlink") {
...
func TestValidateMemory100ReportDirRejectsHashlessUnlistedRootArtifact(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(
		got,
		"unlisted Memory100 artifact hashless-extra.json",
	) {
...
func TestValidateMemory100ReportDirRejectsPathTraversalArtifactRef(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "artifact path") ||
		!strings.Contains(got, "invalid") {
...
func TestValidateMemory100ReportDirRejectsPathTraversalHashEntry(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "Memory100 hash path") ||
		!strings.Contains(got, "invalid") {
...
func TestValidateMemory100ReportDirRejectsHashlessUnlistedNestedArtifact(t *testing.T) {
	...
	writeMemory100JSON(
		t,
		filepath.Join(reportDir, "memory-production", "ram-contract", "hashless-extra.json"),
		map[string]any{
			"schema":   "tetra.fake.hashless.v1",
			"git_head": memory100TestHead,
		},
	...
	if got := err.Error(); !strings.Contains(got, "hashless-extra.json") ||
		!strings.Contains(got, "unlisted") {
...
func TestValidateMemory100ReportDirRejectsCopiedAggregateCommandProvenance(t *testing.T) {
		...
		manifest["commands"] = []any{
			map[string]any{
				...
				"command": ("bash scripts/release/post_v0_4/memory-production-linux-x64-" +
					"smoke.sh --report-dir ") + copiedRoot + "/memory-production",
			},
			map[string]any{
				"name": "ram-contract-gate",
				"command": ("bash scripts/release/post_v0_4/ram-contract-linux-x64-smoke.sh -" +
					"-report-dir ") + copiedRoot + "/ram-contract",
			},
			map[string]any{
				"name": "integrated-gate",
				"command": ("bash scripts/release/post_v0_4/memory-islands-surface-" +
					"production-gate.sh --report-dir ") + copiedRoot + "/integrated",
			},
			map[string]any{
				"name":    "memory-fuzz-short",
				"command": "go run ./tools/cmd/memory-fuzz-short --tier 1 --report-dir " + copiedRoot + "/memory-fuzz --git-head " + memory100TestHead,
			},
			map[string]any{
				"name": "memory-fuzz-validator",
				"command": "go run ./tools/cmd/validate-memory-fuzz-oracle --report " + copiedRoot + ("/memory-fuzz/memory-" +
					"fuzz-oracle.json --artifact-dir ") + copiedRoot + "/memory-fuzz --current-git-head " + memory100TestHead,
			},
			map[string]any{
			...
			map[string]any{
				...
				"command": "go run ./tools/cmd/validate-artifact-hashes --write --root " + copiedRoot + " --out " + copiedRoot + "/artifact-hashes.json",
			},
			map[string]any{
				"name":    "memory-100-validator",
				"command": "go run ./tools/cmd/validate-memory-100-prod-stable --report-dir " + copiedRoot + " --current-git-head " + memory100TestHead,
			},
		}
	})
	err := validateMemory100ReportDir(reportDir, memory100TestHead)
	...
	if got := err.Error(); !strings.Contains(got, "Memory100 command") ||
		!strings.Contains(got, "current report dir") {
...
func TestValidateMemory100ReportDirRejectsAggregateManifestOlderThanChildEvidence(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "generated_at") ||
		!strings.Contains(got, "older than") {
...
func TestValidateMemory100ReportDirRejectsFakeIntegratedManifestEnvelope(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		writeMemory100JSON(
			t,
			filepath.Join(root, "integrated", "memory-islands-surface-production-manifest.json"),
			map[string]any{
	...
	if got := err.Error(); !strings.Contains(got, "integrated manifest") ||
		!strings.Contains(got, "command") {
...
func TestValidateMemory100ReportDirRejectsStaleIntegratedManifestGeneratedAt(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "integrated manifest generated_at") ||
		!strings.Contains(got, "older than child evidence") {
...
func TestValidateMemory100ReportDirRejectsCopiedIntegratedManifestCommandProvenance(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		...
		writeMemory100JSON(
			t,
			filepath.Join(integratedDir, "memory-islands-surface-production-manifest.json"),
			map[string]any{
				"schema":        "tetra.memory-islands-surface.production-gate.v1",
				"status":        "pass",
				"git_head":      memory100TestHead,
	...
	if got := err.Error(); !strings.Contains(got, "integrated manifest command") ||
		!strings.Contains(got, "current integrated report dir") {
...
func TestValidateMemory100ReportDirRejectsUnexpectedIntegratedManifestArtifactRef(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(
		got,
		"unexpected integrated manifest artifact kind docs_only_fake_evidence",
	) {
		t.Fatalf("error = %v, want unexpected integrated artifact ref rejection", err)
	}
}

func TestValidateMemory100ReportDirRejectsMissingIntegratedMemoryReleaseDeclaredArtifactHashCoverage(
	t *testing.T,
) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
	...
	if err == nil {
		t.Fatalf(
			"expected missing integrated Memory release declared artifact hash coverage to fail",
		)
	}
	if got := err.Error(); !strings.Contains(got, "integrated memory artifact-hashes.json") ||
		!strings.Contains(got, "targets.json") {
		t.Fatalf(
			"error = %v, want missing integrated memory release declared artifact hash coverage rejection",
			err,
		)
	}
}
...
func TestValidateMemory100ReportDirRejectsFakeIntegratedMemoryReleaseTargetsReport(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "integrated memory release targets.json") ||
		!strings.Contains(got, "supported") {
...
func TestValidateMemory100ReportDirRejectsFakeIntegratedIslandsDebugSmoke(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		...
		writeMemory100JSON(
			t,
			filepath.Join(integratedDir, "islands-debug-smoke.json"),
			map[string]any{
...
func TestValidateMemory100ReportDirRejectsFakeIntegratedSurfaceReleaseSummary(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		...
		writeMemory100JSON(
			t,
			filepath.Join(surfaceDir, "surface-release-summary.json"),
			map[string]any{
...
func TestValidateMemory100ReportDirRejectsFakeIntegratedSurfaceExperimentalRuntimeReport(
	t *testing.T,
) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		...
		writeMemory100JSON(
			t,
			filepath.Join(experimentalDir, "surface-headless.json"),
			map[string]any{
	...
	if got := err.Error(); !strings.Contains(
		got,
		"integrated surface experimental runtime report",
	) {
...
func TestValidateMemory100ReportDirRejectsIncompleteMemoryFuzzBundle(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "memory fuzz") ||
		!strings.Contains(got, "summary.md") {
...
func TestValidateMemory100ReportDirRejectsMissingMemoryFuzzReproducerDirs(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		if err := os.RemoveAll(
			filepath.Join(root, "memory-fuzz", "reproducers", "compiler-crash"),
		); err != nil {
	...
	if got := err.Error(); !strings.Contains(got, "memory fuzz") ||
		!strings.Contains(got, "reproducers/compiler-crash") {
...
func TestValidateMemory100ReportDirRejectsCopiedMemoryFuzzBundleProvenance(t *testing.T) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		...
		writeMemory100FuzzSummary(
			t,
			filepath.Join(root, "memory-fuzz"),
			memory100TestHead,
			copiedDir,
		)
		writeMemory100HashManifest(t, filepath.Join(root, "memory-fuzz"))
	...
	if got := err.Error(); !strings.Contains(got, "memory fuzz") ||
		!strings.Contains(got, "current artifact dir") {
...
func TestValidateMemory100ReportDirRejectsUnclassifiedMemoryFuzzFailures(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "memory fuzz") ||
		!strings.Contains(got, "unclassified_failures") {
		t.Fatalf("error = %v, want memory fuzz unclassified failure rejection", err)
	}
}

func TestValidateMemory100ReportDirRejectsMemoryFuzzSummaryWithoutReproducibilitySeeds(
	t *testing.T,
) {
	reportDir := writeMemory100Fixture(t, func(root string, manifest map[string]any) {
		summaryPath := filepath.Join(root, "memory-fuzz", "summary.json")
	...
	if got := err.Error(); !strings.Contains(got, "memory fuzz") ||
		!strings.Contains(got, "reproducibility_seeds") {
...
func TestValidateMemory100ReportDirRejectsMockMemoryFuzzSeed(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "memory fuzz") ||
		!strings.Contains(got, "forbidden marker \"mock\"") {
		t.Fatalf("error = %v, want memory fuzz mock seed rejection", err)
	}
}

func TestValidateMemory100ReportDirAcceptsRelativeReportDirWithAbsoluteMemoryFuzzProvenance(
	t *testing.T,
) {
	reportDir := writeMemory100Fixture(t, nil)
	cwd, err := os.Getwd()
...
func TestValidateMemory100ReportDirRejectsDirtyLocalVerdict(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "dirty") ||
		!strings.Contains(got, "MEMORY100_SCOPED_READY_DIRTY") {
...
func TestValidateMemory100ReportDirRejectsCleanDirtyVerdict(t *testing.T) {
	...
	if got := err.Error(); !strings.Contains(got, "clean") ||
		!strings.Contains(got, "MEMORY100_SCOPED_READY_LOCAL") {
