...
func TestRunValidatesSurfacePerformanceBudgetReport(t *testing.T) {
	...
	if err := os.WriteFile(reportPath, []byte(validSurfacePerformanceBudgetReportJSON()), 0o644); err != nil {
...
func TestRunRejectsFasterThanElectronClaim(t *testing.T) {
	dir := t.TempDir()
	reportPath := filepath.Join(dir, "surface-performance-budget.json")
	raw := strings.Replace(validSurfacePerformanceBudgetReportJSON(), `"performance_claim":"none"`, `"performance_claim":"faster than Electron"`, 1)
	raw = strings.Replace(raw, `"electron_comparison":"none"`, `"electron_comparison":"faster than Electron"`, 1)
