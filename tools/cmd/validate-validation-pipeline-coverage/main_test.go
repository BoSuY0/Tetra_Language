...
func TestValidateValidationPipelineCoverageRejectsMissingBuildFileEntrypoint(t *testing.T) {
	path := filepath.Join(t.TempDir(), "coverage.json")
	raw := strings.Replace(validPipelineCoverageForTest(), `{"entrypoint":"BuildFileWithStatsOpt","artifact_path":"app","status":"validated_by_pipeline","validators":["ramcontract.ValidateReport"]},`, "", 1)
	if err := os.WriteFile(path, []byte(raw), 0o644); err != nil {
		t.Fatal(err)
	}
	err := validateValidationPipelineCoverage(path)
	if err == nil || !strings.Contains(err.Error(), "BuildFileWithStatsOpt") {
		t.Fatalf("validateValidationPipelineCoverage error = %v, want missing BuildFileWithStatsOpt rejection", err)
	}
}

func TestValidateValidationPipelineCoverageRejectsMissingObjectEntrypointWhenObjectArtifactPresent(t *testing.T) {
	path := filepath.Join(t.TempDir(), "coverage.json")
	raw := strings.Replace(validPipelineCoverageForTest(), `{"entrypoint":"buildObjectFileWithStatsOpt","status":"formal_exemption_with_reason","exemption":"not exercised by this linux-x64 RAM release fixture; object builds must carry their own RAM coverage evidence"},`, "", 1)
	if err := os.WriteFile(path, []byte(raw), 0o644); err != nil {
		t.Fatal(err)
	}
	err := validateValidationPipelineCoverage(path)
	if err == nil || !strings.Contains(err.Error(), "buildObjectFileWithStatsOpt") {
		t.Fatalf("validateValidationPipelineCoverage error = %v, want missing object entrypoint rejection", err)
	}
}

func TestValidateValidationPipelineCoverageRejectsInvalidExemption(t *testing.T) {
	path := filepath.Join(t.TempDir(), "coverage.json")
	raw := strings.Replace(validPipelineCoverageForTest(), "not exercised by this linux-x64 RAM release fixture; object builds must carry their own RAM coverage evidence", "todo", 1)
	if err := os.WriteFile(path, []byte(raw), 0o644); err != nil {
		t.Fatal(err)
	}
	err := validateValidationPipelineCoverage(path)
	if err == nil || !strings.Contains(err.Error(), "exemption") {
		t.Fatalf("validateValidationPipelineCoverage error = %v, want invalid exemption rejection", err)
	}
}
