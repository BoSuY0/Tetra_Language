...
func TestRunValidatesSurfaceReferenceAppsReport(t *testing.T) {
	...
	if err := os.WriteFile(
		reportPath,
		[]byte(validSurfaceReferenceAppsReportJSON()),
		0o644,
	); err != nil {
...
func TestRunRejectsSurfaceReferenceAppsScreenshotOnlyEvidence(t *testing.T) {
	...
	reportPath := filepath.Join(dir, "surface-reference-apps.json")
	raw := strings.Replace(
		validSurfaceReferenceAppsReportJSON(),
...
func TestRunRejectsSurfaceReferenceAppsMissingMorphToPixels(t *testing.T) {
	...
	raw := strings.Replace(
		validSurfaceReferenceAppsReportJSON(),
		`,"morph_to_pixels":`+morphToPixelsChainJSON(
...
func validSurfaceReferenceAppsReportJSON() string {
	...
	return `{"schema":"tetra.surface.reference-app-suite.v1","model":"surface-reference-app-suite-v1","release_scope":"surface-v1-linux-web","producer":"scripts/release/surface/surface-reference-apps-smoke.sh","app_count":10,"required_targets":["headless","linux-x64-real-window","wasm32-web-browser-canvas"],"apps":[` + strings.Join(
		apps,
		",",
	) + `],"visual_evidence":{"path":"reference-visual/surface-visual-regression.json","schema":"tetra.surface.visual-regression.v1","app_count":10,"pass":true},"negative_guards":{"screenshot_only_rejected":true,"missing_interaction_rejected":true,"missing_accessibility_rejected":true,"missing_performance_rejected":true,"core_widget_usage_rejected":true,"migration_widgets_compatibility_only":true,"no_react_runtime":true,"no_electron_runtime":true,"no_dom_app_ui_tree":true,"no_css_runtime":true,"no_user_js_app_logic":true},"pass":true}` + "\n"
}
...
func referenceAppJSON(shape string, source string, compatibility bool) string {
	...
	base := `{"shape":"` + shape + `","source":"` + source + `","module":"examples.` + strings.TrimSuffix(
		strings.TrimPrefix(strings.ReplaceAll(source, "/", "."), "examples."),
		".tetra",
	) + `","imports":["lib.core.surface","lib.core.block","lib.core.morph"],"recipes":["region.panel","field.text","control.action","command.item"],"beauty_coverage":` + referenceBeautyCoverageJSON(
		shape,
	) + `,"stable_morph_recipes":true,"resolves_to_block":true,"compiles":true,"runs":true,"exit_code":0,"token_theme_conformance":true,"layout_report":true,"interaction_trace":true,"accessibility_snapshot":true,"performance_budget":true,"artifact_hashes":true,"compatibility_widgets":` + boolJSON(
		compatibility,
	)
	if compatibility {
		return base + `,"infrastructure_only":true,"non_product_reason":"legacy widget migration compatibility evidence only","targets":[` + strings.Join(
			targets,
			",",
		) + `]}`
	}
	return base + `,"infrastructure_only":false,"morph_to_pixels":` + morphToPixelsChainJSON(
		source,
