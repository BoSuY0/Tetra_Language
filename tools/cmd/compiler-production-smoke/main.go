...
func main() {
	...
	flag.StringVar(
		...
		"report",
		"",
		"path to write tetra.compiler.production.v1 report",
	)
	flag.StringVar(
		&opt.TetraPath,
		"tetra",
...
func runSmoke(ctx context.Context, opt smokeOptions) error {
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" {
		return fmt.Errorf(
			"compiler production smoke requires linux/amd64 host, got %s/%s",
			runtime.GOOS,
			runtime.GOARCH,
		)
	}
	...
	if opt.TetraPath == "" {
		...
		r.cases = append(
			r.cases,
			compilerprod.CaseReport{
				Name: "fresh CLI compiler build",
				Kind: "positive",
				Ran:  true,
				Pass: true,
			},
...
func (r *smokeRunner) runVersion(ctx context.Context) error {
	...
	if res.err != nil || strings.TrimSpace(res.output) != expected {
		r.cases = append(
			r.cases,
			failedCase(
				compilerprod.VersionCaseName,
				"positive",
				"",
				fmt.Sprintf("want=%s exit=%d output=%s", expected, res.exitCode, res.output),
			),
		)
		return fmt.Errorf(
			"compiler version check failed: want=%s exit=%d output=%s",
			expected,
			res.exitCode,
			res.output,
		)
	...
	r.cases = append(
		r.cases,
		compilerprod.CaseReport{
			Name: compilerprod.VersionCaseName,
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
...
func (r *smokeRunner) runCompileMatrix(ctx context.Context) error {
	...
	build := runCommand(
		...
		"-o",
		nativeOut,
		filepath.Join("examples", "flow", "flow_hello.tetra"),
	)
	r.recordProcess(
		"linux native compile",
		"compile",
		r.tetraPath+" build --target linux-x64",
		build,
	)
	if build.err != nil {
		r.cases = append(
			r.cases,
			failedCase("linux-x64 native compile and run", "positive", "", build.output),
		)
	...
	if run.err != nil || run.exitCode != 0 {
		...
		return fmt.Errorf(
			"linux-x64 native run failed: exit=%d output=%s",
			run.exitCode,
			run.output,
		)
	}
	r.cases = append(
		r.cases,
		compilerprod.CaseReport{
			Name: "linux-x64 native compile and run",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
	...
	if res := runCommand(
		...
		r.cases = append(
			r.cases,
			failedCase("linux-x64 object emission", "positive", "", res.output),
		)
	...
	if err := requireFileWithPrefix(objectOut, []byte("TOBJ")); err != nil {
		r.cases = append(
			r.cases,
			failedCase("linux-x64 object emission", "positive", "", err.Error()),
		)
		return err
	}
	r.cases = append(
		r.cases,
		compilerprod.CaseReport{
			Name: "linux-x64 object emission",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
	...
	if res := runCommand(
		...
		r.cases = append(
			r.cases,
			failedCase("wasm32-wasi module emission", "positive", "", res.output),
		)
	...
	if err := requireFileWithPrefix(wasiOut, []byte{0x00, 0x61, 0x73, 0x6d}); err != nil {
		r.cases = append(
			r.cases,
			failedCase("wasm32-wasi module emission", "positive", "", err.Error()),
		)
		return err
	}
	r.cases = append(
		r.cases,
		compilerprod.CaseReport{
			Name: "wasm32-wasi module emission",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
	...
	if res := runCommand(
		...
		r.cases = append(
			r.cases,
			failedCase("wasm32-web module and loader emission", "positive", "", res.output),
		)
	...
	if err := requireFileWithPrefix(webOut, []byte{0x00, 0x61, 0x73, 0x6d}); err != nil {
		r.cases = append(
			r.cases,
			failedCase("wasm32-web module and loader emission", "positive", "", err.Error()),
		)
		return err
	}
	...
	if err != nil || !strings.Contains(string(loaderRaw), "tetra_web_v0.4.0") {
		r.cases = append(
			...
			failedCase(
				...
				"positive",
				"",
				fmt.Sprintf("web loader invalid: %v", err),
			),
		)
...
func (r *smokeRunner) runFocusedCompilerTests(ctx context.Context) error {
	tests := []struct {
		...
		{
			name:    "frontend parser fixture corpus",
			kind:    "positive",
			pkg:     "./compiler/internal/frontend",
			pattern: "TestParserFixtureCorpus",
		},
		{
			name: "semantic diagnostics stability",
			kind: "negative",
			pkg:  "./compiler/tests/frontend",
			pattern: ("^(TestDiagnosticFromFlowIndentationErrorJSONReady|TestDiagnostic" +
				"FromSafetyEffectErrorUsesEffectCode)$"),
			expectedError: "semantic diagnostic",
		},
		{
			name: "IR verifier diagnostics",
			kind: "negative",
			pkg:  "./compiler/internal/lower",
			pattern: ("^(TestVerifyProgramAcceptsLoweredControlFlow|TestVerifyFuncRejec" +
				"tsUnbalancedReturnStack|TestVerifyProgramRejectsDuplicateFunctionNames)$"),
			expectedError: "IR verifier",
		},
		{
			name: "backend format emission",
			kind: "positive",
			pkg: ("./compiler/internal/backend/linux_x64 ./compiler/internal/" +
				"backend/wasm32_wasi ./compiler/internal/backend/wasm32_web"),
			pattern: ("^(TestCodegenObjectLinuxX64SetsTargetAndUsesSysVRelocs|TestLinkO" +
				"bjectWASIImportExportShape|TestLinkObjectWebImportExportShape|TestLinkOb" +
				"jectWebOutputIsDeterministic)$"),
		},
		{
			name:          "CLI build option diagnostics",
			kind:          "negative",
			pkg:           "./cli/cmd/tetra",
			pattern:       "TestBuildCommandJSONDiagnosticsForOptionValidation",
			expectedError: "unsupported --runtime",
		},
		{
			name:    "compiler cache separates modes",
			kind:    "positive",
			pkg:     "./compiler",
			pattern: "TestBuildCacheSeparatesNativeDebugAndReleaseModes",
		},
	}
...
func (r *smokeRunner) runSmokeProfileCompilation(ctx context.Context) error {
	...
	r.recordProcess(
		"smoke profile compile matrix",
		"stress",
		r.tetraPath+" smoke --target linux-x64 --run=false",
		res,
	)
	if res.err != nil || res.exitCode != 0 {
		r.cases = append(
			r.cases,
			failedCase("smoke profile compilation matrix", "stress", "", res.output),
		)
	...
	r.cases = append(
		r.cases,
		compilerprod.CaseReport{
			Name: "smoke profile compilation matrix",
			Kind: "stress",
			Ran:  true,
			Pass: true,
		},
		...
		Contracts: []compilerprod.ContractReport{
			{
				...
				Status:   "pass",
				Evidence: "parser fixture corpus and positioned diagnostic tests",
			},
			{
				Name:     "semantic safety and type checking",
				Status:   "pass",
				Evidence: "semantic, safety, ownership, and effect diagnostic tests",
			},
			{
				Name:     "IR lowering and verifier",
				Status:   "pass",
				Evidence: "lowering verifier accepts valid IR and rejects invalid stack/signature metadata",
			},
			{
				Name:     "linux-x64 native backend and linker",
				Status:   "pass",
				Evidence: "fresh CLI compiles and runs linux-x64 executable plus object emission",
			},
			{
				Name:     "wasm target emission",
				Status:   "pass",
				Evidence: "wasm32-wasi and wasm32-web modules emit valid wasm headers and web loader",
			},
			{
				Name:     "object interface artifact pipeline",
				Status:   "pass",
				Evidence: "TOBJ object emission and interface-only compile cases",
			},
			{
				Name:     "CLI build check run contract",
				Status:   "pass",
				Evidence: "CLI version, build option diagnostics, and linux-x64 run evidence",
			},
			{
				Name:     "compiler cache and deterministic output",
				Status:   "pass",
				Evidence: "cache mode separation and deterministic backend output tests",
			},
		},
	...
	return []compilerprod.AuditReport{
		{
			...
			Evidence: ("parser fixture corpus and positioned diagnostic tests run under " +
				"compiler production smoke"),
			Result: "pass",
		},
		{
			Requirement: "semantic safety and type checking",
			Artifact: ("compiler/internal/semantics; compiler/tests/semantics; compiler/" +
				"tests/safety; compiler/tests/ownership"),
			Evidence: ("semantic and safety diagnostics remain stable through focused " +
				"compiler tests and memory/parallel gates"),
			Result: "pass",
		},
		{
			Requirement: "IR lowering and verifier",
			Artifact:    "compiler/internal/lower",
			Evidence: ("lowering verifier accepts valid control flow and rejects stack/" +
				"signature metadata drift"),
			Result: "pass",
		},
		{
			Requirement: "linux-x64 native backend and linker",
			Artifact: ("compiler/internal/backend/linux_x64; compiler/internal/format/" +
				"elf; examples/flow/flow_hello.tetra"),
			Evidence: ("fresh CLI emits and runs linux-x64 executable, and emits linux-" +
				"x64 TOBJ object evidence"),
			Result: "pass",
		},
		{
			Requirement: "wasm target emission",
			Artifact: ("compiler/internal/backend/wasm32_wasi; compiler/internal/" +
				"backend/wasm32_web; examples/smoke/basic/hello.tetra"),
			Evidence: ("fresh CLI emits wasm32-wasi and wasm32-web modules with valid " +
				"wasm headers and web loader"),
			Result: "pass",
		},
		{
			...
			Evidence: "object emission and interface-only compile are required compiler production cases",
			Result:   "pass",
		},
		{
			Requirement: "CLI build check run contract",
			Artifact: ("cli/cmd/tetra/tetra_core.go; cli/cmd/tetra/tetra_suite_test.go; " +
				"cli/cmd/tetra/tetra_suite_test.go; docs/spec/policy/cli_contracts.md"),
			Evidence: ("fresh CLI version, linux-x64 compile/run, and build option " +
				"diagnostics are required cases"),
			Result: "pass",
		},
		{
			...
			Evidence: "cache separation and deterministic backend output tests are required cases",
			Result:   "pass",
		},
		{
			Requirement: "release-gate entrypoint",
			Artifact:    "scripts/release/post_v0_4/compiler-production-linux-x64-smoke.sh",
			Evidence: ("entrypoint writes compiler-production-linux-x64.json and runs " +
				"compiler-production-smoke plus validate-compiler-production"),
			Result: "pass",
		},
	}
}
