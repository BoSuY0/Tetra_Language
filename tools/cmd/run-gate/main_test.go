...
func TestDryRunJSONOutputIncludesOrderedPlan(t *testing.T) {
	...
	assertStepIDs(t, plan.Steps, []string{"surface-crash-report-smoke", "validate-surface-release-state"})
	assertReportPaths(t, plan.RequiredReports, []string{"surface-crash-report.json", "surface-release-summary.json", "artifact-hashes.json"})
	assertValidatorIDs(t, plan.Validators, []string{"validate-surface-crash-report", "validate-surface-runtime-release-summary", "validate-artifact-hashes", "validate-surface-release-state"})
	if plan.ArtifactHashes == nil || !plan.ArtifactHashes.Enabled || !plan.ArtifactHashes.Required || plan.ArtifactHashes.Algorithm != "sha256" {
	...
	if plan.ArtifactHashCommandPlan.ManifestPath != manifestPath {
		t.Fatalf("hash manifest path = %q, want %q", plan.ArtifactHashCommandPlan.ManifestPath, manifestPath)
	}
	wantWriteArgs := []string{"go", "run", "./tools/cmd/validate-artifact-hashes", "--write", "--root", resolvedReportDir, "--out", manifestPath}
	if !reflect.DeepEqual(plan.ArtifactHashCommandPlan.Write.Args, wantWriteArgs) {
		t.Fatalf("hash write args = %#v, want %#v", plan.ArtifactHashCommandPlan.Write.Args, wantWriteArgs)
	}
	wantValidateArgs := []string{"go", "run", "./tools/cmd/validate-artifact-hashes", "--manifest", manifestPath}
	if !reflect.DeepEqual(plan.ArtifactHashCommandPlan.Validate.Args, wantValidateArgs) {
		t.Fatalf("hash validate args = %#v, want %#v", plan.ArtifactHashCommandPlan.Validate.Args, wantValidateArgs)
	}
...
func TestRunGateExecutionRejectsRecursiveSameContractDispatch(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), `refusing recursive run-gate execution for contract "fixture-surface-release-v1"`) {
...
func TestRunGateExecutionRejectsUnsafeContractEntrypoint(t *testing.T) {
	cases := []struct {
		...
		{name: "absolute", entrypoint: "/tmp/release-gate.sh", want: "absolute contract entrypoint is not allowed"},
		{name: "parent traversal", entrypoint: "../release-gate.sh", want: "parent traversal is not allowed in contract entrypoint"},
		{name: "dash prefixed", entrypoint: "-c", want: "dash-prefixed contract entrypoint is not allowed"},
	}
...
func TestMissingRequiredFlagsFail(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			args: []string{"--report-dir", filepath.Join("reports", "surface-release-v1"), "--dry-run", "--json"},
...
func TestReportDirValidationRejectsNonEmptyDirectory(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(repoRoot, reportDir, "old.json"), []byte("{}\n"), 0o644); err != nil {
...
func TestRealSurfaceContractRepresentsReleaseGateRequiredReports(t *testing.T) {
	contract, err := gatecontract.Load(filepath.Join("..", "..", "..", "scripts", "release", "surface", "contracts", "surface-release-v1.json"))
	if err != nil {
	...
	if !contains(report.ClaimRefs, "crash_reporting") {
		t.Fatalf("surface-crash-report.json claim_refs = %#v, want crash_reporting", report.ClaimRefs)
	}
	...
	if validator.Command != wantCommand {
		t.Fatalf("validate-surface-crash-report command = %q, want %q", validator.Command, wantCommand)
	}
	for _, nonclaim := range contract.Nonclaims {
		if nonclaim.ID == "not_full_surface_required_report_contract" {
			t.Fatalf("nonclaim %q should be removed from the full required-report contract", nonclaim.ID)
		}
	}
}
...
func writeFixtureContract(t *testing.T, repoRoot string, mutate func(map[string]any)) string {
	...
	doc := map[string]any{
		...
		"claims": []any{
			map[string]any{"id": "crash_reporting", "statement": "Surface crash reporting evidence is produced and validator checked."},
			map[string]any{"id": "surface_release_summary", "statement": "Surface release summary is present and validator checked."},
			map[string]any{"id": "artifact_hash_integrity", "statement": "Release reports are covered by a sha256 artifact hash manifest."},
			map[string]any{"id": "release_state_current", "statement": "Release-state validation is part of this dry-run contract."},
		},
		"nonclaims": []any{
			map[string]any{"id": "not_full_surface_required_report_contract", "statement": "This fixture contract does not enumerate every Surface release-gate required report."},
		},
		"ci_artifacts": []any{
...
func findRequiredReport(reports []gatecontract.RequiredReport, path string) (gatecontract.RequiredReport, bool) {
	for _, report := range reports {
		if report.Path == path {
			return report, true
		}
