...
func run(args []string, stdout, stderr io.Writer) int {
	...
	fs.StringVar(&reportDir, "report-dir", "", "fresh report directory, relative to repository root")
...
func runContractEntrypoint(contractPath, reportDir string, stdout, stderr io.Writer) int {
	...
	if os.Getenv(runGateContractExecEnv) == "1" && os.Getenv(runGateContractIDEnv) == contract.ID {
		fmt.Fprintf(stderr, "error: refusing recursive run-gate execution for contract %q\n", contract.ID)
		return 2
	}
...
func loadContractRunInputs(contractPath, reportDir string) (gatecontract.Contract, string, string, error) {
	...
	if err != nil {
		return gatecontract.Contract{}, "", "", fmt.Errorf("resolve current working directory: %w", err)
	}
...
func cleanContractEntrypoint(entrypoint string) (string, error) {
	...
	if cleaned == "." || cleaned == ".." || strings.HasPrefix(cleaned, ".."+string(os.PathSeparator)) {
		return "", fmt.Errorf("parent traversal is not allowed in contract entrypoint: %s", entrypoint)
	}
...
func runGateEntrypointEnv(base []string, contractID, reportDir string) []string {
	...
	for _, value := range base {
		if envNameIs(value, runGateContractExecEnv) || envNameIs(value, runGateContractIDEnv) || envNameIs(value, runGateContractReportDirEnv) {
			continue
		}
