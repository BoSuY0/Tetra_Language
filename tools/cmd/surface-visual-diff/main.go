...
func run(args []string) error {
	...
	fs.Var(
		&runtimeReports,
		"runtime-report",
		"path to a tetra.surface.runtime.v1 report with block_system evidence; may be repeated",
	)
	fs.Var(
		&blockExamplesReports,
		"block-examples-report",
		"path to a tetra.surface.block-examples.v1 polished reference app report; may be repeated",
	)
	fs.Var(
		&requiredTargets,
		"required-target",
		"visual target that every reference app must cover; may be repeated",
	)
	fs.Var(
		&frameArtifactFlags,
		"frame-artifact",
		"current frame artifact as source,target,order,path; path must be .rgba or .png; may be repeated",
	)
	fs.Var(
		&goldenArtifactFlags,
		"golden-artifact",
		"golden frame artifact as source,target,order,path; path must be .rgba or .png; may be repeated",
	)
	...
	fs.StringVar(
		...
		"git-head",
		"",
		"git head used for visual/golden evidence; defaults to git rev-parse HEAD",
	)
	fs.StringVar(
		&goldenSet,
		"golden-set",
		"surface-visual-regression-v1",
		"visual golden set identifier",
	)
	fs.BoolVar(
		&writeGolden,
		"write-golden",
		false,
		"write missing golden artifacts from current frame artifacts; forbidden in release/product gates",
	)
...
func buildVisualReport(
	runtimeReportPaths []string,
	blockExamplesReportPaths []string,
	requiredTargets []string,
	gitHead string,
	goldenSet string,
	frameArtifacts visualArtifactSpecs,
	goldenArtifacts visualArtifactSpecs,
	writeGolden bool,
) (surface.VisualRegressionReport, error) {
	...
	for _, reportPath := range runtimeReportPaths {
		...
		if runtimeReport.Schema != surface.SchemaV1 {
			return surface.VisualRegressionReport{}, fmt.Errorf(
				"%s schema is %q, want %s",
				reportPath,
				runtimeReport.Schema,
				surface.SchemaV1,
			)
		}
		if runtimeReport.BlockSystem == nil &&
			!hasRuntimeProductVisualFrameEvidence(runtimeReport) {
			return surface.VisualRegressionReport{}, fmt.Errorf(
				"%s missing block_system or product_visual frame visual evidence source",
				reportPath,
			)
		}
		...
		targetReport, err := buildVisualTarget(
			reportPath,
			runtimeReport,
			target,
			gitHead,
	...
	for _, reportPath := range blockExamplesReportPaths {
		...
		for _, example := range examplesReport.Examples {
			app, targetReports, err := visualAppForBlockExample(
				reportPath,
				example,
				requiredTargets,
				runtimeByTarget,
	...
	return surface.VisualRegressionReport{
		...
		GoldenSet: goldenSet,
		GoldenHash: checksum(
			strings.Join(append([]string{goldenSet, gitHead}, hashParts...), "\n"),
		),
...
func validateBlockExamplesReport(path string, report blockExamplesReport) error {
	if report.Schema != "tetra.surface.block-examples.v1" {
		return fmt.Errorf(
			"%s schema is %q, want tetra.surface.block-examples.v1",
			path,
			report.Schema,
		)
	}
	if report.QualityLevel != "block-only-polished-examples-v1" {
		return fmt.Errorf(
			"%s quality_level is %q, want block-only-polished-examples-v1",
			path,
			report.QualityLevel,
		)
	}
	...
	if report.ExampleCount != len(report.Examples) {
		return fmt.Errorf(
			"%s example_count = %d, want len(examples) %d",
			path,
			report.ExampleCount,
			len(report.Examples),
		)
	...
	for _, guard := range []string{
		"core_widget_usage_rejected",
		"missing_accessibility_rejected",
		"missing_hover_focus_pressed_state",
	} {
...
func visualAppForBlockExample(
	reportPath string,
	example blockExampleVisualReport,
	requiredTargets []string,
	runtimeByTarget map[string]visualRuntimeEvidence,
	gitHead string,
	frameArtifacts visualArtifactSpecs,
	goldenArtifacts visualArtifactSpecs,
	writeGolden bool,
) (surface.VisualRegressionAppReport, []surface.VisualRegressionTargetReport, error) {
	app := surface.VisualRegressionAppReport{
	...
	if _, err := sha256File(example.Artifact); err != nil {
		return app, nil, fmt.Errorf(
			"%s example %s artifact hash: %w",
			reportPath,
			example.Path,
			err,
		)
	...
	for _, target := range requiredTargets {
		...
		if !ok {
			return app, nil, fmt.Errorf(
				"%s example %s missing runtime target evidence for %s",
				reportPath,
				example.Path,
				target,
			)
		}
		frame, err := visualFrameForBlockExample(
			example,
			target,
			runtimeEvidence,
			frameArtifacts,
		...
		targetReport := surface.VisualRegressionTargetReport{
			...
			PerformanceEvidence: runtimeEvidence.Report.BlockSystem != nil &&
				runtimeEvidence.Report.BlockSystem.MemoryBudget != nil &&
				example.ChecksumEvidence,
...
func visualFrameForBlockExample(
	example blockExampleVisualReport,
	target string,
	runtimeEvidence visualRuntimeEvidence,
	frameArtifacts visualArtifactSpecs,
	goldenArtifacts visualArtifactSpecs,
	writeGolden bool,
) (surface.VisualRegressionFrameReport, error) {
	width := 320
	height := 200
	stride := 1280
	if runtimeEvidence.Report.BlockSystem != nil &&
		len(runtimeEvidence.Report.BlockSystem.Frames) > 0 {
		frame := runtimeEvidence.Report.BlockSystem.Frames[0]
		width = frame.Width
		height = frame.Height
		stride = frame.Stride
	}
	return visualArtifactFrame(
		example.Path,
		target,
		"reference-app-visual",
		1,
		width,
		height,
		stride,
...
func buildVisualTarget(
	...
	goldenArtifacts visualArtifactSpecs,
	writeGolden bool,
) (surface.VisualRegressionTargetReport, error) {
	frames, err := visualFramesForRuntimeReport(
		report.Source,
		target,
		report,
		frameArtifacts,
	...
	if err != nil {
		return surface.VisualRegressionTargetReport{}, fmt.Errorf(
			"%s target %s: %w",
			reportPath,
			target,
			err,
	...
	return surface.VisualRegressionTargetReport{
		...
		LayoutEvidence: len(report.LayoutConstraints) > 0 && len(report.LayoutPasses) > 0 &&
			len(report.LayoutScrolls) > 0,
		AccessibilityEvidence: report.BlockAccessibilityTree != nil ||
			report.AccessibilityTree != nil,
		PerformanceEvidence: hasVisualPerformanceEvidence(report),
		Frames:              frames,
	}, nil
}

func visualFramesForRuntimeReport(
	source string,
	target string,
	report surface.Report,
	frameArtifacts visualArtifactSpecs,
...
func visualRuntimeFrames(
	...
	for _, frame := range frames {
		...
		visualFrame, err := visualArtifactFrame(
			source,
			target,
			visualRuntimeFrameLabel(target, frame),
			frame.Order,
			frame.Width,
			frame.Height,
			frame.Stride,
			frame.Checksum,
			effectiveFrameArtifacts,
			goldenArtifacts,
			writeGolden,
		)
		if err != nil {
...
func visualFrames(
	...
	runtimeFrames []surface.FrameReport,
	frameArtifacts visualArtifactSpecs,
	goldenArtifacts visualArtifactSpecs,
	writeGolden bool,
) ([]surface.VisualRegressionFrameReport, error) {
...
func visualArtifactFrame(
	...
	if !ok {
		return surface.VisualRegressionFrameReport{}, fmt.Errorf(
			"frame artifact is required for source %s target %s order %d",
			source,
			target,
			order,
		)
	...
	if !ok {
		return surface.VisualRegressionFrameReport{}, fmt.Errorf(
			"golden artifact is required for source %s target %s order %d",
			source,
			target,
			order,
		)
	...
	if normalizeArtifactFilePath(currentPath) == normalizeArtifactFilePath(goldenPath) {
		return surface.VisualRegressionFrameReport{}, fmt.Errorf(
			"self-golden artifact rejected for source %s target %s order %d",
			source,
			target,
			order,
		)
	...
	if err != nil {
		return surface.VisualRegressionFrameReport{}, fmt.Errorf(
			"read frame artifact %s: %w",
			currentPath,
			err,
		)
	}
	if expectedChecksum = normalizeSHA256Checksum(expectedChecksum); expectedChecksum != "" &&
		current.SHA256 != expectedChecksum {
		return surface.VisualRegressionFrameReport{}, fmt.Errorf(
			("frame artifact checksum %s must match runtime frame checksum %s " +
				"for source %s target %s order %d"),
			current.SHA256,
			expectedChecksum,
			source,
			target,
	...
	diffPixels, diffRatioMilli, maxChannelDelta, err := visualArtifactDiff(
		current,
		golden,
		width,
		height,
	...
	pass := diffPixels <= tolerancePixels && diffRatioMilli <= toleranceRatioMilli &&
		maxChannelDelta <= toleranceChannelDelta
...
func hasTokenThemeEvidence(report surface.Report) bool {
	return len(report.VisualFeatures) > 0 || len(report.LayoutFeatures) > 0 ||
...
func visualRenderer(report surface.Report) string {
	...
	if report.RenderCommandStream != nil &&
		strings.TrimSpace(report.RenderCommandStream.Renderer) != "" {
...
func hasVisualPerformanceEvidence(report surface.Report) bool {
	...
	return report.SurfacePerformanceBudget != nil ||
		(report.Morph != nil && report.Morph.MemoryBudget.FrameCount > 0)
}
	...
	for _, value := range values {
		...
		if _, exists := specs[key]; exists {
			return nil, fmt.Errorf(
				"%s duplicate artifact for source %s target %s order %d",
				flagName,
				source,
				target,
				order,
...
func readVisualArtifact(
	path string,
	width int,
	height int,
	stride int,
) (visualArtifactData, error) {
	...
	case "rgba":
		if stride < width*4 {
			return visualArtifactData{}, fmt.Errorf(
				"rgba stride %d is smaller than width*4 %d",
				stride,
				width*4,
			)
		}
		...
		if len(raw) != expectedBytes {
			return visualArtifactData{}, fmt.Errorf(
				"rgba size = %d, want %d bytes for %dx%d stride %d",
				len(raw),
				expectedBytes,
				width,
				height,
...
func visualArtifactDiff(
	current visualArtifactData,
	golden visualArtifactData,
	width int,
	height int,
	stride int,
) (int, int, int, error) {
	if current.Format != golden.Format {
		return 0, 0, 0, fmt.Errorf(
			"current/golden artifact format mismatch: %s vs %s",
			current.Format,
			golden.Format,
		)
	}
...
func visualRGBADiff(
	...
	if len(current) != len(golden) {
		return 0, 0, 0, fmt.Errorf(
			"current/golden rgba byte sizes differ: %d vs %d",
			len(current),
			len(golden),
		)
	}
