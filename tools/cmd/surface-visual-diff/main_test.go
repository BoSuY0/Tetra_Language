...
func TestRunGeneratesValidSurfaceVisualReport(t *testing.T) {
	...
	artifactEvidence := writeVisualArtifactArgs(
		t,
		dir,
	...
	if err := os.WriteFile(
		runtimePath,
		validVisualDiffRuntimeReportJSON(t, false, artifactEvidence.CurrentSHA),
		0o644,
	); err != nil {
...
func TestRunRejectsMajorGoldenDrift(t *testing.T) {
	...
	if err := os.WriteFile(
		runtimePath,
		validVisualDiffRuntimeReportJSON(t, true, artifactEvidence.CurrentSHA),
		0o644,
	); err != nil {
		t.Fatalf("write runtime report: %v", err)
	}
...
func TestRunRejectsMetadataOnlyVisualEvidence(t *testing.T) {
	...
	if err := os.WriteFile(
		runtimePath,
		validVisualDiffRuntimeReportJSON(t, false),
		0o644,
	); err != nil {
		t.Fatalf("write runtime report: %v", err)
	}
...
func TestRunRejectsSelfGoldenArtifact(t *testing.T) {
	...
	runtimePath := filepath.Join(dir, "surface-headless-block-system.json")
	outPath := filepath.Join(dir, "surface-visual-regression.json")
	currentPath := writeRGBAArtifact(
		t,
		filepath.Join(dir, "frames", "self.rgba"),
		320,
		200,
		1280,
		"self-golden",
	...
	if err := os.WriteFile(
		runtimePath,
		validVisualDiffRuntimeReportJSON(t, false, currentSHA),
		0o644,
	); err != nil {
		t.Fatalf("write runtime report: %v", err)
	}
...
func TestRunUsesRuntimeFrameArtifactPathWithBareChecksum(t *testing.T) {
	...
	currentPath := writeRGBAArtifact(
		t,
		filepath.Join(dir, "runtime", "initial.rgba"),
		320,
		200,
		1280,
	...
	goldenPath := writeRGBAArtifact(
		t,
		filepath.Join(dir, "goldens", "initial.rgba"),
		320,
		200,
		1280,
		"runtime-artifact-path",
	...
	if err := json.Unmarshal(
		validVisualDiffRuntimeReportJSON(t, false, bareCurrentSHA),
		&report,
	); err != nil {
	...
	if frame.ArtifactPath != currentPath {
		t.Fatalf(
			"artifact_path = %q, want runtime frame artifact %q",
			frame.ArtifactPath,
			currentPath,
		)
	}
	...
	if frame.ArtifactSHA256 != currentSHA {
		t.Fatalf(
			"artifact_sha256 = %q, want normalized artifact SHA %q",
			frame.ArtifactSHA256,
			currentSHA,
		)
	}
	if !strings.HasPrefix(frame.Checksum, "sha256:") ||
		!strings.HasPrefix(frame.ArtifactSHA256, "sha256:") {
		t.Fatalf(
			"visual frame checksums must be sha256-prefixed: checksum=%q artifact_sha256=%q",
			frame.Checksum,
			frame.ArtifactSHA256,
		)
	}
...
func TestRunWritesGoldenOnlyWithExplicitMode(t *testing.T) {
	...
	currentPath := writeRGBAArtifact(
		t,
		filepath.Join(dir, "frames", "current.rgba"),
		320,
		200,
		1280,
		"write-golden",
...
func TestRunGeneratesPolishedReferenceAppVisualEvidence(t *testing.T) {
	...
	for _, target := range []string{"headless", "linux-x64-real-window", "wasm32-web-browser-canvas"} {
		blockSystemEvidence[target] = writeVisualArtifactArgs(
			t,
			dir,
			"examples/surface/block_core/surface_block_system.tetra",
			target,
	...
	headlessPath := writeVisualDiffRuntimeReport(
		t,
		dir,
		"headless",
		"headless",
		false,
		blockSystemEvidence["headless"].CurrentSHA,
	)
	linuxPath := writeVisualDiffRuntimeReport(
		t,
		dir,
		"linux-x64-real-window",
		"linux-x64",
		false,
		blockSystemEvidence["linux-x64-real-window"].CurrentSHA,
	)
	webPath := writeVisualDiffRuntimeReport(
		t,
		dir,
		"wasm32-web-browser-canvas",
		"wasm32-web",
		false,
		blockSystemEvidence["wasm32-web-browser-canvas"].CurrentSHA,
	)
	examplesPath := filepath.Join(dir, "surface-block-examples.json")
	if err := os.WriteFile(
		examplesPath,
		validBlockExamplesReportJSON(t, dir, false),
		0o644,
	); err != nil {
	...
	for _, source := range []string{
		...
		if app.Targets[0].Frames[0].Checksum == findVisualApp(
			report,
			"examples/surface/block_core/surface_block_system.tetra",
		).Targets[0].Frames[0].Checksum {
...
func TestRunRejectsPolishedReferenceAppWithMissingEvidence(t *testing.T) {
	dir := t.TempDir()
	blockSystemEvidence := writeVisualArtifactArgs(
		t,
		dir,
		"examples/surface/block_core/surface_block_system.tetra",
		"headless",
	...
	runtimePath := writeVisualDiffRuntimeReport(
		t,
		dir,
		"headless",
		"headless",
		false,
		blockSystemEvidence.CurrentSHA,
	)
	examplesPath := filepath.Join(dir, "surface-block-examples.json")
	if err := os.WriteFile(
		examplesPath,
		validBlockExamplesReportJSON(t, dir, true),
		0o644,
	); err != nil {
		t.Fatalf("write block examples report: %v", err)
	}
...
func writeVisualDiffRuntimeReport(
	t *testing.T,
	dir string,
	visualTargetName string,
	reportTarget string,
	drift bool,
	...
	var report surface.Report
	if err := json.Unmarshal(
		validVisualDiffRuntimeReportJSON(t, drift, checksumOverride...),
		&report,
	); err != nil {
		t.Fatalf("decode runtime fixture: %v", err)
	}
...
func findVisualApp(
	report surface.VisualRegressionReport,
	source string,
) *surface.VisualRegressionAppReport {
...
func validBlockExamplesReportJSON(t *testing.T, dir string, missingEvidence bool) []byte {
	...
	for _, name := range []string{"surface_block_command_palette", "surface_block_settings"} {
		...
		examples = append(examples, map[string]any{
			"path":                   "examples/surface/block_apps/" + name + ".tetra",
			"block_only":             true,
			"compiles":               true,
			"runs":                   true,
			"exit_code":              0,
