...
func parseAllocationReportSummary(raw []byte) (allocationReportSummary, error) {
	...
	if report.SchemaVersion != 2 {
		return allocationReportSummary{}, fmt.Errorf(
			"small heap allocation report schema_version = %d, want 2",
			report.SchemaVersion,
		)
	}
	if report.Summary.AllocationCount <= 0 {
		return allocationReportSummary{}, fmt.Errorf(
			"small heap allocation report allocation_count = %d, want positive",
			report.Summary.AllocationCount,
		)
	}
	if report.Summary.RuntimePaths == nil {
		return allocationReportSummary{}, fmt.Errorf(
			"small heap allocation report missing runtime_paths summary",
		)
	}
	return report, nil
}

func validateAllocationLengthContractCorrelation(
	cases []memoryprod.CaseReport,
	report allocationReportSummary,
) error {
	var issues []string
	...
	for _, req := range requireRows {
		if !allocationReportHasLengthContract(report, req) {
			issues = append(
				issues,
				fmt.Sprintf(
					("allocation report missing contract row builtin=%s length_"+
						"status=%s zero=%s negative=%s overflow=%s"),
					req.Builtin,
					req.LengthStatus,
					req.ZeroGuardStatus,
					req.NegativeGuardStatus,
					req.OverflowGuardStatus,
				),
			)
		}
	}
...
func validateAllocationLengthRuntimeCases(cases []memoryprod.CaseReport) []string {
	...
	for _, req := range []struct {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("missing allocation length runtime case %q", req.name),
			)
			continue
		}
		if !c.Ran || !c.Pass {
			issues = append(
				issues,
				fmt.Sprintf("allocation length runtime case %q did not pass", req.name),
			)
		}
		if req.expectedError != "" && !strings.Contains(c.ExpectedError, req.expectedError) {
			issues = append(
				issues,
				fmt.Sprintf(
					"allocation length runtime case %q expected_error = %q, want %q",
					req.name,
					c.ExpectedError,
					req.expectedError,
				),
...
func allocationReportHasLengthContract(
	report allocationReportSummary,
	req allocationReportAllocationSummary,
) bool {
...
func parseMemoryReportEvidence(raw []byte) (memoryReportEvidence, error) {
	...
	if report.SchemaVersion != "tetra.memory-report.v1" {
		return memoryReportEvidence{}, fmt.Errorf(
			"memory report schema_version = %q, want tetra.memory-report.v1",
			report.SchemaVersion,
		)
	}
	return report, nil
...
func validateRawPointerBoundsCorrelation(cases []memoryprod.CaseReport, memoryRaw []byte) error {
	...
	requireRows := []struct {
	...
	}{
		{
			...
			minRows:  1,
			validate: validateUnsafeVerifiedRootAllocationBaseRow,
		},
		...
		{claim: "rejected_upper_bound", minRows: 1, validate: validateUnsafeCheckedRejectedRow},
		{
			...
			minRows:         4,
			validate:        validateUnsafeCheckedRejectedRow,
			requireCheckRow: true,
		},
		{
			claim:    "checked_external_unknown",
			minRows:  1,
			validate: validateUnsafeUnknownConservativeRow,
		},
	}
	for _, req := range requireRows {
		valid := 0
		for _, row := range report.Rows {
			...
			if req.requireCheckRow && row.SourceFactID != "" && !checksByParent[row.SourceFactID] {
				rowIssues = append(
					rowIssues,
					fmt.Sprintf(
						("source_fact_id %q is missing parented raw_bounds_runtime_check_"+
							"normal_build normal_build_check"),
						row.SourceFactID,
					),
				)
			}
			if len(rowIssues) > 0 {
				issues = append(
					issues,
					fmt.Sprintf(
						"raw bounds claim %s row is not correlated: %s",
						req.claim,
						strings.Join(rowIssues, ", "),
					),
				)
				continue
			}
		...
		if valid < req.minRows {
			issues = append(
				issues,
				fmt.Sprintf(
					"raw bounds claim %s has %d correlated row(s), want at least %d",
					req.claim,
					valid,
					req.minRows,
				),
	...
	for _, row := range report.Rows {
		if row.ProvenanceClass == "unsafe_verified_root" ||
			row.UnsafeClass == "unsafe_verified_root" {
			switch row.Claim {
			...
			default:
				issues = append(
					issues,
					fmt.Sprintf(
						"unsafe_verified_root row %q cannot claim %q beyond bounded allocation metadata",
						row.SourceFactID,
						row.Claim,
					),
				)
...
func validateRawSliceGatewayCorrelation(cases []memoryprod.CaseReport, memoryRaw []byte) error {
	...
	requireRows := []struct {
		...
		{claim: "external_unknown", minRows: 1, validate: validateUnsafeUnknownConservativeRow},
		{
			...
			minRows:  1,
			validate: validateUnsafeCheckedDynamicRow,
		},
	...
	for _, req := range requireRows {
		...
		for _, row := range report.Rows {
			...
			if len(rowIssues) > 0 {
				issues = append(
					issues,
					fmt.Sprintf(
						"raw slice claim %s row is not correlated: %s",
						req.claim,
						strings.Join(rowIssues, ", "),
					),
				)
		...
		if valid < req.minRows {
			issues = append(
				issues,
				fmt.Sprintf(
					"raw slice claim %s has %d correlated row(s), want at least %d",
					req.claim,
					valid,
					req.minRows,
				),
...
func validateRawRuntimeCases(cases []memoryprod.CaseReport) []string {
	...
	for _, req := range []struct {
		...
		if !c.Ran || !c.Pass {
			issues = append(
				issues,
				fmt.Sprintf("runtime raw bounds case %q did not pass", req.name),
			)
		}
		if req.expectedError != "" && !strings.Contains(c.ExpectedError, req.expectedError) {
			issues = append(
				issues,
				fmt.Sprintf(
					"runtime raw bounds case %q expected_error = %q, want %q",
					req.name,
					c.ExpectedError,
					req.expectedError,
				),
	...
	for _, req := range []struct {
		...
		if req.expectedError != "" && !strings.Contains(c.ExpectedError, req.expectedError) {
			issues = append(
				issues,
				fmt.Sprintf(
					"runtime raw slice case %q expected_error = %q, want %q",
					req.name,
					c.ExpectedError,
					req.expectedError,
				),
...
func parentedRawBoundsRuntimeChecks(rows []memoryReportEvidenceRow) map[string]bool {
	...
	for _, row := range rows {
		...
		if row.CostClass != "dynamic_check_required" ||
			row.ValidatorName != "raw_bounds_width_validator" ||
			row.ClaimLevel != "validated" {
			continue
		}
	...
	for _, row := range rows {
		if row.Claim == "cap_mem_authorization_only" {
			...
			if len(rowIssues) > 0 {
				issues = append(
					issues,
					fmt.Sprintf(
						"cap.mem authorization row is not evidence-only: %s",
						strings.Join(rowIssues, ", "),
					),
				)
			}
		...
		if strings.Contains(strings.ToLower(row.ValidatorName+" "+row.Reason), "cap_mem") ||
			...
			if proofClaims[row.Claim] {
				issues = append(
					issues,
					fmt.Sprintf("cap.mem authorization cannot validate proof claim %q", row.Claim),
				)
			}
		}
	}
