...
func TestWriteRAMMeasurementReportCapturesMemStatsSnapshots(t *testing.T) {
	...
	for _, name := range []string{"heap_alloc_bytes", "bytes_requested", "bytes_reserved", "bytes_copied", "rss_current", "rss_peak", "per_actor_domain_bytes"} {
		if _, ok := samples[name]; !ok {
			t.Fatalf("metric_samples missing %s: %#v", name, report.MetricSamples)
		}
	}
	if samples["heap_alloc_bytes"].EvidenceClass != "runtime_measured" || samples["heap_alloc_bytes"].Method != "MemStats" {
		t.Fatalf("heap metric sample = %+v, want runtime_measured MemStats", samples["heap_alloc_bytes"])
	}
	if samples["rss_current"].EvidenceClass != "unsupported" || samples["rss_peak"].EvidenceClass != "unsupported" {
		t.Fatalf("rss samples = current %+v peak %+v, want unsupported", samples["rss_current"], samples["rss_peak"])
	}
}
