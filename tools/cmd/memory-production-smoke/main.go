...
func main() {
	...
	flag.StringVar(
		&opt.RAMMeasurementPath,
		"ram-measurement-report",
		"",
		"optional path to write tetra.memory.ram-measurement.v1 MemStats report",
	)
	flag.StringVar(
		&opt.TetraPath,
		"tetra",
	...
	flag.StringVar(
		...
		"git-head",
		"",
		"optional git HEAD provenance to include in the report",
	)
...
func runSmoke(ctx context.Context, opt smokeOptions) error {
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" {
		return fmt.Errorf(
			"memory production smoke requires linux/amd64 host, got %s/%s",
			runtime.GOOS,
			runtime.GOARCH,
		)
	}
...
func (r *smokeRunner) runPositiveApp(ctx context.Context) error {
	...
	r.cases = append(
		r.cases,
		memoryprod.CaseReport{
			Name: "allocator alloc/free lifecycle",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
		memoryprod.CaseReport{
			Name: "memcpy/memset capability path",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
...
func (r *smokeRunner) runCheckedInExamples(ctx context.Context) error {
	examples := []struct {
		...
		{
			path:         filepath.Join("examples", "core", "memory", "core_memory_smoke.tetra"),
			expectedExit: 42,
		},
		{
			path:         filepath.Join("examples", "memory", "ownership", "ownership_smoke.tetra"),
			expectedExit: 42,
		},
		{
			path:         filepath.Join("examples", "flow", "flow_unsafe_cap_mem_smoke.tetra"),
			expectedExit: 42,
		},
	}
		...
		if build.err != nil {
			r.cases = append(
				...
				failedCase(
					...
					"positive",
					"",
					fmt.Sprintf("build %s failed: %s", sourcePath, build.output),
				),
			)
		...
		if run.exitCode != example.expectedExit {
			r.cases = append(
				...
				failedCase(
					...
					"positive",
					"",
					fmt.Sprintf("run %s exit=%d output=%s", sourcePath, run.exitCode, run.output),
				),
			)
			return fmt.Errorf(
				"run memory example %s: exit=%d, want %d output=%s",
				sourcePath,
				run.exitCode,
...
func (r *smokeRunner) runFuzzApp(ctx context.Context) error {
	...
	r.cases = append(
		r.cases,
		memoryprod.CaseReport{
			Name: "deterministic memcpy/memset fuzz",
			Kind: "stress",
			Ran:  true,
			Pass: true,
		},
		...
		if build.err != nil {
			r.cases = append(
				...
				failedCase(
					...
					"negative",
					tc.expectedError,
					fmt.Sprintf("build failed: %s", build.output),
				),
			)
		...
		if !pass {
			r.cases = append(
				...
				failedCase(
					...
					"negative",
					tc.expectedError,
					fmt.Sprintf("exit=%d output=%s", run.exitCode, run.output),
				),
			)
			return fmt.Errorf(
				"%s exit=%d, want %d and error %q: %s",
				tc.name,
				run.exitCode,
				tc.expectedExit,
				tc.expectedError,
...
func (r *smokeRunner) runAllocationLengthRuntimeCoverage(ctx context.Context) error {
	...
	r.recordProcess(
		"allocation length runtime coverage",
		"benchmark",
		"go test ./compiler/tests/semantics -run AllocationLength -count=1",
		res,
	)
	if res.err != nil || res.exitCode != 0 {
		for _, name := range names {
			r.cases = append(
				r.cases,
				failedCase(name, "negative", "allocation length contract", res.output),
			)
		}
	...
	for _, name := range names {
		...
		r.cases = append(
			...
			memoryprod.CaseReport{
				...
				Ran:           true,
				Pass:          true,
				ExpectedError: allocationLengthExpectedError(name),
			},
		)
	}
	return nil
...
func (r *smokeRunner) runCompilerSafetyCases(ctx context.Context) error {
	tests := []struct {
	...
	}{
		{
			...
			pkg:           "./compiler/internal/backend/x64abi",
			pattern:       "TestSysVAllocBytesEmitsDeterministicMmapFailureGuard",
			expectedError: "allocation failure",
		},
		{
			name:          "cap.mem unsafe boundary",
			pkg:           "./compiler/tests/safety",
			pattern:       "TestEpic06RejectsCapMemOutsideUnsafeBlock",
			expectedError: "only allowed in unsafe blocks",
		},
		{
			name:          "reject use-after-free",
			pkg:           "./compiler/tests/safety/diagnostics/core",
			pattern:       "TestSafetyDiagnosticCodesForOptionalPayloadWholeValueConsumeAndFree",
			expectedError: "use-after-free",
		},
		{
			name:          "reject double-free",
			pkg:           "./compiler",
			pattern:       "TestBuildIslandsDebugDoubleFreeRejectedBySemantics",
			expectedError: "double-free",
		},
		{
			name:          "reject borrow escape",
			pkg:           "./compiler/tests/ownership",
			pattern:       "TestOwnershipRejectsBorrowEscapeViaAliasReturn",
			expectedError: "borrow escape",
		},
		{
			name:          "reject aliasing violation",
			pkg:           "./compiler/tests/ownership",
			pattern:       "TestOwnershipRejectsBorrowInoutAlias",
			expectedError: "alias",
		},
		{
			name:          "callable mutable capture heap escape",
			pkg:           "./compiler/tests/safety/diagnostics/core",
			pattern:       "TestSafetyDiagnosticCodesForKeyFamilies/callable_mutable_capture_heap_escape",
...
func (r *smokeRunner) runMemoryShapeCoverageCases(ctx context.Context) error {
	tests := []struct {
	...
	}{
		{
			...
			pattern: ("^(TestFullCallableEscapedNineCaptureReturnPassesSemanticClassifi" +
				"cation|TestFullCallableStructFieldNineCaptureLowersHandleEnvironment|Tes" +
				"tFullCallableEnumPayloadNineCaptureLowersHandleEnvironment)$"),
			kind: "positive",
		},
		{
			name: "slice struct borrow escape coverage",
			pkg:  "./compiler/tests/ownership",
			pattern: ("^(TestOwnershipRejectsBorrowEscapeViaNestedSliceStruct|TestOwner" +
				"shipRejectsBorrowedSliceStructOwnedConsumeInoutCallEscape|TestOwnershipR" +
				"ejectsBorrowedPtrEscapeViaStructInoutAssignment)$"),
			kind:          "negative",
			expectedError: "borrow escape",
		},
		{
			name:          "function-typed slice aggregate borrow escape coverage",
			pkg:           "./compiler/tests/safety/diagnostics/aliases",
...
func (r *smokeRunner) runSmallHeapAllocationBenchmark(ctx context.Context) error {
	...
	sourcePath, err := r.writeSource(
		"small_heap_benchmark",
		smallHeapBenchmarkSource(allocationCount, bytesPerAllocation),
	)
	if err != nil {
		return err
	}
	...
	r.recordProcess(
		"small heap allocation benchmark build",
		"benchmark",
		r.tetraPath+" build --target linux-x64 --emit-alloc-report",
		build,
	)
	if build.err != nil {
	...
	if report.Summary.BytesReserved <= 0 {
		return fmt.Errorf(
			"small heap allocation benchmark: bytes_reserved = %d, want positive",
	...
	if baselineSyscalls <= measuredSyscalls {
		return fmt.Errorf(
			"small heap allocation benchmark: baseline syscalls %d must exceed measured chunk refills %d",
			baselineSyscalls,
			measuredSyscalls,
		)
	}
...
func (r *smokeRunner) runRawPointerBoundsMetadataReport(ctx context.Context) error {
	...
	r.recordProcess(
		"raw pointer bounds metadata report build",
		"benchmark",
		r.tetraPath+" build --target linux-x64 --emit-alloc-report --emit-memory-report",
		build,
	)
	if build.err != nil {
		r.cases = append(
			r.cases,
			failedCase("raw pointer bounds metadata report", "positive", "", build.output),
		)
	...
	if err != nil {
		r.cases = append(
			r.cases,
			failedCase("raw pointer bounds metadata report", "positive", "", err.Error()),
		)
	...
	r.recordProcess(
		"raw pointer bounds memory report validation",
		"benchmark",
		"go "+strings.Join(validateMemoryArgs, " "),
		validateMemory,
	)
	if validateMemory.err != nil {
		r.cases = append(
			r.cases,
			failedCase("raw pointer bounds metadata report", "positive", "", validateMemory.output),
		)
	...
	if report.Summary.RawPointerBoundsStatuses["allocation_base_metadata"] < 1 {
		err := fmt.Errorf(
			"raw pointer bounds allocation report missing allocation_base_metadata summary: %+v",
			report.Summary.RawPointerBoundsStatuses,
		)
		r.cases = append(
			r.cases,
			failedCase("raw pointer bounds metadata report", "positive", "", err.Error()),
	...
	if report.Summary.RawSlicePolicies["external_unknown"] < 1 {
		err := fmt.Errorf(
			"raw pointer bounds allocation report missing external_unknown raw slice policy: %+v",
			report.Summary.RawSlicePolicies,
		)
		r.cases = append(
			r.cases,
			failedCase("raw pointer bounds metadata report", "positive", "", err.Error()),
	...
	for _, claim := range []string{
		...
		if memoryClaims[claim] < 1 {
			err := fmt.Errorf(
				"raw pointer bounds memory report missing %s claim: %+v",
				claim,
				memoryClaims,
			)
			r.cases = append(
	...
	r.cases = append(
		r.cases,
		memoryprod.CaseReport{
			Name: "raw pointer bounds metadata report",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
...
func (r *smokeRunner) runAllocationLengthContractReport(ctx context.Context) error {
	sourcePath, err := r.writeSource(
		"allocation_length_contract_report",
		allocationLengthContractReportSource,
	)
	if err != nil {
		return err
	}
	...
	r.recordProcess(
		"allocation length contract report build",
		"benchmark",
		r.tetraPath+" build --target linux-x64 --emit-alloc-report --emit-memory-report",
		build,
	)
	if build.err != nil {
		r.cases = append(
			r.cases,
			failedCase("allocation length contract report", "positive", "", build.output),
		)
	...
	validateMemory := runCommand(ctx, 30*time.Second, "go", validateMemoryArgs...)
	r.recordProcess(
		"allocation length memory report validation",
		"benchmark",
		"go "+strings.Join(validateMemoryArgs, " "),
		validateMemory,
	)
	if validateMemory.err != nil {
		r.cases = append(
			r.cases,
			failedCase("allocation length contract report", "positive", "", validateMemory.output),
		)
	...
	if err != nil {
		r.cases = append(
			r.cases,
			failedCase("allocation length contract report", "positive", "", err.Error()),
		)
	...
	r.cases = append(
		r.cases,
		memoryprod.CaseReport{
			Name: "allocation length contract report",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
