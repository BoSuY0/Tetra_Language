...
const (
	...
	morphGateSource          = "examples/surface/morph_core/surface_morph_command_palette.tetra"
...
func main() {
	...
	sameCommit := flag.String(
		"same-commit",
		"",
		"require the summary to validate at this git commit",
	)
	flag.Parse()
	if *summaryPath == "" {
	...
	if err := validateSurfaceMorphGateSummaryWithOptions(
		*summaryPath,
		morphGateSummaryValidationOptions{SameCommit: *sameCommit},
	); err != nil {
...
func validateSurfaceMorphGateSummaryWithOptions(
	path string,
	options morphGateSummaryValidationOptions,
) error {
	...
	issues = append(
		issues,
		requireString("release_scope", report.ReleaseScope, morphGateReleaseScope)...)
	...
	issues = append(
		issues,
		requireString("schema_under_test", report.SchemaUnderTest, morphGateSchemaUnderTest)...)
	issues = append(
		issues,
		requireString(
			"token_graph_contract",
			report.TokenGraphContract,
			"docs/spec/surface/surface_token_graph_contract.json",
		)...)
	issues = append(
		issues,
		requireString(
			"token_graph_validator",
			report.TokenGraphValidator,
			"validate-surface-token-graph",
		)...)
	issues = append(
		issues,
		requireString(
			"recipe_authoring_validator",
			report.RecipeAuthoringValidator,
			"validate-surface-morph-report",
		)...)
	issues = append(
		issues,
		requireString(
			"recipe_expansion_report",
			report.RecipeExpansionReport,
			"headless/surface-headless-morph.json#morph.recipe_expansions",
		)...)
	issues = append(
		issues,
		requireString(
			"dependency_gate",
			report.DependencyGate,
	...
	issues = append(
		issues,
		requireBool("same_commit_validated", report.SameCommitValidated, boolPtr(true))...)
	issues = append(
		issues,
		requireBool("artifact_hashes_validated", report.ArtifactHashesValidated, boolPtr(true))...)
	issues = append(
		issues,
		requireStringSlice(
			"reference_recipe_apps",
			report.ReferenceRecipeApps,
			morphGateReferenceRecipeApps,
		)...)
	issues = append(
		issues,
		requireStringSlice("target_evidence", report.TargetEvidence, []string{"headless"})...)
	issues = append(
		issues,
		requireStringSlice("core_primitives", report.CorePrimitives, []string{"Block"})...)
...
func decodeSurfaceMorphGateSummary(path string) (surfaceMorphGateSummary, error) {
	...
	if err := decoder.Decode(&extra); err != io.EOF {
		if err == nil {
			return surfaceMorphGateSummary{}, fmt.Errorf(
				"decode surface morph gate summary: unexpected trailing JSON value",
			)
		}
		return surfaceMorphGateSummary{}, fmt.Errorf("decode surface morph gate summary: %w", err)
	}
