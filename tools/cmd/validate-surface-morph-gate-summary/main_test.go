...
func TestValidateSurfaceMorphGateSummaryRejectsWrongReferenceApps(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "reference_recipe_apps") {
		t.Fatalf(
			"validateSurfaceMorphGateSummary err = %v, want reference_recipe_apps rejection",
			err,
		)
	}
}
...
func TestValidateSurfaceMorphGateSummaryRejectsMissingReferenceApps(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "reference_recipe_apps") {
		t.Fatalf(
			"validateSurfaceMorphGateSummary err = %v, want reference_recipe_apps missing rejection",
			err,
		)
	}
}
...
func TestValidateSurfaceMorphGateSummaryRejectsSameCommitGitHeadMismatch(t *testing.T) {
	...
	err := validateSurfaceMorphGateSummaryWithOptions(
		path,
		morphGateSummaryValidationOptions{SameCommit: "abc123"},
	)
	if err == nil {
		t.Fatal(
			"validateSurfaceMorphGateSummaryWithOptions returned nil, want same-commit/git_head rejection",
		)
	}
	errText := strings.ToLower(err.Error())
	if !strings.Contains(errText, "same-commit") || !strings.Contains(errText, "git_head") {
		t.Fatalf(
			"validateSurfaceMorphGateSummaryWithOptions err = %v, want same-commit/git_head rejection",
			err,
		)
	}
}
...
func writeMorphGateSummaryFixture(t *testing.T, mutate func(map[string]any)) string {
	...
	summary := map[string]any{
		...
		"command_line": ("bash scripts/release/surface/morph-gate.sh --report-dir " +
		...
		"forbidden_core_primitives": []string{
			"Button",
			"Card",
			"TextField",
			"TextBox",
