...
func testDocLines(lines ...string) string {
	return strings.Join(lines, "\n") + "\n"
}

func appendTestDocLines(base string, lines ...string) string {
	return base + "\n" + testDocLines(lines...)
}

func surfaceScopeSentence() string {
	return ("Surface v1 scope is linux-x64 real-window and wasm32-web browser-canvas. " +
		"macOS Surface, Windows Surface, and wasm32-wasi Surface UI are unsupported targets.")
}

func surfaceClaimTierSentence() string {
	return ("Claim tiers: PROD_STABLE_SCOPED, BETA_TARGET_HOST, EXPERIMENTAL, " +
		"UNSUPPORTED, NONCLAIM.")
}

func surfaceBoundarySentence() string {
	return ("Metadata-only accessibility is not production accessibility. DOM UI and user " +
		"JavaScript app logic are outside the Surface model.")
}

func surfaceGateCommand() string {
	return "bash scripts/release/surface/release-gate.sh"
}

func surfaceProductGateCommand() string {
	return "bash scripts/release/surface/product-gate.sh"
}
...
func TestVerifyReleaseTruthDocsRejectsProductionPersistentObjectMemoryClaim(t *testing.T) {
	...
	body := strings.Join([]string{
		"# Release Notes",
		"",
		("Tetra now ships production object memory backed by persistent " +
			"memory, Todium, memoryfield, WAL, FTS, vacuum, retention, stale memory, " +
			"and false memory gates."),
	}, "\n")
	if err := os.WriteFile(doc, []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestVerifyReleaseTruthDocsAllowsPersistentObjectMemoryNonGoal(t *testing.T) {
	...
	body := strings.Join([]string{
		"# Release Notes",
		"",
		("Persistent/object memory is an explicit non-goal for this " +
			"release: no production object memory, no production persistent memory, " +
			"and no Todium or memoryfield production claim exists until retention/" +
			"WAL/FTS/vacuum/stale/false-memory gates exist."),
	}, "\n")
	if err := os.WriteFile(doc, []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestCurrentReleaseTruthDocPathsCoverCurrentUserAndSpecDocs(t *testing.T) {
	...
	for _, want := range []string{
		...
		"docs/spec/flow/v0_2_scope.md",
		"docs/user/reference/examples_index.md",
		"docs/user/start/getting_started.md",
		"docs/user/start/language_tour.md",
	...
	for _, forbidden := range []string{"docs/plans/", "docs/release-notes/"} {
		if strings.Contains(text, forbidden) {
			t.Fatalf(
				"currentReleaseTruthDocPaths should not include historical %s paths: %v",
				forbidden,
				paths,
			)
		}
...
func TestVerifySurfaceReleaseDocsRejectsFakePromotionClaims(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			name: "macos-current",
			body: testDocLines(
				"macOS Surface is current for Surface v1.",
		...
		},
		{
			...
			body: testDocLines(
				"Windows Surface is release-ready for Surface v1.",
				"Unsupported targets: wasm32-wasi.",
				surfaceGateCommand(),
			),
		...
		},
		{
			...
			body: testDocLines(
				"metadata-only accessibility is production accessibility.",
		...
		},
		{
			...
			body: testDocLines(
				"user JS app logic is allowed in Surface apps.",
				"Unsupported targets: macOS, Windows, wasm32-wasi.",
				surfaceGateCommand(),
			),
			want: "user JS",
...
func TestVerifySurfaceReleaseDocsRequireUnsupportedTargetsAndReleaseGate(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			body: testDocLines(
				"Surface v1 scope is linux-x64 real-window and wasm32-web browser-canvas.",
				surfaceGateCommand(),
			),
		...
		},
		{
			...
			body: testDocLines(
				("Surface v1 scope is linux-x64 real-window and wasm32-web browser-" +
					"canvas. Unsupported targets: macOS, Windows, wasm32-wasi."),
			),
		...
		},
		{
			...
			body: testDocLines(
				surfaceScopeSentence(),
				"",
				surfaceGateCommand(),
				surfaceProductGateCommand(),
			),
		...
		},
		{
			...
			body: testDocLines(
				surfaceScopeSentence()+" "+surfaceClaimTierSentence(),
				"",
				surfaceGateCommand(),
			),
	...
	okDoc := writeSurfaceReleaseDoc(
		t,
		testDocLines(
			strings.Join([]string{
				surfaceScopeSentence(),
				surfaceBoundarySentence(),
				surfaceClaimTierSentence(),
			}, " "),
			"",
			surfaceGateCommand(),
			surfaceProductGateCommand(),
...
func TestVerifySurfaceReleaseDocsRequireP28GovernancePerDocument(t *testing.T) {
	fullDoc := writeSurfaceReleaseDoc(
		t,
		testDocLines(
			surfaceScopeSentence()+" "+surfaceClaimTierSentence(),
			"",
	...
	missingTierDoc := writeSurfaceReleaseDoc(
		t,
		testDocLines(
			surfaceScopeSentence(),
			"",
...
func TestSurfaceDocsOverclaimRejectsTmpEvidenceAsCurrentProof(t *testing.T) {
	doc := writeSurfaceReleaseDoc(
		t,
		testDocLines(
			surfaceScopeSentence()+" "+surfaceBoundarySentence(),
			"",
			(surfaceGateCommand()+
...
func TestSurfaceOverclaimRejectsGPUAndNativeWidgets(t *testing.T) {
	doc := writeSurfaceReleaseDoc(
		...
		testDocLines(
			surfaceScopeSentence()+" "+surfaceBoundarySentence(),
			"",
			("GPU rendering is production-supported for Surface v1. Platform-native "+
				"widgets are release-ready."),
			"",
			surfaceGateCommand(),
		),
	)
...
func TestUnsupportedSurfaceTargetsRejectsCrossPlatformProductionClaim(t *testing.T) {
	doc := writeSurfaceReleaseDoc(
		...
		testDocLines(
			surfaceScopeSentence(),
			"",
			("Surface is a production cross-platform UI runtime across macOS, "+
				"Windows, linux, and wasm32-wasi."),
			"",
			surfaceGateCommand(),
		),
	)
...
func TestSurfaceOverclaimRejectsRichTextScreenReaderDOMReactUserJS(t *testing.T) {
	doc := writeSurfaceReleaseDoc(
		...
		testDocLines(
			surfaceScopeSentence(),
			"",
			("Rich text editing is production-supported. Full screen-reader support "+
				"is release-ready. DOM UI is production-supported. React apps are "+
				"current Surface apps. User JS app logic is allowed in Surface apps."),
			"",
			surfaceGateCommand(),
		),
	)
	...
	if err == nil {
		t.Fatalf(
			"expected Surface docs to reject rich-text/screen-reader/DOM/React/user-JS overclaims",
		)
	}
...
func TestSurfaceBlockSystemRejectsCoreWidgetPrimitiveClaims(t *testing.T) {
	doc := writeSurfaceReleaseDoc(
		...
		testDocLines(
			surfaceScopeSentence(),
			"",
			("Button is a core Surface primitive. TextField is a core Surface "+
				"primitive. Card is a core Surface primitive."),
			"",
			surfaceGateCommand(),
		),
	)
...
func TestVerifyFinalMemoryIslandsSurfaceProductionAuditRejectsMissingCommands(t *testing.T) {
	body := strings.ReplaceAll(
		validFinalMemoryIslandsSurfaceProductionAuditBody(),
		"git status --short",
		"",
	)
...
func TestVerifyFinalMemoryIslandsSurfaceProductionAuditRejectsBroadReadyClaim(t *testing.T) {
	doc := writeFinalMemoryIslandsSurfaceProductionAudit(
		t,
		appendTestDocLines(
			validFinalMemoryIslandsSurfaceProductionAuditBody(),
...
func TestVerifyMemoryIslandsFinalProductionReadinessAuditRejectsMissingSections(
	t *testing.T,
) {
	body := strings.ReplaceAll(
		validMemoryIslandsFinalProductionReadinessAuditBody(),
		"## Command Log",
		"## Commands",
	)
	...
	if err == nil {
		t.Fatalf(
			("expected final Memory/Islands audit to reject missing command " +
				"log/artifact hash/residual risk sections"),
		)
	}
...
func TestVerifyMemoryIslandsFinalProductionReadinessAuditRejectsBroadReadyClaim(t *testing.T) {
	doc := writeMemoryIslandsFinalProductionReadinessAudit(
		t,
...
func TestVerifyMemoryIslandsFinalProductionReadinessAuditAcceptsWrappedCommands(t *testing.T) {
	...
	if !strings.Contains(body, original) {
		t.Fatal("test fixture lost race command")
	}
	body = strings.Replace(body, original, wrapped, 1)
	doc := writeMemoryIslandsFinalProductionReadinessAudit(t, body)
	if err := verifyMemoryIslandsFinalProductionReadinessAudit([]string{doc}); err != nil {
		t.Fatalf("verifyMemoryIslandsFinalProductionReadinessAudit: %v", err)
	}
...
func TestVerifyMemoryIslandsFinalActorBenchmarkHandoffRejectsActorProductionClaimWithoutGate(
	t *testing.T,
) {
	doc := writeMemoryIslandsFinalActorBenchmarkHandoff(
		t,
		appendTestDocLines(
			validMemoryIslandsFinalActorBenchmarkHandoffBody(),
			"The production actor runtime is ready now.",
			"The actor production gate passed.",
		),
	)
	err := verifyMemoryIslandsFinalActorBenchmarkHandoff([]string{doc})
	if err == nil {
		t.Fatalf(
			"expected final actor handoff to reject production actor claim without gate evidence",
		)
	}
...
func TestVerifyActorRuntimeFoundationDocsRejectsBroadActorProductionClaim(t *testing.T) {
	doc := writeActorRuntimeFoundationDoc(
		t,
		validActorRuntimeFoundationDocBody()+".\nThe full production actor runtime is ready now.\n",
	)
...
func TestVerifyActorRuntimeFoundationDocsAllowsProdReadyProvenNonClaim(t *testing.T) {
	...
	if err := verifyActorRuntimeFoundationDocs(
		[]string{doc},
		validActorRuntimeFoundationFeatures(),
	); err != nil {
...
func TestVerifyActorRuntimeFoundationDocsRejectsStaleManifestFeature(t *testing.T) {
	...
	features[0].Stability = ("current Linux-x64 runtime evidence without the final actor " +
		"foundation gate")
...
func TestVerifyRAMContractCompilerDocsRejectsIncompleteDocs(t *testing.T) {
	...
	err := verifyRAMContractCompilerDocs(
		paths,
		[]featureManifest{validVerifyDocsRAMContractFeature()},
	)
	if err == nil {
	...
	paths := writeRAMContractDocsSet(
		...
		appendTestDocLines(
			...
			("go run ./tools/cmd/validate-ram-contract-release "+
				"--report reports/ram-contract-release"),
		),
	)
	err := verifyRAMContractCompilerDocs(
		paths,
