...
type featureManifest = verifydocs.FeatureManifest
type memoryProductionContractDocPaths = verifydocs.MemoryProductionContractDocPaths
type networkingRuntimeBoundaryDocPaths = verifydocs.NetworkingRuntimeBoundaryDocPaths
type ramContractCompilerDocPaths = verifydocs.RAMContractCompilerDocPaths

var (
	verifyDoctestBlocks                              = verifydocs.Testing.VerifyDoctestBlocks
	verifyRequiredDoctestBlocks                      = verifydocs.Testing.VerifyRequiredDoctestBlocks
	verifySpecCodeBlocks                             = verifydocs.Testing.VerifySpecCodeBlocks
	verifyWASMBackendPlan                            = verifydocs.Testing.VerifyWASMBackendPlan
	verifyMemoryProductionContractDocs               = verifydocs.Testing.VerifyMemoryProductionContractDocs
	memoryProductionContractRequirements             = verifydocs.Testing.MemoryProductionContractRequirements
	forbiddenPublicPerformanceClaims                 = verifydocs.Testing.ForbiddenPublicPerformanceClaims
	verifyNetworkingRuntimeBoundaryDocs              = verifydocs.Testing.VerifyNetworkingRuntimeBoundaryDocs
	networkingRuntimeBoundaryRequirements            = verifydocs.Testing.NetworkingRuntimeBoundaryRequirements
	verifyFeatureRegistry                            = verifydocs.Testing.VerifyFeatureRegistry
	verifySurfaceBlockSystemFeatureBoundary          = verifydocs.Testing.VerifySurfaceBlockSystemFeatureBoundary
	verifyReleaseTruthDocs                           = verifydocs.Testing.VerifyReleaseTruthDocs
	currentReleaseTruthDocPaths                      = verifydocs.Testing.CurrentReleaseTruthDocPaths
	verifySurfaceReleaseDocs                         = verifydocs.Testing.VerifySurfaceReleaseDocs
	verifyMemoryIslandsSurfaceReleaseDocs            = verifydocs.Testing.VerifyMemoryIslandsSurfaceReleaseDocs
	verifyFinalMemoryIslandsSurfaceProductionAudit   = verifydocs.Testing.VerifyFinalMemoryIslandsSurfaceProductionAudit
	verifyMemoryIslandsFinalProductionReadinessAudit = verifydocs.Testing.VerifyMemoryIslandsFinalProductionReadinessAudit
	verifyMemoryIslandsFinalActorBenchmarkHandoff    = verifydocs.Testing.VerifyMemoryIslandsFinalActorBenchmarkHandoff
	verifyActorRuntimeFoundationDocs                 = verifydocs.Testing.VerifyActorRuntimeFoundationDocs
	defaultActorRuntimeFoundationDocPaths            = verifydocs.Testing.DefaultActorRuntimeFoundationDocPaths
	verifyRAMContractCompilerDocs                    = verifydocs.Testing.VerifyRAMContractCompilerDocs
	currentGitHeadForDocs                            = verifydocs.Testing.CurrentGitHeadForDocs
	currentGitParentForDocs                          = verifydocs.Testing.CurrentGitParentForDocs
	extractTetraDoctests                             = verifydocs.Testing.ExtractTetraDoctests
	verifyStableModuleDoctestCoverage                = verifydocs.Testing.VerifyStableModuleDoctestCoverage
	verifyStableModuleEffectsMetadata                = verifydocs.Testing.VerifyStableModuleEffectsMetadata
	verifyStdlibGuide                                = verifydocs.Testing.VerifyStdlibGuide
	verifyExperimentalModuleMirrors                  = verifydocs.Testing.VerifyExperimentalModuleMirrors
	verifyStableModuleExamples                       = verifydocs.Testing.VerifyStableModuleExamples
	verifyStdlibModulePaths                          = verifydocs.Testing.VerifyStdlibModulePaths
	verifyStableExamplesDoNotImportExperimental      = verifydocs.Testing.VerifyStableExamplesDoNotImportExperimental
	verifyEpic14ExampleIndex                         = verifydocs.Testing.VerifyEpic14ExampleIndex
)
...
func TestVerifyMemoryProductionContractDocsRejectsIncompleteContract(t *testing.T) {
	...
	for _, path := range []string{
		...
		paths.StdlibGuide,
		paths.CoreMemory,
		paths.TargetCapabilityMatrix,
		paths.MemoryCostModel,
		paths.MemoryFuzzOracle,
		paths.MemoryProductionFinal,
		paths.MemoryProductionMap,
		paths.MemoryProductionClaims,
	} {
...
func TestVerifyMemoryProductionContractDocsRejectsFastestBenchmarkClaim(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "fastest language") ||
		!strings.Contains(err.Error(), "official benchmark") {
...
func TestForbiddenPublicPerformanceClaimsAllowsWrappedNonClaimSentence(t *testing.T) {
	cases := []string{
		...
		("This does not promote a full source-level PostgreSQL driver " +
			"API or measured speed comparison. It makes no official " +
			"result claim for TechEmpower and no P20 performance matrix " +
			"claim."),
		("It does not promote a production HTTP server or " +
			"source-level cached-date API. It makes no official result " +
			"claim for TechEmpower and no P20 performance matrix claim."),
		("The final audit records no official benchmark result and no " +
			"target parity evidence from quick output."),
		("It is not a runtime measurement, C++/Rust parity claim, and " +
			"makes no official benchmark result claim."),
		("It is not a full source-level PostgreSQL driver API or " +
			"external production database deployment. It makes no " +
			"official result claim for TechEmpower, no production " +
			"database benchmark claim, and no measured speed comparison " +
			"claim."),
		("`MEM-FUZZ-012` makes no arbitrary unsafe safety claim, no " +
			"full runtime/ABI/target parity proof, and no Memory 100% " +
			"claim."),
...
func TestForbiddenPublicPerformanceClaimsRejectsClaimAfterUnrelatedDoesNot(t *testing.T) {
	...
	if len(claims) == 0 || !strings.Contains(strings.Join(claims, ","), "fastest language") ||
		!strings.Contains(strings.Join(claims, ","), "official benchmark") {
		t.Fatalf(
			"forbiddenPublicPerformanceClaims() = %#v, want fastest/official rejection",
			claims,
		)
	}
}
...
func TestForbiddenPublicPerformanceClaimsAllowsIslandKernelNonClaims(t *testing.T) {
	text := strings.Join([]string{
		("IslandKernel is not complete and remains model-only until " +
			"validate-island-proof evidence exists."),
...
func TestVerifyNetworkingRuntimeBoundaryDocsRejectsIncompleteBoundary(t *testing.T) {
	...
	for _, path := range []string{
		paths.CurrentSurface,
		paths.Stdlib,
		paths.StdlibGuide,
		paths.CoreNet,
		paths.CoreNetworking,
	} {
...
func TestVerifyFeatureRegistryAcceptsRequiredStatuses(t *testing.T) {
	features := []featureManifest{
		...
		},
		{
			...
			Status:    "current",
			Since:     "v0.4.0",
			Scope:     "production UI metadata contract with deterministic tetra.ui.v1 JSON",
		...
		},
		{
			...
			Status: "current",
			Since:  "v0.4.0",
			Scope: ("production Linux-x64 native UI runtime path with native " +
				"runtime widget instances, click/activate events, and state " +
				"and widget updates"),
			Stability: ("tetra.ui.native-runtime.v1 smoke evidence rejects " +
				"metadata-only, web-only, native-shell sidecar-only evidence;" +
				" macOS/Windows remain outside this claim"),
			Docs: []string{
				"docs/spec/core/current_supported_surface.md",
				"docs/spec/ui/ui_v1.md",
				"docs/user/surface/wasm_ui_guide.md",
		...
		},
		{
			...
			Name:   "Cross-platform UI runtime gate",
			Status: "experimental",
			Since:  "v0.4.0",
			Scope: ("tetra.ui.platform-runtime.v1 full-platform UI runtime " +
				"promotion gate requiring real Windows/macOS target-host " +
				"reports"),
			Stability: ("not production until the gate rejects metadata-only, " +
				"runtime-less, startup_failure evidence and accepts " +
				"target-host reports"),
			Docs: []string{
				"docs/spec/core/current_supported_surface.md",
				"docs/spec/ui/ui_v1.md",
		...
		},
		{
			...
			Scope: ("production Linux-x64 distributed actor runtime path with " +
				...
				"remote actor handles, network mailbox send/receive, and " +
				"scoped actor runtime foundation gate evidence through " +
			...
			Stability: ("current Linux-x64 runtime/lowering slice with executable " +
				...
				"nonclaims for non-Linux distributed runtime, distributed " +
				"zero-copy, cluster membership, reconnect/retry production, " +
				"and formal race proof"),
			Docs: []string{
				"docs/spec/core/current_supported_surface.md",
		...
		},
		{
			...
			Docs:      []string{"docs/spec/surface/surface_v1.md"},
		},
		{
			...
			Scope: ("Block-first Surface architecture with Block as the core " +
				"Surface primitive, widgets as recipes/compatibility, " +
				"`tetra.surface.block-system.gate.v1` gate reports, and " +
				"`block_system.memory_budget` evidence under " +
				"reports/surface-block/p18-budget"),
			Stability: ("experimental implementation track with same-commit target " +
				"evidence for headless, linux-x64 real-window, and " +
				"wasm32-web browser-canvas; not production support and no " +
				"production Block claim"),
			Docs: []string{
				"docs/spec/core/current_supported_surface.md",
		...
		{
			ID:        "ui.metadata-legacy",
			Name:      "UI metadata legacy compatibility",
			Status:    "legacy_compatibility",
			Scope:     "legacy metadata compatibility",
			Stability: "compatibility bridge",
			Docs:      []string{"docs/spec/ui/ui_v1.md"},
		},
	}
...
func validVerifyDocsSafetyProductionCoreFeature() featureManifest {
	return featureManifest{
		...
		Scope: ("production local safety model for " +
			"ownership/lifetime/borrow/consume/inout checks, Memory " +
			"Production Core v1 report evidence, memory production final " +
			"audit with artifact map and explicit nonclaims, " +
			"validate-island-proof independent-ish verifier evidence, " +
			"--islands-debug sanitizer smoke, island-proof-fuzz-summary " +
			"deterministic mutation evidence, leak/resource finalization " +
			"evidence, integrated Memory/Islands/Surface release gate " +
			"with memory-islands-surface-production-manifest.json and " +
