...
func TestValidateReportRejectsZeroHeapSuiteRuntimeHeapRegression(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "zero-heap") ||
		!strings.Contains(err.Error(), "heap_allocations") {
		t.Fatalf(
			"ValidateReportBytes heap regression = %v, want zero-heap heap_allocations rejection",
			err,
		)
	}
}
...
func validZeroHeapReport(t *testing.T, dir string) map[string]any {
	...
	for _, category := range zeroheapbench.Categories {
		...
		source := fixture(
			t,
			dir,
			filepath.Join("artifacts", "src", "zero-heap", zeroheapbench.Slug(category)+".tetra"),
			"source",
		)
		...
		alloc := fixture(
			t,
			dir,
			filepath.Join("artifacts", "bin", name+".alloc.json"),
			`{"totals":{"heap":0},"summary":{"bytes_requested":128,"bytes_reserved":128,"domains":[]}}`,
		)
		result := map[string]any{
			...
			"raw_output_artifacts": []string{
				fixture(t, dir, filepath.Join("artifacts", "raw", name+".stdout.txt"), "stdout"),
			},
			"tetra_metadata": map[string]any{
				"proof_report": fixture(
					t,
					dir,
					filepath.Join("artifacts", "bin", name+".proof.json"),
					`{"kind":"proof"}`,
				),
				"bounds_report": fixture(
					t,
					dir,
					filepath.Join("artifacts", "bin", name+".bounds.json"),
					`{"totals":{"left":0}}`,
				),
				...
				"perf_blocker_report": fixture(
					t,
					dir,
					filepath.Join("artifacts", "bin", name+".perf.json"),
					`{"benchmarks":[]}`,
				),
				"backend_report": fixture(
					t,
					dir,
					filepath.Join("artifacts", "bin", name+".backend.json"),
					`{"summary":{"register_path":1}}`,
				),
				...
				"memory_evidence": map[string]any{
					...
					"bytes_committed": unsupportedMetricFixture(
						"allocation report does not expose committed bytes",
					),
					"bytes_copied": allocationMetricFixture(0, alloc),
					"rss_current": unsupportedMetricFixture(
						"zero-heap suite does not measure process RSS",
					),
					"rss_peak": unsupportedMetricFixture(
						"zero-heap suite does not measure process RSS",
					),
					"domain_bytes_evidence": unsupportedMetricFixture(
...
func heapSidecarFixture(
	...
	return fixture(
		t,
		dir,
		filepath.Join("artifacts", "heap-telemetry", name, "iteration-01.heap.json"),
		string(raw),
	)
}
