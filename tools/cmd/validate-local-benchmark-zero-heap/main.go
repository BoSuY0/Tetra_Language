...
func main() {
	...
	if *reportPath == "" {
		fmt.Fprintln(
			os.Stderr,
			("usage: validate-local-benchmark-zero-heap --report reports/" +
				"local-benchmark-zero-heap-v1/report.json"),
		)
		os.Exit(2)
	}
...
func validateReport(parsed report, root string) error {
	...
	if len(parsed.Results) != len(zeroheapbench.Categories) {
		return fmt.Errorf(
			"report has %d zero-heap rows, want %d",
			len(parsed.Results),
...
func validateRow(result row, root string) error {
	...
	if strings.TrimSpace(result.AlgorithmID) == "" ||
		strings.TrimSpace(result.InputDescription) == "" {
	...
	if result.BinarySizeBytes <= 0 {
		return fmt.Errorf(
			"%s binary_size_bytes = %d, want positive",
			prefix,
			result.BinarySizeBytes,
		)
	}
	if result.CompileTimeMS < 0 || result.MedianRuntimeMS <= 0 ||
		len(result.RunMeasurementsMS) == 0 {
	...
	if result.TetraMetadata.HeapAllocations != 0 {
		return fmt.Errorf(
			"%s zero-heap tetra_metadata.heap_allocations = %d, want 0",
			prefix,
			result.TetraMetadata.HeapAllocations,
		)
	}
	if result.TetraMetadata.BoundsLeft < 0 {
		return fmt.Errorf(
			"%s bounds_left = %d, want non-negative",
			prefix,
			result.TetraMetadata.BoundsLeft,
		)
	}
...
func validateMemoryEvidence(
	...
	if evidence.Schema != schemaBenchmarkMemoryV1 {
		return fmt.Errorf(
			"%s memory evidence schema = %q, want %q",
			prefix,
			evidence.Schema,
...
func validateZeroHeapMetric(prefix string, name string, metric memoryMetric, root string) error {
	...
	if metric.EvidenceClass != "runtime_measured" {
		return fmt.Errorf(
			"%s evidence_class = %q, want runtime_measured",
			metricPrefix,
	...
	if metric.Bytes != 0 || metric.CurrentBytes != 0 || metric.PeakBytes != 0 ||
		metric.TotalAllocBytes != 0 ||
		metric.AllocationCount != 0 {
	...
	if sample.HeapCurrentBytes != 0 || sample.HeapPeakBytes != 0 ||
		sample.HeapTotalAllocBytes != 0 ||
		sample.HeapAllocationCount != 0 {
