...
func TestValidateReportRejectsMissingTetraMemoryEvidence(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "memory evidence") {
		t.Fatalf("ValidateReportBytes missing memory evidence = %v, want memory evidence rejection", err)
	}
}
...
func TestValidateReportRejectsMissingRuntimeFeatureEvidence(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "runtime feature evidence") {
		t.Fatalf("ValidateReportBytes missing runtime feature evidence = %v, want runtime feature evidence rejection", err)
	}
}
...
func TestValidateReportRejectsRuntimeFeatureMetadataMismatch(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "runtime_features_required") {
		t.Fatalf("ValidateReportBytes runtime feature mismatch = %v, want runtime_features_required rejection", err)
	}
}
...
func TestValidateReportRejectsMissingRuntimeObjectPlan(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "runtime_object_plan") {
		t.Fatalf("ValidateReportBytes missing runtime object plan = %v, want runtime_object_plan rejection", err)
	}
}
...
func TestValidateReportRejectsRuntimeObjectPlanMismatch(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "runtime_object_linked") {
		t.Fatalf("ValidateReportBytes runtime object plan mismatch = %v, want runtime_object_linked rejection", err)
	}
}
...
func TestValidateReportRejectsHeapAllocationWithoutReasonCodes(t *testing.T) {
  ...
  "functions": [{
	...
	if err == nil || !strings.Contains(err.Error(), "heap reason") {
		t.Fatalf("ValidateReportBytes heap allocation without reason codes = %v, want heap reason rejection", err)
	}
}
...
func TestValidateReportRejectsHeapReasonMetadataMismatch(t *testing.T) {
  ...
  "functions": [{
	...
	if err == nil || !strings.Contains(err.Error(), "metadata.heap_reason_codes") {
		t.Fatalf("ValidateReportBytes heap reason metadata mismatch = %v, want metadata heap reason rejection", err)
	}
}
...
func TestValidateReportRejectsAllocationWithoutMemoryBackendEvidence(t *testing.T) {
  ...
  "functions": [{
	...
	if err == nil || !strings.Contains(err.Error(), "memory_backend") {
		t.Fatalf("ValidateReportBytes allocation without memory backend evidence = %v, want memory_backend rejection", err)
	}
}
...
func TestValidateReportRejectsRuntimeRSSPeakWithoutSidecarArtifact(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "rss_peak") {
		t.Fatalf("ValidateReportBytes RSS peak without sidecar artifact = %v, want rss_peak rejection", err)
	}
}
...
func TestValidateReportRejectsFakeRuntimeRSSMethods(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "current heap method", metric: "rss_current", method: heaptelemetry.MethodLinuxX64HeapTelemetryV1},
		...
		{name: "peak heap method", metric: "rss_peak", method: heaptelemetry.MethodLinuxX64HeapTelemetryV1},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil || !strings.Contains(err.Error(), tc.metric) {
				t.Fatalf("ValidateReportBytes fake RSS method %q = %v, want %s rejection", tc.method, err, tc.metric)
			}
		})
	}
}
...
func TestValidateReportRejectsRuntimeRSSCurrentWithoutLiveSample(t *testing.T) {
	...
	err = ValidateReportBytes(raw, dir)
	if err == nil || !strings.Contains(err.Error(), "rss_current") {
		t.Fatalf("ValidateReportBytes RSS current without live sample = %v, want rss_current rejection", err)
	}
}
...
func TestValidateReportRejectsRSSPeakOverLocalBudget(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "rss budget") || !strings.Contains(err.Error(), "integer loops") || !strings.Contains(err.Error(), "rss_peak") {
		t.Fatalf("ValidateReportBytesWithRSSBudgetPolicy over budget = %v, want rss budget rss_peak rejection", err)
	}
}
...
func TestValidateReportBlocksRSSBudgetWhenHostProfileDiffers(t *testing.T) {
	...
	if err := ValidateReportBytesWithRSSBudgetPolicy(raw, dir, policy); err != nil {
		t.Fatalf("ValidateReportBytesWithRSSBudgetPolicy host mismatch should block without failing: %v", err)
	}
}
...
func TestValidateReportRejectsRSSBudgetPolicyWithoutLocalNonClaim(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "non_claim") {
		t.Fatalf("ValidateReportBytesWithRSSBudgetPolicy missing local nonclaim = %v, want non_claim rejection", err)
	}
}
...
func TestValidateReportRejectsRuntimeHeapEvidenceWithoutSidecarArtifact(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "heap_alloc_bytes") {
		t.Fatalf("ValidateReportBytes heap without sidecar artifact = %v, want heap_alloc_bytes rejection", err)
	}
}
...
func TestValidateReportRejectsRuntimeHeapEvidenceMissingSidecarFile(t *testing.T) {
	...
	err = ValidateReportBytes(raw, dir)
	if err == nil || !strings.Contains(err.Error(), "heap_alloc_bytes") {
		t.Fatalf("ValidateReportBytes missing heap sidecar = %v, want heap_alloc_bytes rejection", err)
	}
}
...
func TestValidateReportRejectsFakeRuntimeHeapMethods(t *testing.T) {
	for _, method := range []string{"MemStats", "allocation_report_summary", "linux_proc_status"} {
		t.Run(method, func(t *testing.T) {
			...
			err = ValidateReportBytes(raw, dir)
			if err == nil || !strings.Contains(err.Error(), "heap_alloc_bytes") {
				t.Fatalf("ValidateReportBytes fake heap method %q = %v, want heap_alloc_bytes rejection", method, err)
			}
		})
	}
}
...
func TestValidateReportRejectsMeasuredTetraRowWithUnsupportedRuntimeHeapEvidence(t *testing.T) {
	...
	err = ValidateReportBytes(raw, dir)
	if err == nil || !strings.Contains(err.Error(), "heap_alloc_bytes") {
		t.Fatalf("ValidateReportBytes measured unsupported heap = %v, want heap_alloc_bytes rejection", err)
	}
}
...
func TestValidateReportRejectsZeroHeapRequiredRowWithHeapAllocationCount(t *testing.T) {
	...
	err = ValidateReportBytes(raw, dir)
	if err == nil || !strings.Contains(err.Error(), "zero-heap-required") || !strings.Contains(err.Error(), "allocation_count") {
		t.Fatalf("ValidateReportBytes zero-heap allocation count = %v, want zero-heap-required allocation_count rejection", err)
	}
}
...
func TestValidateReportRejectsZeroHeapRequiredRowWithHeapTotalAllocBytes(t *testing.T) {
	...
	err = ValidateReportBytes(raw, dir)
	if err == nil || !strings.Contains(err.Error(), "zero-heap-required") || !strings.Contains(err.Error(), "total_alloc_bytes") {
		t.Fatalf("ValidateReportBytes zero-heap total alloc bytes = %v, want zero-heap-required total_alloc_bytes rejection", err)
	}
}
...
func TestValidateReportRejectsZeroHeapRequiredRowWithMetadataHeapAllocations(t *testing.T) {
	...
	err = ValidateReportBytes(raw, dir)
	if err == nil || !strings.Contains(err.Error(), "zero-heap-required") || !strings.Contains(err.Error(), "heap_allocations") {
		t.Fatalf("ValidateReportBytes zero-heap metadata heap allocations = %v, want zero-heap-required heap_allocations rejection", err)
	}
}
...
func TestValidateReportRejectsZeroHeapRequiredRowWithUnsupportedHeapEvidence(t *testing.T) {
	...
	err = ValidateReportBytes(raw, dir)
	if err == nil || !strings.Contains(err.Error(), "heap_alloc_bytes") {
		t.Fatalf("ValidateReportBytes zero-heap unsupported heap = %v, want heap_alloc_bytes rejection", err)
	}
}
...
func TestValidateReportRejectsZeroHeapRequiredRowWithBlockedHeapEvidence(t *testing.T) {
	...
	err = ValidateReportBytes(raw, dir)
	if err == nil || !strings.Contains(err.Error(), "heap_alloc_bytes") {
		t.Fatalf("ValidateReportBytes zero-heap blocked heap = %v, want heap_alloc_bytes rejection", err)
	}
}
...
func TestValidateReportAcceptsBuildFailedTetraRowWithMissingBuildArtifacts(t *testing.T) {
	...
	metadata["runtime_object_plan"] = map[string]any{
		...
		"blocked_reason":                      "Tetra build failed before runtime object plan artifacts were produced",
	}
	metadata["memory_evidence"] = blockedMemoryEvidenceFixture("Tetra build failed before memory artifacts were produced")
...
func TestValidateReportRejectsStaleGitCommitWhenRootIsGitWorktree(t *testing.T) {
	dir := t.TempDir()
	runGit(t, dir, "init")
	runGit(t, dir, "-c", "user.name=Tetra Test", "-c", "user.email=tetra@example.invalid", "commit", "--allow-empty", "-m", "init")
	report := validTier1Report(t, dir)
	...
	for _, category := range requiredP20Categories {
		...
		for _, language := range requiredLanguages {
			...
			if language == "tetra" {
				...
				heapSidecar := heapSidecarFixtureWithTotals(t, dir, name, heapCurrent, heapPeak, heapTotal, heapCount)
				backendReport := fixture(t, dir, "artifacts/"+name+".backend.json", `{"kind":"backend","summary":{"register_path":1,"stack_fallback":0,"runtime_features_required":[],"runtime_features_linked":[],"runtime_features_initialized":[],"runtime_lazy_init_blockers":[],"runtime_feature_evidence_class":"lowered_ir_static_plan","runtime_feature_evidence_method":"backend_report_lowered_ir_scan_v1","runtime_object_plan":{"evidence_class":"native_runtime_object_plan","evidence_method":"native_link_runtime_object_plan_v1","runtime_used":false,"runtime_object_linked":false,"runtime_object_initialized":false,"runtime_object_features_required":[],"runtime_object_features_linked":[],"runtime_object_features_initialized":[],"runtime_object_lazy_init_blockers":[]}}}`)
				allocationReport := fixture(t, dir, "artifacts/"+name+".alloc.json", `{"kind":"allocation_plan","totals":{"heap":0},"summary":{"bytes_requested":128,"bytes_reserved":128,"bytes_committed":128,"bytes_released":128,"heap_reason_codes":{},"memory_backend_classes":{},"memory_backend_operations":{},"domains":[{"domain_id":"domain:process","kind":"process","requested_bytes":128,"reserved_bytes":128,"committed_bytes":128,"released_bytes":128}]},"functions":[]}`)
...
func memoryEvidenceFixture(name string, allocationReport string, heapSidecar string, rssSidecar string, heapCurrent uint64, heapPeak uint64, heapTotal uint64, heapCount uint64) map[string]any {
	return map[string]any{
...
func heapSidecarFixtureWithTotals(t *testing.T, dir string, name string, current uint64, peak uint64, total uint64, count uint64) string {
	t.Helper()
...
func setRuntimeHeapEvidence(t *testing.T, dir string, row map[string]any, current uint64, peak uint64, total uint64, count uint64) {
	t.Helper()
...
func localRSSBudgetPolicyFixture(peakBudget uint64, variancePercent float64, hostMismatch bool, includeLocalNonClaims bool) []byte {
	host := map[string]any{
		"goos":       "linux",
		"goarch":     "amd64",
		"cpus":       8,
