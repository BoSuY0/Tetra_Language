...
func validateRuntimeFeatureEvidence(name string, rowStatus string, metadata tetraMetadata, root string) error {
	...
	if rowStatus != "measured" {
		if metadata.RuntimeFeatureEvidence.EvidenceClass != "blocked" {
			return fmt.Errorf("%s for %s row must be blocked, got %q", prefix, rowStatus, metadata.RuntimeFeatureEvidence.EvidenceClass)
		}
		if metadata.RuntimeFeatureEvidence.Method != "missing_build_artifacts" {
			return fmt.Errorf("%s blocked method = %q, want missing_build_artifacts", prefix, metadata.RuntimeFeatureEvidence.Method)
		}
		...
		if len(metadata.RuntimeFeaturesRequired) != 0 || len(metadata.RuntimeFeaturesLinked) != 0 || len(metadata.RuntimeFeaturesInitialized) != 0 {
			return fmt.Errorf("%s for %s row must not claim runtime feature arrays", prefix, rowStatus)
		}
		if metadata.RuntimeObjectPlan.EvidenceClass != "blocked" {
			return fmt.Errorf("%s runtime_object_plan for %s row must be blocked, got %q", prefix, rowStatus, metadata.RuntimeObjectPlan.EvidenceClass)
		}
		if metadata.RuntimeObjectPlan.EvidenceMethod != "missing_build_artifacts" {
			return fmt.Errorf("%s runtime_object_plan blocked method = %q, want missing_build_artifacts", prefix, metadata.RuntimeObjectPlan.EvidenceMethod)
		}
		if strings.TrimSpace(metadata.RuntimeObjectPlan.BlockedReason) == "" {
			return fmt.Errorf("%s runtime_object_plan blocked evidence requires blocked_reason", prefix)
		}
	...
	if evidence.EvidenceClass != runtimeFeatureEvidenceClass {
		return fmt.Errorf("%s evidence_class = %q, want %q", prefix, evidence.EvidenceClass, runtimeFeatureEvidenceClass)
	}
	if evidence.Method != runtimeFeatureEvidenceMethod {
		return fmt.Errorf("%s method = %q, want %q", prefix, evidence.Method, runtimeFeatureEvidenceMethod)
	}
	...
	if evidence.SourceArtifact != metadata.BackendReport {
		return fmt.Errorf("%s source_artifact = %q, want backend_report %q", prefix, evidence.SourceArtifact, metadata.BackendReport)
	}
	...
	if summary.EvidenceClass != runtimeFeatureEvidenceClass {
		return fmt.Errorf("%s backend report evidence_class = %q, want %q", prefix, summary.EvidenceClass, runtimeFeatureEvidenceClass)
	}
	if summary.EvidenceMethod != runtimeFeatureEvidenceMethod {
		return fmt.Errorf("%s backend report evidence_method = %q, want %q", prefix, summary.EvidenceMethod, runtimeFeatureEvidenceMethod)
	}
	if err := validateRuntimeObjectPlanEvidence(prefix, metadata.RuntimeObjectPlan, summary.ObjectPlan); err != nil {
		return err
	}
	...
	if containsString(metadata.RuntimeFeaturesLinked, "unknown_runtime") || containsString(metadata.RuntimeFeaturesInitialized, "unknown_runtime") {
		return fmt.Errorf("%s unknown_runtime must not be linked or initialized", prefix)
	}
	if containsString(metadata.RuntimeFeaturesRequired, "unknown_runtime") && !hasRuntimeBlockerPrefix(metadata.RuntimeLazyInitBlockers, "unknown_runtime_call:") {
...
func validateHeapReasonEvidence(name string, rowStatus string, metadata tetraMetadata, root string) error {
	...
	for _, fn := range report.Functions {
		for _, alloc := range fn.Allocations {
			...
			usesHeap := allocationReportRowUsesHeap(alloc.Storage, alloc.PlannedStorage, alloc.ActualLoweringStorage, alloc.RuntimePath)
			if !usesHeap {
				if len(alloc.HeapReasonCodes) != 0 {
					return fmt.Errorf("%s allocation %s has heap_reason_codes without heap storage", prefix, label)
				}
				continue
			}
			if len(alloc.HeapReasonCodes) == 0 {
				return fmt.Errorf("%s allocation %s heap allocation requires heap_reason_codes", prefix, label)
			}
			codes, err := validateHeapReasonCodeSlice(prefix+" allocation "+label, alloc.HeapReasonCodes)
			if err != nil {
				return err
			}
			reasonCodes, err := validateReasonCodeSlice(prefix+" allocation "+label+" reason_codes", alloc.ReasonCodes)
			if err != nil {
				return err
			}
			for _, code := range codes {
				if !containsString(reasonCodes, code) {
					return fmt.Errorf("%s allocation %s heap reason code %q missing from reason_codes", prefix, label, code)
				}
				observed[code]++
			}
		}
	...
	if !intMapsEqual(observed, summary) {
		return fmt.Errorf("%s summary.heap_reason_codes = %v, want observed heap allocation codes %v", prefix, summary, observed)
	}
	metadataCodes, err := validateHeapReasonCodeSlice(prefix+" metadata.heap_reason_codes", metadata.HeapReasonCodes)
	if err != nil {
		return err
	}
	expectedMetadataCodes := sortedMapKeys(summary)
	if !stringSlicesEqual(metadataCodes, expectedMetadataCodes) {
		return fmt.Errorf("%s metadata.heap_reason_codes = %v, want allocation report summary keys %v", prefix, metadataCodes, expectedMetadataCodes)
	}
	return nil
}

func validateAllocationMemoryBackendEvidence(name string, rowStatus string, metadata tetraMetadata, root string) error {
	...
	for _, fn := range report.Functions {
		for _, alloc := range fn.Allocations {
			...
			if err := validateAllocationMemoryBackendRow(prefix+" allocation "+label, alloc.RuntimePath, alloc.BytesCommitted, alloc.BytesReleased, *alloc.MemoryBackend); err != nil {
				return err
			}
	...
	if !intMapsEqual(observedClasses, normalizedIntSummary(report.Summary.MemoryBackendClasses)) {
		return fmt.Errorf("%s summary.memory_backend_classes = %v, want observed %v", prefix, normalizedIntSummary(report.Summary.MemoryBackendClasses), observedClasses)
	}
	if !intMapsEqual(observedOps, normalizedIntSummary(report.Summary.MemoryBackendOperations)) {
		return fmt.Errorf("%s summary.memory_backend_operations = %v, want observed %v", prefix, normalizedIntSummary(report.Summary.MemoryBackendOperations), observedOps)
	}
	if !intMapsEqual(observedEvidenceClasses, normalizedIntSummary(report.Summary.MemoryBackendEvidenceClasses)) {
		return fmt.Errorf("%s summary.memory_backend_evidence_classes = %v, want observed %v", prefix, normalizedIntSummary(report.Summary.MemoryBackendEvidenceClasses), observedEvidenceClasses)
	}
	if metadata.MemoryEvidence != nil {
		if metadata.MemoryEvidence.BytesCommitted.EvidenceClass == "allocation_report_estimate" && int64(metadata.MemoryEvidence.BytesCommitted.Bytes) != committed {
			return fmt.Errorf("%s metadata memory_evidence.bytes_committed = %d, want allocation summary %d", prefix, metadata.MemoryEvidence.BytesCommitted.Bytes, committed)
		}
		if metadata.MemoryEvidence.BytesReleased.EvidenceClass == "allocation_report_estimate" && int64(metadata.MemoryEvidence.BytesReleased.Bytes) != released {
			return fmt.Errorf("%s metadata memory_evidence.bytes_released = %d, want allocation summary %d", prefix, metadata.MemoryEvidence.BytesReleased.Bytes, released)
		}
	}
	return nil
}

func validateAllocationMemoryBackendRow(prefix string, runtimePath string, bytesCommitted int64, bytesReleased int64, evidence allocationMemoryBackendEvidence) error {
	if evidence.Schema != "tetra.memory.backend-allocation.v1" {
		return fmt.Errorf("%s schema = %q, want tetra.memory.backend-allocation.v1", prefix, evidence.Schema)
	}
	...
	if evidence.RuntimePath != runtimePath {
		return fmt.Errorf("%s runtime_path = %q, want allocation runtime_path %q", prefix, evidence.RuntimePath, runtimePath)
	}
	...
	if err := validateMemoryBackendClassForRuntimePath(prefix, runtimePath, evidence.BackendClass); err != nil {
		return err
	}
	...
	case "runtime_measured", "allocation_report_estimate":
		if evidence.UnsupportedReason != "" || evidence.BlockedReason != "" {
			return fmt.Errorf("%s measured/estimated memory_backend must not include unsupported or blocked reason", prefix)
		}
		...
		if bytesCommitted != evidence.CommitBytes {
			return fmt.Errorf("%s bytes_committed = %d, want memory_backend commit_bytes %d", prefix, bytesCommitted, evidence.CommitBytes)
		}
		if bytesReleased != evidence.ReleaseBytes {
			return fmt.Errorf("%s bytes_released = %d, want memory_backend release_bytes %d", prefix, bytesReleased, evidence.ReleaseBytes)
		}
	case "unsupported":
		...
		if evidence.BlockedReason != "" {
			return fmt.Errorf("%s unsupported memory_backend must not include blocked_reason", prefix)
		}
		if evidence.ReserveBytes != 0 || evidence.CommitBytes != 0 || evidence.DecommitBytes != 0 || evidence.ReleaseBytes != 0 ||
			evidence.FootprintCurrentBytes != 0 || evidence.FootprintPeakBytes != 0 {
	...
	case "blocked":
		...
		if evidence.UnsupportedReason != "" {
			return fmt.Errorf("%s blocked memory_backend must not include unsupported_reason", prefix)
		}
		if evidence.ReserveBytes != 0 || evidence.CommitBytes != 0 || evidence.DecommitBytes != 0 || evidence.ReleaseBytes != 0 ||
			evidence.FootprintCurrentBytes != 0 || evidence.FootprintPeakBytes != 0 {
		...
		if bytesCommitted != 0 || bytesReleased != 0 {
			return fmt.Errorf("%s blocked memory_backend must not report committed/released bytes", prefix)
		}
	}
	return nil
}

func validateMemoryBackendClassForRuntimePath(prefix string, runtimePath string, backendClass string) error {
	switch runtimePath {
	case "per_core_small_heap", "small_heap_bump":
		if backendClass != "small_heap" {
			return fmt.Errorf("%s backend_class = %q, want small_heap for %s", prefix, backendClass, runtimePath)
		}
	case "large_mmap":
		if backendClass != "large_backend" {
			return fmt.Errorf("%s backend_class = %q, want large_backend for large_mmap", prefix, backendClass)
		}
	case "explicit_island", "region":
		if backendClass != "region" {
			return fmt.Errorf("%s backend_class = %q, want region for %s", prefix, backendClass, runtimePath)
		}
	case "stack_frame", "eliminated":
		if backendClass != "none" {
			return fmt.Errorf("%s backend_class = %q, want none for %s", prefix, backendClass, runtimePath)
		}
	case "external":
		if backendClass != "external" {
			return fmt.Errorf("%s backend_class = %q, want external for external runtime path", prefix, backendClass)
		}
	case "heap":
		if backendClass != "conservative_heap" {
			return fmt.Errorf("%s backend_class = %q, want conservative_heap for heap runtime path", prefix, backendClass)
		}
	case "unknown_conservative":
		if backendClass != "unknown" {
			return fmt.Errorf("%s backend_class = %q, want unknown for unknown_conservative", prefix, backendClass)
		}
	}
	return nil
}
...
func allocationReportRowUsesHeap(storage string, planned string, actual string, runtimePath string) bool {
...
func readBackendRuntimeFeatureSummary(root string, path string) (backendRuntimeFeatureSummary, error) {
	resolved, err := resolveExistingPath(root, path)
	if err != nil {
...
func validateRuntimeObjectPlanEvidence(prefix string, got runtimeObjectPlan, want runtimeObjectPlan) error {
	if got.EvidenceClass != runtimeObjectPlanEvidenceClass {
		return fmt.Errorf("%s runtime_object_plan evidence_class = %q, want %q", prefix, got.EvidenceClass, runtimeObjectPlanEvidenceClass)
	}
	if got.EvidenceMethod != runtimeObjectPlanEvidenceMethod {
		return fmt.Errorf("%s runtime_object_plan evidence_method = %q, want %q", prefix, got.EvidenceMethod, runtimeObjectPlanEvidenceMethod)
	}
	if want.EvidenceClass != runtimeObjectPlanEvidenceClass {
		return fmt.Errorf("%s backend report runtime_object_plan evidence_class = %q, want %q", prefix, want.EvidenceClass, runtimeObjectPlanEvidenceClass)
	}
	if want.EvidenceMethod != runtimeObjectPlanEvidenceMethod {
		return fmt.Errorf("%s backend report runtime_object_plan evidence_method = %q, want %q", prefix, want.EvidenceMethod, runtimeObjectPlanEvidenceMethod)
	}
	for _, item := range []struct {
		label string
		got   bool
		...
		if item.got != item.want {
			return fmt.Errorf("%s runtime_object_plan %s = %v, want backend report %v", prefix, item.label, item.got, item.want)
		}
	}
	for _, item := range []struct {
		label string
		...
		{"runtime_object_features_required", got.RuntimeObjectFeaturesRequired, want.RuntimeObjectFeaturesRequired},
		{"runtime_object_features_linked", got.RuntimeObjectFeaturesLinked, want.RuntimeObjectFeaturesLinked},
		{"runtime_object_features_initialized", got.RuntimeObjectFeaturesInitialized, want.RuntimeObjectFeaturesInitialized},
		{"runtime_object_lazy_init_blockers", got.RuntimeObjectLazyInitBlockers, want.RuntimeObjectLazyInitBlockers},
	} {
		...
		if !stringSlicesEqual(gotValues, wantValues) {
			return fmt.Errorf("%s runtime_object_plan %s = %v, want backend report %v", prefix, item.label, gotValues, wantValues)
		}
		if err := validateRuntimeFeatureLabels(prefix+" runtime_object_plan "+item.label, gotValues); err != nil {
			return err
		}
	}
	if !got.RuntimeUsed && (len(got.RuntimeObjectFeaturesLinked) != 0 || len(got.RuntimeObjectFeaturesInitialized) != 0) {
		return fmt.Errorf("%s runtime_object_plan has linked/initialized features while runtime_used=false", prefix)
	}
	return nil
}
...
func validateZeroHeapRequirement(name string, category string, rowStatus string, metadata tetraMetadata, root string) error {
	...
	if metadata.HeapAllocations != 0 {
		return fmt.Errorf("%s tetra_metadata.heap_allocations = %d, want 0", prefix, metadata.HeapAllocations)
	}
	if metadata.MemoryEvidence == nil {
	...
	if metric.EvidenceClass != "runtime_measured" {
		return fmt.Errorf("%s heap_alloc_bytes evidence_class = %q, want runtime_measured", prefix, metric.EvidenceClass)
	}
	if metric.TotalAllocBytes != 0 {
		return fmt.Errorf("%s heap_alloc_bytes total_alloc_bytes = %d, want 0", prefix, metric.TotalAllocBytes)
	}
	if metric.AllocationCount != 0 {
		return fmt.Errorf("%s heap_alloc_bytes allocation_count = %d, want 0", prefix, metric.AllocationCount)
	}
	if metric.Bytes != 0 {
		return fmt.Errorf("%s heap_alloc_bytes bytes = %d, want 0", prefix, metric.Bytes)
	}
	if metric.CurrentBytes != 0 {
		return fmt.Errorf("%s heap_alloc_bytes current_bytes = %d, want 0", prefix, metric.CurrentBytes)
	}
	...
	if sample.Program != name {
		return fmt.Errorf("%s heap_alloc_bytes sidecar program = %q, want %q", prefix, sample.Program, name)
	}
	if sample.HeapTotalAllocBytes != 0 {
		return fmt.Errorf("%s heap_alloc_bytes sidecar total_alloc_bytes = %d, want 0", prefix, sample.HeapTotalAllocBytes)
	}
	if sample.HeapAllocationCount != 0 {
		return fmt.Errorf("%s heap_alloc_bytes sidecar allocation_count = %d, want 0", prefix, sample.HeapAllocationCount)
	}
	if sample.HeapCurrentBytes != 0 {
		return fmt.Errorf("%s heap_alloc_bytes sidecar current_bytes = %d, want 0", prefix, sample.HeapCurrentBytes)
	}
	if sample.HeapPeakBytes != 0 {
		return fmt.Errorf("%s heap_alloc_bytes sidecar peak_bytes = %d, want 0", prefix, sample.HeapPeakBytes)
	}
	return nil
}

func validateMemoryEvidence(name string, rowStatus string, evidence *memoryEvidence, root string) error {
	if evidence == nil {
		return fmt.Errorf("benchmark %s missing memory evidence", name)
	}
	if evidence.Schema != schemaBenchmarkMemoryV1 {
		return fmt.Errorf("benchmark %s memory evidence schema = %q, want %q", name, evidence.Schema, schemaBenchmarkMemoryV1)
	}
	if err := validateHeapAllocBytesMetric(name, rowStatus, evidence.HeapAllocBytes, root); err != nil {
		return err
	}
	if err := validateRSSMetrics(name, rowStatus, evidence.RSSCurrent, evidence.RSSPeak, root); err != nil {
		return err
	}
	for _, item := range []struct {
		name            string
		...
		if err := validateMemoryMetric(name, item.name, item.metric, root, false, item.requireArtifact); err != nil {
			return err
		}
	}
	if len(evidence.DomainBytes) == 0 {
		...
		default:
			return fmt.Errorf("benchmark %s memory evidence domain_bytes empty requires unsupported or blocked evidence", name)
		}
	}
	for i, domain := range evidence.DomainBytes {
		if strings.TrimSpace(domain.DomainID) == "" {
			return fmt.Errorf("benchmark %s memory evidence domain_bytes[%d] missing domain_id", name, i)
		}
		if !allowedMemoryDomainKinds[domain.Kind] {
			return fmt.Errorf("benchmark %s memory evidence domain_bytes[%d] kind %q is not allowed", name, i, domain.Kind)
		}
		if err := validateEvidenceClass("benchmark "+name+" memory evidence domain_bytes", domain.EvidenceClass); err != nil {
			return err
		}
		if strings.TrimSpace(domain.Method) == "" {
			return fmt.Errorf("benchmark %s memory evidence domain_bytes[%d] missing method", name, i)
		}
		if domain.EvidenceClass == "allocation_report_estimate" {
			if domain.Method != "allocation_report_summary" {
				return fmt.Errorf("benchmark %s memory evidence domain_bytes[%d] allocation estimate method = %q, want allocation_report_summary", name, i, domain.Method)
			}
			if err := requireExistingPath(root, domain.SourceArtifact); err != nil {
				return fmt.Errorf("benchmark %s memory evidence domain_bytes[%d] source_artifact: %w", name, i, err)
			}
		}
	}
	return nil
...
func validateRSSMetrics(name string, rowStatus string, current memoryMetric, peak memoryMetric, root string) error {
	if rowStatus != "measured" {
		...
		if current.EvidenceClass != "blocked" {
			return fmt.Errorf("benchmark %s memory evidence rss_current for %s row must be blocked, got %q", name, rowStatus, current.EvidenceClass)
		}
		if peak.EvidenceClass != "blocked" {
			return fmt.Errorf("benchmark %s memory evidence rss_peak for %s row must be blocked, got %q", name, rowStatus, peak.EvidenceClass)
		}
		return nil
	}
...
func validateRSSPeakMetric(name string, metric memoryMetric, root string) (rsstelemetry.Sample, error) {
	...
	if metric.EvidenceClass != "runtime_measured" {
		return rsstelemetry.Sample{}, fmt.Errorf("%s for measured linux Tetra row must be runtime_measured, got %q", prefix, metric.EvidenceClass)
	}
	if metric.Method != rsstelemetry.MethodLinuxWait4RusageMaxRSSV1 {
		return rsstelemetry.Sample{}, fmt.Errorf("%s method = %q, want %q", prefix, metric.Method, rsstelemetry.MethodLinuxWait4RusageMaxRSSV1)
	}
	if strings.TrimSpace(metric.SourceArtifact) == "" {
		return rsstelemetry.Sample{}, fmt.Errorf("%s source_artifact is required for runtime_measured evidence", prefix)
	}
	resolved, err := resolveExistingPath(root, metric.SourceArtifact)
	if err != nil {
	...
	if sample.Program != name {
		return rsstelemetry.Sample{}, fmt.Errorf("%s sidecar program = %q, want %q", prefix, sample.Program, name)
	}
	...
	if metric.Bytes != 0 && metric.Bytes != sample.RSSPeakBytes {
		return rsstelemetry.Sample{}, fmt.Errorf("%s bytes = %d, want sidecar rss_peak_bytes %d", prefix, metric.Bytes, sample.RSSPeakBytes)
	}
	if metric.PeakBytes != 0 && metric.PeakBytes != sample.RSSPeakBytes {
		return rsstelemetry.Sample{}, fmt.Errorf("%s peak_bytes = %d, want sidecar rss_peak_bytes %d", prefix, metric.PeakBytes, sample.RSSPeakBytes)
	}
	return sample, nil
}

func validateRSSCurrentMetric(name string, metric memoryMetric, peakArtifact string, peakSample rsstelemetry.Sample, root string) error {
	...
	if metric.EvidenceClass == "blocked" {
		...
		if peakSample.SampleCount > 0 {
			return fmt.Errorf("%s is blocked but RSS sidecar has %d live samples", prefix, peakSample.SampleCount)
		}
		return nil
	}
	if metric.EvidenceClass != "runtime_measured" {
		return fmt.Errorf("%s for measured linux Tetra row must be runtime_measured or blocked, got %q", prefix, metric.EvidenceClass)
	}
	if metric.Method != rsstelemetry.MethodLinuxProcfsStatusVmRSSV1 {
		return fmt.Errorf("%s method = %q, want %q", prefix, metric.Method, rsstelemetry.MethodLinuxProcfsStatusVmRSSV1)
	}
	if strings.TrimSpace(metric.SourceArtifact) == "" {
	...
	if metric.SourceArtifact != peakArtifact {
		return fmt.Errorf("%s source_artifact = %q, want rss_peak source_artifact %q", prefix, metric.SourceArtifact, peakArtifact)
	}
	resolved, err := resolveExistingPath(root, metric.SourceArtifact)
	if err != nil {
		return fmt.Errorf("%s source_artifact: %w", prefix, err)
	...
	if sample.SampleCount == 0 {
		return fmt.Errorf("%s cannot be runtime_measured because sidecar sample_count is zero", prefix)
	}
	if metric.Bytes != 0 && metric.Bytes != sample.RSSCurrentBytes {
		return fmt.Errorf("%s bytes = %d, want sidecar rss_current_bytes %d", prefix, metric.Bytes, sample.RSSCurrentBytes)
	}
	if metric.CurrentBytes != 0 && metric.CurrentBytes != sample.RSSCurrentBytes {
		return fmt.Errorf("%s current_bytes = %d, want sidecar rss_current_bytes %d", prefix, metric.CurrentBytes, sample.RSSCurrentBytes)
	}
	return nil
}

func validateHeapAllocBytesMetric(name string, rowStatus string, metric memoryMetric, root string) error {
	...
	if rowStatus != "measured" {
		...
		if metric.EvidenceClass != "blocked" {
			return fmt.Errorf("%s for %s row must be blocked, got %q", prefix, rowStatus, metric.EvidenceClass)
		}
		return nil
	}
	if err := validateEvidenceClass(prefix, metric.EvidenceClass); err != nil {
	...
	if metric.EvidenceClass != "runtime_measured" {
		return fmt.Errorf("%s for measured linux-x64 Tetra row must be runtime_measured, got %q", prefix, metric.EvidenceClass)
	}
	if metric.Method != heaptelemetry.MethodLinuxX64HeapTelemetryV1 {
		return fmt.Errorf("%s method = %q, want %q", prefix, metric.Method, heaptelemetry.MethodLinuxX64HeapTelemetryV1)
	}
	if strings.TrimSpace(metric.SourceArtifact) == "" {
		return fmt.Errorf("%s source_artifact is required for runtime_measured evidence", prefix)
	}
	...
	if metric.Bytes != 0 && metric.Bytes != sample.HeapPeakBytes {
		return fmt.Errorf("%s bytes = %d, want sidecar heap_peak_bytes %d", prefix, metric.Bytes, sample.HeapPeakBytes)
	}
	if metric.CurrentBytes != 0 && metric.CurrentBytes != sample.HeapCurrentBytes {
		return fmt.Errorf("%s current_bytes = %d, want sidecar heap_current_bytes %d", prefix, metric.CurrentBytes, sample.HeapCurrentBytes)
	}
	if metric.PeakBytes != 0 && metric.PeakBytes != sample.HeapPeakBytes {
		return fmt.Errorf("%s peak_bytes = %d, want sidecar heap_peak_bytes %d", prefix, metric.PeakBytes, sample.HeapPeakBytes)
	}
	if metric.TotalAllocBytes != 0 && metric.TotalAllocBytes != sample.HeapTotalAllocBytes {
		return fmt.Errorf("%s total_alloc_bytes = %d, want sidecar heap_total_alloc_bytes %d", prefix, metric.TotalAllocBytes, sample.HeapTotalAllocBytes)
	}
	if metric.AllocationCount != 0 && metric.AllocationCount != sample.HeapAllocationCount {
		return fmt.Errorf("%s allocation_count = %d, want sidecar heap_allocation_count %d", prefix, metric.AllocationCount, sample.HeapAllocationCount)
	}
	return nil
}

func validateMemoryMetric(name string, metricName string, metric memoryMetric, root string, rssMetric bool, requireArtifact bool) error {
	...
	if rssMetric {
		...
		if metric.EvidenceClass == "runtime_measured" && strings.EqualFold(metric.Method, "MemStats") {
