...
func TestValidateWASISmokeReportRejectsArtifactReportThatRanCases(t *testing.T) {
	...
	if err == nil ||
		!strings.Contains(err.Error(), "artifact/import preflight report cannot include runner") {
...
func TestValidateWASISmokeReportRejectsUnexpectedSourceContract(t *testing.T) {
	raw := strings.Replace(
		runtimeReport(`"runner":"node-wasi",`, runtimeCases(true, true)),
...
func TestValidateWASISmokeReportRejectsMissingMultiReturnEvidence(t *testing.T) {
	raw := strings.Replace(
		runtimeReport(`"runner":"node-wasi",`, runtimeCases(true, true)),
		caseJSON(
			"wasm_multi_return_3_smoke",
			"examples/wasm/wasm_multi_return_3_smoke.tetra",
			0,
			"true",
			`"actual_exit":0,`,
		),
...
func TestValidateWASISmokeReportRejectsUnsupportedDiagnosticDrift(t *testing.T) {
	raw := strings.Replace(
		...
		`"expected_diagnostic":"runtime supported on wasm32"`,
		1,
	)
	err := validateWASISmokeReport([]byte(raw), validationModeRuntime)
	if err == nil || !strings.Contains(err.Error(), "time_sleep_smoke") ||
		!strings.Contains(err.Error(), "expected_diagnostic") {
...
func runtimeCases(ran bool, includeActualExit bool) string {
	...
	return caseJSON(
		...
		caseJSON(
			...
			0,
			ranLiteral,
			actualForExpected(includeActualExit, 0),
		) + "," +
		caseJSON(
			"wasm_globals_smoke",
			"examples/wasm/wasm_globals_smoke.tetra",
			0,
			ranLiteral,
			actual,
		) + "," +
		caseJSON(
			"wasm_multi_return_2_smoke",
			"examples/wasm/wasm_multi_return_2_smoke.tetra",
			0,
			ranLiteral,
			actual,
		) + "," +
		...
		caseJSON(
			"wasm_multi_return_4_smoke",
			"examples/wasm/wasm_multi_return_4_smoke.tetra",
			0,
			ranLiteral,
			actual,
		) + "," +
		...
		unsupportedCaseJSON(
			...
			0,
			"runtime not supported on wasm32",
		) + "," +
		unsupportedCaseJSON(
			"task_smoke",
