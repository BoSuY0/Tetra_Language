...
func collectWASM32WebBrowserCanvasProcessEvidence(sourcePath string, artifactDir string, scenarioName string) (surfaceProcessEvidence, error) {
	...
	expectedTraceAppExit := surfaceComponentAppExpectedExitForSource("wasm32-web-browser-canvas-"+scenarioName, sourcePath)
	traceFrames, err := writeBrowserCanvasSurfaceTrace(tracePath, wasmPath, browserTrace, expectedTraceAppExit)
	if err != nil {
		return surfaceProcessEvidence{}, err
	}
	traceArtifact, err := artifactReport(tracePath, "runner-trace")
	...
	if browserTrace.AppExitCode != expectedTraceAppExit {
		return surfaceProcessEvidence{}, fmt.Errorf("wasm32-web browser canvas app exit code = %d, want %d", browserTrace.AppExitCode, expectedTraceAppExit)
	}
	if len(traceFrames) < 2 {
	...
	if scenarioName == "studio-shell" {
		before := traceFrames[0]
		if !browserTrace.Canvas.Readback {
			return surfaceProcessEvidence{}, fmt.Errorf("wasm32-web browser-canvas Morph studio-shell trace missing RGBA readback evidence")
		}
		if after.Order != 5 || after.Width != 320 || after.Height != 200 || after.Stride != 1280 || !after.Presented {
			return surfaceProcessEvidence{}, fmt.Errorf("wasm32-web browser-canvas Morph studio-shell after-frame = %#v, want order-5 presented 320x200 RGBA frame", after)
		}
		if before.Checksum == after.Checksum {
			return surfaceProcessEvidence{}, fmt.Errorf("wasm32-web browser-canvas Morph studio-shell frame checksums did not change across browser input/readback: %#v", traceFrames)
		}
		if !browserTraceHasNativeEvents(browserTrace, []string{"pointerup", "keydown", "beforeinput"}) {
			return surfaceProcessEvidence{}, fmt.Errorf("wasm32-web browser-canvas Morph studio-shell trace missing required native browser input events: %#v", browserTrace.BrowserEvents)
		}
		sidecarScan, err := scanLegacyUISidecarArtifacts(artifactDir, sidecarScanOptions{AllowCompilerOwnedWASMLoader: true})
		if err != nil {
			return surfaceProcessEvidence{}, err
...
func writeBrowserCanvasSurfaceTrace(path string, wasmPath string, raw browserCanvasTrace, expectedAppExit int) ([]surface.FrameReport, error) {
	...
	if raw.AppExitCode != expectedAppExit {
		return nil, fmt.Errorf("browser canvas Surface trace app_exit_code = %d, want %d", raw.AppExitCode, expectedAppExit)
	}
	...
	for _, frame := range raw.Frames {
		...
		frameArtifactPath := filepath.Join(filepath.Dir(path), fmt.Sprintf("surface-browser-canvas-frame-order-%d.rgba", reportOrder))
		if err := os.WriteFile(frameArtifactPath, canvasPixels, 0o644); err != nil {
			return nil, fmt.Errorf("write browser canvas frame artifact %s: %w", frameArtifactPath, err)
		}
