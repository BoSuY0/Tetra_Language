...
type morphReportValidationOptions struct {
	SameCommit string
}
...
func validateSurfaceMorphReport(path string) error {
...
func validateSurfaceMorphReportWithOptions(path string, options morphReportValidationOptions) error {
	...
	if report.Morph == nil {
		return fmt.Errorf("surface Morph report requires morph evidence")
	}
...
func validateMorphReportArtifactFiles(reportDir string, artifacts []surface.ArtifactReport) error {
