...
func main() {
	outDir := flag.String(
		"out-dir",
		"reports/local-benchmark-zero-heap-v1",
...
func executeSpec(
	...
	if err != nil {
		...
		result.TetraMetadata = missingTetraMetadata(
			spec.BinaryRelPath,
	...
	measurements, heapEvidence, heapArtifacts, runErr := runIterationsWithHeapTelemetry(
		opt.Timeout,
		runCommand,
		env,
		opt.Iterations,
	...
	if result.Status != "measured" {
		result.TetraMetadata.MemoryEvidence = *blockedMemoryEvidence(
			("Tetra zero-heap benchmark run failed before runtime heap " +
				"telemetry could be trusted: ") + result.Error,
		)
	}
	return result
}
...
func runIterationsWithHeapTelemetry(
	...
	outDir string,
) ([]float64, *runtimeHeapEvidence, []string, error) {
...
func collectMemoryEvidence(
	...
	if err := readJSON(allocationReport, &parsed); err != nil {
		return blockedMemoryEvidence(
			"allocation report was unavailable while collecting zero-heap memory evidence",
		)
	}
...
func writeSummary(path string, parsed report) error {
	...
	b.WriteString(
		"Status: local Tetra-only zero-heap guardrail evidence. No official, cross-language performance, zero RSS, or universal zero-heap claim is made.\n\n",
	)
