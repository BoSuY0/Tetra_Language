...
func TestActorBenchmarkPrepRowsAreTierZeroWithRawArtifactsAndNonClaims(t *testing.T) {
	...
	for _, want := range []string{
		...
		if row.ImprovementRatio != 0 {
			t.Fatalf(
				"%s improvement_ratio = %.3f, want 0 for Tier 0 prep",
				want,
		...
		if strings.Contains(want, "zero_copy_move") &&
			strings.Contains(lowerClaim, "production runtime") {
...
func TestBuildReportCarriesActorMemoryDomainEvidence(t *testing.T) {
	...
	if len(report.ActorMemoryDomains) != 2 {
		t.Fatalf(
			"actor memory domains = %#v, want copy and owned-region domains",
			report.ActorMemoryDomains,
		)
	}
	...
	for _, domain := range report.ActorMemoryDomains {
		if domain.RuntimeMeasured || domain.RuntimeBlockedReason == "" {
			t.Fatalf(
				"actor domain runtime evidence = measured:%v reason:%q, want explicit local-model block",
				domain.RuntimeMeasured,
				domain.RuntimeBlockedReason,
			)
		}
	...
	if !foundOwnedRegion || !foundByteBackpressure {
		t.Fatalf(
			"actor memory domains missing owned-region:%v byte-backpressure:%v",
			foundOwnedRegion,
			foundByteBackpressure,
		)
	}
}
...
func TestParseParallelSchedulerEvidenceRequiresActorMemoryDomainsForObjectEvidence(t *testing.T) {
	...
	if len(gotBenchmarks) != len(benchmarks) || len(gotDomains) != len(domains) {
		t.Fatalf(
			"parsed benchmarks/domains = %d/%d, want %d/%d",
			len(gotBenchmarks),
			len(gotDomains),
			len(benchmarks),
			len(domains),
	...
	if _, _, err := parseParallelSchedulerEvidence(string(raw)); err == nil ||
		!strings.Contains(err.Error(), "actor_memory_domains") {
		t.Fatalf(
			("parseParallelSchedulerEvidence without actor domains error = %v," +
				" want actor_memory_domains rejection"),
			err,
		)
	}
}
