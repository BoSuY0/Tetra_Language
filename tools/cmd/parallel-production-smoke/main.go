...
func main() {
	...
	flag.StringVar(
		...
		"report",
		"",
		"path to write tetra.parallel.production.v1 report",
	)
	flag.StringVar(
		&opt.TetraPath,
		"tetra",
...
func runSmoke(ctx context.Context, opt smokeOptions) error {
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" {
		return fmt.Errorf(
			"parallel production smoke requires linux/amd64 host, got %s/%s",
			runtime.GOOS,
			runtime.GOARCH,
		)
	}
...
func (r *smokeRunner) runExecutableEvidence(ctx context.Context) error {
	if err := r.runExample(ctx, executableCase{
		...
		sourcePath:    filepath.Join("examples", "async", "wait_composition_smoke.tetra"),
		expectedExit:  0,
	...
	if err := r.runExample(ctx, executableCase{
		...
		sourcePath:    filepath.Join("examples", "async", "deadline_aware_waits_smoke.tetra"),
		expectedExit:  0,
	...
	if err := r.runExample(ctx, executableCase{
		...
		sourcePath:    filepath.Join("examples", "actors", "actors_tagged_stress.tetra"),
		expectedExit:  0,
		runtimeMode:   "builtin",
	...
	if err := r.runExample(ctx, executableCase{
		...
		sourcePath:    filepath.Join("examples", "tasks", "task_bounded_stress.tetra"),
		expectedExit:  42,
		recordProcess: false,
		cases: []parallelprod.CaseReport{
	...
	if err := r.runExample(ctx, executableCase{
		...
		sourcePath:    filepath.Join("examples", "tasks", "task_group_lifecycle_smoke.tetra"),
		expectedExit:  42,
		recordProcess: false,
		cases: []parallelprod.CaseReport{
	...
	if err := r.runExample(ctx, executableCase{
		...
		sourcePath:    filepath.Join("examples", "tasks", "task_group_cancel_smoke.tetra"),
		expectedExit:  1,
		recordProcess: false,
		cases: []parallelprod.CaseReport{
	...
	if err := os.WriteFile(
		sourcePath,
		[]byte(strings.TrimLeft(cancellationStormSource, "\n")),
		0o644,
	); err != nil {
		return err
	}
...
func (r *smokeRunner) runCompilerEvidence(ctx context.Context) error {
	tests := []struct {
	...
	}{
		{
			...
			kind:          "negative",
			pkg:           "./compiler",
			pattern:       "TestActorMailboxFullReturnsCheckedBackpressure",
			expectedError: "mailbox full/backpressure",
		},
		{
			name:          "message pool exhaustion",
			kind:          "negative",
			pkg:           "./compiler",
			pattern:       "TestActorMessagePoolExhaustionReturnsCheckedFailure",
		...
		},
		{
			...
			kind:          "negative",
			pkg:           "./compiler",
			pattern:       "TestActorInvalidHandleSendReturnsCheckedFailure",
		...
		},
		{
			...
			kind:          "negative",
			pkg:           "./compiler",
			pattern:       "TestActorSendToDoneActorReturnsCheckedFailure",
			expectedError: "done actor",
		},
		{
			name:          "actor failure handling",
			kind:          "negative",
			pkg:           "./tools/cmd/distributed-actor-runtime-smoke",
			pattern:       "TestBuildReportProducesValidDistributedActorRuntimeEvidence",
			expectedError: "missing-node failure",
		},
		{
			name:          "invalid handle diagnostics",
			kind:          "negative",
			pkg:           "./compiler/tests/ownership",
			pattern:       "TestReleaseTraceabilityLifetimeAndRaceSafetyNegativeActorTaskOwnership",
		...
		},
		{
			...
			kind:          "negative",
			pkg:           "./cli/cmd/tetra",
			pattern:       "TestCheckCommandJSONDiagnosticsForResourceDoubleJoinCode",
		...
		},
		{
			...
			kind:          "negative",
			pkg:           "./cli/cmd/tetra",
			pattern:       "TestCheckCommandJSONDiagnosticsForTaskGroupUseAfterCloseCode",
		...
		{
			name:    "race-safety rejection matrix",
			kind:    "positive",
			pkg:     "./compiler/internal/actorsafety",
			pattern: "TestTypedActorOwnershipTransferCoverageRequiresRaceSafetyMatrix",
		},
		{
			name:    "actor island boundary proof",
			kind:    "positive",
			pkg:     "./compiler/internal/semantics",
			pattern: "TestMemoryBoundaryHandoffCoverageRecordsActorTaskIslandFacts",
		},
		{
			name:    "actor broker leak cleanup",
			kind:    "positive",
		...
		},
		{
			...
			kind:          "negative",
			pkg:           "./compiler",
			pattern:       "TestTaskGroupCancelWakesJoinUntilBeforeDeadlineBuildAndRun",
		...
		},
		{
			...
			kind:          "negative",
			pkg:           "./compiler",
			pattern:       "TestTaskGroupCancelWakesActorRecvUntilBeforeDeadlineBuildAndRun",
			expectedError: "actor recv cancel wake",
		},
		{
			name:    "nested cancellation propagation",
			kind:    "positive",
			pkg:     "./compiler",
			pattern: "TestTaskCancellationCheckpointInheritedByNestedChildBuildAndRun",
		},
		{
			name:    "task actor mailbox handoff",
			kind:    "positive",
			pkg:     "./compiler",
			pattern: "TestTaskSpawnsActorAndReceivesMailboxReplyBuildAndRun",
		},
		{
			name:          "actor fanout mailbox drain soak",
			kind:          "stress",
			pkg:           "./compiler",
			pattern:       "TestActorFanoutMailboxDrainSoakBuildAndRun",
			iterations:    512,
			seed:          "actor-fanout-mailbox-drain-v1",
			maxDurationMS: 90000,
		},
	}
...
func (r *smokeRunner) runBoundaryCoverageEvidence(ctx context.Context) error {
	tests := []struct {
	...
	}{
		{
			...
			pattern: ("^(TestTaskSpawnRequiresRuntimeUse|TestTaskSpawnRejectsMutableGlo" +
				"balTarget|TestActorSpawnRejectsMutableGlobalTarget|TestTaskSpawnAllowsIm" +
				"mutableGlobalTarget|TestTaskSpawnGroupTypedRejectsMutableGlobalTarget)$"),
		},
		{
			pkg: "./compiler/tests/safety/effects",
			pattern: ("^(TestEffectsAliasesAndUnsafeRemainSeparate|TestEffectsRequireAc" +
				"torsUse|TestUnsafeStillRequiredWithEffectGroups)$"),
		},
	}
	for _, tc := range tests {
		res := runCommand(ctx, 90*time.Second, "go", "test", tc.pkg, "-run", tc.pattern, "-count=1")
		if res.err != nil || res.exitCode != 0 {
			r.cases = append(
				r.cases,
				failedCase("safe unsafe forbidden boundary coverage", "positive", "", res.output),
			)
			return fmt.Errorf(
				"safe unsafe forbidden boundary coverage evidence failed: %s",
				res.output,
			)
		}
	}
...
func (r *smokeRunner) writeReport() error {
	report := buildReportWithBenchmarks(
	...
	if len(r.actorMemoryDomains) > 0 {
		report.ActorMemoryDomains = append(
			[]parallelprod.ActorMemoryDomainReport(nil),
			r.actorMemoryDomains...)
	}
...
func buildReportWithBenchmarks(
	source string,
	processes []parallelprod.ProcessReport,
	cases []parallelprod.CaseReport,
	benchmarks []parallelprod.BenchmarkReport,
		...
		Contracts: []parallelprod.ContractReport{
			{
				...
				Status: "pass",
				Evidence: ("linux-x64 cooperative scheduler fairness, wait composition, and " +
					"many task stress evidence"),
			},
			{
				Name:   "join cancel deadline select group lifecycle",
				Status: "pass",
				Evidence: ("task join, cancellation, deadline timeout, cancel-wakes-" +
					"deadline-join, actor recv cancel wake, select2 readiness, and group " +
					"lifecycle smokes"),
			},
			{
				Name:   "actor mailbox backpressure and failure handling",
				Status: "pass",
				Evidence: ("actor capacity/backpressure, checked message pool exhaustion, " +
					"checked invalid-handle/done-actor send failures, task actor mailbox " +
					"handoff, and distributed missing-node failure evidence"),
			},
			{
				Name:   "task actor thread boundary transfer rules",
				Status: "pass",
				Evidence: ("compiler and CLI diagnostics for actor/task transfer, actor/" +
					"island boundary proof, double join, and closed group boundaries"),
			},
			{
				Name:   "race safety model",
				Status: "pass",
				Evidence: ("conservative shared mutable actor/task boundary rejections and " +
					"race-safety rejection matrix evidence"),
			},
			{
				Name:     "safe unsafe forbidden parallelism boundary",
				Status:   "pass",
				Evidence: "actor/task docs plus diagnostics for supported, unsafe, and forbidden boundaries",
			},
		},
...
func machineReadableDiagnosticsForCases(
	cases []parallelprod.CaseReport,
) []parallelprod.DiagnosticReport {
...
func (r *smokeRunner) runSchedulerPrototypeEvidence(ctx context.Context) error {
	...
	r.recordProcess(
		"parallel scheduler prototype tests",
		"benchmark",
	...
	r.cases = append(
		r.cases,
		parallelprod.CaseReport{
			Name: "per-core scheduler prototype",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
		parallelprod.CaseReport{
			Name: "two-core work stealing scheduler model",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
		parallelprod.CaseReport{
			Name: "zero-copy region message benchmark",
			Kind: "positive",
			Ran:  true,
			Pass: true,
		},
	...
	r.recordProcess(
		"parallel scheduler prototype evidence",
		"benchmark",
		"go run ./compiler/cmd/parallelrt-evidence",
		evidence,
	)
	if evidence.err != nil || evidence.exitCode != 0 {
		return fmt.Errorf(
			"parallel scheduler prototype benchmark evidence failed: %s",
			evidence.output,
		)
	}
...
func parseParallelSchedulerEvidence(
	raw string,
) ([]parallelprod.BenchmarkReport, []parallelprod.ActorMemoryDomainReport, error) {
	var evidence struct {
	...
	if err := json.Unmarshal([]byte(raw), &evidence); err == nil && len(evidence.Benchmarks) > 0 {
		if len(evidence.ActorMemoryDomains) == 0 {
			return nil, nil, fmt.Errorf(
				"parse parallel scheduler prototype evidence: actor_memory_domains is empty",
			)
		}
...
func attachBenchmarkRawArtifacts(
	benchmarks []parallelprod.BenchmarkReport,
	rawPath string,
) []parallelprod.BenchmarkReport {
	...
	return []parallelprod.AuditReport{
		{
			...
			Artifact: ("examples/tasks/task_bounded_stress.tetra; examples/async/wait_" +
				"composition_smoke.tetra; compiler/internal/actorsrt/actorsrt_core.go"),
			Evidence: ("scheduler fairness, many tasks stress, join, cancel, deadline, " +
				"select, and task group lifecycle cases ran on linux-x64"),
			Result: "pass",
		},
		{
			Requirement: "join/cancel/deadline/select/group lifecycle",
			Artifact: ("examples/tasks/task_group_lifecycle_smoke.tetra; examples/tasks/" +
				"task_group_cancel_smoke.tetra; examples/async/deadline_aware_waits_" +
				"smoke.tetra; examples/async/wait_composition_smoke.tetra"),
			Evidence: ("required lifecycle cases cover join, cancellation, deadline " +
				"timeout, cancel-wakes-deadline-join, actor recv cancel wake, select " +
				"readiness, task group lifecycle, and nested cancellation propagation"),
			Result: "pass",
		},
		{
			Requirement: "actor mailbox backpressure and failure handling",
			Artifact:    "compiler/compiler_suite_test.go; tools/cmd/distributed-actor-runtime-smoke",
			Evidence: ("actor capacity/backpressure, checked message pool exhaustion, " +
				"checked invalid-handle/done-actor send failures, task actor mailbox " +
				"handoff, and missing-node failure/status evidence are required cases"),
			Result: "pass",
		},
		{
			Requirement: "task/actor/thread-boundary transfer rules",
			Artifact: ("compiler/tests/ownership/actor_task/actor_task_ownership_" +
				"test.go; cli/cmd/tetra/tetra_suite_test.go"),
			Evidence: ("task and actor ownership transfer, actor/island boundary proof, " +
				"resource double join, and task group use-after-close diagnostics are " +
				"required cases"),
			Result: "pass",
		},
		{
			...
			Artifact: ("compiler/tests/ownership/actor_task/actor_task_ownership_" +
				"test.go; docs/spec/runtime/actors.md"),
			Evidence: ("shared mutable race-safety rejection and race-safety rejection " +
				"matrix evidence are required until a broader race-safe model is " +
				"implemented"),
			Result: "pass",
		},
		{
			Requirement: "stress evidence for tasks, actor messages, cancellation storms, and timeouts",
			Artifact: ("examples/tasks/task_bounded_stress.tetra; examples/actors/" +
				"actors_tagged_stress.tetra; tools/cmd/parallel-production-smoke"),
			Evidence: ("many tasks stress, many actor messages stress, actor fanout " +
				...
				"broker leak cleanup cases ran with bounded stress metadata"),
			Result: "pass",
		},
		{
			...
			Artifact: ("docs/spec/runtime/actors.md; docs/user/platform/async_actors_" +
				...
				"effects_test.go"),
			Evidence: ("documentation defines supported actor/task runtime, transfer " +
				"boundaries, and unsupported guarantees; safe unsafe forbidden boundary " +
				"coverage runs compiler tests for allowed immutable task targets, " +
				"missing runtime/actors effects, unsafe-only operations, and forbidden " +
				"mutable actor/task targets"),
			Result: "pass",
		},
		{
			...
			Evidence: ("negative parallel cases require stable expected_error evidence " +
				"for cancellation, deadline, backpressure, invalid handle, double join, " +
		...
		},
		{
			...
			Evidence: ("parallel production gate must run producer, validator, and " +
