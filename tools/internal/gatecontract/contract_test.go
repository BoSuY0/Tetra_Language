...
func TestDecodeNormalizesCommandAndStatementParts(t *testing.T) {
	got, err := Decode(contractJSON(t, func(doc map[string]any) {
		step := doc["steps"].([]any)[0].(map[string]any)
		delete(step, "command")
		step["command_parts"] = []any{
			"go",
			"test",
			"./tools/validators/surface",
			"-run",
			"TestValidateCrashReport",
			"-count=1",
		}

		validator := doc["validators"].([]any)[0].(map[string]any)
		delete(validator, "command")
		validator["command_parts"] = []any{
			"go",
			"run",
			"./tools/cmd/validate-surface-crash-report",
			"--report",
			"reports/surface/crash-report.json",
		}

		claim := doc["claims"].([]any)[0].(map[string]any)
		delete(claim, "statement")
		claim["statement_parts"] = []any{
			"Surface",
			"crash",
			"reporting",
			"evidence",
		...
		nonclaim := doc["nonclaims"].([]any)[0].(map[string]any)
		delete(nonclaim, "statement")
		nonclaim["statement_parts"] = []any{
			"This",
			"gate",
			"does",
			"not",
	...
	if err != nil {
		t.Fatalf("Decode rejected contract with split text fields: %v", err)
	}

	if got.Steps[0].Command !=
		"go test ./tools/validators/surface -run TestValidateCrashReport -count=1" {
		t.Fatalf("step command = %q", got.Steps[0].Command)
	}
	if len(got.Steps[0].CommandParts) != 0 {
		t.Fatalf("step command parts were not normalized away: %#v", got.Steps[0].CommandParts)
	}
	if got.Validators[0].Command !=
		"go run ./tools/cmd/validate-surface-crash-report --report reports/surface/crash-report.json" {
		t.Fatalf("validator command = %q", got.Validators[0].Command)
	}
	if got.Claims[0].Statement != "Surface crash reporting evidence is present." {
		t.Fatalf("claim statement = %q", got.Claims[0].Statement)
	}
	if got.Nonclaims[0].Statement !=
		"This gate does not claim every Surface release requirement is complete." {
		t.Fatalf("nonclaim statement = %q", got.Nonclaims[0].Statement)
	}
}
	...
	doc := map[string]any{
		...
		"steps": []any{
			map[string]any{
				...
				"kind": "go-test",
				"command": ("go test ./tools/validators/surface -run " +
					"TestValidateCrashReport -count=1"),
				"working_dir":           ".",
				"required":              true,
		...
		"validators": []any{
			map[string]any{
				...
				"kind": "go-command",
				"command": ("go run ./tools/cmd/validate-surface-crash-report --report " +
					"reports/surface/crash-report.json"),
			},
		},
