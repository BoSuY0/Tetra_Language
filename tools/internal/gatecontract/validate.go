...
func Validate(contract Contract) error {
	...
	for i, validator := range contract.Validators {
		if validator.ID == "" {
			issues = append(issues, fmt.Sprintf("validators[%d]: missing required field %q", i, "id"))
			continue
		}
		...
		if validator.Kind == "" {
			issues = append(issues, fmt.Sprintf("validator %q: missing required field %q", validator.ID, "kind"))
		}
		if validator.Command == "" {
			issues = append(issues, fmt.Sprintf("validator %q: missing required field %q", validator.ID, "command"))
		}
	}
	...
	for i, claim := range contract.Claims {
		...
		if claim.Statement == "" {
			issues = append(issues, fmt.Sprintf("claim %q: missing required field %q", claim.ID, "statement"))
		}
	}
	...
	for i, step := range contract.Steps {
		...
		for j, ref := range step.ValidatorRefs {
			...
			if _, ok := validatorIDs[ref]; !ok {
				issues = append(issues, fmt.Sprintf("%s validator_refs[%d] references missing validator %q", stepLabel, j, ref))
			}
		}
	}

	reportPaths := map[string]struct{}{}
	hashesRequired := contract.ArtifactHashes != nil && contract.ArtifactHashes.RequiresReportHashes()
	for i, report := range contract.RequiredReports {
		reportLabel := fmt.Sprintf("required_reports[%d]", i)
		if report.Path == "" {
			issues = append(issues, fmt.Sprintf("%s: missing required field %q", reportLabel, "path"))
		} else {
		...
		if report.Validator != "" {
			if _, ok := validatorIDs[report.Validator]; !ok {
				issues = append(issues, fmt.Sprintf("%s references missing validator %q", reportLabel, report.Validator))
			}
		}
		if hashesRequired && !report.ArtifactHashRequired {
			issues = append(issues, fmt.Sprintf("%s has artifact_hash_required=false while artifact_hashes are required/enabled", reportLabel))
		}
		for j, ref := range report.ClaimRefs {
			...
			if _, ok := claimIDs[ref]; !ok {
				issues = append(issues, fmt.Sprintf("%s claim_refs[%d] references missing claim %q", reportLabel, j, ref))
			}
		}
	}
