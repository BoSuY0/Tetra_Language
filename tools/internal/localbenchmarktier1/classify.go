...
func classifyCategory(category string, rows []benchmarkRow, threshold float64) (string, string) {
	...
	if !ok || tetra.Status != "measured" {
		return "blocked by missing feature", ("Tetra did not produce a measured local row for " +
			"this category.")
	}
	...
	if category == "HTTP plaintext/json" || category == "PostgreSQL single/multiple/update" ||
		category == "JSON parse/stringify" {
		return "invalid/inconclusive", ("This Tier 1 run measures deterministic local helper " +
			"kernels, not a full local service/database benchmark for this category.")
	}
	if tetra.TetraMetadata != nil {
		if heapSensitiveCategory(category) && tetra.TetraMetadata.HeapAllocations > 0 {
			return "blocked by heap allocation", fmt.Sprintf(
				"Tetra allocation report records %d heap allocations.",
				tetra.TetraMetadata.HeapAllocations,
			)
		}
		if boundsSensitiveCategory(category) && tetra.TetraMetadata.BoundsLeft > 0 {
			return "blocked by bounds check", fmt.Sprintf(
				"Tetra bounds report records %d bounds checks left.",
				tetra.TetraMetadata.BoundsLeft,
			)
		}
		if tetra.TetraMetadata.BackendPath == "fallback" ||
			tetra.TetraMetadata.BackendPath == "stack" {
			return "blocked by fallback backend", ("Tetra backend report selected stack/fallback " +
				"path for at least one function.")
		}
	}
	competitors := measuredCompetitorMedians(rows)
	if len(competitors) != 3 || tetra.MedianRuntimeMS <= 0 {
		return "invalid/inconclusive", ("One or more competitor rows did not produce measured " +
			"local timing.")
	}
	...
	if tetra.MedianRuntimeMS < fastest*(1-threshold) {
		return "faster than C/C++/Rust locally", fmt.Sprintf(
			"Tetra median %.3f ms is more than %.0f%% below the fastest local competitor median %.3f ms.",
			tetra.MedianRuntimeMS,
			threshold*100,
			fastest,
		)
	}
	if tetra.MedianRuntimeMS <= fastest*(1+threshold) {
		return "comparable", fmt.Sprintf(
			"Tetra median %.3f ms is within %.0f%% of the fastest local competitor median %.3f ms.",
			tetra.MedianRuntimeMS,
			threshold*100,
			fastest,
		)
	}
	return "slower", fmt.Sprintf(
		"Tetra median %.3f ms is more than %.0f%% above the fastest local competitor median %.3f ms.",
		tetra.MedianRuntimeMS,
		threshold*100,
		fastest,
	)
...
func actorRuntimeReadinessBlockers(row benchmarkRow) []string {
	...
	var blockers []string
	if row.Status != "measured" || row.MedianRuntimeMS <= 0 ||
		!hasRawOutputArtifact(row.RawOutputArtifacts) {
		blockers = append(
			blockers,
			"Measured actor runtime row with raw output artifacts is missing.",
		)
	}
	if metadata.BackendPath != "register" {
		blockers = append(
			blockers,
			fmt.Sprintf("Backend path is %q, not register.", metadata.BackendPath),
		)
	}
	if len(metadata.BackendBlockers) != 0 {
		blockers = append(
			blockers,
			"Backend blockers: "+strings.Join(metadata.BackendBlockers, ", ")+".",
		)
	}
...
func hasActorDomainMemoryEvidence(evidence *memoryEvidence) bool {
	...
	for _, domain := range evidence.DomainBytes {
		...
		if domain.RequestedBytes > 0 || domain.ReservedBytes > 0 || domain.CommittedBytes > 0 ||
...
func measuredMemoryEvidenceClass(class string) bool {
	return class != "" && class != "unsupported" && class != "blocked" &&
		class != "allocation_report_estimate"
}

func actorRuntimeClassificationReason(row benchmarkRow) string {
	reason := ("Current local actor/task runtime evidence is bounded and not a " +
		"production parallel benchmark claim.")
...
func classifyBinarySize(rows []benchmarkRow) (string, string) {
	...
	for _, language := range []string{"c", "cpp", "rust"} {
		...
		if !ok || row.BinarySizeBytes <= 0 {
			return "invalid/inconclusive", ("One or more competitor binary_size_bytes values are " +
				"missing for binary-size category.")
		}
		sizes[language] = row.BinarySizeBytes
	}
	return "comparable", fmt.Sprintf(
		("binary_size_bytes local evidence: Tetra=%d, C=%d, C++=%d, " +
			"Rust=%d; no binary-size superiority or production-size claim is " +
			"promoted."),
		tetra.BinarySizeBytes,
		sizes["c"],
		sizes["cpp"],
		sizes["rust"],
	)
...
func classifyCompileTime(rows []benchmarkRow, threshold float64) (string, string) {
	...
	for _, language := range []string{"c", "cpp", "rust"} {
		...
		if !ok || row.CompileTimeMS <= 0 {
			return "invalid/inconclusive", ("One or more competitor compile_time_ms values are " +
				"missing for compile-time category.")
		}
	...
	if tetra.CompileTimeMS < fastest*(1-threshold) {
		return "faster than C/C++/Rust locally", fmt.Sprintf(
			("Tetra compile_time_ms %.3f is more than %.0f%% below the " +
				"fastest local competitor compile_time_ms %.3f."),
			tetra.CompileTimeMS,
			threshold*100,
			fastest,
		)
	...
	if tetra.CompileTimeMS <= fastest*(1+threshold) {
		return "comparable", fmt.Sprintf(
			("Tetra compile_time_ms %.3f is within %.0f%% of the fastest " +
				"local competitor compile_time_ms %.3f."),
			tetra.CompileTimeMS,
			threshold*100,
			fastest,
		)
	...
	return "slower", fmt.Sprintf(
		("Tetra compile_time_ms %.3f is more than %.0f%% above the " +
			"fastest local competitor compile_time_ms %.3f."),
		tetra.CompileTimeMS,
		threshold*100,
		fastest,
