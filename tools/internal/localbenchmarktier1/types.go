...
var requiredP20Categories = specs.RequiredCategories

var requiredLanguages = specs.RequiredLanguages

type benchmarkSpec = specs.Spec
...
func (d memoryDomainByte) MarshalJSON() ([]byte, error) {
	type commonDomainByte struct {
		DomainID       string `json:"domain_id"`
		Kind           string `json:"kind"`
		RequestedBytes uint64 `json:"requested_bytes,omitempty"`
		ReservedBytes  uint64 `json:"reserved_bytes,omitempty"`
	...
	common := commonDomainByte{
		DomainID:       d.DomainID,
		Kind:           d.Kind,
		RequestedBytes: d.RequestedBytes,
		ReservedBytes:  d.ReservedBytes,
		CommittedBytes: d.CommittedBytes,
		ReleasedBytes:  d.ReleasedBytes,
	...
	if d.Kind != "actor" || !d.ActorDomainFieldsSet {
		return json.Marshal(common)
	}
	type actorDomainByte struct {
		commonDomainByte
		MailboxCurrentBytes uint64 `json:"mailbox_current_bytes"`
		MailboxPeakBytes    uint64 `json:"mailbox_peak_bytes"`
		ByteBudget          uint64 `json:"byte_budget"`
	...
	return json.Marshal(actorDomainByte{
