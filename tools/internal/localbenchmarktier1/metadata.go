...
func collectTetraMetadata(
	name string,
	binaryPath string,
	optimizerArtifact string,
	runtimeHeap *runtimeHeapEvidence,
	runtimeRSS *runtimeRSSEvidence,
) *tetraMetadata {
	...
	return &tetraMetadata{
		...
		HeapReasonCodes:            heapReasonCodes,
		PerfBlockers:               readPerfBlockers(perf, name, heap, heapReasonCodes, boundsLeft),
...
func missingTetraMetadata(binaryPath string, optimizerArtifact string) *tetraMetadata {
	return &tetraMetadata{
		...
		RuntimeFeatureEvidence: blockedRuntimeFeatureEvidence(
			"Tetra build failed before runtime feature artifacts were produced",
		),
		RuntimeObjectPlan: blockedRuntimeObjectPlan(
			"Tetra build failed before runtime object plan artifacts were produced",
		),
		HeapReasonCodes: []string{},
		MemoryEvidence: blockedMemoryEvidence(
...
func collectMemoryEvidence(
	...
	if err := readJSON(allocationReport, &report); err != nil {
		return blockedMemoryEvidence(
			"allocation report was unavailable while collecting benchmark memory evidence",
		)
	}
	...
	if len(domains) == 0 {
		domainEvidence = unsupportedMemoryMetric(
			"not_collected",
			"allocation report summary does not include memory domains",
		)
	}
	bytesCopied := allocationReportMetric(copied, allocationReport)
	if runtimeDomains, runtimeDomainEvidence, runtimeBytesCopied, ok := runtimeDomainMemoryEvidence(
		runtimeHeap,
	); ok {
		domains = runtimeDomains
		domainEvidence = runtimeDomainEvidence
		bytesCopied = runtimeBytesCopied
	}
	heapMetric := unsupportedMemoryMetric(
		"not_collected",
		"Tier 1 runner did not collect runtime heap telemetry for this benchmark process",
	)
	if runtimeHeap != nil {
		heapMetric = runtimeHeapMetric(runtimeHeap)
	}
	rssCurrent := unsupportedMemoryMetric(
		"not_collected",
		"Tier 1 runner does not measure process RSS per benchmark row",
	)
	rssPeak := unsupportedMemoryMetric(
		"not_collected",
		"Tier 1 runner does not measure process RSS per benchmark row",
	)
	if runtimeRSS != nil {
		...
		if runtimeRSS.Sample.SampleCount > 0 {
			...
			rssCurrent = blockedMemoryMetric(
				"RSS sampler did not observe a live current RSS sample for this benchmark process",
			)
		}
	}
...
func runtimeDomainMemoryEvidence(
	evidence *runtimeHeapEvidence,
) ([]memoryDomainByte, memoryMetric, memoryMetric, bool) {
	if evidence == nil || len(evidence.Sample.DomainBytes) == 0 {
		return nil, memoryMetric{}, memoryMetric{}, false
	}
	domains := make([]memoryDomainByte, 0, len(evidence.Sample.DomainBytes))
	var copied uint64
	for _, domain := range evidence.Sample.DomainBytes {
		copied += domain.BytesCopied
		reportDomain := memoryDomainByte{
			DomainID:       domain.DomainID,
			Kind:           domain.Kind,
			RequestedBytes: domain.RequestedBytes,
			ReservedBytes:  domain.ReservedBytes,
		...
		if domain.Kind == "actor" && domain.ActorDomainFieldsSet {
			...
			reportDomain.ActorDomainFieldsSet = true
		}
	...
	bytesCopied := memoryMetric{
		Bytes:          copied,
		EvidenceClass:  "runtime_measured",
		Method:         heaptelemetry.MethodLinuxX64HeapTelemetryV1,
		SourceArtifact: evidence.SourceArtifact,
	}
	return domains, domainEvidence, bytesCopied, true
}
...
func readRuntimeFeatureMetadata(path string) runtimeFeatureMetadata {
	...
	if readJSON(path, &report) != nil {
		return runtimeFeatureMetadata{
			...
			Initialized:      []string{},
			LazyInitBlockers: []string{},
			Evidence: blockedRuntimeFeatureEvidence(
				"backend report was unavailable while collecting runtime feature evidence",
			),
			ObjectPlan: blockedRuntimeObjectPlan(
				"backend report was unavailable while collecting runtime object plan evidence",
			),
		}
	}
	...
	return runtimeFeatureMetadata{
		...
		ObjectPlan: runtimeObjectPlan{
			...
			RuntimeObjectFeaturesRequired: normalizedRuntimeFeatureSlice(
				objectPlan.RuntimeObjectFeaturesRequired,
			),
			RuntimeObjectFeaturesLinked: normalizedRuntimeFeatureSlice(
				objectPlan.RuntimeObjectFeaturesLinked,
			),
			RuntimeObjectFeaturesInitialized: normalizedRuntimeFeatureSlice(
				objectPlan.RuntimeObjectFeaturesInitialized,
			),
			RuntimeObjectLazyInitBlockers: normalizedRuntimeFeatureSlice(
				objectPlan.RuntimeObjectLazyInitBlockers,
			),
		},
	}
}
...
func readPerfBlockers(
	...
	for _, row := range report.Benchmarks {
		if row.Benchmark == benchmark {
			return filterResolvedPerfBlockers(
				row.ReasonCodes,
				heapAllocations,
				heapReasonCodes,
				boundsLeft,
...
func filterResolvedPerfBlockers(
	reasonCodes []string,
	heapAllocations int,
	heapReasonCodes []string,
	boundsLeft int,
	...
	for _, code := range reasonCodes {
		if code == "allocation.local_call_heap_fallback" && heapAllocations == 0 {
			continue
		}
		if code == "allocation.unknown_call" &&
			!hasReasonCode(heapReasonCodes, "heap.required_unknown_call") {
			continue
		}
		if code == "allocation.return_escape" &&
			!hasReasonCode(heapReasonCodes, "heap.required_escape_return") {
			continue
		}
		if code == "bounds.missing_dominance" && boundsLeft == 0 {
			continue
		}
		out = append(out, code)
	}
