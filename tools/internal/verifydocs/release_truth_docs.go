...
type verificationErrors []string
...
func (errs verificationErrors) Error() string {
	...
	for _, err := range errs {
		lines = append(lines, "verify-docs: "+err)
	}
	return strings.Join(lines, "\n")
}

// Run verifies repository documentation against the generated manifest.
func Run(root, manifestPath string) error {
	if root != "" {
	...
	if manifestPath == "" {
		manifestPath = filepath.FromSlash("docs/generated/manifest.json")
	}
...
func run(manifestPath string) error {
	...
	checkContains("docs/spec/runtime/actors.md", m.RuntimeABI.ActorsSupportedTargets)
	...
	checkContains("docs/spec/runtime/runtime_abi.md", runtimeSymbols)
	...
	checkContains("docs/spec/runtime/unsafe.md", unsafeBuiltins)
	...
	checkContains(filepath.FromSlash("cli/cmd/tetra/tetra_core.go"), triples)
	...
	if err := verifyDoctestBlocks(
		[]string{"README.md", "docs/spec/flow/flow_syntax_mvp.md", "docs/spec/ui/ui_v1.md"},
	); err != nil {
		errs = append(errs, err.Error())
	}
	...
	if err := verifyStableExamplesDoNotImportExperimental(
		stableModuleExamplePaths(stableModulePaths),
	); err != nil {
		errs = append(errs, err.Error())
	}
	...
	if err := verifyStdlibGuide(
		filepath.FromSlash("docs/user/platform/standard_library_guide.md"),
		stableModulePaths,
		experimentalModulePaths,
	); err != nil {
		errs = append(errs, err.Error())
	}
	if err := verifyEpic14ExampleIndex(
		filepath.FromSlash("docs/user/reference/examples_index.md"),
	); err != nil {
		errs = append(errs, err.Error())
	}
	...
	if err := verifyMemoryIslandsSurfaceReleaseDocs(
		memoryIslandsSurfaceReleaseDocPaths(),
	); err != nil {
		errs = append(errs, err.Error())
	}
	if err := verifyMemoryIslandsFinalProductionReadinessAudit(
		memoryIslandsFinalProductionReadinessAuditDocPaths(),
	); err != nil {
		errs = append(errs, err.Error())
	}
	if err := verifyMemoryIslandsFinalActorBenchmarkHandoff(
		memoryIslandsFinalActorBenchmarkHandoffDocPaths(),
	); err != nil {
		errs = append(errs, err.Error())
	}
	if err := verifyActorRuntimeFoundationDocs(
		defaultActorRuntimeFoundationDocPaths(),
		m.Features,
	); err != nil {
		errs = append(errs, err.Error())
	}
	if err := verifyFinalMemoryIslandsSurfaceProductionAudit(
		finalMemoryIslandsSurfaceProductionAuditDocPaths(),
	); err != nil {
		errs = append(errs, err.Error())
	}
	...
	if err := verifyWASMBackendPlan(
		"docs/backend/wasm_backend_plan.md",
		ctarget.WASMTriples(),
	); err != nil {
		errs = append(errs, err.Error())
	}
	if err := verifyNetworkingRuntimeBoundaryDocs(
		defaultNetworkingRuntimeBoundaryDocPaths(),
	); err != nil {
		errs = append(errs, err.Error())
	}
	if err := verifyMemoryProductionContractDocs(
		defaultMemoryProductionContractDocPaths(),
	); err != nil {
		errs = append(errs, err.Error())
	}
	if err := verifyRAMContractCompilerDocs(
		defaultRAMContractCompilerDocPaths(),
		m.Features,
	); err != nil {
		errs = append(errs, err.Error())
	}
	...
	if len(errs) > 0 {
		return verificationErrors(errs)
	}
	return nil
}

type FeatureManifest = featureManifest
type MemoryProductionContractDocPaths = memoryProductionContractDocPaths
type NetworkingRuntimeBoundaryDocPaths = networkingRuntimeBoundaryDocPaths
type RAMContractCompilerDocPaths = ramContractCompilerDocPaths

type testingHooks struct{}

var Testing testingHooks

func (testingHooks) VerifyDoctestBlocks(paths []string) error {
	return verifyDoctestBlocks(paths)
}

func (testingHooks) VerifyRequiredDoctestBlocks(paths []string) error {
	return verifyRequiredDoctestBlocks(paths)
}

func (testingHooks) VerifySpecCodeBlocks(paths []string) error {
	return verifySpecCodeBlocks(paths)
}

func (testingHooks) VerifyWASMBackendPlan(path string, plannedTargets []string) error {
	return verifyWASMBackendPlan(path, plannedTargets)
}

func (testingHooks) VerifyMemoryProductionContractDocs(
	paths MemoryProductionContractDocPaths,
) error {
	return verifyMemoryProductionContractDocs(paths)
}

func (testingHooks) MemoryProductionContractRequirements(
	paths MemoryProductionContractDocPaths,
) []memoryProductionContractRequirement {
	return memoryProductionContractRequirements(paths)
}

func (testingHooks) ForbiddenPublicPerformanceClaims(text string) []string {
	return forbiddenPublicPerformanceClaims(text)
}

func (testingHooks) VerifyNetworkingRuntimeBoundaryDocs(
	paths NetworkingRuntimeBoundaryDocPaths,
) error {
	return verifyNetworkingRuntimeBoundaryDocs(paths)
}

func (testingHooks) NetworkingRuntimeBoundaryRequirements(
	paths NetworkingRuntimeBoundaryDocPaths,
) []networkingRuntimeBoundaryRequirement {
	return networkingRuntimeBoundaryRequirements(paths)
}

func (testingHooks) VerifyFeatureRegistry(features []FeatureManifest) error {
	return verifyFeatureRegistry(features)
}

func (testingHooks) VerifySurfaceBlockSystemFeatureBoundary(
	features map[string]FeatureManifest,
) error {
	return verifySurfaceBlockSystemFeatureBoundary(features)
}

func (testingHooks) VerifyReleaseTruthDocs(paths []string) error {
	return verifyReleaseTruthDocs(paths)
}

func (testingHooks) CurrentReleaseTruthDocPaths() []string {
	return currentReleaseTruthDocPaths()
}

func (testingHooks) VerifySurfaceReleaseDocs(paths []string) error {
	return verifySurfaceReleaseDocs(paths)
}

func (testingHooks) VerifyMemoryIslandsSurfaceReleaseDocs(paths []string) error {
	return verifyMemoryIslandsSurfaceReleaseDocs(paths)
}

func (testingHooks) VerifyFinalMemoryIslandsSurfaceProductionAudit(paths []string) error {
	return verifyFinalMemoryIslandsSurfaceProductionAudit(paths)
}

func (testingHooks) VerifyMemoryIslandsFinalProductionReadinessAudit(paths []string) error {
	return verifyMemoryIslandsFinalProductionReadinessAudit(paths)
}

func (testingHooks) VerifyMemoryIslandsFinalActorBenchmarkHandoff(paths []string) error {
	return verifyMemoryIslandsFinalActorBenchmarkHandoff(paths)
}

func (testingHooks) VerifyActorRuntimeFoundationDocs(
	paths []string,
	features []FeatureManifest,
) error {
	return verifyActorRuntimeFoundationDocs(paths, features)
}

func (testingHooks) DefaultActorRuntimeFoundationDocPaths() []string {
	return defaultActorRuntimeFoundationDocPaths()
}

func (testingHooks) VerifyRAMContractCompilerDocs(
	paths RAMContractCompilerDocPaths,
	features []FeatureManifest,
) error {
	return verifyRAMContractCompilerDocs(paths, features)
}

func (testingHooks) CurrentGitHeadForDocs() (string, bool) {
	return currentGitHeadForDocs()
}

func (testingHooks) CurrentGitParentForDocs() (string, bool) {
	return currentGitParentForDocs()
}

func (testingHooks) ExtractTetraDoctests(doc string) ([]string, error) {
	return extractTetraDoctests(doc)
}

func (testingHooks) VerifyStableModuleDoctestCoverage(paths []string) error {
	return verifyStableModuleDoctestCoverage(paths)
}

func (testingHooks) VerifyStableModuleEffectsMetadata(paths []string) error {
	return verifyStableModuleEffectsMetadata(paths)
}

func (testingHooks) VerifyStdlibGuide(
	path string,
	stablePaths []string,
	experimentalPaths []string,
) error {
	return verifyStdlibGuide(path, stablePaths, experimentalPaths)
}

func (testingHooks) VerifyExperimentalModuleMirrors(paths []string) error {
	return verifyExperimentalModuleMirrors(paths)
}

func (testingHooks) VerifyStableModuleExamples(paths []string) error {
	return verifyStableModuleExamples(paths)
}

func (testingHooks) VerifyStdlibModulePaths(
	stablePaths []string,
	experimentalPaths []string,
) error {
	return verifyStdlibModulePaths(stablePaths, experimentalPaths)
}

func (testingHooks) VerifyStableExamplesDoNotImportExperimental(paths []string) error {
	return verifyStableExamplesDoNotImportExperimental(paths)
}

func (testingHooks) VerifyEpic14ExampleIndex(path string) error {
	return verifyEpic14ExampleIndex(path)
}
	...
	for _, path := range paths {
		...
		for _, pattern := range patterns {
			if pattern.re.MatchString(text) {
				errs = append(
					errs,
					fmt.Sprintf("%s: misleading release language matched %q", path, pattern.label),
				)
			}
		}
		for _, claim := range forbiddenPublicPerformanceClaims(text) {
			errs = append(
				errs,
				fmt.Sprintf("%s: forbidden %s claim in release truth docs", path, claim),
			)
		}
...
func verifyDoctestBlocksWithPolicy(paths []string, requireAtLeastOne bool) error {
	for _, path := range paths {
		...
		for i, block := range blocks {
			if _, err := compiler.ParseFile(
				[]byte(block),
				fmt.Sprintf("%s#doctest%d", path, i+1),
			); err != nil {
...
func currentReleaseTruthDocPaths() []string {
	return []string{
		"README.md",
		filepath.FromSlash("docs/spec/core/current_supported_surface.md"),
		filepath.FromSlash("docs/spec/memory/islands.md"),
		filepath.FromSlash("docs/spec/surface/surface_v1.md"),
		filepath.FromSlash("docs/spec/flow/v0_2_scope.md"),
		filepath.FromSlash("docs/spec/policy/v1_feature_status.md"),
		filepath.FromSlash("docs/spec/flow/v1_scope.md"),
		filepath.FromSlash("docs/user/platform/async_actors_guide.md"),
		filepath.FromSlash("docs/user/platform/eco_package_guide.md"),
		filepath.FromSlash("docs/user/reference/examples_index.md"),
		filepath.FromSlash("docs/user/start/getting_started.md"),
		filepath.FromSlash("docs/user/start/language_tour.md"),
		filepath.FromSlash("docs/user/platform/ownership_effects_guide.md"),
		filepath.FromSlash("docs/user/platform/standard_library_guide.md"),
		filepath.FromSlash("docs/user/surface/surface_guide.md"),
		filepath.FromSlash("docs/user/start/troubleshooting.md"),
		filepath.FromSlash("docs/user/surface/wasm_ui_guide.md"),
		filepath.FromSlash("docs/release/surface/memory_islands_surface_scope.md"),
	}
}
...
func surfaceReleaseDocPaths() []string {
	return []string{
		filepath.FromSlash("docs/spec/core/current_supported_surface.md"),
		filepath.FromSlash("docs/spec/surface/surface_v1.md"),
		filepath.FromSlash("docs/release/surface/surface_v1_release_contract.md"),
		filepath.FromSlash("docs/release/surface/surface_v1_release_notes.md"),
		filepath.FromSlash("docs/release/surface/surface_v1_release_audit.md"),
		filepath.FromSlash("docs/user/reference/examples_index.md"),
		filepath.FromSlash("docs/user/surface/surface_guide.md"),
		filepath.FromSlash("docs/user/surface/surface_cookbook.md"),
		filepath.FromSlash("docs/user/surface/surface_morph_recipe_cookbook.md"),
	}
}
...
func memoryIslandsFinalProductionReadinessAuditDocPaths() []string {
	return []string{
		filepath.FromSlash(
			"docs/audits/memory/islands/memory-islands-final-production-readiness.md",
		),
	}
}
...
func memoryIslandsFinalActorBenchmarkHandoffDocPaths() []string {
	return []string{
		filepath.FromSlash("docs/audits/memory/islands/memory-islands-final-production-handoff.md"),
	}
}

func defaultActorRuntimeFoundationDocPaths() []string {
	return []string{
		filepath.FromSlash("docs/spec/runtime/actors.md"),
		filepath.FromSlash("docs/user/platform/async_actors_guide.md"),
		filepath.FromSlash("docs/design/actor_region_transfer.md"),
		filepath.FromSlash("docs/audits/runtime/actors/actor-runtime-production-boundary-v1.md"),
		filepath.FromSlash(
			"docs/audits/runtime/actors/actor-runtime-production-foundation-final.md",
		),
...
func verifyActorRuntimeFoundationDocs(paths []string, features []featureManifest) error {
	...
	for _, path := range paths {
		...
		for _, claim := range forbiddenActorRuntimeFoundationClaims(text) {
			errs = append(
				errs,
				fmt.Sprintf("%s: forbidden actor runtime foundation claim %q", path, claim),
			)
		}
	}
...
func verifyActorRuntimeFoundationManifestFeature(features []featureManifest) error {
	...
	for _, required := range []string{
		...
		if !strings.Contains(haystack, required) {
			return fmt.Errorf(
				"feature registry actors.distributed-runtime missing actor foundation phrase %q",
				required,
			)
		}
	}
...
func forbiddenActorRuntimeFoundationClaims(text string) []string {
	...
	for _, phrase := range []string{
		...
		for {
			...
			if !explicitActorFoundationNonClaimContext(clause) &&
				!explicitActorFoundationNonClaimContext(sentence) {
...
func verifyMemoryIslandsSurfaceReleaseDocs(paths []string) error {
	...
	for _, path := range paths {
		...
		for _, claim := range forbiddenMemoryIslandsSurfaceReleaseClaims(text) {
			errs = append(
				errs,
				fmt.Sprintf("%s: forbidden Memory/Islands/Surface release claim %q", path, claim),
			)
		}
	}
	...
	for _, required := range []string{
		...
		if !strings.Contains(combinedLower, required) {
			errs = append(
				errs,
				fmt.Sprintf("Memory/Islands/Surface release docs missing %q", required),
			)
		}
	}
	if len(errs) > 0 {
...
func verifyFinalMemoryIslandsSurfaceProductionAudit(paths []string) error {
	...
	for _, path := range paths {
		...
		for _, claim := range forbiddenFinalMemoryIslandsSurfaceProductionAuditClaims(text) {
			errs = append(
				errs,
				fmt.Sprintf(
					"%s: forbidden final Memory/Islands/Surface audit claim %q",
					path,
					claim,
				),
			)
	...
	for _, required := range []string{
		...
		("go test -buildvcs=false ./tools/cmd/verify-docs -run " +
		...
		if !strings.Contains(combinedLower, required) {
			errs = append(
				errs,
				fmt.Sprintf("final Memory/Islands/Surface audit missing %q", required),
			)
		}
	}
...
func verifyMemoryIslandsFinalProductionReadinessAudit(paths []string) error {
	...
	for _, path := range paths {
		...
		for _, claim := range forbiddenFinalMemoryIslandsSurfaceProductionAuditClaims(text) {
			errs = append(
				errs,
				fmt.Sprintf("%s: forbidden final Memory/Islands audit claim %q", path, claim),
			)
		}
	}
	combinedText := combined.String()
	...
	for _, required := range []string{
		...
		("memory-production-linux-x64-smoke.sh --report-dir reports/" +
			"memory-islands-ideal/final/memory-production"),
		("memory-islands-surface-production-gate.sh --report-dir reports/" +
			"memory-islands-ideal/final/integrated"),
		...
		if !containsFinalAuditEvidence(combinedLower, required) {
...
func containsFinalAuditEvidence(text, required string) bool {
	if strings.Contains(text, required) {
		return true
	}
	return strings.Contains(foldFinalAuditWhitespace(text), foldFinalAuditWhitespace(required))
}

func foldFinalAuditWhitespace(text string) string {
...
func verifyMemoryIslandsFinalActorBenchmarkHandoff(paths []string) error {
	...
	for _, path := range paths {
		...
		for _, claim := range forbiddenFinalActorBenchmarkHandoffClaims(text) {
			errs = append(
				errs,
				fmt.Sprintf("%s: forbidden final actor/benchmark handoff claim %q", path, claim),
			)
		}
	}
	combinedText := combined.String()
	...
	for _, path := range paths {
		...
		if strings.Contains(text, "/tmp/") {
			errs = append(
				errs,
				fmt.Sprintf(
					"%s: Surface release docs must not use /tmp paths as current release evidence",
					path,
				),
			)
		}
		...
		for _, clause := range surfaceReleaseDocClauses(text) {
			...
			if containsAnySubstring(lower, []string{"macos surface", "macos/windows surface"}) &&
				...
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: macOS Surface fake current claim: %q",
						path,
			...
			if containsAnySubstring(lower, []string{"windows surface", "macos/windows surface"}) &&
				...
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: Windows Surface fake current claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			}
			if strings.Contains(lower, "wasm32-wasi") && strings.Contains(lower, "surface") &&
				surfaceReleaseClaimPromotes(lower) &&
				!surfaceReleaseExplicitNonClaimContext(lower) {
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: wasm32-wasi Surface fake current claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if strings.Contains(lower, "cross-platform") && surfaceReleaseClaimPromotes(lower) &&
				...
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: cross-platform Surface fake production claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if strings.Contains(lower, "gpu") && surfaceReleaseClaimPromotes(lower) &&
				...
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: GPU Surface fake production claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if containsAnySubstring(
				lower,
				[]string{"platform-native widget", "native widget", "platform widget"},
			) &&
				surfaceReleaseClaimPromotes(lower) &&
				!surfaceReleaseExplicitNonClaimContext(lower) {
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: native widget Surface fake production claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if strings.Contains(lower, "rich text") && surfaceReleaseClaimPromotes(lower) &&
				...
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: rich text Surface fake production claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if containsAnySubstring(lower, []string{"screen-reader", "screen reader", "at-spi"}) &&
				surfaceReleaseClaimPromotes(
					lower,
				) && !surfaceReleaseExplicitNonClaimContext(lower) {
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: screen-reader Surface fake production claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			}
			if strings.Contains(lower, "metadata-only") &&
				strings.Contains(lower, "production accessibility") &&
				!surfaceReleaseExplicitNonClaimContext(lower) {
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: metadata-only accessibility fake production claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			}
			if containsAnySubstring(lower, []string{"dom ui", "html ui"}) &&
				surfaceReleaseClaimPromotes(lower) &&
				!surfaceReleaseExplicitNonClaimContext(lower) {
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: DOM UI fake production claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if strings.Contains(lower, "dom ui") && strings.Contains(lower, "surface model") &&
				...
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: DOM UI fake Surface model claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if strings.Contains(lower, "react") && surfaceReleaseClaimPromotes(lower) &&
				...
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: React Surface fake production claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if containsAnySubstring(
				lower,
				[]string{"core surface primitive", "surface core primitive", "core primitive"},
			) &&
				containsAnySubstring(
					lower,
					[]string{"button", "textfield", "text field", "card", "sidebar", "modal"},
				) &&
				!surfaceReleaseExplicitNonClaimContext(lower) {
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: core widget primitive fake Surface claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if containsAnySubstring(lower, []string{"user js", "user javascript"}) &&
				...
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: user JS fake allowance claim: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
			...
			if strings.Contains(lower, "final current claim") {
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: final current claim ownership must stay with P29: %q",
						path,
						strings.TrimSpace(clause),
					),
				)
	...
	for _, required := range []string{"unsupported", "macos", "windows", "wasm32-wasi"} {
		if !strings.Contains(combinedLower, required) {
			errs = append(
				errs,
				fmt.Sprintf(
					"Surface release docs missing unsupported targets evidence: %s",
					required,
				),
			)
		}
...
func verifySurfaceReleaseDocP28Governance(path string, text string) []string {
	...
	for _, tier := range []string{
		...
		if !strings.Contains(text, tier) {
			errs = append(
				errs,
				fmt.Sprintf("%s: Surface release doc missing claim tier %s", path, tier),
			)
		}
	}
	if !strings.Contains(text, "bash scripts/release/surface/product-gate.sh") {
		errs = append(
			errs,
			fmt.Sprintf("%s: Surface release doc missing product-gate.sh command link", path),
		)
	}
	return errs
}
