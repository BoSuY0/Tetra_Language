...
func defaultRAMContractCompilerDocPaths() ramContractCompilerDocPaths {
	return ramContractCompilerDocPaths{
		...
		Spec:   filepath.FromSlash("docs/spec/memory/ram_contract_report_schema.md"),
		User:   filepath.FromSlash("docs/user/platform/ram_contracts.md"),
		Readiness: filepath.FromSlash(
			"docs/audits/memory/ram-raw/ram-contract-compiler-readiness.md",
		),
		Handoff: filepath.FromSlash(
			"docs/audits/memory/ram-raw/ram-contract-compiler-handoff.md",
		),
	}
}

func ramContractCompilerRequirements(
	paths ramContractCompilerDocPaths,
) []ramContractCompilerRequirement {
...
func verifyRAMContractCompilerDocs(
	paths ramContractCompilerDocPaths,
	...
	for _, requirement := range ramContractCompilerRequirements(paths) {
		...
		for _, want := range requirement.Required {
			if !strings.Contains(text, want) {
				errs = append(
					errs,
					fmt.Sprintf(
						"%s: missing %q for %s RAM contract docs",
						requirement.Path,
						want,
						requirement.Name,
					),
		...
		for _, claim := range forbiddenRAMContractClaims(text) {
			errs = append(
				errs,
				fmt.Sprintf("%s: forbidden RAM contract claim %q", requirement.Path, claim),
			)
		}
		for _, flag := range unsupportedRAMContractValidatorFlags(text) {
			errs = append(
				errs,
				fmt.Sprintf(
					"%s: unsupported RAM contract validator flag %q",
					requirement.Path,
					flag,
				),
			)
		...
		if requirement.Name == "readiness audit" {
			if head, ok := staleRAMContractReadinessGitHead(text); ok {
				errs = append(
					errs,
					fmt.Sprintf("%s: stale readiness git head %s", requirement.Path, head),
				)
			}
		}
	}
...
func verifyRAMContractManifestFeature(features []featureManifest) error {
	...
	if feature.Status != "current" {
		return fmt.Errorf(
			"feature registry compiler.ram-contracts status = %s, want current",
			feature.Status,
		)
	}
	...
	for _, required := range []string{
		...
		if !strings.Contains(haystack, required) {
			return fmt.Errorf(
				"feature registry compiler.ram-contracts missing RAM contract phrase %q",
				required,
			)
		}
	}
	for _, claim := range forbiddenRAMContractClaims(feature.Scope + " " + feature.Stability) {
		return fmt.Errorf(
			"feature registry compiler.ram-contracts forbidden RAM contract claim %q",
			claim,
		)
	}
	return nil
