...
func verifyStableModuleExamples(paths []string) error {
	for _, path := range paths {
		...
		if !strings.Contains(string(raw), moduleRef) {
			return fmt.Errorf(
				"%s: stable module example %q does not reference %q",
				path,
				example,
				moduleRef,
			)
...
func stableModuleExamplePath(moduleName string) string {
	switch moduleName {
	case "accessibility", "block", "component", "draw", "morph", "style":
		return filepath.FromSlash(
			fmt.Sprintf("examples/core/surface/core_%s_smoke.tetra", moduleName),
		)
	case "async":
		return filepath.FromSlash("examples/async/core_async_smoke.tetra")
	case "capability":
		return filepath.FromSlash("examples/core/memory/core_memory_smoke.tetra")
	case "collections", "json", "math", "serialization", "slices", "strings":
		return filepath.FromSlash(fmt.Sprintf("examples/core/data/core_%s_smoke.tetra", moduleName))
	case "crypto", "memory":
		return filepath.FromSlash(
			fmt.Sprintf("examples/core/memory/core_%s_smoke.tetra", moduleName),
		)
	case "filesystem", "http", "io", "net", "networking", "time":
		return filepath.FromSlash(
			fmt.Sprintf("examples/core/platform/core_%s_smoke.tetra", moduleName),
		)
	case "i18n", "surface", "surface_app", "surface_app_shell", "text", "widgets":
		return filepath.FromSlash(
			fmt.Sprintf("examples/core/surface_app/core_%s_smoke.tetra", moduleName),
		)
	case "postgres", "sync", "testing":
		return filepath.FromSlash(
			fmt.Sprintf("examples/core/runtime/core_%s_smoke.tetra", moduleName),
		)
	default:
...
func verifyStdlibModulePath(path string, wantDir string, wantPrefix string, stable bool) error {
	...
	rel, ok := stdlibModuleRelPath(path, wantDir)
	if !ok {
	...
	if len(parts) < 1 || len(parts) > 2 {
		return fmt.Errorf(
			"%s: expected module file directly under lib/%s or one bucket below it",
			path,
			wantDir,
		)
	}
	if len(parts) == 2 && parts[0] != name {
		...
		if parent == "" || strings.HasSuffix(parent, ".parts") {
			return fmt.Errorf("%s: expected bucketed module path under lib/%s", path, wantDir)
		}
	}
	raw, err := os.ReadFile(path)
	if err != nil {
...
func stdlibModuleRelPath(path string, wantDir string) (string, bool) {
	...
	if index := strings.LastIndex(clean, marker); index >= 0 {
		return clean[index+len(marker):], true
	}
	return "", false
}
	...
	for _, path := range paths {
		...
		for _, imp := range file.Imports {
			if imp.Path == "lib.experimental" || strings.HasPrefix(imp.Path, "lib.experimental.") {
				return fmt.Errorf(
					"%s: stable example imports experimental module %q",
					path,
					imp.Path,
				)
			}
...
func verifyStableModuleEffectsMetadata(paths []string) error {
	for _, path := range paths {
		...
		if !sameEffectSet(metadataEffects, declaredEffects) {
			return fmt.Errorf(
				"%s: effects metadata mismatch: got %s want %s",
				path,
				formatEffectSet(metadataEffects),
				formatEffectSet(declaredEffects),
			)
		}
	}
	return nil
...
func verifyExperimentalModuleMirrors(paths []string) error {
	for _, path := range paths {
		...
		if !strings.Contains(text, "Experimental") ||
			!strings.Contains(text, "no stability guarantees") {
...
func verifyStdlibGuide(path string, stablePaths []string, experimentalPaths []string) error {
	...
	for _, modulePath := range stablePaths {
		...
		if !strings.Contains(row.example, expectedExample) {
			errs = append(
				errs,
				fmt.Sprintf(
					"lib.core.%s example mismatch: got %q want %q",
					name,
					row.example,
					expectedExample,
				),
		...
		if !sameEffectSet(guideEffects, declaredEffects) {
			errs = append(
				errs,
				fmt.Sprintf(
					"lib.core.%s effects mismatch: got %s want %s",
					name,
					formatEffectSet(guideEffects),
					formatEffectSet(declaredEffects),
				),
			)
		}
	...
	for _, modulePath := range experimentalPaths {
		...
		if !strings.Contains(text, experimentalImport) {
			errs = append(
				errs,
				fmt.Sprintf("missing experimental guide row for lib.experimental.%s", name),
			)
		}
		if !strings.Contains(text, stableImport) {
			errs = append(
				errs,
				fmt.Sprintf("missing stable replacement for lib.experimental.%s", name),
			)
		}
	}
	if len(experimentalPaths) > 0 &&
		(!strings.Contains(
			text,
			"Experimental mirror",
		) || !strings.Contains(
		...
		errs = append(
			errs,
			"experimental mirrors section must state Experimental mirror and no stability guarantees",
		)
	}
	if len(errs) > 0 {
...
func parseStdlibGuideStableRows(text string) map[string]stdlibGuideStableRow {
	...
	for _, line := range strings.Split(text, "\n") {
		trimmed := strings.TrimSpace(line)
		if strings.HasPrefix(trimmed, "## ") &&
			!strings.HasPrefix(trimmed, "## Stable Module Choices") {
			inStableTable = false
		}
...
func stableModuleDeclaredEffects(path string, raw []byte) ([]string, error) {
	effects := map[string]struct{}{}
	if err := addStableModuleDeclaredEffects(effects, path, raw); err != nil {
		return nil, err
	}
	partPaths, err := stableModulePartPaths(path)
	if err != nil {
		return nil, err
	}
	for _, partPath := range partPaths {
		partRaw, err := os.ReadFile(partPath)
		if err != nil {
			return nil, fmt.Errorf("%s: %v", partPath, err)
		}
		if err := addStableModuleDeclaredEffects(effects, partPath, partRaw); err != nil {
			return nil, err
		}
	}
	return sortedEffectNames(effects), nil
...
func stableModulePartPaths(path string) ([]string, error) {
	partsDir := strings.TrimSuffix(path, filepath.Ext(path)) + ".parts"
	entries, err := os.ReadDir(partsDir)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("%s: %v", partsDir, err)
	}
	var paths []string
	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".tetra" {
			continue
		}
		paths = append(paths, filepath.Join(partsDir, entry.Name()))
	}
	sort.Strings(paths)
	return paths, nil
}

func addStableModuleDeclaredEffects(effects map[string]struct{}, path string, raw []byte) error {
	file, err := compiler.ParseFile(raw, path)
	if err != nil {
		return fmt.Errorf("%s: %v", path, err)
	}
...
func currentStableModulePaths() []string {
	return currentModulePaths(filepath.FromSlash("lib/core"))
}

func currentExperimentalModulePaths() []string {
	return currentModulePaths(filepath.FromSlash("lib/experimental"))
}

func currentModulePaths(root string) []string {
	var paths []string
	err := filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		if entry.IsDir() {
			if strings.HasSuffix(entry.Name(), ".parts") {
...
func verifyEpic14ExampleIndex(path string) error {
	...
	if !strings.Contains(text, "## Epic 14 Verification Commands") &&
		!strings.Contains(text, "## Epic 15 Verification Commands") {
		missing = append(
			missing,
			"heading ## Epic 14 Verification Commands or ## Epic 15 Verification Commands",
		)
	}
	if !strings.Contains(text, "## Troubleshooting Notes (Epic 14)") &&
		!strings.Contains(text, "## Troubleshooting Notes (Epic 15)") {
		missing = append(
			missing,
			"heading ## Troubleshooting Notes (Epic 14) or ## Troubleshooting Notes (Epic 15)",
		)
	}
	...
	if len(missing) > 0 {
		return fmt.Errorf(
			"%s: missing Epic 14 index coverage: %s",
			path,
