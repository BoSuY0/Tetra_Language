...
func TestBuildRenderCommandStreamIncludesGlyphIconRasterEvidence(t *testing.T) {
	...
	for i, command := range first.Commands {
		if command.Order != i+1 || command.Source != source || command.SourceNodeID != "block:2" ||
			command.Recipe != "morph.search_input" {
			t.Fatalf("command[%d] = %#v, want source-linked Block scene command", i, command)
		}
	}
	text := first.Commands[8]
	icon := first.Commands[9]
	if text.Command != "text" || text.MarkerOnly || text.RasterHash == "" ||
		text.RasterCoverage <= 0 ||
		text.RasterFormat != "builtin-5x7-alpha-mask-v1" {
		t.Fatalf("text command = %#v, want non-marker glyph raster evidence", text)
	}
	if icon.Command != "icon" || icon.MarkerOnly || icon.RasterHash == "" ||
		icon.RasterCoverage <= 0 ||
		icon.RasterFormat != "builtin-icon-mask-raster-v1" {
...
func TestRenderCommandStreamRGBAProducesRendererOwnedFrameChecksum(t *testing.T) {
	...
	if frame.Width != 64 || frame.Height != 48 || frame.Stride != 256 ||
		len(frame.Pixels) != 64*48*4 {
		t.Fatalf(
			"frame dimensions = %dx%d stride=%d bytes=%d, want 64x48 stride=256 bytes=%d",
			frame.Width,
			frame.Height,
			frame.Stride,
			len(frame.Pixels),
