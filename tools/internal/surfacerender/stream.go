...
func BuildCommandStream(
	...
	if isMorphGuestDashboardSource(source) {
		...
		commands = renderGuestDashboardFrameCommands(source, snapshot, width, height)
	}
...
func isMorphGuestDashboardSource(source string) bool {
	source = strings.ReplaceAll(strings.TrimSpace(source), "\\", "/")
	return strings.HasSuffix(source, "surface_morph_guest_dashboard.tetra")
}
...
func renderCommandsFromSnapshot(
	source string,
	snapshot *surface.BlockSceneSnapshotReport,
) []surface.RenderCommandReport {
	...
	for _, node := range snapshot.Nodes {
		...
		for layerIndex, layer := range node.Paint.Layers {
			...
			renderCommand := renderCommandFromLayer(
				source,
				snapshot.BlockSceneHash,
				node,
				layer,
				layerIndex,
				order,
...
func renderStudioShellFrameCommands(
	source string,
	snapshot *surface.BlockSceneSnapshotReport,
	width int,
	height int,
	...
	node := func(name string) surface.BlockSceneNodeReport {
		...
		return surface.BlockSceneNodeReport{
			BlockID: 1,
	...
	specs := []commandSpec{
		{
			nodeName: "RenderedStudioShell",
			kind:     "fill",
			rect:     surface.RectReport{X: 0, Y: 0, W: width, H: height},
			color:    "#0e1216ff",
		},
		...
		{nodeName: "DashboardShell", kind: "overlay", rect: contentRect, color: "#10182000"},
		{
			nodeName: "CommandPalette",
			kind:     "text",
			rect:     surface.RectReport{X: commandRect.X + 14, Y: commandRect.Y + 12, W: 72, H: 7},
			color:    "#e8eef400",
		},
		{
			nodeName: "ToolbarActions",
			kind:     "icon",
			rect: surface.RectReport{
				X: toolbarRect.X + 10,
				Y: toolbarRect.Y + 10,
...
func renderGuestDashboardFrameCommands(
	source string,
	snapshot *surface.BlockSceneSnapshotReport,
	width int,
	height int,
	...
	recentPanel := surface.RectReport{X: margin, Y: 22, W: panelWidth, H: 58}
	overviewPanel := surface.RectReport{X: margin, Y: 88, W: panelWidth, H: height - 98}
	if overviewPanel.H < 78 {
		overviewPanel.H = 78
	}
	centerX := width / 2
	iconW := 14
	iconH := 18
	recentIcon := surface.RectReport{
		X: centerX - iconW/2,
		Y: recentPanel.Y + 17,
		W: iconW,
		H: iconH,
	}
	overviewIcon := surface.RectReport{
		X: centerX - iconW/2,
		Y: overviewPanel.Y + 26,
		W: iconW,
		H: iconH,
	}
	titleRect := surface.RectReport{X: margin, Y: 7, W: 158, H: 11}
	recentTitle := surface.RectReport{X: recentPanel.X + 8, Y: recentPanel.Y + 8, W: 90, H: 7}
	recentEmpty := surface.RectReport{X: centerX - 31, Y: recentPanel.Y + 44, W: 62, H: 6}
	overviewTitle := surface.RectReport{X: overviewPanel.X + 8, Y: overviewPanel.Y + 8, W: 72, H: 7}
	overviewDivider := surface.RectReport{
		X: overviewPanel.X + 8,
		Y: overviewPanel.Y + 20,
		W: overviewPanel.W - 16,
		H: 1,
	}
	overviewHeadline := surface.RectReport{X: centerX - 60, Y: overviewPanel.Y + 62, W: 120, H: 7}
	overviewBody := surface.RectReport{X: centerX - 72, Y: overviewPanel.Y + 79, W: 144, H: 6}
	...
	specs := []commandSpec{
		{
			nodeName: "GuestDashboardPage",
			kind:     "fill",
			rect:     surface.RectReport{X: 0, Y: 0, W: width, H: height},
			color:    "#ffffffff",
		},
		{nodeName: "PersonalCabinetTitle", kind: "text", rect: titleRect, color: "#111827ff"},
		{
			nodeName: "RecentCoursesPanel",
			kind:     "shadow",
			rect:     recentPanel,
			color:    "#00000012",
			blur:     2,
		...
		{nodeName: "RecentCoursesPanel", kind: "radius_clip", rect: recentPanel, radius: 3},
		{
			nodeName: "RecentCoursesPanel",
			kind:     "overlay",
			rect:     recentPanel,
		...
		{nodeName: "RecentCoursesTitle", kind: "text", rect: recentTitle, color: "#111827ff"},
		{
			nodeName: "RecentCoursesEmptyIcon",
			kind:     "fill",
			rect:     recentIcon,
			color:    "#e5e7ebff",
			radius:   1,
		...
		},
		{
			nodeName: "RecentCoursesEmptyIcon",
			kind:     "icon",
			rect:     surface.RectReport{X: recentIcon.X + 3, Y: recentIcon.Y + 4, W: 8, H: 8},
			color:    "#aab1baff",
		},
		{nodeName: "RecentCoursesEmptyState", kind: "text", rect: recentEmpty, color: "#4b5563ff"},
		{
			nodeName: "CourseOverviewPanel",
			kind:     "shadow",
			rect:     overviewPanel,
			color:    "#00000012",
			blur:     2,
		...
		{nodeName: "CourseOverviewPanel", kind: "radius_clip", rect: overviewPanel, radius: 3},
		{
			nodeName: "CourseOverviewPanel",
			kind:     "gradient",
			rect:     overviewPanel,
		...
		{nodeName: "CourseOverviewTitle", kind: "text", rect: overviewTitle, color: "#111827ff"},
		{
			nodeName: "CourseOverviewDivider",
			kind:     "fill",
			rect:     overviewDivider,
			color:    "#d7dbe0ff",
		},
		{
			nodeName: "CourseOverviewEmptyState",
			kind:     "image_fill",
			rect:     overviewIcon,
			color:    "#ffffff00",
			radius:   1,
		...
		},
		{
			nodeName: "CourseOverviewEmptyState",
			kind:     "icon",
			rect:     surface.RectReport{X: overviewIcon.X + 3, Y: overviewIcon.Y + 4, W: 8, H: 8},
			color:    "#aab1baff",
		},
		{
			nodeName: "CourseOverviewHeadline",
			kind:     "text",
			rect:     overviewHeadline,
			color:    "#111827ff",
		},
		{nodeName: "CourseOverviewBody", kind: "text", rect: overviewBody, color: "#111827ff"},
	}
	commands := make([]surface.RenderCommandReport, 0, len(specs))
	for i, spec := range specs {
		layer := surface.BlockScenePaintLayerSpecReport{
...
func renderCommandFromRect(
	...
	return renderCommandFromLayer(
		...
		order-1,
		order,
		normalizeRenderCommandKind(layer.Kind),
	)
}

func renderCommandFromLayer(
	source string,
	blockSceneHash string,
	node surface.BlockSceneNodeReport,
	layer surface.BlockScenePaintLayerSpecReport,
...
func attachRasterProof(source string, blockSceneHash string, command *surface.RenderCommandReport) {
	...
	case "text":
		...
		command.RasterHash = sha256Text(fmt.Sprintf(
			"text-raster|%s|%s|%s|%s|%d|%d|%d|%d",
			source,
			blockSceneHash,
