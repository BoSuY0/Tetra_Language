...
func validateMemory100RAMContractBundleAt(ramDir string, gitHead string) []string {
	...
	if err := ramvalidate.ReadStrictJSONFile(reportPath, &report); err != nil {
		...
		if gitHead != "" && report.GitHead != gitHead {
			issues = append(
				issues,
				fmt.Sprintf(("ram-contract-report.json git_head %s does not match Memory100 "+
					"git_head %s"), report.GitHead, gitHead),
			)
		}
	}
	...
	if reportOK && heapOK && copyOK {
		issues = append(
			issues,
			validateMemory100RAMHeapCopyClassification(report, heapBlockers, copyBlockers)...)
	}
	return issues
}

func validateMemory100RAMHeapCopyClassification(
	report ramvalidate.Report,
	heapBlockers ramvalidate.BlockerReport,
	copyBlockers ramvalidate.BlockerReport,
) []string {
		...
		if memory100RAMRowIsHeap(row) {
			...
			if memory100RAMUnclassified(row.ValidationStatus) {
				issues = append(
					issues,
					fmt.Sprintf(
						"ram-contract-report.json heap row %d site_id %q has unclassified validation_status %q",
						i,
						row.SiteID,
			...
			if len(nonEmptyMemory100Strings(row.Blockers)) == 0 {
				issues = append(
					issues,
					fmt.Sprintf(
						"ram-contract-report.json heap row %d site_id %q has no classified blockers",
						i,
						row.SiteID,
					),
				)
			...
			for _, blocker := range row.Blockers {
				if memory100RAMUnclassified(blocker) {
					issues = append(
						issues,
						fmt.Sprintf(
							"ram-contract-report.json heap row %d site_id %q has unclassified blocker %q",
							i,
							row.SiteID,
							blocker,
						),
		...
		if memory100RAMRowIsCopy(row) {
			...
			if memory100RAMUnclassified(row.ValidationStatus) {
				issues = append(
					issues,
					fmt.Sprintf(
						"ram-contract-report.json copy row %d site_id %q has unclassified validation_status %q",
						i,
						row.SiteID,
						row.ValidationStatus,
					),
			...
			if memory100RAMUnclassified(row.CopyReason) {
				issues = append(
					issues,
					fmt.Sprintf(
						"ram-contract-report.json copy row %d site_id %q has unclassified copy_reason %q",
						i,
						row.SiteID,
						row.CopyReason,
					),
	...
	for i, row := range heapBlockers.Rows {
		if memory100RAMUnclassified(strings.Join(row.Blockers, "\n")) {
			issues = append(
				issues,
				fmt.Sprintf(
					"heap-blockers.json row %d site_id %q has unclassified blockers",
					i,
					row.SiteID,
				),
			)
	...
	for i, row := range copyBlockers.Rows {
		if memory100RAMUnclassified(row.CopyReason) {
			issues = append(
				issues,
				fmt.Sprintf(
					"copy-blockers.json row %d site_id %q has unclassified copy_reason",
					i,
					row.SiteID,
				),
			)
...
func validateMemory100AllocationLoweringRAMConsistency(reportDir string) []string {
	allocationPath := filepath.Join(
		reportDir,
		"allocation-lowering",
		"allocation-lowering-report.json",
	)
	...
	for _, decision := range allocation.Decisions {
		...
		for siteID := range coveredSites {
			...
			if !ok {
				issues = append(
					issues,
					fmt.Sprintf(
						("allocation lowering decision %s covered_site_ids site_id %q "+
							"missing from ram-contract-report.json"),
						decision.Name,
						siteID,
					),
				)
			...
			if !memory100AllocationActualStorageMatchesRAMRow(decision, row) {
				issues = append(
					issues,
					fmt.Sprintf(
						("allocation lowering decision %s actual_lowering_storage %q "+
							"contradicts ram-contract-report.json site_id %q placement %q"),
						decision.Name,
						decision.ActualLoweringStorage,
						siteID,
						row.Placement,
					),
				)
			...
			case "heap_fallback_blocker":
				if len(heapSites) > 0 {
					issues = append(
						issues,
						fmt.Sprintf(
							("allocation lowering decision %s is not_observed but ram-"+
								"contract-report.json has heap rows %v"),
							decision.Name,
							memory100SortedSetKeys(heapSites),
						),
					)
				}
			case "copy_blocker":
				if len(copySites) > 0 {
					issues = append(
						...
						fmt.Sprintf(
							("allocation lowering decision %s is not_observed but ram-"+
								"contract-report.json has copy rows %v"),
							decision.Name,
							memory100SortedSetKeys(copySites),
						),
					)
				}
			}
			...
			case "heap_fallback_blocker":
				for siteID := range coveredSites {
					if _, ok := heapSites[siteID]; !ok {
						issues = append(
							issues,
							fmt.Sprintf(
								"allocation lowering decision %s covered_site_ids site_id %q is not a heap RAM row",
								decision.Name,
								siteID,
							),
						)
					...
					if _, ok := heapBlockerSites[siteID]; !ok {
						issues = append(
							...
							fmt.Sprintf(
								("allocation lowering decision %s covered_site_ids site_id %q "+
									"missing from heap-blockers.json"),
								decision.Name,
								siteID,
							),
						)
				...
				for _, siteID := range memory100MissingSetKeys(heapSites, coveredSites) {
					issues = append(
						issues,
						fmt.Sprintf(
							"allocation lowering decision %s missing heap RAM row site_id %q in covered_site_ids",
							decision.Name,
							siteID,
						),
					)
			...
			case "copy_blocker":
				for siteID := range coveredSites {
					if _, ok := copySites[siteID]; !ok {
						issues = append(
							issues,
							fmt.Sprintf(
								"allocation lowering decision %s covered_site_ids site_id %q is not a copy RAM row",
								decision.Name,
								siteID,
							),
						)
					...
					if _, ok := copyBlockerSites[siteID]; !ok {
						issues = append(
							...
							fmt.Sprintf(
								("allocation lowering decision %s covered_site_ids site_id %q "+
									"missing from copy-blockers.json"),
								decision.Name,
								siteID,
							),
						)
				...
				for _, siteID := range memory100MissingSetKeys(copySites, coveredSites) {
					issues = append(
						issues,
						fmt.Sprintf(
							"allocation lowering decision %s missing copy RAM row site_id %q in covered_site_ids",
							decision.Name,
							siteID,
						),
					)
		...
		if proofBackedTrustedRows == 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					("allocation lowering decision %s is proof-backed but ram-"+
						"contract-report.json has no proof-backed trusted rows"),
					decision.Name,
				),
			)
		}
		for siteID := range coveredSites {
			if _, ok := proofBackedTrustedSites[siteID]; !ok {
				issues = append(
					issues,
					fmt.Sprintf(
						("allocation lowering decision %s covered_site_ids site_id %q is "+
							"not a proof-backed trusted RAM row"),
						decision.Name,
						siteID,
					),
				)
	...
	for _, siteID := range memory100MissingSetKeys(proofBackedTrustedSites, proofCoveredSites) {
		issues = append(
			issues,
			fmt.Sprintf(
				("allocation lowering missing proof-backed trusted RAM row site_"+
					"id %q in proven covered_site_ids"),
				siteID,
			),
		)
	}
...
func memory100AllocationActualStorageMatchesRAMRow(
	decision memory100AllocationLoweringDecision,
	row ramvalidate.Row,
) bool {
...
func validateMemory100RawMemoryContract(path string, gitHead string) []string {
	...
	if report.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf("raw memory contract status is %q, want pass", report.Status),
		)
	}
	if gitHead != "" && report.GitHead != gitHead {
		issues = append(
			issues,
			fmt.Sprintf(
				"raw memory contract git_head %s does not match Memory100 git_head %s",
				report.GitHead,
				gitHead,
			),
		)
	...
	for _, name := range []string{
		"core.alloc_bytes",
		"core.ptr_add",
		"raw_slice_from_parts",
		"raw_load_store_metadata",
		"memcpy_u8",
		"memset_u8",
		"cap.mem",
	} {
		...
		if len(nonEmptyMemory100Strings(operation.SourceArtifacts)) == 0 {
			issues = append(
				issues,
				fmt.Sprintf("raw memory contract operation %s missing source_artifacts", name),
			)
		}
		if len(
			nonEmptyMemory100Strings(operation.PositiveTests),
		)+len(
			nonEmptyMemory100Strings(operation.NegativeTests),
		) == 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"raw memory contract operation %s missing positive_tests or negative_tests",
					name,
				),
			)
		}
		...
		case "core.alloc_bytes":
			issues = append(
				issues,
				requireMemory100RawEvidence(
					name,
					"source_artifacts",
					operation.SourceArtifacts,
			...
			issues = append(
				...
				requireMemory100RawEvidence(
					name,
					"positive_tests",
					operation.PositiveTests,
					"allocation-base metadata",
				)...)
		case "core.ptr_add":
			issues = append(
				issues,
		...
		case "cap.mem":
			...
			issues = append(
				...
				requireMemory100RawEvidence(
					...
					"non_claims",
					operation.NonClaims,
					"no arbitrary external pointer safety claim",
				)...)
		}
	}
	return issues
...
func requireMemory100RawEvidence(
	operation string,
	field string,
	values []string,
	wants ...string,
	...
	for _, want := range wants {
		if !strings.Contains(joined, strings.ToLower(want)) {
			issues = append(
				issues,
				fmt.Sprintf(
					"raw memory contract operation %s %s missing %q",
					operation,
					field,
					want,
				),
...
func validateMemory100AllocationLowering(path string, gitHead string) []string {
	...
	if report.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf("allocation lowering status is %q, want pass", report.Status),
		)
	}
	if gitHead != "" && report.GitHead != gitHead {
		issues = append(
			issues,
			fmt.Sprintf(
				"allocation lowering git_head %s does not match Memory100 git_head %s",
				report.GitHead,
				gitHead,
			),
		)
	...
	for _, name := range []string{
		"stack_trusted_no_escape",
		"heap_fallback_blocker",
		"copy_blocker",
		"lowering_storage_match",
	} {
		...
		if strings.TrimSpace(decision.PlannedStorage) == "" ||
			strings.TrimSpace(decision.ActualLoweringStorage) == "" {
			issues = append(
				issues,
				fmt.Sprintf("allocation lowering decision %s missing planned/actual storage", name),
			)
		}
		if len(nonEmptyMemory100Strings(decision.SourceArtifacts)) == 0 {
			issues = append(
				issues,
				fmt.Sprintf("allocation lowering decision %s missing source_artifacts", name),
			)
		}
		...
		default:
			issues = append(
				issues,
				fmt.Sprintf(
					"allocation lowering decision %s has unknown status %q",
					name,
					decision.Status,
				),
			)
		...
		if planned != "" && actual != "" && !strings.EqualFold(planned, actual) {
			if strings.TrimSpace(decision.BlockerArtifact) == "" {
				issues = append(
					issues,
					fmt.Sprintf(
						"allocation lowering decision %s planned/actual mismatch %s -> %s requires blocker_artifact",
						name,
						planned,
						actual,
					),
			...
			issues = append(
				issues,
				requireMemory100AllocationField(name, "blocker_reason", decision.BlockerReason)...)
			issues = append(
				issues,
				requireMemory100AllocationField(name, "budget_impact", decision.BudgetImpact)...)
			issues = append(
				issues,
				requireMemory100AllocationField(name, "grade_impact", decision.GradeImpact)...)
		}
		if status == "not_observed" {
			if strings.TrimSpace(decision.ProofArtifact) != "" ||
				strings.TrimSpace(decision.BlockerArtifact) != "" {
				issues = append(
					issues,
					fmt.Sprintf(
						"allocation lowering decision %s is not_observed but carries proof/blocker artifact",
						name,
					),
				)
			}
			if len(nonEmptyMemory100Strings(decision.CoveredSiteIDs)) > 0 {
				issues = append(
					issues,
					fmt.Sprintf(
						"allocation lowering decision %s is not_observed but carries covered_site_ids",
						name,
					),
				)
			}
		...
		if len(nonEmptyMemory100Strings(decision.CoveredSiteIDs)) == 0 {
			issues = append(
				issues,
				fmt.Sprintf("allocation lowering decision %s missing covered_site_ids", name),
			)
		}
		if status == "proven" && strings.TrimSpace(decision.ProofArtifact) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"allocation lowering decision %s status proven requires proof_artifact",
					name,
				),
			)
		}
		if status == "blocked" && strings.TrimSpace(decision.BlockerArtifact) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"allocation lowering decision %s status blocked requires blocker_artifact",
					name,
				),
			)
		}
		if strings.TrimSpace(decision.ProofArtifact) == "" &&
			...
			issues = append(
				issues,
				fmt.Sprintf(
					"allocation lowering decision %s missing proof_artifact or blocker_artifact",
					name,
				),
			)
		}
		...
		case "heap_fallback_blocker":
			if !strings.Contains(
				filepath.ToSlash(decision.BlockerArtifact),
				"ram-contract/heap-blockers.json",
			) {
				issues = append(
					issues,
					fmt.Sprintf(
						("allocation lowering decision %s blocker_artifact must reference "+
							"ram-contract/heap-blockers.json"),
						name,
					),
				)
			...
			issues = append(
				issues,
				requireMemory100AllocationCoverage(
					name,
					decision.ValidatorCoverage,
...
func requireMemory100AllocationCoverage(
	decision string,
	values []string,
	wants ...string,
) []string {
	...
	for _, want := range wants {
		if !strings.Contains(joined, strings.ToLower(want)) {
			issues = append(
				issues,
				fmt.Sprintf(
					"allocation lowering decision %s validator_coverage missing %q",
					decision,
					want,
				),
			)
