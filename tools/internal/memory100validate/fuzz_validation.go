...
func validateMemory100MemoryFuzzBundle(dir string, gitHead string) []string {
	...
	if err != nil {
		issues = append(
			issues,
			fmt.Sprintf("memory fuzz summary.md missing or unreadable: %v", err),
		)
	} else {
	...
	if err := readMemory100StrictJSON(
		filepath.Join(dir, "island-proof-fuzz-summary.json"),
		&proofSummary,
	); err != nil {
		issues = append(
			issues,
			fmt.Sprintf("memory fuzz island-proof-fuzz-summary.json invalid: %v", err),
		)
	} else {
	...
	issues = append(
		issues,
		validateMemory100MemoryFuzzHashManifest(filepath.Join(dir, "artifact-hashes.json"), dir)...)
	return issues
}
...
func validateMemory100MemoryFuzzSummary(
	summary memory100MemoryFuzzSummary,
	gitHead string,
	artifactDir string,
) []string {
	var issues []string
	...
	if summary.SchemaVersion != "tetra.memory-fuzz-short.summary.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"memory fuzz summary.json schema_version is %q, want tetra.memory-fuzz-short.summary.v1",
				summary.SchemaVersion,
			),
		)
	}
	if summary.Kind != "tier1_short_ci_smoke" || summary.Tier != "tier1_short_ci_smoke" ||
		summary.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf(
				("memory fuzz summary.json identity/status must record passing "+
					"Tier 1 short CI smoke, got kind=%q tier=%q status=%q"),
				summary.Kind,
				summary.Tier,
				summary.Status,
			),
	...
	issues = append(
		issues,
		validateMemory100MemoryFuzzReproducibilitySeeds(summary.ReproducibilitySeeds)...)
	for key, want := range map[string]string{
		...
		if got != want {
			issues = append(
				issues,
				fmt.Sprintf("memory fuzz summary.json artifact %s is %q, want %q", key, got, want),
			)
		}
		if strings.TrimSpace(got) != "" {
			if err := validateMemory100SafeRel(got); err != nil {
				issues = append(
					issues,
					fmt.Sprintf("memory fuzz summary.json artifact %s path invalid: %v", key, err),
				)
			}
		}
	}
	...
	for _, command := range summary.Commands {
		if command.Status != "pass" {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz summary.json command %s status is %q, want pass",
					command.Name,
					command.Status,
				),
			)
		...
		case "memory-fuzz-short":
			if strings.Contains(command.Command, "go run ./tools/cmd/memory-fuzz-short") &&
				memory100CommandContainsAnyPath(command.Command, "--report-dir", artifactDirs) &&
				strings.Contains(command.Command, "--git-head "+gitHead) {
				sawRunner = true
			}
		case "validate-memory-fuzz-oracle":
			if strings.Contains(
				...
				memory100CommandContainsAnyPath(command.Command, "--report", oraclePaths) &&
				memory100CommandContainsAnyPath(command.Command, "--artifact-dir", artifactDirs) &&
				strings.Contains(command.Command, "--current-git-head "+gitHead) {
				sawValidator = true
			}
		}
	}
	if !sawRunner {
		issues = append(
			issues,
			fmt.Sprintf(
				("memory fuzz summary.json missing memory-fuzz-short same-commit "+
					"command provenance for current artifact dir %s"),
				artifactDirLabel,
			),
		)
	}
	if !sawValidator {
		issues = append(
			issues,
			fmt.Sprintf(
				("memory fuzz summary.json missing validate-memory-fuzz-oracle "+
					"same-commit command provenance for current artifact dir %s"),
				artifactDirLabel,
			),
		)
	}
...
func validateMemory100MemoryFuzzReproducibilitySeeds(seeds []string) []string {
	...
	if len(seeds) < 12 {
		issues = append(
			issues,
			fmt.Sprintf(
				"memory fuzz summary.json reproducibility_seeds has %d entries, want at least 12 for v0-v11",
				len(seeds),
			),
		)
	}
	...
	for _, seed := range seeds {
		...
		if text == "" {
			issues = append(
				issues,
				"memory fuzz summary.json reproducibility_seeds contains empty seed",
			)
			continue
		}
		...
		for _, forbidden := range []string{"todo", "placeholder", "fake", "mock"} {
			if strings.Contains(lower, forbidden) {
				issues = append(
					issues,
					fmt.Sprintf(
						"memory fuzz summary.json reproducibility_seeds contains forbidden marker %q",
						forbidden,
					),
				)
			}
		...
		if seen[text] {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz summary.json reproducibility_seeds duplicate seed %q",
					text,
				),
			)
		}
	...
	for i := 0; i < 12; i++ {
		if !strings.Contains(joined, fmt.Sprintf(":v%d:", i)) {
			issues = append(
				issues,
				fmt.Sprintf("memory fuzz summary.json reproducibility_seeds missing v%d seed", i),
			)
		}
	}
	return issues
...
func validateMemory100MemoryFuzzFailureClassification(summary memory100MemoryFuzzSummary) []string {
	...
	for _, count := range counts {
		if count.value == nil {
			issues = append(
				issues,
				fmt.Sprintf("memory fuzz summary.json %s is required", count.name),
			)
			continue
		}
		if *count.value < 0 {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz summary.json %s is %d, want non-negative",
					count.name,
					*count.value,
				),
			)
	...
	if values["classified_failures"]+values["unclassified_failures"] != values["observed_failures"] {
		issues = append(
			issues,
			("memory fuzz summary.json classified_failures + unclassified_" +
				"failures must equal observed_failures"),
		)
	}
	if values["release_blocking_failures"] > values["observed_failures"] {
		issues = append(
			issues,
			fmt.Sprintf(
				"memory fuzz summary.json release_blocking_failures is %d, exceeds observed_failures %d",
	...
	if values["unclassified_failures"] != 0 {
		issues = append(
			issues,
			fmt.Sprintf(
				"memory fuzz summary.json unclassified_failures is %d, want 0",
				values["unclassified_failures"],
			),
		)
	}
	if summary.Status == "pass" &&
		...
		issues = append(
			issues,
			fmt.Sprintf(
				("memory fuzz summary.json passing Tier 1 summary must record "+
					"zero observed/classified/release_blocking failures, got observed=%d "+
					"classified=%d release_blocking=%d"),
				values["observed_failures"],
...
func validateMemory100IslandProofFuzzSummary(summary memory100IslandProofFuzzSummary) []string {
	...
	if summary.SchemaVersion != "tetra.island-proof-fuzz-summary.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				("memory fuzz island-proof-fuzz-summary.json schema_version is %q,"+
					" want tetra.island-proof-fuzz-summary.v1"),
				summary.SchemaVersion,
			),
		)
	}
	if summary.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf(
				"memory fuzz island-proof-fuzz-summary.json status is %q, want pass",
				summary.Status,
			),
		)
	}
	if summary.Total < 10 {
		issues = append(
			issues,
			fmt.Sprintf(
				"memory fuzz island-proof-fuzz-summary.json total is %d, want at least 10",
				summary.Total,
			),
		)
	}
	if summary.Accepted != 0 || summary.Rejected != summary.Total {
		issues = append(
			issues,
			fmt.Sprintf(
				("memory fuzz island-proof-fuzz-summary.json counts total=%d "+
					"rejected=%d accepted=%d, want all rejected"),
				summary.Total,
				summary.Rejected,
				summary.Accepted,
			),
	...
	for _, c := range summary.Cases {
		if c.Status != "rejected" {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz island proof fuzz case %s status is %q, want rejected",
					c.Name,
					c.Status,
				),
			)
	...
	for _, name := range []string{
		...
		if !seen[name] {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz island-proof-fuzz-summary.json missing mutation case %s",
					name,
				),
			)
		}
...
func validateMemory100MemoryFuzzHashManifest(hashPath string, dir string) []string {
	...
	if manifest.Schema != memory100HashSchema {
		issues = append(
			issues,
			fmt.Sprintf(
				"memory fuzz artifact-hashes.json schema is %q, want %s",
				manifest.Schema,
				memory100HashSchema,
			),
		)
	...
	if manifest.Root != "." {
		issues = append(
			issues,
			fmt.Sprintf("memory fuzz artifact-hashes.json root is %q, want .", manifest.Root),
		)
	}
	...
	for _, artifact := range manifest.Artifacts {
		if err := validateMemory100SafeRel(artifact.Path); err != nil {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz artifact-hashes.json path %q is invalid: %v",
					artifact.Path,
					err,
				),
			)
		...
		if lastPath != "" && artifact.Path < lastPath {
			issues = append(
				issues,
				"memory fuzz artifact-hashes.json artifacts must be sorted by path",
			)
		}
		...
		if _, ok := seen[artifact.Path]; ok {
			issues = append(
				issues,
				fmt.Sprintf("duplicate memory fuzz hash entry for %s", artifact.Path),
			)
		}
		...
		if err != nil {
			issues = append(
				issues,
				fmt.Sprintf("hash memory fuzz artifact %s: %v", artifact.Path, err),
			)
			continue
		}
		if actual.Size != artifact.Size {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz size mismatch for %s: got %d want %d",
					artifact.Path,
					actual.Size,
					artifact.Size,
				),
		...
		if actual.SHA256 != artifact.SHA256 {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz sha256 mismatch for %s: got %s want %s",
					artifact.Path,
					actual.SHA256,
					artifact.SHA256,
				),
		...
		if actual.Schema != artifact.Schema {
			issues = append(
				issues,
				fmt.Sprintf(
					"memory fuzz schema mismatch for %s: got %q want %q",
					artifact.Path,
					actual.Schema,
					artifact.Schema,
				),
	...
	for _, rel := range []string{
		...
		if _, ok := seen[rel]; !ok {
			issues = append(
				issues,
				fmt.Sprintf("missing memory fuzz hash manifest entry for %s", rel),
			)
		}
	}
...
func validateMemory100ProofTransitionReport(path string, gitHead string) []string {
	...
	if report.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf("proof transition report status is %q, want pass", report.Status),
		)
	}
	if gitHead != "" && report.GitHead != gitHead {
		issues = append(
			issues,
			fmt.Sprintf(
				"proof transition report git_head %s does not match Memory100 git_head %s",
				report.GitHead,
				gitHead,
			),
		)
	...
	for i, row := range report.Rows {
		...
		if !memory100KnownProofTransition(transition) {
			issues = append(
				issues,
				fmt.Sprintf(
					"proof transition row %s has unknown transition %q",
					name,
					row.Transition,
				),
			)
		...
		if len(nonEmptyMemory100Strings(row.SourceArtifacts)) == 0 {
			issues = append(
				issues,
				fmt.Sprintf("proof transition row %s missing source_artifacts", name),
			)
		}
		...
		case "preserved", "refined":
			if strings.TrimSpace(row.BeforeArtifact) == "" ||
				strings.TrimSpace(row.AfterArtifact) == "" {
				issues = append(
					issues,
					fmt.Sprintf(
						"proof transition row %s transition %s requires before_artifact and after_artifact",
						name,
						transition,
					),
				)
		...
		case "invalidated", "requires_recheck":
			...
			if !strings.Contains(action, "recheck") && !strings.Contains(action, "block") {
				issues = append(
					issues,
					fmt.Sprintf(
						"proof transition row %s transition %s requires consumer_action with recheck or block",
						name,
						transition,
					),
				)
		...
		case "new":
			if strings.TrimSpace(row.AfterArtifact) == "" {
				issues = append(
					issues,
					fmt.Sprintf(
						"proof transition row %s transition new requires after_artifact",
						name,
					),
				)
			}
	...
	for _, transition := range []string{
		...
		if !seenTransitions[transition] {
			issues = append(
				issues,
				fmt.Sprintf("proof transition report missing transition %s", transition),
			)
		}
	}
	for name, wantTransition := range required {
		...
		if row.Transition != wantTransition {
			issues = append(
				issues,
				fmt.Sprintf(
					"proof transition row %s transition is %q, want %q",
					name,
					row.Transition,
					wantTransition,
				),
...
func validateMemory100ProofTransitionEvidence(row memory100ProofTransitionRow) []string {
	...
	for _, want := range wants[row.Name] {
		if !strings.Contains(text, want) {
			issues = append(
				issues,
				fmt.Sprintf("proof transition row %s evidence missing %q", row.Name, want),
			)
		}
	}
	return issues
...
func validateMemory100RuntimeMemoryContractTargetMatrix(
	path string,
	gitHead string,
	targetMatrix []string,
) []string {
	return validateMemory100RuntimeMemoryContract(
		path,
		gitHead,
		nonEmptyMemory100Strings(targetMatrix),
	)
}

func validateMemory100RuntimeMemoryContract(
	path string,
	gitHead string,
	targetMatrix []string,
) []string {
	...
	if report.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf("runtime memory contract status is %q, want pass", report.Status),
		)
	}
	if gitHead != "" && report.GitHead != gitHead {
		issues = append(
			issues,
			fmt.Sprintf(
				"runtime memory contract git_head %s does not match Memory100 git_head %s",
				report.GitHead,
				gitHead,
			),
		)
	...
	for _, want := range []string{
		...
		if !memory100ContainsFold(report.NonClaims, want) {
			issues = append(
				issues,
				fmt.Sprintf("runtime memory contract missing non_claim %q", want),
			)
		}
	}
	...
	for i, row := range report.Rows {
		...
		if strings.TrimSpace(row.RuntimeStatus) == "" || strings.TrimSpace(row.MemoryRun) == "" ||
			strings.TrimSpace(row.MemoryClaimLevel) == "" {
			issues = append(
				issues,
				fmt.Sprintf(
					"runtime memory row %s missing runtime_status, memory_run, or memory_claim_level",
					target,
				),
			)
		}
		...
		if len(nonEmptyMemory100Strings(row.SourceArtifacts)) == 0 {
			issues = append(
				issues,
				fmt.Sprintf("runtime memory row %s missing source_artifacts", target),
			)
		}
		if len(nonEmptyMemory100Strings(row.Tests)) == 0 {
		...
		if row.IncludedInMemory100TargetMatrix {
			if row.MemoryClaimLevel != "production_host_runtime" {
				issues = append(
					issues,
					fmt.Sprintf(
						"runtime memory row %s is included but claim level is %q, want production_host_runtime",
						target,
			...
			if row.RuntimeStatus != "production" || row.MemoryRun != "yes" {
				issues = append(
					issues,
					fmt.Sprintf(
						"runtime memory row %s is included but runtime_status=%q memory_run=%q, want production/yes",
						target,
						row.RuntimeStatus,
						row.MemoryRun,
					),
			...
			if !memory100ContainsFold(
				append(append([]string{row.Evidence}, row.SourceArtifacts...), row.Tests...),
				"runtime hardening",
			) {
				issues = append(
					issues,
					fmt.Sprintf(
						"runtime memory row %s included production evidence must mention runtime hardening",
						target,
					),
				)
			}
			if !memory100ContainsFold(
				...
				issues = append(
					issues,
					fmt.Sprintf(
						"runtime memory row %s included production evidence must mention runtimeabi",
						target,
					),
				)
			}
			...
			if strings.TrimSpace(row.ExcludedReason) == "" {
				issues = append(
					issues,
					fmt.Sprintf("runtime memory row %s is excluded but missing excluded_reason", target),
				)
			}
			if row.MemoryClaimLevel == "production_host_runtime" {
				issues = append(
					issues,
					fmt.Sprintf("runtime memory row %s is excluded but claims production_host_runtime", target),
				)
			}
		}
	}
	for target, wantClaim := range required {
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf("runtime memory contract missing target row %s", target),
			)
			continue
		}
		if row.MemoryClaimLevel != wantClaim {
			issues = append(
				issues,
				fmt.Sprintf(
					"runtime memory row %s claim level is %q, want %q",
					target,
					row.MemoryClaimLevel,
					wantClaim,
				),
		...
		if target == "linux-x64" && !row.IncludedInMemory100TargetMatrix {
			issues = append(
				issues,
				"runtime memory row linux-x64 must be included in Memory100 target matrix",
			)
		}
		if target != "linux-x64" && row.IncludedInMemory100TargetMatrix {
			issues = append(
				issues,
				fmt.Sprintf(
					("runtime memory row %s must not be included in Memory100 target "+
						"matrix without target-host evidence"),
					target,
				),
			)
		}
	...
	if len(targetMatrix) > 0 {
		...
		for _, missing := range memory100MissingSetKeys(matrixSet, included) {
			issues = append(
				issues,
				fmt.Sprintf(
					"Memory100 target_matrix target %s missing included runtime memory row",
					missing,
				),
			)
		}
		for _, extra := range memory100MissingSetKeys(included, matrixSet) {
			issues = append(
				issues,
				fmt.Sprintf(
					"runtime memory included target %s missing from Memory100 target_matrix",
					extra,
				),
			)
		}
