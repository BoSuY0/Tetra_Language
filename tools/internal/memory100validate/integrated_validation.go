...
func validateMemory100IntegratedSurfaceEvidence(integratedDir string, gitHead string) []string {
	...
	issues = append(
		issues,
		validateMemory100IntegratedSurfaceReleaseSummary(
			filepath.Join(surfaceDir, "surface-release-summary.json"),
			gitHead,
		)...)
	issues = append(
		issues,
		validateMemory100NestedHashManifest(
			filepath.Join(surfaceDir, "artifact-hashes.json"),
			surfaceDir,
			"integrated surface release artifact-hashes.json",
		)...)
	issues = append(
		issues,
		validateMemory100IntegratedSurfaceExperimentalRegression(
			filepath.Join(integratedDir, "surface-experimental-regression"),
		)...)
	issues = append(
		issues,
		validateMemory100IntegratedSafeViewLifetimeSummary(
			filepath.Join(integratedDir, "safe-view-lifetime", "safe-view-lifetime-summary.json"),
		)...)
	issues = append(
		issues,
		validateMemory100IntegratedSurfaceAPIStabilitySummary(
			filepath.Join(
				integratedDir,
...
func validateMemory100IntegratedSurfaceExperimentalRegression(dir string) []string {
	issues := validateMemory100NestedHashManifest(
		filepath.Join(dir, "artifact-hashes.json"),
		dir,
		"integrated surface experimental artifact-hashes.json",
	)
	...
	for _, rel := range paths {
		...
		if err := readMemory100JSON(path, &envelope); err != nil {
			issues = append(
				issues,
				fmt.Sprintf("integrated surface experimental JSON %s invalid: %v", rel, err),
			)
			continue
		}
		...
		if err != nil {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated surface experimental runtime report %s unreadable: %v",
					rel,
					err,
				),
			)
		...
		if err := surface.ValidateReport(raw); err != nil {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated surface experimental runtime report %s invalid: %v",
					rel,
					err,
				),
			)
...
func validateMemory100IntegratedSurfaceAPIStabilitySummary(path string) []string {
	...
	if err := readMemory100JSON(path, &summary); err != nil {
		return []string{
			fmt.Sprintf("integrated surface API stability summary missing or invalid: %v", err),
		}
	}
	var issues []string
	if summary.Schema != "tetra.surface.api-stability.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated surface API stability summary schema is %q, want tetra.surface.api-stability.v1",
				summary.Schema,
			),
		)
	}
	if summary.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated surface API stability summary status is %q, want pass",
				summary.Status,
			),
		)
	}
	if summary.ReleaseScope != "surface-v1-linux-web" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated surface API stability summary release_scope is %q, want surface-v1-linux-web",
	...
	if !summary.DocsManifestValidated {
		issues = append(
			issues,
			"integrated surface API stability summary docs_manifest_validated must be true",
		)
	}
	return issues
}
...
func validateMemory100IntegratedSafeViewLifetimeSummary(path string) []string {
	...
	if err := readMemory100JSON(path, &summary); err != nil {
		return []string{
			fmt.Sprintf("integrated safe-view lifetime summary missing or invalid: %v", err),
		}
	}
	var issues []string
	if summary.Schema != "tetra.safe-view-lifetime.gate.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated safe-view lifetime summary schema is %q, want tetra.safe-view-lifetime.gate.v1",
				summary.Schema,
			),
		)
	}
	if summary.Status != "pass" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated safe-view lifetime summary status is %q, want pass",
				summary.Status,
			),
		)
	}
	...
	if !summary.ReleaseBlocking {
		issues = append(
			issues,
			"integrated safe-view lifetime summary release_blocking must be true",
		)
	}
	return issues
}
...
func validateMemory100IntegratedSurfaceReleaseSummary(path string, gitHead string) []string {
	...
	if err := readMemory100JSON(path, &summary); err != nil {
		return []string{
			fmt.Sprintf("integrated surface release summary missing or invalid: %v", err),
		}
	}
	var issues []string
	if summary.Schema != "tetra.surface.release.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated surface release summary schema is %q, want tetra.surface.release.v1",
				summary.Schema,
			),
		)
	}
	if summary.Status != "current" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated surface release summary status is %q, want current",
				summary.Status,
			),
		)
	}
	if gitHead != "" && summary.GitHead != gitHead {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated surface release summary git_head %s does not match Memory100 git_head %s",
				summary.GitHead,
				gitHead,
			),
		)
	...
	if summary.ReleaseScope != "surface-v1-linux-web" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated surface release summary release_scope is %q, want surface-v1-linux-web",
				summary.ReleaseScope,
			),
		)
	}
...
func validateMemory100IntegratedNestedMemory(integratedDir string, gitHead string) []string {
	...
	if err := readMemory100JSON(productionPath, &envelope); err != nil {
		issues = append(
			issues,
			fmt.Sprintf("integrated memory production report envelope: %v", err),
		)
	} else {
		if memory100SchemaOf(envelope) != "tetra.memory.production.v1" {
			issues = append(
				issues,
				fmt.Sprintf(("integrated memory production report schema is %q, want "+
					"tetra.memory.production.v1"), memory100SchemaOf(envelope)),
			)
		}
		if gitHead != "" && envelope.GitHead != gitHead {
			issues = append(
				issues,
				fmt.Sprintf(("integrated memory production report git_head %s does not match "+
					"Memory100 git_head %s"), envelope.GitHead, gitHead),
			)
		}
	}
	for _, issue := range validateMemory100MemoryReleaseManifest(
	...
	issues = append(
		issues,
		validateMemory100NestedHashManifestWithRequired(
			filepath.Join(memoryDir, "artifact-hashes.json"),
			memoryDir,
			"integrated memory artifact-hashes.json",
...
func validateMemory100IntegratedIslandsDebugSmoke(path string, gitHead string) []string {
	...
	if report.Target != "linux-x64" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated islands debug smoke target is %q, want linux-x64",
				report.Target,
			),
		)
	}
	if report.GitHead != "" && gitHead != "" && !memory100SameGitHead(report.GitHead, gitHead) {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated islands debug smoke git_head %s does not match Memory100 git_head %s",
				report.GitHead,
				gitHead,
			),
		)
	...
	if report.Total == nil || report.Passed == nil || report.Failed == nil {
		...
		if *report.Total != len(report.Cases) || *report.Passed != passed || *report.Failed != failed {
			issues = append(
				issues,
				fmt.Sprintf(("integrated islands debug smoke counts mismatch: got total=%d "+
					"passed=%d failed=%d computed total=%d passed=%d failed=%d"), *report.Total, *report.Passed, *report.Failed, len(report.Cases), passed, failed),
			)
		}
	}
	foundTrap := false
	for _, c := range report.Cases {
		...
		if c.SrcPath != "examples/memory/islands/islands_overflow.tetra" {
			issues = append(
				issues,
				fmt.Sprintf(
					("integrated islands debug smoke trap src_path is %q, want "+
		...
		if c.ExpectedExit == 0 {
			issues = append(
				issues,
				"integrated islands debug smoke trap expected_exit must be non-zero",
			)
		}
		if c.ActualExit == nil {
			...
			issues = append(
				issues,
				fmt.Sprintf("integrated islands debug smoke trap actual_exit=%d, want %d", *c.ActualExit, c.ExpectedExit),
			)
		}
		if !c.Ran || !c.Pass {
...
func validateMemory100NestedRAMContract(ramDir string, gitHead string, label string) []string {
	...
	for _, issue := range validateMemory100RAMContractReleaseManifest(
	...
	issues = append(
		issues,
		validateMemory100NestedHashManifest(
			filepath.Join(ramDir, "artifact-hashes.json"),
			ramDir,
			label+" RAM contract artifact-hashes.json",
		)...)
	for _, issue := range validateMemory100RAMContractFuzzOracle(
...
func validateMemory100NestedHashManifestWithRequired(
	hashPath string,
	dir string,
	label string,
	...
	if manifest.Schema != memory100HashSchema {
		issues = append(
			issues,
			fmt.Sprintf("%s schema is %q, want %s", label, manifest.Schema, memory100HashSchema),
		)
	}
	if manifest.Root != "." {
	...
	for _, artifact := range manifest.Artifacts {
		if err := validateMemory100SafeRel(artifact.Path); err != nil {
			issues = append(
				issues,
				fmt.Sprintf("%s path %q is invalid: %v", label, artifact.Path, err),
			)
			continue
		}
		...
		if err != nil {
			issues = append(
				issues,
				fmt.Sprintf("hash %s artifact %s: %v", label, artifact.Path, err),
			)
			continue
		}
...
func validateMemory100IntegratedManifest(path string, gitHead string) []string {
	...
	if manifest.Schema != "tetra.memory-islands-surface.production-gate.v1" {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated manifest schema is %q, want tetra.memory-islands-surface.production-gate.v1",
				manifest.Schema,
			),
		)
	}
	...
	if gitHead != "" && manifest.GitHead != gitHead {
		issues = append(
			issues,
			fmt.Sprintf(
				"integrated manifest git_head %s does not match Memory100 git_head %s",
				manifest.GitHead,
				gitHead,
			),
		)
	...
	issues = append(
		issues,
		validateMemory100IntegratedCommands(manifest.Commands, integratedDir, gitHead)...)
	issues = append(issues, validateMemory100IntegratedArtifactRefs(manifest.Artifacts)...)
	return issues
}

func validateMemory100IntegratedCommands(
	commands []memory100Command,
	integratedDir string,
	gitHead string,
) []string {
	...
	for name, fragment := range requiredMemory100IntegratedCommands {
		...
		if !strings.Contains(text, fragment) {
			issues = append(
				issues,
				fmt.Sprintf("integrated manifest command %s must contain %q", name, fragment),
			)
		}
	}
	issues = append(
		issues,
		validateMemory100IntegratedCommandProvenance(seen, integratedDir, gitHead)...)
	return issues
}

func validateMemory100IntegratedCommandProvenance(
	...
	pathRequirements := []pathRequirement{
		...
		{name: "surface-release-gate", flag: "--report-dir", rel: "surface-release-v1"},
		{
			...
			flag: "--report-dir",
			rel:  "surface-experimental-regression",
		},
	...
	for _, requirement := range pathRequirements {
		...
		if !memory100CommandContainsAnyPath(
			...
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated manifest command %s must use %s under the current integrated report dir for %s",
					requirement.name,
					requirement.flag,
					requirement.rel,
				),
	...
	if text := strings.TrimSpace(commands["integrated-release-validator"]); text != "" &&
		strings.TrimSpace(gitHead) != "" {
		if !strings.Contains(text, "--current-git-head "+gitHead) {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated manifest command integrated-release-validator must use --current-git-head %s",
					gitHead,
				),
			)
		}
	...
	for _, artifact := range artifacts {
		...
		if !requiredKinds[artifact.Kind] {
			issues = append(
				issues,
				fmt.Sprintf(
					"unexpected integrated manifest artifact kind %s at %s",
					artifact.Kind,
					artifact.Path,
				),
			)
	...
	for _, required := range requiredMemory100IntegratedArtifacts {
		...
		if artifact.Path != required.Path {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated manifest artifact %s path is %q, want %s",
					required.Kind,
					artifact.Path,
					required.Path,
				),
		...
		if required.Schema != "" && artifact.Schema != required.Schema {
			issues = append(
				issues,
				fmt.Sprintf(
					"integrated manifest artifact %s schema is %q, want %s",
					required.Kind,
					artifact.Schema,
					required.Schema,
				),
