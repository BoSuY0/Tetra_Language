...
func reportPathParts(reportPath string) ([]string, error) {
	...
	for _, part := range rawParts {
		...
		if part == ".." {
			return nil, fmt.Errorf("parent traversal is not allowed in required report path: %s", reportPath)
		}
...
func rejectSymlinkComponents(root string, parts []string, reportPath string) error {
	...
	for i, part := range parts {
		...
		if info.Mode()&os.ModeSymlink != 0 {
			...
			return fmt.Errorf("symlink required report path component is not allowed: %s", reportPath)
		}
	}
	return nil
}
