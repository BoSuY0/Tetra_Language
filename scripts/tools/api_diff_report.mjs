...
function usage() {
  console.error(
    "Usage: node scripts/tools/api_diff_report.mjs --docs <path> " +
      "[--baseline <path>] [--diff-out <path>] [--write-baseline] " +
      "[--enforce <none|no-breaking|no-change>]",
  );
}
...
function entryIdentity(section, entry) {
  ...
  const keywordMatch =
    /^(const|val|var|struct|enum|protocol|extension|impl|state|view)\s+([^(:\s]+)/.exec(trimmed);
  if (keywordMatch) {
...
function parseAPIDocs(md) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const raw = line.slice("<!-- tetra-api-metadata:".length, line.length - "-->".length).trim();
  ...
  switch (kind) {
    case "added":
      return {
        ...
        review_note:
          "New API surface; confirm it is intentional v1 scope before updating the baseline.",
      };
    case "removed":
      return {
        review_status: "breaking_requires_review",
        review_note:
          "Previously baselined API surface was removed; restore it or approve " +
          "a breaking baseline update.",
      };
    case "changed":
      return {
        review_status: "breaking_requires_review",
        review_note:
          "Baselined API signature or metadata changed; restore it or approve " +
...
function buildReviewSummary(totalChanges) {
  return {
    ...
    baseline_policy: "docs/spec/policy/api_diff_policy.md",
    checklist: [
      "Classify added entries as intentional v1 scope or remove them before baseline update.",
      "Classify changed entries as deliberate compatibility decisions or restore " +
        "the previous signature.",
      "Classify removed entries as deliberate breaking decisions or restore " +
...
function buildDiff(baseline, parsed, baselinePath, candidatePath) {
  ...
  const ids = [...new Set([...baseSymbols.keys(), ...candSymbols.keys()])].sort((a, b) =>
    a.localeCompare(b),
  );
...
function main() {
  ...
  if (parsed.metadata.api_hash !== parsed.apiSurfaceSHA256) {
    throw new Error(
      `api hash mismatch: metadata=${parsed.metadata.api_hash} computed=${parsed.apiSurfaceSHA256}`,
    );
  }
  if (Number(parsed.metadata.entry_count) !== parsed.symbols.length) {
    throw new Error(
      `entry_count mismatch: metadata=${parsed.metadata.entry_count} ` +
        `parsed=${parsed.symbols.length}`,
    );
  }
