#!/usr/bin/env bash
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo_root="$(cd "$script_dir/../../.." && pwd)"
cd "$repo_root"

report_dir=""
release_artifact="tetra.release.v0_2_0.gate-report.v1"
formatter_gate_command='./tetra fmt --check examples lib __rt compiler/selfhostrt && go test ./compiler/... ./cli/... -run "Format|Formatter|Comment" -count=1'

usage() {
  cat << 'USAGE'
Usage: bash scripts/release/v0_2_0/gate.sh [--report-dir DIR]

Notes:
- This gate is for the current v0.2.0 release line.
- It requires ./tetra version == v0.2.0.
- Artifact mapping: tetra.release.v0_2_0.gate-report.v1
- Formatter closure gate: ./tetra fmt --check examples lib __rt compiler/selfhostrt && go test ./compiler/... ./cli/... -run "Format|Formatter|Comment" -count=1
USAGE
}

check_report_dir_fresh() {
  if [[ (-e "$report_dir" || -L "$report_dir") && ! -d "$report_dir" ]]; then
    echo "release_v0_2_0_gate: refusing to use non-directory report path: $report_dir" >&2
    echo "release_v0_2_0_gate: choose a fresh --report-dir directory" >&2
    exit 2
  fi
  if [[ ! -d "$report_dir" ]]; then
    return 0
  fi
  local first_entry
  local find_report_dir="$report_dir"
  if [[ "$find_report_dir" == -* ]]; then
    find_report_dir="./$find_report_dir"
  fi
  first_entry="$(find -H "$find_report_dir" -mindepth 1 -print -quit)"
  if [[ -n "$first_entry" ]]; then
    echo "release_v0_2_0_gate: refusing to reuse non-empty report directory: $report_dir" >&2
    echo "release_v0_2_0_gate: choose a fresh --report-dir so stale reports cannot be reused" >&2
    exit 2
  fi
}

while [[ $# -gt 0 ]]; do
  case "$1" in
    --report-dir)
      if [[ $# -lt 2 || -z "${2:-}" ]]; then
        echo "release_v0_2_0_gate: --report-dir requires a directory" >&2
        usage >&2
        exit 2
      fi
      report_dir="$2"
      shift 2
      ;;
    -h | --help)
      usage
      exit 0
      ;;
    *)
      echo "release_v0_2_0_gate: unknown option $1" >&2
      usage >&2
      exit 2
      ;;
  esac
done

if [[ -n "$report_dir" ]]; then
  if [[ "$report_dir" == -* ]]; then
    report_dir="./$report_dir"
  fi
  check_report_dir_fresh
fi

echo "release_v0_2_0_gate: bootstrapping local binaries before version preflight" >&2
bash scripts/dev/bootstrap.sh >&2

version="$(./tetra version 2> /dev/null || true)"
if [[ "$version" != "v0.2.0" ]]; then
  echo "release_v0_2_0_gate: blocked: expected ./tetra version to be v0.2.0, got '${version:-<missing>}'" >&2
  echo "release_v0_2_0_gate: promote version metadata and rerun bootstrap before final release checks." >&2
  exit 1
fi

echo "release_v0_2_0_gate: using shared release gate workflow with v0.2.0 boundaries" >&2
echo "release_v0_2_0_gate: artifact mapping $release_artifact" >&2
echo "release_v0_2_0_gate: formatter closure gate: $formatter_gate_command" >&2
if [[ -n "$report_dir" ]]; then
  exec env \
    TETRA_RELEASE_GATE_VERSION=v0.2.0 \
    TETRA_RELEASE_GATE_ARTIFACT="$release_artifact" \
    TETRA_RELEASE_GATE_COMMAND="bash scripts/release/v0_2_0/gate.sh" \
    TETRA_RELEASE_GATE_ACTOR_DIAGNOSTIC_CONTAINS="actor declarations currently support state fields and func methods only" \
    bash "$repo_root/scripts/release/v0_1_3/gate.sh" --report-dir "$report_dir"
fi
exec env \
  TETRA_RELEASE_GATE_VERSION=v0.2.0 \
  TETRA_RELEASE_GATE_ARTIFACT="$release_artifact" \
  TETRA_RELEASE_GATE_COMMAND="bash scripts/release/v0_2_0/gate.sh" \
  TETRA_RELEASE_GATE_ACTOR_DIAGNOSTIC_CONTAINS="actor declarations currently support state fields and func methods only" \
  bash "$repo_root/scripts/release/v0_1_3/gate.sh"
