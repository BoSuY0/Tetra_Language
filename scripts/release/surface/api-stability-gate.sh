...
rg -n '^module lib\.core\.(surface|draw|component|widgets|accessibility|text|style)$' lib/core/*.tetra >"$module_report"
module_count="$(wc -l <"$module_report" | tr -d ' ')"
if [[ "$module_count" != "${#stable_modules[@]}" ]]; then
  echo "error: expected ${#stable_modules[@]} stable Surface module declarations, got $module_count" >&2
  cat "$module_report" >&2
  exit 1
fi

if rg -n '^module lib\.core\..*(experimental|v[0-9]+|_[vV][0-9]+)' lib/core/*.tetra; then
  echo "error: stable lib.core Surface modules must not use experimental or versioned suffixes" >&2
  exit 1
fi
