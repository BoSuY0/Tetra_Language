...
if [[ "$product_claim" == true && "$final_signoff" == true ]]; then
  mrb_category_evidence="integrated Morph rendered beauty gate validated with clean product claim and final signoff"
fi
...
cat > "$claims_dir/claim-governance-summary.json" <<JSON
{
  ...
  "comparison_doc": "docs/user/surface_electron_comparison.md",
  "pass": true
}
JSON
