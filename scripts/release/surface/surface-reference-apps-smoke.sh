...
check_reference_source() {
  ...
  local source="$2"
  local scan_path="$3"
  if rg -n 'React|Electron|Chromium|DOM|CSS|JavaScript|platform_widget|native_widget|platform widget|native widget|lib\.core\.component' "$source" > "$scan_path"; then
  ...
  elif rg -n 'lib\.core\.widgets|Button|Card|TextField|TextBox|Sidebar|Modal' "$source" >> "$scan_path"; then
