...
tokens_path="lib/core/morph.tetra"
recipes_path="lib/core/morph.tetra"
...
go run ./tools/cmd/surface-runtime-smoke --mode headless-morph --source "$source_path" --report "$mrb_runtime_report"
...
go run ./cli/cmd/tetra surface dev --source "$source_path" --target linux-x64 --out-dir "$artifact_dir" --report "$report_path" --morph-rendered-beauty-report "$mrb_report" --change-file "token:$tokens_path" --change-file "recipe:$recipes_path" --change-file "source:$source_path"
