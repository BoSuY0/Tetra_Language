...
surface_wasm32_web_browser_cleanup() {
  if [[ -n "${surface_wasm32_web_browser_active_pid:-}" ]] && kill -0 "$surface_wasm32_web_browser_active_pid" 2>/dev/null; then
...
surface_wasm32_web_browser_run surface-runtime-smoke yes go run ./tools/cmd/surface-runtime-smoke --mode wasm32-web-release-browser --source examples/surface_release_form.tetra --report "$report_path"
