...
if [[ "$report_dir_rel" == ..* || "$report_dir_rel" == /* ]]; then
	echo "error: template report dir must stay inside repository: $report_dir" >&2
	exit 2
fi
...
check_template_source() {
	...
	for forbidden_pattern in \
		...
		'platform widget' \
		'native widget'; do
		if rg -n "$forbidden_pattern" "$source_path" >"$scan_path"; then
			echo "error: generated Surface template imports forbidden runtime: $source_path" >&2
			cat "$scan_path" >&2
			exit 1
		fi
	done
...
for kind in "${template_kinds[@]}"; do
	...
	package_entry="{\"path\":$(json_string "$package_path"),"
	package_entry+="\"kind\":\"tar.gz\",\"sha256\":$(json_string "$package_sha"),"
	package_entry+="\"pass\":true}"
...
mrb_golden_order_1="$mrb_source,headless,1,"
mrb_golden_order_1+="$mrb_golden_dir/order-1-initial.rgba"
mrb_golden_order_2="$mrb_source,headless,2,"
mrb_golden_order_2+="$mrb_golden_dir/order-2-focused.rgba"
mrb_golden_order_3="$mrb_source,headless,3,"
mrb_golden_order_3+="$mrb_golden_dir/order-3-motion.rgba"
...
go run ./tools/cmd/validate-surface-morph-rendered-beauty \
	--report "$mrb_report" \
	--morph-to-pixels-chain-out "$mrb_chain"
...
templates_json="$(
	IFS=,
	printf "%s" "${template_entries[*]}"
)"
packages_json="$(
	IFS=,
	printf "%s" "${package_entries[*]}"
)"
