...
surface_linux_x64_release_window_cleanup() {
  if [[ -n "${surface_linux_x64_release_window_active_pid:-}" ]] && kill -0 "$surface_linux_x64_release_window_active_pid" 2>/dev/null; then
...
surface_linux_x64_release_window_run "runtime-smoke" "yes" go run ./tools/cmd/surface-runtime-smoke --mode linux-x64-release-window --source examples/surface_release_form.tetra --report "$report_path"
