...
go run ./tools/cmd/validate-surface-morph-report --report "$runtime_report" --same-commit "$git_head"
...
write_drift_golden "$artifact_dir/surface-morph-rendered-beauty-frame-order-1-initial.rgba" "$golden_dir/order-1-initial.rgba"
write_drift_golden "$artifact_dir/surface-morph-rendered-beauty-frame-order-2-focused.rgba" "$golden_dir/order-2-focused.rgba"
write_drift_golden "$artifact_dir/surface-morph-rendered-beauty-frame-order-3-motion.rgba" "$golden_dir/order-3-motion.rgba"
...
go run ./tools/cmd/validate-surface-morph-report --report "$linux_runtime_report" --same-commit "$git_head"
...
write_drift_golden "$linux_artifact_dir/surface-morph-real-window-frame-order-1.rgba" "$linux_golden_dir/order-1-initial.rgba"
write_drift_golden "$linux_artifact_dir/surface-morph-real-window-frame-order-5.rgba" "$linux_golden_dir/order-5-active.rgba"
...
go run ./tools/cmd/validate-surface-morph-report --report "$wasm_runtime_report" --same-commit "$git_head"
...
write_drift_golden "$wasm_artifact_dir/surface-browser-canvas-frame-order-1.rgba" "$wasm_golden_dir/order-1-initial.rgba"
write_drift_golden "$wasm_artifact_dir/surface-browser-canvas-frame-order-5.rgba" "$wasm_golden_dir/order-5-focused.rgba"

go run ./tools/cmd/surface-visual-diff \
  --runtime-report "$wasm_runtime_report" \
  --required-target wasm32-web-browser-canvas \
  --golden-artifact "$source_path,wasm32-web-browser-canvas,1,$wasm_golden_dir/order-1-initial.rgba" \
  --golden-artifact "$source_path,wasm32-web-browser-canvas,5,$wasm_golden_dir/order-5-focused.rgba" \
...
if [[ "${#missing_renderer_owned_targets[@]}" -gt 0 ]]; then
  missing_renderer_owned_text="$(IFS=', '; printf '%s' "${missing_renderer_owned_targets[*]}")"
  stable_promotion_blockers+=("renderer-owned stable proof missing for ${missing_renderer_owned_text}")
fi
...
cat > "$summary_path" <<JSON
{
  ...
  "contract": "docs/spec/surface_morph_rendered_beauty_contract.json",
  "block_contract": "docs/spec/surface_block_contract.json",
...
for report in "${required_reports[@]}"; do
  if [[ ! -s "$report_dir/$report" ]]; then
    echo "error: required Surface Morph rendered beauty gate report missing or empty: $report_dir/$report" >&2
    exit 1
  fi
done
