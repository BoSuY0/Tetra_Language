...
if [[ -e "$report_dir_abs" && ! -d "$report_dir_abs" ]]; then
	surface_release_guard_reject \
	  "surface_widget_migration_smoke:" \
...
for owned_path in "$report_path" "$work_dir" "$migration_dir"; do
	if [[ -e "$owned_path" ]]; then
		echo "surface_widget_migration_smoke: refusing existing artifact path: $owned_path" >&2
		exit 2
	fi
done
...
source_path="examples/surface/reference_forms/surface_reference_migration.tetra"
widgets_smoke_path="examples/core/surface_app/core_widgets_smoke.tetra"
...
for forbidden_pattern in \
	'future core widget primitive' \
	'widget primary future core' \
	'platform native widget runtime' \
	'native widget runtime' \
	'breaking widget API' \
	'docs-only widget migration claim'; do
	if rg -n "$forbidden_pattern" "$source_path" "$widgets_smoke_path" \
		>"$work_dir/source-scan.txt"; then
		echo "surface_widget_migration_smoke: forbidden future/runtime vocabulary" >&2
...
for required in 'import lib\.core\.widgets as widgets' 'import lib\.core\.block as block' 'import lib\.core\.morph as morph'; do
	if ! rg -n "$required" "$source_path" >>"$work_dir/source-scan.txt"; then
		echo "surface_widget_migration_smoke: missing import pattern $required" >&2
		exit 1
	fi
done
if ! rg -n 'widgets\.migration_block_only_core_primitive\(\)' \
	"$source_path" "$widgets_smoke_path" >>"$work_dir/source-scan.txt"; then
	echo "surface_widget_migration_smoke: Block must be the only core primitive" >&2
	exit 1
fi
...
if [[ "$widgets_smoke_exit" -ne 42 ]]; then
	echo "surface_widget_migration_smoke: core_widgets exit $widgets_smoke_exit, want 42" >&2
	exit 1
fi
