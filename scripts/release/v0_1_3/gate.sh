...
check_report_dir_fresh() {
  ...
  if [[ -n "$first_entry" ]]; then
    echo "release_v0_1_3_gate: refusing to reuse non-empty report directory: $report_dir" >&2
    echo "release_v0_1_3_gate: choose a fresh --report-dir so stale reports cannot be reused" >&2
    exit 2
  fi
}
