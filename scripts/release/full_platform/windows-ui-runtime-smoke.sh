...
usage() {
  cat << 'USAGE'
Usage: bash scripts/release/full_platform/windows-ui-runtime-smoke.sh [--report-dir DIR]

Writes the Windows platform UI runtime smoke report. The report is only a
production pass on a real Windows target-host runner with runtime-backed UI
...
if [[ -n "$external_report" ]]; then
  if [[ ! -f "$external_report" ]]; then
    echo "error: TETRA_WINDOWS_UI_RUNTIME_REPORT does not name a readable file: $external_report" >&2
    exit 2
  fi
