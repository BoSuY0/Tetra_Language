...
evidence_path="${TETRA_WINDOWS_UI_RUNTIME_EVIDENCE:-}"

usage() {
  cat <<'USAGE'
Usage: bash scripts/release/full_platform/windows-ui-runtime-smoke.sh [--report-dir DIR] [--evidence PATH]

Copies and validates a Windows target-host tetra.ui.platform.v1 runtime report.
On non-Windows hosts without --evidence it writes a blocked report and exits
...
if [[ -n "$evidence_path" ]]; then
  ...
  echo "Windows UI runtime target-host report: $report_path"
  exit 0
fi
...
cat >"$report_path" <<JSON
{
  ...
  "source": "scripts/release/full_platform/windows-ui-runtime-smoke.sh",
  "blocker": "cannot collect production UI runtime evidence on this host; provide a Windows target-host runtime report via --evidence",
  "processes": [],
  "contracts": [],
  "widgets": [],
  "events": [],
...
go run ./tools/cmd/validate-artifact-hashes --write --root "$report_dir" --out "$report_dir/artifact-hashes.json"
go run ./tools/cmd/validate-artifact-hashes --manifest "$report_dir/artifact-hashes.json"
echo "Windows UI runtime blocked report: $report_path" >&2
