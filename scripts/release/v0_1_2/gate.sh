...
report_dir=""
release_version="${TETRA_RELEASE_GATE_VERSION:-v0.1.2}"

usage() {
  cat <<'USAGE'
Usage: bash scripts/release/v0_1_2/gate.sh [--report-dir DIR]
USAGE
}

check_report_dir_fresh() {
  if [[ ( -e "$report_dir" || -L "$report_dir" ) && ! -d "$report_dir" ]]; then
    echo "release_v0_1_2_gate: refusing to use non-directory report path: $report_dir" >&2
    echo "release_v0_1_2_gate: choose a fresh --report-dir directory" >&2
    exit 2
  fi
  ...
  if [[ -n "$first_entry" ]]; then
    echo "release_v0_1_2_gate: refusing to reuse non-empty report directory: $report_dir" >&2
    echo "release_v0_1_2_gate: choose a fresh --report-dir so stale reports cannot be reused" >&2
    exit 2
  fi
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --report-dir)
      if [[ $# -lt 2 || -z "${2:-}" ]]; then
        echo "release_v0_1_2_gate: --report-dir requires a directory" >&2
        usage >&2
        exit 2
      fi
      report_dir="$2"
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "release_v0_1_2_gate: unknown option $1" >&2
      usage >&2
      exit 2
      ;;
  esac
...
if [[ -z "$report_dir" ]]; then
  ...
  report_dir="$state_home/tetra-language/release-v0_1_2-gate-$(date -u +%Y%m%d-%H%M%S)"
fi
if [[ "$report_dir" == -* ]]; then
...
write_summary() {
  ...
  {
    echo "# Tetra v0.1.2 Release Gate Report"
    echo
    echo "- status: \`$status\`"
...
# Tetra v0.1.2 Known Issues

Generated by \`scripts/release/v0_1_2/gate.sh\`.
...
- Last release gate command: \`bash scripts/release/v0_1_2/gate.sh --report-dir $report_dir\`
...
echo "== v0.1.2 preflight =="
echo "report_dir: $report_dir"

run_step "bootstrap tetra binaries" bash scripts/dev/bootstrap.sh
run_step "version preflight (v0.1.2 required)" check_release_version
if [[ "$failed_count" -gt 0 ]]; then
  write_summary "blocked"
  echo
  echo "v0.1.2 release gate blocked at version preflight: expected v0.1.2 before mandatory release checks" >&2
...
if [[ "$failed_count" -gt 0 ]]; then
  write_summary "blocked"
  echo
  echo "v0.1.2 release gate blocked: $failed_count step(s) failed" >&2
  echo "summary: $summary_md" >&2
  echo "json: $summary_json" >&2
  exit 1
fi
...
write_summary "pass"
echo
echo "v0.1.2 release gate passed"
