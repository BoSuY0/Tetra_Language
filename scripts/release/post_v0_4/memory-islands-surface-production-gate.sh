...
islands_debug_command="$(
  join_command \
    go run ./cli/cmd/tetra smoke \
    --target linux-x64 \
    --run=true \
    --islands-debug \
    --report "$islands_debug_report_arg"
)"
validate_islands_debug_command="$(
  join_command \
    go run ./tools/cmd/smoke-report-to-checklist \
    --validate-only \
    --report "$islands_debug_report_arg"
)"
surface_release_command="$(
  join_command \
    bash scripts/release/surface/release-gate.sh \
    --report-dir "$surface_release_dir"
)"
surface_experimental_command="$(
  join_command \
    bash scripts/release/surface/gate.sh \
    --report-dir "$surface_experimental_dir"
)"
safe_view_command="$(
  join_command \
    bash scripts/release/safe-view-lifetime/gate.sh \
    --report-dir "$safe_view_dir"
)"
surface_api_command="$(
  join_command \
    bash scripts/release/surface/api-stability-gate.sh \
    --report-dir "$surface_api_dir"
)"
...
integrated_release_validator_command="$(
  join_command \
    go run ./tools/cmd/validate-memory-islands-surface-production \
  ...
  "commands": [
    {
      ...
      "command":
        $(json_string "$memory_production_command")
    },
    {
      "name": "islands-debug-smoke",
      "command":
        $(json_string "$islands_debug_command")
    },
    {
      "name": "validate-islands-debug-smoke",
      "command":
        $(json_string "$validate_islands_debug_command")
    },
    {
      "name": "surface-release-gate",
      "command":
        $(json_string "$surface_release_command")
    },
    {
      "name": "surface-experimental-regression-gate",
      "command":
        $(json_string "$surface_experimental_command")
    },
    {
      "name": "safe-view-lifetime-gate",
      "command":
        $(json_string "$safe_view_command")
    },
    {
      "name": "surface-api-stability-gate",
      "command":
        $(json_string "$surface_api_command")
    },
    {
      "name": "validate-manifest",
      "command":
        $(json_string "$validate_manifest_command")
    },
    {
      "name": "verify-docs",
      "command":
        $(json_string "$verify_docs_command")
    },
    {
      "name": "artifact-hashes-write",
      "command":
        $(json_string "$artifact_hashes_write_command")
    },
    {
      "name": "artifact-hashes-validate",
      "command":
        $(json_string "$artifact_hashes_validate_command")
    },
    {
      "name": "integrated-release-validator",
      "command":
        $(json_string "$integrated_release_validator_command")
    }
  ],
  "artifacts": [
    {
