...
git_status_json="$(git status --short --branch | python3 -c 'import json, sys; print(json.dumps([line.rstrip("\\n") for line in sys.stdin]))')"

mkdir -p "$raw_memory_dir" "$allocation_dir" "$proof_store_dir" "$proof_transition_dir" "$runtime_memory_dir" "$memory_fuzz_dir" "$semantic_safety_dir" "$leak_resource_dir" "$docs_manifest_dir"
  ...
  "operations": [
    {
      ...
      "source_artifacts": ["compiler/internal/runtimeabi/raw_pointer_bounds_test.go", "memory-production/memory-production-linux-x64.json"],
    ...
    },
    {
      ...
      "source_artifacts": ["compiler/internal/runtimeabi/raw_pointer_bounds_test.go", "compiler/tests/semantics/memory_ideal_v5_raw_pointer_test.go", "memory-production/memory-production-linux-x64.json"],
      "negative_tests": ["outside unsafe", "negative length", "i32 byte overflow"]
    },
    {
      "name": "raw_load_store_metadata",
      "source_artifacts": ["compiler/internal/plir/plir_test.go", "compiler/internal/lower/raw_memory_test.go", "compiler/internal/memoryfacts/from_plir_test.go", "memory-production/memory-production-linux-x64.json"],
      "positive_tests": ["IRMemWriteI32Offset", "IRMemReadI32Offset", "core.store_u8/core.load_u8 raw memory gateway UnsafeChecked"],
      "negative_tests": ["checked_external_unknown raw store/load remains conservative", "rejected_access_width_overflow raw load/store width rejection"],
      "non_claims": ["no arbitrary external pointer safety claim"]
    },
    {
      "name": "memcpy_u8",
      "source_artifacts": ["lib/core/memory.tetra", "compiler/internal/lower/raw_memory_test.go", "memory-production/memory-production-linux-x64.json"],
    ...
    {
      "name": "cap.mem",
      "source_artifacts": ["lib/core/capability.tetra", "compiler/internal/ramcontract/validate_test.go", "memory-production/memory-production-linux-x64.json"],
...
python3 - "$allocation_dir/allocation-lowering-report.json" "$git_head" "$ram_contract_dir/heap-blockers.json" "$ram_contract_dir/copy-blockers.json" <<'PY'
import json
import sys
...
def blocker_decision(name, planned, artifact, reason, budget, grade, validators, sites):
    base = {
        ...
        "source_artifacts": [artifact, "ram-contract/ram-contract-report.json", "ram-contract/memory-grade-report.json"],
    }
    if not sites:
...
report = {
    ...
    "decisions": [
        ...
        blocker_decision(
            ...
            "conservative heap fallback remains explicit until no-escape/lifetime proof is available",
        ...
        blocker_decision(
            ...
            "copy decision remains explicit until alias/lifetime proof removes the copy requirement",
  ...
  "rows": [
    {
      ...
      "source_artifacts": ["compiler/internal/proof/term.go", "compiler/internal/proof/validate_test.go"],
      "tests": ["go test ./compiler/internal/proof -run TestProofStoreRejectsStaleStableHashForSemanticFields -count=1"]
    },
    {
      ...
      "source_artifacts": ["compiler/translation_validation_v2.go", "compiler/translation_validation_v2_test.go"],
      "tests": ["go test ./compiler -run TestP23TranslationValidationV2CoversSupportedOptimizerSubset -count=1"]
    },
    {
      ...
      "tests": ["go test ./compiler/internal/validation -run TestValidateTranslationRejectsMissingProofIDAfterTransform -count=1"]
    },
    {
      ...
      "source_artifacts": ["compiler/internal/opt/manager.go", "compiler/internal/opt/manager_test.go"],
    ...
    },
    {
      ...
      "source_artifacts": ["compiler/internal/plir/plir_test.go", "compiler/internal/lower/proof_bce_test.go"],
      "tests": ["go test ./compiler/internal/plir ./compiler/internal/lower -run 'Proof|Invalidates|Unchecked' -count=1"]
    },
    {
      ...
      "source_artifacts": ["compiler/internal/validation/validation_test.go", "compiler/internal/proof/validate_test.go"],
      "tests": ["go test ./compiler/internal/validation ./compiler/internal/proof -run 'UnknownLiveProof|MissingProofID|ProofStoreRejectsMissingProofID' -count=1"]
    }
  ],
  "non_claims": [
  ...
  "rows": [
    {
      ...
      "source_artifacts": ["memory-production/targets.json", "compiler/runtime_hardening_v1.go", "compiler/internal/runtimeabi/runtimeabi_test.go"],
      "tests": ["go test ./compiler -run 'RuntimeHardening|RuntimeAllocation|RawPointerBoundsABI|ActorRuntimeProductionBoundary|OOM|Stack|Allocator|Region' -count=1"],
    ...
    },
    {
      ...
      "tests": ["go test ./tools/cmd/validate-memory-100-prod-stable -run RuntimeMemory -count=1"],
  ...
  "rows": [
    {
      ...
      "source_artifacts": ["compiler/tests/ownership/ownership_test.go", "compiler/tests/semantics/borrow_escape_matrix_test.go"],
      "tests": ["go test ./compiler/tests/ownership ./compiler/tests/semantics -run 'Borrow.*Return|BorrowEscape' -count=1"]
    },
    {
      "name": "borrowed_view_owned_aggregate_escape",
      "kind": "negative",
      "evidence": "borrowed slice/ptr fields cannot escape through owned aggregate returns, consume, or inout calls",
      "source_artifacts": ["compiler/tests/ownership/ownership_test.go"],
      "tests": ["go test ./compiler/tests/ownership -run 'Borrowed.*Aggregate|BorrowedSlice.*ConsumeInout|BorrowedPtr.*Struct' -count=1"]
    },
    {
      "name": "borrowed_text_host_boundary_copy",
      "kind": "negative",
      "evidence": "borrowed text/view host and actor/task boundaries require explicit copy or are rejected",
      "source_artifacts": ["compiler/tests/semantics/string_view_test.go", "compiler/internal/actorsafety/sendability_test.go"],
      "tests": ["go test ./compiler/tests/semantics ./compiler/internal/actorsafety -run 'Borrowed|StringView|copy|ActorBoundary' -count=1"]
    },
    {
    ...
    },
    {
      ...
      "source_artifacts": ["compiler/tests/semantics/surface_stdlib_test.go", "integrated/safe-view-lifetime/safe-view-lifetime-summary.json"],
    ...
    },
    {
      ...
      "source_artifacts": ["compiler/tests/runtime/resource_finalization_test.go", "compiler/tests/ownership/ownership_test.go"],
      "tests": ["go test ./compiler/tests/runtime ./compiler/tests/ownership -run 'Present|Close|UseAfter|Freed|Consume' -count=1"]
    },
    {
      "name": "resource_finalizer_double_close",
      "kind": "negative",
      "evidence": "resource finalization diagnostics reject missing finalizer and double-close/double-free cases",
      "source_artifacts": ["compiler/tests/runtime/resource_finalization_test.go", "compiler/tests/safety/safety_diagnostics_test.go"],
      "tests": ["go test ./compiler/tests/runtime ./compiler/tests/safety -run 'Resource|Finalization|Double|Close|Free' -count=1"]
    },
    {
      "name": "actor_task_non_sendable_transfer",
      "kind": "negative",
      "evidence": "actor/task message transfer rejects borrowed or non-sendable memory/resources unless explicitly copied/moved",
      "source_artifacts": ["compiler/tests/ownership/actor_task_ownership_test.go", "compiler/internal/actorsafety/sendability_test.go", "compiler/internal/actorsafety/ownership_transfer_test.go"],
      "tests": ["go test ./compiler/tests/ownership ./compiler/internal/actorsafety -run 'Actor|Task|Send|Transfer|Borrowed|NonSendable' -count=1"]
    }
  ],
  "non_claims": [
...
cat > "$aggregate_manifest_path" <<MANIFEST
{
  ...
  "commands": [
    {"name": "memory-production-gate", "command": "bash scripts/release/post_v0_4/memory-production-linux-x64-smoke.sh --report-dir $(json_escape "$memory_production_dir")"},
    ...
    {"name": "integrated-gate", "command": "bash scripts/release/post_v0_4/memory-islands-surface-production-gate.sh --report-dir $(json_escape "$integrated_dir")"},
    ...
    {"name": "memory-fuzz-validator", "command": "go run ./tools/cmd/validate-memory-fuzz-oracle --report $(json_escape "$memory_fuzz_dir")/memory-fuzz-oracle.json --artifact-dir $(json_escape "$memory_fuzz_dir") --current-git-head $git_head"},
    {"name": "docs-claim-policy", "command": "go run ./tools/cmd/verify-docs --manifest docs/generated/manifest.json"},
    ...
    {"name": "memory-100-validator", "command": "go run ./tools/cmd/validate-memory-100-prod-stable --report-dir $(json_escape "$report_dir_arg") --current-git-head $git_head"}
  ],
  "artifacts": [
    ...
    {"path": "memory-production/artifact-hashes.json", "kind": "memory_production_hash_manifest", "schema": "tetra.release-artifact-hashes.v1alpha1"},
    ...
    {"path": "ram-contract/artifact-hashes.json", "kind": "ram_contract_hash_manifest", "schema": "tetra.release-artifact-hashes.v1alpha1"},
    ...
    {"path": "memory-fuzz/artifact-hashes.json", "kind": "memory_fuzz_hash_manifest", "schema": "tetra.release-artifact-hashes.v1alpha1"},
    ...
    {"path": "integrated/artifact-hashes.json", "kind": "integrated_hash_manifest", "schema": "tetra.release-artifact-hashes.v1alpha1"},
