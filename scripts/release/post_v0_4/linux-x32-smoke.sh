...
build-only status until full x32 runtime/stdlib/FFI promotion evidence exists.
USAGE
}
...
cd "$repo_root"
: "${GOCACHE:=$repo_root/.cache/go-build-linux-x32-smoke}"
export GOCACHE
mkdir -p "$report_dir"
...
if "${tetra_cmd[@]}" test --diagnostics=json --target x32 --format=json "$runner_src" >"$report_dir/linux-x32-runner.json" 2>"$report_dir/linux-x32-runner.err.json"; then
  ...
  code=$?
  if [[ "$code" -eq 2 ]] && grep -q 'no host fallback' "$report_dir/linux-x32-runner.err.json"; then
    mv "$report_dir/linux-x32-runner.err.json" "$report_dir/linux-x32-runner.json"
  else
    cat "$report_dir/linux-x32-runner.err.json" >&2
    exit "$code"
  fi
fi
...
echo "linux-x32 smoke reports: $report_dir"
echo "linux-x32 artifact hashes: $report_dir/artifact-hashes.json"
