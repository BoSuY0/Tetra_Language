...
report_dir="$repo_root/reports/post-v0.4-compiler"

usage() {
  cat << 'USAGE'
Usage: bash scripts/release/post_v0_4/memory-parallel-compiler-production-linux-x64-gate.sh [--report-dir DIR]

Runs the ordered Linux-x64 production evidence gates for:
  1. tetra.memory.production.v1
  2. tetra.parallel.production.v1
  3. tetra.compiler.production.v1
USAGE
}
...
echo "post-v0.4 Memory/Parallelism/Compiler production gate report dir: $report_dir"
echo "required schemas: tetra.memory.production.v1, tetra.parallel.production.v1, tetra.compiler.production.v1"
