...
Modes:
  ...
  --full   Run the full v0.1.x stabilization gate with logs and summaries.
...
check_version_prefix() {
  ...
  case "$version" in
    v0.1.*)
      ...
      ;;
    *)
      echo "expected v0.1.x, got $version" >&2
      exit 1
      ;;
  esac
}
...
if [[ "$json_only" != true ]]; then
  printf 'Tetra v0.1.x test wrapper\n'
  printf 'mode: %s\n' "$mode"
  printf 'report_dir: %s\n\n' "$report_dir"
fi
