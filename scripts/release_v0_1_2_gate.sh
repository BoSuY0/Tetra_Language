...
usage() {
  cat <<'USAGE'
Usage: bash scripts/release_v0_1_2_gate.sh [--report-dir DIR]
USAGE
}
...
check_release_version() {
  ...
  if [[ "$version" != v0.1.2 ]]; then
    echo "expected v0.1.2 version, got '${version:-<missing>}'" >&2
    return 1
  fi
}
...
Generated by \`scripts/release_v0_1_2_gate.sh\`.
...
- Last release gate command: \`bash scripts/release_v0_1_2_gate.sh --report-dir $report_dir\`
