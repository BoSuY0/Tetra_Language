...
default_go_cache="${XDG_CACHE_HOME:-$HOME/.cache}/tetra-language/go-build-toon-format-check"
...
if run_tetra run \
  --diagnostics=json \
  --target not-a-target \
  > "$tmp_dir/diagnostic-json.stdout" \
  2> "$diagnostic_json"; then
...
if run_tetra run \
  --diagnostics=toon \
  --target not-a-target \
  > "$tmp_dir/diagnostic-toon.stdout" \
  2> "$diagnostic_toon"; then
...
run_tetra lsp --stdio-smoke examples/flow/flow_hello.tetra --format=json > "$tmp_dir/lsp-smoke.json"
run_tetra lsp --stdio-smoke examples/flow/flow_hello.tetra --format=toon > "$tmp_dir/lsp-smoke.toon"
...
if run_tetra lsp \
  --stdio \
  --format=toon \
  > "$tmp_dir/lsp-stdio-format.stdout" \
  2> "$tmp_dir/lsp-stdio-format.stderr"; then
...
go run ./tools/cmd/validate-smoke-list \
  --report "$tmp_dir/smoke-list.json" \
  --examples-root examples \
  --format=json
go run ./tools/cmd/validate-smoke-list \
  --report "$tmp_dir/smoke-list.toon" \
  --examples-root examples \
  --format=toon
...
go run ./tools/cmd/smoke-report-to-checklist \
  --validate-only \
  --report "$tmp_dir/host-smoke.toon" \
  --format=toon
...
JSON
go run ./tools/cmd/json-to-toon \
  --in "$summary_dir/summary.json" \
  --out "$summary_dir/summary.toon"
go run ./tools/cmd/validate-test-all-summary \
  --summary "$summary_dir/summary.json" \
  --report-dir "$summary_dir" \
  --format=json
go run ./tools/cmd/validate-test-all-summary \
  --summary "$summary_dir/summary.toon" \
  --report-dir "$summary_dir" \
  --format=toon
...
capsule App:
  ...
  permission "io"
CAPSULE
run_tetra eco verify \
  --target linux-x64 \
  --lock "$tmp_dir/eco.lock.json" \
  --lock-format=both \
  "$eco_capsule"
...
run_tetra eco seed import \
  --seed "$tmp_dir/eco.seed.toon" \
  --seed-format=toon \
  --lock "$tmp_dir/eco.seed.lock.json" \
  --lock-format=both \
...
go run ./tools/cmd/validate-eco-needmap \
  --needmap "$tmp_dir/eco.needmap.json" \
  --format=json
go run ./tools/cmd/validate-eco-needmap \
  --needmap "$tmp_dir/eco.needmap.toon" \
  --format=toon

webrt_pattern='TestAcceptExplicitTOON'
webrt_pattern+='|TestServer(JSON|DB)EndpointSupportsExplicitTOONAccept'
webrt_pattern+='|TestServerDBEndpointsSupportExplicitTOONAccept'
go_test ./compiler/internal/webrt -run "$webrt_pattern" -count=1

cli_pattern='TestLSPCommandSmokeTOONFormat'
cli_pattern+='|TestLSPStdioRejectsTOONFormat'
cli_pattern+='|TestSmokeCommandWritesTOONReportMirror'
cli_pattern+='|TestSmokeCommandListsCasesAsTOON'
go_test ./cli/cmd/tetra -run "$cli_pattern" -count=1

toon_tool_packages=(
  ./tools/cmd/json-to-toon
  ./tools/cmd/validate-test-all-summary
  ...
  ./tools/cmd/validate-eco-seed
  ./tools/cmd/validate-eco-needmap
)
go_test "${toon_tool_packages[@]}" \
  -run 'TOON|JSONToTOON|Smoke|Summary|Manifest|Eco' \
  -count=1
