...
if [[ -z "$report_dir" ]]; then
  report_dir="/tmp/release-v0_1_3-gate-$(date -u +%Y%m%d-%H%M%S)"
fi
...
write_summary() {
  ...
  {
    echo "# Tetra v0.1.3 Release Gate Report"
    echo
    echo "- status: \`$status\`"
...
check_release_version() {
  ...
  if [[ "$version" != v0.1.3 ]]; then
    echo "expected v0.1.3 version, got '${version:-<missing>}'" >&2
    return 1
  fi
}
...
# Tetra v0.1.3 Known Issues

Generated by \`scripts/release_v0_1_3_gate.sh\`.
...
- Last release gate command: \`bash scripts/release_v0_1_3_gate.sh --report-dir $report_dir\`
...
if [[ "$failed_count" -gt 0 ]]; then
  ...
  echo
  echo "v0.1.3 release gate blocked at version preflight: expected v0.1.3 before mandatory release checks" >&2
...
if [[ "$failed_count" -gt 0 ]]; then
  write_summary "blocked"
  echo
  echo "v0.1.3 release gate blocked: $failed_count step(s) failed" >&2
  echo "summary: $summary_md" >&2
  echo "json: $summary_json" >&2
  exit 1
fi
...
write_summary "pass"
echo
echo "v0.1.3 release gate passed"
