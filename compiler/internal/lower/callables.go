...
func collectFunctionTypedParamTargets(checked *semantics.CheckedProgram, module string) map[string]map[string][]string {
	...
	var addStructLiteralFieldEdges func(caller semantics.CheckedFunc, destFunc, destPrefix, structType string, value frontend.Expr, destFields map[string]semantics.FunctionFieldInfo)
	addStructLiteralFieldEdges = func(caller semantics.CheckedFunc, destFunc, destPrefix, structType string, value frontend.Expr, destFields map[string]semantics.FunctionFieldInfo) {
		...
		typeInfo, ok := checked.Types[structType]
		...
		for _, init := range lit.Fields {
			...
			if field.FunctionTypeValue {
				if _, ok := destFields[init.Name]; ok {
					...
					if target, ok := callableTargetFromAssignedExpr(init.Value, caller, checked.FuncSigs, checked.GlobalsByModule[caller.Module]); ok {
					...
					if sourceSig, exists := checked.FuncSigs[resolved]; exists && len(sourceSig.ReturnFunctionFields) > 0 {
						for fieldName, field := range destFields {
							...
							edges = append(edges, callableTargetEdge{
								...
								param:       destFieldName,
								sourceFunc:  resolved,
								sourceParam: returnFieldPrefix + sourceField,
							})
						}
					}
				}
			...
			addStructLiteralFieldEdges(caller, destFunc, destPrefix+nestedPrefix, field.TypeName, init.Value, trimFunctionFields(destFields, nestedPrefix))
		}
	}

	var addStructLiteralEnumPayloadFieldEdges func(caller semantics.CheckedFunc, destFunc, destPrefix, structType string, value frontend.Expr, destFields map[string]semantics.FunctionFieldInfo)
	addStructLiteralEnumPayloadFieldEdges = func(caller semantics.CheckedFunc, destFunc, destPrefix, structType string, value frontend.Expr, destFields map[string]semantics.FunctionFieldInfo) {
		...
		for _, init := range lit.Fields {
			...
			if info, ok := checked.Types[field.TypeName]; ok && info.Kind == semantics.TypeEnum {
				fieldPrefix := init.Name + "#"
				if payloads := enumPayloadTargetsFromExpr(init.Value, caller, checked.FuncSigs, checked.Types); len(payloads) > 0 {
					for payloadKey, payload := range payloads {
			...
			addStructLiteralEnumPayloadFieldEdges(caller, destFunc, destPrefix+nestedPrefix, field.TypeName, init.Value, trimFunctionFields(destFields, nestedPrefix))
		}
	}
...
func callableTargetFromAssignedExpr(expr frontend.Expr, caller semantics.CheckedFunc, funcs map[string]semantics.FuncSig, globals map[string]semantics.GlobalInfo) (string, bool) {
	...
	case *frontend.ClosureExpr:
		return callableClosureTargetName(caller, e, funcs), e.Name != ""
	case *frontend.CallExpr:
		resolved := e.Name
		if builtin, ok := semantics.ResolveBuiltinAlias(resolved); ok {
			resolved = builtin
	...
	if global, ok := globals[id.Name]; ok && global.FunctionTypeValue && global.FunctionValue != "" {
...
func enumPayloadTargetsFromExpr(
	...
	for i, arg := range call.Args {
		...
		target, ok := callableTargetFromAssignedExpr(arg, caller, funcs, nil)
		if !ok {
			continue
		}
...
func enumCaseConstructorInfoForTargets(call *frontend.CallExpr, types map[string]*semantics.TypeInfo) (string, semantics.EnumCaseInfo, bool) {
	...
	if !ok || info.Kind != semantics.TypeEnum {
		if altName, altInfo, found := findUniqueEnumByShortNameInLower(typeName, types); found {
			typeName = altName
			info = altInfo
		} else {
			return "", semantics.EnumCaseInfo{}, false
