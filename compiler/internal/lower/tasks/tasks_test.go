...
func TestLowerWrapperUsesInjectedUnsupportedDiagnostic(t *testing.T) {
	_, err := LowerWrapper(Wrapper{Name: "bad", SlotCount: 9}, func(pos frontend.Position, format string, args ...interface{}) error {
		return &frontend.DiagnosticError{Info: frontend.Diagnostic{Code: "TETRA3002", Message: "unsupported"}}
	})
	if err == nil {
...
func TestLowerWrapperBuildsSmallSlotIR(t *testing.T) {
	...
	if len(fn.Instrs) == 0 || fn.Instrs[0].Kind != ir.IRCall || fn.Instrs[0].Name != "worker" || fn.Instrs[0].RetSlots != 3 {
...
func TestCallExprCloneAndThrowingLayout(t *testing.T) {
	orig := &frontend.CallExpr{Name: "worker", Args: []frontend.Expr{&frontend.NumberExpr{Value: 1}}}
	...
	if !IsTypedTaskJoinCall("core.task_join_i32_typed") || TypedTaskJoinRuntimeSymbol(6) != "__tetra_task_join_typed_6" {
