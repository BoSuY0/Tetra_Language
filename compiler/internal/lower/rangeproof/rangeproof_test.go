...
func TestModuloDivisorNameAndProofID(t *testing.T) {
	numerator, divisor, ok := ModuloDivisorName(&frontend.BinaryExpr{
		Left: &frontend.BinaryExpr{
			Left:  &frontend.IdentExpr{Name: "i"},
			Op:    frontend.TokenStar,
			Right: &frontend.NumberExpr{Value: 17},
		},
		Op:    frontend.TokenPercent,
		Right: &frontend.IdentExpr{Name: "n"},
	})
	if !ok {
		t.Fatalf("expected modulo divisor name")
	}
	if divisor != "n" {
		t.Fatalf("divisor = %q, want n", divisor)
	}
	if _, ok := numerator.(*frontend.BinaryExpr); !ok {
		t.Fatalf("numerator = %#v, want binary expression", numerator)
	}

	if _, _, ok := ModuloDivisorName(&frontend.BinaryExpr{
		Left:  &frontend.IdentExpr{Name: "i"},
		Op:    frontend.TokenPercent,
		Right: &frontend.NumberExpr{Value: 4},
	}); ok {
		t.Fatalf("numeric divisor should not produce an allocation-length alias proof")
	}

	id := ModuloBoundsProofID("idx", "state.items", frontend.Position{Line: 12, Col: 7})
	if id != "proof:modulo:idx:state_items:12:7" {
		t.Fatalf("modulo proof id = %q", id)
	}
}
...
func TestBuiltinClassifiers(t *testing.T) {
	...
	if !IsSliceCopyBuiltinName("core.slice_copy_i32") ||
		IsSliceCopyBuiltinName("core.slice_window_i32") {
		t.Fatalf("slice copy classifier mismatch")
	}
	if !IsBorrowOrViewBuiltinName("core.slice_window_i32") ||
		!IsBorrowOrViewBuiltinName("core.string_prefix") {
