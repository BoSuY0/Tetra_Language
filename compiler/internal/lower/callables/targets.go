...
func TargetFromAssignedExpr(expr frontend.Expr, caller semantics.CheckedFunc, funcs map[string]semantics.FuncSig, globals map[string]semantics.GlobalInfo) (string, bool) {
...
func ClosureTargetName(caller semantics.CheckedFunc, closure *frontend.ClosureExpr, funcs map[string]semantics.FuncSig) string {
...
func FindUniqueEnumByShortName(shortName string, types map[string]*semantics.TypeInfo) (string, *semantics.TypeInfo, bool) {
	if shortName == "" {
		return "", nil, false
	}
