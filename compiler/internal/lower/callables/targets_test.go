...
func TestFunctionFieldAndImportTargets(t *testing.T) {
	...
	if target, ok := ImportedFunctionTargetFromExpr(importExpr, map[string]string{"math": "std.math"}, map[string]semantics.FuncSig{"std.math.add": {}}); !ok || target != "std.math.add" {
...
func TestEnumCaseConstructorInfoUsesResolvedAndShortTypeNames(t *testing.T) {
	...
	typeName, caseInfo, ok := EnumCaseConstructorInfoForTargets(&frontend.CallExpr{Name: "pkg.Result.Ok"}, types)
	if !ok || typeName != "pkg.Result" || caseInfo.Ordinal != 1 {
		t.Fatalf("constructor info = %q %#v %v", typeName, caseInfo, ok)
	}

	typeName, caseInfo, ok = EnumCaseConstructorInfoForTargets(&frontend.CallExpr{Name: "Result.Ok"}, types)
	if !ok || typeName != "pkg.Result" || caseInfo.Ordinal != 1 {
...
func TestTrimAndResolveFunctionFields(t *testing.T) {
	...
	field, ok, err := ResolveFunctionFieldName("handler.nested.callback", map[string]semantics.LocalInfo{
