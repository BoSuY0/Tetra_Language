...
func TestRawPointerBoundsTracksAllocationBaseAndDerivedOffsets(t *testing.T) {
	...
	if root.Status != RawPointerBoundsAllocationBase || root.BaseID != "p" || root.BaseBytes != 16 || root.OffsetBytes != 0 {
	...
	if derived.Status != RawPointerBoundsDerivedOffset || derived.BaseID != "p" || derived.BaseBytes != 16 || derived.OffsetBytes != 4 {
...
func TestRawPointerBoundsDiagnosticsForImpossiblePtrAdd(t *testing.T) {
	...
	if neg == nil || neg.Code != RawPointerDiagnosticNegativePtrAdd || !strings.Contains(neg.Message, "negative ptr_add offset") {
		t.Fatalf("negative ptr_add diagnostic = %+v, want %s", neg, RawPointerDiagnosticNegativePtrAdd)
	}
	if negBounds.Status != RawPointerBoundsRejectedNegativeOffset {
		t.Fatalf("negative ptr_add status = %q, want %q", negBounds.Status, RawPointerBoundsRejectedNegativeOffset)
	}

	upperBounds, upper := DeriveRawPointerBounds(root, 4, 1)
	if upper == nil || upper.Code != RawPointerDiagnosticAllocationUpperBound || !strings.Contains(upper.Message, "allocation upper bound") {
		t.Fatalf("upper-bound ptr_add diagnostic = %+v, want %s", upper, RawPointerDiagnosticAllocationUpperBound)
	}
	if upperBounds.Status != RawPointerBoundsRejectedUpperBound {
		t.Fatalf("upper-bound ptr_add status = %q, want %q", upperBounds.Status, RawPointerBoundsRejectedUpperBound)
	}
	...
	if widthBounds.Status != RawPointerBoundsRejectedAccessWidthOverflow {
		t.Fatalf("access-width status = %q, want %q", widthBounds.Status, RawPointerBoundsRejectedAccessWidthOverflow)
	}
}
...
func TestRawPointerBoundsRejectsOffsetWidthIntegerOverflow(t *testing.T) {
	...
	if bounds.Status != RawPointerBoundsRejectedAccessWidthOverflow {
		t.Fatalf("overflow status = %q, want %q", bounds.Status, RawPointerBoundsRejectedAccessWidthOverflow)
	}
}
...
func TestUnknownRawPointerBoundsStayCheckedAndRawSliceExternal(t *testing.T) {
	...
	if diag != nil {
		t.Fatalf("unknown ptr_add should stay checked rather than claim allocation-base rejection: %+v", diag)
	}
	...
	if verifiedSlice.Status != RawSliceBoundsVerifiedAllocationRoot || !verifiedSlice.VerifiedAllocationRoot {
...
func TestRawSliceBoundsRejectsLengthArithmeticOverflow(t *testing.T) {
	...
	if rawSlice.Status != RawSliceBoundsRejectedLengthOverflow || rawSlice.VerifiedAllocationRoot {
		t.Fatalf("raw slice with overflowing length bytes = %+v, want rejected length overflow", rawSlice)
	}
...
func TestRawSliceBoundsRejectsNegativeLengthForVerifiedRoot(t *testing.T) {
	...
	if string(rawSlice.Status) != "rejected_negative_length" || rawSlice.VerifiedAllocationRoot {
		t.Fatalf("raw slice with negative length = %+v, want rejected_negative_length without verified root", rawSlice)
	}
}
