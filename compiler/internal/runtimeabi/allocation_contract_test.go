...
func TestRuntimeAllocationContractForMakeSlice(t *testing.T) {
	...
	if contract.ZeroSizeBehavior != AllocationZeroCanonicalEmpty {
		t.Fatalf("zero-size behavior = %q, want %q", contract.ZeroSizeBehavior, AllocationZeroCanonicalEmpty)
	}
	if contract.NegativeBehavior != AllocationRejectBeforeAllocator || contract.OverflowBehavior != AllocationRejectBeforeAllocator {
		t.Fatalf("make_u8 guards = negative %q overflow %q, want reject before allocator", contract.NegativeBehavior, contract.OverflowBehavior)
	}
...
func TestRuntimeAllocationContractDistinguishesAllocBytesAndIsland(t *testing.T) {
	...
	if raw.ZeroSizeBehavior != AllocationZeroInvalidPrecondition {
		t.Fatalf("alloc_bytes zero-size behavior = %q, want invalid precondition", raw.ZeroSizeBehavior)
	}
	...
	if abi.AllocationBaseStatus != RawPointerBoundsAllocationBase || abi.UnknownPointerStatus != RawPointerBoundsCheckedExternalUnknown {
		t.Fatalf("raw pointer bounds ABI = %+v, want allocation-base and checked-unknown statuses", abi)
	}
	...
	if island.ZeroSizeBehavior != AllocationZeroCanonicalEmptyNoMetadata {
		t.Fatalf("island_make_i32 zero-size behavior = %q, want no-metadata empty", island.ZeroSizeBehavior)
	}
...
func TestRuntimeAllocationContractValidationRejectsWeakSpecs(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "power-of-two") {
		t.Fatalf("ValidateRuntimeAllocationContract error = %v, want power-of-two alignment diagnostic", err)
	}
