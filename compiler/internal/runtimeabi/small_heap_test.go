...
func TestSmallHeapClassForBytes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if ok != tc.ok {
				t.Fatalf("SmallHeapClassForBytes(%d) ok=%v, want %v (class=%+v)", tc.bytes, ok, tc.ok, cls)
			}
...
func TestAlignSmallHeapBytes(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if ok != tc.ok || got != tc.want {
			t.Fatalf("AlignSmallHeapBytes(%d) = %d,%v want %d,%v", tc.bytes, got, ok, tc.want, tc.ok)
		}
	}
}
...
func TestPerCoreSmallHeapAllocatorABIDefinesMetadataAndReuse(t *testing.T) {
	...
	if abi.ChunkBytes != SmallHeapChunkBytes || abi.Alignment != SmallHeapAlignment {
		t.Fatalf("allocator ABI chunk/alignment = %d/%d, want %d/%d", abi.ChunkBytes, abi.Alignment, SmallHeapChunkBytes, SmallHeapAlignment)
	}
	if abi.MetadataBytesPerCore <= 0 {
		t.Fatalf("metadata bytes per core = %d, want positive metadata size", abi.MetadataBytesPerCore)
	}
...
func TestPerCoreSmallHeapAllocatorReusesFreedSameClassBlocks(t *testing.T) {
	...
	if report.Cores[0].FreeListBlocks["small_32"] != 0 {
		t.Fatalf("core 0 free list = %+v, want reused class drained", report.Cores[0].FreeListBlocks)
	}
}
