...
func TestValidateMemoryReportRejectsUnsafeCheckedGenericPromotions(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			name:       "safe known",
			claim:      "safe_known",
			provenance: ProvenanceSafeKnown,
			want:       "unsafe_checked",
		},
		{
...
func TestValidateMemoryReportRejectsUnsafeVerifiedRootGenericClaims(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "unsafe_verified_root") {
		t.Fatalf(
			"ValidateReport error = %v, want unsafe_verified_root generic-claim rejection",
			err,
		)
	}
}
	...
	for _, storage := range []StorageClass{StorageStack, StorageRegion, StorageFunctionTempRegion} {
		t.Run(string(storage), func(t *testing.T) {
			...
			if err == nil || !strings.Contains(err.Error(), "unsafe_verified_root") ||
				...
				t.Fatalf(
					"ValidateReport error = %v, want unsafe_verified_root trusted-storage rejection",
					err,
				)
			}
		})
	...
	for _, planned := range []StorageClass{StorageStack, StorageRegion, StorageFunctionTempRegion} {
		t.Run(string(planned), func(t *testing.T) {
			...
			if err == nil ||
				!strings.Contains(
					err.Error(),
					"validated "+string(planned)+" claim cannot lower as Heap",
				) {
				t.Fatalf(
					"ValidateReport error = %v, want trusted storage heap fallback rejection",
					err,
				)
			}
		})
...
func TestValidateMemoryReportRejectsHeapHeapConservativeFallbackWithoutReason(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "reason") {
		t.Fatalf(
			"ValidateReport error = %v, want heap/heap conservative fallback reason rejection",
			err,
		)
	}
}
...
func TestValidateMemoryReportRejectsValidatedCallbackInoutConservativeProjection(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "conservative") {
		t.Fatalf(
			"ValidateReport error = %v, want conservative noalias boundary validation rejection",
			err,
		)
	}
}
...
func TestValidateReportProjectionRejectsDroppedNormalBuildCheck(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "normal_build_check") {
		t.Fatalf(
			"ValidateReportProjection error = %v, want normal_build_check preservation rejection",
			err,
		)
	}
}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := ValidateReport(mutated); err != nil {
				t.Fatalf(
					"mutated report should remain standalone-valid before projection check: %v",
					err,
				)
			}
			...
			if err == nil || !strings.Contains(err.Error(), tc.field) {
				t.Fatalf(
					"ValidateReportProjection error = %v, want %s projection rejection",
					err,
					tc.field,
				)
			}
