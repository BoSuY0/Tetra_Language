...
func FirstUnsupportedRuntimeBuiltin(
	funcs []ir.IRFunc,
	target string,
) (frontend.Position, string, bool) {
...
func RuntimeNameForBuiltin(name string, target string) (string, bool) {
	switch {
	case name == "__tetra_actor_node_connect",
	...
	case strings.HasPrefix(name, "__tetra_surface_"):
		...
		return "surface", true
	case strings.HasPrefix(name, "__tetra_time_"),
...
func BlockedIRPolicy(kind ir.IRInstrKind) (IRPolicy, bool) {
	...
	case ir.IRMemWriteArchPtr:
		return IRPolicy{
			...
			Category: "raw architectural pointer memory access",
		}, true
...
func EmitUIArtifacts(outputPath string, target string, checked *semantics.CheckedProgram) error {
	...
	if target == "wasm32-web" {
		...
		html := wasm32_web.UIHTMLPage(
			filepath.Base(outputPath),
			filepath.Base(WebLoaderPath(outputPath)),
			filepath.Base(uiModulePath),
		)
