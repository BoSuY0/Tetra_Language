...
func TestRuntimeObjectSignatureAndAnnotation(t *testing.T) {
	sig, ok := buildruntime.RuntimeObjectSignature("__tetra_fs_exists")
	if !ok {
		t.Fatalf("RuntimeObjectSignature did not find filesystem symbol")
	}
	if sig.ParamSlots != 3 || sig.ReturnSlots != 1 {
		t.Fatalf(
			"RuntimeObjectSignature slots = (%d, %d), want (3, 1)",
			sig.ParamSlots,
			sig.ReturnSlots,
		)
	}
	...
	if obj.Symbols[0].ParamSlots != 3 || obj.Symbols[0].ReturnSlots != 1 {
		t.Fatalf(
			"annotated slots = (%d, %d), want (3, 1)",
			obj.Symbols[0].ParamSlots,
			obj.Symbols[0].ReturnSlots,
		)
	}
...
func TestValidateRuntimeObjectSymbols(t *testing.T) {
	...
	if err := buildruntime.ValidateRuntimeObjectSymbols(
		obj,
		"missing filesystem runtime object",
		buildruntime.RequiredFilesystemRuntimeSymbols(),
	); err != nil {
		t.Fatalf("ValidateRuntimeObjectSymbols error = %v", err)
	}

	err := buildruntime.ValidateRuntimeObjectSymbols(
		nil,
		"missing filesystem runtime object",
		buildruntime.RequiredFilesystemRuntimeSymbols(),
	)
