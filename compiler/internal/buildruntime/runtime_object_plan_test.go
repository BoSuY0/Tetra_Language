...
func TestDecideRuntimeObjectPlan(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			target:          "linux-x86",
			usage:           RuntimeObjectPlanUsage{TimeRuntimeUsed: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := DecideRuntimeObjectPlan(tt.target, tt.override, CapabilitiesForTarget(tt.target), tt.usage)
			if got.RuntimeUsed != tt.wantRuntimeUsed || got.TimeOnlyRuntime != tt.wantTimeOnly || got.LinuxMinimalRuntime != tt.wantMinimal {
				t.Fatalf("DecideRuntimeObjectPlan = %+v, want runtime=%v timeOnly=%v minimal=%v", got, tt.wantRuntimeUsed, tt.wantTimeOnly, tt.wantMinimal)
			}
		})
	}
}
