...
func SelectRuntimeMode(requested buildapi.RuntimeMode, usage UsageProfile) (buildapi.RuntimeMode, error) {
	switch requested {
...
func RuntimeModeForNativeTarget(target string, requested buildapi.RuntimeMode, selected buildapi.RuntimeMode, usage UsageProfile) (buildapi.RuntimeMode, error) {
	caps := CapabilitiesForTarget(target)
	if !caps.SelfHostActorsRuntime || caps.BuiltinRuntime || requested != buildapi.RuntimeAuto || selected != buildapi.RuntimeBuiltin {
		return selected, nil
	}
...
func SelectRuntimeModeForNativeTarget(target string, requested buildapi.RuntimeMode, usage UsageProfile) (buildapi.RuntimeMode, error) {
	selected, err := SelectRuntimeMode(requested, usage)
	if err != nil {
		if requested == buildapi.RuntimeSelfHost && SelfHostRuntimeSupportsNativeUsage(target, usage) {
			return buildapi.RuntimeSelfHost, nil
		}
		return 0, err
	}
