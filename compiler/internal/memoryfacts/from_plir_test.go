...
func TestFromPLIRAndAllocPlanProjectsIslandMemoryRefFields(t *testing.T) {
	...
	if row.IslandID != "island:isl" || row.Epoch != 1 || row.BaseID != "alloc_intent:xs" {
		t.Fatalf("island memory ref fields = island_id:%q epoch:%d base_id:%q", row.IslandID, row.Epoch, row.BaseID)
	}
	if err := ValidateReportProjection(graph, report); err != nil {
...
func TestFromPLIRAndAllocPlanProjectsIslandEpochAdvancedFact(t *testing.T) {
	...
	if row.IslandID != "island:isl" || row.Epoch != 2 || row.BaseID != "token:isl" {
		t.Fatalf("island epoch advancement fields = island_id:%q epoch:%d base_id:%q", row.IslandID, row.Epoch, row.BaseID)
	}
	if err := ValidateReportProjection(graph, report); err != nil {
...
func TestFromPLIRAndAllocPlanRejectsModuleBoundaryMissingFunctionSummary(t *testing.T) {
	prog := &plir.Program{Funcs: []plir.Function{{
		...
		Ops:   []plir.Operation{{ID: "extern_call", Kind: plir.OpCall, Source: "ffi.tetra:3:5", Inputs: []string{"p"}, Note: "ffi.unknown external call"}},
		Facts: []plir.Fact{{ID: "unknown_p", Kind: plir.FactProvenanceUnknown, ValueID: "param:p", Source: "ffi.tetra:2:19", Reason: "external pointer provenance unknown"}},
	}}}

	_, err := FromPLIRAndAllocPlan("program", prog, nil)
	if err == nil || !strings.Contains(err.Error(), "summary completeness") || !strings.Contains(err.Error(), "FunctionSummary") {
		t.Fatalf("FromPLIRAndAllocPlan error = %v, want missing FunctionSummary completeness rejection", err)
	}
}
...
func TestFromPLIRAndAllocPlanEmitsBorrowCopyVocabulary(t *testing.T) {
	prog := &plir.Program{Funcs: []plir.Function{{
		...
		Ops: []plir.Operation{
			...
			{ID: "op_copy_into", Kind: plir.OpCall, Source: "test.tetra:6:31", Inputs: []string{"copied", "dst"}, Note: "core.slice_copy_into_u8 copies into caller-owned destination without allocation source:copied destination:dst dest_capacity_check:normal_build overlap:distinct_roots"},
		},
		Facts: []plir.Fact{
...
func TestMemoryIdealB03ProjectsCopyIntoOverlapAndCapacityFacts(t *testing.T) {
	prog := &plir.Program{Funcs: []plir.Function{{
		...
		Ops: []plir.Operation{
			{ID: "op_copy_into_overlap", Kind: plir.OpCall, Source: "test.tetra:6:12", Inputs: []string{"src", "dst"}, Note: "core.slice_copy_into_u8 copies into caller-owned destination without allocation source:src destination:dst dest_capacity_check:normal_build overlap:known_overlap"},
		},
		Facts: []plir.Fact{
			{ID: "f_src_window", Kind: plir.FactDerivedWindow, ValueID: "view:src", Range: "xs[0..3]", Source: "test.tetra:4:18", Reason: "safe slice view range is checked before construction"},
			{ID: "f_dst_window", Kind: plir.FactDerivedWindow, ValueID: "view:dst", Range: "xs[1..4]", Source: "test.tetra:5:18", Reason: "safe slice view range is checked before construction"},
			{ID: "f_src_borrow", Kind: plir.FactBorrowedImm, ValueID: "view:src", Source: "test.tetra:4:18", Reason: "source view borrowed"},
			{ID: "f_dst_borrow", Kind: plir.FactBorrowedMut, ValueID: "view:dst", Source: "test.tetra:5:18", Reason: "destination view borrowed mutably"},
		},
	}}}
	...
	if lengthRow.CostClass != CostDynamicCheckRequired || !lengthRow.NormalBuildCheck || lengthRow.ParentFactID == "" {
	...
	if overlapRow.ClaimLevel != ClaimRejected || overlapRow.CostClass != CostUnsupportedRejected || overlapRow.ParentFactID == "" {
...
func TestMemoryIdealB03CopyIntoDistinctOwnedAndUnknownBuffers(t *testing.T) {
	prog := &plir.Program{Funcs: []plir.Function{{
		...
		Ops: []plir.Operation{
			{ID: "op_copy_into_distinct", Kind: plir.OpCall, Source: "test.tetra:5:12", Inputs: []string{"src", "dst"}, Note: "core.slice_copy_into_u8 copies into caller-owned destination without allocation source:src destination:dst dest_capacity_check:normal_build overlap:distinct_roots"},
			{ID: "op_copy_into_unknown", Kind: plir.OpCall, Source: "test.tetra:9:12", Inputs: []string{"raw", "dst"}, Note: "core.slice_copy_into_u8 copies into caller-owned destination without allocation source:raw destination:dst dest_capacity_check:normal_build overlap:unknown_conservative"},
		},
		Facts: []plir.Fact{
			{ID: "f_src_owned", Kind: plir.FactOwned, ValueID: "alloc_intent:src", Source: "test.tetra:3:17", Reason: "source owns allocation"},
			{ID: "f_dst_owned", Kind: plir.FactOwned, ValueID: "alloc_intent:dst", Source: "test.tetra:4:17", Reason: "destination owns allocation"},
			{ID: "f_raw_unknown", Kind: plir.FactProvenanceUnknown, ValueID: "view:raw", Source: "test.tetra:8:17", Reason: "raw slice gateway has external provenance"},
		},
	}}}
	...
	report := BuildReportFromGraph(graph)
	distinctRow, ok := reportRowByClaimAndSource(report, "copy_into_destination_fact_id", "test.tetra:5:12")
	if !ok {
	...
	if distinctRow.CostClass != CostZeroCostProven || distinctRow.ParentFactID == "" {
		t.Fatalf("distinct copy_into destination row = %+v, want parented zero-cost destination fact", distinctRow)
	}
	unknownRow, ok := reportRowByClaimAndSource(report, "copy_into_overlap_conservative", "test.tetra:9:12")
	if !ok {
		t.Fatalf("unknown copy_into missing conservative overlap row:\n%+v", report.Rows)
	}
	if unknownRow.ClaimLevel != ClaimConservative || unknownRow.CostClass != CostConservativeFallback || unknownRow.ParentFactID == "" {
		t.Fatalf("unknown copy_into row = %+v, want parented conservative fallback", unknownRow)
	}
	if _, ok := reportRowByClaimAndSource(report, "copy_into_destination_fact_id", "test.tetra:9:12"); ok {
...
func TestFromPLIRAndAllocPlanProjectsSliceViewDynamicBoundsChecks(t *testing.T) {
	prog := &plir.Program{Funcs: []plir.Function{{
		...
		Ops: []plir.Operation{
			{ID: "op_window_u16", Kind: plir.OpSliceWindow, Source: "test.tetra:4:22", Inputs: []string{"xs", "1", "2"}, Outputs: []string{"view:mid"}, Note: "core.slice_window_u16 range xs[1..3] elem_width:2 elem_shift:1 bounds_check:normal_build"},
			{ID: "op_string_prefix", Kind: plir.OpSliceWindow, Source: "test.tetra:5:27", Inputs: []string{"text", "3"}, Outputs: []string{"view:text"}, Note: "core.string_prefix range text[0..3] elem_width:1 elem_shift:0 bounds_check:normal_build"},
		},
		Facts: []plir.Fact{
			{ID: "f_mid_window", Kind: plir.FactDerivedWindow, ValueID: "view:mid", Range: "xs[1..3]", Source: "test.tetra:4:22", Reason: "safe slice view range is checked before construction"},
			{ID: "f_mid_borrow", Kind: plir.FactBorrowedImm, ValueID: "view:mid", Source: "test.tetra:4:22", Reason: "safe slice view"},
			{ID: "f_mid_region", Kind: plir.FactRegionAlive, ValueID: "view:mid", Source: "test.tetra:4:22", Region: "fn:main"},
			{ID: "f_mid_no_escape", Kind: plir.FactNoEscape, ValueID: "view:mid", Source: "test.tetra:4:22", Reason: "slice view may not escape its owner"},
			{ID: "f_text_window", Kind: plir.FactDerivedWindow, ValueID: "view:text", Range: "text[0..3]", Source: "test.tetra:5:27", Reason: "safe String view range is checked before construction"},
			{ID: "f_text_borrow", Kind: plir.FactBorrowedImm, ValueID: "view:text", Source: "test.tetra:5:27", Reason: "safe String view"},
			{ID: "f_text_region", Kind: plir.FactRegionAlive, ValueID: "view:text", Source: "test.tetra:5:27", Region: "fn:main"},
			{ID: "f_text_no_escape", Kind: plir.FactNoEscape, ValueID: "view:text", Source: "test.tetra:5:27", Reason: "String view may not escape its owner"},
		},
	}}}
		...
		t.Run(tt.name, func(t *testing.T) {
			plan := &allocplan.Plan{Functions: []allocplan.FunctionPlan{{
				...
				Allocations: []allocplan.Allocation{{
					...
					Reason:                "boundary storage remains evidence-only until production runtime validation exists",
			...
			for _, row := range report.Rows {
				...
				if row.ClaimLevel == ClaimValidated || row.ValidatorStatus == ValidatorPass {
					t.Fatalf("%s row was validated without production runtime proof: %+v", tt.name, row)
				}
				if row.PlannedStorage != tt.wantReportStorage || row.ActualLoweringStorage != tt.wantReportStorage || row.LoweredArtifactID == "" {
...
func TestFromPLIRAndAllocPlanProjectsCallbackInoutBoundaryWithoutNoAlias(t *testing.T) {
	prog := &plir.Program{Funcs: []plir.Function{{
		...
		Ops: []plir.Operation{
			{ID: "op_callback_inout", Kind: plir.OpCall, Source: "test.tetra:5:5", Inputs: []string{"dst"}, Note: "cb alias_boundary:function_typed_inout"},
		},
		Facts: []plir.Fact{
			{ID: "f_dst_borrow", Kind: plir.FactBorrowedMut, ValueID: "param:dst", Source: "test.tetra:4:19", Reason: "inout parameter borrowed mutably"},
			{ID: "f_dst_region", Kind: plir.FactRegionAlive, ValueID: "param:dst", Source: "test.tetra:4:19", Region: "fn:callbackBoundary", Reason: "function region is alive"},
			{ID: "f_dst_provenance", Kind: plir.FactProvenanceKnown, ValueID: "param:dst", Source: "test.tetra:4:19", Reason: "parameter provenance known before callback boundary"},
		},
	}}}
	...
	if row.ParentFactID == "" ||
		...
		t.Fatalf("callback_inout_conservative row = %+v, want parented conservative invalidation", row)
	}
}
...
func TestFromPLIRAndAllocPlanProjectsUnknownExternalNoAliasBoundaryWithoutNoAlias(t *testing.T) {
	prog := &plir.Program{Funcs: []plir.Function{{
		...
		Ops: []plir.Operation{
			{ID: "op_unknown_external", Kind: plir.OpCall, Source: "test.tetra:5:5", Inputs: []string{"dst"}, Note: "ffi.external unknown external call alias_boundary:unknown_external_call"},
		},
		Facts: []plir.Fact{
			{ID: "f_dst_borrow", Kind: plir.FactBorrowedMut, ValueID: "param:dst", Source: "test.tetra:4:19", Reason: "inout parameter borrowed mutably"},
			{ID: "f_dst_region", Kind: plir.FactRegionAlive, ValueID: "param:dst", Source: "test.tetra:4:19", Region: "fn:externalBoundary", Reason: "function region is alive"},
			{ID: "f_dst_provenance", Kind: plir.FactProvenanceKnown, ValueID: "param:dst", Source: "test.tetra:4:19", Reason: "parameter provenance known before unknown external boundary"},
		},
	}}}
	...
	report := BuildReportFromGraph(graph)
	if got := countReportClaim(report, "no_alias"); got != 0 {
		t.Fatalf("unknown external boundary report has %d no_alias rows, want none:\n%+v", got, report.Rows)
	}
	...
	if row.ParentFactID == "" ||
		...
		row.ClaimLevel != ClaimConservative ||
		row.ValidatorStatus != ValidatorNotApplicable {
		t.Fatalf("ffi_noalias_invalidated_by_external_call row = %+v, want parented conservative invalidation", row)
	}
}
...
func TestFromPLIRAndAllocPlanEmitsFunctionSummaryVocabulary(t *testing.T) {
	...
	for _, row := range report.Rows {
		...
		if row.Claim == "cap_mem_authorization_only" &&
			...
			t.Fatalf("cap_mem_authorization_only row = %+v, want evidence-only authorization without safe provenance/noalias proof", row)
		}
	}
}
