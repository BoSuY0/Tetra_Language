...
func (g *Graph) DeriveFact(parentID FactID, f Fact) (FactID, error) {
	...
	if isUnsafeUnknown(parent) && isSafeProvenance(f.ProvenanceClass) {
		return "", fmt.Errorf(
			"memoryfacts: unsafe_unknown fact %q cannot derive %s",
			parentID,
			f.ProvenanceClass,
		)
	}
...
func (g *Graph) validateFact(f Fact) error {
	...
	if f.ValidationState == ValidationPass && requiresLoweredArtifact(f) &&
		f.LoweredArtifactID == "" {
		return fmt.Errorf(
			"memoryfacts: validated lowering/storage fact %q requires lowered_artifact_id",
			f.ID,
		)
	}
	...
	if f.ValidationState == ValidationPass && boundsTypedProofClaim(f.Claim) {
		if missing := missingTypedProofFields(
			...
			return fmt.Errorf(
				"memoryfacts: validated bounds proof fact %q requires typed proof fields: %s",
				f.ID,
	...
	if dynamicRawRuntimeCheckCostDisallowed(f.Claim, f.CostClass) {
		return fmt.Errorf(
			"memoryfacts: dynamic raw check fact %q with claim %q must use %s, got %s",
			f.ID,
			f.Claim,
	...
	if zeroCostProvenClaimDisallowed(f) {
		return fmt.Errorf(
			"memoryfacts: zero_cost_proven fact %q requires validated compiler-owned proof for claim %q",
			f.ID,
			f.Claim,
		)
	}
	if f.CostClass == CostDynamicCheckRequired && !f.NormalBuildCheck {
		return fmt.Errorf(
			"memoryfacts: dynamic_check_required fact %q requires normal_build_check",
			f.ID,
		)
	}
	if hasSafeProvenanceFromUnsafeUnknown(f.ProvenanceClass, f.UnsafeClass) {
		return fmt.Errorf(
			"memoryfacts: safe provenance %s cannot be sourced from unsafe_unknown",
			f.ProvenanceClass,
		)
	}
	if isUnsafeUnknown(f) && unsafeUnknownOptimizationClaim(f.Claim, f.AliasState) {
		return fmt.Errorf(
			"memoryfacts: unsafe_unknown fact %q cannot authorize optimization claim %q",
			f.ID,
			f.Claim,
		)
	}
	...
	if f.ValidationState == ValidationPass && conservativeNoAliasBoundaryClaim(f.Claim) {
		return fmt.Errorf(
			"memoryfacts: conservative noalias boundary fact %q cannot be validated",
			f.ID,
		)
	}
	...
	if f.CostClass == CostDynamicCheckRequired && memoryOptimizationClaim(f.Claim, f.AliasState) &&
		!f.NormalBuildCheck {
		return fmt.Errorf(
			("memoryfacts: dynamic_check_required optimization claim %q " +
				"requires normal_build_check for fact %q"),
			f.Claim,
			f.ID,
		)
	}
	if bareBoundsCheckEliminatedClaim(f.Claim) {
		return fmt.Errorf(
			("memoryfacts: bounds_check_eliminated fact %q requires compiler-" +
				"owned proof id; use bounds_check_removed_with_proof_id"),
			f.ID,
		)
	}
	if unsafeVerifiedRootDisallowedClaim(f.ProvenanceClass, f.UnsafeClass, f.Claim) {
		return fmt.Errorf(
			"memoryfacts: unsafe_verified_root fact %q cannot claim %q without bounded raw metadata",
			f.ID,
			f.Claim,
		)
	}
	if unsafeCheckedDisallowedClaim(f.ProvenanceClass, f.UnsafeClass, f.Claim) {
		return fmt.Errorf(
			"memoryfacts: unsafe_checked fact %q cannot claim %q outside checked runtime/bounds evidence",
			f.ID,
			f.Claim,
		)
	}
	if capMemDisallowedProofClaim(f.Claim, f.ValidatorName, f.Reason) {
		return fmt.Errorf(
			("memoryfacts: cap.mem authorization fact %q cannot claim %q; " +
				"cap.mem authorizes raw operations only and does not prove pointer " +
				"validity, bounds, ownership, noalias, or safe provenance"),
			f.ID,
			f.Claim,
		)
	}
	if f.ValidationState == ValidationPass &&
		unsafeExternalRootTrustedStorage(
			f.ProvenanceClass,
			f.UnsafeClass,
			f.StoragePlan,
			f.ActualLoweringStorage,
		) {
		return fmt.Errorf(
			"memoryfacts: unsafe/external root %s/%s fact %q cannot validate trusted storage lowering %q/%q",
			f.ProvenanceClass,
			f.UnsafeClass,
			f.ID,
			f.StoragePlan,
	...
	if f.ValidationState == ValidationPass &&
		validatedTrustedStorageHeapFallback(f.StoragePlan, f.ActualLoweringStorage) {
		return fmt.Errorf(
			"memoryfacts: validated %s claim cannot lower as Heap for fact %q",
			f.StoragePlan,
			f.ID,
		)
	}
	if f.ValidationState == ValidationPass &&
		runtimeProofRequiredStorage(f.StoragePlan, f.ActualLoweringStorage) {
		return fmt.Errorf(
			("memoryfacts: validated runtime boundary storage %q/%q for fact " +
				"%q requires production runtime proof"),
			f.StoragePlan,
			f.ActualLoweringStorage,
			f.ID,
		)
	}
	if f.ValidationState == ValidationPass && strings.Contains(f.Claim, "no_alias") &&
		!validatedNoAliasState(f.AliasState) {
		return fmt.Errorf(
			("memoryfacts: validated no_alias fact %q requires unique or " +
				"mutable_exclusive alias state, got unknown alias %q"),
			f.ID,
			f.AliasState,
		)
	}
	...
	if f.ActualLoweringStorage != "" && !knownStorageClass(f.ActualLoweringStorage) {
		return fmt.Errorf(
			"memoryfacts: unknown actual_lowering_storage %q",
			f.ActualLoweringStorage,
		)
	}
	if (f.StoragePlan == "") != (f.ActualLoweringStorage == "") {
		return fmt.Errorf(
			"memoryfacts: fact %q requires planned_storage and actual_lowering_storage together",
			f.ID,
		)
	}
	if storageFallbackRequiresReason(f.StoragePlan, f.ActualLoweringStorage, f.CostClass) &&
		strings.TrimSpace(f.Reason) == "" {
		return fmt.Errorf(
			"memoryfacts: storage/conservative fallback fact %q requires reason",
			f.ID,
		)
	}
...
func islandBackedFact(f Fact) bool {
	return strings.TrimSpace(f.IslandID) != "" || f.StoragePlan == StorageExplicitIsland ||
		f.ActualLoweringStorage == StorageExplicitIsland
}
