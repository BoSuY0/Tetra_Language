	...
	for _, fn := range prog.Funcs {
		...
		for _, pf := range fn.Facts {
			...
			if !ok && (pf.ValueID != "" || plirFactRequiresValue(pf.Kind)) {
				return fmt.Errorf(
					"memoryfacts: plir function %s fact %q references missing value_id %q",
					fn.Name,
					pf.ID,
					pf.ValueID,
				)
		...
		for _, pf := range fn.Facts {
			...
			case plir.FactOwned:
				if isCopyAllocationValue(value) {
					if err := addCopyMetadataFacts(
						graph,
						parent,
						value,
						opsByOutput[pf.ValueID],
						factIDs,
						values,
					); err != nil {
						return err
			...
			case plir.FactDerivedWindow:
				if err := addSliceViewBoundsCheckFact(
					graph,
					parent,
					value,
					opsByOutput[pf.ValueID],
		...
		for _, op := range fn.Ops {
			...
			if isSafeWrapperPromotionOperation(op) {
				if err := addSafeWrapperPromotionRejectionFact(
					graph,
					fn.Name,
					op,
					values,
