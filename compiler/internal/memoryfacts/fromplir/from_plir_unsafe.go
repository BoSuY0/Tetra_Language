...
func isCopyAllocationValue(value plir.Value) bool {
	return value.Kind == plir.ValueAllocIntent && value.Alloc != nil &&
		copyAllocationBuiltin(value.Alloc.Builtin)
}
...
func sourceFactIDForPath(
	path string,
	factIDs map[plirFactKey]FactID,
	values map[string]plir.Value,
) FactID {
	for _, valueID := range candidateValueIDs(path, values) {
		for _, kind := range []plir.FactKind{
			plir.FactBorrowedImm,
			plir.FactBorrowedMut,
			plir.FactProvenanceKnown,
			plir.FactOwned,
			plir.FactNoEscape,
		} {
...
func claimForPLIRFact(fact plir.Fact) string {
	if fact.Kind == plir.FactProvenanceUnknown &&
		strings.Contains(strings.ToLower(fact.Reason), "raw slice") {
...
func isSafeWrapperPromotionOperation(op plir.Operation) bool {
	...
	return strings.Contains(note, "safe wrapper") &&
		(strings.Contains(
			note,
			"external",
		) || strings.Contains(
...
func finalizeUnsafeOperationFact(
	graph *Graph,
	id FactID,
	claim string,
	...
	if rawBoundsRuntimeCheckClaim(claim) && provenance == ProvenanceUnsafeChecked &&
...
func addRawBoundsRuntimeCheckFact(graph *Graph, parent Fact) error {
	_, err := graph.DeriveFact(parent.ID, Fact{
		...
		Reason: ("Memory Ideal v6 keeps raw bounds width/overflow uncertainty as " +
			"a normal-build check or trap"),
	})
	return err
}
...
func addUnsafeUnknownRejectedSafeFacts(graph *Graph, parent Fact) error {
	_, err := graph.DeriveFact(parent.ID, Fact{
		...
		Reason: ("Memory Ideal v5 rejects unsafe_unknown promotion to safe_known, " +
			"provenance_known, or noalias facts"),
	})
	return err
}

func costClassForUnsafeOperationClaim(
	claim string,
	provenance ProvenanceClass,
	unsafeClass UnsafeClass,
) CostClass {
	...
	switch claim {
	case "derived_allocation_offset",
