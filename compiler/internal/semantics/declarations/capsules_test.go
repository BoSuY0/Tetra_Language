...
func TestValidateCapsuleDeclsRejectsDuplicateAndInvalidMetadata(t *testing.T) {
	...
	if err := ValidateCapsuleDecls(duplicate); err == nil ||
		!strings.Contains(err.Error(), "duplicate capsule metadata key") {
	...
	if err := ValidateCapsuleDecls(invalidKey); err == nil ||
		!strings.Contains(err.Error(), "invalid capsule metadata key") {
	...
	nonLiteral := &frontend.FileAST{
		Capsules: []*frontend.CapsuleDecl{
			{
				Entries: []frontend.CapsuleEntryDecl{
					{Key: "permissions.io", Value: &frontend.IdentExpr{Name: "x"}},
				},
			},
		},
	}
	if err := ValidateCapsuleDecls(nonLiteral); err == nil ||
		!strings.Contains(err.Error(), "must be a literal") {
