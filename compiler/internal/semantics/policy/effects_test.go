...
func TestNormalizeEffectsExpandsAliasesAndGroups(t *testing.T) {
	got, err := NormalizeEffects(
		[]string{"effects.cap.io", "cap.mem", "privacy"},
		frontend.Position{},
		nil,
	)
	if err != nil {
...
func TestNormalizeEffectDeclTracksExplicitEffectsAndCapabilityGroups(t *testing.T) {
	got, err := NormalizeEffectDecl(
		[]string{"effects.cap.io", "cap.mem", "capsule.io"},
		frontend.Position{},
		nil,
	)
	if err != nil {
	...
	for _, effect := range []string{"capability", "io", "mmio"} {
		if _, ok := got.Explicit[effect]; ok {
			t.Fatalf(
				"Explicit unexpectedly contains grouped member %q in %#v",
				effect,
				got.Explicit,
			)
		}
...
func TestNormalizeEffectsUsesDiagnosticCallbackForUnknownEffects(t *testing.T) {
	...
	called := false
	_, err := NormalizeEffects(
		[]string{"unknown.effect"},
		frontend.Position{},
