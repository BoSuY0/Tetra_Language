...
func TestParseRequestViewBorrowsHeadersWithoutAllocating(t *testing.T) {
	raw := []byte("GET /json?queries=2 HTTP/1.1\r\nHost: localhost\r\nConnection: keep-alive\r\n\r\n")
	...
	if consumed != len(raw) || string(req.Path) != "/json" || string(req.Query) != "queries=2" || !req.KeepAlive {
	...
	if report.HeapAllocations != 0 || report.HeaderViewsBorrowed != 2 || report.HeaderValuesCopied != 0 || report.RequestStorage != stdlibrt.StorageBorrowed {
...
func TestAppendResponseWithReportRecordsBufferStorage(t *testing.T) {
	...
	if report.ResponseBufferStorage != stdlibrt.StorageRegion || report.RegionID != "http-response" || report.HeapAllocations != 0 {
...
func TestRequestRegionScopeInjectsRegionForHTTPJSONAndResetsAfterWrite(t *testing.T) {
	...
	handler := func(req RequestView, region *stdlibrt.Region) (Response, error) {
		...
		value, parsed, parseErr := jsonrt.ParseValueView(req.Body, jsonrt.ParseViewOptions{Region: region})
		jsonReport = parsed
		if parseErr != nil {
			return Response{}, parseErr
		}
		if value.Kind != jsonrt.StringKind || !bytes.Equal(value.String.Bytes, wantDecoded) || value.String.Storage != stdlibrt.StorageRegion {
			return Response{}, errors.New("json body was not decoded into request region")
		}
		return Response{StatusCode: 200, ContentType: "text/plain", Body: body, KeepAlive: req.KeepAlive}, nil
	}
	...
	if report.Request.RequestStorage != stdlibrt.StorageRegion || report.Response.ResponseBufferStorage != stdlibrt.StorageRegion {
		t.Fatalf("request/response storage report = %#v/%#v, want region", report.Request, report.Response)
	}
	if report.HeapAllocations != 0 || jsonReport.HeapAllocations != 0 || jsonReport.RegionTemporaries != 1 {
		t.Fatalf("heap/json reports = request %#v json %#v, want region temporaries without heap", report, jsonReport)
	}
	if report.BytesUsedBeforeReset <= 0 || scope.RegionUsed() != 0 {
		t.Fatalf("region reset evidence = used_before=%d used_after=%d", report.BytesUsedBeforeReset, scope.RegionUsed())
	}
}
