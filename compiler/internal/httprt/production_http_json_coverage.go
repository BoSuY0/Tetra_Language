...
func ValidateProductionHTTPJSONCoverage(report ProductionHTTPJSONCoverageReport) error {
	...
	if report.FullProductionWebStackClaimed {
		return fmt.Errorf(
			"production HTTP/JSON coverage: full production web stack claim is forbidden for P19.2",
		)
	}
	if report.OfficialTechEmpowerResultClaimed {
		return fmt.Errorf(
			"production HTTP/JSON coverage: official TechEmpower claim is forbidden for P19.2",
		)
	}
	if report.ProductionPostgreSQLStackClaimed {
		return fmt.Errorf(
			"production HTTP/JSON coverage: production PostgreSQL stack claim is forbidden for P19.2",
		)
	}
	if report.P20PerformanceMatrixClaimed {
		return fmt.Errorf(
			"production HTTP/JSON coverage: P20 performance matrix claim is forbidden for P19.2",
		)
	}
	if report.CPlusPlusRustParityClaimed {
		return fmt.Errorf(
			"production HTTP/JSON coverage: C++/Rust parity claim is forbidden for P19.2",
		)
	}
	if report.RuntimeBehaviorChanged {
		return fmt.Errorf(
			("production HTTP/JSON coverage: runtime behavior change " +
				"claim is forbidden for report-only P19.2 coverage"),
		)
	}
	...
	if len(report.Rows) != len(expectedStatus) {
		return fmt.Errorf(
			"production HTTP/JSON coverage: row count = %d, want %d",
			len(report.Rows),
			len(expectedStatus),
		)
	}
	...
	for _, row := range report.Rows {
		...
		if row.Status != wantStatus {
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q status = %q, want %q",
				row.ID,
				row.Status,
				wantStatus,
			)
		...
		if strings.TrimSpace(row.Name) == "" || strings.TrimSpace(row.Evidence) == "" ||
			...
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q missing evidence or boundary",
				row.ID,
			)
		}
		if len(row.RequiredFacts) == 0 {
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q missing required facts",
				row.ID,
			)
		}
		if row.ClaimsFullProductionWebStack {
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q claims full production web stack",
				row.ID,
			)
		}
		if row.ClaimsOfficialTechEmpowerResult {
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q claims official TechEmpower result",
				row.ID,
			)
		}
		if row.ClaimsProductionPostgreSQLStack {
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q claims production PostgreSQL stack",
				row.ID,
			)
		}
		if row.ClaimsP20PerformanceMatrix {
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q claims P20 performance matrix",
				row.ID,
			)
		}
		if row.ClaimsCPlusPlusRustParity {
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q claims C++/Rust parity",
				row.ID,
			)
		}
		if row.ClaimsRuntimeBehaviorChange {
			return fmt.Errorf(
				"production HTTP/JSON coverage: row %q claims runtime behavior change",
				row.ID,
			)
		}
	}
	...
	checks := []struct {
		...
		{
			ProductionHTTPJSONRequestHeadParser,
			[]string{"lib/core/io/http.tetra", "request_head_len_bytes_at", "ParseRequestView"},
		},
		{
			ProductionHTTPJSONPipelinedRequestHeads,
			[]string{"pipelining", "consumed", "examples/core/platform/core_http_smoke.tetra"},
		},
		{ProductionHTTPJSONHeadersBodyKeepAlive, []string{"Content-Length", "Body", "KeepAlive"}},
		{
			ProductionHTTPJSONNoHeapRequestView,
			[]string{
				"HeapAllocations=0",
				"HeaderViewsBorrowed",
		...
		{
			ProductionHTTPJSONJSONParseStringify,
			[]string{"lib/core/data/json.tetra", "ParseValueView", "AppendMessageObject"},
		},
		{
			ProductionHTTPJSONResponseBuilder,
			[]string{
		...
		{
			ProductionHTTPJSONDateCacheBoundary,
			[]string{
				"HTTPDateCache",
				"FormatWithReport",
		...
		{
			ProductionHTTPJSONWritevSendfileBoundary,
			[]string{
				"netrt.Writev",
				"netrt.Sendfile",
		...
		{
			ProductionHTTPJSONSourceFirstBenchmark,
			[]string{"p19.2_http_json_source_first", "HTTP plaintext", "HTTP JSON"},
		},
	}
	for _, check := range checks {
		...
		for _, want := range check.wants {
			if !containsProductionHTTPJSONRowText(row, want) {
				return fmt.Errorf(
					"production HTTP/JSON coverage: row %q missing fact %q",
					row.ID,
					want,
				)
			}
...
func productionHTTPJSONRequestHeadParserRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		RequiredFacts: []string{
			("lib/core/io/http.tetra::request_head_len_bytes_at locates " +
				"CRLFCRLF request head boundaries from Tetra source"),
			("compiler/internal/httprt/request_view.go::ParseRequestView " +
				"parses method, request target, version, and headers without " +
				"string conversion"),
			("compiler/internal/httprt/http1.go::ParseRequest remains the " +
				"current string-copying server parser and is not the " +
				"zero-heap claim path"),
		},
		Evidence: ("lib/core/io/http.tetra::request_head_len_bytes_at; " +
			"compiler/internal/httprt/request_view.go::ParseRequestView; " +
			"compiler/internal/httprt/http1_test.go::" +
			"TestParseRequestHandlesPartialAndPipelinedRequests"),
		Boundary: ("narrow HTTP/1.1 request-head parsing only; no HTTP/2, TLS, " +
			"chunked transfer decoding, or complete production proxy " +
			"behavior is claimed"),
		SourceFirst: true,
	}
}
...
func productionHTTPJSONPipelinedRequestHeadsRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		Status: ProductionHTTPJSONImplementedNarrow,
		RequiredFacts: []string{
			("examples/core/platform/core_http_smoke.tetra proves " +
				"source-level pipelining by using request_head_len_bytes_at " +
				"to start the second request"),
			("ParseRequestView and ParseRequest report consumed bytes so " +
				"callers can continue parsing a pipelined buffer"),
			("compiler/internal/webrt/webrt_test.go covers plaintext/json " +
				"pipelining through the current local server path"),
		},
		Evidence: ("examples/core/platform/core_http_smoke.tetra; " +
			"compiler/internal/httprt/http1_test.go::" +
			"TestParseRequestHandlesPartialAndPipelinedRequests; " +
		...
		Boundary: ("pipelining evidence is local HTTP/1.1 request sequencing; " +
			"it is not an external load-test throughput or P20 " +
			"performance claim"),
		SourceFirst: true,
	}
}
...
func productionHTTPJSONHeadersBodyKeepAliveRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		Evidence: ("compiler/internal/httprt/request_view.go::" +
			"applyHeaderMetadataView; " +
			"compiler/internal/httprt/request_view_test.go::" +
			"TestRequestRegionScopeInjectsRegionForHTTPJSONAndResetsAfter" +
			"Write; compiler/internal/httprt/http1_test.go::" +
			"TestParseRequestHandlesBodyMetadataAndBoundaries"),
		Boundary: ("header/body coverage is for non-chunked HTTP/1.1 request " +
			"bodies under configured limits"),
		SourceFirst: false,
	}
}
...
func productionHTTPJSONNoHeapRequestViewRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		RequiredFacts: []string{
			...
			("RequestParseReport records HeaderViewsBorrowed and " +
				"HeaderValuesCopied so copy behavior is auditable"),
			("TestParseRequestViewBorrowsHeadersWithoutAllocating uses " +
				"testing.AllocsPerRun to prove the hot request-head parse " +
				"path"),
		},
		Evidence: ("compiler/internal/httprt/request_view.go::" +
			"RequestParseReport; " +
			"compiler/internal/httprt/request_view_test.go::" +
			"TestParseRequestViewBorrowsHeadersWithoutAllocating"),
		Boundary: ("zero-heap evidence is limited to " +
			"request-view/request-region parsing; the current server " +
			"connection buffers and string parser are outside this claim"),
		SourceFirst:   false,
		BorrowedViews: true,
		NoHeapHotPath: true,
	}
...
func productionHTTPJSONJSONParseStringifyRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		Status: ProductionHTTPJSONImplementedNarrow,
		RequiredFacts: []string{
			("lib/core/data/json.tetra writes deterministic JSON message " +
				"objects and escaped strings from Tetra source"),
			("compiler/internal/jsonrt/view.go::ParseValueView parses " +
				"borrowed/region JSON values and reports copies only when " +
				"escaped strings require decoding"),
			("compiler/internal/jsonrt/json.go::AppendMessageObject " +
				"serializes the TechEmpower JSON endpoint payload"),
		},
		Evidence: ("lib/core/data/json.tetra; " +
			"examples/core/data/core_json_smoke.tetra; " +
			"compiler/internal/jsonrt/view_test.go::" +
			"TestParseValueViewBorrowsUnescapedStringsWithoutHeap; " +
			"compiler/internal/jsonrt/json_test.go::" +
			"TestAppendMessageObjectWritesTechEmpowerPayload"),
		Boundary: ("JSON support is byte-oriented and ASCII escape focused; " +
			"this is not a full public JSON DOM/API stability claim"),
		SourceFirst: true,
	}
}
...
func productionHTTPJSONResponseBuilderRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		Status: ProductionHTTPJSONImplementedNarrow,
		RequiredFacts: []string{
			"lib/core/io/http.tetra::write_plaintext_response writes the source-level plaintext response",
			("lib/core/io/http.tetra::write_json_message_response writes " +
				"the source-level JSON response using lib.core.json"),
			("compiler/internal/httprt/request_view.go::" +
				"AppendResponseWithReport records response buffer storage " +
				"and heap allocation facts"),
		},
		Evidence: ("lib/core/io/http.tetra::write_plaintext_response; " +
			"lib/core/io/http.tetra::write_json_message_response; " +
			"compiler/internal/httprt/request_view_test.go::" +
			"TestAppendResponseWithReportRecordsBufferStorage; " +
			"examples/core/platform/core_http_smoke.tetra"),
		Boundary: ("response building covers deterministic headers/body " +
			"serialization; socket write scheduling and throughput " +
...
func productionHTTPJSONDateCacheBoundaryRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		Status: ProductionHTTPJSONImplementedNarrow,
		RequiredFacts: []string{
			("compiler/internal/webrt/date_cache.go::HTTPDateCache caches " +
				"formatted IMF-fixdate values by UTC Unix second"),
			("compiler/internal/webrt/date_cache.go::FormatWithReport " +
				"reports refresh/reuse facts for P19.2 evidence"),
			"compiler/internal/webrt/server.go::Server.date uses HTTPDateCache when Config.DateFunc is nil",
			("compiler/internal/webrt/server.go::Config.NowFunc gives " +
				"deterministic server-date cache tests without overriding " +
				"Date"),
			("compiler/internal/webrt/webrt_test.go::" +
				"TestHTTPDateCacheRefreshesOncePerSecond proves same-second " +
				"reuse and boundary refresh"),
			("compiler/internal/webrt/webrt_test.go::" +
				"TestServerDateFuncOverrideBypassesCache proves DateFunc " +
				"override priority"),
		},
		MissingFacts: []string{
			"source-level lib.core.http cached-date helper remains out of scope",
		...
		Evidence: ("compiler/internal/webrt/date_cache.go; " +
			"compiler/internal/webrt/webrt_test.go::" +
			"TestHTTPDateCacheRefreshesOncePerSecond; " +
			"compiler/internal/webrt/webrt_test.go::" +
			"TestServerDateUsesPerSecondCacheWhenDateFuncAbsent; " +
			"compiler/internal/webrt/webrt_test.go::" +
			"TestServerPlaintextKeepAliveAndPipelining"),
		Boundary: ("Date caching is implemented only as an internal per-server " +
			"UTC-second cache; no source-level cached-date API, " +
			"cross-worker cache, performance claim, or full production " +
			"web-stack promotion is made"),
		SourceFirst: false,
	}
}
...
func productionHTTPJSONWritevSendfileBoundaryRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		Status: ProductionHTTPJSONImplementedNarrow,
		RequiredFacts: []string{
			("compiler/internal/netrt/netrt_linux.go::Writev calls Linux " +
				"SYS_WRITEV over non-empty byte chunks"),
			"compiler/internal/netrt/netrt_linux.go::Sendfile delegates to Linux syscall.Sendfile",
			("compiler/internal/netrt/netrt_unsupported.go::Writev " +
				"returns ErrUnsupported on non-Linux targets"),
			("compiler/internal/netrt/netrt_unsupported.go::Sendfile " +
				"returns ErrUnsupported on non-Linux targets"),
			("compiler/internal/netrt/netrt_linux_test.go::" +
				"TestWritevWritesMultipleBuffersOnConnectedTCP proves " +
				"netrt.Writev on a connected TCP fd"),
			("compiler/internal/netrt/netrt_linux_test.go::" +
				"TestSendfileCopiesFileBytesToConnectedTCPAndAdvancesOffset " +
				"proves netrt.Sendfile on a file-to-TCP fd and offset " +
				"advancement"),
			("compiler/internal/webrt/server.go::flush / webrt.flush " +
				"remains single-buffer netrt.Write in this helper slice"),
		},
		MissingFacts: []string{
		...
		Evidence: ("compiler/internal/netrt/netrt_linux.go::Writev; " +
			"compiler/internal/netrt/netrt_linux.go::Sendfile; " +
			"compiler/internal/netrt/netrt_linux_test.go::" +
			"TestWritevWritesMultipleBuffersOnConnectedTCP; " +
			"compiler/internal/netrt/netrt_linux_test.go::" +
			"TestSendfileCopiesFileBytesToConnectedTCPAndAdvancesOffset; " +
			"compiler/internal/webrt/server.go::flush"),
		Boundary: ("writev/sendfile support is limited to Linux netrt helpers; " +
			"no HTTP static-file response path, webrt scatter/gather " +
			"integration, zero-copy production file-serving, performance " +
			"claim, or cross-platform parity is made"),
		SourceFirst: false,
	}
}
...
func productionHTTPJSONSourceFirstBenchmarkRow() ProductionHTTPJSONEvidenceRow {
	return ProductionHTTPJSONEvidenceRow{
		...
		RequiredFacts: []string{
			...
			("HTTP plaintext and HTTP JSON benchmark rows must list proof," +
				" allocation, bounds, and P19.2 evidence artifacts"),
		},
		Evidence: ("tools/cmd/truth-bench-harness/main.go::" +
			"policyForBenchmarkScope; " +
			"tools/cmd/truth-bench-harness/main_test.go::" +
			"TestP19HTTPJSONSourceFirstScopeRequiresTetraOnlyHTTPAndJSONR" +
			"ows; " +
			"reports/production-http-json-v1/benchmarks/http-json-source-" +
			"first-report.json"),
		Boundary: ("source-first benchmark gate records local dry-run/source " +
			"evidence only; it is not an official TechEmpower result, " +
			"P20 performance matrix, or C++/Rust parity claim"),
		SourceFirst: true,
	}
}
