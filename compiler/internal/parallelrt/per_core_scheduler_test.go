...
func TestPerCoreSchedulerCoverageCoversP18PlanList(t *testing.T) {
	...
	for _, row := range report.Rows {
		...
		if row.Status != PerCoreSchedulerImplementedNarrow {
			t.Fatalf(
				...
				row.ID,
				row.Status,
				PerCoreSchedulerImplementedNarrow,
			)
		}
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerPerCoreQueues],
		"per-core queues",
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerWorkStealing],
		"work stealing",
		"Steals=1",
		"StepCore",
	)
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerBoundedTypedMailboxes],
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerBackpressure],
		"backpressure",
		"ErrMailboxFull",
		"blocking_recv_yield",
	)
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerTimersSleepWake],
		"timers",
		"sleep_ms",
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerStructuredTaskGroups],
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerCancellationCheckpoints],
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerActorPingPong],
		"actor ping-pong",
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerFanoutFanin],
		"fanout/fanin",
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerTaskGroupCancel],
		"task group cancel",
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerBackpressureOverflow],
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerMailboxFairness],
		"mailbox fairness",
		"FIFO receive",
	...
	requirePerCoreFacts(
		t,
		byID[PerCoreSchedulerStressRaceDetector],
		"stress evidence",
		"many tasks stress",
...
func TestPerCoreSchedulerCoverageRejectsFakeClaims(t *testing.T) {
	...
	if err := ValidatePerCoreSchedulerCoverage(fakeActorRuntime); err == nil ||
	...
	if err := ValidatePerCoreSchedulerCoverage(fakeRace); err == nil ||
		!strings.Contains(err.Error(), "race detector") {
	...
	if err := ValidatePerCoreSchedulerCoverage(missingStress); err == nil ||
	...
	if err := ValidatePerCoreSchedulerCoverage(fakeRuntimeChange); err == nil ||
	...
	if err := ValidatePerCoreSchedulerCoverage(noNonClaim); err == nil ||
...
func clonePerCoreSchedulerCoverage(
	report PerCoreSchedulerCoverageReport,
) PerCoreSchedulerCoverageReport {
	clone := report
