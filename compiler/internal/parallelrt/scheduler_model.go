...
func (m *SchedulerModel) domainMovesForMessage(msg Message, targetOwner string) []DomainOwnershipMove {
	if m == nil {
		return nil
	}
...
func PrototypeActorMemoryDomains() ([]ActorMemoryDomainReport, error) {
	...
	if _, err := regionBox.Send(Message{
		...
		return nil, fmt.Errorf("prototype actor memory domain: large owned region send error = %v, want ErrMailboxFull", err)
	}
...
func ValidateActorMemoryDomainReport(report ActorMemoryDomainReport) error {
	...
	if !report.RuntimeMeasured && strings.TrimSpace(report.RuntimeBlockedReason) == "" {
		return fmt.Errorf("actor memory domain report: runtime_blocked_reason is required when runtime_measured=false")
	}
	...
	if report.Domain.Kind != runtimeabi.DomainActor {
		return fmt.Errorf("actor memory domain report: domain kind = %q, want actor", report.Domain.Kind)
	}
	...
	if report.Mailbox.QueuedMessages < 0 || report.Mailbox.QueuedBytes < 0 || report.Mailbox.ReclaimedBytes < 0 {
		return fmt.Errorf("actor memory domain report: mailbox byte/message counts must not be negative")
	}
	if report.Mailbox.CapacityBytes > 0 && report.Mailbox.QueuedBytes > report.Mailbox.CapacityBytes {
	...
	if report.MessagePool.LiveBytes != report.Mailbox.QueuedBytes {
		return fmt.Errorf("actor memory domain report: message pool live bytes = %d, want mailbox queued bytes %d", report.MessagePool.LiveBytes, report.Mailbox.QueuedBytes)
	}
	if report.MessagePool.ReclaimedBytes != report.Mailbox.ReclaimedBytes {
		return fmt.Errorf("actor memory domain report: message pool reclaimed bytes = %d, want mailbox reclaimed bytes %d", report.MessagePool.ReclaimedBytes, report.Mailbox.ReclaimedBytes)
	}
	if report.MessagePool.CapacityBytes != report.Mailbox.CapacityBytes {
		return fmt.Errorf("actor memory domain report: message pool capacity bytes = %d, want mailbox capacity bytes %d", report.MessagePool.CapacityBytes, report.Mailbox.CapacityBytes)
	}
	ownedRegionBytes := sumOwnedRegionBytes(report.OwnedRegions)
	if ownedRegionBytes > report.Mailbox.QueuedBytes {
		return fmt.Errorf("actor memory domain report: owned region bytes = %d, want <= queued bytes %d", ownedRegionBytes, report.Mailbox.QueuedBytes)
	}
	if report.Domain.CurrentBytes != int64(report.Mailbox.QueuedBytes) {
		return fmt.Errorf("actor memory domain report: domain current bytes = %d, want queued bytes %d", report.Domain.CurrentBytes, report.Mailbox.QueuedBytes)
	}
	if report.Domain.CommittedBytes != int64(report.Mailbox.CapacityBytes) {
		return fmt.Errorf("actor memory domain report: committed bytes = %d, want mailbox capacity bytes %d", report.Domain.CommittedBytes, report.Mailbox.CapacityBytes)
	}
	if report.Domain.ReleasedBytes != int64(report.Mailbox.ReclaimedBytes) {
		return fmt.Errorf("actor memory domain report: released bytes = %d, want reclaimed bytes %d", report.Domain.ReleasedBytes, report.Mailbox.ReclaimedBytes)
	}
	for _, owned := range report.OwnedRegions {
		...
		if owned.DomainID != report.Domain.DomainID {
			return fmt.Errorf("actor memory domain report: owned region %s domain_id = %q, want %q", owned.RegionName, owned.DomainID, report.Domain.DomainID)
		}
		if owned.OwnerID != report.ActorID {
			return fmt.Errorf("actor memory domain report: owned region %s owner_id = %q, want actor %q", owned.RegionName, owned.OwnerID, report.ActorID)
		}
		if owned.Bytes <= 0 {
			return fmt.Errorf("actor memory domain report: owned region %s bytes must be positive", owned.RegionName)
		}
	}
	switch report.Backpressure.Status {
	case "available", "message_limit_reached", "byte_limit_reached":
	default:
		return fmt.Errorf("actor memory domain report: unknown backpressure status %q", report.Backpressure.Status)
	}
