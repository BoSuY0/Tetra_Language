...
func TestSmallScalarMustSendByCopy(t *testing.T) {
	...
	err := checker.Check(
		[]Event{{Kind: EventSend, Value: "count", Mode: SendMove, Site: "app.tetra:3"}},
	)
	if err == nil || !strings.Contains(err.Error(), "small scalar") ||
		!strings.Contains(err.Error(), "by copy") {
		t.Fatalf("scalar move send error = %v, want copy-only contract", err)
	}
	checker = NewChecker([]Value{{Name: "count", Type: "i32", Kind: ValueCopy}})
	if err := checker.Check(
		[]Event{{Kind: EventSend, Value: "count", Mode: SendCopy, Site: "app.tetra:4"}},
	); err != nil {
...
func TestBorrowedSliceAcrossActorBoundaryRejectsUnlessCopied(t *testing.T) {
	...
	err := checker.Check(
		[]Event{{Kind: EventSend, Value: "xs", Mode: SendBorrowed, Site: "app.tetra:7"}},
	)
	if err == nil ||
		!strings.Contains(
			err.Error(),
	...
	checker = NewChecker([]Value{{Name: "xs", Type: "[]u8", Kind: ValueBorrowed}})
	if err := checker.Check(
		[]Event{{Kind: EventSend, Value: "xs", Mode: SendCopy, Site: "app.tetra:8"}},
	); err != nil {
...
func TestOwnedStringCanCopyOrMoveDependingOwnership(t *testing.T) {
	...
	if err := checker.Check(
		[]Event{{Kind: EventSend, Value: "text", Mode: SendCopy, Site: "app.tetra:9"}},
	); err != nil {
		t.Fatalf("owned String copy send should pass: %v", err)
	}
	checker = NewChecker([]Value{{Name: "text", Type: "String", Kind: ValueOwned}})
	if err := checker.Check(
		[]Event{{Kind: EventSend, Value: "text", Mode: SendMove, Site: "app.tetra:10"}},
	); err != nil {
		t.Fatalf("owned String move send should pass: %v", err)
	}
	checker = NewChecker([]Value{{Name: "text", Type: "String", Kind: ValueOwned}})
	err := checker.Check(
		[]Event{{Kind: EventSend, Value: "text", Mode: SendBorrowed, Site: "app.tetra:11"}},
	)
...
func TestOwnedRegionMustMoveAndSenderUseAfterMoveRejects(t *testing.T) {
	...
	err := checker.Check(
		[]Event{{Kind: EventSend, Value: "request_region", Mode: SendCopy, Site: "app.tetra:10"}},
	)
...
func TestActorSendabilityStableDiagnosticsMatrix(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			events: []Event{
				{Kind: EventSend, Value: "payload", Mode: SendBorrowed, Site: "worker.t4:7"},
			},
			want: []string{
				"actor sendability",
				"worker.t4:7",
...
func TestUnsafePointerRequiresExplicitUnsafeSendContract(t *testing.T) {
	...
	err := checker.Check(
		[]Event{{Kind: EventSend, Value: "raw", Mode: SendUnsafe, Site: "app.tetra:20"}},
	)
	if err == nil ||
		!strings.Contains(
			err.Error(),
	...
	checker = NewChecker(
		[]Value{{Name: "raw", Type: "ptr", Kind: ValueUnsafePtr, UnsafeSendContract: true}},
	)
	if err := checker.Check(
		[]Event{{Kind: EventSend, Value: "raw", Mode: SendUnsafe, Site: "app.tetra:21"}},
	); err != nil {
...
func TestTypedMailboxRequiresCapacityAndBackpressure(t *testing.T) {
	if err := VerifyMailbox(
		Mailbox{Name: "worker", Message: "Frame", Capacity: 32, Backpressure: "block"},
	); err != nil {
