...
func TestTypedActorOwnershipTransferCoverageCoversP18PlanList(t *testing.T) {
	...
	expected := []TypedActorOwnershipTransferID{
		...
		TypedActorOwnershipWrapperPayloadRejection,
	...
	for _, id := range expected {
		...
		if row.Status != TypedActorOwnershipImplementedNarrow {
			t.Fatalf(
				"row %q status = %q, want %q",
				id,
				row.Status,
				TypedActorOwnershipImplementedNarrow,
			)
		}
	...
	for _, want := range []string{"borrowed view rejected", "explicit copy accepted", ".copy()"} {
		if !hasOwnershipTransferText(
			byID[TypedActorOwnershipBorrowedViewCopyBoundary].RequiredFacts,
			want,
		) {
			t.Fatalf(
				"borrowed-view row missing fact %q: %#v",
				want,
				byID[TypedActorOwnershipBorrowedViewCopyBoundary].RequiredFacts,
			)
		}
	...
	for _, want := range []string{"owned region can move", "bytes_copied=0", "zero_copy_move"} {
		if !hasOwnershipTransferText(byID[TypedActorOwnershipOwnedRegionMove].RequiredFacts, want) {
			t.Fatalf(
				"owned-region row missing fact %q: %#v",
				want,
				byID[TypedActorOwnershipOwnedRegionMove].RequiredFacts,
			)
		}
	}
	for _, want := range []string{"cannot use consumed value", "cannot use moved region after send"} {
		if !hasOwnershipTransferText(
			byID[TypedActorOwnershipSenderUseAfterMove].RequiredFacts,
			want,
		) {
			t.Fatalf(
				"sender-use row missing fact %q: %#v",
				want,
				byID[TypedActorOwnershipSenderUseAfterMove].RequiredFacts,
			)
		}
	...
	for _, want := range []string{
		...
		if !hasOwnershipTransferText(
			byID[TypedActorOwnershipReceiverOwnsMovedRegion].RequiredFacts,
			want,
		) {
			t.Fatalf(
				"receiver row missing fact %q: %#v",
				want,
				byID[TypedActorOwnershipReceiverOwnsMovedRegion].RequiredFacts,
			)
		}
	...
	for _, want := range []string{"FactMoved", "OpActorSend", "core.send_typed"} {
		if !hasOwnershipTransferText(byID[TypedActorOwnershipPLIRMovedFacts].RequiredFacts, want) {
			t.Fatalf(
				"PLIR row missing fact %q: %#v",
				want,
				byID[TypedActorOwnershipPLIRMovedFacts].RequiredFacts,
			)
		}
	}
	for _, want := range []string{"ActorMemoryDomain", "byte_limit_reached", "DomainMoves"} {
		if !hasOwnershipTransferText(
			...
			t.Fatalf(
				"runtime mailbox row missing fact %q: %#v",
				want,
				byID[TypedActorOwnershipRuntimeMailboxRepresentation].RequiredFacts,
			)
		}
...
func TestTypedActorOwnershipTransferCoverageRejectsFakeClaims(t *testing.T) {
	...
	if err := ValidateTypedActorOwnershipTransferCoverage(distributed); err == nil ||
		!strings.Contains(err.Error(), "distributed zero-copy") {
	...
	if err := ValidateTypedActorOwnershipTransferCoverage(runtimeChanged); err == nil ||
		!strings.Contains(err.Error(), "runtime behavior") {
	...
	if err := ValidateTypedActorOwnershipTransferCoverage(missingPLIR); err == nil ||
		!strings.Contains(err.Error(), "PLIR moved facts") {
	...
	if err := ValidateTypedActorOwnershipTransferCoverage(missingStress); err == nil ||
		!strings.Contains(err.Error(), "stress diagnostics") {
	...
	if err := ValidateTypedActorOwnershipTransferCoverage(noNonClaim); err == nil ||
...
func cloneTypedActorOwnershipTransferReport(
	report TypedActorOwnershipTransferReport,
) TypedActorOwnershipTransferReport {
	clone := report
