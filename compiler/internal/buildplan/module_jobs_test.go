...
func TestEffectiveWorkerCount(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.want {
				t.Fatalf("EffectiveWorkerCount(%d, %d, %d) = %d, want %d", tt.requested, tt.maxJobs, tt.fallback, got, tt.want)
			}
		})
	}
}
...
func TestApplyModuleObjectMetadata(t *testing.T) {
	...
	if obj.CompilerVersion != "test-version" || obj.PublicAPIHash != "api-hash" {
		t.Fatalf("metadata = (%q, %q), want test-version/api-hash", obj.CompilerVersion, obj.PublicAPIHash)
	}
