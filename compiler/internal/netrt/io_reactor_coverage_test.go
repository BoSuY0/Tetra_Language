...
func TestIOReactorCoverageCoversP18PlanList(t *testing.T) {
	...
	requireIOReactorFacts(
		t,
		byID[IOReactorLinuxEpollV1],
		"epoll v1",
		"NewPoller",
		"EPOLLIN",
		"EPOLLOUT",
	...
	requireIOReactorFacts(
		t,
		byID[IOReactorWASIWebBoundary],
		"WASI/web event adapters",
		"not implemented",
	)
	requireIOReactorFacts(
		t,
		byID[IOReactorNonblockingAcceptReadWrite],
		"Accept4",
		"SOCK_NONBLOCK",
		"Read",
		"Write",
	...
	requireIOReactorFacts(
		t,
		byID[IOReactorReadinessPolling],
		"Poller.Wait",
		"wait-one readiness",
	...
	requireIOReactorFacts(
		t,
		byID[IOReactorTaskWakeupsFromIO],
		"I/O readiness wakes",
		"poller.Wait",
		"core.task_spawn_i32",
	)
	requireIOReactorFacts(
		t,
		byID[IOReactorTimerIntegration],
		"50ms poll timeout",
		"sleep_ms",
	...
	if byID[IOReactorLinuxEpollV1].Status != IOReactorImplementedNarrow {
		t.Fatalf(
			"linux epoll status = %q, want %q",
			byID[IOReactorLinuxEpollV1].Status,
			IOReactorImplementedNarrow,
		)
	}
	for _, id := range []IOReactorEvidenceID{
		IOReactorIOUringFuture,
		IOReactorKqueueBoundary,
		IOReactorIOCPBoundary,
		IOReactorWASIWebBoundary,
		...
		if byID[id].Status != IOReactorBoundaryDocumented {
			t.Fatalf(
				"platform boundary row %q status = %q, want %q",
				id,
				byID[id].Status,
				IOReactorBoundaryDocumented,
			)
		}
	}
}
...
func TestIOReactorCoverageRejectsFakeClaims(t *testing.T) {
	...
	if err := ValidateIOReactorCoverage(fakeWebStack); err == nil ||
	...
	if err := ValidateIOReactorCoverage(fakeParity); err == nil ||
		!strings.Contains(err.Error(), "cross-platform") {
	...
	if err := ValidateIOReactorCoverage(fakeIOUring); err == nil ||
		!strings.Contains(err.Error(), "io_uring") {
	...
	if err := ValidateIOReactorCoverage(fakeRuntimeChange); err == nil ||
	...
	if err := ValidateIOReactorCoverage(fakeKqueue); err == nil ||
		!strings.Contains(err.Error(), "kqueue") {
	...
	if err := ValidateIOReactorCoverage(missingStress); err == nil ||
		!strings.Contains(err.Error(), "stress") {
	...
	if err := ValidateIOReactorCoverage(missingHTTP); err == nil ||
	...
	if err := ValidateIOReactorCoverage(noNonClaim); err == nil ||
