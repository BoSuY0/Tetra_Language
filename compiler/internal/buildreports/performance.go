...
func p20PerformanceBlockers() []PerformanceBlockerRow {
	rows := []PerformanceBlockerRow{
		...
		},
		{
			...
			CostClass: "conservative_fallback",
			Evidence:  "allocation reports may prove NoEscape for read-only local call arguments while keeping heap storage until call-aware stack or region lowering is validated",
			NextStep:  "prove call-aware stack or region lowering for the local call boundary, or keep the explicit heap fallback",
		},
		{
...
func p20PerformanceBenchmarkExplanations() []PerformanceBenchmarkExplanation {
	specs := []struct {
		...
		{benchmark: "hash_table_tetra", category: "hash table", reasons: []string{"allocation.local_call_heap_fallback", "inline.code_size_budget"}},
