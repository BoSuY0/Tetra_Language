...
func BuildLayoutReport(target string, checked *semantics.CheckedProgram) LayoutReport {
	report := LayoutReport{
		...
		Claims: []string{
			...
			("Default struct layout is compiler-owned and does not promise C " +
				"field order, padding, or public ABI layout."),
			("repr(C) locks layout for ABI-facing code and denies field " +
				"reordering, padding removal, hot/cold splitting, scalar replacement, " +
				"and AoS-to-SoA transforms."),
			"public ABI/exported FFI requires explicit repr(C).",
			("No field reordering, padding removal, hot/cold splitting, " +
				"scalar replacement, AoS-to-SoA transform, performance change, or " +
				"runtime behavior change is claimed by this report."),
		},
	}
	if checked == nil {
		return report
	...
	for _, st := range checked.Structs {
		...
		row := LayoutDecisionRow{
			...
			Reason: ("default struct layout is compiler-owned; public ABI/exported " +
				"FFI requires explicit repr(C)"),
		}
		if info != nil {
...
func ValidateLayoutReport(report LayoutReport) error {
	...
	if report.Summary.Structs != len(report.Decisions) {
		return fmt.Errorf(
			"layout report summary structs = %d, decisions = %d",
			report.Summary.Structs,
	...
	if !reflect.DeepEqual(report.Summary, counts) {
		return fmt.Errorf(
			"layout report summary mismatch: got %+v want %+v",
			report.Summary,
			counts,
		)
	}
...
func validateLayoutDecisionRow(row LayoutDecisionRow) error {
	...
	if strings.TrimSpace(row.Decision) == "" || strings.TrimSpace(row.PublicABI) == "" ||
		containsWeakReportText(row.Reason) {
	...
	case "exported_ffi_explicit_repr_c":
		if row.Repr != frontend.StructReprC {
			return fmt.Errorf(
				"layout decision row %s claims exported FFI explicit repr(C) without repr(C)",
				row.Type,
			)
		}
	case "exported_ffi_missing_explicit_repr":
		return fmt.Errorf(
			"layout decision row %s is exported public ABI without explicit repr(C)",
			row.Type,
		)
	default:
		return fmt.Errorf(
			"layout decision row %s has unknown public ABI state %q",
			row.Type,
			row.PublicABI,
		)
	}
	...
	case frontend.StructReprC:
		...
		if row.Decision != "abi_locked_repr_c" {
			return fmt.Errorf(
				"layout decision row %s repr(C) decision = %q",
				row.Type,
				row.Decision,
			)
		}
		if len(row.AllowedTransforms) != 0 {
			return fmt.Errorf(
				"layout decision row %s repr(C) must not allow layout transforms",
				row.Type,
			)
		}
		for _, transform := range p21LayoutTransforms {
			if !stringListContains(row.DeniedTransforms, transform) {
				return fmt.Errorf(
					"layout decision row %s repr(C) missing denied transform %q",
					row.Type,
					transform,
				)
			}
	...
	default:
		if row.ABILocked {
			return fmt.Errorf(
				"layout decision row %s default struct must not claim ABI lock",
				row.Type,
			)
		}
		if row.Decision != "compiler_owned_default" {
			return fmt.Errorf(
				"layout decision row %s default decision = %q",
				row.Type,
				row.Decision,
			)
		}
		for _, transform := range p21LayoutTransforms {
			if !stringListContains(row.AllowedTransforms, transform) {
				return fmt.Errorf(
					"layout decision row %s default struct missing allowed transform %q",
					row.Type,
					transform,
				)
			}
...
func collectStructLayoutABITypeUse(
	typeName string,
