...
func actorMailboxMessageTypeFromCall(
...
func actorMailboxRowForMessage(
	typeName string,
	types map[string]*semantics.TypeInfo,
	target string,
) (ActorMailboxRow, bool) {
...
func actorTransferRowsForSend(
	...
	if call.Name == "core.send" {
		return actorTransferRowsForScalarSend(function, call)
	}
	if call.Name != "core.send_typed" || len(call.Args) < 2 {
		return nil
	}
	msgCall, ok := call.Args[1].(*frontend.CallExpr)
	if !ok {
	...
	for i, payloadType := range caseInfo.PayloadTypes {
		...
		if row, ok := actorTransferRowForPayload(
			function,
			call,
			msgType,
			caseInfo,
...
func actorTransferRowsForScalarSend(
	function string,
	call *frontend.CallExpr,
) []ActorTransferRow {
	...
	return []ActorTransferRow{{
		Function:                   function,
		Site:                       reportPos(call.At),
		MessageType:                "scalar:i32",
		Case:                       "core.send",
		PayloadType:                "i32",
		Ownership:                  "copy",
		TransferMode:               "copy",
		RuntimePath:                "actor_mailbox_scalar_value_slot",
		BytesCopied:                actorMailboxSlotWidthBytes(""),
		ZeroCopy:                   false,
		ClaimLevel:                 "evidence_only",
		BoundaryScope:              "local_scalar_mailbox",
		ProductionRuntimeValidated: false,
		Reason:                     "scalar i32 payload crosses actor mailbox by value-slot copy",
	}}
}
...
func actorTransferRowForPayload(
	...
	slotBytes := reportPayloadSlotCount(
		caseInfo,
		index,
		payloadType,
		types,
	) * actorMailboxSlotWidthBytes(
		"",
	)
	...
	case semantics.TypeStr, semantics.TypeSlice:
		...
		if reportTypeKind(payloadType, types) == semantics.TypeSlice {
			...
			base.Reason = ("owned region-backed slice moves with its island owner in the " +
				"same typed actor payload")
			return base, true
		}
...
func reportEnumCaseConstructor(
	call *frontend.CallExpr,
	types map[string]*semantics.TypeInfo,
...
func reportOwnedRegionSliceOwner(expr frontend.Expr, owners []string) string {
	...
	if call, ok := expr.(*frontend.CallExpr); ok && len(call.Args) > 0 &&
		reportIsIslandMakeCall(call.Name) {
