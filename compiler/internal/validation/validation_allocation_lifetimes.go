...
func ValidateAllocationLoweringWithSummaryProgram(
	plan *allocplan.Plan,
	prog *ir.IRProgram,
	summaryProg *ir.IRProgram,
) error {
	return validateAllocationLowering(plan, prog, summaryProg)
}

func validateAllocationLowering(
	plan *allocplan.Plan,
	prog *ir.IRProgram,
	summaryProg *ir.IRProgram,
) error {
	...
	for _, fn := range plan.Functions {
		for _, alloc := range fn.Allocations {
			...
			case allocplan.StorageFunctionTempRegion:
				...
				if !ok {
					return fmt.Errorf(
						("allocation lowering validation: %s allocation %q reports actual " +
							"FunctionTempRegion for unsupported builtin %q"),
						fn.Name,
						alloc.ID,
						alloc.Builtin,
					)
			...
			case allocplan.StorageExplicitIsland:
				...
				if !ok {
					return fmt.Errorf(
						("allocation lowering validation: %s allocation %q reports actual " +
							"ExplicitIsland for unsupported builtin %q"),
						fn.Name,
						alloc.ID,
						alloc.Builtin,
					)
				...
				if strings.TrimSpace(alloc.RegionID) == "" {
					return fmt.Errorf(
						("allocation lowering validation: %s allocation %q reports actual " +
							"ExplicitIsland without region id"),
						fn.Name,
						alloc.ID,
					)
				}
				if strings.TrimSpace(alloc.Lifetime) == "" {
					return fmt.Errorf(
						("allocation lowering validation: %s allocation %q reports actual " +
							"ExplicitIsland without lifetime"),
						fn.Name,
						alloc.ID,
					)
				}
		...
		for i, instr := range fn.Instrs {
			...
			case ir.IRStackSliceU8, ir.IRStackSliceU16, ir.IRStackSliceI32:
				...
				if !ok {
					return fmt.Errorf(
						("allocation lowering validation: %s instruction %d stack-lowers " +
							"%q without matching allocation plan"),
						fn.Name,
						i,
						instr.Name,
					)
				...
				if want == allocplan.StorageEliminated && instr.ArgSlots != 0 {
					return fmt.Errorf(
						"allocation lowering validation: %s eliminated allocation %q has %d stack backing slots",
						fn.Name,
						instr.Name,
						instr.ArgSlots,
					)
				...
				if want == allocplan.StorageStack && instr.ArgSlots <= 0 {
					return fmt.Errorf(
						"allocation lowering validation: %s stack allocation %q has no backing slots",
						fn.Name,
						instr.Name,
					)
				}
			...
			case ir.IRRegionMakeSliceU8, ir.IRRegionMakeSliceU16, ir.IRRegionMakeSliceI32:
				...
				if !ok {
					return fmt.Errorf(
						("allocation lowering validation: %s instruction %d emits %s for " +
							"%q without matching function-temp region allocation plan"),
						fn.Name,
						i,
						regionIRKindName(instr.Kind),
						instr.Name,
					)
				}
				if want != instr.Kind {
					return fmt.Errorf(
						("allocation lowering validation: %s allocation %q reports actual " +
							"FunctionTempRegion for %s but IR emitted %s"),
						fn.Name,
						instr.Name,
						regionIRKindName(want),
						regionIRKindName(instr.Kind),
					)
				}
				...
				if seenRegion[fn.Name][instr.Name] {
					return fmt.Errorf(
						"allocation lowering validation: %s emits duplicate function-temp region slice for %q",
						fn.Name,
						instr.Name,
					)
				}
			...
			case ir.IRIslandMakeSliceU8, ir.IRIslandMakeSliceU16, ir.IRIslandMakeSliceI32:
				if strings.TrimSpace(instr.Name) == "" {
					return fmt.Errorf(
						("allocation lowering validation: %s instruction %d emits %s " +
							"without allocation name; no matching IR island slice"),
						fn.Name,
						i,
						islandIRKindName(instr.Kind),
					)
				}
				...
				if !ok {
					return fmt.Errorf(
						("allocation lowering validation: %s instruction %d emits %s for " +
							"%q without matching explicit island allocation plan"),
						fn.Name,
						i,
						islandIRKindName(instr.Kind),
						instr.Name,
				...
				if want.kind != instr.Kind {
					return fmt.Errorf(
						("allocation lowering validation: %s allocation %q reports actual " +
							"ExplicitIsland for %s but IR emitted %s"),
						fn.Name,
						instr.Name,
						islandIRKindName(want.kind),
						islandIRKindName(instr.Kind),
					)
				}
				...
				if seenIsland[fn.Name][instr.Name] {
					return fmt.Errorf(
						"allocation lowering validation: %s emits duplicate explicit island slice for %q",
						fn.Name,
						instr.Name,
					)
				}
	...
	for fn, allocs := range expected {
		for id, storage := range allocs {
			if !seen[fn][id] {
				return fmt.Errorf(
					("allocation lowering validation: %s allocation %q reports actual " +
						"%s but no matching IR stack slice was emitted"),
					fn,
					id,
					storage,
				)
	...
	for fn, allocs := range expectedRegion {
		for id, kind := range allocs {
			if !seenRegion[fn][id] {
				return fmt.Errorf(
					("allocation lowering validation: %s allocation %q reports actual " +
						"FunctionTempRegion for %s but no matching IR function-temp region slice " +
						"was emitted"),
					fn,
					id,
					regionIRKindName(kind),
				)
			}
		}
	}
	for fn, allocs := range expectedIsland {
		for id, want := range allocs {
			if !seenIsland[fn][id] {
				return fmt.Errorf(
					("allocation lowering validation: %s allocation %q reports actual " +
						"ExplicitIsland for %s but no matching IR island slice was emitted"),
					fn,
					id,
					islandIRKindName(want.kind),
				)
...
func validateStackAllocationsDoNotEscape(
	prog *ir.IRProgram,
	stackAllocs map[string]map[string]bool,
...
func validateFunctionTempRegionResets(
	prog *ir.IRProgram,
	regionAllocs map[string]map[string]bool,
) error {
	for _, fn := range prog.Funcs {
...
func validateFunctionTempRegionResetsInFunc(fn ir.IRFunc, tracked map[string]bool) error {
	...
	for len(work) > 0 {
		...
		if cur.idx < 0 || cur.idx >= len(fn.Instrs) {
			if cur.active {
				return fmt.Errorf(
					("allocation lowering validation: %s function-temp region reset " +
						"does not dominate function exit"),
					fn.Name,
				)
			}
			continue
...
func stepFunctionTempRegionResetState(
	fn ir.IRFunc,
	cur functionTempRegionResetState,
	labels map[int]int,
	tracked map[string]bool,
) ([]functionTempRegionResetState, error) {
		...
		if tracked[instr.Name] {
			if !entered {
				return nil, fmt.Errorf(
					("allocation lowering validation: %s instruction %d function-temp " +
						"region enter does not dominate make for %q"),
					fn.Name,
					cur.idx,
					instr.Name,
				)
	...
	case ir.IRReturn:
		if active {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s instruction %d function-temp " +
					"region reset does not dominate return"),
				fn.Name,
				cur.idx,
			)
		}
	...
	case ir.IRJmp:
		...
		if !ok {
			return nil, fmt.Errorf(
				"allocation lowering validation: %s instruction %d jumps to unknown label %d",
				fn.Name,
				cur.idx,
				instr.Label,
			)
...
func validateExplicitIslandLifetimes(
	prog *ir.IRProgram,
	summaryProg *ir.IRProgram,
	expected map[string]map[string]islandExpectation,
) error {
...
func initialExplicitIslandLifetimeLocals(
	fn ir.IRFunc,
	tracked map[string]islandExpectation,
) ([]string, error) {
	...
	for alloc, expectation := range tracked {
		...
		if slot < 0 || slot >= fn.ParamSlots || slot >= len(locals) {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s allocation %q explicit " +
					"island handle parameter slot %d is outside params=%d locals=%d"),
				fn.Name,
				alloc,
				slot,
				fn.ParamSlots,
...
func stepExplicitIslandLifetimeState(
	fn ir.IRFunc,
	cur islandLifetimeState,
	labels map[int]int,
	expected map[string]islandExpectation,
	summaries map[string]islandReturnSummary,
) ([]islandLifetimeState, error) {
	instr := fn.Instrs[cur.idx]
	...
	if !ok {
		return nil, fmt.Errorf(
			"allocation lowering validation: %s instruction %d has unknown IR kind %d",
			fn.Name,
			cur.idx,
			instr.Kind,
		)
	...
	case ir.IRReturn:
		if name := firstEscapingTrackedExplicitIslandSlice(stack, expected); name != "" {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s instruction %d explicit " +
					"island allocation %q escapes via return"),
				fn.Name,
				cur.idx,
				name,
			)
		...
		if tag := firstFreedIslandUse(stack, freed); tag != "" {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s instruction %d explicit " +
					"island use after free via return of %s"),
				fn.Name,
				cur.idx,
				tag,
			)
	...
	case ir.IRIslandFree:
		...
		if freed[tag] {
			return nil, fmt.Errorf(
				"allocation lowering validation: %s instruction %d explicit island double free for %s",
				fn.Name,
				cur.idx,
				tag,
			)
	...
	case ir.IRIslandReset:
		...
		if freed[tag] {
			return nil, fmt.Errorf(
				"allocation lowering validation: %s instruction %d explicit island reset after free for %s",
				fn.Name,
				cur.idx,
				tag,
			)
		...
		if live := firstLiveIslandSliceForHandle(stack, locals, tag); live != "" {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s instruction %d explicit " +
					"island reset while live slice %s still references %s"),
				fn.Name,
				cur.idx,
				live,
				tag,
	...
	case ir.IRIslandMakeSliceU8, ir.IRIslandMakeSliceU16, ir.IRIslandMakeSliceI32:
		...
		if _, ok := expected[instr.Name]; ok {
			if tag := firstFreedIslandUse(popped, freed); tag != "" {
				return nil, fmt.Errorf(
					("allocation lowering validation: %s instruction %d explicit " +
						"island allocation %q use after free via operands of %s"),
					fn.Name,
					cur.idx,
					instr.Name,
					tag,
			...
			if tag == "" {
				return nil, fmt.Errorf(
					("allocation lowering validation: %s instruction %d explicit " +
						"island allocation %q has no active island handle operand"),
					fn.Name,
					cur.idx,
					instr.Name,
				)
			...
			if freed[tag] {
				return nil, fmt.Errorf(
					("allocation lowering validation: %s instruction %d explicit " +
						"island allocation %q use after free for %s"),
					fn.Name,
					cur.idx,
					instr.Name,
					tag,
	...
	case ir.IRSliceWindow, ir.IRSlicePrefix, ir.IRSliceSuffix:
		...
		if tag := firstFreedIslandUse(popped, freed); tag != "" {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s instruction %d explicit " +
					"island use after free via slice view of %s"),
				fn.Name,
				cur.idx,
				tag,
			)
	...
	case ir.IRCall:
		...
		if tag := firstFreedIslandUse(popped, freed); tag != "" {
			return nil, fmt.Errorf(
				"allocation lowering validation: %s instruction %d explicit island use after free via %s",
				fn.Name,
				cur.idx,
				tag,
			)
...
func stepIslandReturnSummaryState(
	fn ir.IRFunc,
	...
	case ir.IRIslandMakeSliceU8, ir.IRIslandMakeSliceU16, ir.IRIslandMakeSliceI32:
		popped, rest := popStackTags(stack, pop)
		if tag := explicitIslandMakeHandleOperandTag(popped); strings.HasPrefix(
			tag,
			"island-param:",
		) {
			stack = append(rest, explicitIslandSliceTag(instr.Name, tag), "")
...
func firstExplicitIslandValueTag(tags []string) string {
	for _, tag := range tags {
		if strings.HasPrefix(tag, "island:") || strings.HasPrefix(tag, "island-param:") ||
			strings.HasPrefix(tag, "island-slice:") {
			return tag
		}
	}
	return ""
...
func firstEscapingTrackedExplicitIslandSlice(
	tags []string,
	expected map[string]islandExpectation,
) string {
	for _, tag := range tags {
		...
		if expectation.handleParamSlotKnown &&
			islandParamSlot(handleTag) == expectation.handleParamSlot {
			continue
		}
		return name
	}
...
func stepStackCallSummaryState(
	fn ir.IRFunc,
	cur stackEscapeState,
	labels map[int]int,
) ([]stackEscapeState, []string, bool) {
...
func validateFunctionStackAllocationsDoNotEscape(
	fn ir.IRFunc,
	tracked map[string]bool,
	callSummaries map[string]stackCallSummary,
) error {
...
func stepStackEscapeState(
	fn ir.IRFunc,
	cur stackEscapeState,
	labels map[int]int,
	tracked map[string]bool,
	callSummaries map[string]stackCallSummary,
) ([]stackEscapeState, error) {
	instr := fn.Instrs[cur.idx]
	stack := append([]string(nil), cur.stack...)
	locals := append([]string(nil), cur.locals...)
	pop, push, ok := validationStackEffect(instr)
	...
	case ir.IRReturn:
		if tag := firstStackTag(stack); tag != "" {
			return nil, fmt.Errorf(
				"allocation lowering validation: %s instruction %d stack allocation %q escapes via return",
				fn.Name,
				cur.idx,
				tag,
			)
	...
	case ir.IRCall:
		...
		if tag := firstStackTag(popped); tag != "" {
			...
			if !ok {
				return nil, fmt.Errorf(
					"allocation lowering validation: %s instruction %d stack allocation %q escapes via call %q",
					fn.Name,
					cur.idx,
					tag,
					instr.Name,
	...
	case ir.IRStoreGlobal:
		...
		if tag := firstStackTag(popped); tag != "" {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s instruction %d stack " +
					"allocation %q escapes via global store"),
				fn.Name,
				cur.idx,
				tag,
			)
	...
	case ir.IRIndexStoreI32, ir.IRIndexStoreU8, ir.IRIndexStoreU16:
		...
		if len(popped) > 0 && popped[0] != "" {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s instruction %d stack " +
					"allocation %q escapes via indexed store value"),
				fn.Name,
				cur.idx,
				popped[0],
			)
	...
	case ir.IRMemWriteI32, ir.IRMemWriteU8, ir.IRMemWritePtr, ir.IRMemWriteArchPtr,
		...
		if len(popped) > 0 && popped[0] != "" {
			return nil, fmt.Errorf(
				("allocation lowering validation: %s instruction %d stack " +
					"allocation %q escapes via memory store value"),
				fn.Name,
				cur.idx,
				popped[0],
			)
