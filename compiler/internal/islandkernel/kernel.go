...
func CanBorrow(req BorrowRequest) Result {
	...
	if req.Ref.IslandID == "" || req.Token.IslandID == "" ||
		req.Ref.IslandID != req.Token.IslandID {
		return reject("borrow.island_mismatch", "borrow requires matching island identity")
	}
	if req.Ref.Epoch != req.Token.Epoch {
		return reject(
			"borrow.stale_epoch",
			"borrow epoch must match the current island token epoch",
		)
	}
...
func CanReturn(req EscapeRequest) Result {
	...
	return accept(
		"escape.return_owned",
		"owned value may return without borrowing island-local memory",
	)
}
...
func CanCaptureClosure(req EscapeRequest) Result {
	if isBorrowed(req.Ref) {
		return reject(
			"escape.closure_borrow",
			"borrowed island reference cannot be captured by an escaping closure",
		)
	}
...
func CanMoveIsland(req TokenRequest) Result {
	...
	if req.Token.IslandID == "" || req.Token.OwnerID == "" {
		return conservative(
			"token.move_missing_identity",
			"moving an island requires token identity and owner evidence",
		)
	}
...
func CanResetIsland(req TokenRequest) Result {
	...
	res := accept(
		"token.reset_epoch_advanced",
		"reset advances the island epoch and invalidates old references",
	)
	res.ConsumesToken = true
...
func CanClaimNoAlias(req NoAliasRequest) Result {
	if unsafeOrExternal(req.Left) || unsafeOrExternal(req.Right) {
		return reject(
			"noalias.unsafe_external",
			"unsafe or external memory cannot authorize noalias",
		)
	}
	if req.Left.IslandID != "" && req.Right.IslandID != "" &&
		req.Left.IslandID != req.Right.IslandID &&
		proofMatches(req.Proof, req.Left, ProofNoAlias, OperationNoAlias) {
		return accept(
			"noalias.distinct_proven_islands",
			"distinct live islands with verified proof may claim narrow noalias",
		)
	}
	return conservative(
		"noalias.proof_required",
		"noalias requires distinct island identity and a verified proof",
	)
}
...
func CanEliminateBoundsCheck(req ProofRequest) Result {
	if proofMatches(req.Proof, req.Ref, ProofBounds, req.Operation) {
		return accept(
			"bounds.proof_verified",
			"bounds check elimination has a verified proof for this ref and operation",
		)
	}
...
func CanLowerAsExplicitIsland(req StorageRequest) Result {
	if unsafeOrExternal(req.Ref) {
		return reject(
			"storage.unsafe_external",
			"unsafe or external memory cannot authorize trusted storage",
		)
	}
	if req.EscapesLifetime {
		return reject(
			"storage.explicit_island_escape",
			"explicit island storage cannot be trusted for escaping values",
		)
	}
	if req.PlannedStorage != StorageExplicitIsland || req.ActualStorage != StorageExplicitIsland {
		return reject(
			"storage.explicit_island_mismatch",
			"explicit island storage requires planned and actual island lowering",
		)
	}
	if proofMatches(req.Proof, storageProofRef(req), ProofStorage, OperationExplicitIslandStorage) {
		return accept(
			"storage.explicit_island_trusted",
			"explicit island storage matches plan, lowering, and proof",
		)
	}
	return conservative(
		"storage.explicit_island_proof_required",
		"explicit island storage requires a verified storage proof",
	)
}

func CanPromoteUnsafeRoot(req UnsafeRequest) Result {
	if req.Ref.Provenance == ProvenanceUnsafeUnknown || req.Ref.UnsafeClass == UnsafeUnknown ||
		req.Ref.IslandID == ExternalUnsafeIsland {
		return reject(
			"unsafe.unknown_promotion",
			"unsafe_unknown and external roots cannot promote to safe memory",
		)
	}
	if req.Ref.UnsafeClass == UnsafeVerifiedRoot && req.Ref.Bounds.Known &&
		req.Ref.Bounds.InBounds {
		return accept(
			"unsafe.verified_root_bounded",
			"bounded unsafe verified root may remain checked",
		)
	}
	return conservative(
		"unsafe.runtime_contract_required",
		"unsafe roots require a bounded runtime contract",
	)
}
...
func CanTrustStorage(req StorageRequest) Result {
	...
	if req.ActualStorage == StorageHeap && req.PlannedStorage != StorageHeap {
		return reject(
			"storage.heap_fallback_not_trusted",
			"heap fallback cannot satisfy a trusted storage claim",
		)
	}
	if req.EscapesLifetime {
		return reject(
			"storage.escape_not_trusted",
			"escaping values cannot satisfy trusted storage",
		)
	}
	if req.PlannedStorage == req.ActualStorage &&
		proofMatches(req.Proof, storageProofRef(req), ProofStorage, req.Proof.Operation) {
		return accept(
			"storage.trusted_with_proof",
			"storage claim matches plan, actual lowering, and proof",
		)
	}
	return conservative(
		"storage.proof_required",
		"trusted storage requires matching lowering and proof",
	)
}
...
func CanEraseRuntimeCheck(req ProofRequest) Result {
	if proofMatches(req.Proof, req.Ref, req.Proof.Kind, req.Operation) {
		return accept(
			"runtime_check.erase_verified",
			"runtime check may be erased only with verified proof",
		)
	}
...
func boundaryDecision(req BoundaryRequest, boundary string) Result {
	if req.Transfer == TransferBorrowedView || isBorrowed(req.Ref) {
		return reject(
			"boundary."+boundary+"_borrow",
			"borrowed island views cannot cross "+boundary+" boundaries",
		)
	}
	switch req.Transfer {
	case TransferOwned, TransferMoved, TransferSerialized:
		return accept(
			"boundary."+boundary+"_owned",
			"owned, moved, or serialized values may cross "+boundary+" boundaries",
		)
	default:
		return conservative(
			"boundary."+boundary+"_unknown_transfer",
			"unknown transfer kind stays conservative",
		)
	}
}
