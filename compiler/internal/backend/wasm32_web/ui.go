...
func UIModule(uiJSONFile string) []byte {
	...
	src := strings.Join([]string{
		...
		"}",
		"",
		"function updateStateFromBindingInput(state, view, binding, value) {",
		"  const source = String(binding.source || \"\");",
		"  if (!source.startsWith(\"state.\")) {",
		"    return;",
		"  }",
		"  const viewState = stateForView(state, view);",
		"  const field = statePath(source);",
		"  if (binding.type === \"i32\" || binding.type === \"u8\" || binding.type === \"u16\") {",
		"    viewState[field] = Number.parseInt(value, 10) || 0;",
		"  } else if (binding.type === \"bool\") {",
		"    viewState[field] = value === \"true\";",
		"  } else {",
		"    viewState[field] = String(value);",
		"  }",
		"}",
		"",
		"function renderInputControl(host, state, view, binding) {",
		"  if (!(binding.type === \"str\" || binding.type === \"String\")) {",
		"    return;",
		"  }",
		"  const input = document.createElement(\"input\");",
		"  input.type = \"text\";",
		"  input.setAttribute(\"data-tetra-kind\", \"input\");",
		"  input.setAttribute(\"data-tetra-event\", \"input\");",
		"  input.setAttribute(\"data-tetra-input-binding\", binding.name);",
		"  input.value = bindingValue(state, view, binding);",
		"  input.addEventListener(\"input\", () => {",
		"    updateStateFromBindingInput(state, view, binding, input.value);",
		...
		"}",
		"",
		"function dispatchTetraEvent(host, state, view, event) {",
		...
		"  select.setAttribute(\"data-tetra-event\", \"select\");",
		...
		"  const dispatch = () => dispatchTetraEvent(host, state, view, event);",
		...
		"        renderSelectControl(host, state, view, event);",
		"        continue;",
		"      }",
		"      const button = document.createElement(\"button\");",
		"      button.type = \"button\";",
		"      button.setAttribute(\"data-tetra-event\", event.name);",
		"      button.textContent = \"event \" + event.name + \" -> \" + event.command;",
		"      button.addEventListener(\"click\", () => {",
		"        dispatchTetraEvent(host, state, view, event);",
		"      });",
