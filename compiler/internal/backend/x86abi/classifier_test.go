...
func TestI386SysVClassifierUsesStackArguments(t *testing.T) {
	...
	if classifier.Name() != "i386-sysv" || classifier.StackCleanup() != StackCleanupCaller {
		t.Fatalf(
			"classifier identity = %s cleanup=%s, want i386-sysv caller cleanup",
			classifier.Name(),
	...
	if got := plan.Return; got.Register != "eax" || got.SizeBytes != 4 ||
		got.Class != ABIClassInteger ||
		got.Extension != ABIExtendNone {
...
func TestI386SysVClassifierScalarReturns(t *testing.T) {
	...
	if got := plan.Return; got.Register != "edx:eax" ||
	...
	if got := plan.Return; got.Register != "st0" || got.Class != ABIClassX87 ||
...
func TestI386SysVClassifierStructReturnUsesHiddenSRet(t *testing.T) {
	...
	if got := plan.Params[0]; got.Class != ABIClassMemory || got.StackOffsetBytes != 4 ||
		got.StackSlotBytes != 8 ||
		got.SizeBytes != 8 ||
		got.AlignBytes != 4 {
		t.Fatalf(
			"struct param = %#v, want by-value stack memory copy after hidden sret pointer",
			got,
		)
	}
...
func TestI386SysVClassifierRejectsNonX86Targets(t *testing.T) {
	for _, raw := range []string{"x64", "x32"} {
		if _, err := NewClassifier(mustTarget(t, raw)); err == nil ||
			!strings.Contains(err.Error(), "x86abi classifier requires x86 i386-sysv") {
...
func TestI386SysVVarargsRemainCallerCleanedStackArguments(t *testing.T) {
	...
	if !plan.Variadic || plan.FixedParamCount != 1 || plan.VarargStartIndex != 1 ||
		plan.StackCleanup != StackCleanupCaller {
