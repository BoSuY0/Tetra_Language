...
func emitCall(
	...
	if instr.ArgSlots < 0 || instr.RetSlots < 0 {
		return fmt.Errorf(
			"x86 backend: call %q has negative ABI slots args=%d rets=%d in function '%s'",
			instr.Name,
			instr.ArgSlots,
			instr.RetSlots,
			fn.Name,
	...
	if instr.RetSlots > 4 {
		return fmt.Errorf(
			"x86 backend: unsupported call return slots %d in function '%s' call %q",
			instr.RetSlots,
			fn.Name,
			instr.Name,
		)
...
func emitReturn(e *emitter, fn ir.IRFunc, pop func(int) error) error {
	...
	default:
		return fmt.Errorf(
			"x86 backend: unsupported return slots %d in function '%s'",
			fn.ReturnSlots,
			fn.Name,
		)
	}
