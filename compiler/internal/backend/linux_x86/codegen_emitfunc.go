...
func buildObject(
	...
	for _, patch := range callPatches {
		...
		relocs = append(
			relocs,
			tobj.Reloc{Kind: tobj.RelocCallRel32, At: uint32(patch.at), Name: patch.name},
		)
	}
...
func emitFunc(
	...
	localSlotOffset := func(slot int) (int32, error) {
		if slot < 0 || slot >= fn.LocalSlots {
			return 0, fmt.Errorf(
				"x86 backend: local slot %d out of bounds in function '%s' (locals=%d)",
				slot,
				fn.Name,
				fn.LocalSlots,
			)
	...
	for _, instr := range fn.Instrs {
		...
		case ir.IRStrLit:
			if dataBlobs == nil {
				return fmt.Errorf(
					"x86 backend: missing data blobs for string literal in function '%s'",
					fn.Name,
				)
			}
			if len(instr.Str) == 0 {
		...
		case ir.IRLoadGlobal:
			if instr.Local < 0 {
				return fmt.Errorf(
					"x86 backend: global slot %d out of bounds in function '%s'",
					instr.Local,
					fn.Name,
				)
			}
		...
		case ir.IRLabel:
			if instr.Label < 0 {
				return fmt.Errorf(
					"x86 backend: negative label %d in function '%s'",
					instr.Label,
					fn.Name,
				)
			}
			if _, exists := labelOffsets[instr.Label]; exists {
				return fmt.Errorf(
					"x86 backend: duplicate label %d in function '%s'",
					instr.Label,
					fn.Name,
				)
			}
		...
		case ir.IRSymAddr:
			if instr.Name == "" {
				return fmt.Errorf(
					"x86 backend: symbol address is missing name in function '%s'",
					fn.Name,
				)
			}
			at := e.movEaxImm32Patch()
		...
		default:
			return fmt.Errorf(
				"x86 backend: unsupported IR instruction %v in function '%s'",
				instr.Kind,
				fn.Name,
			)
		}
	...
	for _, patch := range patches {
		...
		if !ok {
			return fmt.Errorf(
				"x86 backend: missing label %d in function '%s'",
				patch.label,
				fn.Name,
			)
		}
