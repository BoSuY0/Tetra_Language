...
func hintForDiagnosticMessage(msg string) string {
	...
	case strings.Contains(msg, "planned feature"):
		return ("Use the supported v1.0 syntax surface, or keep this source " +
			"behind a later-release feature gate.")
	case strings.Contains(msg, "capsule requires at least one metadata entry"):
		return ("Add at least one metadata entry inside the capsule block, for " +
			"example: id: \"tetra://app\".")
...
func bridgeFlowSyntax(src []byte, filename string, opts flowBridgeOptions) ([]byte, error) {
	...
	for lineNo, line := range lines {
		...
		if err != nil {
			return nil, diagnosticErrorf(
				Position{File: filename, Line: lineNo + 1, Col: col},
				err.Error(),
			)
		}
		...
		if pendingBlockIndent >= 0 {
			if !(isCaseBlockKind(
				pendingBlockKind,
			) && indent == pendingBlockIndent && strings.HasPrefix(
				content,
				"case ",
			)) &&
				indent <= pendingBlockIndent {
				return nil, diagnosticErrorf(
					Position{File: filename, Line: lineNo + 1, Col: 1},
		...
		for len(blocks) > 0 {
			...
			if strings.HasPrefix(content, "case ") && isCaseBlockKind(top.kind) &&
				indent == top.indent {
				break
			}
	...
	if pendingBlockIndent >= 0 {
		return nil, diagnosticErrorf(
			Position{File: filename, Line: len(lines), Col: 1},
			"expected indented block after ':'",
		)
	}
	for len(blocks) > 0 {
	...
	for {
		...
		if ch == '\\' {
			...
			case '"':
				out = append(out, '"')
			case 'x':
				if l.idx+1 >= len(l.src) || !isHexDigit(l.src[l.idx]) ||
					!isHexDigit(l.src[l.idx+1]) {
	...
	for p.cur.typ != TokenEOF {
		...
		case TokenSemicolon:
			if public {
				return nil, diagnosticErrorf(
					p.cur.pos,
					"pub must be followed by a declaration or import",
				)
			}
			if err := p.next(); err != nil {
				return nil, err
		...
		case TokenModule:
			...
			if seenFunc || seenStruct || seenEnum || seenState || seenView || seenActor ||
				seenGlobal ||
				seenCapsule {
		...
		case TokenImport:
			...
			if len(items) > 0 {
				if p.cur.typ == TokenAs {
					return nil, diagnosticErrorf(
						p.cur.pos,
						"selective imports cannot use module alias",
					)
				}
			...
			file.Imports = append(
				file.Imports,
				ImportDecl{At: pos, Path: path, Alias: alias, Items: items, Public: public},
			)
			if err := p.consumeOptionalSemicolon(); err != nil {
				return nil, err
			}
		...
		case TokenStruct:
			if seenFunc || seenView || seenActor || seenGlobal {
				return nil, diagnosticErrorf(
					p.cur.pos,
					"struct must appear before globals/functions",
				)
			}
			seenStruct = true
		...
		case TokenIdent:
			...
			case "state":
				if seenFunc || seenView || seenActor || seenGlobal {
					return nil, diagnosticErrorf(
						p.cur.pos,
						"state must appear before views/globals/functions",
					)
				}
				seenState = true
			...
			case "view":
				if seenFunc || seenActor || seenGlobal {
					return nil, diagnosticErrorf(
						p.cur.pos,
						"view must appear before globals/functions",
					)
				}
				seenView = true
			...
			case "impl":
				...
				if seenFunc || seenView || seenActor || seenGlobal {
					return nil, diagnosticErrorf(
						p.cur.pos,
						"impl must appear before globals/functions",
					)
				}
			...
			case "protocol":
				if seenFunc || seenView || seenActor || seenGlobal {
					return nil, diagnosticErrorf(
						p.cur.pos,
						"protocol must appear before globals/functions",
					)
				}
			...
			case "extension":
				if seenFunc || seenView || seenActor || seenGlobal {
					return nil, diagnosticErrorf(
						p.cur.pos,
						"extension must appear before globals/functions",
					)
				}
			...
			case "actor":
				if seenFunc || seenView || seenGlobal {
					return nil, diagnosticErrorf(
						p.cur.pos,
						"actor must appear before globals/functions",
					)
				}
				seenActor = true
			...
			case "capsule":
				if seenFunc || seenGlobal {
					return nil, diagnosticErrorf(
						p.cur.pos,
						"capsule must appear before globals/functions",
					)
				}
				seenCapsule = true
			...
			default:
				...
				return nil, p.unexpected(
					"module/import/capsule/enum/struct/state/view/extension/var/val/const/fn/closure",
				)
			}
		...
		default:
			...
			return nil, p.unexpected(
				"module/import/capsule/enum/struct/state/view/extension/var/val/fn/closure",
			)
		}
	}
...
func (p *parser) parseFuncSignatureDecl() (FuncSigDecl, error) {
	...
	if len(clauses) > 0 {
		return FuncSigDecl{}, diagnosticErrorf(
			clauses[0].At,
			"semantic clauses are not allowed in protocol requirements",
		)
	}
	if err := p.consumeOptionalSemicolon(); err != nil {
		return FuncSigDecl{}, err
	...
	for p.cur.typ != TokenRBrace && p.cur.typ != TokenEOF {
		...
		for _, param := range fn.Params {
			if param.Name == "self" {
				return nil, diagnosticErrorf(
					param.At,
					("actor methods do not support explicit self parameters yet; use " +
						"core.self() inside the method body"),
				)
			}
		}
...
func (p *parser) unsupportedActorMemberDiagnostic() error {
	if p.cur.typ == TokenIdent {
		...
		case "state":
			return diagnosticErrorf(
				p.cur.pos,
				"actor declarations do not support nested state blocks yet; define state outside the actor",
			)
		case "self":
			return diagnosticErrorf(
				p.cur.pos,
				"actor declarations do not support self members yet; use core.self() inside a func method",
			)
		default:
	...
	return diagnosticErrorf(
		p.cur.pos,
		"actor declarations currently support state fields and func methods only",
	)
}
	...
	for p.cur.typ != TokenRBrace && p.cur.typ != TokenEOF {
		...
		case "bind":
			...
			view.Bindings = append(
				view.Bindings,
				ViewBindingDecl{At: entryPos, Name: nameTok.lit, Type: typ, Value: value},
			)
		case "event":
			entryPos := p.cur.pos
			if err := p.next(); err != nil {
			...
			view.Events = append(
				view.Events,
				ViewEventDecl{At: entryPos, Name: eventTok.lit, Command: cmdTok.lit},
			)
		case "command":
			entryPos := p.cur.pos
			if err := p.next(); err != nil {
			...
			view.Commands = append(
				view.Commands,
				ViewCommandDecl{At: entryPos, Name: cmdTok.lit, Body: body},
			)
		case "style":
			entryPos := p.cur.pos
			if err := p.next(); err != nil {
			...
			view.Styles = append(
				view.Styles,
				ViewStyleDecl{At: entryPos, Name: nameTok.lit, Type: typ, Value: value},
			)
		case "accessibility", "a11y":
			...
			view.Accessibility = append(
				view.Accessibility,
				ViewAccessibilityDecl{At: entryPos, Name: nameTok.lit, Type: typ, Value: value},
			)
		default:
			return nil, p.unexpected("bind/event/command/style/accessibility")
		}
...
func (p *parser) parseRelational() (Expr, error) {
	...
	if p.cur.typ == TokenLess || p.cur.typ == TokenGreater || p.cur.typ == TokenGreaterEq ||
		p.cur.typ == TokenLessEq {
		opTok := p.cur
		if err := p.next(); err != nil {
			return nil, err
		}
		...
		if p.cur.typ == TokenLess || p.cur.typ == TokenGreater || p.cur.typ == TokenGreaterEq ||
			...
			return nil, diagnosticErrorf(
				p.cur.pos,
				"chaining relational operators is not supported",
			)
		}
		left = &BinaryExpr{At: opTok.pos, Op: opTok.typ, Left: left, Right: right}
	}
...
func (p *parser) parsePrimary() (Expr, error) {
	...
	case TokenIdent:
		if p.cur.lit == "closure" && p.peek.typ == TokenLParen {
			return nil, diagnosticErrorf(
				p.cur.pos,
				("closure literal expressions use 'fn(...) -> Type'; named " +
					"callables use top-level 'closure Name(...)' declarations"),
			)
		}
		pos := p.cur.pos
		parts, err := p.parsePathParts()
		...
		if p.cur.typ == TokenLParen {
			...
			return p.parsePostfix(
				&CallExpr{At: pos, Name: name, TypeArgs: typeArgs, Args: args, ArgLabels: labels},
			)
		}
		if !hasTypeArgs {
		...
		if hasTypeArgs {
			return nil, diagnosticErrorf(
				pos,
				"generic type arguments require a call or struct literal",
			)
		}
...
func (p *parser) parseClosureExpr() (Expr, error) {
	...
	if p.cur.typ == TokenIdent {
		return nil, diagnosticErrorf(
			p.cur.pos,
			"closure literals cannot have names; use top-level closure declarations for named callables",
		)
	}
	typeParams, typeParamBounds, err := p.parseTypeParamsWithBounds(true)
	if err != nil {
...
func cloneCompoundTarget(expr Expr) Expr {
	...
	case *FieldAccessExpr:
		return &FieldAccessExpr{At: e.At, Base: cloneCompoundTarget(
			e.Base,
		), Field: e.Field, EnumType: e.EnumType, EnumOrdinal: e.EnumOrdinal}
	case *IndexExpr:
		return &IndexExpr{At: e.At, Base: cloneCompoundTarget(
			e.Base,
		), Index: cloneCompoundTarget(
			e.Index,
		)}
	case *NumberExpr:
	...
	case *BinaryExpr:
		return &BinaryExpr{At: e.At, Op: e.Op, Left: cloneCompoundTarget(
			e.Left,
		), Right: cloneCompoundTarget(
			e.Right,
		)}
	case *CallExpr:
...
func plannedFeatureError(pos Position, feature string) error {
	return diagnosticErrorf(
		pos,
		"planned feature '%s' is not implemented in the Tetra v1.0 profile",
		feature,
	)
}
...
func parseFileDiagnosticsWithTopLevelRecovery(
	src []byte,
	filename string,
) (*FileAST, []Diagnostic) {
	lines := strings.Split(string(src), "\n")
	...
	for attempt := 0; attempt <= len(lines); attempt++ {
		...
		start, end, ok := topLevelDeclarationSpanForDiagnostic(
			lines,
...
func previousDeclarationLineForSyntheticClose(
	lines []string,
	idx int,
	diagnostic Diagnostic,
) (int, bool) {
	...
	currentIsRecoveryBoundary := isBlankOrCommentLine(current) ||
		(isTopLevelSourceLine(current) && startsRecoverableTopLevelDeclaration(current))
...
func startsRecoverableTopLevelDeclaration(line string) bool {
	...
	return startsFunctionLikeTopLevelDeclaration(trimmed) ||
		startsNonFunctionTopLevelDeclaration(trimmed)
}
...
func (p *parser) parseMatchStmt() (Stmt, error) {
	...
	for p.cur.typ != TokenRBrace && p.cur.typ != TokenEOF {
		...
		cases = append(
			cases,
			MatchCase{At: casePos, Pattern: pattern, Guard: guard, Default: isDefault, Body: body},
		)
	}
	if _, err := p.expect(TokenRBrace); err != nil {
		return nil, err
...
func (p *parser) parseMatchPattern() (Expr, error) {
	...
	case TokenIdent:
		...
		if p.cur.typ == TokenLParen {
			...
			return nil, diagnosticErrorf(
				pos,
				"payload match patterns require qualified enum case syntax",
			)
		}
		return buildFieldAccess(parts, pos), nil
	case TokenNone:
...
func (p *parser) parseEnumPayloadPatternBindings() ([]string, error) {
	...
	if p.cur.typ == TokenRParen {
		return nil, diagnosticErrorf(
			payloadPos,
			"enum payload pattern requires at least one binding",
		)
	}
	var bindings []string
	...
	for {
		...
		if _, exists := seenBindings[nameTok.lit]; exists {
			return nil, diagnosticErrorf(
				nameTok.pos,
				"duplicate enum payload binding '%s'",
				nameTok.lit,
			)
		}
		...
		if p.cur.typ == TokenRParen {
			return nil, diagnosticErrorf(
				commaPos,
				"enum payload pattern does not allow a trailing comma",
			)
		}
	}
	if _, err := p.expect(TokenRParen); err != nil {
...
func (p *parser) parseFunctionModifiers() ([]string, []SemanticClause, error) {
	...
	for {
		...
		if first, exists := seenClauses[clause.Name]; exists {
			return nil, nil, diagnosticErrorf(
				clause.At,
...
func (p *parser) parseSemanticClause() (SemanticClause, bool, error) {
	...
	case "noalloc", "noblock", "realtime", "nothrow", "privacy":
		...
		if p.cur.typ == TokenLParen {
			return SemanticClause{}, false, diagnosticErrorf(
				clause.At,
				"semantic clause '%s' does not take arguments",
				clause.Name,
			)
		}
	...
	for {
		...
		if p.cur.typ == TokenColon {
			if !allowBounds {
				return nil, nil, diagnosticErrorf(
					p.cur.pos,
					"generic type parameter bounds are only supported on functions",
				)
			}
			if err := p.next(); err != nil {
				return nil, nil, err
	...
	for {
		...
		if p.cur.typ == TokenIdent && isOwnershipMarker(p.cur.lit) {
			...
			if p.cur.typ == TokenIdent && isOwnershipMarker(p.cur.lit) {
				return nil, diagnosticErrorf(
					p.cur.pos,
					("ownership marker '%s' cannot follow ownership marker '%s'; use " +
						"exactly one of borrow, inout, or consume before the parameter type"),
					p.cur.lit,
					ownershipTok.lit,
				)
			}
			if !p.startsTypeRef() {
				return nil, diagnosticErrorf(
					ownershipTok.pos,
					"ownership marker '%s' must be followed by a parameter type",
					ownershipTok.lit,
				)
			}
		}
		...
		params = append(
			params,
			ParamDecl{At: nameTok.pos, Name: nameTok.lit, Type: typeRef, Ownership: ownership},
		)
		if p.cur.typ != TokenComma {
			break
		}
...
func (p *parser) parseReprStructDecl(public bool) (*StructDecl, error) {
	...
	if nameTok.lit != StructReprC {
		return nil, diagnosticErrorf(
			nameTok.pos,
			"unsupported struct representation '%s'",
			nameTok.lit,
		)
	}
...
func (p *parser) parseEnumDecl(public bool) (*EnumDecl, error) {
	...
	for p.cur.typ != TokenRBrace && p.cur.typ != TokenEOF {
		...
		cases = append(
			cases,
			EnumCaseDecl{At: casePos, Name: nameTok.lit, Payload: payload, HasPayload: hasPayload},
		)
...
func (p *parser) parseEnumCasePayload() ([]TypeRef, bool, error) {
	...
	if p.cur.typ == TokenRParen {
		return nil, false, diagnosticErrorf(
			payloadPos,
			"enum payload list must contain at least one type",
		)
	}
	var payload []TypeRef
	for {
		...
		if p.cur.typ == TokenRParen {
			return nil, false, diagnosticErrorf(
				commaPos,
				"enum payload declaration does not allow a trailing comma",
			)
		}
	}
	if _, err := p.expect(TokenRParen); err != nil {
...
func (p *parser) parseReturnTypeRef() (TypeRef, string, error) {
	...
	if p.cur.typ == TokenIdent && p.cur.lit == "borrow" {
		...
		if p.cur.typ == TokenIdent && isOwnershipMarker(p.cur.lit) {
			return TypeRef{}, "", diagnosticErrorf(
				p.cur.pos,
				("ownership marker '%s' cannot follow return ownership marker " +
					"'%s'; use exactly one return ownership marker before the return type"),
				p.cur.lit,
				ownershipTok.lit,
			)
		}
		if !p.startsTypeRef() {
			return TypeRef{}, "", diagnosticErrorf(
				ownershipTok.pos,
				"expected return type after `borrow`",
			)
		}
	}
...
func (p *parser) parseFunctionTypeRef() (TypeRef, error) {
	...
	for p.cur.typ != TokenRParen {
		...
		if p.cur.typ == TokenIdent && isOwnershipMarker(p.cur.lit) {
			...
			if p.cur.typ == TokenIdent && isOwnershipMarker(p.cur.lit) {
				return TypeRef{}, diagnosticErrorf(
					p.cur.pos,
					("ownership marker '%s' cannot follow ownership marker '%s'; use " +
						"exactly one of borrow, inout, or consume before the function type " +
						"parameter"),
					p.cur.lit,
					ownershipTok.lit,
				)
			...
			if !p.startsTypeRef() {
				return TypeRef{}, diagnosticErrorf(
					ownershipTok.pos,
					"ownership marker '%s' must be followed by a function type parameter",
					ownershipTok.lit,
				)
			}
		}
		...
		if p.cur.typ == TokenRParen {
			return TypeRef{}, diagnosticErrorf(
				commaPos,
				"function type parameter list does not allow a trailing comma",
			)
		}
	}
	if _, err := p.expect(TokenRParen); err != nil {
	...
	if len(clauses) > 0 {
		return TypeRef{}, diagnosticErrorf(
			clauses[0].At,
			"semantic clauses are not allowed in function types",
		)
	}
	return TypeRef{
		At:              at,
		Kind:            TypeRefFunction,
		Params:          params,
