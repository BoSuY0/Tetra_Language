...
func TestExecutableBackendForTarget(t *testing.T) {
	...
	if backend.Name != "linux-x64" || backend.OS != ctarget.OSLinux || backend.Format != ctarget.FormatELF {
		t.Fatalf("linux-x64 backend = (%s, %s, %s), want linux-x64/linux/elf", backend.Name, backend.OS, backend.Format)
	}
...
func TestCodegenOptionsForTarget(t *testing.T) {
	...
	if !got.IslandsDebug || !got.DebugInfo || !got.ReleaseOptimize || !got.EmitRuntimeHeapTelemetry {
		t.Fatalf("CodegenOptionsForTarget did not preserve build flags: %#v", got)
	}
	if got.PointerWidthBits != tgt.PointerWidthBits || got.NativeIntWidthBits != tgt.NativeIntWidthBits || got.RegisterWidthBits != tgt.RegisterWidthBits {
	...
	if got.RuntimeHeapTelemetryDir != "telemetry" || got.RuntimeHeapTelemetryProgram != "prog" || got.RuntimeHeapTelemetryMain != "main" {
...
func TestValidateRuntimeHeapTelemetryBuildOptions(t *testing.T) {
	linuxX64 := mustTarget(t, "linux-x64")
	err := ValidateRuntimeHeapTelemetryBuildOptions(linuxX64, buildapi.BuildOptions{RuntimeHeapTelemetryDir: "telemetry"})
...
func TestLinkExecutableDispatchesBackend(t *testing.T) {
	...
	if gotPath != "out.bin" || gotMain != "main" || len(gotObjects) != 1 || gotObjects[0] != objects[0] {
		t.Fatalf("LinkExecutable dispatch = path=%q main=%q objects=%v", gotPath, gotMain, gotObjects)
	}
