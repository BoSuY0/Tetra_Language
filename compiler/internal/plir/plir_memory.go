...
func (b *builder) recordAllocBytesCall(name string, call *frontend.CallExpr, targetName string) {
	...
	b.addOperation(Operation{
		...
		Outputs:     []string{id},
		UnsafeClass: UnsafeVerifiedRoot,
		Note: ("alloc_bytes raw allocation-base metadata: zero invalid, " +
			"negative and overflow reject before allocation"),
	})
...
func (b *builder) recordRawPtrAddCall(name string, call *frontend.CallExpr, targetName string) {
	...
	if baseRoot := b.rawPointerBaseRoot(base); baseRoot != "" {
		...
		if !offsetKnown {
			...
			validDerived = false
			note = fmt.Sprintf(
				"%s raw_pointer_bounds: %s base:%s offset:%s",
				name,
				status,
				baseRoot,
				offset,
		...
		if offsetKnown && offsetBytes < 0 {
			...
			validDerived = false
			note = fmt.Sprintf(
				"%s raw_pointer_bounds: %s base:%s offset:%d width:%d",
				name,
				status,
				baseRoot,
				offsetBytes,
		...
		if offsetSumOK && offsetKnown && offsetBytes >= 0 {
			if baseBytes, bytesKnown := b.rawPointerBaseByteSize(baseRoot); bytesKnown &&
				offsetKnown {
...
func (b *builder) recordRawMemoryAccessCall(
	name string,
		...
		if root := b.rawPointerBaseRoot(ptr); root != "" {
			if baseBytes, bytesKnown := b.rawPointerBaseByteSize(root); bytesKnown {
				if offsetBytes, offsetKnown := b.rawPointerOffsetBytes(ptr); offsetKnown {
					...
					if err == nil {
						derived, _ := runtimeabi.DeriveRawPointerBounds(
							rootBounds,
							offsetBytes,
							rawMemoryAccessWidthBytes(name),
						)
						status = derived.Status
					}
				}
	...
	note := fmt.Sprintf(
		"%s raw memory gateway: %s pointer:%s width:%d",
		name,
		status,
		ptr,
		rawMemoryAccessWidthBytes(name),
	...
	if offsetBytes, offsetKnown := b.rawPointerOffsetBytes(ptr); offsetKnown {
		note = fmt.Sprintf(
			"%s raw memory gateway: %s pointer:%s offset:%d width:%d",
			name,
			status,
			ptr,
			offsetBytes,
...
func (b *builder) recordMakeSliceCall(
	name string,
	elem string,
	call *frontend.CallExpr,
	targetName string,
	...
	b.addValue(value)
	note := "make<" + elem + ("> length contract: zero valid, negative and overflow reject " +
		"before allocation")
	if prov.Kind == ProvenanceIsland {
		note = "island_make<" + elem + ("> length contract: zero valid, negative and overflow " +
			"reject before island metadata access")
	}
	b.addOperation(
		Operation{
			Kind:    OpAllocIntent,
	...
	if prov.Kind == ProvenanceIsland {
		b.addFact(
			b.islandAllocationFact(
				FactProvenanceKnown,
				id,
				islandFactToken,
				"",
				"compiler-known allocation intent",
			),
		)
		b.addFact(
			b.islandAllocationFact(
				...
				islandFactToken,
				"",
				"slice metadata is opaque in safe code",
			),
		)
		...
		b.addFact(
			b.islandAllocationFact(
				...
				islandFactToken,
				value.Region,
				"island region allocator returns 16-byte aligned payloads",
			),
		)
		return
	}
	b.addFact(
		Fact{Kind: FactProvenanceKnown, ValueID: id, Reason: "compiler-known allocation intent"},
	)
	b.addFact(
		Fact{Kind: FactLenStable, ValueID: id, Reason: "slice metadata is opaque in safe code"},
	)
	b.addFact(Fact{Kind: FactRegionAlive, ValueID: id, Region: value.Region})
}

func (b *builder) islandAllocationFact(
	kind FactKind,
	valueID string,
	token islandTokenState,
	region string,
	reason string,
) Fact {
	return Fact{
...
func (b *builder) recordRawSliceCall(
	name string,
	elem string,
	call *frontend.CallExpr,
	targetName string,
	...
	if root := b.rawPointerBaseRoot(ptr); root != "" {
		if baseBytes, bytesKnown := b.rawPointerBaseByteSize(root); bytesKnown {
			if lengthConst, lengthKnown := evalConstInt64(callArg(call, 1)); lengthKnown {
				...
				sliceBounds := runtimeabi.RawSliceBoundsFromParts(
					runtimeabi.RawPointerBoundsMetadata{
						Status:                 ptrStatus,
						BaseID:                 root,
						BaseBytes:              baseBytes,
					...
					lengthConst,
					int64(elementSize(elem)),
				)
				status = sliceBounds.Status
				switch status {
				case runtimeabi.RawSliceBoundsVerifiedAllocationRoot,
					runtimeabi.RawSliceBoundsRejectedNegativeLength,
					runtimeabi.RawSliceBoundsRejectedLengthOverflow:
					unsafeClass = UnsafeChecked
					provenance.Root = "raw_parts:" + root
					note = fmt.Sprintf(
						"%s raw_slice_bounds: %s base:%s offset:%d length:%d length_bytes:%d elem_size:%d",
						name,
						status,
						root,
						offsetBytes,
						lengthConst,
						sliceBounds.LengthBytes,
						elementSize(elem),
					)
				default:
					note = fmt.Sprintf(
						"%s raw_slice_bounds: %s base:%s offset:%d length:%s elem_size:%d",
						name,
						status,
						root,
						offsetBytes,
	...
	b.reclassifyMemoryBinding(
		targetName,
		provenance,
		"raw slice gateway has external provenance unless an unsafe proof supplies more facts",
	)
}
...
func (b *builder) recordSliceViewCall(
	name string,
	valueType string,
	method string,
	call *frontend.CallExpr,
	...
	if invalidView {
		...
		b.addOperation(
			Operation{
				...
				Inputs:  callInputs(call.Args),
				Outputs: []string{id},
				Note:    name + " invalid range is rejected before construction",
			},
		)
	...
	b.addOperation(
		Operation{
			...
			Outputs: []string{id},
			Note: fmt.Sprintf(
				"%s range %s elem_width:%d elem_shift:%d bounds_check:normal_build",
				name,
				windowRange,
				width,
				shift,
	...
	if known {
		b.addFact(
			Fact{
				...
				ValueID: id,
				Reason:  "slice view provenance is derived from source slice",
			},
		)
		b.addFact(
			Fact{
				Kind:    FactLenStable,
				ValueID: id,
				Reason:  "safe slice view metadata is constructed by checked compiler builtin",
			},
		)
		return
	}
	b.addFact(
		Fact{
			Kind:    FactProvenanceUnknown,
			ValueID: id,
			Reason:  "slice view source provenance is external or unknown",
		},
	)
	b.reclassifyMemoryBinding(
		targetName,
		prov,
		"slice view source provenance is external or unknown",
	)
}

func (b *builder) recordBorrowCall(
	name string,
	valueType string,
	call *frontend.CallExpr,
	targetName string,
	...
	b.addOperation(
		Operation{
			...
			Inputs:  callInputs(call.Args),
			Outputs: []string{id},
			Note:    name + " creates borrowed view without allocation",
		},
	)
	...
	if known {
		...
		b.addFact(
			Fact{
				Kind:    FactLenStable,
				ValueID: id,
				Reason:  "borrowed view header is immutable in safe code",
			},
		)
	} else {
		b.addFact(
...
func (b *builder) recordCopyCall(
	...
	op := b.addOperation(
		Operation{
			...
			Inputs:  callInputs(call.Args),
			Outputs: []string{id},
			Note:    name + " creates owned copy with new provenance",
		},
	)
	...
	b.addFact(
		Fact{
			Kind:    FactLenStable,
			ValueID: id,
			Reason:  "copy result metadata is owned by the new allocation",
		},
	)
	b.addFact(Fact{Kind: FactRegionAlive, ValueID: id, Region: value.Region})
	...
	for _, kind := range []ValueKind{ValueAllocIntent, ValueView, ValueParam, ValueLocal} {
		...
		if value, ok := b.values[id]; ok {
			...
			case ProvenanceExternal:
				return Provenance{
					Kind: ProvenanceExternal,
					Root: "derived:" + value.Provenance.Root,
				}, false
			default:
				return Provenance{
