...
func ValidateInterfaceImplementationProviders(world *module.World, checked *semantics.CheckedProgram, linked []LinkedObject) error {
	...
	for _, moduleName := range modules {
		if _, ok := providers[moduleName]; !ok {
			return fmt.Errorf("missing implementation object for interface module '%s'; pass --link-object with a matching TOBJ", moduleName)
		}
	}
	return nil
}

func ValidateInterfaceImplementationSymbols(world *module.World, checked *semantics.CheckedProgram, moduleName string, obj *tobj.Object, path string) error {
	...
	for _, name := range UnsupportedInterfaceModuleGenericSymbols(world, moduleName) {
		return fmt.Errorf("implementation object for interface module '%s' cannot satisfy generic export '%s'; precompiled link objects require monomorphic exported functions (%s)", moduleName, name, path)
	}
	for _, name := range ExpectedInterfaceModuleSymbols(world, moduleName) {
		sym, ok := symbols[name]
		if !ok {
			return fmt.Errorf("implementation object for interface module '%s' missing exported symbol '%s' (%s)", moduleName, name, path)
		}
		if !sym.HasSignature {
			return fmt.Errorf("implementation object for interface module '%s' symbol '%s' missing signature metadata (%s)", moduleName, name, path)
		}
