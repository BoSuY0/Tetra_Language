...
func ValidateProductionPostgresCoverage(report ProductionPostgresCoverageReport) error {
	...
	if report.OfficialTechEmpowerResultClaimed {
		return fmt.Errorf(
			"production PostgreSQL coverage: official TechEmpower claim is forbidden for P19.3",
		)
	}
	if report.ProductionDatabaseBenchmarkClaimed {
		return fmt.Errorf(
			"production PostgreSQL coverage: production database benchmark claim is forbidden for P19.3",
		)
	}
	if report.P20PerformanceMatrixClaimed {
		return fmt.Errorf(
			"production PostgreSQL coverage: P20 performance matrix claim is forbidden for P19.3",
		)
	}
	if report.CPlusPlusRustParityClaimed {
		return fmt.Errorf(
			"production PostgreSQL coverage: C++/Rust parity claim is forbidden for P19.3",
		)
	}
	if report.ExternalProductionDatabaseClaimed {
		return fmt.Errorf(
			"production PostgreSQL coverage: external production database claim is forbidden for P19.3",
		)
	}
	if report.FullSourceLevelDriverClaimed {
		return fmt.Errorf(
			"production PostgreSQL coverage: full source-level driver claim is forbidden for P19.3",
		)
	}
	if report.RuntimeBehaviorChanged {
		return fmt.Errorf(
			("production PostgreSQL coverage: runtime behavior change " +
				"claim is forbidden for report-only P19.3 coverage"),
		)
	}
	...
	if len(report.Rows) != len(expectedStatus) {
		return fmt.Errorf(
			"production PostgreSQL coverage: row count = %d, want %d",
			len(report.Rows),
			len(expectedStatus),
		)
	}
	...
	for _, row := range report.Rows {
		...
		if row.Status != wantStatus {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q status = %q, want %q",
				row.ID,
				row.Status,
				wantStatus,
			)
		...
		if strings.TrimSpace(row.Name) == "" || strings.TrimSpace(row.Evidence) == "" ||
			...
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q missing evidence or boundary",
				row.ID,
			)
		}
		if len(row.RequiredFacts) == 0 {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q missing required facts",
				row.ID,
			)
		}
		if row.ClaimsOfficialTechEmpowerResult {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q claims official TechEmpower result",
				row.ID,
			)
		}
		if row.ClaimsProductionDatabaseBenchmark {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q claims production database benchmark",
				row.ID,
			)
		}
		if row.ClaimsP20PerformanceMatrix {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q claims P20 performance matrix",
				row.ID,
			)
		}
		if row.ClaimsCPlusPlusRustParity {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q claims C++/Rust parity",
				row.ID,
			)
		}
		if row.ClaimsExternalProductionDatabase {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q claims external production database",
				row.ID,
			)
		}
		if row.ClaimsFullSourceLevelDriver {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q claims full source-level driver",
				row.ID,
			)
		}
		if row.ClaimsRuntimeBehaviorChange {
			return fmt.Errorf(
				"production PostgreSQL coverage: row %q claims runtime behavior change",
				row.ID,
			)
		}
	}
	...
	checks := []struct {
		...
		{
			ProductionPostgresStartupSCRAM,
			[]string{
				"readStartupReady",
				"AuthSASL",
		...
		{
			ProductionPostgresPreparedStatements,
			[]string{
				"PreparedQueryFormat",
				"AppendParse",
		...
		{
			ProductionPostgresBinaryProtocol,
			[]string{
				"AppendBindFormat",
				"BinaryFormat",
		...
		{
			ProductionPostgresPoolingBackpressure,
			[]string{
				"NewPool",
				"ErrPoolExhausted",
		...
		{
			ProductionPostgresBorrowedRegionRowDecode,
			[]string{
		...
		{
			ProductionPostgresEndpointWorkloads,
			[]string{
				"/db",
				"/queries",
				"/updates",
		...
		{
			ProductionPostgresSourceFirstBenchmark,
			[]string{"p19.3_postgres_source_first", "DB single query", "DB fortunes"},
		},
		{
			ProductionPostgresLiveBenchmarkHonesty,
			[]string{
	...
	for _, check := range checks {
		...
		for _, want := range check.wants {
			if !containsProductionPostgresRowText(row, want) {
				return fmt.Errorf(
					"production PostgreSQL coverage: row %q missing fact %q",
					row.ID,
					want,
				)
			}
...
func productionPostgresStartupSCRAMRow() ProductionPostgresEvidenceRow {
	return ProductionPostgresEvidenceRow{
		...
		RequiredFacts: []string{
			("compiler/internal/pgrt/wire.go::Connect writes " +
				"AppendStartupMessage before reading startup auth frames"),
			("compiler/internal/pgrt/wire.go::readStartupReady handles " +
				"AuthSASL, AuthSASLContinue, and AuthSASLFinal for " +
				"SCRAM-SHA-256"),
			("compiler/internal/pgrt/wire_test.go::" +
				"TestClientCompletesSCRAMSHA256Authentication proves the " +
				"local startup/SCRAM exchange"),
		},
		Evidence: ("compiler/internal/pgrt/wire.go::readStartupReady; " +
			"compiler/internal/pgrt/scram.go; " +
			"compiler/internal/pgrt/wire_test.go::" +
			"TestClientCompletesSCRAMSHA256Authentication; " +
			"compiler/internal/pgrt/wire_test.go"),
		Boundary: ("startup/SCRAM evidence is local PostgreSQL wire-protocol " +
			"compatibility; it is not TLS, channel binding, or external " +
			"production database deployment evidence"),
		SourceFirst: false,
	}
}
...
func productionPostgresPreparedStatementsRow() ProductionPostgresEvidenceRow {
	return ProductionPostgresEvidenceRow{
		...
		Status: ProductionPostgresImplementedNarrow,
		RequiredFacts: []string{
			("compiler/internal/pgrt/wire.go::PreparedQueryFormat " +
				"prepares statements before Bind/Describe/Execute/Sync"),
			("compiler/internal/pgrt/wire.go::AppendParse and " +
				"AppendBindFormat build extended-protocol frames"),
			("compiler/internal/pgrt/wire_test.go::" +
				"TestClientPreparedQueryUsesExtendedProtocol proves Parse " +
				"then Bind/Describe/Execute/Sync sequencing"),
		},
		Evidence: ("compiler/internal/pgrt/wire.go::Prepare; " +
			"compiler/internal/pgrt/wire.go::PreparedQueryFormat; " +
			"compiler/internal/pgrt/wire_test.go::" +
			"TestClientPreparedQueryUsesExtendedProtocol; " +
			"compiler/internal/webrt/webrt_test.go::" +
			"TestServerDBEndpointUsesPoolAndSerializesWorld"),
		Boundary: ("prepared-statement evidence covers named statement reuse in " +
			"the local runtime client; it does not claim a public " +
			"source-level driver API or query planner behavior"),
		SourceFirst: false,
	}
}
...
func productionPostgresBinaryProtocolRow() ProductionPostgresEvidenceRow {
	return ProductionPostgresEvidenceRow{
		...
		RequiredFacts: []string{
			...
			("compiler/internal/pgrt/wire.go::BinaryFormat and " +
				"AppendInt4Binary encode TechEmpower int4 parameters"),
			("compiler/internal/pgrt/wire_test.go::" +
				"TestAppendBindBinaryFormatsAndDecodeInt4 proves binary int4 " +
				"bind/decode evidence"),
		},
		Evidence: ("compiler/internal/pgrt/wire.go::AppendBindFormat; " +
			"compiler/internal/pgrt/wire.go::AppendInt4Binary; " +
			"compiler/internal/pgrt/wire.go::DecodeInt4; " +
			"compiler/internal/pgrt/wire_test.go::" +
			"TestAppendBindBinaryFormatsAndDecodeInt4"),
		Boundary: ("binary protocol support is bounded to int4 " +
			"parameters/results used by the DB endpoint path; no " +
			"complete PostgreSQL binary type matrix is claimed"),
		SourceFirst: false,
	}
}
...
func productionPostgresPoolingBackpressureRow() ProductionPostgresEvidenceRow {
	return ProductionPostgresEvidenceRow{
		...
		RequiredFacts: []string{
			...
			("compiler/internal/pgrt/pool.go::Checkout returns " +
				"ErrPoolExhausted instead of over-opening when maxOpen is " +
				"reached"),
			("compiler/internal/pgrt/pool_test.go::" +
				"TestPoolReusesReleasedConnectionAndCapsOpenConnections " +
				"proves reuse and backpressure"),
		},
		Evidence: ("compiler/internal/pgrt/pool.go::NewPool; " +
			"compiler/internal/pgrt/pool.go::Checkout; " +
			"compiler/internal/pgrt/pool_test.go::" +
			"TestPoolReusesReleasedConnectionAndCapsOpenConnections; " +
			"compiler/internal/pgrt/pool_test.go::" +
			"TestPoolStatsTrackOpenIdleInUseAndClosedState"),
		Boundary: ("pooling evidence is a bounded local cap/reuse model; no " +
			"adaptive production pool, queue wait policy, or throughput " +
			"claim is made"),
		SourceFirst:  false,
		Backpressure: true,
	}
...
func productionPostgresBorrowedRegionRowDecodeRow() ProductionPostgresEvidenceRow {
	return ProductionPostgresEvidenceRow{
		...
		Status: ProductionPostgresImplementedNarrow,
		RequiredFacts: []string{
			("compiler/internal/pgrt/wire.go::DecodeDataRowBorrowed " +
				"returns RowStorageBorrowed with borrowed cell slices"),
			"compiler/internal/pgrt/wire.go::RowDecodeReport records BorrowedCells and CopiedCells",
			("compiler/internal/pgrt/wire_test.go::" +
				"TestDecodeDataRowBorrowedDoesNotCopyCells mutates the " +
				"backing payload to prove borrowing"),
		},
		Evidence: ("compiler/internal/pgrt/wire.go::DecodeDataRowBorrowed; " +
			"compiler/internal/pgrt/wire.go::RowDecodeReport; " +
			"compiler/internal/pgrt/wire_test.go::" +
			"TestDecodeDataRowBorrowedDoesNotCopyCells"),
		Boundary: ("borrowed row decode evidence is tied to caller-owned frame " +
			"payload lifetime; no external region allocator integration " +
			"or all-row zero-copy guarantee is claimed"),
		SourceFirst:   false,
		BorrowedViews: true,
	}
}
...
func productionPostgresEndpointWorkloadsRow() ProductionPostgresEvidenceRow {
	return ProductionPostgresEvidenceRow{
		...
		Status: ProductionPostgresImplementedNarrow,
		RequiredFacts: []string{
			("compiler/internal/webrt/techempower.go exposes /db single " +
				"query through DBHandler and fetchWorld"),
			("compiler/internal/webrt/techempower.go exposes /queries " +
				"multiple queries and /updates read-then-update paths"),
			("compiler/internal/webrt/techempower.go exposes /fortunes " +
				"through FortunesHandler and fetchFortunes"),
			("compiler/internal/webrt/webrt_test.go covers /db, /queries, " +
				"and /updates local fake-PostgreSQL wire paths"),
			("compiler/internal/webrt/webrt_test.go::" +
				"TestServerFortunesEndpointFetchesSortsAndEscapesHTML covers " +
				"/fortunes sorting and escaping"),
		},
		Evidence: ("compiler/internal/webrt/techempower.go::DBHandler; " +
			"compiler/internal/webrt/techempower.go::QueriesHandler; " +
			"compiler/internal/webrt/techempower.go::UpdatesHandler; " +
			"compiler/internal/webrt/techempower.go::FortunesHandler; " +
			"compiler/internal/webrt/webrt_test.go"),
		Boundary: ("endpoint evidence covers local runtime correctness with " +
			"fake wire servers and existing local SCRAM harness " +
			"artifacts; it is not an official benchmark or measured " +
			"production DB deployment claim"),
		SourceFirst: false,
	}
}
...
func productionPostgresSourceFirstBenchmarkRow() ProductionPostgresEvidenceRow {
	return ProductionPostgresEvidenceRow{
		...
		RequiredFacts: []string{
			...
			("p19.3_postgres_source_first requires Tetra-source DB single " +
				"query, DB multiple queries, DB updates, and DB fortunes rows"),
			("DB single query, DB multiple queries, DB updates, and DB " +
				"fortunes rows must list proof, allocation, bounds, and " +
		...
		Evidence: ("tools/cmd/truth-bench-harness/main.go::" +
			...
			"TestP19PostgresSourceFirstScopeRequiresTetraOnlyDBEndpointRo" +
			"ws; " +
		...
		Boundary: ("source-first benchmark gate records local dry-run/source " +
			...
			"production database benchmark, P20 performance matrix, or " +
			"C++/Rust parity claim"),
		SourceFirst: true,
	}
}
...
func productionPostgresLiveBenchmarkHonestyRow() ProductionPostgresEvidenceRow {
	return ProductionPostgresEvidenceRow{
		...
		RequiredFacts: []string{
			("reports/production-postgres-v1/benchmarks/postgres-source-fi" +
				"rst-report.json records p19.3_postgres_source_first dry-run " +
				"source coverage for DB single query, DB multiple queries, " +
				"DB updates, and DB fortunes rows"),
			("docs/benchmarks/techempower_scram_single_query_local_report." +
				"json validates with tools/cmd/validate-techempower-report " +
				"and covers all six local endpoints including /db, /queries, " +
				"/updates, and /fortunes"),
			("docs/benchmarks/techempower_scram_single_query_matrix_local_" +
				"report.json validates with " +
				"tools/cmd/validate-techempower-report and records " +
				"SCRAM-SHA-256 PostgreSQL 16.9.0 /db matrix evidence"),
			("docs/benchmarks/techempower_scram_endpoint_matrix_local_repo" +
				"rt.json validates with " +
				"tools/cmd/validate-techempower-report and records " +
				"SCRAM-SHA-256 /queries, /updates, and /fortunes matrix " +
				"evidence"),
			("tools/validators/techempower/report_test.go rejects weak " +
				"SCRAM metadata, spoofed command provenance, missing git " +
				"head, grid mismatches, resource snapshot gaps, summary " +
				"mismatches, and weak placeholder evidence"),
		},
		Evidence: ("docs/benchmarks/techempower_scram_single_query_local_report." +
			"json; " +
			"docs/benchmarks/techempower_scram_single_query_matrix_local_" +
			"report.json; " +
			"docs/benchmarks/techempower_scram_endpoint_matrix_local_repo" +
			"rt.json; tools/cmd/validate-techempower-report; " +
			"tools/validators/techempower/report_test.go"),
		Boundary: ("live local SCRAM reports measure the local Tetra runtime " +
			"and PostgreSQL path honestly; they are not an official " +
			"TechEmpower result, production database benchmark, P20 " +
			"performance matrix, C++/Rust parity claim, external " +
			"production database deployment, measured speed comparison, " +
