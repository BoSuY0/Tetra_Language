...
func validateAllocationMemoryBackendEvidence(function string, alloc Allocation) error {
	...
	if err := runtimeabi.ValidateMemoryBackendAllocationEvidence(*alloc.MemoryBackend); err != nil {
		return fmt.Errorf("allocplan verifier: %s allocation %q invalid memory backend evidence: %w", function, alloc.ValueID, err)
	}
	runtimePath := RuntimePathForAllocation(alloc)
	if alloc.MemoryBackend.RuntimePath != runtimePath {
		return fmt.Errorf("allocplan verifier: %s allocation %q memory backend runtime_path %q does not match allocation runtime_path %q", function, alloc.ValueID, alloc.MemoryBackend.RuntimePath, runtimePath)
	}
	...
	case runtimeabi.MemoryFootprintEstimated, runtimeabi.MemoryFootprintMeasured:
		if int64(alloc.BytesCommitted) != alloc.MemoryBackend.CommitBytes {
			return fmt.Errorf("allocplan verifier: %s allocation %q bytes_committed %d does not match memory_backend commit_bytes %d", function, alloc.ValueID, alloc.BytesCommitted, alloc.MemoryBackend.CommitBytes)
		}
		if int64(alloc.BytesReleased) != alloc.MemoryBackend.ReleaseBytes {
			return fmt.Errorf("allocplan verifier: %s allocation %q bytes_released %d does not match memory_backend release_bytes %d", function, alloc.ValueID, alloc.BytesReleased, alloc.MemoryBackend.ReleaseBytes)
		}
	default:
		if alloc.BytesCommitted != 0 || alloc.BytesReleased != 0 {
			return fmt.Errorf("allocplan verifier: %s allocation %q unsupported/blocked memory backend evidence must not report committed or released bytes", function, alloc.ValueID)
		}
	}
	return nil
}
...
func validateAllocationReasonCodes(function string, alloc Allocation) error {
	if allocationUsesHeap(alloc) {
		if len(alloc.HeapReasonCodes) == 0 {
			return fmt.Errorf("allocplan verifier: %s allocation %q missing heap reason code", function, alloc.ValueID)
		}
	} else if len(alloc.HeapReasonCodes) > 0 {
		return fmt.Errorf("allocplan verifier: %s allocation %q has heap reason codes without heap storage", function, alloc.ValueID)
	}
	for _, group := range []struct {
		...
		for _, code := range group.codes {
			if strings.TrimSpace(code) == "" {
				return fmt.Errorf("allocplan verifier: %s allocation %q has empty %s entry", function, alloc.ValueID, group.label)
			}
			if strings.TrimSpace(code) != code {
				return fmt.Errorf("allocplan verifier: %s allocation %q has untrimmed %s entry %q", function, alloc.ValueID, group.label, code)
			}
			if seen[code] {
				return fmt.Errorf("allocplan verifier: %s allocation %q has duplicate %s entry %q", function, alloc.ValueID, group.label, code)
			}
			seen[code] = true
			if group.heap && !isKnownHeapReasonCode(code) {
				return fmt.Errorf("allocplan verifier: %s allocation %q has unknown heap reason code %q", function, alloc.ValueID, code)
			}
		}
	}
	for _, code := range alloc.HeapReasonCodes {
		if !contains(alloc.ReasonCodes, code) {
			return fmt.Errorf("allocplan verifier: %s allocation %q heap reason code %q missing from reason_codes", function, alloc.ValueID, code)
		}
	}
	return nil
}
