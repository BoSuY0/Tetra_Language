		...
		for _, alloc := range fn.Allocations {
			...
			if alloc.MemoryBackend != nil {
				...
				if len(alloc.MemoryBackend.Operations) > 0 {
					fmt.Fprintf(&b, " memory_backend_ops: %s", formatMemoryBackendOperations(alloc.MemoryBackend.Operations))
				}
				if alloc.MemoryBackend.EvidenceClass != "" {
					fmt.Fprintf(&b, " memory_backend_evidence: %s", alloc.MemoryBackend.EvidenceClass)
				}
				if alloc.MemoryBackend.BlockedReason != "" {
					fmt.Fprintf(&b, " memory_backend_blocked: %s", alloc.MemoryBackend.BlockedReason)
				}
				if alloc.MemoryBackend.UnsupportedReason != "" {
					fmt.Fprintf(&b, " memory_backend_unsupported: %s", alloc.MemoryBackend.UnsupportedReason)
				}
			}
			if alloc.RawPointerBoundsStatus != "" {
			...
			if alloc.Domain != nil {
				fmt.Fprintf(&b, " domain_id: %s domain_kind: %s", alloc.Domain.DomainID, alloc.Domain.Kind)
			}
			if alloc.DebugMode != "" {
...
func formatDomainSummary(domains []runtimeabi.MemoryDomainSummary) string {
	...
	for _, domain := range domains {
		parts = append(parts, fmt.Sprintf("%s=%d/%d", domain.DomainID, domain.RequestedBytes, domain.ReservedBytes))
	}
	return strings.Join(parts, ",")
}
