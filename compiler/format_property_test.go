...
`,
	...
	for name, src := range fixtures {
		t.Run(name, func(t *testing.T) {
			once, err := FormatSource([]byte(src), name+".tetra")
			if err != nil {
				t.Fatalf("format once: %v", err)
			}
			twice, err := FormatSource(once, name+".tetra")
			if err != nil {
	...
	for _, root := range roots {
		...
		t.Run(filepath.ToSlash(root), func(t *testing.T) {
			...
			for _, path := range files {
				...
				before, err := ParseFile(raw, path)
				if err != nil {
					t.Fatalf("parse original %s: %v", path, err)
				}
				once, err := FormatSource(raw, path)
				if err != nil {
				...
				twice, err := FormatSource(once, path)
				if err != nil {
...
func fileSurfaceSignature(file *FileAST) string {
	var parts []string
