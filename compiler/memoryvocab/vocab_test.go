...
func TestIslandKernelClaimVocabularyRegistered(t *testing.T) {
	...
	if got := RequiredIslandKernelClaimValidator(
		ClaimIslandProofVerified,
	); got != "validate-island-proof" {
		t.Fatalf(
			"RequiredIslandKernelClaimValidator(%q) = %q, want validate-island-proof",
			ClaimIslandProofVerified,
			got,
		)
	}
	if !IslandKernelClaimValidatorMismatch(ClaimIslandProofVerified, "memory_report_validator") {
		t.Fatalf(
			"IslandKernelClaimValidatorMismatch should reject island proof without validate-island-proof",
		)
	}
...
func TestUnsafeExternalRootTrustedStorageVocabulary(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if !UnsafeExternalRootTrustedStorage(
				...
				t.Fatalf(
					"UnsafeExternalRootTrustedStorage(%q, %q, Stack, Stack) = false, want true",
					tc.provenance,
					tc.unsafe,
				)
			}
			if UnsafeExternalRootTrustedStorage(
				...
				t.Fatalf(
					"UnsafeExternalRootTrustedStorage(%q, %q, Heap, Heap) = true, want false",
					tc.provenance,
					tc.unsafe,
				)
			}
	...
	if UnsafeExternalRootTrustedStorage(
		...
		t.Fatalf(
			"safe owned storage should not be classified as unsafe external root trusted storage",
		)
	}
}
