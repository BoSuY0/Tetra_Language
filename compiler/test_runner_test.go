...
func TestTestRunnerSourcesIncludeCaseMetadata(t *testing.T) {
    ...
    expect 40 + 2 == 42
`)
	cases, err := TestRunnerSources(src, "math_test.tetra")
	if err != nil {
...
test "module case":
    expect fortyTwo() == 42
`)
	cases, err := TestRunnerSources(src, "app/tests.tetra")
	if err != nil {
		t.Fatalf("TestRunnerSources: %v", err)
	}
	if len(cases) != 1 {
...
func TestTestRunnerSourcesDeclareEffectsForSyntheticFunctions(t *testing.T) {
    ...
    expect true
`)
	cases, err := TestRunnerSources(src, "io_test.tetra")
	if err != nil {
		t.Fatalf("TestRunnerSources: %v", err)
	}
	generated := string(cases[0].Source)
...
func TestTestRunnerReportCountsPassFailMetadata(t *testing.T) {
	...
	report := NewTestRunnerReport([]TestRunnerResult{pass, fail})
