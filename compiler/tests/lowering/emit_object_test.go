...
func TestEmitObjectRequiresMain(t *testing.T) {
	...
	if _, err := compiler.BuildFileWithStatsOpt(
		...
		outPath,
		"linux-x64",
		compiler.BuildOptions{Emit: compiler.EmitObject},
	); err == nil {
		t.Fatalf("expected error, got nil")
	}
}
...
func TestEmitLibraryBuildOnlyAcrossNativeTargets(t *testing.T) {
	...
	if err := os.WriteFile(
		srcPath,
		[]byte("fun foo(a: i32, b: i32, c: i32, d: i32, e: i32, f: i32, g: i32): i32 { return a + b + c + d + e + f + g }\n"),
		0o644,
	); err != nil {
		t.Fatalf("write source: %v", err)
	}
...
func TestEmitLibraryLinuxX86WritesRealI386TOBJ(t *testing.T) {
	...
	if err := os.WriteFile(
		srcPath,
		[]byte("fun foo(a: i32, b: i32): i32 { return a + b }\n"),
		0o644,
	); err != nil {
		t.Fatalf("write source: %v", err)
	}
