...
uses runtime:
    ...
    return core.task_join_i32(task)
`,
			want: ("task_spawn_i32 target 'worker' touches mutable global state and " +
...
`,
		...
		{
			name: ("RaceSafetyRejectsTaskTargetFunctionTypedMultiReturnAliasReturned" +
				"CallbackWithMutableTarget"),
			src: `
var g: Int
...
uses runtime:
    ...
    case TaskErr.failed:
        0
`,
			want: ("task_spawn_i32_typed target 'worker' touches mutable global " +
...
uses runtime:
    ...
    return core.task_join_i32(task)
`,
			want: ("task_spawn_group_i32 target 'worker' touches mutable global " +
				"state and cannot cross task boundary"),
		},
		{
			name: ("RaceSafetyRejectsTypedTaskGroupTargetReturnedEnumPayloadDirectCl" +
				"osureWithMutableGlobalTarget"),
			src: `
enum TaskErr:
    case failed
...
uses runtime:
    ...
    case TaskErr.failed:
        0
`,
			want: ("task_spawn_group_i32_typed target 'worker' touches mutable " +
				"global state and cannot cross task boundary"),
		},
		{
