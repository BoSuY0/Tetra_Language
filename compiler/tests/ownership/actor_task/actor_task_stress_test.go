...
func TestActorTaskBoundedStressExamples(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			path:     testkit.RepoPath(t, "examples", "tasks", "task_bounded_stress.tetra"),
			wantExit: 42,
		},
		{
			name:     "task group cancel smoke",
			path:     testkit.RepoPath(t, "examples", "tasks", "task_group_cancel_smoke.tetra"),
			wantExit: 1,
		},
		{
			name:     "task group lifecycle smoke",
			path:     testkit.RepoPath(t, "examples", "tasks", "task_group_lifecycle_smoke.tetra"),
			wantExit: 42,
		},
		{
			name: "actor task dogfood",
			path: testkit.RepoPath(
				t,
				"examples",
				"projects",
				"dogfood_actor_task",
