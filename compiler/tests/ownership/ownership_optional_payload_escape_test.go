...
`,
	...
	testhelpers.RequireCheckWorldFilesErrorContains(
		...
		("borrowed value derived from 'maybe' cannot be passed to non-" +
			"borrow parameter 1 of 'lib.sink.sink'"),
	)
}

func TestOwnershipRejectsCrossModuleBorrowedPtrOptionalMatchBindingImportedConsumedCall(
	t *testing.T,
) {
	files := map[string]string{
		"lib/sink.t4": `module lib.sink
...
`,
	...
	testhelpers.RequireCheckWorldFilesErrorContains(
		...
		files,
		"app/main.t4",
		"borrowed value derived from 'maybe' cannot be consumed by 'lib.sink.sink'",
	)
}
...
`,
	...
	testhelpers.RequireCheckWorldFilesErrorContains(
		...
		files,
		"app/main.t4",
		"borrowed value derived from 'maybe' cannot be passed as inout to 'lib.sink.mutate'",
	)
}
...
func TestOwnershipRejectsCrossModuleBorrowedSliceOptionalPayloadBindingImportedCallEscapes(
	t *testing.T,
) {
	tests := []struct {
		name     string
...
func TestOwnershipRejectsCrossModuleBorrowedSliceOptionalPayloadBindingInoutAssignment(
	t *testing.T,
) {
	tests := []struct {
		name string
...
`,
	...
	testhelpers.RequireCheckWorldFilesErrorContains(
		...
		files,
		"app/main.t4",
		"cannot use consumed value 'returned.$elem'",
	)
}
