...
`
			file, err := compiler.ParseFile(
				[]byte(src),
				"borrowed_slice_optional_payload_"+strings.ReplaceAll(tt.name, " ", "_")+".tetra",
			)
			if err != nil {
...
func assertBorrowedSliceOptionalPayloadDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want borrowed slice optional-payload escape %q",
			diag.Message,
			wantText,
		)
	}
...
`,
	...
	for _, tt := range sameModuleTests {
		t.Run("same module "+tt.name, func(t *testing.T) {
			file, err := compiler.ParseFile(
				[]byte(tt.src),
				"borrowed_nested_slice_struct_escape.tetra",
			)
			if err != nil {
				t.Fatalf("ParseFile: %v", err)
			}
...
`,
			...
			wantText: ("aggregate 'lib.model.BufBox' contains borrowed slice field " +
				"'buf' that cannot be stored in global"),
		},
	}
...
func assertBorrowedNestedSliceStructDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want borrowed nested slice struct escape %q",
			diag.Message,
			wantText,
		)
	}
...
`,
	...
	for _, tt := range sameModuleTests {
		t.Run("same module "+tt.name, func(t *testing.T) {
			file, err := compiler.ParseFile(
				[]byte(tt.src),
				"borrowed_nested_slice_enum_payload_escape.tetra",
			)
			if err != nil {
				t.Fatalf("ParseFile: %v", err)
			}
...
func assertBorrowedNestedSliceEnumPayloadDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want borrowed nested slice enum-payload escape %q",
			diag.Message,
			wantText,
		)
	}
...
`
		file, err := compiler.ParseFile(
			[]byte(src),
			"borrowed_ptr_aggregate_optional_global_escape.tetra",
		)
		if err != nil {
			t.Fatalf("ParseFile: %v", err)
		}
...
func assertBorrowedPtrAggregateOptionalGlobalAssignmentDiagnostic(t *testing.T, err error) {
	...
	if !strings.Contains(
		...
		t.Fatalf(
			"message = %q, want borrowed ptr aggregate optional global assignment escape",
			diag.Message,
		)
	}
	raw, err := json.Marshal(diag)
