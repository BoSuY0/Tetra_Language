...
`)
		assertTaskHandleAliasTransferDiagnostic(
			t,
			err,
			"cannot use consumed value 'returned.handle'",
		)
	})
...
func assertTaskHandleAliasTransferDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want task-handle alias transfer diagnostic %q",
			diag.Message,
			wantText,
		)
	}
...
`,
			...
			_, err = compiler.CheckWorld(world)
			assertPtrContainingNestedAggregateCallDiagnostic(
				t,
				err,
				strings.Replace(tt.wantText, "'sink'", "'lib.sink.sink'", 1),
			)
		})
	}
}
...
func assertPtrContainingNestedAggregateCallDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want ptr-containing/nested aggregate call diagnostic %q",
			diag.Message,
			wantText,
		)
	}
...
`,
			...
			_, err = compiler.CheckWorld(world)
			assertPtrEnumPayloadCallDiagnostic(
				t,
				err,
				strings.Replace(tt.wantText, "'sink'", "'lib.sink.sink'", 1),
			)
...
`,
			...
			_, err = compiler.CheckWorld(world)
			assertPtrOptionalPayloadCallDiagnostic(
				t,
				err,
				strings.Replace(tt.wantText, "'sink'", "'lib.sink.sink'", 1),
			)
...
func assertPtrOptionalPayloadCallDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want ptr optional-payload call diagnostic %q",
			diag.Message,
			wantText,
		)
	}
...
`,
			...
			_, err = compiler.CheckWorld(world)
			assertSliceOptionalPayloadCallDiagnostic(
				t,
				err,
				strings.Replace(tt.wantText, "'sink'", "'lib.sink.sink'", 1),
			)
...
func assertSliceOptionalPayloadCallDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want slice optional-payload call diagnostic %q",
			diag.Message,
			wantText,
		)
	}
...
func TestSafetyDiagnosticCodesForFunctionTypedSliceAggregateCallbackCallRejections(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			var err error
			if tt.cross {
				files := functionTypedSliceCallbackCrossModuleFiles(
					tt.target,
					tt.aggregate,
					tt.mode,
				)
				...
				err = testkit.CheckProgram(
					functionTypedSliceCallbackSource(tt.target, tt.aggregate, tt.mode, false),
				)
			}
...
func functionTypedSliceCallbackSource(target, aggregate, mode string, qualified bool) string {
	return functionTypedSliceCallbackTypeDecls(aggregate, false) + "\n" +
		...
		"func caller(" + functionTypedSliceCallbackParam(
		target,
		aggregate,
		mode,
		qualified,
...
func functionTypedSliceCallbackBody(target, aggregate, mode string, qualified bool) string {
	...
	if mode == "consume" {
		...
		prefix := "    let " + local + ": " + functionTypedSliceCallbackParamType(
			aggregate,
			"owned",
			qualified,
		) + " = " + arg + "\n"
	...
	if mode == "inout" {
		local := functionTypedSliceCallbackLocalName(aggregate)
		prefix := "    var " + local + ": " + functionTypedSliceCallbackParamType(
			aggregate,
			"owned",
			qualified,
		) + " = " + arg + "\n"
...
func assertFunctionTypedSliceAggregateCallbackDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want function-typed slice aggregate callback diagnostic %q",
			diag.Message,
			wantText,
		)
	}
