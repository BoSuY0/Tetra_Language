...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(
				string(raw),
				`"code":"`+compiler.DiagnosticCodeSafetyLifetime+`"`,
			) {
...
func assertResourceAliasFinalizationDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want resource alias finalization diagnostic %q",
			diag.Message,
			wantText,
		)
	}
...
uses actors:
    ...
    return 0
`)
		assertActorTaskOptionalPayloadAliasTransferDiagnostic(
			t,
			err,
...
func assertActorTaskOptionalPayloadAliasTransferDiagnostic(
	t *testing.T,
	err error,
	wantText string,
) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want actor/task optional-payload alias transfer diagnostic %q",
			diag.Message,
			wantText,
		)
	}
