...
func TestSafetyDiagnosticCodesForGenericProtocolRequirementOwnershipMismatch(t *testing.T) {
	tests := []struct {
		...
		{
			name: "same module",
			wantText: ("method 'Box.map' does not match protocol 'Mapper' requirement " +
				"'map': parameter 2 ownership differs: expected 'consume', got 'owned'"),
		},
		{
			name:  "cross module",
			cross: true,
			wantText: ("method 'lib.model.Box.map' does not match protocol " +
				"'lib.model.Mapper' requirement 'map': parameter 2 ownership differs: " +
				"expected 'consume', got 'owned'"),
		},
	}
...
func TestSafetyDiagnosticCodesForProtocolImplOwnershipMismatch(t *testing.T) {
	tests := []struct {
		...
		{
			name: "same module",
			wantText: ("method 'Box.sink' does not match protocol 'Sink' requirement " +
				"'sink': parameter 1 ownership differs: expected 'consume', got 'owned'"),
		},
		{
			name:  "cross module",
			cross: true,
			wantText: ("method 'lib.model.Box.sink' does not match protocol " +
				"'lib.model.Sink' requirement 'sink': parameter 1 ownership differs: " +
				"expected 'consume', got 'owned'"),
		},
	}
...
func assertProtocolImplOwnershipMismatchDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want protocol impl ownership mismatch diagnostic %q",
			diag.Message,
			wantText,
		)
	}
...
func assertGenericProtocolRequirementOwnershipMismatchDiagnostic(
	t *testing.T,
	err error,
	wantText string,
) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want generic protocol ownership mismatch diagnostic %q",
			diag.Message,
			wantText,
		)
	}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(
				string(raw),
				`"code":"`+compiler.DiagnosticCodeSafetyOwnership+`"`,
			) {
...
func assertOptionalPayloadWholeValueDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want optional-payload whole-value diagnostic %q",
			diag.Message,
			wantText,
		)
	}
...
func assertBorrowedPtrAggregateReturnDiagnostic(t *testing.T, err error, borrowedName string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want borrowed ptr aggregate return escape %q",
			diag.Message,
			wantText,
		)
	}
...
`
			file, err := compiler.ParseFile(
				[]byte(src),
				"borrowed_ptr_optional_payload_"+strings.ReplaceAll(tt.name, " ", "_")+".tetra",
			)
			if err != nil {
				t.Fatalf("ParseFile: %v", err)
			}
...
func assertBorrowedPtrOptionalPayloadDiagnostic(t *testing.T, err error, wantText string) {
	...
	if !strings.Contains(diag.Message, wantText) {
		t.Fatalf(
			"message = %q, want borrowed ptr optional-payload escape %q",
			diag.Message,
			wantText,
		)
	}
