...
`,
			want: ("task_spawn_i32_typed target 'worker' uses effect 'capability' " +
				"and cannot cross task boundary"),
		},
	}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			requireCheckFileErrorContainsAll(
				t,
				tt.src,
...
`)
	...
	if !hasPlan250InstrKind(&firstMain, ir.IRSubI32) ||
		!hasPlan250InstrKind(&firstMain, ir.IRJmpIfZero) {
		t.Fatalf("main missing budget guard instructions: %#v", firstMain.Instrs)
	}
	if len(firstMain.Instrs) != len(secondMain.Instrs) {
		t.Fatalf(
			"budget lowering changed instruction count: %d != %d",
			len(firstMain.Instrs),
			len(secondMain.Instrs),
		)
	}
...
`)
	...
	for _, kind := range []ir.IRInstrKind{ir.IRIndexStoreI32, ir.IRIndexLoadI32Unchecked} {
...
func hasPlan250BudgetGuardBefore(fn compiler.IRFunc, chargedIdx int) bool {
	...
	for guardStart := chargedIdx - 8; guardStart >= 0 && guardStart >= chargedIdx-16; guardStart-- {
		if !matchesPlan250BudgetGuardAt(
			fn.Instrs,
			guardStart,
			policy.BudgetLocal,
			policy.FailLabel,
...
func matchesPlan250BudgetGuardAt(
	instrs []ir.IRInstr,
	idx int,
	budgetLocal int,
	failLabel int,
