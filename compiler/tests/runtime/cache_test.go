...
func TestBuildCacheReuse(t *testing.T) {
	...
	stats1, err := compiler.BuildFileWithStats(entry, outPath, "linux-x64")
	if err != nil {
	...
	stats2, err := compiler.BuildFileWithStats(entry, outPath, "linux-x64")
	if err != nil {
	...
	stats3, err := compiler.BuildFileWithStats(entry, outPath, "linux-x64")
	if err != nil {
		t.Fatalf("build3: %v", err)
	}
	testkit.AssertModules(t, stats3.CompiledModules, []string{"engine.render"})
	testkit.AssertModules(t, stats3.CacheHits, []string{"app.game"})
}
...
func TestBuildCacheUnrelatedSignatureChangeDoesNotRebuild(t *testing.T) {
	...
	testkit.AssertModules(t, stats3.CompiledModules, []string{"engine.audio"})
...
func TestBuildCacheAddingUnusedExportDoesNotRebuildConsumer(t *testing.T) {
	...
	testkit.AssertModules(t, stats2.CompiledModules, []string{"engine.render"})
	testkit.AssertModules(t, stats2.CacheHits, []string{"app.game"})
}
...
func TestBuildCacheTransitiveDoesNotRebuildRoot(t *testing.T) {
	...
	testkit.AssertModules(t, stats2.CacheHits, []string{"app.game", "engine.core", "engine.math"})
	...
	testkit.AssertModules(t, stats3.CompiledModules, []string{"engine.core", "engine.math"})
	testkit.AssertModules(t, stats3.CacheHits, []string{"app.game"})
}
...
func TestBuildCacheStructDependencyChangeRebuildsConsumer(t *testing.T) {
	...
	testkit.AssertModules(t, stats1.CompiledModules, []string{"app.game", "engine.math"})
	if len(stats1.CacheHits) != 0 {
		t.Fatalf("unexpected cache hits on first build")
	}
	...
	testkit.AssertModules(t, stats2.CacheHits, []string{"app.game", "engine.math"})
	...
	testkit.AssertModules(t, stats3.CompiledModules, []string{"app.game", "engine.math"})
...
func TestBuildCacheSliceModuleChangeRebuildsOnlyProducer(t *testing.T) {
	...
	testkit.AssertModules(t, stats1.CompiledModules, []string{"app.game", "engine.arr"})
	...
	testkit.AssertModules(t, stats2.CacheHits, []string{"app.game", "engine.arr"})
	...
	testkit.AssertModules(t, stats3.CompiledModules, []string{"engine.arr"})
	testkit.AssertModules(t, stats3.CacheHits, []string{"app.game"})
}
...
func TestBuildCacheCorruptObjectFallsBackToRebuild(t *testing.T) {
	...
	obj, err := compiler.ReadObject(corruptPath)
	if err != nil {
...
func TestBuildCacheTargetChangeInvalidatesObjects(t *testing.T) {
	...
	stats1, err := compiler.BuildFileWithStats(entry, linuxOut, "linux-x64")
	if err != nil {
	...
	stats2, err := compiler.BuildFileWithStats(entry, macosOut, "macos-x64")
	if err != nil {
	...
	stats3, err := compiler.BuildFileWithStats(entry, linuxOut, "linux-x64")
	if err != nil {
