...
func TestTaskGroupFinalizationRejectsInterproceduralOptionalMatchPayloadAliasAfterClose(
	t *testing.T,
) {
	testkit.RequireFileCheckErrorContains(t, `
func pass(maybe: task.group?) -> task.group?:
...
func TestTaskGroupFinalizationRejectsCrossModuleTransitiveInterproceduralAliasAfterClose(
	t *testing.T,
) {
	files := map[string]string{
		"lib/resources.t4": `module lib.resources
