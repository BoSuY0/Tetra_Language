...
`,
	...
	requireCheckWorldFilesErrorContains(
		...
		files,
		"app/main.t4",
		"cannot use freed resource 'returned.$elem'",
	)
}
...
func TestIslandFinalizationRejectsInterproceduralOptionalWrappedReturnAliasDoubleFree(
	t *testing.T,
) {
	testkit.RequireFileCheckErrorContains(t, `
...
func TestIslandFinalizationRejectsInterproceduralStructOptionalPayloadAliasDoubleFree(
	t *testing.T,
) {
	testkit.RequireFileCheckErrorContains(t, `
struct MaybeBox:
