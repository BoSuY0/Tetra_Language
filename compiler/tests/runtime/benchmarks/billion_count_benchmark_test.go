...
func TestTetraCountToOneBillion(t *testing.T) {
	...
	t.Logf(
		"Tetra counted to %d in %s (%.0f counts/s)",
		tetraBillionCountLimit,
		elapsed,
		float64(tetraBillionCountLimit)/elapsed.Seconds(),
	)
}
...
func BenchmarkTetraCountToOneBillion(b *testing.B) {
	...
	if b.N > 0 && totalRuntime > 0 {
		...
		b.ReportMetric(
			float64(tetraBillionCountLimit*int64(b.N))/totalRuntime.Seconds(),
			"counts/s",
		)
	}
}
...
func buildBillionCountProgram(tb testing.TB, tgt target.Target) string {
	...
	if _, err := compiler.BuildFileWithStatsOpt(
		...
		outPath,
		tgt.Triple,
		compiler.BuildOptions{ReleaseOptimize: true},
	); err != nil {
