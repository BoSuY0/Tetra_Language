...
func TestBuildFunctionTypedThrowingImportedPublicGlobalMutableLocalReassignmentDirectTrySmoke(
	t *testing.T,
) {
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" {
		t.Skip("linux/amd64 only")
...
func TestBuildFunctionTypedThrowingImportedPublicGlobalStructFieldReassignmentDirectTrySmoke(
	t *testing.T,
) {
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" {
		t.Skip("linux/amd64 only")
...
func TestBuildFunctionTypedThrowingImportedPublicGlobalStructFieldInitializerDirectTrySmoke(
	t *testing.T,
) {
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" {
		t.Skip("linux/amd64 only")
...
func TestBuildFunctionTypedThrowingImportedPublicGlobalEnumPayloadReassignmentDirectTrySmoke(
	t *testing.T,
) {
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" {
		t.Skip("linux/amd64 only")
...
func main() -> Int:
    ...
    return math.cb(40)
`,
			want: ("imported mutable function-typed global 'math.cb' cannot be " +
				"called directly across module boundary; cross-module mutable global-" +
				"data ABI is not available, expose a module-local function wrapper or " +
				"immutable public function-typed global"),
		},
		{
...
func main() -> Int:
    ...
    return local(40)
`,
			want: ("imported mutable function-typed global 'math.cb' cannot be used " +
				"across module boundary; cross-module mutable global-data ABI is not " +
				"available, expose a module-local function wrapper or immutable public " +
