...
func TestBuildDeterministicAcrossJobs(t *testing.T) {
	...
	if _, err := compiler.BuildFileWithStatsOpt(entry1, out1, "linux-x64", compiler.BuildOptions{Jobs: 1}); err != nil {
	...
	if _, err := compiler.BuildFileWithStatsOpt(entry2, out2, "linux-x64", compiler.BuildOptions{Jobs: 8}); err != nil {
...
func TestConcurrentBuildsSameCache(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		_, err := compiler.BuildFileWithStatsOpt(entry, out1, "linux-x64", compiler.BuildOptions{Jobs: 4})
		errs <- err
	}()
	go func() {
		defer wg.Done()
		_, err := compiler.BuildFileWithStatsOpt(entry, out2, "linux-x64", compiler.BuildOptions{Jobs: 4})
		errs <- err
	}()
	wg.Wait()
	close(errs)
...
func TestBuildCacheHitNoLowering(t *testing.T) {
	...
	stats1, err := compiler.BuildFileWithStatsOpt(entry, out, "linux-x64", compiler.BuildOptions{Jobs: 2})
	if err != nil {
	...
	stats2, err := compiler.BuildFileWithStatsOpt(entry, out, "linux-x64", compiler.BuildOptions{Jobs: 2})
	if err != nil {
