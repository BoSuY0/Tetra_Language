...
// ---- closures_budget_privacy_test.go ----

func TestCapturedFunctionTypedReturnNestedStructReassignmentCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_return_nested_struct_reassign_global_escape.tetra",
	)
	if err != nil {
...
func TestCapturedFunctionTypedReturnNestedStructFieldReassignmentCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_return_nested_struct_field_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func main() -> Int:
    ...
    return f(41)
`,
			want: ("closure capture 'y' is mutable; direct ptr closure calls " +
				"would observe mutable locals by reference, so use a " +
				"function-typed fnptr binding for by-value snapshot capture"),
		},
		{
...
func main() -> Int:
    ...
    return f(41)
`,
			want: ("closure capture 'box' has unsupported type 'PtrBox'; only " +
				"immutable local Int/Bool/String, simple struct, enum, and " +
				"optional captures without ptr/resource fields are supported " +
				"by the direct ptr-closure capture ABI"),
		},
		{
...
func main() -> Int:
    return 0
`,
			want: ("closure capture 'box' has unsupported type 'ActorBox'; only " +
				"immutable local Int/Bool/String, simple struct, enum, and " +
				"optional captures without ptr/resource fields are supported " +
				"by the direct ptr-closure capture ABI"),
		},
...
`,
			want: ("capturing closure literal captures 'y' but is not let-bound;" +
				" only let-bound local direct calls can capture immutable " +
				"Int/Bool/String values and simple structs without " +
				"ptr/resource fields under the direct ptr-closure ABI"),
		},
	}
...
func assertBudgetFailureTail(t *testing.T, fn *compiler.IRFunc, want []ir.IRInstrKind) {
	...
	if len(got) != len(want) {
		t.Fatalf(
			"%s budget failure tail length = %d, want %d: %#v",
			fn.Name,
			len(got),
			len(want),
			got,
	...
	for i, kind := range want {
		if got[i].Kind != kind {
			t.Fatalf(
				"%s budget failure tail[%d] = %v, want %v: %#v",
				fn.Name,
				i,
				got[i].Kind,
				kind,
...
func assertBudgetFailureTailImms(t *testing.T, fn *compiler.IRFunc, want []int32) {
	...
	if len(got) != len(want) {
		t.Fatalf(
			"%s budget failure const count = %d, want %d: got %v",
			fn.Name,
			len(got),
			len(want),
			got,
	...
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf(
				"%s budget failure const[%d] = %d, want %d: got %v",
				fn.Name,
				i,
				got[i],
				want[i],
...
// ---- closures_global_escape_parameter_return_test.go ----

func TestImportedCapturedFunctionTypedEnumParameterWholeReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
...
func main() -> Int:
    let base: Int = 1
` + "    let choice: callbacks.MaybeCallback = callbacks.echo(" +
			"callbacks.MaybeCallback.some(fn(x: Int) -> Int:\n" + `
        return x + base
    ))
    match choice:
...
`)
	file, err := compiler.ParseFile(
		src,
		"parameter_enum_parameter_whole_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestImportedFunctionTypedEnumParameterWholeReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported function-typed enum-parameter whole-return global escape diagnostic",
		)
	}
	want := "function-typed parameter 'f' cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedEnumParameterPayloadReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_enum_parameter_payload_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestImportedCapturedFunctionTypedEnumParameterPayloadReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured enum-parameter payload-return global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedInlineEnumPayloadReturnRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured inline-enum-parameter payload-return global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedInlineParameterReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
var cb: fn(Int) -> Int = add0
...
func TestImportedCapturedFunctionTypedParameterReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
...
func TestCapturedFunctionTypedParameterReturnedStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_returned_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestFunctionTypedParameterReturnedStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"function_typed_parameter_returned_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedInlineParameterReturnedStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_inline_parameter_returned_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured inline parameter returned-struct-field global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParameterStructFieldReturnRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured parameter returned-struct-field global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedFunctionTypedParameterReturnedStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/pack.t4": `module lib.pack
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported function-typed parameter returned-struct-field global escape diagnostic",
		)
	}
	want := "function-typed parameter 'f' cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedParameterReturnedNestedStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_returned_nested_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter returned-nested-struct-field global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestFunctionTypedParameterReturnedNestedStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"parameter_returned_nested_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected function-typed parameter returned-nested-struct-field global escape diagnostic",
		)
	}
	want := "function-typed parameter 'f' cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedInlineParamNestedStructFieldReturnRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_inline_parameter_returned_nested_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured inline parameter returned-nested-struct-field global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamNestedStructFieldReturnRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/pack.t4": `module lib.pack
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured parameter returned-nested-struct-field global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedFunctionTypedParameterReturnedNestedStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/pack.t4": `module lib.pack
...
`,
	...
	if err == nil {
		t.Fatalf(
			("expected imported function-typed parameter " +
				"returned-nested-struct-field global escape diagnostic"),
		)
	}
	want := "function-typed parameter 'f' cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedParameterAliasReturnedStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_alias_returned_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedParamFieldAliasStructFieldReturnRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_field_alias_returned_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter field-alias returned-struct-field global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedParameterReturnedEnumPayloadCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_returned_enum_payload_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestFunctionTypedParameterReturnedEnumPayloadCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"function_typed_parameter_returned_enum_payload_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedParameterAliasReturnedEnumPayloadCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_alias_returned_enum_payload_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedParamFieldAliasEnumPayloadReturnRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_field_alias_returned_enum_payload_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter field-alias returned-enum-payload global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedInlineParameterReturnedEnumPayloadCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_inline_parameter_returned_enum_payload_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured inline parameter returned-enum-payload global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamEnumPayloadReturnRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/pack.t4": `module lib.pack
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured parameter returned-enum-payload global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedFunctionTypedParameterReturnedEnumPayloadCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/pack.t4": `module lib.pack
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported function-typed parameter returned-enum-payload global escape diagnostic",
		)
	}
	want := "function-typed parameter 'f' cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedParamStructFieldReassignRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_returned_struct_field_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter returned-struct-field reassignment global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedParamEnumPayloadReassignRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_returned_enum_payload_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter returned-enum-payload reassignment global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
// ---- closures_global_escape_parameter_test.go ----
...
func TestCapturedFunctionTypedReturnStructFieldReassignmentCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_return_struct_field_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedReturnMutableLocalReassignmentCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
var cb: fn(Int) -> Int = add0
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_return_mutable_local_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedReturnEnumPayloadReassignmentCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_return_enum_payload_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedReturnedStructEnumPayloadCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_returned_struct_enum_payload_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestImportedCapturedFunctionTypedReturnedEnumPayloadCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
...
func TestCapturedParamReturnEnumPayloadReassignRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_return_enum_payload_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter-return enum payload reassignment global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedReturnWholeStructReassignmentCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_return_whole_struct_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestDirectClosureWholeStructReassignSnapshotsToGlobal(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_direct_closure_whole_struct_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestDirectClosureWholeNestedStructReassignSnapshotsToGlobal(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_direct_closure_whole_nested_struct_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
`)
	...
	want := ("function-typed parameter 'f' cannot be stored in global " +
		"function-typed value 'cb'; global escape requires a direct " +
...
func TestFunctionTypedParameterMutableLocalReassignmentCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
var cb: fn(Int) -> Int = add0
...
`)
	file, err := compiler.ParseFile(
		src,
		"function_typed_parameter_mutable_local_reassignment_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected function-typed parameter mutable-local-reassignment global escape diagnostic",
		)
	}
	want := "function-typed parameter 'f' cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
`)
	file, err := compiler.ParseFile(
		src,
		"function_typed_parameter_struct_field_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
`)
	file, err := compiler.ParseFile(
		src,
		"function_typed_parameter_enum_payload_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedParameterAliasReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
var cb: fn(Int) -> Int = add0
...
`)
	file, err := compiler.ParseFile(
		src,
		"function_typed_parameter_alias_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedParameterFieldReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"function_typed_parameter_field_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedParameterReassignedFieldReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_reassigned_field_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedParameterEnumPayloadReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_enum_payload_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedParamReassignedEnumPayloadReturnRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_reassigned_enum_payload_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter reassigned-enum-payload-return global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedStructParameterFieldReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_struct_parameter_field_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestImportedCapturedFunctionTypedStructParameterFieldReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured struct-parameter field-return global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedInlineStructParamFieldReturnRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured inline-struct-parameter field-return global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedNestedStructParamFieldReturnRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured nested-struct-parameter field-return global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedFunctionTypedStructParameterWholeReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
func main() -> Int:
    let base: Int = 1
` + "    let box: callbacks.Box = callbacks.echo(" +
			"callbacks.Box(holder: callbacks.Holder(cb: fn(x: Int) -> Int:\n" + `
        return x + base
    )))
    cb = box.holder.cb
    return 0
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured struct-parameter whole-return global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
`)
	file, err := compiler.ParseFile(
		src,
		"parameter_struct_parameter_whole_return_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestImportedFunctionTypedStructParameterWholeReturnCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported function-typed struct-parameter whole-return global escape diagnostic",
		)
	}
	want := "function-typed parameter 'f' cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
// ---- closures_global_escape_return_test.go ----
...
`)
	...
	want := ("global-escaped function value captures mutable local " +
		"'total'; mutable by-reference captures require a proven " +
		"lifetime and synchronization model")
	if !strings.Contains(err.Error(), want) {
		t.Fatalf("error = %v, want substring %q", err, want)
	}
}
...
func TestCapturedFunctionTypedDirectClosureWholeEnumReassignmentCanSnapshotIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_direct_closure_whole_enum_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedFunctionTypedReturnCallStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	...
	want := ("captured function value cannot be stored in global " +
		"function-typed value 'cb'; global escape requires a direct " +
		"fnptr snapshot with known captures and bounded environment " +
		"slots")
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedFunctionTypedReturnCallEnumPayloadCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
enum MaybeCallback:
...
func TestCapturedFunctionTypedParameterReturnLocalAliasCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	src := []byte(`
var cb: fn(Int) -> Int = add0
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_return_local_alias_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
...
func TestCapturedParamReturnMutableLocalReassignRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
var cb: fn(Int) -> Int = add0
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_return_mutable_local_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter-return mutable-local-reassignment global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestCapturedParamReturnStructFieldReassignRejectsGlobalEscape(
	t *testing.T,
) {
	src := []byte(`
struct Holder:
...
`)
	file, err := compiler.ParseFile(
		src,
		"captured_parameter_return_struct_field_reassign_global_escape.tetra",
	)
	if err != nil {
		t.Fatalf("ParseFile: %v", err)
	}
	...
	if err == nil {
		t.Fatalf(
			"expected captured parameter-return struct-field-reassignment global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedFunctionTypedParameterReturnLocalAliasCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
func TestImportedCapturedParamReturnMutableLocalReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			("expected imported captured parameter-return " +
				"mutable-local-reassignment global escape diagnostic"),
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedFunctionTypedParameterReturnStructFieldCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured parameter-return struct-field global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamReturnStructFieldReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured parameter-return struct-field-reassignment global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamReturnNestedStructFieldReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			("expected imported captured parameter-return " +
				"nested-struct-field-reassignment global escape diagnostic"),
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamReturnWholeStructReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured parameter-return whole-struct-reassignment global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamReturnStructValuedFieldReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			("expected imported captured parameter-return " +
				"struct-valued-field-reassignment global escape diagnostic"),
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamReturnWholeNestedStructReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			("expected imported captured parameter-return " +
				"whole-nested-struct-reassignment global escape diagnostic"),
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedFunctionTypedParameterReturnEnumPayloadCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured parameter-return enum-payload global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamReturnEnumPayloadReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured parameter-return enum-payload-reassignment global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedParamReturnStructEnumPayloadReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			("expected imported captured parameter-return struct-field " +
				"enum-payload reassignment global escape diagnostic"),
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedFunctionTypedReturnedStructEnumPayloadCannotEscapeIntoGlobalFunctionValue(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
func TestImportedCapturedImportedStructEnumPayloadReturnRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured imported-returned-struct enum-payload global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedStructEnumPayloadWholeStructReassignRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			("expected imported captured returned-struct enum-payload " +
				"whole-struct-reassignment global escape diagnostic"),
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
func TestImportedCapturedNestedStructEnumPayloadReturnRejectsGlobalEscape(
	t *testing.T,
) {
	files := map[string]string{
		"lib/callbacks.t4": `module lib.callbacks
...
`,
	...
	if err == nil {
		t.Fatalf(
			"expected imported captured nested-returned-struct enum-payload global escape diagnostic",
		)
	}
	want := "captured function value cannot be stored in global function-typed value 'cb'"
	if !strings.Contains(err.Error(), want) {
...
// ---- closures_semantic_clauses_test.go ----
...
func main() -> Int:
    ...
    return id(1)
`,
			want: ("generic closure 'id' requires the generic direct-call " +
...
func main() -> Int:
    ...
    return 0
`,
			want: ("generic closure literal captures local 'base'; generic " +
...
func pick() -> fn(Int) -> Int:
    ...
    return id
`,
			want: ("generic function symbol 'id' cannot be returned as " +
...
func pick() -> fn(Int) -> Int:
    return 1
`,
			want: ("function-typed return must use a supported fnptr source: " +
...
func main() -> Int:
    ...
    return p(41)
`,
			want: ("function value 'p' cannot be called through the supported " +
...
func main() -> Int:
    ...
    return 0
`,
			want: ("function-typed local 'f' initializer must be a " +
...
func main() -> Int:
    ...
    return f(41)
`,
			want: ("generic function symbol 'id' cannot initialize " +
				"function-typed local 'f'; local fnptr ABI requires a " +
				"monomorphic target"),
		},
		{
...
func main() -> Int:
    return 0
`,
			want: ("callback argument for 'apply' has no known fnptr target " +
				"under semantic clause 'noalloc'; pass a direct named " +
...
`)
	...
	if string(sig.ReturnFunctionEscapeKind) != "heap" || !sig.ReturnFunctionHandleValue {
		t.Fatalf(
			"return function escape metadata = (%q, %v), want (heap, true)",
			sig.ReturnFunctionEscapeKind,
			sig.ReturnFunctionHandleValue,
		)
	}
	mainIndex := -1
	...
	if string(cbInfo.FunctionEscapeKind) != "heap" || !cbInfo.FunctionHandleValue {
		t.Fatalf(
			"cb escape metadata = (%q, %v), want (heap, true)",
			cbInfo.FunctionEscapeKind,
			cbInfo.FunctionHandleValue,
		)
	}
}
...
`)
	...
	if string(cbInfo.FunctionEscapeKind) != "heap" || !cbInfo.FunctionHandleValue {
		t.Fatalf(
			"holder.cb escape metadata = (%q, %v), want (heap, true)",
			cbInfo.FunctionEscapeKind,
			cbInfo.FunctionHandleValue,
		)
	}
...
`)
	...
	if string(payloadInfo.FunctionEscapeKind) != "heap" || !payloadInfo.FunctionHandleValue {
		t.Fatalf(
			"choice payload escape metadata = (%q, %v), want (heap, true)",
			payloadInfo.FunctionEscapeKind,
...
// ---- inlining_specialization_test.go ----
...
`)
	...
	if afterCmps := countIRKind(after, ir.IRCmpEqI32); afterCmps >= beforeCmps {
		t.Fatalf(
			"known enum case discriminator compare was not folded: before=%d after=%d instrs=%#v",
			beforeCmps,
			afterCmps,
			after.Instrs,
		)
...
`)
	...
	if !hasPassDecision(
		...
		t.Fatalf(
			"missing proven-some optional branch fold decision: %#v",
			report.Passes[0].Decisions,
		)
	}
	after := findIRFunc(t, irProg.Funcs, "main")
	if afterBranches := countIRKind(after, ir.IRJmpIfZero); afterBranches >= beforeBranches {
		t.Fatalf(
			"proven-some optional branch was not folded: before=%d after=%d instrs=%#v",
			beforeBranches,
			afterBranches,
			after.Instrs,
		)
...
`)
	...
	if calls := countIRCallName(after, "Vec2.sum"); calls != 0 {
		t.Fatalf(
			"static extension call was not inlined: remaining=%d instrs=%#v",
			calls,
			after.Instrs,
		)
	}
...
`)
	...
	if calls := countIRCallName(before, "Vec2.draw"); calls != 1 {
		t.Fatalf(
			"pre-optimization main calls Vec2.draw %d times, want 1: %#v",
			calls,
			before.Instrs,
		)
	}
	...
	if !hasPassDecisionCallee(
		...
		t.Fatalf(
			"missing static protocol conformance inline decision: %#v",
			report.Passes[0].Decisions,
		)
	}
	after := findIRFunc(t, irProg.Funcs, "main")
	if calls := countIRCallName(after, "Vec2.draw"); calls != 0 {
		t.Fatalf(
			"static protocol conformance call was not inlined: remaining=%d instrs=%#v",
			calls,
			after.Instrs,
		)
	}
...
func TestBuildCacheHitNoLowering(t *testing.T) {
	...
	stats1, err := compiler.BuildFileWithStatsOpt(
		entry,
		out,
		"linux-x64",
		compiler.BuildOptions{Jobs: 2},
	)
	if err != nil {
	...
	stats2, err := compiler.BuildFileWithStatsOpt(
		entry,
		out,
		"linux-x64",
		compiler.BuildOptions{Jobs: 2},
