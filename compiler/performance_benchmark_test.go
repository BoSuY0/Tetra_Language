	...
	for name, src := range examples {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				if err := BuildFile(src, out, "linux-x64"); err != nil {
					b.Fatalf("BuildFile: %v", err)
				}
			}
		})
	}
...
func BenchmarkFormatRepresentativeSources(b *testing.B) {
	...
	for name, path := range sources {
		...
		b.Run(name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				if _, err := FormatSource(raw, path); err != nil {
					b.Fatalf("FormatSource: %v", err)
				}
			}
		})
	}
...
func BenchmarkGenerateAPIDocsDogfoodProjects(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		if _, err := GenerateAPIDocs(paths); err != nil {
			b.Fatalf("GenerateAPIDocs: %v", err)
		}
	}
}
	...
	for _, tc := range cases {
		b.Run(tc.name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				if err := BuildFile(tc.src, out, tc.target); err != nil {
					b.Fatalf("BuildFile: %v", err)
				}
