...
func TestEmitLibraryAllowsNoMainAndWritesTOBJ(t *testing.T) {
	...
	if obj.CompilerVersion != Version() {
		t.Fatalf("compiler version = %q, want %q", obj.CompilerVersion, Version())
	}
...
`)
	...
	wantHash, err := InterfaceFingerprintFromSource(src, srcPath)
	if err != nil {
	...
	if !objectHasSymbol(obj, "math.core.add") {
...
func TestEmitLibraryBuildOnlyAcrossNativeTargets(t *testing.T) {
	...
	for _, target := range targets {
		t.Run(target, func(t *testing.T) {
			...
			if _, err := BuildFileWithStatsOpt(srcPath, outPath, target, BuildOptions{Emit: EmitLibrary}); err != nil {
