...
uses mem, io:
    return v.x + v.y
`)
	got := AnalyzeLSPSource(src, "vec.tetra")
...
func TestAnalyzeLSPSourceSemanticDiagnostics(t *testing.T) {
	got := AnalyzeLSPSource([]byte(`func main() -> Int:
    print("missing uses\n")
    return 0
`), "bad_semantic.tetra")
	if len(got.Diagnostics) != 1 {
	...
	if diag.Code != "TETRA2001" || diag.File != "bad_semantic.tetra" || diag.Line != 2 {
...
func TestAnalyzeLSPSourceImportedFileDoesNotReportMissingWorkspace(t *testing.T) {
	got := AnalyzeLSPSource([]byte(`import lib.core.math as math
...
`), 0o644); err != nil {
	...
	got, err := AnalyzeLSPFile(mainPath)
	if err != nil {
