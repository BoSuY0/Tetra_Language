...
func FeatureRegistry() []FeatureInfo {
	features := []FeatureInfo{
		...
		},
		{
			...
			Status:    FeatureStatusCurrent,
			Since:     "v0.2.0",
			Scope:     "conservative borrow/inout/consume marker checks for local calls, aliasing, use-after-consume, and borrow escape diagnostics",
		...
		},
		{
			...
			Status:    FeatureStatusCurrent,
			Since:     "v0.2.0",
			Scope:     "conservative resource finalization checks for task handles, task groups, island handles, region-backed slices, and structs containing them, including double-use and ambiguous provenance diagnostics",
		...
		},
		{
			...
			Status:    FeatureStatusCurrent,
			Since:     "v0.2.0",
			Scope:     "conservative actor/task ownership transfer checks for worker entrypoints, sendable results, handle transfer, and use-after-transfer diagnostics",
			Stability: "supported conservative local MVP; distributed actors, full race-safety proofs, cancellation semantics, and structured concurrency remain outside the current support claim",
		...
		},
		{
			...
			Status:    FeatureStatusPlanned,
			Scope:     "planned full SSA lifetime analysis for richer interprocedural lifetime proofs, precise merge reasoning, and broader alias modeling",
			Stability: "planned only; no current v0.2.0 support guarantee and not required for the conservative ownership/resource MVP",
			Docs:      []string{"docs/spec/current_supported_surface.md", "docs/spec/ownership_v1.md", "docs/spec/v1_scope.md"},
		},
		{
