...
func main() {
	...
	allocReportPath := flag.String(
		"alloc-report",
		"",
		"optional path to paired allocation report for lowered_artifact_id validation",
	)
	flag.Parse()
	if *reportPath == "" {
...
func readAllocationReport(path string) (allocationPlanReport, error) {
	...
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		...
		return allocationPlanReport{}, fmt.Errorf(
			"trailing data after allocation report JSON: %w",
			err,
		)
	}
	return report, nil
...
func validateLoweredArtifactsAgainstAllocationReport(
	report memoryReport,
	allocReport allocationPlanReport,
) error {
	var issues []string
	if allocReport.SchemaVersion != 2 || allocReport.Kind != "allocation_plan" {
		issues = append(
			issues,
			fmt.Sprintf(
				"allocation report schema_version/kind = %d/%q, want 2/allocation_plan",
				allocReport.SchemaVersion,
				allocReport.Kind,
			),
		)
	}
	...
	for _, fn := range allocReport.Functions {
		for _, alloc := range fn.Allocations {
			if strings.TrimSpace(fn.Name) == "" || strings.TrimSpace(alloc.ID) == "" ||
				strings.TrimSpace(alloc.ActualLoweringStorage) == "" {
				continue
			}
			issues = append(
				issues,
				validateAllocationArtifactContract(
					fmt.Sprintf("allocation report %s/%s", fn.Name, alloc.ID),
					alloc,
				)...)
			artifact := allocationArtifact{functionName: fn.Name, allocation: alloc}
			if id := strings.TrimSpace(alloc.LoweredArtifactID); id != "" {
				artifacts[id] = artifact
			}
			legacyID := fmt.Sprintf("ir:%s:%s:%s", fn.Name, alloc.ID, alloc.ActualLoweringStorage)
			artifacts[legacyID] = artifact
		}
	}
	for i, row := range report.Rows {
		if !memoryfacts.RowRequiresArtifact(row.PlannedStorage, row.ActualLoweringStorage, row.Claim) || strings.TrimSpace(row.LoweredArtifactID) == "" {
			continue
		}
		...
		if !ok {
			issues = append(
				issues,
				fmt.Sprintf(
					"row %d: lowered_artifact_id %q is not present in allocation report",
					i,
...
func validateAllocationArtifactContract(prefix string, alloc allocationReportAllocation) []string {
	...
	if builtin == "" {
		issues = append(
			issues,
			fmt.Sprintf("%s: builtin is required for allocation length contract evidence", prefix),
		)
	}
	if lengthStatus == "" {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: length_status is required for allocation length contract evidence",
				prefix,
			),
		)
	...
	if zeroGuard == "" {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: zero_guard_status is required for allocation length contract evidence",
				prefix,
			),
		)
	...
	if negativeGuard == "" {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: negative_guard_status is required for allocation length contract evidence",
				prefix,
			),
		)
		...
		issues = append(
			issues,
			fmt.Sprintf("%s: unknown negative_guard_status %q", prefix, negativeGuard),
		)
	}
	if overflowGuard == "" {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: overflow_guard_status is required for allocation length contract evidence",
				prefix,
			),
		)
		...
		issues = append(
			issues,
			fmt.Sprintf("%s: unknown overflow_guard_status %q", prefix, overflowGuard),
		)
	}
	if allocationArtifactUsesHeap(alloc) {
		if len(alloc.HeapReasonCodes) == 0 {
			issues = append(
				issues,
				fmt.Sprintf("%s: heap allocation requires heap_reason_codes", prefix),
			)
		}
		issues = append(
			issues,
			validateAllocationReasonCodes(prefix, alloc.ReasonCodes, alloc.HeapReasonCodes)...)
	...
	if zeroGuard != wantZero {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: builtin %q zero_guard_status %q does not match contract %q",
				prefix,
				builtin,
				zeroGuard,
				wantZero,
	...
	if negativeGuard != wantNegative {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: builtin %q negative_guard_status %q does not match contract %q",
				prefix,
				builtin,
				negativeGuard,
				wantNegative,
	...
	if overflowGuard != wantOverflow {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: builtin %q overflow_guard_status %q does not match contract %q",
				prefix,
				builtin,
				overflowGuard,
				wantOverflow,
...
func validateAllocationReasonCodes(
	prefix string,
	reasonCodes []string,
	...
	for _, code := range heapReasonCodes {
		if strings.TrimSpace(code) == "" {
			issues = append(
				issues,
				fmt.Sprintf("%s: heap_reason_codes contains an empty entry", prefix),
			)
			continue
		}
		if strings.TrimSpace(code) != code {
...
func allocationGuardContractForBuiltin(
	builtin string,
) (zero string, negative string, overflow string, ok bool) {
	switch {
...
func validateRowAgainstAllocationArtifact(
	index int,
	row memoryReportRow,
	...
	if row.FunctionID != "" && row.FunctionID != artifact.functionName {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: function_id %q does not match allocation report function %q for lowered_artifact_id %q",
				prefix,
				row.FunctionID,
	...
	if shouldCompareAllocationSiteID(row.AllocationSiteID) && row.AllocationSiteID != alloc.ID {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: allocation_site_id %q does not match allocation report id %q for lowered_artifact_id %q",
				prefix,
	...
	if shouldCompareReportSiteID(row.SiteID) && alloc.SiteID != "" && row.SiteID != alloc.SiteID {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: site_id %q does not match allocation report site_id %q for lowered_artifact_id %q",
				prefix,
				row.SiteID,
				alloc.SiteID,
				row.LoweredArtifactID,
	...
	if row.PlannedStorage != "" && row.PlannedStorage != alloc.PlannedStorage {
		issues = append(
			issues,
			fmt.Sprintf(
				("%s: planned_storage %q does not match allocation report planned_"+
					"storage %q for lowered_artifact_id %q"),
				prefix,
				row.PlannedStorage,
	...
	if row.ActualLoweringStorage != "" && row.ActualLoweringStorage != alloc.ActualLoweringStorage {
		issues = append(
			issues,
			fmt.Sprintf(
				("%s: actual_lowering_storage %q does not match allocation report "+
					"actual_lowering_storage %q for lowered_artifact_id %q"),
				prefix,
...
func shouldCompareAllocationSiteID(id string) bool {
	id = strings.TrimSpace(id)
	return id != "" && !strings.HasPrefix(id, "core.")
}

func shouldCompareReportSiteID(id string) bool {
	return strings.HasPrefix(strings.TrimSpace(id), "allocsite:")
}
...
func firstNonDeterministicReportRow(
	rows []memoryReportRow,
) (int, memoryReportRow, memoryReportRow, bool) {
...
func validateRow(index int, row memoryReportRow) []string {
	...
	if memoryfacts.DynamicRawRuntimeCheckCostDisallowed(row.Claim, row.CostClass) {
		issues = append(
			issues,
			fmt.Sprintf(
				"%s: dynamic raw check claim %q must use dynamic_check_required, got %s",
				prefix,
				row.Claim,
				row.CostClass,
			),
	...
	if memoryfacts.ZeroCostProvenClaimDisallowed(
		row.Claim,
		row.CostClass,
		row.ClaimLevel,
		row.PlannedStorage,
	...
	if row.ClaimLevel == "validated" &&
		memoryfacts.IslandKernelClaimValidatorMismatch(row.Claim, row.ValidatorName) {
		issues = append(
			issues,
			fmt.Sprintf(
				...
				prefix,
				row.Claim,
				memoryfacts.RequiredIslandKernelClaimValidator(row.Claim),
			),
		)
	}
	...
	if unsafeUnknownRow(row) && memoryfacts.UnsafeUnknownOptimizationClaim(row.Claim, row.AliasState) {
		issues = append(
			issues,
			fmt.Sprintf(
	...
	if memoryfacts.BroadNoAliasClaim(row.Claim) {
		issues = append(
			issues,
			fmt.Sprintf("%s: broad noalias claim %q is outside Memory Ideal v0", prefix, row.Claim),
		)
	}
	if row.ClaimLevel == "validated" && memoryfacts.ConservativeNoAliasBoundaryClaim(row.Claim) {
		issues = append(
			issues,
			fmt.Sprintf(
	...
	if row.CostClass == "dynamic_check_required" &&
		memoryfacts.MemoryOptimizationClaim(row.Claim, row.AliasState) &&
	...
	if memoryfacts.BareBoundsCheckEliminatedClaim(row.Claim) {
		issues = append(
			issues,
			fmt.Sprintf(
	...
	if memoryfacts.UnsafeVerifiedRootDisallowedClaim(row.ProvenanceClass, row.UnsafeClass, row.Claim) {
		issues = append(
			issues,
			fmt.Sprintf(
	...
	if memoryfacts.UnsafeCheckedDisallowedClaim(row.ProvenanceClass, row.UnsafeClass, row.Claim) {
		issues = append(
			issues,
			fmt.Sprintf(
	...
	if memoryfacts.CapMemDisallowedProofClaim(row.Claim, row.ValidatorName, row.Reason) {
		issues = append(
			issues,
			fmt.Sprintf(
	...
	if row.ClaimLevel == "validated" &&
		memoryfacts.UnsafeExternalRootTrustedStorage(
			row.ProvenanceClass,
			row.UnsafeClass,
			row.PlannedStorage,
			row.ActualLoweringStorage,
	...
	if row.ClaimLevel == "validated" && strings.Contains(row.Claim, "no_alias") &&
	...
	if memoryfacts.RowRequiresArtifact(row.PlannedStorage, row.ActualLoweringStorage, row.Claim) && row.LoweredArtifactID == "" {
		issues = append(issues, prefix+": storage/lowering claim requires lowered_artifact_id")
	}
	if row.ClaimLevel == "validated" &&
		memoryfacts.ValidatedTrustedStorageHeapFallback(row.PlannedStorage, row.ActualLoweringStorage) {
		issues = append(
			issues,
	...
	if row.ClaimLevel == "validated" &&
		memoryfacts.RuntimeProofRequiredStorage(row.PlannedStorage, row.ActualLoweringStorage) {
		issues = append(
			issues,
			fmt.Sprintf(
...
func knownSourceStage(value string) bool {
	return memoryfacts.KnownSourceStage(value)
}

func knownProvenance(value string) bool {
	return memoryfacts.KnownProvenanceClass(value)
}

func safeProvenance(value string) bool {
	return memoryfacts.SafeProvenanceClass(value)
}

func unsafeUnknownRow(row memoryReportRow) bool {
	return memoryfacts.UnsafeUnknownRow(row.ProvenanceClass, row.UnsafeClass)
}

func claimRequiresParentFactID(claim string) bool {
	return memoryfacts.ClaimRequiresParentFactID(claim)
}

func knownCostClass(value string) bool {
	return memoryfacts.KnownCostClass(value)
}

func knownReportClaim(value string) bool {
	return memoryfacts.KnownReportClaim(value)
}

func knownStorage(value string) bool {
	return memoryfacts.KnownStorageClass(value)
}

func knownAliasState(value string) bool {
	return memoryfacts.KnownAliasState(value)
}

func validatedNoAliasState(value string) bool {
	return memoryfacts.ValidatedNoAliasState(value)
}

func knownUnsafe(value string) bool {
	return memoryfacts.KnownUnsafeClass(value)
}

func knownClaimLevel(value string) bool {
	return memoryfacts.KnownClaimLevel(value)
}

func knownValidatorStatus(value string) bool {
	return memoryfacts.KnownValidatorStatus(value)
}

func islandBackedReportRow(row memoryReportRow) bool {
	return strings.TrimSpace(row.IslandID) != "" ||
		row.PlannedStorage == string(memoryfacts.StorageExplicitIsland) ||
		row.ActualLoweringStorage == string(memoryfacts.StorageExplicitIsland)
}
