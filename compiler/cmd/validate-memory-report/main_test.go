...
func TestValidateMemoryReportWithAllocReportAcceptsExplicitLoweredArtifactID(t *testing.T) {
	dir := t.TempDir()
	reportPath := filepath.Join(dir, "memory-report.json")
	const explicitID = "ir:main:40:40:alloc_bytes"
	raw := strings.Replace(
		validSchemaV1MemoryReport(),
		`"lowered_artifact_id": "ir:main:alloc_bytes:0"`,
		`"lowered_artifact_id": "`+explicitID+`"`,
		1,
	)
	if err := os.WriteFile(reportPath, []byte(raw), 0o644); err != nil {
		t.Fatal(err)
...
func TestValidateMemoryReportWithAllocReportAllowsLoweringStyleSiteIDs(t *testing.T) {
	dir := t.TempDir()
	reportPath := filepath.Join(dir, "memory-report.json")
	const explicitID = "ir:main:40:40:alloc_bytes"
	raw := strings.Replace(
		...
		`"site_id": "`+explicitID+`",
      "allocation_site_id": "core.alloc_bytes",`,
		1,
	)
	raw = strings.Replace(
		raw,
