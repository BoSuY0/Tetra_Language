...
func TestValidateMemoryReportVocabularyMatchesRegistry(t *testing.T) {
	for _, stage := range memoryfacts.SourceStages() {
		if !knownSourceStage(stage) {
			t.Fatalf("CLI validator rejects registered source stage %q", stage)
		}
	}
	for _, class := range memoryfacts.CostClasses() {
		if !knownCostClass(class) {
			t.Fatalf("CLI validator rejects registered cost class %q", class)
		}
	}
	for _, level := range memoryfacts.ClaimLevels() {
		if !knownClaimLevel(level) {
			t.Fatalf("CLI validator rejects registered claim level %q", level)
		}
	}
	for _, claim := range memoryfacts.ReportClaims() {
		if !knownReportClaim(claim) {
			t.Fatalf("CLI validator rejects registered report claim %q", claim)
		}
	}
	for _, claim := range memoryfacts.ParentRequiredClaims() {
		if !claimRequiresParentFactID(claim) {
			t.Fatalf(
				"CLI validator does not require parent_fact_id for registered derived claim %q",
				claim,
			)
		}
	}
	if !memoryfacts.UnsafeUnknownOptimizationClaim(memoryfacts.ClaimBoundsCheckEliminated, "") {
		t.Fatalf(
			("CLI validator should classify bounds_check_eliminated as " +
				"registered unsafe_unknown optimization"),
		)
	}
	if !memoryfacts.UnsafeCheckedDisallowedClaim(
		string(memoryfacts.ProvenanceUnsafeChecked),
		string(memoryfacts.UnsafeChecked),
		memoryfacts.ClaimNoAlias,
	) {
	...
	if memoryfacts.UnsafeCheckedDisallowedClaim(
		string(memoryfacts.ProvenanceUnsafeChecked),
		string(memoryfacts.UnsafeChecked),
		memoryfacts.ClaimRawBoundsRuntimeCheckNormalBuild,
	) {
		t.Fatalf(
			"CLI validator should allow checked raw bounds runtime evidence through shared vocabulary",
		)
	}
}
...
func TestValidateMemoryReportRejectsIslandProofClaimWithoutIslandVerifier(t *testing.T) {
	...
	raw := strings.Replace(
		validSchemaV1MemoryReport(),
		`"claim": "allocation_base_metadata"`,
		`"claim": "`+memoryfacts.ClaimIslandProofVerified+`"`,
		1,
	)
	raw = strings.Replace(
		raw,
	...
	if got := err.Error(); !strings.Contains(got, memoryfacts.ClaimIslandProofVerified) ||
