...
const (
	abiVerificationSchemaV1  = "tetra.abi.verification.v1"
	abiVerificationScopeP211 = "p21.1_abi_verification"
)

const (
	abiVerificationTaskCorpus           = "abi_test_corpus"
	abiVerificationTaskAggregateReturns = "struct_enum_slice_string_return_validation"
	abiVerificationTaskCallBoundary     = "call_boundary_validation"
	abiVerificationTaskFFIReprC         = "ffi_repr_c_tests"
)

type ABIVerificationReport struct {
	Schema    string
	Scope     string
	Claims    []string
	NonClaims []string
	Targets   []ABIVerificationTargetRow
	Tasks     []ABIVerificationTaskRow
}

type ABIVerificationTargetRow struct {
	Target       string
	ABI          string
	Status       string
	TaskCoverage []string
...
type ABIVerificationTaskRow struct {
	ID       string
	Name     string
	Targets  []string
	Evidence []string
...
func BuildP21ABIVerificationReport() ABIVerificationReport {
	targets := p21ABIVerificationTargets()
	tasks := p21ABIVerificationTaskIDs()
	return ABIVerificationReport{
		Schema: abiVerificationSchemaV1,
		Scope:  abiVerificationScopeP211,
		Claims: []string{
		...
		Tasks: []ABIVerificationTaskRow{
			...
			{
				ID:       abiVerificationTaskCallBoundary,
			...
			{
				ID:       abiVerificationTaskFFIReprC,
...
func ValidateP21ABIVerificationReport(report ABIVerificationReport) error {
	if report.Schema != abiVerificationSchemaV1 {
		return fmt.Errorf("ABI verification report schema = %q, want %q", report.Schema, abiVerificationSchemaV1)
	}
	if report.Scope != abiVerificationScopeP211 {
		return fmt.Errorf("ABI verification report scope = %q, want %q", report.Scope, abiVerificationScopeP211)
	}
	if err := validateABIStrings("claim", report.Claims, true); err != nil {
		return err
	}
	...
	for _, row := range report.Targets {
		...
		if err := validateABIStrings("target "+row.Target+" evidence", row.Evidence, false); err != nil {
			return err
		}
		if err := validateABIStrings("target "+row.Target+" claim", row.Claims, true); err != nil {
			return err
		}
	...
	for _, target := range p21ABIVerificationTargets() {
	...
	for _, row := range report.Tasks {
		...
		if err := validateABIStrings("task "+row.ID+" evidence", row.Evidence, false); err != nil {
			return err
		}
		for _, target := range p21ABIVerificationTargets() {
	...
	for _, nonClaim := range p21ABIVerificationNonClaims() {
		if !stringSliceHas(report.NonClaims, nonClaim) {
			return fmt.Errorf("ABI verification report missing non-claim %q", nonClaim)
		}
	}
	if err := validateABIStrings("non-claim", report.NonClaims, false); err != nil {
		return err
	}
	return nil
}
...
func p21ABIVerificationTaskIDs() []string {
	return []string{
		abiVerificationTaskCorpus,
		abiVerificationTaskAggregateReturns,
		abiVerificationTaskCallBoundary,
		abiVerificationTaskFFIReprC,
	}
}
...
func validateABIStrings(label string, values []string, rejectBroadClaims bool) error {
	if len(values) == 0 {
