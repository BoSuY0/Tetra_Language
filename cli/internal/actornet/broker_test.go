...
func TestBrokerRoutesFramesBetweenLoopbackNodesAndWritesReport(t *testing.T) {
	...
	if report.Runtime != "actornet" || report.Transport != "loopback-tcp" {
		t.Fatalf(
			"report identity = %q/%q, want actornet/loopback-tcp",
			report.Runtime,
			report.Transport,
		)
	}
	if report.RoutedFrames != 1 || report.AcceptedConnections != 2 {
		t.Fatalf(
			"report counts = routed %d accepted %d, want 1/2",
			report.RoutedFrames,
...
func TestBrokerMissingDestinationNodeDownDoesNotRetryOrReconnect(t *testing.T) {
	...
	if report.AcceptedConnections != 1 {
		t.Fatalf(
			"AcceptedConnections = %d, want one connection and no reconnect",
			report.AcceptedConnections,
		)
	}
	...
	if report.RoutedFrames != 0 {
		t.Fatalf(
			"RoutedFrames = %d, want missing destination to stay status/failure evidence only",
			report.RoutedFrames,
		)
	}
}
...
func TestBrokerRejectsDuplicateNodeWithFrameError(t *testing.T) {
	...
	if got := readTestFrame(t, primary); got.Type != actorwire.FrameHelloAck ||
...
func TestBrokerRejectsForgedSourceNode(t *testing.T) {
	...
	if got := readTestFrame(t, source); got.Type != actorwire.FrameHelloAck ||
		got.Status != actorwire.StatusOK {
	...
	if got := readTestFrame(t, dest); got.Type != actorwire.FrameHelloAck ||
		got.Status != actorwire.StatusOK {
	...
	if got.Type != actorwire.FrameError ||
		...
		t.Fatalf(
			"forged-source response = %+v, want frame_error decode_error for registered node",
			got,
		)
	}
...
func TestBrokerCloseReopenWithoutGoroutineLeak(t *testing.T) {
	...
	for cycle := 0; cycle < cycles; cycle++ {
		...
		if got := readTestFrame(t, node); got.Type != actorwire.FrameHelloAck ||
			got.DestNodeID != 1 {
			_ = node.Close()
			_ = broker.Close()
