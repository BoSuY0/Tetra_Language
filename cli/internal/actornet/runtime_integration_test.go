...
`), 0o644); err != nil {
	...
	case result := <-done:
		...
		if result.exitCode != 0 {
			t.Fatalf("runtime smoke exit code = %d output=%q, want remote recv not to underflow mailbox count", result.exitCode, result.output)
		}
	case <-ctx.Done():
		t.Fatalf("runtime smoke timed out")
	}
...
`), 0o644); err != nil {
	...
	case result := <-done:
		...
		if result.exitCode != 0 {
			t.Fatalf("runtime smoke exit code = %d output=%q, want inbound overflow to surface as node-down status", result.exitCode, result.output)
		}
	case <-ctx.Done():
		t.Fatalf("runtime smoke timed out")
	}
