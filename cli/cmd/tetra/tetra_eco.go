...
func runEco(args []string, stdout io.Writer, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(
			stderr,
			("usage: tetra eco " +
				"<verify|artifacts|pack|unpack|vault|seed|needmap|trust|materialize|publi" +
				"sh|download|tetrahub> [options]"),
		)
		return 2
	}
	if isHelpArgs(args) {
		...
		fmt.Fprintln(
			stdout,
			("lock generation/validation is available through workflows such " +
				"as: tetra eco verify --lock <path> <Capsule.t4>"),
		)
		return 0
	}
	switch args[0] {
...
func runEcoVerify(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	lockFormat := fs.String(
		"lock-format",
		outputformat.JSON,
		"lock output format: json, toon, or both",
	)
...
func runEcoArtifactsBuild(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	checkOnly := fs.Bool(
		"check",
		false,
		"dry-run and report pending artifact changes without writing files",
	)
	allTargets := fs.Bool(
		"all-targets",
		false,
		"build artifacts for every native target listed in Capsule.t4",
	)
...
func runEcoArtifactsCheck(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	allTargets := fs.Bool(
		"all-targets",
		false,
		"check artifacts for every native target listed in Capsule.t4",
	)
	if err := fs.Parse(args); err != nil {
		if err == flag.ErrHelp {
			return 0
...
func buildCapsuleArtifacts(capsulePath string, opt capsuleArtifactBuildOptions) error {
	...
	if err := appendGeneratedArtifactsToCapsule(
		rootManifest.Path,
		rootManifest.Artifacts,
		generated,
	); err != nil {
		return err
	}
...
func resolveArtifactBuildTargets(
...
func resolveDeclaredArtifactTargets(
	manifest capsuleManifest,
	targetFlag string,
	allTargets bool,
) ([]string, error) {
...
func resolveNativeCapsuleTargets(
	manifest capsuleManifest,
	opt nativeTargetResolutionOptions,
) ([]string, error) {
	if opt.TargetFlag != "" {
	...
	if opt.AllTargets {
		...
		if opt.RequireAny && len(targets) == 0 {
			return nil, fmt.Errorf(
				"%s --all-targets found no native targets in Capsule.t4",
				opt.Command,
			)
		}
		return targets, nil
...
func resolveNativeCapsuleTarget(raw string, opt nativeTargetResolutionOptions) ([]string, error) {
	...
	if isWASMTargetTriple(target) {
		if opt.RejectWASM {
			return nil, fmt.Errorf(
				"%s requires a native object target, got %s",
				opt.Command,
				target,
			)
		}
...
func planCapsuleArtifacts(
	capsulePath string,
	targetFlag string,
	allTargets bool,
) (capsuleArtifactPlan, error) {
	manifests, err := parseCapsuleGraphArgs([]string{capsulePath})
	if err != nil {
...
func planDeclaredCapsuleArtifacts(
	capsulePath string,
	targetFlag string,
	allTargets bool,
) (capsuleArtifactPlan, error) {
...
func planCapsuleArtifactsForTargets(
	manifests []capsuleManifest,
	targets []string,
) (capsuleArtifactPlan, error) {
	if len(manifests) == 0 {
			...
			for _, target := range targets {
				expected = append(expected, generatedCapsuleArtifact{
					Artifact: capsuleArtifact{
						Kind:   "object",
						Target: target,
						Path:   objectArtifactRelPath(module.Module, target),
					},
					Module: module.Module,
					Source: module.Path,
				})
...
func checkDeclaredCapsuleArtifacts(
	capsulePath string,
	targetFlag string,
	lockPath string,
	allTargets bool,
		...
		case "interface":
			...
			if err != nil {
				issues = append(
					issues,
					artifactIssue{
						Kind:   "invalid interface artifact",
						Module: item.Module,
						Path:   item.Artifact.Path,
						Detail: err.Error(),
						Repair: repair,
			...
			if actual != expected {
				issues = append(issues, artifactIssue{
					...
					Path:   item.Artifact.Path,
					Detail: fmt.Sprintf(
						"expected public API %s, artifact has %s",
						expected,
						actual,
					),
					Repair: repair,
		...
		case "object":
			...
			if obj.Target != "" && obj.Target != item.Artifact.Target {
				issues = append(issues, artifactIssue{
					...
					Path:   item.Artifact.Path,
					Detail: fmt.Sprintf(
						"expected target %s, object has %s",
			...
			if obj.PublicAPIHash != expectedAPI {
				issues = append(issues, artifactIssue{
					...
					Path:   item.Artifact.Path,
					Detail: fmt.Sprintf(
						"expected public API %s, object has %s",
						expectedAPI,
						obj.PublicAPIHash,
					),
					Repair: repair,
		...
		case "seed":
			...
			if seed.Lock.GraphSHA256 != currentLock.GraphSHA256 {
				issues = append(issues, artifactIssue{
					...
					Path: item.Artifact.Path,
					Detail: fmt.Sprintf(
						"expected dependency graph %s, seed has %s",
						currentLock.GraphSHA256,
						seed.Lock.GraphSHA256,
					),
					Repair: repair,
				})
			}
...
func artifactRepairCommand(capsulePath string, targets []string, allTargets bool) string {
	if allTargets {
		return fmt.Sprintf(
			"tetra eco artifacts build --all-targets --lock %s %s",
			compiler.SemanticLockFileName,
			filepath.ToSlash(capsulePath),
		)
	}
	target := ""
	if len(targets) > 0 {
	...
	if target != "" {
		return fmt.Sprintf(
			"tetra eco artifacts build --target %s --lock %s %s",
			target,
			compiler.SemanticLockFileName,
			filepath.ToSlash(capsulePath),
		)
	...
	return fmt.Sprintf(
		"tetra eco artifacts build --lock %s %s",
		compiler.SemanticLockFileName,
		filepath.ToSlash(capsulePath),
	)
}
...
func dependencyArtifactBuildOptions(
	root string,
	manifest capsuleManifest,
	jobs int,
) (compiler.BuildOptions, error) {
	depRoots, _, err := projectDependencyGraph(
		root,
		manifest,
	...
	for _, sourceRoot := range roots {
		...
		err = filepath.WalkDir(dir, func(path string, d os.DirEntry, walkErr error) error {
			...
			if first, ok := seenModules[file.Module]; ok {
				return fmt.Errorf(
					"%s: duplicate dependency module %s (first %s)",
					path,
					file.Module,
					first,
				)
...
func interfaceArtifactRelPath(module string) string {
	return filepath.ToSlash(
		filepath.Join("interfaces", moduleArtifactRelPrefix(module)+compiler.T4InterfaceExtension),
	)
}

func objectArtifactRelPath(module string, target string) string {
	return filepath.ToSlash(
		filepath.Join("artifacts", moduleArtifactRelPrefix(module)+"."+target+".tobj"),
	)
}
...
func appendGeneratedArtifactsToCapsule(
	path string,
	existing []capsuleArtifact,
...
func lockArtifactsFromCapsuleArtifacts(artifacts []capsuleArtifact) []ecoLockArtifact {
	...
	for _, artifact := range artifacts {
		out = append(
			out,
			ecoLockArtifact{Kind: artifact.Kind, Target: artifact.Target, Path: artifact.Path},
		)
	}
		...
		for j := range lock.Capsules[i].Artifacts {
			...
			if err != nil {
				return fmt.Errorf(
					"%s: read artifact %s: %w",
					lock.Capsules[i].ID,
					lock.Capsules[i].Artifacts[j].Path,
					err,
				)
			}
			...
			case "interface":
				...
				if moduleName == "" {
					return fmt.Errorf(
						"%s: artifact %s missing module declaration",
						lock.Capsules[i].ID,
						lock.Capsules[i].Artifacts[j].Path,
					)
				}
			...
			case "object":
				...
				if lock.Capsules[i].Artifacts[j].Target != "" && obj.Target != "" &&
					lock.Capsules[i].Artifacts[j].Target != obj.Target {
					return fmt.Errorf(
						"%s: artifact %s target mismatch: manifest %s, object %s",
						lock.Capsules[i].ID,
						lock.Capsules[i].Artifacts[j].Path,
						lock.Capsules[i].Artifacts[j].Target,
						obj.Target,
					)
				}
				if obj.Target != "" {
	...
	for _, capsule := range lock.Capsules {
		...
		for _, dep := range capsule.Dependencies {
			...
			if !strings.HasPrefix(dep.ID, "tetra://") {
				return fmt.Errorf(
					"lock capsule %s dependency %s must use tetra:// prefix",
					capsule.ID,
					dep.ID,
				)
			}
			if !isCapsuleSemver(dep.Version) {
				return fmt.Errorf(
					"lock capsule %s dependency %s has invalid semver %s",
					capsule.ID,
					dep.ID,
					dep.Version,
				)
			...
			if dep.Path != "" && strings.Contains(dep.Path, "\\") {
				return fmt.Errorf(
					"lock capsule %s dependency %s has non-normalized path %s",
					capsule.ID,
					dep.ID,
					dep.Path,
				)
			...
			if _, exists := seenDeps[key]; exists {
				return fmt.Errorf(
					"lock capsule %s has duplicate dependency %s %s",
					capsule.ID,
					dep.ID,
					dep.Version,
				)
...
func normalizeLock(lock *ecoLock) {
	...
	sort.Slice(
		lock.Capsules,
		func(i, j int) bool { return lock.Capsules[i].ID < lock.Capsules[j].ID },
	)
	if lock.GraphSHA256 == "" {
		setEcoLockGraphHash(lock)
	}
	...
	for _, capsule := range lock.Capsules {
		...
		for _, dep := range capsule.Dependencies {
			if dep.Path != "" {
				lines = append(
					lines,
					fmt.Sprintf("    dependency %q %q %q", dep.ID, dep.Version, dep.Path),
				)
			} else {
	...
	for i, line := range strings.Split(string(raw), "\n") {
		...
		if strings.HasPrefix(content, "manifest ") {
			...
			value, err := parseCapsuleString(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "manifest ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
			if value != capsuleManifestSchemaV1 {
				return capsuleManifest{}, fmt.Errorf(
					"%s:%d: unsupported manifest schema %s",
					path,
					i+1,
					value,
				)
		...
		if strings.HasPrefix(content, "capsule ") {
			...
			if sawName {
				return capsuleManifest{}, fmt.Errorf(
					"%s:%d: duplicate capsule declaration",
					path,
					i+1,
				)
			}
		...
		if strings.HasPrefix(content, "id ") {
			...
			value, err := parseCapsuleString(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "id ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
			if !strings.HasPrefix(value, "tetra://") {
				return capsuleManifest{}, fmt.Errorf(
					"%s:%d: capsule id must use tetra:// prefix",
					path,
					i+1,
				)
			}
		...
		if strings.HasPrefix(content, "version ") {
			...
			value, err := parseCapsuleString(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "version ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
			if !isCapsuleSemver(value) {
				return capsuleManifest{}, fmt.Errorf(
					"%s:%d: capsule version must use semver x.y.z",
					path,
					i+1,
				)
			}
		...
		if strings.HasPrefix(content, "entry ") {
			...
			value, err := parseCapsuleBareOrQuoted(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "entry ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
		...
		if strings.HasPrefix(content, "source ") {
			...
			value, err := parseCapsuleBareOrQuoted(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "source ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
		...
		if strings.HasPrefix(content, "target ") {
			...
			value, err := parseCapsuleBareOrQuoted(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "target ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
		...
		if strings.HasPrefix(content, "effect ") {
			...
			value, err := parseCapsuleString(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "effect ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
		...
		if strings.HasPrefix(content, "permission ") {
			...
			value, err := parseCapsuleString(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "permission ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
		...
		if strings.HasPrefix(content, "dependency ") {
			...
			dep, err := parseCapsuleDependency(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "dependency ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
		...
		if strings.HasPrefix(content, "artifact ") {
			section = ""
			artifact, err := parseCapsuleArtifact(
				path,
				i+1,
				strings.TrimSpace(strings.TrimPrefix(content, "artifact ")),
			)
			if err != nil {
				return capsuleManifest{}, err
			}
...
func parseCapsuleDependencyFields(
	path string,
	line int,
	fields []string,
) (capsuleDependency, error) {
	if len(fields) != 2 && len(fields) != 3 {
		return capsuleDependency{}, fmt.Errorf(
			"%s:%d: dependency expects id, version, and optional path",
			path,
			line,
		)
	}
	...
	if !isCapsuleSemver(fields[1]) {
		return capsuleDependency{}, fmt.Errorf(
			"%s:%d: dependency version must use semver x.y.z",
			path,
			line,
		)
	}
...
func parseCapsuleArtifact(path string, line int, value string) (capsuleArtifact, error) {
	...
	if len(fields) != 2 && len(fields) != 3 {
		return capsuleArtifact{}, fmt.Errorf(
			"%s:%d: artifact expects kind, optional target, and path",
			path,
			line,
		)
	}
	...
	if len(fields) == 3 {
		if kind != "object" {
			return capsuleArtifact{}, fmt.Errorf(
				"%s:%d: only object artifacts accept a target",
				path,
				line,
			)
		}
...
func appendCapsuleArtifact(
	...
	for _, existing := range manifest.Artifacts {
		if existing.Kind == artifact.Kind && existing.Target == artifact.Target &&
			existing.Path == artifact.Path {
			return fmt.Errorf(
				"%s:%d: duplicate artifact %s %s",
				path,
				line,
				artifact.Kind,
				artifact.Path,
...
func validateCapsuleGraph(manifests []capsuleManifest, target string) error {
	...
	for _, manifest := range manifests {
		...
		if manifest.ManifestSchema != capsuleManifestSchemaV1 {
			return fmt.Errorf(
				"%s: unsupported manifest schema %s",
				manifest.Path,
		...
		if target != "" && len(manifest.Targets) > 0 && !containsString(manifest.Targets, target) {
			return fmt.Errorf(
				"%s: target mismatch for %s: does not support %s",
				manifest.Path,
				manifest.ID,
				target,
			)
	...
	for _, manifest := range manifests {
		for _, dep := range manifest.Dependencies {
			...
			if found.Version != dep.Version {
				return fmt.Errorf(
					"%s: dependency %s version mismatch: want %s, got %s",
					manifest.Path,
					dep.ID,
					dep.Version,
					found.Version,
			...
			for _, effect := range found.Effects {
				if !containsString(manifest.Effects, effect) {
					return fmt.Errorf(
						"%s: missing required effect %s for dependency %s",
						manifest.Path,
						effect,
						dep.ID,
					)
			...
			for _, permission := range found.Permissions {
				if !containsString(manifest.Permissions, permission) {
					return fmt.Errorf(
						"%s: missing required permission %s for dependency %s",
						manifest.Path,
						permission,
						dep.ID,
					)
...
func runEcoMaterialize(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if _, err := writeEcoStructuredFile(
		filepath.Join(outDir, "tetra.materialization.json"),
		metadataFormat,
		meta,
	); err != nil {
		fmt.Fprintln(stderr, err)
...
func parseEcoMaterializeArgs(
	args []string,
) (pkgPath string, target string, trustPath string, outDir string, metadataFormat string, err error) {
	outDir = "."
...
func runEcoTrustSnapshot(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	format := fs.String(
		"format",
		outputformat.JSON,
		"trust snapshot output format: json, toon, or both",
	)
	if err := fs.Parse(args); err != nil {
		if err == flag.ErrHelp {
			return 0
	...
	sort.Slice(
		snapshot.Capsules,
		func(i, j int) bool { return snapshot.Capsules[i].ID < snapshot.Capsules[j].ID },
	)
...
func buildNeedMap(lock ecoLock, rawLock []byte) ecoNeedMap {
	...
	sort.Slice(
		needMap.Capsules,
		func(i, j int) bool { return needMap.Capsules[i].ID < needMap.Capsules[j].ID },
	)
...
func prepareEcoFileInsideRootNoSymlink(root string, slashRel string, label string) (string, error) {
	...
	if relCheck == "." || relCheck == ".." ||
		strings.HasPrefix(relCheck, ".."+string(os.PathSeparator)) {
		return "", fmt.Errorf("unsafe %s path %s", label, slashRel)
	}
...
func writeEcoFileInsideRootNoSymlink(
	root string,
	slashRel string,
	raw []byte,
	perm os.FileMode,
...
func writeEcoJSONFileInsideRootNoSymlink(
	root string,
	slashRel string,
	v interface{},
	label string,
...
func runEcoTetraHubPublish(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	metaPath, err := publishPackage(
		resolvedPkgPath,
		*store,
		*target,
		*trustPath,
		ecoTetraHubLabel(*channel),
		*channel,
	)
	if err != nil {
		fmt.Fprintln(stderr, err)
...
func publishPackage(
	...
	pkgOutPath, err := writeEcoFileInsideRootNoSymlink(
		registry,
		ecoPublishTargetFileRel(targetRel, ecoPublishedPackageFileName),
		pkgRaw,
		0o644,
		"publish package",
	)
	...
	metaPath, err := writeEcoJSONFileInsideRootNoSymlink(
		registry,
		ecoPublishTargetFileRel(targetRel, ecoPublishMetadataFileName),
		meta,
		"publish metadata",
	)
	if err != nil {
...
func buildEcoPublishMetadata(
	...
	return ecoPublishMetadata{
		...
		Downloads: []ecoPublishDownload{
			{
				Target: target,
				Path: ecoPublishDownloadPath(
					manifest.ID,
					manifest.Version,
					target,
					ecoPublishedPackageFileName,
				),
			},
		},
	}
...
func attachEcoPublishTrust(
	...
	if _, err := writeEcoFileInsideRootNoSymlink(
		targetDir,
		trustFile,
		raw,
		0o644,
		"publish trust snapshot",
	); err != nil {
		return err
	}
	tier := "unknown"
...
func downloadPackage(
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			return "", fmt.Errorf(
				"target %s not available for %s %s (available: %s)",
				target,
				id,
				version,
...
func mirrorPublishedPackage(
	fromStore string,
	toStore string,
	id string,
	version string,
	target string,
) (ecoMirrorReport, error) {
	...
	if err := validateEcoPublishMetadataForDownload(
		meta,
		id,
		version,
		target,
...
func fetchPublishedPackage(
	...
	if meta.Trust != nil {
		...
		if hex.EncodeToString(trustSum[:]) != trustHash {
			return ecoMirrorReport{}, fmt.Errorf(
				"trust snapshot hash mismatch for %s/%s",
				sourceURL,
				trustPathRel,
			)
		}
...
func writeMirroredPublishedPackage(opt ecoMirrorWriteOptions) (ecoMirrorReport, error) {
	destPackagePath, err := writeEcoFileInsideRootNoSymlink(
		opt.DestinationStore,
		opt.PackageRel,
		opt.RawPackage,
		0o644,
		"mirror package",
	...
	if opt.Meta.Trust != nil {
		if _, err := writeEcoFileInsideRootNoSymlink(
			opt.DestinationStore,
			ecoPublishTargetFileRel(opt.TargetRel, opt.TrustRel),
			opt.RawTrust,
			0o644,
			"mirror trust snapshot",
		); err != nil {
			return ecoMirrorReport{}, err
		}
	}
...
func validateEcoPublishMetadataEnvelope(
	meta ecoPublishMetadata,
	id string,
	version string,
	target string,
	...
	if meta.Trust != nil {
		if err := validateEcoPublishMetadataPath(
...
func fetchEcoHTTPStoreFile(baseURL string, relPath string) ([]byte, error) {
	...
	if len(raw) > maxEcoHTTPFetchBytes {
		return nil, fmt.Errorf(
			"GET %s: response exceeds %d bytes",
			requestURL,
			maxEcoHTTPFetchBytes,
		)
	}
	return raw, nil
}
...
func validateEcoPublishPackageBytes(meta ecoPublishMetadata, path string, raw []byte) error {
	if int64(len(raw)) != meta.Package.Size {
		return fmt.Errorf(
			"package size mismatch for %s: metadata=%d actual=%d",
			path,
			meta.Package.Size,
			len(raw),
		)
...
func validateEcoPublishMetadataForDownload(
	meta ecoPublishMetadata,
	id string,
	version string,
	target string,
	...
	if !isSupportedEcoPublishSchemaChannel(meta.Schema, meta.Channel) {
		return fmt.Errorf(
			"unsupported publish metadata in %s",
			filepath.Join(targetDir, ecoPublishMetadataFileName),
		)
	}
	if err := validateEcoPublishMetadataIdentity(meta, id, version, target); err != nil {
		return err
...
func validateEcoPublishMetadataIdentity(
	meta ecoPublishMetadata,
	id string,
	version string,
	target string,
...
func validateEcoPublishDownloads(
	meta ecoPublishMetadata,
	id string,
	version string,
	target string,
...
func runEcoSeedImport(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	seedFormat := fs.String(
		"seed-format",
		outputformat.Auto,
		"seed input format: auto, json, or toon",
	)
...
func upsertVaultRecord(records []vaultRecord, record vaultRecord) []vaultRecord {
	for i, existing := range records {
		if existing.Hash == record.Hash && existing.Kind == record.Kind &&
