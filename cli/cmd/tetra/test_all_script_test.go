...
if [[ "${1:-}" == "run" && "${2:-}" == "./tools/cmd/ram-contract-fuzz-short" ]]; then
  ...
  if [[ -n "$report_dir" ]]; then
    mkdir -p -- "$report_dir"
    printf '{"schema_version":"tetra.ram-contract-report.v1","target":"linux-x64","git_head":"0123456789abcdef0123456789abcdef01234567","rows":[{"id":"stack-budget","storage_class":"stack","heap_bytes":0,"copy_bytes":0,"bounded":true,"proof_id":"proof-stack","blocker_id":""}],"blockers":[],"summary":{"heap_bytes":0,"copy_bytes":0,"unbounded_rows":0}}\n' >"$report_dir/ram-contract-report.json"
    printf '{"schema_version":"tetra.memory-grade-report.v1","target":"linux-x64","git_head":"0123456789abcdef0123456789abcdef01234567","grade":"A","rows":[{"id":"stack-budget","class":"zero_heap","heap_bytes":0,"copy_bytes":0}]}\n' >"$report_dir/memory-grade-report.json"
    printf '{"schema_version":"tetra.proof-store-summary.v1","git_head":"0123456789abcdef0123456789abcdef01234567","proofs":[{"id":"proof-stack","kind":"bounds","hash":"sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"}],"invalid_references":[]}\n' >"$report_dir/proof-store-summary.json"
    printf '{"schema_version":"tetra.validation-pipeline-coverage.v1","git_head":"0123456789abcdef0123456789abcdef01234567","stages":[{"name":"allocplan","covered":true},{"name":"ramcontract","covered":true},{"name":"validators","covered":true}]}\n' >"$report_dir/validation-pipeline-coverage.json"
    printf '{"schema_version":"tetra.ram-blockers.v1","kind":"heap","git_head":"0123456789abcdef0123456789abcdef01234567","blockers":[]}\n' >"$report_dir/heap-blockers.json"
    printf '{"schema_version":"tetra.ram-blockers.v1","kind":"copy","git_head":"0123456789abcdef0123456789abcdef01234567","blockers":[]}\n' >"$report_dir/copy-blockers.json"
    printf '{"schema_version":"tetra.ram-contract-fuzz-oracle.v1","git_head":"0123456789abcdef0123456789abcdef01234567","artifact_dir":"%s","mutations":[{"name":"missing-blocker","status":"rejected"},{"name":"stale-proof","status":"rejected"}],"validated_reports":["ram-contract-report.json","memory-grade-report.json","proof-store-summary.json","validation-pipeline-coverage.json","heap-blockers.json","copy-blockers.json"]}\n' "$report_dir" >"$report_dir/ram-contract-fuzz-oracle.json"
  fi
  exit 0
fi
if [[ "${1:-}" == "test" ]]; then
